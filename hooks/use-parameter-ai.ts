/**
 * useParameterAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the parameter-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.parameter.generate.*`, update
...
export interface UseParameterAiConfig {
...
export interface UseParameterAiReturn {
  ...
  /** Emit a parameter generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useParameterAi(
  config: UseParameterAiConfig = {},
): UseParameterAiReturn {
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `parameter.generate.completed` nor `parameter.generate.failed` fires for a
      ...
      const response = transport.send("/parameter/generate", {
        instructions: [],
        config: {
