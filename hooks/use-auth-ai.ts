/**
 * useAuthAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the auth-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.auth.generate.*`, update
...
export interface UseAuthAiConfig {
...
export interface UseAuthAiReturn {
  ...
  /** Emit a auth generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useAuthAi(
  config: UseAuthAiConfig = {},
): UseAuthAiReturn {
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `auth.generate.completed` nor `auth.generate.failed` fires for a
      ...
      const response = transport.send("/auth/generate", {
        instructions: [],
        config: {
