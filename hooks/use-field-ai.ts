/**
 * useFieldAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the field-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.field.generate.*`, update
...
export interface UseFieldAiConfig {
...
export interface UseFieldAiReturn {
  ...
  /** Emit a field generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useFieldAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `field.generate.completed` nor `field.generate.failed` fires for a
      ...
      const response = transport.send("/field/generate", {
        instructions: [],
        config: {
