/**
 * useModelAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the model-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.model.generate.*`, update
...
export interface UseModelAiConfig {
...
export interface UseModelAiReturn {
  ...
  /** Emit a model generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useModelAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `model.generate.completed` nor `model.generate.failed` fires for a
      ...
      const response = transport.send("/model/generate", {
        instructions: [],
        config: {
