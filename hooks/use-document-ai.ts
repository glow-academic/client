/**
 * useDocumentAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the document-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.document.generate.*`, update
...
export interface UseDocumentAiConfig {
...
export interface UseDocumentAiReturn {
  ...
  /** Emit a document generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useDocumentAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `document.generate.completed` nor `document.generate.failed` fires for a
      ...
      const response = transport.send("/document/generate", {
        instructions: [],
        config: {
