/**
 * useProfileAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the profile-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.profile.generate.*`, update
...
export interface UseProfileAiReturn {
  ...
  /** Emit a profile generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useProfileAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `profile.generate.completed` nor `profile.generate.failed` fires for a
      ...
      const response = transport.send("/profile/generate", {
        instructions: [],
        config: {
