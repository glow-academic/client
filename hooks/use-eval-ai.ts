/**
 * useEvalAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the eval-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.eval.generate.*`, update
...
export interface UseEvalAiConfig {
...
export interface UseEvalAiReturn {
  ...
  /** Emit a eval generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useEvalAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `eval.generate.completed` nor `eval.generate.failed` fires for a
      ...
      const response = transport.send("/eval/generate", {
        instructions: [],
        config: {
