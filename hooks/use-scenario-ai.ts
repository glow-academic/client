/**
 * useScenarioAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the scenario-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.scenario.generate.*`, update
...
export interface UseScenarioAiReturn {
  ...
  /** Emit a scenario generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useScenarioAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `scenario.generate.completed` nor `scenario.generate.failed` fires for a
      ...
      const response = transport.send("/scenario/generate", {
        instructions: [],
        config: {
