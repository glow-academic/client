/**
 * useRubricAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the rubric-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.rubric.generate.*`, update
...
export interface UseRubricAiReturn {
  ...
  /** Emit a rubric generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useRubricAi(
  config: UseRubricAiConfig = {},
): UseRubricAiReturn {
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `rubric.generate.completed` nor `rubric.generate.failed` fires for a
      ...
      const response = transport.send("/rubric/generate", {
        instructions: [],
        config: {
