/**
 * useAgentAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the agent-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.agent.generate.*`, update
...
export interface UseAgentAiConfig {
...
export interface UseAgentAiReturn {
  ...
  /** Emit a agent generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useAgentAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `agent.generate.completed` nor `agent.generate.failed` fires for a
      ...
      const response = transport.send("/agent/generate", {
        instructions: [],
        config: {
