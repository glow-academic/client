/**
 * useSimulationAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the simulation-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.simulation.generate.*`, update
...
export interface UseSimulationAiReturn {
  ...
  /** Emit a simulation generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useSimulationAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `simulation.generate.completed` nor `simulation.generate.failed` fires for a
      ...
      const response = transport.send("/simulation/generate", {
        instructions: [],
        config: {
