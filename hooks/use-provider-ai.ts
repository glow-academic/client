/**
 * useProviderAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the provider-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.provider.generate.*`, update
...
export interface UseProviderAiConfig {
...
export interface UseProviderAiReturn {
  ...
  /** Emit a provider generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useProviderAi(
  config: UseProviderAiConfig = {},
): UseProviderAiReturn {
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `provider.generate.completed` nor `provider.generate.failed` fires for a
      ...
      const response = transport.send("/provider/generate", {
        instructions: [],
        config: {
