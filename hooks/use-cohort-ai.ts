/**
 * useCohortAi — per-artifact composition of the three generation primitives.
 *
 ...
 *
 * Replaces the cohort-level call into the deprecated `useArtifactAi`. When
 * server event names canonicalize to `artifacts.cohort.generate.*`, update
...
export interface UseCohortAiReturn {
  ...
  /** Emit a cohort generation. `resourceTypes` is the list of resources to generate. */
  generate: (
...
export function useCohortAi(
  ...
  const generate = useCallback(
    (resourceTypes: string[], options: Record<string, unknown> = {}): boolean => {
      ...
      // `cohort.generate.completed` nor `cohort.generate.failed` fires for a
      ...
      const response = transport.send("/cohort/generate", {
        instructions: [],
        config: {
