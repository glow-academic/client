...
- **Integration tests**: For all endpoints (`server/tests/integration/api/v5/` and `server/tests/integration/socket/v5/`)
...
- `database/scripts/load-modules.sh` - Assemble seed SQL from YAML config + module files
- `database/seeds/` - Timestamped seed files (generated by fresh-db)

### Server
- `server/app/routes/v5/api/main/[resource]/` - Artifact endpoints (BFF aggregation + permissions)
- `server/app/routes/v5/api/resources/[resource]/` - Resource endpoints (cached data-access, `*_internal()`)
- `server/app/routes/v5/api/views/[domain]/` - View endpoints (MV reads, declarative SQL filters)
- `server/app/routes/v5/socket/artifacts/[resource]/` - WebSocket handlers (generate, complete, progress, error)
...
|-------|----------|---------|
| **Views** | `server/app/routes/v5/api/views/` | Read layer — queries MVs with declarative SQL filters, exposes `*_internal()` |
| **Resources** | `server/app/routes/v5/api/resources/` | Cached data-access functions, exposes `*_internal()` for reuse |
| **Artifacts** | `server/app/routes/v5/api/main/` | BFF aggregation — combines views + resources + permissions |
| **Socket** | `server/app/routes/v5/socket/artifacts/` | WebSocket handlers for AI generation |
...
#### API Endpoints (HTTP)
- **1 SQL file per route**: `server/app/sql/queries/[resource]/[operation]_complete.sql`
...
**See `server/app/routes/v5/STANDARDS.md` for complete API standards.**

#### WebSocket Endpoints
- **1 event per file**: Each Python file defines exactly one event handler (`@sio.event` or `@internal_sio.on`)
- **1 SQL file per event**: Each event has exactly one SQL file in `server/app/sql/[resource]/[operation]_complete.sql`
...
**See `server/app/routes/v5/socket/STANDARDS.md` for complete WebSocket standards.**
...
- **Versioned types**: Include version (e.g. `v5`) in type names for future compatibility

### Intended Outcomes (v5)
...
- **Maintainable upgrades**: Versioned endpoints/types (`v5`) allow safe iteration without breaking older contracts
...
### File Locations
- **SQL**: `server/app/sql/queries/[resource]/[operation]_complete.sql`
- **Artifacts**: `server/app/routes/v5/api/main/[resource]/[operation].py`
- **Resources**: `server/app/routes/v5/api/resources/[resource]/[operation].py`
- **Views**: `server/app/routes/v5/api/views/[domain]/[resource]/[operation].py`
- **Socket**: `server/app/routes/v5/socket/artifacts/[resource]/[event].py`
- **Tests**: `server/tests/integration/api/v5/[resource]/test_[operation].py`
- **Server actions**: `client/app/(main)/[resource]/page.tsx`
- **Components**: `client/components/[resource]/[Component].tsx`

### SQL Naming Convention
- Location: `server/app/sql/queries/[resource]/`
...
- **Client**: `InputOf<"/api/v5/[resource]/[operation]", "post">` and `OutputOf<...>`
...
- Create handler file: `server/app/routes/v5/socket/[resource]/[operation].py`
- Register with `@internal_sio.on("[event_name]")`
- Export `_impl` function for direct calls when return values needed
- Use single SQL file per event: `app/v5/sql/[resource]/[operation]_complete.sql`
...
**WebSocket events:**
- `server/app/routes/v5/socket/[resource]/[operation].py` - Event handlers
- `server/app/sql/[resource]/[operation]_complete.sql` - SQL files (one per event)
