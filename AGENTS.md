...
6. Export updated schema: `make export-db schema`
...
### Server
- `server/app/api/v4/artifacts/[resource]/` - Artifact endpoints (BFF aggregation + permissions)
- `server/app/api/v4/resources/[resource]/` - Resource endpoints (cached data-access, `*_internal()`)
- `server/app/api/v4/views/[domain]/` - View endpoints (MV reads, declarative SQL filters)
- `server/app/socket/v4/artifacts/[resource]/` - WebSocket handlers (generate, complete, progress, error)
- `server/app/sql/v4/queries/[resource]/` - SQL files (one per route)
- `server/app/sql/types.py` - Auto-generated types (via `make sql-compile`)
- `server/app/utils/` - Utility functions (sql_helper, sql_nest, etc.)
...
### Server: Three-Layer BFF Architecture with PostgreSQL Functions

**⚠️ CRITICAL: All endpoints follow the three-layer BFF architecture pattern**

#### Three-Layer Pattern

| Layer | Location | Purpose |
|-------|----------|---------|
| **Views** | `server/app/api/v4/views/` | Read layer — queries MVs with declarative SQL filters, exposes `*_internal()` |
| **Resources** | `server/app/api/v4/resources/` | Cached data-access functions, exposes `*_internal()` for reuse |
| **Artifacts** | `server/app/api/v4/artifacts/` | BFF aggregation — combines views + resources + permissions |
| **Socket** | `server/app/socket/v4/artifacts/` | WebSocket handlers for AI generation |
| **Permissions** | `*/permissions.py` per artifact | Pure Python business logic — no SQL |

#### SQL Compilation & Type Flow
```
SQL files (server/app/sql/v4/queries/)
    ↓ make sql-compile (executes functions, introspects, generates types)
...
**After editing SQL:** Run `make sql-compile` to regenerate types.

#### API Endpoints (HTTP)
- **1 SQL file per route**: `server/app/sql/v4/queries/[resource]/[operation]_complete.sql`
- **PostgreSQL functions**: SQL files define functions with `RETURNS TABLE`, not raw queries
- **Composite types**: Nested structures use composite types in `types` schema (never JSONB)
- **Auto-generated types**: `make sql-compile` generates Pydantic models from SQL introspection
- **Two-pass fetching**: Artifact endpoints do Pass 1 (SQL for IDs/metadata), Pass 2 (parallel `*_internal()` calls)
- **No inline SQL**: All SQL in `.sql` files, executed via `execute_sql_typed()`
...
### File Locations
- **SQL**: `server/app/sql/v4/queries/[resource]/[operation]_complete.sql`
- **Artifacts**: `server/app/api/v4/artifacts/[resource]/[operation].py`
- **Resources**: `server/app/api/v4/resources/[resource]/[operation].py`
- **Views**: `server/app/api/v4/views/[domain]/[resource]/[operation].py`
- **Socket**: `server/app/socket/v4/artifacts/[resource]/[event].py`
...
- Location: `server/app/sql/v4/queries/[resource]/`
- Pattern: `[operation]_[resource]_complete.sql`
- Examples: `get_persona_access_complete.sql`, `save_persona_complete.sql`
