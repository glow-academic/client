...
### Server
- `server/app/v5/api/main/[resource]/` - Artifact endpoints (BFF aggregation + permissions)
- `server/app/v5/api/resources/[resource]/` - Resource endpoints (cached data-access, `*_internal()`)
- `server/app/v5/api/views/[domain]/` - View endpoints (MV reads, declarative SQL filters)
- `server/app/v5/socket/artifacts/[resource]/` - WebSocket handlers (generate, complete, progress, error)
...
|-------|----------|---------|
| **Views** | `server/app/v5/api/views/` | Read layer — queries MVs with declarative SQL filters, exposes `*_internal()` |
| **Resources** | `server/app/v5/api/resources/` | Cached data-access functions, exposes `*_internal()` for reuse |
| **Artifacts** | `server/app/v5/api/main/` | BFF aggregation — combines views + resources + permissions |
| **Socket** | `server/app/v5/socket/artifacts/` | WebSocket handlers for AI generation |
...
#### API Endpoints (HTTP)
- **1 SQL file per route**: `server/app/v5/sql/queries/[resource]/[operation]_complete.sql`
...
**See `server/app/v5/STANDARDS.md` for complete API standards.**

#### WebSocket Endpoints
- **1 event per file**: Each Python file defines exactly one event handler (`@sio.event` or `@internal_sio.on`)
- **1 SQL file per event**: Each event has exactly one SQL file in `server/app/v5/sql/[resource]/[operation]_complete.sql`
...
**See `server/app/v5/socket/STANDARDS.md` for complete WebSocket standards.**
...
### File Locations
- **SQL**: `server/app/v5/sql/queries/[resource]/[operation]_complete.sql`
- **Artifacts**: `server/app/v5/api/main/[resource]/[operation].py`
- **Resources**: `server/app/v5/api/resources/[resource]/[operation].py`
- **Views**: `server/app/v5/api/views/[domain]/[resource]/[operation].py`
- **Socket**: `server/app/v5/socket/artifacts/[resource]/[event].py`
...
- Location: `server/app/v5/sql/queries/[resource]/`
...
- Create handler file: `server/app/v5/socket/[resource]/[operation].py`
...
**WebSocket events:**
- `server/app/v5/socket/[resource]/[operation].py` - Event handlers
- `server/app/v5/sql/[resource]/[operation]_complete.sql` - SQL files (one per event)
