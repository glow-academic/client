 ...
 * cross-request cache. */
const getSimulationContextById = cache(
  async (id: string): Promise<ContextOut> =>
    api.post("/simulation/context", { body: { entity_id: id } } as ContextIn) as Promise<ContextOut>,
);
...
export async function generateMetadata({
  ...
  try {
    ...
    const context = await getSimulationContextById(simulationId);
    return {
...
export default async function EditSimulationPage({
  ...
  try {
    ...
    const [simulationData, context, draftsResult, groupResult] = await Promise.all([
      ...
      api.post("/simulation/drafts", { body: { page_limit: 50, page_offset: 0 } }),
      api.post(
        "/simulation/group",
