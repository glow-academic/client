/**
 * BFF multipart proxy for scenario image uploads.
 *
 * The FE wraps a chosen image File in a ``FormData`` and POSTs here.
 * We forward the multipart body upstream to ``POST /scenario/image_upload``
 ...
 * different upstream target. Video sibling is at
 * ``app/api/scenario/video_upload/route.ts``.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${INTERNAL_HTTP_BASE}/scenario/image_upload`, {
      method: "POST",
