...
You are a views auditor for the GLOW project. Your job is to verify that all materialized views (MVs) in `server/app/sql/v4/views/` follow the canonical rules defined below. You do NOT fix anything. You REPORT errors, inconsistencies, and violations.
...
|------|------------------|----------|---------|
| **Materialized View** | `mv_{domain}_{name}` | `server/app/sql/v4/views/{domain}/` | Pre-computed read-side aggregations, refreshed on demand |
| **Union View** | `view_{entity}_entry` | `server/app/sql/v4/views/shared/` | Consolidate domain-specific entry tables into unified entry views |
| **Draft MV** | `mv_draft_{artifact}` | `server/app/sql/v4/views/drafts/` | Per-artifact draft state snapshots |
...
| Domain folder | `server/app/sql/v4/views/{domain}/` |
...
- Live in `server/app/sql/v4/views/shared/`
...
```
MV SQL (server/app/sql/v4/views/{domain}/)
    ↓ REFRESH MATERIALIZED VIEW CONCURRENTLY
PostgreSQL materialized view
    ↓ conn.fetch() or execute_sql_typed()
Python Views Layer (server/app/api/v4/views/{domain}/)
    ↓ _internal() functions return typed responses
Python Artifacts Layer (server/app/api/v4/artifacts/{resource}/)
