...
|-------|----------|---------|
| **Access Check SQL** | `server/app/sql/v4/queries/{artifact}s/check_{artifact}_duplicate_access_complete.sql` | Light access check |
| **Duplicate SQL** | `server/app/sql/v4/queries/{artifact}s/duplicate_{artifact}_complete.sql` | Creates new artifact, links all junctions from source |
| **Python Handler** | `server/app/api/v4/artifacts/{artifact}/duplicate.py` | Permission check, name creation, transaction, audit |
| **Permissions** | `server/app/api/v4/artifacts/{artifact}/permissions.py` | `compute_can_duplicate()` — simple role check |

Reference: `server/app/api/v4/artifacts/persona/duplicate.py`, `duplicate_persona_complete.sql`
...
Reference: `server/app/api/v4/artifacts/persona/duplicate.py`
...
for artifact_dir in server/app/api/v4/artifacts/*/; do
  ...
  found=$(find server/app/sql/v4/queries/ -name "check_${artifact}_duplicate_access*" 2>/dev/null | head -1)
...
for artifact_dir in server/app/api/v4/artifacts/*/; do
  artifact=$(basename "$artifact_dir")
  [ ! -f "${artifact_dir}duplicate.py" ] && continue
  found=$(find server/app/sql/v4/queries/ -name "duplicate_${artifact}_complete*" 2>/dev/null | head -1)
...
```bash
for sql_file in server/app/sql/v4/queries/*/duplicate_*_complete.sql; do
...
2. **The persona duplicate is the gold standard.** Reference: `server/app/api/v4/artifacts/persona/duplicate.py`.
