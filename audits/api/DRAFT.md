...
|-------|----------|---------|
| **Request Types** | `server/app/routes/v5/api/main/{artifact}/types.py` | Optional nested resource actions with `to_tuple()` + `from_request()` |
| **Access Check SQL** | `server/app/sql/queries/{artifact}s/check_{artifact}_duplicate_access_complete.sql` | Lighter access check (shared with duplicate) |
| **Draft SQL** | `server/app/sql/queries/{artifact}s/patch_{artifact}_draft_complete.sql` | Delete-then-insert connection pattern with version control |
| **Python Handler** | `server/app/routes/v5/api/main/{artifact}/draft.py` | Permission check, transaction, cache invalidation |
| **Permissions** | `server/app/routes/v5/api/main/{artifact}/permissions.py` | `compute_can_draft()` — simple role check |

Reference: `server/app/routes/v5/api/main/persona/draft.py`, `persona/types.py`
...
Reference: `server/app/routes/v5/api/main/persona/draft.py`
...
Reference: `server/app/routes/v5/api/main/persona/permissions.py`, `draft.py`
...
```bash
for sql_file in server/app/sql/queries/*/patch_*_draft_complete.sql; do
...
2. **The persona draft is the gold standard.** Reference: `server/app/routes/v5/api/main/persona/draft.py`.
