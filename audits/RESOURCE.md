...
| **API Endpoints** | `server/app/routes/v5/api/resources/{resource}/` | Cached data-access: get, search, create, docs |
| **UI Components** | `client/components/resources/{Resource}.tsx` | Standardized form components with AI generation, draft, autosave |
| **SQL Queries** | `server/app/sql/queries/resources/{resource}/` | One SQL file per endpoint operation |
...
Every value in the `resource_type` enum must have a corresponding folder at `server/app/routes/v5/api/resources/{resource}/`. Conversely, no API folder should exist for a name that is NOT in the `resource_type` enum.
...
Resource SQL queries (`server/app/sql/queries/resources/{resource}/`) must NOT filter by `domain_id`.
...
Resources that participate in AI generation must have a socket handler module at `server/app/routes/v5/socket/resources/{resource}/` with 6 files:

```
server/app/routes/v5/socket/resources/{resource}/
...
Every per-resource socket handler module must be imported and registered in the dispatcher (`server/app/routes/v5/socket/resources/dispatcher.py`) handler dicts:
...
ls -d server/app/routes/v5/api/resources/*/ | xargs -I{} basename {} | grep -v __pycache__ | sort
```
...
for dir in server/app/routes/v5/api/resources/*/; do
  name=$(basename "$dir")
...
for resource in $(psql postgresql://myuser:mypassword@localhost:5432/mydb -t -A -c \
  ...
  [ ! -f "server/app/routes/v5/api/resources/$resource/create.py" ] && echo "MISSING create.py: $resource"
done
```
...
grep -rl "domain_id" server/app/sql/queries/resources/ --include="*.sql" | grep -v "/domains/"

# Check Python endpoint files for domain_id
grep -rl "domain_id" server/app/routes/v5/api/resources/ --include="*.py" | grep -v "/domains/"
```
...
echo "=== GET/SEARCH SQL ==="
for resource_dir in server/app/sql/queries/resources/*/; do
...
echo "=== CREATE SQL ==="
for sql_file in server/app/sql/queries/resources/*_complete.sql; do
  [ -f "$sql_file" ] || continue
...
for f in server/app/routes/v5/api/resources/*/get.py; do
  grep -qL "get_cached" "$f" 2>/dev/null && echo "MISSING CACHE: $f"
done

# search.py without get_cached
for f in server/app/routes/v5/api/resources/*/search.py; do
  grep -qL "get_cached" "$f" 2>/dev/null && echo "MISSING CACHE: $f"
done

# create.py without invalidate_tags
for f in server/app/routes/v5/api/resources/*/create.py; do
...
for resource_dir in server/app/routes/v5/socket/resources/*/; do
  resource=$(basename "$resource_dir")
...
dispatcher="server/app/routes/v5/socket/resources/dispatcher.py"
for resource_dir in server/app/routes/v5/socket/resources/*/; do
  resource=$(basename "$resource_dir")
  [ "$resource" = "__pycache__" ] && continue
