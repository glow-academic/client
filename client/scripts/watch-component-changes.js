...
const { scanComponentFiles, generateTestFiles, generateCoverageReport } = require('./generate-component-tests');
...
function checkForChanges() {
  ...
  Object.keys(currentModTimes).forEach(componentPath => {
    ...
    } else if (currentModTimes[componentPath] !== cachedModTimes[componentPath]) {
  ...
  removedComponents.forEach(componentPath => {
    ...
    if (fs.existsSync(testFilePath)) {
      try {
        ...
        console.error(`❌ Error removing test file ${testFilePath}:`, error.message);
      }
    }
  });
}
