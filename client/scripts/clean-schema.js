...
function cleanSchema() {
  ...
  try {
    ...
    for (const importName of importsToCheck) {
      ...
      const usageRegex = new RegExp(`\\b${importName}\\b(?!\\s*[}\\s]*from)`, 'g');
      ...
      if (!isUsed) {
        ...
        const standaloneImportRegex = new RegExp(`import\\s*{\\s*${importName}\\s*}\\s*from\\s*["'][^"']*["']\\s*;?\\s*\\n?`, 'g');
        schemaContent = schemaContent.replace(standaloneImportRegex, '');
        
        // Remove from multi-import lines
        const multiImportRegex = new RegExp(`(import\\s*{[^}]*),\\s*${importName}\\s*([^}]*}\\s*from\\s*["'][^"']*["'])`, 'g');
        schemaContent = schemaContent.replace(multiImportRegex, '$1$2');
        
        // Remove if it's the first item in multi-import
        const firstItemRegex = new RegExp(`(import\\s*{)\\s*${importName}\\s*,\\s*([^}]*}\\s*from\\s*["'][^"']*["'])`, 'g');
