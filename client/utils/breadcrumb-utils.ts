 ...
 * 06/18/2025
 */
import { resolveBreadcrumb } from "@/utils/api/breadcrumbs/resolve-breadcrumb";
...
const fetchNameForId = async (id: string, context: string): Promise<string> => {
  try {
    log.info("breadcrumb.fetch_name.start", {
      message: `Fetching name for ${context} ID ${id}`,
      context: { function: "fetchNameForId", contextType: context, id },
    });

    const name = await resolveBreadcrumb(id, context);

    log.info("breadcrumb.fetch_name.success", {
      message: `Successfully fetched name for ${context} ID ${id}`,
      context: {
        function: "fetchNameForId",
        contextType: context,
        id,
        resolvedName: name,
      },
