  ...
  try {
    ...
    switch (userProfile.role) {
      case "superadmin":
        ...
        break;
      case "admin":
        // Admins can simulate any profile except themselves, other admins, and superadmins
        simulatableProfiles = await db
          .select()
          .from(profiles)
          .where(ne(profiles.id, userProfile.id));
      ...
      case "instructional":
        // Instructional staff can simulate any profile except themselves, other instructional staff, admins, and superadmins
        simulatableProfiles = await db
          .select()
          .from(profiles)
          .where(ne(profiles.id, userProfile.id));
