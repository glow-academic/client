...
export const simulations = pgTable(
  "simulations",
  {
    ...
    departmentId: uuid("department_id").notNull(),
    outputGuardrailActive: boolean("output_guardrail_active")
      .default(false)
      .notNull(),
    inputGuardrailActive: boolean("input_guardrail_active")
      .default(false)
      .notNull(),
...
export const userProfiles = pgTable(
  ...
  (table) => [
    uniqueIndex("user_profiles_one_primary_per_user")
      .using("btree", table.userId.asc().nullsLast().op("int4_ops"))
      .where(sql`is_primary`),
    index("user_profiles_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops"),
    ),
    index("user_profiles_user_id_is_primary_idx").using(
      "btree",
    ...
    foreignKey({
      columns: [table.profileId],
      foreignColumns: [profiles.id],
      name: "user_profiles_profile_id_fkey",
    }).onDelete("cascade"),
    primaryKey({
...
export const departmentAgents = pgTable(
  "department_agents",
  {
    departmentId: uuid("department_id").notNull(),
    role: text().notNull(),
  ...
  (table) => [
    index("department_agents_agent_id_idx").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops"),
    ),
    index("department_agents_department_id_role_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops"),
    ...
    primaryKey({
      columns: [table.departmentId, table.role],
      name: "department_agents_pkey",
    }),
  ],
);
...
export const simulationTags = pgTable(
  "simulation_tags",
  {
    simulationId: uuid("simulation_id").notNull(),
    idx: integer().notNull(),
  ...
  (table) => [
    index("simulation_tags_simulation_id_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("uuid_ops"),
    ),
    index("simulation_tags_text_idx").using(
      "btree",
      sql`simulation_id`,
      sql`lower(tag)`,
    ),
    uniqueIndex("simulation_tags_unique_text_per_sim").using(
      "btree",
      sql`simulation_id`,
      sql`lower(tag)`,
    ),
    foreignKey({
      columns: [table.simulationId],
      foreignColumns: [simulations.id],
      name: "simulation_tags_simulation_id_fkey",
    }).onDelete("cascade"),
    primaryKey({
      columns: [table.simulationId, table.idx],
      name: "simulation_tags_pkey",
    }),
  ],
);

export const simulationTagDocuments = pgTable(
  "simulation_tag_documents",
  {
    simulationId: uuid("simulation_id").notNull(),
    tagIdx: integer("tag_idx").notNull(),
    documentId: uuid("document_id").notNull(),
    active: boolean().default(true).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
      .defaultNow()
  ...
  (table) => [
    index("simulation_tag_documents_document_id_idx").using(
      "btree",
      table.documentId.asc().nullsLast().op("uuid_ops"),
    ),
    index("simulation_tag_documents_simulation_id_tag_idx_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("int4_ops"),
      table.tagIdx.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
      columns: [table.documentId],
      foreignColumns: [documents.id],
      name: "simulation_tag_documents_document_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.simulationId, table.tagIdx],
      foreignColumns: [simulationTags.simulationId, simulationTags.idx],
      name: "simulation_tag_documents_tag_fk",
    }).onDelete("cascade"),
    primaryKey({
      columns: [table.simulationId, table.tagIdx, table.documentId],
      name: "simulation_tag_documents_pkey",
    }),
  ],
);

export const simulationTagParameterItems = pgTable(
  ...
  (table) => [
    index("simulation_tag_parameter_items_parameter_item_id_idx").using(
      "btree",
      table.parameterItemId.asc().nullsLast().op("uuid_ops"),
    ),
    index("simulation_tag_parameter_items_simulation_id_tag_idx_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("int4_ops"),
      table.tagIdx.asc().nullsLast().op("int4_ops"),
    ),
    foreignKey({
      columns: [table.parameterItemId],
      foreignColumns: [parameterItems.id],
      name: "simulation_tag_parameter_items_parameter_item_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.simulationId, table.tagIdx],
      foreignColumns: [simulationTags.simulationId, simulationTags.idx],
      name: "simulation_tag_parameter_items_tag_fk",
    }).onDelete("cascade"),
    primaryKey({
      columns: [table.simulationId, table.tagIdx, table.parameterItemId],
      name: "simulation_tag_parameter_items_pkey",
    }),
  ],
);
export const vTaggedDocuments = pgView("v_tagged_documents", {
  simulationId: uuid("simulation_id"),
  tag: text(),
  documentId: uuid("document_id"),
  documentName: text("document_name"),
}).as(
  sql`SELECT st.simulation_id, st.tag, std.document_id, d.name AS document_name FROM simulation_tags st JOIN simulation_tag_documents std ON std.simulation_id = st.simulation_id AND std.tag_idx = st.idx JOIN documents d ON d.id = std.document_id`,
);

export const vTaggedParameterItems = pgView("v_tagged_parameter_items", {
  simulationId: uuid("simulation_id"),
  tag: text(),
  parameterItemId: uuid("parameter_item_id"),
  parameterItemName: text("parameter_item_name"),
}).as(
  sql`SELECT st.simulation_id, st.tag, stpi.parameter_item_id, pi.name AS parameter_item_name FROM simulation_tags st JOIN simulation_tag_parameter_items stpi ON stpi.simulation_id = st.simulation_id AND stpi.tag_idx = st.idx JOIN parameter_items pi ON pi.id = stpi.parameter_item_id`,
);
