...
async function _getSimulationMessagesByChat(chatId: string) {
  try {
    ...
    await log.error("query.fetch_by_fk.failed", {
      message: "Error fetching simulation_messages by chat",
      subject: { entityType: "simulation_messages" },
      context: { function: "_getSimulationMessagesByChat", file: "utils/queries/simulation_messages/get-simulation-messages-by-chat.ts", foreignKey: "chatId", foreignId: String(chatId) },
      error,
    });
    throw error;
  }
