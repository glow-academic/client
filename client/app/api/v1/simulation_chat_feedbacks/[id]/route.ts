...
import { simulationChatFeedbackRepo, SimulationChatFeedbackUpdateSchema } from "@/lib/repos/simulationChatFeedbackRepo";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  return handle(
    async () => { await simulationChatFeedbackRepo.remove(id); return {}; },
    (e: unknown) =>
