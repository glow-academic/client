...
import { simulationCrowdsourcedMessageRepo, SimulationCrowdsourcedMessageUpdateSchema } from "@/lib/repos/simulationCrowdsourcedMessageRepo";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  return handle(
    async () => { await simulationCrowdsourcedMessageRepo.remove(id); return {}; },
    (e: unknown) =>
