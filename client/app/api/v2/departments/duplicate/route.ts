...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${SERVER_URL}/api/v2/departments/duplicate`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to duplicate department" },
    ...
    console.error("Error in department duplicate route:", error);
    return NextResponse.json(
