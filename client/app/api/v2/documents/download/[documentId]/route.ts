/**
 * app/api/v2/documents/download/[documentId]/route.ts
...
export async function GET(
  ...
  try {
    ...
    await log.info("download.document.start", {
      ...
      context: {
        function: "GET",
        file: "app/api/v2/documents/download/[documentId]/route.ts",
      },
    });

    // Forward the request to FastAPI backend
    const response = await fetch(
      `${getApiBase()}/api/v2/documents/download/${documentId}`,
      {
        method: "GET",
        headers: {
