...
describe("Scenarios", () => {
  ...
  describe("Rendering", () => {
    it("should render without crashing", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Office Hours Help Session")
        ).toBeInTheDocument();
      });
    });
    ...
    it("should display scenario descriptions", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            "A scenario where students seek help during office hours"
          )
        ).toBeInTheDocument();
        expect(
          screen.getByText("Students working together on problem sets")
        ).toBeInTheDocument();
        expect(
          screen.getByText("Reviewing key concepts before an exam")
        ).toBeInTheDocument();
      });
    });
    ...
    it("should display edit and delete buttons for each scenario", async () => {
      ...
      await waitFor(() => {
        ...
        const editButtonsCount = editButtons.filter(
          (button) =>
    ...
    it("should have correct accessibility attributes", async () => {
      ...
      await waitFor(() => {
        ...
        const scenarios = screen.getAllByText(
          /Help Session|Study Session|Review Session/
        );
  ...
  describe("User Interactions", () => {
    ...
    it("should handle delete button clicks", async () => {
      ...
      if (deleteButton) {
        ...
        await waitFor(() => {
          ...
          expect(
            screen.getByText(/This will permanently delete the scenario/)
          ).toBeInTheDocument();
        });
      }
    });
    ...
    it("should handle hover effects on cards", async () => {
      ...
      const scenarioCard = screen
        .getByText("Office Hours Help Session")
        .closest("div");
      if (scenarioCard) {
  ...
  describe("API Integration", () => {
    ...
    it("should handle delete errors", async () => {
      const user = userEvent.setup();
      vi.mocked(deleteScenario).mockRejectedValue(new Error("Delete failed"));
  ...
  describe("Loading States", () => {
    it("should handle loading state", () => {
      vi.mocked(getAllScenarios).mockImplementation(
        () => new Promise(() => {})
      ); // Never resolves
      ...
      expect(
        screen.queryByText("Office Hours Help Session")
    ...
    it("should show deleting state", async () => {
      ...
      vi.mocked(deleteScenario).mockImplementation(
        () =>
          new Promise((resolve) => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle scenarios with very long names and descriptions", async () => {
      ...
      vi.mocked(getAllScenarios).mockResolvedValue(scenariosWithLongText);

      renderWithProviders(<Scenarios />);

      await waitFor(() => {
        expect(
          screen.getByText(/This is a very long scenario name/)
        ).toBeInTheDocument();
        expect(
          screen.getByText(/This is an extremely long description/)
        ).toBeInTheDocument();
      });
    });
