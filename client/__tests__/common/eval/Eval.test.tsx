...
describe("Eval Component", () => {
  beforeEach(() => {
    ...
    vi.mocked(getEvalRunsByEval).mockResolvedValue(mockEvalRuns);
  ...
  describe("Create Mode", () => {
    ...
    it("has correct initial form values", async () => {
      ...
      await waitFor(() => {
        ...
        const maxTurnsInput = screen.getByLabelText(
          /Max Turns/
        ) as HTMLInputElement;
        const parallelRunsInput = screen.getByLabelText(
          /Parallel Runs/
        ) as HTMLInputElement;
  ...
  describe("Edit Mode", () => {
    ...
    it("shows update button", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /Update Evaluation/ })
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Form Validation", () => {
    it("shows validation errors for empty required fields", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("At least one scenario must be selected")
        ).toBeInTheDocument();
        expect(
          screen.getByText("At least one agent must be selected")
        ).toBeInTheDocument();
        expect(
          screen.getByText("At least one rubric must be selected")
        ).toBeInTheDocument();
      });
    });
    ...
    it("validates max turns range", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Max turns must be between 1 and 100")
        ).toBeInTheDocument();
      });
    });
    ...
    it("validates parallel runs range", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Parallel runs must be between 1 and 10")
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Form Interactions", () => {
    ...
    it("adds scenarios correctly", async () => {
      ...
      const addScenarioSelect = screen.getByRole("combobox", {
        name: /Add scenario/,
      });
      await user.click(addScenarioSelect);

      const scenarioOption = screen.getByRole("option", {
        name: /Test Scenario/,
      });
  ...
  describe("Form Submission", () => {
    it("submits create form successfully", async () => {
      ...
      vi.mocked(createEval).mockResolvedValue({
        id: "new-eval-id",
      ...
      // Fill out the form
      await user.type(
        screen.getByLabelText(/Evaluation Name/),
        "Test Evaluation"
      );
      await user.type(screen.getByLabelText(/Description/), "Test Description");

      // Select base agent
      const baseAgentSelect = screen.getByRole("combobox", {
        name: /Select base agent/,
      });
      await user.click(baseAgentSelect);
      const baseAgentOption = screen.getByRole("option", {
        name: /Base Agent/,
      });
      ...
      // Add agent
      const addAgentSelect = screen.getByRole("combobox", {
        name: /Add agent/,
      });
      ...
      // Add rubric
      const addRubricSelect = screen.getByRole("combobox", {
        name: /Add rubric/,
      });
    ...
    it("handles create form submission error", async () => {
      ...
      vi.mocked(createEval).mockRejectedValue(new Error("Creation failed"));
      ...
      await waitFor(() => {
        expect(toast.error).toHaveBeenCalledWith(
          "Failed to create evaluation: Creation failed"
        );
      });
    });
  });
  ...
  describe("Loading States", () => {
    it("shows loading state when fetching eval data", () => {
      vi.mocked(getEval).mockImplementation(() => new Promise(() => {})); // Never resolves
    ...
    it("shows submitting state during form submission", async () => {
      ...
      vi.mocked(createEval).mockImplementation(() => new Promise(() => {})); // Never resolves
