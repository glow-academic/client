...
describe('Agent', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should have correct form fields and accessibility attributes', () => {
      ...
      expect(subtitleInput).toHaveAttribute('placeholder', 'Brief description of the agent');
      expect(descriptionTextarea).toHaveAttribute('required');
      expect(descriptionTextarea).toHaveAttribute('placeholder', 'Detailed behavior description and personality traits');
      expect(systemPromptTextarea).toHaveAttribute('required');
      expect(systemPromptTextarea).toHaveAttribute('placeholder', 'System prompt that defines how the agent should behave and respond');
    ...
    it('should show helper text for system prompt and temperature', () => {
      ...
      expect(screen.getByText('This prompt defines the agent\'s behavior and personality in conversations.')).toBeInTheDocument();
      expect(screen.getByText('Temperature value for response randomness (0-100). Lower values are more deterministic.')).toBeInTheDocument();
    });
  });
  ...
  describe('User Interactions', () => {
    ...
    it('should show loading state during submission', async () => {
      const { createAgent } = await import('@/utils/mutations/agents/create-agent');
      (createAgent as any).mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
  ...
  describe('API Integration', () => {
    ...
    it('should populate form data in edit mode', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByDisplayValue('Existing System Prompt')).toBeInTheDocument();
    ...
    it('should handle update agent submission', async () => {
      ...
      const { getAgent } = await import('@/utils/queries/agents/get-agent');
      const { updateAgent } = await import('@/utils/mutations/agents/update-agent');
      (getAgent as any).mockResolvedValue(mockAgent);
