...
const mockStandardGroups = [
  {
    ...
    shortName: "Student Learning",
    description:
      "Ability to guide students to discover solutions independently",
    points: 50,
    passPoints: 35,
    rubricId: "rubric-1",
    createdAt: "2024-01-15T10:00:00Z",
...
const mockStandards = [
  {
    ...
    name: "Excellent (5)",
    description:
      "Consistently guides students to discover solutions independently",
    points: 5,
  ...
  },
  {
    ...
    name: "Excellent (5)",
    description:
      "Demonstrates comprehensive understanding of course objectives",
    points: 5,
...
describe("Rubric", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(createRubric).mockResolvedValue({
      id: "new-rubric-id",
      name: "Test Rubric",
    ...
    vi.mocked(updateRubric).mockResolvedValue(undefined);
  });
  ...
  describe("Rendering", () => {
    ...
    it("should render in create mode by default", () => {
      ...
      expect(screen.getByText("Create Rubric")).toBeInTheDocument();
      expect(
        screen.getByText("Create a new evaluation rubric with scoring criteria")
      ).toBeInTheDocument();
    });
    ...
    it("should render with showAdvancedFeatures enabled", async () => {
      renderWithProviders(
        <Rubric rubricId="rubric-1" mode="edit" showAdvancedFeatures={true} />
      );

      await waitFor(() => {
        expect(
          screen.getByText("Teaching Assistant Evaluation Rubric")
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Form Handling", () => {
    ...
    it("should validate pass points constraints", async () => {
      ...
      await waitFor(() => {
        expect(toast.error).toHaveBeenCalledWith(
          "Pass points must be between 0 and total points"
        );
      });
    });
  ...
  describe("Edit Mode", () => {
    ...
    it("should populate form fields with existing data", async () => {
      renderWithProviders(
        <Rubric rubricId="rubric-1" mode="edit" showAdvancedFeatures={false} />
      );

      await waitFor(() => {
        expect(
          screen.getByDisplayValue("Teaching Assistant Evaluation Rubric")
        ).toBeInTheDocument();
        expect(
          screen.getByDisplayValue(
            "A comprehensive rubric for evaluating TA performance"
          )
        ).toBeInTheDocument();
    ...
    it("should handle rubric not found error", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText("The rubric you're looking for doesn't exist.")
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Advanced Features", () => {
    it("should display standard groups when advanced features are enabled", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Facilitates student-driven learning")
        ).toBeInTheDocument();
        expect(
          screen.getByText("Demonstrates understanding of course objectives")
        ).toBeInTheDocument();
      });
    });
    ...
    it("should handle collapsible standard groups", async () => {
      ...
      const groupHeader = screen
        .getByText("Facilitates student-driven learning")
        .closest("div");
      if (groupHeader) {
  ...
  describe("Loading States", () => {
    it("should show loading state while fetching rubric data", () => {
      vi.mocked(getRubric).mockImplementation(() => new Promise(() => {})); // Never resolves
    ...
    it("should handle loading states for standard groups", () => {
      vi.mocked(getStandardGroupsByRubric).mockImplementation(
        () => new Promise(() => {})
      );
  ...
  describe("API Integration", () => {
    ...
    it("should fetch standards when standard groups are available", async () => {
      ...
      await waitFor(() => {
        expect(getStandardsByStandardGroups).toHaveBeenCalledWith([
          "group-1",
          "group-2",
        ]);
      });
  ...
  describe("Edge Cases", () => {
    it("should handle empty standard groups", async () => {
      vi.mocked(getStandardGroupsByRubric).mockResolvedValue([]);
    ...
    it("should handle component unmounting during API calls", async () => {
      const { unmount } = renderWithProviders(
        <Rubric rubricId="rubric-1" mode="edit" />
      );
    ...
    it("should handle form submission errors", async () => {
      const user = userEvent.setup();
      vi.mocked(createRubric).mockRejectedValue(new Error("Creation failed"));
