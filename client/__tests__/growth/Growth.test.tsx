...
vi.mock('@/utils/queries/simulation_attempts/get-simulation-attempts-by-user', () => ({
...
describe('Growth', () => {
  ...
  beforeEach(() => {
    ...
    // Apply mocks
    require('@/utils/queries/users/get-user').getUser.mockResolvedValue(mockUser);
    require('@/utils/queries/agents/get-all-agents').getAllAgents.mockResolvedValue(mockAgents);
    ...
    require('@/utils/queries/simulation_attempts/get-simulation-attempts-by-user').getSimulationAttemptsByUser.mockResolvedValue(mockAttempts);
  ...
  describe('API Integration', () => {
    it('should handle API calls correctly', async () => {
      ...
      await waitFor(() => {
        expect(require('@/utils/queries/users/get-user').getUser).toHaveBeenCalledWith('test-user-id');
        expect(require('@/utils/queries/simulation_attempts/get-simulation-attempts-by-user').getSimulationAttemptsByUser).toHaveBeenCalledWith('test-user-id');
      });
    });

    it('should handle loading states', () => {
      // Mock loading state
      require('@/utils/queries/users/get-user').getUser.mockImplementation(() => new Promise(() => {}));
    ...
    it('should handle no data state', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Complete some teaching sessions to see your growth metrics.')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Data Display', () => {
    it('should display calculated metrics', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Average performance score')).toBeInTheDocument();
        expect(screen.getByText('Adapting to student needs')).toBeInTheDocument();
        expect(screen.getByText('Active listening ability')).toBeInTheDocument();
    ...
    it('should display insights', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Performance Insights')).toBeInTheDocument();
        expect(screen.getByText('Personalized recommendations for improvement')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle API errors gracefully', async () => {
      require('@/utils/queries/users/get-user').getUser.mockRejectedValue(new Error('API Error'));
