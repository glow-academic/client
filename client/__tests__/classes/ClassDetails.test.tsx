...
vi.mock('@/utils/queries/simulation_attempts/get-simulation-attempts-by-class', () => ({
...
describe('ClassDetails', () => {
  ...
  beforeEach(() => {
    ...
    require('@/utils/queries/topics/get-topics-by-class').getTopicsByClass.mockResolvedValue(mockTopics);
    require('@/utils/queries/simulations/get-simulations-by-class').getSimulationsByClass.mockResolvedValue(mockSimulations);
    require('@/utils/queries/simulation_attempts/get-simulation-attempts-by-class').getSimulationAttemptsByClass.mockResolvedValue(mockAttempts);
    require('@/utils/queries/simulation_chats/get-simulation-chats-by-attempts').getSimulationChatsByAttempts.mockResolvedValue(mockChats);
  ...
  describe('API Integration', () => {
    it('should handle API calls correctly', async () => {
      ...
      await waitFor(() => {
        expect(require('@/utils/queries/classes/get-class').getClass).toHaveBeenCalledWith('test-class-id');
        expect(require('@/utils/queries/topics/get-topics-by-class').getTopicsByClass).toHaveBeenCalledWith(['test-class-id']);
        expect(require('@/utils/queries/simulations/get-simulations-by-class').getSimulationsByClass).toHaveBeenCalledWith(['test-class-id']);
      });
    });

    it('should handle loading states', () => {
      // Mock loading state
      require('@/utils/queries/classes/get-class').getClass.mockImplementation(() => new Promise(() => {}));
    ...
    it('should handle error states when class not found', async () => {
      require('@/utils/queries/classes/get-class').getClass.mockResolvedValue(null);
      
      renderWithProviders(<ClassDetails classId="non-existent-id" />);
      
      await waitFor(() => {
        expect(screen.getByText('Class Not Found')).toBeInTheDocument();
        expect(screen.getByText("The class you're looking for doesn't exist.")).toBeInTheDocument();
      });
    });
  });
