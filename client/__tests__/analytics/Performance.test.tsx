...
vi.mock('@/utils/queries/simulation_chat_grades/get-simulation-chat-grades-by-simulationchats', () => ({
  getSimulationChatGradesBySimulationChats: vi.fn(() => Promise.resolve([
    ...
    { id: '2', simulationChatId: '2', score: 72, passed: true, timeTaken: 450, rubricId: '1' },
  ])),
}));
...
describe('Performance', () => {
  ...
  describe('Data Integration', () => {
    ...
    it('should display skill categories dynamically', async () => {
      renderWithProviders(<Performance />);
      ...
      // Should show dynamic skill categories based on standard groups
      expect(screen.getAllByText('Communicationskills').length).toBeGreaterThan(0);
      expect(screen.getAllByText('Problemsolving').length).toBeGreaterThan(0);
    });
  });
 ...
 * - Uses hooks: useMemo, useQuery, users, user, userId, username
 * - Uses router: false
 * - Has API calls: true
