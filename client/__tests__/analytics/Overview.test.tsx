...
vi.mock('@/utils/queries/standard_groups/get-standard-groups-by-rubrics', () => ({
  getStandardGroupsByRubrics: vi.fn(() => Promise.resolve([
    { id: '1', name: 'Personality Consistency', rubricId: '1', points: 25, passPoints: 18 },
    { id: '2', name: 'Learning Behavior', rubricId: '1', points: 25, passPoints: 18 },
  ])),
}));

vi.mock('@/utils/queries/standards/get-standards-by-standardgroups', () => ({
  getStandardsByStandardGroups: vi.fn(() => Promise.resolve([
    { id: '1', name: 'Maintains Character Voice', standardGroupId: '1', points: 12 },
    { id: '2', name: 'Knowledge Progression', standardGroupId: '2', points: 15 },
  ])),
}));
...
vi.mock('@/utils/queries/simulation_chat_grades/get-simulation-chat-grades-by-simulationchats', () => ({
  getSimulationChatGradesBySimulationChats: vi.fn(() => Promise.resolve([
    { id: '1', simulationChatId: '1', score: 85, timeTaken: 1800, createdAt: new Date().toISOString() },
    { id: '2', simulationChatId: '2', score: 72, timeTaken: 1200, createdAt: new Date().toISOString() },
  ])),
}));
  ...
  describe('Rendering', () => {
    ...
    it('should render performance trends chart', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Training scores and session completion over time')).toBeInTheDocument();
      });
    });
    ...
    it('should render daily session activity chart', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Training session volume and completion rates')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('should have accessible card components', async () => {
      ...
      await waitFor(() => {
        ...
        const cards = screen.getAllByText('Active TAs').map(el => el.closest('[role]'));
