...
export function useScenarioParameterItems(filters?: unknown) {
  return useQuery({
    ...
    queryFn: () =>
      api<ScenarioParameterItem[]>("/api/v1/scenario_parameter_items"),
  });
}
...
export function useCreateScenarioParameterItem() {
  ...
  return useMutation({
    mutationFn: (payload: ScenarioParameterItemCreate) =>
      api<ScenarioParameterItem>("/api/v1/scenario_parameter_items", {
        method: "POST",
        body: JSON.stringify(payload),
      }),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: scenarioParameterItemKeys.all }),
  });
}
