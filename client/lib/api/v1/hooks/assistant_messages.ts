...
export function useCreateAssistantMessage() {
  ...
  return useMutation({
    mutationFn: (payload: AssistantMessageCreate) =>
      api<AssistantMessage>("/api/v1/assistant_messages", {
        method: "POST",
...
export function useUpdateAssistantMessage(id?: string) {
  ...
  return useMutation({
    mutationFn: (patch: AssistantMessageUpdate & { id?: string }) => {
      ...
      return api<AssistantMessage>(`/api/v1/assistant_messages/${resolvedId}`, {
        method: "PATCH",
    ...
    onSuccess: (_data, variables) => {
      ...
      if (resolvedId && resolvedId !== "") {
        qc.invalidateQueries({
          queryKey: assistantMessageKeys.detail(resolvedId),
        });
      } else {
        qc.invalidateQueries({ queryKey: assistantMessageKeys.all });
      }
...
export function useDeleteAssistantMessage(id?: string) {
  ...
  return useMutation({
    mutationFn: (arg?: { id?: string } | string) => {
      ...
      return api<void>(`/api/v1/assistant_messages/${resolvedId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () =>
...
export function useAssistantMessagesByChatId(id: string) {
  return useQuery<AssistantMessage[]>({
    ...
    queryFn: () =>
      api<AssistantMessage[]>(`/api/v1/assistant_messages/by/chatId/${id}`),
...
export function useAssistantMessagesByChatIdBatch(ids: string[]) {
  return useQuery<AssistantMessage[]>({
    ...
    queryFn: () =>
      api<AssistantMessage[]>(`/api/v1/assistant_messages/by/chatId/batch`, {
        method: "POST",
