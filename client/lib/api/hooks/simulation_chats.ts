...
export function useCreateSimulationChat() {
  ...
  return useMutation({
    mutationFn: (payload: SimulationChatCreate) =>
      api<SimulationChat>("/api/v1/simulation_chats", {
        method: "POST",
...
export function useUpdateSimulationChat(id?: string) {
  ...
  return useMutation({
    mutationFn: (patch: SimulationChatUpdate & { id?: string }) => {
      ...
      return api<SimulationChat>(`/api/v1/simulation_chats/${resolvedId}`, {
        method: "PATCH",
    ...
    onSuccess: (_data, variables) => {
      ...
      if (resolvedId && resolvedId !== "") {
        qc.invalidateQueries({
          queryKey: simulationChatKeys.detail(resolvedId),
        });
      } else {
...
export function useDeleteSimulationChat(id?: string) {
  ...
  return useMutation({
    mutationFn: (arg?: { id?: string } | string) => {
      ...
      return api<void>(`/api/v1/simulation_chats/${resolvedId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: simulationChatKeys.all }),
...
export function useSimulationChatsByScenarioId(id: string) {
  return useQuery<SimulationChat[]>({
    ...
    queryFn: () =>
      api<SimulationChat[]>(`/api/v1/simulation_chats/by/scenarioId/${id}`),
...
export function useSimulationChatsByScenarioIdBatch(ids: string[]) {
  return useQuery<SimulationChat[]>({
    ...
    queryFn: () =>
      api<SimulationChat[]>(`/api/v1/simulation_chats/by/scenarioId/batch`, {
        method: "POST",
...
export function useSimulationChatsByAttemptId(id: string) {
  return useQuery<SimulationChat[]>({
    ...
    queryFn: () =>
      api<SimulationChat[]>(`/api/v1/simulation_chats/by/attemptId/${id}`),
    enabled: id !== undefined && id !== null && id !== "",
  });
}
...
export function useSimulationChatsByAttemptIdBatch(ids: string[]) {
  return useQuery<SimulationChat[]>({
    ...
    queryFn: () =>
      api<SimulationChat[]>(`/api/v1/simulation_chats/by/attemptId/batch`, {
        method: "POST",
        body: JSON.stringify({ ids }),
      }),
    enabled: Array.isArray(ids) && ids.length > 0,
