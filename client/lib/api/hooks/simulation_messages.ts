...
export function useCreateSimulationMessage() {
  ...
  return useMutation({
    mutationFn: (payload: SimulationMessageCreate) =>
      api<SimulationMessage>("/api/v1/simulation_messages", {
        method: "POST",
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      if (resolvedId && resolvedId !== "") {
        qc.invalidateQueries({
          queryKey: simulationMessageKeys.detail(resolvedId),
        });
      } else {
        qc.invalidateQueries({ queryKey: simulationMessageKeys.all });
      }
...
export function useDeleteSimulationMessage(id?: string) {
  ...
  return useMutation({
    mutationFn: (arg?: { id?: string } | string) => {
      ...
      return api<void>(`/api/v1/simulation_messages/${resolvedId}`, {
        method: "DELETE",
      });
    },
    onSuccess: () => {
...
export function useSimulationMessagesByChatId(id: string) {
  return useQuery<SimulationMessage[]>({
    ...
    queryFn: () =>
      api<SimulationMessage[]>(`/api/v1/simulation_messages/by/chatId/${id}`),
...
export function useSimulationMessagesByChatIdBatch(ids: string[]) {
  return useQuery<SimulationMessage[]>({
    ...
    queryFn: () =>
      api<SimulationMessage[]>(`/api/v1/simulation_messages/by/chatId/batch`, {
        method: "POST",
