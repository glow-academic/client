...
export const SimulationMessageCreateSchema =
  createInsertSchema(simulationMessages);
export const SimulationMessageUpdateSchema =
  SimulationMessageCreateSchema.partial();
...
export const simulationMessageRepo = {
  ...
  async list() {
    ...
    return db
      ...
      .orderBy(simulationMessages.createdAt ?? simulationMessages.id);
  },
  async find(id: string) {
    ...
    if (!rows[0])
      throw HttpError.notFound(
        "SimulationMessage with id " + id + " not found",
      );
    return rows[0];
  },
  ...
  async listBySimulationChat(simulationChatId: string) {
    ...
    return db
      .select()
      .from(simulationMessages)
      .where(eq(simulationMessages.chatId, simulationChatId));
  },
  ...
  async listBySimulationChats(simulationChatIds: string[]) {
    ...
    return db
      .select()
      .from(simulationMessages)
      .where(inArray(simulationMessages.chatId, simulationChatIds));
  },
};
