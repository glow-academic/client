...
export const simulationRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(simulations).orderBy(simulations.createdAt ?? simulations.id);
  },
  async find(id: string) {
    const db = await getDb();
    const rows = await db.select().from(simulations).where(eq(simulations.id, id)).limit(1);
    if (!rows[0]) throw HttpError.notFound("Simulation with id " + id + " not found");
    return rows[0];
  },

  async update(id: string, patch: SimulationUpdate) {
    const db = await getDb();
    const rows = await db.update(simulations).set(patch).where(eq(simulations.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("Simulation with id " + id + " not found");
    return rows[0];
  },
  ...
  async remove(id: string) {
    const db = await getDb();
    const rows = await db.delete(simulations).where(eq(simulations.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("Simulation with id " + id + " not found");
  },

  async listByRubric(rubricId: string) {
    const db = await getDb();
    return db.select().from(simulations).where(eq(simulations.rubricId, rubricId));
  },

  async listByRubrics(rubricIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(rubricIds) || rubricIds.length === 0) return [];
    return db.select().from(simulations).where(inArray(simulations.rubricId, rubricIds));
  },
};