...
export const ScenarioObjectiveCreateSchema =
  createInsertSchema(scenarioObjectives);
export const ScenarioObjectiveUpdateSchema =
  ScenarioObjectiveCreateSchema.partial();
...
export const scenarioObjectiveRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(scenarioObjectives).orderBy(scenarioObjectives.idx);
  },
  ...
  async update(_id: unknown, _patch: ScenarioObjectiveUpdate) {
  ...
  async listByScenario(scenarioId: string) {
    ...
    return db
      ...
      .where(eq(scenarioObjectives.scenarioId, scenarioId));
  },
  ...
  async listByScenarios(scenarioIds: string[]) {
    ...
    return db
      .select()
      .from(scenarioObjectives)
      .where(inArray(scenarioObjectives.scenarioId, scenarioIds));
  },
};
