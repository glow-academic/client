...
export const parameterItemRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(parameterItems).orderBy(parameterItems.createdAt ?? parameterItems.id);
  },
  async find(id: string) {
    const db = await getDb();
    const rows = await db.select().from(parameterItems).where(eq(parameterItems.id, id)).limit(1);
    if (!rows[0]) throw HttpError.notFound("ParameterItem with id " + id + " not found");
    return rows[0];
  },

  async update(id: string, patch: ParameterItemUpdate) {
    const db = await getDb();
    const rows = await db.update(parameterItems).set(patch).where(eq(parameterItems.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("ParameterItem with id " + id + " not found");
    return rows[0];
  },
  ...
  async remove(id: string) {
    const db = await getDb();
    const rows = await db.delete(parameterItems).where(eq(parameterItems.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("ParameterItem with id " + id + " not found");
  },

  async listByParameter(parameterId: string) {
    const db = await getDb();
    return db.select().from(parameterItems).where(eq(parameterItems.parameterId, parameterId));
  },

  async listByParameters(parameterIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(parameterIds) || parameterIds.length === 0) return [];
    return db.select().from(parameterItems).where(inArray(parameterItems.parameterId, parameterIds));
  },
};