...
export const simulationTagRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(simulationTags).orderBy(simulationTags.createdAt);
  },
  ...
  async update(_id: unknown, _patch: SimulationTagUpdate) {
  ...
  async listBySimulation(simulationId: string) {
    ...
    return db
      .select()
      .from(simulationTags)
      .where(eq(simulationTags.simulationId, simulationId));
  },
  ...
  async listBySimulations(simulationIds: string[]) {
    ...
    return db
      .select()
      .from(simulationTags)
      .where(inArray(simulationTags.simulationId, simulationIds));
  },
};
