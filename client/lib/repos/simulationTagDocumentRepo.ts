...
export type SimulationTagDocumentCreate =
  typeof simulationTagDocuments.$inferInsert;
export type SimulationTagDocumentUpdate = Partial<SimulationTagDocumentCreate>;

// Schemas derived from Drizzle table
export const SimulationTagDocumentCreateSchema = createInsertSchema(
  simulationTagDocuments,
);
export const SimulationTagDocumentUpdateSchema =
  SimulationTagDocumentCreateSchema.partial();
...
export const simulationTagDocumentRepo = {
  ...
  async list() {
    ...
    return db
      .select()
      .from(simulationTagDocuments)
      .orderBy(simulationTagDocuments.createdAt);
  },
  ...
  async update(_id: unknown, _patch: SimulationTagDocumentUpdate) {
  ...
  async listByDocument(documentId: string) {
    ...
    return db
      .select()
      .from(simulationTagDocuments)
      .where(eq(simulationTagDocuments.documentId, documentId));
  },
  ...
  async listByDocuments(documentIds: string[]) {
    ...
    return db
      .select()
      .from(simulationTagDocuments)
      .where(inArray(simulationTagDocuments.documentId, documentIds));
  },
  ...
  async listBySimulationTag(simulationTagId: string) {
    ...
    return db
      .select()
      .from(simulationTagDocuments)
      .where(eq(simulationTagDocuments.simulationId, simulationTagId));
  },
  ...
  async listBySimulationTags(simulationTagIds: string[]) {
    ...
    return db
      .select()
      .from(simulationTagDocuments)
      .where(inArray(simulationTagDocuments.simulationId, simulationTagIds));
  },
};
