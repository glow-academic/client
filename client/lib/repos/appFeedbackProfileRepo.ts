...
export const AppFeedbackProfileCreateSchema =
  createInsertSchema(appFeedbackProfiles);
export const AppFeedbackProfileUpdateSchema =
  AppFeedbackProfileCreateSchema.partial();
...
export const appFeedbackProfileRepo = {
  ...
  async list() {
    ...
    return db
      .select()
      .from(appFeedbackProfiles)
      .orderBy(appFeedbackProfiles.createdAt);
  },
  ...
  async update(_id: unknown, _patch: AppFeedbackProfileUpdate) {
  ...
  async listByAppFeedback(appFeedbackId: number) {
    ...
    return db
      .select()
      .from(appFeedbackProfiles)
      .where(eq(appFeedbackProfiles.appFeedbackId, appFeedbackId));
  },

  async listByAppFeedbacks(appFeedbackIds: number[]) {
    const db = await getDb();
    if (!Array.isArray(appFeedbackIds) || appFeedbackIds.length === 0)
      return [];
    return db
      .select()
      .from(appFeedbackProfiles)
      .where(inArray(appFeedbackProfiles.appFeedbackId, appFeedbackIds));
  },
  ...
  async listByProfile(profileId: string) {
    ...
    return db
      .select()
      .from(appFeedbackProfiles)
      .where(eq(appFeedbackProfiles.profileId, profileId));
  },
  ...
  async listByProfiles(profileIds: string[]) {
    ...
    return db
      .select()
      .from(appFeedbackProfiles)
      .where(inArray(appFeedbackProfiles.profileId, profileIds));
  },
};
