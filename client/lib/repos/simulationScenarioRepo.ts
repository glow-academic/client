...
export const SimulationScenarioCreateSchema = createInsertSchema(simulationScenarios);
export const SimulationScenarioUpdateSchema = SimulationScenarioCreateSchema.partial();
...
export const simulationScenarioRepo = {
  async create(payload: SimulationScenarioCreate) {
    ...
    const rows = await db.insert(simulationScenarios).values(payload).returning();
    return rows[0];
  },

  async list() {
    const db = await getDb();
    return db.select().from(simulationScenarios).orderBy(simulationScenarios.createdAt ?? simulationScenarios.id);
  },
  ...
  async update(_id: unknown, _patch: SimulationScenarioUpdate) { throw new HttpError(400, "Not supported"); },
  ...
  async listBySimulations(simulationIds: string[]) {
    ...
    return db.select().from(simulationScenarios).where(inArray(simulationScenarios.simulationId, simulationIds));
  },

  async listByScenario(scenarioId: string) {
    const db = await getDb();
    return db.select().from(simulationScenarios).where(eq(simulationScenarios.scenarioId, scenarioId));
  },

  async listByScenarios(scenarioIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(scenarioIds) || scenarioIds.length === 0) return [];
    return db.select().from(simulationScenarios).where(inArray(simulationScenarios.scenarioId, scenarioIds));
  },
};