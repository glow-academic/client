...
export const userProfileRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(userProfiles).orderBy(userProfiles.createdAt ?? userProfiles.id);
  },
  ...
  async update(_id: unknown, _patch: UserProfileUpdate) { throw new HttpError(400, "Not supported"); },
  ...
  async listByProfile(profileId: string) {
    const db = await getDb();
    return db.select().from(userProfiles).where(eq(userProfiles.profileId, profileId));
  },

  async listByProfiles(profileIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(profileIds) || profileIds.length === 0) return [];
    return db.select().from(userProfiles).where(inArray(userProfiles.profileId, profileIds));
  },
};