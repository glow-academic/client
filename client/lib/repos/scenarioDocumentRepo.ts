...
export const ScenarioDocumentCreateSchema = createInsertSchema(scenarioDocuments);
export const ScenarioDocumentUpdateSchema = ScenarioDocumentCreateSchema.partial();
...
export const scenarioDocumentRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(scenarioDocuments).orderBy(scenarioDocuments.createdAt ?? scenarioDocuments.id);
  },
  ...
  async update(_id: unknown, _patch: ScenarioDocumentUpdate) { throw new HttpError(400, "Not supported"); },
  async remove(_id: unknown) { throw new HttpError(400, "Not supported"); },

  async listByScenario(scenarioId: string) {
    const db = await getDb();
    return db.select().from(scenarioDocuments).where(eq(scenarioDocuments.scenarioId, scenarioId));
  },

  async listByScenarios(scenarioIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(scenarioIds) || scenarioIds.length === 0) return [];
    return db.select().from(scenarioDocuments).where(inArray(scenarioDocuments.scenarioId, scenarioIds));
  },

  async listByDocument(documentId: string) {
    const db = await getDb();
    return db.select().from(scenarioDocuments).where(eq(scenarioDocuments.documentId, documentId));
  },

  async listByDocuments(documentIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(documentIds) || documentIds.length === 0) return [];
    return db.select().from(scenarioDocuments).where(inArray(scenarioDocuments.documentId, documentIds));
  },
};