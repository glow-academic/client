...
export type SimulationChatGradeCreate =
  typeof simulationChatGrades.$inferInsert;
export type SimulationChatGradeUpdate = Partial<SimulationChatGradeCreate>;

// Schemas derived from Drizzle table
export const SimulationChatGradeCreateSchema =
  createInsertSchema(simulationChatGrades);
export const SimulationChatGradeUpdateSchema =
  SimulationChatGradeCreateSchema.partial();
...
export const simulationChatGradeRepo = {
  ...
  async list() {
    ...
    return db
      ...
      .orderBy(simulationChatGrades.createdAt ?? simulationChatGrades.id);
  },
  async find(id: string) {
    ...
    if (!rows[0])
      throw HttpError.notFound(
        "SimulationChatGrade with id " + id + " not found",
      );
    return rows[0];
  },
  ...
  async listByRubric(rubricId: string) {
    ...
    return db
      .select()
      .from(simulationChatGrades)
      .where(eq(simulationChatGrades.rubricId, rubricId));
  },
  ...
  async listBySimulationChats(simulationChatIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(simulationChatIds) || simulationChatIds.length === 0)
      return [];
    return db
      .select()
      .from(simulationChatGrades)
