...
export const standardRepo = {
  ...
  async list() {
    const db = await getDb();
    return db.select().from(standards).orderBy(standards.createdAt ?? standards.id);
  },
  async find(id: string) {
    const db = await getDb();
    const rows = await db.select().from(standards).where(eq(standards.id, id)).limit(1);
    if (!rows[0]) throw HttpError.notFound("Standard with id " + id + " not found");
    return rows[0];
  },

  async update(id: string, patch: StandardUpdate) {
    const db = await getDb();
    const rows = await db.update(standards).set(patch).where(eq(standards.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("Standard with id " + id + " not found");
    return rows[0];
  },
  ...
  async remove(id: string) {
    const db = await getDb();
    const rows = await db.delete(standards).where(eq(standards.id, id)).returning();
    if (!rows[0]) throw HttpError.notFound("Standard with id " + id + " not found");
  },

  async listByStandardGroup(standardGroupId: string) {
    const db = await getDb();
    return db.select().from(standards).where(eq(standards.standardGroupId, standardGroupId));
  },

  async listByStandardGroups(standardGroupIds: string[]) {
    const db = await getDb();
    if (!Array.isArray(standardGroupIds) || standardGroupIds.length === 0) return [];
    return db.select().from(standards).where(inArray(standards.standardGroupId, standardGroupIds));
  },
};