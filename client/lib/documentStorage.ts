 ...
 */
export async function getDocumentsByProfile(profile: string): Promise<StoredDocument[]> {
  try {
    // Try to fetch from API first
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/documents/${profile}`);
    if (!response.ok) {
      // API failed, use localStorage fallback
      console.warn(`API unavailable, using localStorage fallback for documents/${profile}`);
 ...
 * Falls back to localStorage if API fails
 */
export async function uploadDocument(file: File, profile: string): Promise<StoredDocument | null> {
  try {
...
export async function deleteDocument(documentId: string): Promise<boolean> {
  try {
    ...
    if (!response.ok) {
      // API failed, use localStorage fallback
      console.warn(`API unavailable, using localStorage fallback to delete document ${documentId}`);
