/**
 * DocumentFields.tsx
 * Resource component for document-type field selection
 * Uses SelectableGrid for field selection with search/filter support
 * Manages document_field_ids array and reports to parent
 */
...
export interface DocumentFieldsProps {
  ...
  createDocumentFieldsAction?:
    | ((input: CreateDraftDocumentFieldsIn) => Promise<CreateDraftDocumentFieldsOut>)
    | undefined;
}
...
export function DocumentFields({
  ...
  const handleSelect = useCallback(
    async (fieldId: string) => {
      ...
      if (isSelected) {
        ...
        if (
          ...
          try {
            await createDocumentFieldsAction({
              body: {
                agent_id: agent_id,
                group_id: group_id,
                field_id: fieldId,
            ...
            console.error(
              `Failed to create document field resource for ${fieldId}:`,
              error
            );
          }
        }
    ...
    [ids, onChange, createDocumentFieldsAction, agent_id, group_id]
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <SelectableGrid<DocumentFieldItem>
        items={fieldItems}
        selectedId={null}
        selectedIds={ids}
        ...
        emptyMessage="No document fields found."
        disabled={disabled}
      />
    </div>
  );
