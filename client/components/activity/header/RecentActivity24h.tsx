/**
 * RecentActivity24h.tsx
 * Fast and dumb UI component for displaying recent activity (24h) metric.
...
export interface RecentActivity24hProps {
  recentActivity24h: number;
  hasDataAvailable: boolean;
  status: "success" | "warning" | "danger" | "neutral";
}

export default function RecentActivity24h({
  recentActivity24h,
  hasDataAvailable,
  status,
}: RecentActivity24hProps) {
    ...
    >
      ...
      <CardContent className="flex-1 flex flex-col justify-center">
        <div className={`text-2xl font-bold ${textClasses}`}>
          {hasDataAvailable ? `${recentActivity24h}` : "0"}
        </div>
      </CardContent>
    </Card>
  );
