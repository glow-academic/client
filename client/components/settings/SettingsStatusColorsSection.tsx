/**
 * SettingsStatusColorsSection.tsx
 * Status colors section with individual cards for each color picker and threshold sliders
 */
"use client";
...
interface StatusColorCardProps {
  ...
  renderThresholdInHeader?: boolean;
}

export const StatusColorCard = forwardRef<HTMLDivElement, StatusColorCardProps>(
  (
    {
      label,
      fieldName,
...
export function SettingsStatusColorsSection({
  colors,
  originalColors,
  onColorChange,
  onThresholdChange,
  ...
  isReadonly,
  cardRefs,
}: SettingsStatusColorsSectionProps) {
  // Constraint logic: ensure danger < warning < success
  const handleDangerThresholdChange = (value: number) => {
    const newValue = Math.min(value, colors.warning_threshold - 1);
    onThresholdChange("danger_threshold", Math.max(0, newValue));
  };
  ...
  const handleSuccessThresholdChange = (value: number) => {
    const newValue = Math.max(value, colors.warning_threshold + 1);
    onThresholdChange("success_threshold", Math.min(100, newValue));
  };

  return (
    <div className="space-y-6">
      {/* Individual Status Color Cards */}
      <StatusColorCard
        ref={cardRefs?.successRef}
        label="Success"
        fieldName="success"
        value={colors.success}
        originalValue={originalColors.success}
        onColorChange={onColorChange}
        onReset={() => onResetColor("success")}
        threshold={{
          value: colors.success_threshold,
          min: colors.warning_threshold,
          max: 100,
          onValueChange: handleSuccessThresholdChange,
        }}
      ...
      <StatusColorCard
        ...
        originalValue={originalColors.warning}
        onColorChange={onColorChange}
        onReset={() => onResetColor("warning")}
        threshold={{
          value: colors.warning_threshold,
          min: colors.danger_threshold,
          max: colors.success_threshold,
      ...
      <StatusColorCard
        ...
        fieldName="error"
        value={colors.error}
        originalValue={originalColors.error}
        onColorChange={onColorChange}
        onReset={() => onResetColor("error")}
        threshold={{
          value: colors.danger_threshold,
          min: 0,
          max: colors.warning_threshold,
          onValueChange: handleDangerThresholdChange,
        }}
        stepStatus={stepStatus}
