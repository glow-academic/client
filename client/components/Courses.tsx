...
export default function Courses() {
    ...
    const coursesWithStats = useMemo<CourseWithStats[]>(() => {
        ...
        return classes.map(course => {
            ...
            const assignedTAs = users.filter(user => 
                !user.admin && user.classes && user.classes.includes(course.id)
            ).length;

            // Count chats for this course
            const courseChats = chats.filter(chat => chat.classId === course.id).length;
    ...
    if (isLoadingUsers || isLoadingClasses || isLoadingChats) {
        return <div className="flex justify-center items-center p-10">Loading courses...</div>;
    }
                        ...
                        <CardContent>
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center text-sm">
                                    ...
                                    <span><strong>{course.chatCount}</strong> Interactions</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
