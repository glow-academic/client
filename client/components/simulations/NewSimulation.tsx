 ...
 * Updated to use unified get/save endpoints (following personas pattern)
...
export default function NewSimulation({
  simulationId,
  simulationData: serverSimulationData,
  saveSimulationAction,
  patchSimulationDraftAction,
}: SimulationProps) {
  ...
  // Extract body type from server action type for type safety
  type SaveSimulationBody = SaveSimulationIn extends { body: infer B }
    ? B
    : never;

  // Use server action directly (no mutations needed)
  const handleSaveSimulation = useCallback(
    async (body: SaveSimulationBody) => {
      if (!saveSimulationAction) {
        throw new Error("saveSimulationAction is required");
      }
      await saveSimulationAction({ body });
    },
    [saveSimulationAction]
  );
  ...
  useEffect(() => {
    if (serverSimulationData?.name && simulationId && isEditMode) {
      setEntityMetadata({
        entityId: simulationId,
        entityName: serverSimulationData.name,
        entityType: "simulation",
      });
    }
    return () => clearEntityMetadata();
  }, [
    serverSimulationData,
    simulationId,
    isEditMode,
    setEntityMetadata,
    clearEntityMetadata,
  ...
  // Use server-provided data (unified for both modes)
  const simulationData = serverSimulationData;
  ...
  const initialDraftState = useMemo((): DraftState => {
    ...
    simulationData?.practice_simulation,
    simulationData?.department_ids,
    simulationData?.scenario_ids,
    simulationData?.,
    simulationData?.simulation_text_domain_id,
    simulationData?.simulation_voice_domain_id,
  ...
  const rubricMapping = useMemo(() => {
    const rubrics =
      (simulationData && "rubrics" in simulationData
    ...
    async (_formDataLocal: unknown) => {
      ...
      try {
        ...
        if (simulationId) {
          // UPDATE mode
          const updatePayload = {
            input_simulation_id: simulationId,
          ...
          await handleSaveSimulation(updatePayload);
          toast.success("Simulation updated successfully!");
        } else {
          // CREATE mode
          const createPayload = {
            input_simulation_id: null, // NULL for create mode
            title: draftState.title,
            description: draftState.description ?? "",
            department_ids: finalDepartmentIds || [],
            active: draftState.active || true,
          ...
          await handleSaveSimulation(createPayload);
