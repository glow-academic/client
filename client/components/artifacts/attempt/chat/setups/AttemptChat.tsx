...
export function AttemptChat({
  ...
  const messagesByChat = useMemo(
    () => groupMessagesByChat(attemptData?.entries?.attempt_message),
    [attemptData?.entries?.attempt_message, groupMessagesByChat]
  );
  ...
  useEffect(() => {
    ...
    if (initialAttemptData.current_chat_index !== undefined) {
      ...
      } else if (initialAttemptData.entries?.attempt_chat) {
        const currentChatId = attemptData?.entries?.attempt_chat?.[currentChatIndex]?.id;
        const currentChatStillExists = initialAttemptData.entries?.attempt_chat.some(
          (c) => c.id === currentChatId
        );
        if (!currentChatStillExists && initialAttemptData.entries?.attempt_chat.length > 0) {
          setCurrentChatIndex(initialAttemptData.current_chat_index ?? 0);
        }
      }
    }
  ...
  useEffect(() => {
    if (!pendingNextChatIdRef.current || !attemptData?.entries?.attempt_chat) return;
    const nextChatId = pendingNextChatIdRef.current;
    const sortedChats = [...attemptData.entries?.attempt_chat]
  ...
  const currentChat = useMemo(() => {
    const chats = attemptData?.entries?.attempt_chat ?? [];
  ...
  const chats = useMemo(
    () =>
      attemptData?.entries?.attempt_chat?.filter(
  ...
  useEffect(() => {
    ...
    attemptData?.entries?.attempt_chat,
  ...
  const chatHeaderProps: ChatHeaderProps = useMemo(() => {
    ...
    return {
      ...
      current_dynamic_rubric: (() => {
        const chatData = attemptData?.entries?.attempt_chat?.[currentChatIndex];
      ...
      chats:
        attemptData?.entries?.attempt_chat?.map((c) => ({
          id: c.id || "",
