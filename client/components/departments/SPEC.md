...
- `api_create_departments_v4` returns only the new resource `id`; it does not return metadata columns from `departments_resource` (created_at, updated_at, active, generated, mcp, call_id, group_id when present). If the UI needs those without a follow-up fetch, extend the SQL response or add a read endpoint.
