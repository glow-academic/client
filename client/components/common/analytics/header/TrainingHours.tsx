/**
 * TrainingHours.tsx
 * This is used to show the total training hours.
...
export interface TrainingHoursProps {
  color?: ColorTheme;
  timeRange?: TimeRange;
  title?: string;
  showDialog?: boolean;
...
export default function TrainingHours({
  ...
  // Calculate total training hours for the selected time range
  const totalTrainingHours = useMemo(() => {
    ...
    // Calculate total hours (assuming each chat session is roughly 30 minutes)
    return Math.round(filteredChats.length * 0.5 * 10) / 10;
  }, [chats, timeRange]);

  // Training hours trend data
  const trainingHoursTrend = useMemo(() => {
    if (!chats) return [];
    ...
    return Array.from({ length: Math.min(days, 14) }, (_, i) => {
      ...
      // Calculate hours for this day (assuming 30 minutes per chat)
      const hours = Math.round(dayChats.length * 0.5 * 10) / 10;
      ...
      {showDialog && (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>Training Hours Trend</DialogTitle>
            </DialogHeader>
            <div className="h-64">
                ...
                <AreaChart data={trainingHoursTrend}>
                  ...
                  <Tooltip
                    formatter={(value: number, name: string) => [
                      ...
                      name === "hours" ? "Training Hours" : "Sessions",
                    ]}
                  />
                  <Area
                    type="monotone"
