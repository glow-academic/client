/**
 * HighestScore.tsx
 * Displays the highest score metric using analytics endpoint.
...
export default function HighestScore({
  ...
  const { data, isLoading, isError } = useAnalyticsHighestScore(filters, true);

  // 2) Derive values from MetricResponse (method + dataPoints + trendData)
  const { highestScore, scoreTrend, hasDataAvailable } = useMemo(() => {
    ...
    const current = computeCurrent(resp.method, points); // returns number 0..100 for avg/max, etc.
  ...
  const colorConfig = useMemo(() => {
    ...
    if (highestScore < thresholds.danger) return COLOR_CONFIGS.danger;
    if (highestScore < thresholds.warning) return COLOR_CONFIGS.warning;
    return COLOR_CONFIGS.success;
  }, [highestScore, thresholds, hasDataAvailable]);
