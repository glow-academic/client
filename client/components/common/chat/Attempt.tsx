...
export default function Attempt({ attemptId }: { attemptId: string }) {
    ...
    const currentChat = React.useMemo(() => {
        ...
        const chat = chats.find(chat => chat.scenarioId === currentChatSimulationId);
        return chat || chats[0];
    ...
    const currentDynamicRubric = useMemo((): DynamicRubric | null => {
        ...
        const chatGrade = grades.find(grade => grade.simulationChatId === currentChat.id);
    ...
    const allDynamicRubrics = useMemo((): DynamicRubric[] => {
        if (!chats || !grades || !feedbacks || !standards || !standardGroups) return [];
    ...
    useEffect(() => {
        if (chats && chats.length > 0 && simulation?.scenarioIds && currentChatIndex === 0) {
            // Find the first incomplete chat
            const firstIncompleteIndex = simulation.scenarioIds.findIndex((scenarioId: string) => {
                const chat = chats.find((c: SimulationChat) => c.scenarioId === scenarioId);
    ...
    useEffect(() => {
        if (currentChat?.completed && !showResults) {
            ...
            if (isFreshlyCompleted) {
                if (!isSingleChatAttempt && currentChatIndex < (simulation?.scenarioIds?.length || 0) - 1) {
                    // Move to next chat after a short delay (only for multi-chat attempts)
                    const timer = setTimeout(() => {
                        setCurrentChatIndex(prev => {
                            const nextIndex = prev + 1;
                            toast.success(`Moving to chat ${nextIndex + 1} of ${simulation?.scenarioIds?.length || 0}`);
                            return nextIndex;
                        });
                    }, 2000);
    ...
    }, [currentChat?.completed, currentChat?.id, currentChatIndex, simulation?.scenarioIds?.length, showResults, isSingleChatAttempt, freshlyCompletedChats]);
    ...
    const aggregatedResults = useMemo(() => {
        ...
        const totalScore = allDynamicRubrics.reduce((sum: number, rubric: DynamicRubric) => sum + rubric.score, 0);
        const averageScore = totalScore / allDynamicRubrics.length;
        const passedChats = allDynamicRubrics.filter((rubric: DynamicRubric) => rubric.passed).length;
        ...
        const totalTime = chats
            ? chats
                ...
                .reduce((sum: number, chat: SimulationChat) => sum + calculateActualTimeTaken(chat), 0)
            : 0;
                    ...
                    {!isSingleChatAttempt && (
                        <div className="grid gap-4">
                            ...
                            {allDynamicRubrics.map((rubric: DynamicRubric, index: number) => {
                                const chat = chats?.find((c: SimulationChat) => c.id === rubric.chatId);
                                ...
                                <div className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
                                    {isSingleChatAttempt ? (
                                        ...
                                        <>
                                            ...
                                            <span data-testid="chat-counter">Chat {currentChatIndex + 1} of {simulation?.scenarioIds?.length || 0}</span>
                                            {scenario && (
                                                <>
                                                    <span>•</span>
                                                    {formatScenarioInfo(scenario!)}
