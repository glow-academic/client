...
export interface ReasoningCardGridProps {
  selectedIds: string[];
...
export function ReasoningCardGrid({
  selectedIds,
  onSelect,
  ...
  const filteredReasoningLevels = React.useMemo(() => {
    let filtered = baseReasoningLevels;
    ...
    // Sort: selected levels first, then unselected by name
  ...
  }, [baseReasoningLevels, searchTerm, selectedIds]);
    ...
    <TooltipProvider>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[272px] overflow-y-auto py-2 px-2">
          {filteredReasoningLevels.length === 0 ? (
            <div className="col-span-full text-center py-8 text-muted-foreground">
              No reasoning levels found. Try adjusting your search.
            </div>
          ) : (
