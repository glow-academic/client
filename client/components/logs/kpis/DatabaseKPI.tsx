/**
 * DatabaseKPI.tsx
 * Health KPI component for Database service.
 */
"use client";
...
export interface DatabaseKPIProps {
  ok: boolean;
  latency_ms: number;
  error: string;
  trend: TrendData[];
...
export default function DatabaseKPI({
  ok,
  latency_ms,
  error,
  trend,
  ...
  const displayColor = isMounted ? chartColor : chartColorFallbacks.chart4;
  ...
  const gradientStyle = useMemo(() => {
    const color = displayColor || chartColorFallbacks.chart4;
    return {
    ...
    <>
      ...
      >
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Server className="h-4 w-4" style={{ color: displayColor }} />
        </CardHeader>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>Database Health Trend</DialogTitle>
            <DialogDescription hidden>
              This chart shows database health metrics over time.
            </DialogDescription>
          </DialogHeader>
