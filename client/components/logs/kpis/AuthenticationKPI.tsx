/**
 * AuthenticationKPI.tsx
 * Health KPI component for Authentication/Keycloak service.
 */
"use client";
...
export interface AuthenticationKPIProps {
  ok: boolean;
  latency_ms: number;
  error: string;
  trend: TrendData[];
...
export default function AuthenticationKPI({
  ok,
  latency_ms,
  error,
  trend,
}: AuthenticationKPIProps) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isMounted, setIsMounted] = useState(false);
  const chartColors = useChartColors();
  const chartColor = chartColors[4]; // chart-5
  ...
  const displayColor = isMounted ? chartColor : chartColorFallbacks.chart5;
    ...
    <>
      ...
      >
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Shield className="h-4 w-4" style={{ color: displayColor }} />
        </CardHeader>
