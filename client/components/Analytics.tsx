...
export default function Analytics() {
    ...
    const getScoreBadgeVariant = (score: number) => {
        if (score >= 80) return { className: "bg-green-50 text-green-700 border-green-200" }
        if (score >= 70) return { className: "bg-amber-50 text-amber-700 border-amber-200" }
    ...
    const needAttentionTAs = useMemo<TAWithScore[]>(() => {
        ...
        const taScores: Record<string, { scores: number[], name: string, username: string }> = {}
    ...
    const interactionsTrend = useMemo(() => {
        ...
        const previous = interactionsTimeData[interactionsTimeData.length - 2].value
    ...
    const studentTypePerformance = useMemo(() => {
        ...
        const result = {
            happy: typeScores.happy.length > 0
                ? Math.round(typeScores.happy.reduce((sum, score) => sum + score, 0) / typeScores.happy.length)
                : 0,
            confused: typeScores.confused.length > 0
                ? Math.round(typeScores.confused.reduce((sum, score) => sum + score, 0) / typeScores.confused.length)
                : 0,
            aggressive: typeScores.aggressive.length > 0
                ? Math.round(typeScores.aggressive.reduce((sum, score) => sum + score, 0) / typeScores.aggressive.length)
                : 0
        };
    ...
    if (isLoadingUsers || isLoadingChats || isLoadingRubrics) {
        return <div className="flex justify-center items-center p-10">Loading analytics...</div>
    }
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                >
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{teachingAssistants.length}</div>
                ...
                >
                    ...
                    <CardContent>
                        <div className="flex items-center">
                            <div className={`text-2xl font-bold ${getScoreColorClass(averageScore)}`}>
                                {averageScore}%
                            </div>
                            {scoreTrend.value > 0 && (
                                <Badge
                                    variant="outline"
                                    className={scoreTrend.isPositive
                    ...
                    <CardContent>
                        <div className="flex items-center">
                            ...
                            {interactionsTrend.value > 0 && (
                                <Badge
                                    variant="outline"
                                    className={interactionsTrend.isPositive
                                        ? "ml-2 bg-green-50 text-green-700 border-green-200"
                                        : "ml-2 bg-red-50 text-red-700 border-red-200"}
                                >
                ...
                >
                    ...
                    <CardContent>
                        <div className="flex items-center">
                            <div className="text-2xl font-bold">{needAttentionTAs.length}</div>
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold">{studentTypePerformance.happy}%</span>
                                </div>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold">{studentTypePerformance.confused}%</span>
                                </div>
                                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                                    <div
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold">{studentTypePerformance.aggressive}%</span>
                                </div>
                                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                                    <div
                                        ...
                                        <div key={ta.id} className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <Avatar className="h-8 w-8">
                                                    <AvatarFallback className="bg-primary/10">{ta.initials}</AvatarFallback>
                                                </Avatar>
                                                <span>{ta.name}</span>
                                            </div>
                                ...
                                <div className="space-y-2">
                                    {taPerformers.needImprovement.map((ta) => (
                                        <div key={ta.id} className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <Avatar className="h-8 w-8">
                                                    <AvatarFallback className="bg-red-100 text-red-800">{ta.initials}</AvatarFallback>
                                                </Avatar>
                                                <span>{ta.name}</span>
                                            </div>
        ...
        <Dialog open={show} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto py-4">
                    {tas.map((ta) => (
                        <div key={ta.id} className="flex items-center justify-between p-3 bg-muted/20 rounded-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm text-muted-foreground">{ta.username}@purdue.edu</div>
                                </div>
                            </div>
                        </div>
                    ))}
                    {tas.length === 0 && (
                        <p className="text-center text-muted-foreground">No teaching assistants found</p>
                    )}
                </div>
            </DialogContent>
        </Dialog>
...
function AvgScoreModal({ show, onClose, scoreData, avgScore, CustomTooltip }: AvgScoreModalProps) {
    if (!show) return null
            ...
            <DialogContent className="sm:max-w-xl">
                ...
                <div className="space-y-4 py-4">
                    <div className="flex justify-center items-center mb-4">
                        <div className={`text-4xl font-bold ${avgScore >= 80 ? 'text-green-600' : avgScore >= 70 ? 'text-amber-600' : 'text-red-600'}`}>
                            {avgScore}%
                        </div>
                    </div>
...
function TotalInteractionsModal({ show, onClose, interactionsData, totalInteractions, CustomTooltip }: TotalInteractionsModalProps) {
    if (!show) return null
...
function NeedAttentionModal({ show, onClose, tas, getInitials }: NeedAttentionModalProps) {
    if (!show) return null
        ...
        <Dialog open={show} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-md">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto py-4">
                    ...
                    {tas.length === 0 && (
                        <p className="text-center text-muted-foreground">All TAs are performing well!</p>
                    )}
                </div>
            </DialogContent>
        </Dialog>
