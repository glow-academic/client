# Field Component + Resource SQL Spec
...
- Uses server-provided data and actions to hydrate `field` data and persist changes.
...
- Navigates to `Field.tsx` for edit/view flows and uses resource IDs for batch operations (duplicate/delete).

## SQL Resource Creation (api_create_fields_v4)
- **Parameters**: `agent_id, group_id, field_id, mcp`.
- **Behavior**:
  1. Validates tooling for the `field` resource.
  2. Assembles `arguments_raw` from schema templates.
  3. Creates a `calls` record and inserts into `fields_resource` (or `field_resource` for singular naming).
  4. Associates runs/messages and links them to the provided `group_id`.
- **Return value**: resource `id` (or `fields_id` in resource-only tables).

## Implementation Guidelines (resource-first)
- **Create resources first**: Always create resource records (via `api_create_*_v4` functions) before linking them to `field` in junction tables.
- **Use explicit IDs**: Persist `field_id` and `<resource>_id` / `<resource>_ids` in form state; never store raw display values in local state.
