...
export function WebSocketProvider({
  ...
  const setupCommonEventHandlers = useCallback((socket: Socket) => {
    ...
    socket.on(
      ...
      (_data: {
        ...
        // Window event dispatched - assistant-context will handle refresh
      },
    );
    ...
    socket.on(
      ...
      (data: {
        ...
        // Dispatch window event for streaming updates
        window.dispatchEvent(
          new CustomEvent("assistant_message_token", {
            detail: {
              messageId: data.message_id,
              chatId: data.chat_id,
    ...
    socket.on(
      ...
      (data: {
        ...
        // Window event dispatched - simulation-context handles state updates directly
        window.dispatchEvent(
    ...
    socket.on(
      ...
      (_data: { message_id: string; chat_id: string }) => {
        // Window event dispatched - contexts will handle their own refreshes
      },
    );

    socket.on("title_updated", (data: { chat_id: string; title: string }) => {
      // Dispatch window event for title updates
      window.dispatchEvent(
    ...
    socket.on(
      ...
      (data: { tool_name: string; chat_id: string }) => {
        // Dispatch window event for tool call creation
        window.dispatchEvent(
          new CustomEvent("tool_call_created", {
            detail: {
    ...
    socket.on(
      "tool_call_completed",
      (data: { tool_name: string; chat_id: string }) => {
        // Dispatch window event for tool call completion
        window.dispatchEvent(
          new CustomEvent("tool_call_completed", {
            detail: {
              tool_name: data.tool_name,
              chat_id: data.chat_id,
            },
    ...
    socket.on(
      ...
      (data: {
        ...
        if (data.success) {
          ...
          // Window event dispatched - contexts/pages will handle their own refreshes
          ...
          // Window event dispatched - home/practice pages will handle refresh via router.refresh()
        } else {
