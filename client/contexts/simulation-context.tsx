  ...
  useEffect(() => {
    ...
    const handleSimulationMessageComplete = (event: CustomEvent) => {
      if (event.detail.chatId === currentChatIdRef.current) {
        ...
        // Refresh SSR data to get final message content
        router.refresh();
    ...
    const handleSimulationNewMessage = (event: CustomEvent) => {
      // Refresh SSR data when new messages are created for the current chat
      if (event.detail.chatId === currentChatIdRef.current) {
        router.refresh();
      }
    };
    ...
    // Listen for new message events to refresh SSR data
    window.addEventListener(
    ...
    return () => {
      ...
      // Remove new message event listener
      window.removeEventListener(
        "simulationNewMessage",
        handleSimulationNewMessage as EventListener
      );
