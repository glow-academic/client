...
vi.mock("@/utils/drizzle/db", () => ({
  db: {
    select: vi.fn(() => ({
      from: vi.fn(() => mockSchema.appLogs || []),
      where: vi.fn(() => mockSchema.appLogs?.[0] || null),
    })),
    insert: vi.fn(() => ({
      values: vi.fn(() => ({
        returning: vi.fn(() => mockSchema.appLogs || []),
      })),
    })),
    update: vi.fn(() => ({
      set: vi.fn(() => ({
    ...
    delete: vi.fn(() => ({
      where: vi.fn(() => mockSchema.appLogs || []),
    })),
  },
  db_url: "mock-database-url",
}));

// Mock the logger to prevent server-side logging issues
...
// Add mock for getAppLogs from utils/logs/get-logs
