...
export class MockDb {
  ...
  simulationTagDocuments: SimulationTagDocument[];
  simulationTagParameterItems: SimulationTagParameterItem[];
  simulationTags: SimulationTag[];
  ...
  constructor() {
    ...
    this.profileDepartments = JSON.parse(JSON.stringify(mockSchema.profileDepartments));
    ...
    this.simulationScenarios = JSON.parse(JSON.stringify(mockSchema.simulationScenarios));
    this.simulationTagDocuments = JSON.parse(JSON.stringify(mockSchema.simulationTagDocuments));
    this.simulationTagParameterItems = JSON.parse(JSON.stringify(mockSchema.simulationTagParameterItems));
    this.simulationTags = JSON.parse(JSON.stringify(mockSchema.simulationTags));
  ...
  getAgent(id: string) { return this.agents.find(item => item.id === id) || null; }
  ...
  getAllAssistantChats() { return this.assistantChats; }
  getAssistantChat(id: string) { return this.assistantChats.find(item => item.id === id) || null; }
  ...
  updateAssistantChat(id: string, data: Partial<AssistantChat>) {
    ...
    if (itemIndex === -1) return null;
    this.assistantChats[itemIndex] = { ...this.assistantChats[itemIndex], ...data } as AssistantChat;
  ...
  getAllAssistantMessages() { return this.assistantMessages; }
  getAssistantMessage(id: string) { return this.assistantMessages.find(item => item.id === id) || null; }
  getAssistantMessagesByAssistantChat(assistantChatId: string) { 
    return this.assistantMessages.filter(item => item.chatId === assistantChatId); 
  }
  ...
  updateAssistantMessage(id: string, data: Partial<AssistantMessage>) {
    const itemIndex = this.assistantMessages.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.assistantMessages[itemIndex] = { ...this.assistantMessages[itemIndex], ...data } as AssistantMessage;
  ...
  getAllAssistantToolCalls() { return this.assistantToolCalls; }
  getAssistantToolCall(id: string) { return this.assistantToolCalls.find(item => item.id === id) || null; }
  getAssistantToolCallsByAssistantChat(assistantChatId: string) { 
    return this.assistantToolCalls.filter(item => item.chatId === assistantChatId); 
  }
  ...
  updateAssistantToolCall(id: string, data: Partial<AssistantToolCall>) {
    const itemIndex = this.assistantToolCalls.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.assistantToolCalls[itemIndex] = { ...this.assistantToolCalls[itemIndex], ...data } as AssistantToolCall;
  ...
  getAllCohortProfiles() { return this.cohortProfiles; }
  getCohortProfilesByCohort(cohortId: string) { 
    return this.cohortProfiles.filter(item => item.cohortId === cohortId); 
  }
  getCohortProfilesByProfile(profileId: string) { 
    return this.cohortProfiles.filter(item => item.profileId === profileId); 
  }

  // COHORTPROFILES Mutations
  createCohortProfile(data: Partial<CohortProfile>) {
    const newItem = createMockCohortProfile({ ...data, id: data.id ?? faker.string.uuid() });
    this.cohortProfiles.push(newItem);
    return newItem;
  }

  // COHORTSIMULATIONS Queries
  getAllCohortSimulations() { return this.cohortSimulations; }
  getCohortSimulationsByCohort(cohortId: string) { 
    return this.cohortSimulations.filter(item => item.cohortId === cohortId); 
  }
  getCohortSimulationsBySimulation(simulationId: string) { 
    return this.cohortSimulations.filter(item => item.simulationId === simulationId); 
  }

  // COHORTSIMULATIONS Mutations
  createCohortSimulation(data: Partial<CohortSimulation>) {
    const newItem = createMockCohortSimulation({ ...data, id: data.id ?? faker.string.uuid() });
    this.cohortSimulations.push(newItem);
    return newItem;
  }

  // COHORTS Queries
  getAllCohorts() { return this.cohorts; }
  getCohort(id: string) { return this.cohorts.find(item => item.id === id) || null; }
  getCohortsByDepartment(departmentId: string) { 
    return this.cohorts.filter(item => item.departmentId === departmentId); 
  }
  ...
  // DEPARTMENTAGENTS Queries
  getAllDepartmentAgents() { return this.departmentAgents; }
  getDepartmentAgentsByDepartment(departmentId: string) { 
    return this.departmentAgents.filter(item => item.departmentId === departmentId); 
  }
  getDepartmentAgentsByAgent(agentId: string) { 
    return this.departmentAgents.filter(item => item.agentId === agentId); 
  }

  // DEPARTMENTAGENTS Mutations
  createDepartmentAgent(data: Partial<DepartmentAgent>) {
    const newItem = createMockDepartmentAgent({ ...data, id: data.id ?? faker.string.uuid() });
    this.departmentAgents.push(newItem);
    return newItem;
  }

  // DEPARTMENTS Queries
  getAllDepartments() { return this.departments; }
  getDepartment(id: string) { return this.departments.find(item => item.id === id) || null; }

  // DEPARTMENTS Mutations
  createDepartment(data: Partial<Department>) {
    const newItem = createMockDepartment({ ...data, id: data.id ?? faker.string.uuid() });
    this.departments.push(newItem);
    return newItem;
  }
  updateDepartment(id: string, data: Partial<Department>) {
    const itemIndex = this.departments.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.departments[itemIndex] = { ...this.departments[itemIndex], ...data } as Department;
    return this.departments[itemIndex];
  }
  deleteDepartment(id: string) {
    const itemIndex = this.departments.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    const deletedItem = this.departments.splice(itemIndex, 1);
    return deletedItem[0];
  }
  ...
  getDocumentsByDepartment(departmentId: string) { 
    return this.documents.filter(item => item.departmentId === departmentId); 
  }
  ...
  updateDocument(id: string, data: Partial<Document>) {
    ...
    if (itemIndex === -1) return null;
    this.documents[itemIndex] = { ...this.documents[itemIndex], ...data } as Document;
  ...
  getModel(id: string) { return this.models.find(item => item.id === id) || null; }
  getModelsByProvider(providerId: string) { 
    return this.models.filter(item => item.providerId === providerId); 
  }
  ...
  getAllParameterItems() { return this.parameterItems; }
  getParameterItem(id: string) { return this.parameterItems.find(item => item.id === id) || null; }
  getParameterItemsByParameter(parameterId: string) { 
    return this.parameterItems.filter(item => item.parameterId === parameterId); 
  }
  ...
  updateParameterItem(id: string, data: Partial<ParameterItem>) {
    ...
    if (itemIndex === -1) return null;
    this.parameterItems[itemIndex] = { ...this.parameterItems[itemIndex], ...data } as ParameterItem;
  ...
  // PARAMETERS Queries
  getAllParameters() { return this.parameters; }
  getParameter(id: string) { return this.parameters.find(item => item.id === id) || null; }
  getParametersByDepartment(departmentId: string) { 
    return this.parameters.filter(item => item.departmentId === departmentId); 
  }
  ...
  updateParameter(id: string, data: Partial<Parameter>) {
    ...
    if (itemIndex === -1) return null;
    this.parameters[itemIndex] = { ...this.parameters[itemIndex], ...data } as Parameter;
  ...
  // PERSONAS Queries
  getAllPersonas() { return this.personas; }
  getPersona(id: string) { return this.personas.find(item => item.id === id) || null; }
  getPersonasByModel(modelId: string) { 
    return this.personas.filter(item => item.modelId === modelId); 
  }
  getPersonasByDepartment(departmentId: string) { 
    return this.personas.filter(item => item.departmentId === departmentId); 
  }
  ...
  updatePersona(id: string, data: Partial<Persona>) {
    ...
    if (itemIndex === -1) return null;
    this.personas[itemIndex] = { ...this.personas[itemIndex], ...data } as Persona;
  ...
  // PROFILEDEPARTMENTS Queries
  getAllProfileDepartments() { return this.profileDepartments; }
  getProfileDepartmentsByProfile(profileId: string) { 
    return this.profileDepartments.filter(item => item.profileId === profileId); 
  }
  getProfileDepartmentsByDepartment(departmentId: string) { 
    return this.profileDepartments.filter(item => item.departmentId === departmentId); 
  }

  // PROFILEDEPARTMENTS Mutations
  createProfileDepartment(data: Partial<ProfileDepartment>) {
    const newItem = createMockProfileDepartment({ ...data, id: data.id ?? faker.string.uuid() });
    this.profileDepartments.push(newItem);
    return newItem;
  }

  // PROFILES Queries
  getAllProfiles() { return this.profiles; }
  getProfile(id: string) { return this.profiles.find(item => item.id === id) || null; }
  ...
  updateProfile(id: string, data: Partial<Profile>) {
    ...
    if (itemIndex === -1) return null;
    this.profiles[itemIndex] = { ...this.profiles[itemIndex], ...data } as Profile;
  ...
  // PROVIDERS Queries
  getAllProviders() { return this.providers; }
  getProvider(id: string) { return this.providers.find(item => item.id === id) || null; }
  getProvidersByDepartment(departmentId: string) { 
    return this.providers.filter(item => item.departmentId === departmentId); 
  }
  ...
  updateProvider(id: string, data: Partial<Provider>) {
    ...
    if (itemIndex === -1) return null;
    this.providers[itemIndex] = { ...this.providers[itemIndex], ...data } as Provider;
  ...
  // RUBRICS Queries
  getAllRubrics() { return this.rubrics; }
  getRubric(id: string) { return this.rubrics.find(item => item.id === id) || null; }
  getRubricsByDepartment(departmentId: string) { 
    return this.rubrics.filter(item => item.departmentId === departmentId); 
  }
  ...
  // SCENARIODOCUMENTS Queries
  getAllScenarioDocuments() { return this.scenarioDocuments; }
  getScenarioDocumentsByScenario(scenarioId: string) { 
    return this.scenarioDocuments.filter(item => item.scenarioId === scenarioId); 
  }
  getScenarioDocumentsByDocument(documentId: string) { 
    return this.scenarioDocuments.filter(item => item.documentId === documentId); 
  }

  // SCENARIODOCUMENTS Mutations
  createScenarioDocument(data: Partial<ScenarioDocument>) {
    const newItem = createMockScenarioDocument({ ...data, id: data.id ?? faker.string.uuid() });
    this.scenarioDocuments.push(newItem);
    return newItem;
  }

  // SCENARIOOBJECTIVES Queries
  getAllScenarioObjectives() { return this.scenarioObjectives; }
  getScenarioObjectivesByScenario(scenarioId: string) { 
    return this.scenarioObjectives.filter(item => item.scenarioId === scenarioId); 
  }

  // SCENARIOOBJECTIVES Mutations
  createScenarioObjective(data: Partial<ScenarioObjective>) {
    const newItem = createMockScenarioObjective({ ...data, id: data.id ?? faker.string.uuid() });
    this.scenarioObjectives.push(newItem);
    return newItem;
  }

  // SCENARIOPARAMETERITEMS Queries
  getAllScenarioParameterItems() { return this.scenarioParameterItems; }
  getScenarioParameterItemsByScenario(scenarioId: string) { 
    return this.scenarioParameterItems.filter(item => item.scenarioId === scenarioId); 
  }
  getScenarioParameterItemsByParameterItem(parameterItemId: string) { 
    return this.scenarioParameterItems.filter(item => item.parameterItemId === parameterItemId); 
  }

  // SCENARIOPARAMETERITEMS Mutations
  createScenarioParameterItem(data: Partial<ScenarioParameterItem>) {
    const newItem = createMockScenarioParameterItem({ ...data, id: data.id ?? faker.string.uuid() });
    this.scenarioParameterItems.push(newItem);
    return newItem;
  }
  ...
  getScenarioTreeByScenario(scenarioId: string) { 
    return this.scenarioTree.filter(item => item.parentId === scenarioId); 
  }
  getScenarioTreeByScenario(scenarioId: string) { 
    return this.scenarioTree.filter(item => item.childId === scenarioId); 
  }

  // SCENARIOTREE Mutations
  createScenarioTree(data: Partial<ScenarioTree>) {
    const newItem = createMockScenarioTree({ ...data, id: data.id ?? faker.string.uuid() });
    this.scenarioTree.push(newItem);
    return newItem;
  }
  ...
  updateScenario(id: string, data: Partial<Scenario>) {
    ...
    if (itemIndex === -1) return null;
    this.scenarios[itemIndex] = { ...this.scenarios[itemIndex], ...data } as Scenario;
  ...
  updateSimulationAttempt(id: string, data: Partial<SimulationAttempt>) {
    const itemIndex = this.simulationAttempts.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.simulationAttempts[itemIndex] = { ...this.simulationAttempts[itemIndex], ...data } as SimulationAttempt;
  ...
  updateSimulationChatFeedback(id: string, data: Partial<SimulationChatFeedback>) {
    const itemIndex = this.simulationChatFeedbacks.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.simulationChatFeedbacks[itemIndex] = { ...this.simulationChatFeedbacks[itemIndex], ...data } as SimulationChatFeedback;
  ...
  getAllSimulationChatGrades() { return this.simulationChatGrades; }
  getSimulationChatGrade(id: string) { return this.simulationChatGrades.find(item => item.id === id) || null; }
  getSimulationChatGradesByRubric(rubricId: string) { 
    return this.simulationChatGrades.filter(item => item.rubricId === rubricId); 
  }
  getSimulationChatGradesBySimulationChat(simulationChatId: string) { 
    return this.simulationChatGrades.filter(item => item.simulationChatId === simulationChatId); 
  }
  ...
  updateSimulationChatGrade(id: string, data: Partial<SimulationChatGrade>) {
    const itemIndex = this.simulationChatGrades.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.simulationChatGrades[itemIndex] = { ...this.simulationChatGrades[itemIndex], ...data } as SimulationChatGrade;
  ...
  updateSimulationChat(id: string, data: Partial<SimulationChat>) {
    ...
    if (itemIndex === -1) return null;
    this.simulationChats[itemIndex] = { ...this.simulationChats[itemIndex], ...data } as SimulationChat;
  ...
  getAllSimulationMessages() { return this.simulationMessages; }
  getSimulationMessage(id: string) { return this.simulationMessages.find(item => item.id === id) || null; }
  getSimulationMessagesBySimulationChat(simulationChatId: string) { 
    return this.simulationMessages.filter(item => item.chatId === simulationChatId); 
  }
  ...
  updateSimulationMessage(id: string, data: Partial<SimulationMessage>) {
    const itemIndex = this.simulationMessages.findIndex(item => item.id === id);
    if (itemIndex === -1) return null;
    this.simulationMessages[itemIndex] = { ...this.simulationMessages[itemIndex], ...data } as SimulationMessage;
  ...
  // SIMULATIONSCENARIOS Queries
  getAllSimulationScenarios() { return this.simulationScenarios; }
  getSimulationScenariosBySimulation(simulationId: string) { 
    return this.simulationScenarios.filter(item => item.simulationId === simulationId); 
  }
  getSimulationScenariosByScenario(scenarioId: string) { 
    return this.simulationScenarios.filter(item => item.scenarioId === scenarioId); 
  }

  // SIMULATIONSCENARIOS Mutations
  createSimulationScenario(data: Partial<SimulationScenario>) {
    const newItem = createMockSimulationScenario({ ...data, id: data.id ?? faker.string.uuid() });
    this.simulationScenarios.push(newItem);
    return newItem;
  }

  // SIMULATIONTAGDOCUMENTS Queries
  getAllSimulationTagDocuments() { return this.simulationTagDocuments; }
  getSimulationTagDocumentsByDocument(documentId: string) { 
    return this.simulationTagDocuments.filter(item => item.documentId === documentId); 
  }
  getSimulationTagDocumentsBySimulationTag(simulationTagId: string) { 
    return this.simulationTagDocuments.filter(item => item.simulationId === simulationTagId); 
  }

  // SIMULATIONTAGDOCUMENTS Mutations
  createSimulationTagDocument(data: Partial<SimulationTagDocument>) {
    const newItem = createMockSimulationTagDocument({ ...data, id: data.id ?? faker.string.uuid() });
    this.simulationTagDocuments.push(newItem);
    return newItem;
  }

  // SIMULATIONTAGPARAMETERITEMS Queries
  getAllSimulationTagParameterItems() { return this.simulationTagParameterItems; }
  getSimulationTagParameterItemsByParameterItem(parameterItemId: string) { 
    return this.simulationTagParameterItems.filter(item => item.parameterItemId === parameterItemId); 
  }
  getSimulationTagParameterItemsBySimulationTag(simulationTagId: string) { 
    return this.simulationTagParameterItems.filter(item => item.simulationId === simulationTagId); 
  }

  // SIMULATIONTAGPARAMETERITEMS Mutations
  createSimulationTagParameterItem(data: Partial<SimulationTagParameterItem>) {
    const newItem = createMockSimulationTagParameterItem({ ...data, id: data.id ?? faker.string.uuid() });
    this.simulationTagParameterItems.push(newItem);
    return newItem;
  }

  // SIMULATIONTAGS Queries
  getAllSimulationTags() { return this.simulationTags; }
  getSimulationTagsBySimulation(simulationId: string) { 
    return this.simulationTags.filter(item => item.simulationId === simulationId); 
  }

  // SIMULATIONTAGS Mutations
  createSimulationTag(data: Partial<SimulationTag>) {
    const newItem = createMockSimulationTag({ ...data, id: data.id ?? faker.string.uuid() });
    this.simulationTags.push(newItem);
    return newItem;
  }

  // SIMULATIONS Queries
  getAllSimulations() { return this.simulations; }
  getSimulation(id: string) { return this.simulations.find(item => item.id === id) || null; }
  getSimulationsByRubric(rubricId: string) { 
    return this.simulations.filter(item => item.rubricId === rubricId); 
  }
  getSimulationsByDepartment(departmentId: string) { 
    return this.simulations.filter(item => item.departmentId === departmentId); 
  }
  ...
  updateSimulation(id: string, data: Partial<Simulation>) {
    ...
    if (itemIndex === -1) return null;
    this.simulations[itemIndex] = { ...this.simulations[itemIndex], ...data } as Simulation;
  ...
  getAllStandardGroups() { return this.standardGroups; }
  getStandardGroup(id: string) { return this.standardGroups.find(item => item.id === id) || null; }
  ...
  updateStandardGroup(id: string, data: Partial<StandardGroup>) {
    ...
    if (itemIndex === -1) return null;
    this.standardGroups[itemIndex] = { ...this.standardGroups[itemIndex], ...data } as StandardGroup;
  ...
  // STANDARDS Queries
  getAllStandards() { return this.standards; }
  getStandard(id: string) { return this.standards.find(item => item.id === id) || null; }
  getStandardsByStandardGroup(standardGroupId: string) { 
    return this.standards.filter(item => item.standardGroupId === standardGroupId); 
  }
  ...
  updateStandard(id: string, data: Partial<Standard>) {
    ...
    if (itemIndex === -1) return null;
    this.standards[itemIndex] = { ...this.standards[itemIndex], ...data } as Standard;
