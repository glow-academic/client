...
export class MockDb {
  ...
  constructor() {
    ...
    this.simulationHints = JSON.parse(
      JSON.stringify(mockSchema.simulationHints)
    );
  ...
  getDepartment(id: string) {
    return this.departments.find((item) => item.id === id) || null;
  }

  // DEPARTMENTS Mutations
  createDepartment(data: Partial<Department>) {
    const newItem = createMockDepartment({
      ...data,
      id: data.id ?? faker.string.uuid(),
    });
  ...
  getAllSimulationHints() {
    return this.simulationHints;
  }
  getSimulationHint(id: string) {
    return this.simulationHints.find((item) => item.id === id) || null;
  }
  getSimulationHintsBySimulationMessage(simulationMessageId: string) {
    return this.simulationHints.filter(
      (item) => item.simulationMessageId === simulationMessageId
    );
  }
  ...
  createSimulationHint(data: Partial<SimulationHint>) {
    const newItem = createMockSimulationHint({
      ...data,
      id: data.id ?? faker.string.uuid(),
    });
  ...
  updateSimulationHint(id: string, data: Partial<SimulationHint>) {
    ...
    if (itemIndex === -1) return null;
    this.simulationHints[itemIndex] = {
      ...this.simulationHints[itemIndex],
      ...data,
    } as SimulationHint;
