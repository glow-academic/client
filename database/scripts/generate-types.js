...
function generateTypesContent(tables, enums) {
  ...
  const tableImports = tables
    .map((table) => `${table} as ${capitalize(table)}`)
    .join(",\n  ");
  ...
  const enumTypes = enums
    .map(
      (enumName) =>
        `type ${capitalize(
          enumName
        )} = (typeof ${enumName}.enumValues)[number];`
    )
    .join("\n");

  // Generate exports with singular names
  const tableExports = tables
    .map((table) => capitalize(singularize(table)))
    .join(",\n  ");
  const enumExports = enums
    .map((enumName) => capitalize(enumName))
    .join(",\n  ");
...
function generateTypes() {
  ...
  // Write the types file to client
  fs.writeFileSync(CLIENT_TYPES_PATH, typesContent);

  console.log(`\n✅ Generated ${CLIENT_TYPES_PATH}`);
  console.log(
