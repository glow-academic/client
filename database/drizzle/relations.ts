...
import { profiles, appFeedback, users, agents, scenarios, classes, assistantChats, assistantMessages, assistantToolCalls, dashboards, models, schedules, events, simulationAttempts, simulationChats, simulationMessages, simulations, rubrics, simulationChatGrades, standardGroups, standards, topics, simulationSketches, documents, simulationChatFeedbacks } from "./schema";
...
export const simulationChatsRelations = relations(simulationChats, ({one, many}) => ({
	...
	simulationSketches: many(simulationSketches),
}));
...
export const simulationSketchesRelations = relations(simulationSketches, ({one}) => ({
	simulationChat: one(simulationChats, {
		fields: [simulationSketches.chatId],
		references: [simulationChats.id]
	}),
}));
