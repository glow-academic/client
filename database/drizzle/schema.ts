...
export const profiles = pgTable(
  ...
  (table) => [
    uniqueIndex("profiles_alias_unique").using(
      "btree",
      table.alias.asc().nullsLast().op("text_ops")
    ),
  ]
);
...
export const profileRequestLimits = pgTable(
  ...
  (table) => [
    index("profile_request_limits_profile_id_idx").using(
      "btree",
    ...
    check(
      "profile_request_limits_requests_per_day_check",
      sql`requests_per_day > 0`
    ),
  ]
);

export const profileActivity = pgTable(
  "profile_activity",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index("profile_activity_created_at_idx").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamptz_ops")
    ),
    index("profile_activity_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops")
    ),
    index("profile_activity_profile_id_last_active_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("timestamptz_ops"),
      table.lastActive.asc().nullsLast().op("timestamptz_ops")
    ),
    foreignKey({
      columns: [table.profileId],
      foreignColumns: [profiles.id],
...
export const providerEndpoints = pgTable(
  ...
  (table) => [
    index("provider_endpoints_provider_id_idx").using(
      "btree",
...
export const standardGroups = pgTable(
  ...
  (table) => [
    index("standard_groups_id_rubric_idx").using(
      "btree",
      table.id.asc().nullsLast().op("uuid_ops"),
      table.rubricId.asc().nullsLast().op("uuid_ops")
    ),
    index("standard_groups_rubric_idx").using(
      "btree",
      table.id.asc().nullsLast().op("uuid_ops"),
      table.rubricId.asc().nullsLast().op("uuid_ops")
    ),
...
export const standards = pgTable(
  ...
  (table) => [
    index("standards_group_idx").using(
      "btree",
      table.standardGroupId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
...
export const appLogs = pgTable("app_logs", {
  ...
  correlationId: text("correlation_id")
    .default("default.correlation")
    .notNull(),
  actor: jsonb().default({ userId: null, profileId: null }).notNull(),
  subject: jsonb().default({ entityId: null, entityType: null }).notNull(),
  context: jsonb()
    .default({ route: null, function: null, component: null })
    .notNull(),
  error: jsonb()
    .default({ code: null, name: null, stack: null, message: null })
    .notNull(),
  createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
    .defaultNow()
    .notNull(),
...
export const prompts = pgTable(
  ...
  (table) => [
    index("prompts_created_at_idx").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamptz_ops")
    ),
  ]
...
export const debugInfo = pgTable(
  "debug_info",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
...
export const scenarios = pgTable(
  "scenarios",
  {
    ...
    inputGuardrailEnabled: boolean("input_guardrail_enabled")
      .default(false)
      .notNull(),
    outputGuardrailEnabled: boolean("output_guardrail_enabled")
      .default(false)
      .notNull(),
    copyPasteAllowed: boolean("copy_paste_allowed").default(false).notNull(),
  },
  (table) => [
    index("scenarios_id_active_idx").using(
      "btree",
...
export const scenarioProblemStatements = pgTable(
  ...
  (table) => [
    uniqueIndex("scenario_problem_statements_one_active_per_scenario")
      .using("btree", table.scenarioId.asc().nullsLast().op("uuid_ops"))
      .where(sql`active`),
    index("scenario_problem_statements_scenario_id_active_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("bool_ops"),
      table.active.asc().nullsLast().op("bool_ops")
    ),
    index("scenario_problem_statements_scenario_id_idx").using(
      "btree",
...
export const simulations = pgTable(
  ...
  (table) => [
    index("simulations_id_active_idx").using(
      "btree",
      table.id.asc().nullsLast().op("bool_ops"),
      table.active.asc().nullsLast().op("bool_ops")
    ),
...
export const simulationTimeLimits = pgTable(
  ...
  (table) => [
    index("simulation_time_limits_simulation_id_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    check(
      "simulation_time_limits_time_limit_seconds_check",
      sql`time_limit_seconds > 0`
    ),
  ]
);
...
export const simulationAttempts = pgTable(
  ...
  (table) => [
    index("simulation_attempts_archived_idx").using(
      "btree",
...
export const simulationChats = pgTable(
  ...
  (table) => [
    index("simulation_chats_id_created_idx").using(
      "btree",
...
export const simulationMessages = pgTable(
  ...
  (table) => [
    index("simulation_messages_chat_created_type_idx").using(
      "btree",
      table.chatId.asc().nullsLast().op("enum_ops"),
      table.createdAt.asc().nullsLast().op("timestamptz_ops"),
...
export const simulationChatGrades = pgTable(
  ...
  (table) => [
    index("scg_chat_created_idx").using(
      "btree",
      table.simulationChatId.asc().nullsLast().op("timestamptz_ops"),
      table.createdAt.desc().nullsFirst().op("timestamptz_ops")
    ),
    index("scg_chat_rubric_created_idx").using(
      "btree",
      table.simulationChatId.asc().nullsLast().op("uuid_ops"),
      table.rubricId.asc().nullsLast().op("timestamptz_ops"),
      table.createdAt.desc().nullsFirst().op("uuid_ops")
    ),
    index("simulation_chat_grades_latest_idx").using(
      "btree",
      table.simulationChatId.asc().nullsLast().op("uuid_ops"),
      table.createdAt.desc().nullsFirst().op("uuid_ops")
    ),
...
export const simulationChatFeedbacks = pgTable(
  ...
  (table) => [
    index("scf_grade_idx").using(
      "btree",
      table.simulationChatGradeId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
...
export const scenarioObjectives = pgTable(
  "scenario_objectives",
  {
    scenarioId: uuid("scenario_id").notNull(),
    idx: integer().notNull(),
  ...
  (table) => [
    index("scenario_objectives_scenario_id_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.scenarioId, table.idx],
      name: "scenario_objectives_pkey",
    }),
  ]
);

export const attemptChats = pgTable(
  "attempt_chats",
  {
  ...
  (table) => [
    index("attempt_chats_attempt_id_chat_id_idx").using(
      "btree",
      table.attemptId.asc().nullsLast().op("uuid_ops"),
      table.chatId.asc().nullsLast().op("uuid_ops")
    ),
    index("attempt_chats_attempt_id_idx").using(
      "btree",
      table.attemptId.asc().nullsLast().op("uuid_ops")
    ),
    index("attempt_chats_chat_id_idx").using(
      "btree",
      table.chatId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
      ...
      name: "attempt_chats_attempt_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.chatId],
      foreignColumns: [simulationChats.id],
      name: "attempt_chats_chat_id_fkey",
    }).onDelete("cascade"),
    primaryKey({
      columns: [table.attemptId, table.chatId],
      name: "attempt_chats_pkey",
    }),
  ]
);

export const simulationHints = pgTable(
  "simulation_hints",
  {
  ...
  (table) => [
    index("simulation_hints_simulation_message_id_idx").using(
      "btree",
      table.simulationMessageId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.simulationMessageId, table.idx],
      name: "simulation_hints_pkey",
    }),
  ]
);
...
export const documentDepartments = pgTable(
  ...
  (table) => [
    index("document_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("document_departments_document_id_idx").using(
      "btree",
    ...
    primaryKey({
      ...
      name: "document_departments_pkey",
    }),
  ]
);

export const rubricDepartments = pgTable(
  "rubric_departments",
  {
    rubricId: uuid("rubric_id").notNull(),
    departmentId: uuid("department_id").notNull(),
  ...
  (table) => [
    index("rubric_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("rubric_departments_rubric_id_idx").using(
      "btree",
      table.rubricId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.rubricId, table.departmentId],
      name: "rubric_departments_pkey",
    }),
  ]
);

export const promptDepartments = pgTable(
  "prompt_departments",
  {
  ...
  (table) => [
    index("prompt_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("prompt_departments_prompt_id_idx").using(
      "btree",
    ...
    primaryKey({
      columns: [table.promptId, table.departmentId],
      name: "prompt_departments_pkey",
    }),
  ]
);

export const personaDepartments = pgTable(
  "persona_departments",
  {
  ...
  (table) => [
    index("persona_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("persona_departments_persona_id_idx").using(
      "btree",
      table.personaId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.personaId, table.departmentId],
      name: "persona_departments_pkey",
    }),
  ]
);

export const personaPrompts = pgTable(
  "persona_prompts",
  {
    personaId: uuid("persona_id").notNull(),
    promptId: uuid("prompt_id").notNull(),
  ...
  (table) => [
    index("persona_prompts_persona_id_active_idx").using(
      "btree",
      table.personaId.asc().nullsLast().op("bool_ops"),
      table.active.asc().nullsLast().op("uuid_ops")
    ),
    index("persona_prompts_persona_id_idx").using(
      "btree",
      table.personaId.asc().nullsLast().op("uuid_ops")
    ),
    index("persona_prompts_prompt_id_idx").using(
      "btree",
      table.promptId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.personaId, table.promptId],
      name: "persona_prompts_pkey",
    }),
  ]
);

export const modelRunModels = pgTable(
  "model_run_models",
  {
    modelRunId: uuid("model_run_id").notNull(),
    modelId: uuid("model_id").notNull(),
  ...
  (table) => [
    index("model_run_models_model_id_idx").using(
      "btree",
      table.modelId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("one_model_per_run").using(
      "btree",
      table.modelRunId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
      columns: [table.modelRunId],
      foreignColumns: [modelRuns.id],
    ...
    primaryKey({
      columns: [table.modelRunId, table.modelId],
      name: "model_run_models_pkey",
    }),
  ]
);

export const modelRunPersonas = pgTable(
  "model_run_personas",
  {
    modelRunId: uuid("model_run_id").notNull(),
    personaId: uuid("persona_id").notNull(),
  ...
  (table) => [
    index("model_run_personas_persona_id_idx").using(
      "btree",
      table.personaId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("one_persona_per_run").using(
      "btree",
      table.modelRunId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.modelRunId, table.personaId],
      name: "model_run_personas_pkey",
    }),
  ]
);

export const modelRunAgents = pgTable(
  "model_run_agents",
  {
    modelRunId: uuid("model_run_id").notNull(),
  ...
  (table) => [
    index("model_run_agents_agent_id_idx").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("one_agent_per_run").using(
      "btree",
      table.modelRunId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.modelRunId, table.agentId],
      name: "model_run_agents_pkey",
    }),
  ]
);

export const modelRunProfiles = pgTable(
  "model_run_profiles",
  {
    modelRunId: uuid("model_run_id").notNull(),
    profileId: uuid("profile_id").notNull(),
  ...
  (table) => [
    index("model_run_profiles_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("one_profile_per_run").using(
      "btree",
      table.modelRunId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.modelRunId, table.profileId],
      name: "model_run_profiles_pkey",
    }),
  ]
);
...
export const agentDepartments = pgTable(
  ...
  (table) => [
    index("agent_departments_agent_id_idx").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops")
    ),
    index("agent_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.agentId, table.departmentId],
      name: "agent_departments_pkey",
    }),
  ]
);
...
export const agentPrompts = pgTable(
  ...
  (table) => [
    index("agent_prompts_agent_id_active_idx").using(
      "btree",
      table.agentId.asc().nullsLast().op("bool_ops"),
      table.active.asc().nullsLast().op("uuid_ops")
    ),
    index("agent_prompts_agent_id_idx").using(
      "btree",
      table.agentId.asc().nullsLast().op("uuid_ops")
    ),
    index("agent_prompts_prompt_id_idx").using(
      "btree",
      table.promptId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.agentId, table.promptId],
      name: "agent_prompts_pkey",
    }),
  ]
);

export const parameterItemDepartments = pgTable(
  "parameter_item_departments",
  {
  ...
  (table) => [
    index("parameter_item_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("parameter_item_departments_parameter_item_id_idx").using(
      "btree",
      table.parameterItemId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.parameterItemId, table.departmentId],
      name: "parameter_item_departments_pkey",
    }),
  ]
);

export const scenarioDepartments = pgTable(
  ...
  (table) => [
    index("scenario_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("scenario_departments_scenario_id_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.scenarioId, table.departmentId],
      name: "scenario_departments_pkey",
    }),
  ]
);
...
export const scenarioPersonas = pgTable(
  ...
  (table) => [
    uniqueIndex("scenario_personas_one_active_per_scenario")
      .using("btree", table.scenarioId.asc().nullsLast().op("uuid_ops"))
      .where(sql`active`),
    index("scenario_personas_persona_id_idx").using(
      "btree",
      table.personaId.asc().nullsLast().op("uuid_ops")
    ),
    index("scenario_personas_scenario_active_idx")
      .using(
        "btree",
        table.scenarioId.asc().nullsLast().op("uuid_ops"),
        table.personaId.asc().nullsLast().op("uuid_ops")
    ...
    index("scenario_personas_scenario_id_active_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops"),
      table.active.asc().nullsLast().op("uuid_ops")
    ),
    ...
    primaryKey({
      columns: [table.scenarioId, table.personaId],
      name: "scenario_personas_pkey",
    }),
  ]
);

export const scenarioParameterItems = pgTable(
  ...
  (table) => [
    index("scenario_parameter_items_parameter_item_id_idx").using(
      "btree",
      table.parameterItemId.asc().nullsLast().op("uuid_ops")
    ),
    index("scenario_parameter_items_scenario_id_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.scenarioId, table.parameterItemId],
      name: "scenario_parameter_items_pkey",
    }),
  ]
);
...
export const scenarioDocuments = pgTable(
  ...
  (table) => [
    index("scenario_documents_document_id_idx").using(
      "btree",
      table.documentId.asc().nullsLast().op("uuid_ops")
    ),
    index("scenario_documents_scenario_id_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.scenarioId, table.documentId],
      name: "scenario_documents_pkey",
    }),
  ]
);

export const documentParameterItems = pgTable(
  "document_parameter_items",
  {
    documentId: uuid("document_id").notNull(),
    parameterItemId: uuid("parameter_item_id").notNull(),
    active: boolean().default(true).notNull(),
  ...
  (table) => [
    index("document_parameter_items_document_id_idx").using(
      "btree",
      table.documentId.asc().nullsLast().op("uuid_ops")
    ),
    index("document_parameter_items_parameter_item_id_idx").using(
      "btree",
      table.parameterItemId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.documentId, table.parameterItemId],
      name: "document_parameter_items_pkey",
    }),
  ]
);

export const scenarioTree = pgTable(
  "scenario_tree",
  {
  ...
  (table) => [
    index("scenario_tree_child_id_idx").using(
      "btree",
      table.childId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("scenario_tree_one_parent_per_child").using(
      "btree",
      table.childId.asc().nullsLast().op("uuid_ops")
    ),
    index("scenario_tree_parent_id_idx").using(
      "btree",
    ...
    primaryKey({
      ...
      name: "scenario_tree_pkey",
    }),
  ]
);

export const simulationDepartments = pgTable(
  ...
  (table) => [
    index("simulation_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    index("simulation_departments_simulation_id_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.simulationId, table.departmentId],
      name: "simulation_departments_pkey",
    }),
  ]
);

export const attemptProfiles = pgTable(
  "attempt_profiles",
  {
    attemptId: uuid("attempt_id").notNull(),
    profileId: uuid("profile_id").notNull(),
  ...
  (table) => [
    index("attempt_profiles_attempt_active_idx")
      .using(
        "btree",
        table.attemptId.asc().nullsLast().op("uuid_ops"),
        table.profileId.asc().nullsLast().op("uuid_ops")
      ...
      .where(sql`(active = true)`),
    index("attempt_profiles_attempt_id_active_idx").using(
      "btree",
      table.attemptId.asc().nullsLast().op("bool_ops"),
      table.active.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("attempt_profiles_one_active_per_attempt")
      .using("btree", table.attemptId.asc().nullsLast().op("uuid_ops"))
      .where(sql`active`),
    index("attempt_profiles_profile_active_idx")
      .using(
        "btree",
        table.profileId.asc().nullsLast().op("uuid_ops"),
        table.attemptId.asc().nullsLast().op("uuid_ops")
      ...
      .where(sql`(active = true)`),
    index("attempt_profiles_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.attemptId, table.profileId],
      name: "attempt_profiles_pkey",
    }),
  ]
);

export const cohortProfiles = pgTable(
  "cohort_profiles",
  {
  ...
  (table) => [
    index("cohort_profiles_cohort_id_idx").using(
      "btree",
      table.cohortId.asc().nullsLast().op("uuid_ops")
    ),
    index("cohort_profiles_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.cohortId, table.profileId],
      name: "cohort_profiles_pkey",
    }),
  ]
);

export const cohortSimulations = pgTable(
  "cohort_simulations",
  {
    cohortId: uuid("cohort_id").notNull(),
    simulationId: uuid("simulation_id").notNull(),
  ...
  (table) => [
    index("cohort_simulations_cohort_id_idx").using(
      "btree",
      table.cohortId.asc().nullsLast().op("uuid_ops")
    ),
    index("cohort_simulations_simulation_id_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.cohortId, table.simulationId],
      name: "cohort_simulations_pkey",
    }),
  ]
);

export const cohortDepartments = pgTable(
  "cohort_departments",
  {
    cohortId: uuid("cohort_id").notNull(),
    departmentId: uuid("department_id").notNull(),
  ...
  (table) => [
    index("cohort_departments_cohort_id_idx").using(
      "btree",
      table.cohortId.asc().nullsLast().op("uuid_ops")
    ),
    index("cohort_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.cohortId, table.departmentId],
      name: "cohort_departments_pkey",
    }),
  ]
);

export const profileDepartments = pgTable(
  "profile_departments",
  {
    profileId: uuid("profile_id").notNull(),
    departmentId: uuid("department_id").notNull(),
  ...
  (table) => [
    index("profile_departments_department_id_idx").using(
      "btree",
      table.departmentId.asc().nullsLast().op("uuid_ops")
    ),
    uniqueIndex("profile_departments_one_primary_per_profile")
      .using("btree", table.profileId.asc().nullsLast().op("uuid_ops"))
      .where(sql`is_primary`),
    index("profile_departments_profile_id_is_primary_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("bool_ops"),
    ...
    primaryKey({
      columns: [table.profileId, table.departmentId],
      name: "profile_departments_pkey",
    }),
  ]
);

export const appFeedbackProfiles = pgTable(
  "app_feedback_profiles",
  {
  ...
  (table) => [
    index("app_feedback_profiles_app_feedback_id_idx").using(
      "btree",
      table.appFeedbackId.asc().nullsLast().op("int4_ops")
    ),
    index("app_feedback_profiles_profile_id_idx").using(
      "btree",
      table.profileId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.appFeedbackId, table.profileId, table.role],
      name: "app_feedback_profiles_pkey",
    }),
  ]
);
...
export const simulationScenarios = pgTable(
  ...
  (table) => [
    uniqueIndex("simulation_scenarios_position_uniq").using(
      "btree",
      table.simulationId.asc().nullsLast().op("int4_ops"),
      table.position.asc().nullsLast().op("int4_ops")
    ),
    index("simulation_scenarios_scenario_id_idx").using(
      "btree",
      table.scenarioId.asc().nullsLast().op("uuid_ops")
    ),
    index("simulation_scenarios_simulation_id_idx").using(
      "btree",
      table.simulationId.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
    ...
    primaryKey({
      columns: [table.simulationId, table.scenarioId],
      name: "simulation_scenarios_pkey",
    }),
  ]
);
export const analytics = pgMaterializedView("analytics", {
  chatId: uuid("chat_id"),
  ...
  chatCreatedAt: timestamp("chat_created_at", {
    withTimezone: true,
    mode: "string",
  }),
