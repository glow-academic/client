...
{% set draft = entries.draft_tool if entries and entries.draft_tool else None %}
...
{% endfor %}
{% endif %}
{% if "args" in all_gen_types and resources.args and resources.args|length > 0 %}
Args:
{% for item in resources.args %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "arg_positions" in all_gen_types and resources.arg_positions and resources.arg_positions|length > 0 %}
Arg Positions:
{% for item in resources.arg_positions %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "args_outputs" in all_gen_types and resources.args_outputs and resources.args_outputs|length > 0 %}
Args Outputs:
{% for item in resources.args_outputs %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "bindings" in all_gen_types and resources.bindings and resources.bindings|length > 0 %}
Bindings:
{% for item in resources.bindings %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "domains" in all_gen_types and resources.domains and resources.domains|length > 0 %}
Domains:
{% for item in resources.domains %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
