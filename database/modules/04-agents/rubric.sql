...
{% set draft = entries.draft_rubric if entries and entries.draft_rubric else None %}
...
{% endfor %}
{% endif %}
{% if "points" in all_gen_types and resources.points and resources.points|length > 0 %}
Points:
{% for item in resources.points %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "standard_groups" in all_gen_types and resources.standard_groups and resources.standard_groups|length > 0 %}
Standard Groups:
{% for item in resources.standard_groups %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "standards" in all_gen_types and resources.standards and resources.standards|length > 0 %}
Standards:
{% for item in resources.standards %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
