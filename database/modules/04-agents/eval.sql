...
{% set draft = entries.draft_eval if entries and entries.draft_eval else None %}
...
{% endfor %}
{% endif %}
{% if "run_positions" in all_gen_types and resources.run_positions and resources.run_positions|length > 0 %}
Run Positions:
{% for item in resources.run_positions %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "runs_rubrics" in all_gen_types and resources.runs_rubrics and resources.runs_rubrics|length > 0 %}
Runs Rubrics:
{% for item in resources.runs_rubrics %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
...
{% endfor %}
{% endif %}
{% if "group_positions" in all_gen_types and resources.group_positions and resources.group_positions|length > 0 %}
Group Positions:
{% for item in resources.group_positions %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
{% if "groups_rubrics" in all_gen_types and resources.groups_rubrics and resources.groups_rubrics|length > 0 %}
Groups Rubrics:
{% for item in resources.groups_rubrics %}
- id: {{ item.id }} | {{ item.name }}
{% endfor %}
{% endif %}
