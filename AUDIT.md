...
| File | Purpose |
|------|---------|
| `server/app/infra/activity/audit.py` | `audit_activity()` dependency + `audit_set()` helper |
| `server/app/infra/activity/logger.py` | HTTP activity logger (called by middleware) |
| `server/app/infra/activity/websocket_logger.py` | WebSocket activity logger |
| `server/app/infra/activity/insert.py` | DB insert for HTTP audits |
| `server/app/infra/activity/insert_websocket.py` | DB insert for WebSocket audits |
...
All domain generation handlers (agent, simulation, persona, etc.) emit to a single internal `generate_artifact` event in `server/app/routes/v5/socket/artifacts/generate.py`. Audit logging is already added at this chokepoint — individual generation handlers do NOT need their own audit calls.
