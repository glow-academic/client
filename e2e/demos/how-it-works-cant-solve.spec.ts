...
// # coverage: full-page tour of a STRUGGLING graded attempt review — the
// lowest-scoring attempt that still carries a real conversation. A
// completed+graded attempt opens on its RUBRIC SCORECARD by default (the server
// sets show_results, so AttemptChat auto-swaps to RubricView). That annotated,
// low-scoring grade IS the headline: it shows where the learner's vague input
// failed to move the rubric. We tour the scorecard, flip to the conversation
// transcript via the rubric view-toggle (and back) to read the stalled dialog,
// then exercise the Objectives / Documents header toggles. Read-only
...
//
// Resolution gotcha (why this isn't a one-line attemptDemo): the extreme-scored
...
// always-present header `timer` pill, and resolve the worst-scored attempt that
 ...
 *  transcript (entries.attempt_message). Probes the top viewable attempts (by
 *  the requested extreme) via /attempt/get; falls back to the extreme-scored
 *  one so we still record a substantive scorecard even if none carry messages. */
...
test.describe("demo: how-it-works cant-solve", () => {
  test("tour a struggling, low-scoring attempt and its scorecard", async ({
    page,
    demo,
    request,
  }) => {
    ...
    const id = await resolveRichAttempt(request, "worst");
    ...
    // active. Everything after this is guarded. (A message-bearing attempt also
    // mounts the transcript container, so anchoring on `timer` alone avoids a
    ...
    // For a struggling attempt this surfaces the not-passed / low-score
    // criteria and the feedback explaining where the input fell short.
    await scrollToText(page, /not passed|incomplete|failed|score|standard|rubric/i).catch(
      () => undefined,
    );
    if (await timer.isVisible().catch(() => false)) {
      await timer.hover().catch(() => undefined); // reveals the score tooltip
      await demoPause();
    }
    for (let i = 0; i < 2; i++) {
    ...
    await scrollToText(page, /feedback|criteria|points|standard|improve/i).catch(() => undefined);
    await pauseForDemo(900);

    // ---- 2. Flip to the conversation transcript (rubric Table toggle) ----
    // The stalled dialog: vague learner input that never advances the persona.
    await toggleByIcon(
      page,
      "lucide-table",
      async () => {
        ...
        await scrollToText(page, /assistant|reply|message|persona|feedback/i).catch(
          () => undefined,
        );
      },
      demoPause,
    ...
    await scrollToText(page, /not passed|incomplete|score|standard/i).catch(() => undefined);
    await pauseForDemo(900);

    await saveDemoVideo(page, "how-it-works-cant-solve");
  });
});
