// # coverage: DRAFT-WORKFLOW tour of the eval NEW form (/platform/evals/new) —
// a single-focus draft clip (distinct from evals-create, which fills the form).
// After the form settles, this clip CENTERS on the SaveToolbar draft workflow:
// it opens the "Drafts picker" trigger to reveal the saved-drafts list, types
// into the draft-search (pressSequentially so the typing shows) to filter the
// list then clears it, hovers a draft entry to surface it, and reveals the
// autosave toggle (draft-autosave-toggle) WITHOUT flipping it — then Escape.
// Then ONE smooth top->bottom scroll through the eval form's two step sections
// (Basics → Models), holding ~1s on each — no jumping back and forth. Every beat
// past the one page-ready assertion (artifact-form) is guarded
// (isVisible().catch / scrollToText no-ops on an absent target), so an empty
// draft list (picker still shows) or an unmounted section is a clean no-op.
// Strictly NON-DESTRUCTIVE: nothing is submitted (the "Create Eval" button is
// never clicked), no draft is switched into (Escape only), the autosave toggle
// is only revealed, never flipped, and no model is ever selected.
...
// The step sections the eval form renders, top->bottom. Each entry is
...
test.describe("demo: evals draft", () => {
  test("tours the eval draft workflow in the SaveToolbar", async ({ page }) => {
    ...
    if (await draftTrigger.isVisible().catch(() => false)) {
      ...
      if (await draftSearch.isVisible().catch(() => false)) {
        ...
        await draftSearch.pressSequentially("eval", { delay: 55 }).catch(() => undefined);
        await pauseForDemo(1_300);
    ...
    // BEAT 5 — ONE smooth top->bottom scroll through the eval form's step
    // sections (Basics → Models), holding ~1s on each so its title reads — no
    // jumping back and forth. Each scroll is a guarded no-op for a section that
    // isn't mounted.
    ...
    // End held back at the top of the form (Basics) so the clip closes on the
    // draft's substantive content.
