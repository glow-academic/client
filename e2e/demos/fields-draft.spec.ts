// # coverage: DRAFT-WORKFLOW tour of the field NEW form (/management/fields/new) —
// distinct from fields-create (which centers the form fields). After the form
...
// scroll through the form's step sections (Basic Information → Conditional
// Parameters) gives context. Every beat past the one page-ready assertion
// (artifact-form) is guarded (isVisible().catch / scrollToText no-ops on an
// absent target), so an empty draft list (picker still shows) or an unmounted
// section is a clean no-op. Strictly NON-DESTRUCTIVE: nothing is submitted (the
// "Create Field" button is never clicked), no draft is switched into (Escape
// only), and the autosave toggle is only revealed, never flipped.
...
test.describe("demo: fields draft", () => {
  test("tours the field draft workflow in the SaveToolbar", async ({ page }) => {
    ...
    // BEAT 5 — brief context scroll top->bottom through the field form's step
