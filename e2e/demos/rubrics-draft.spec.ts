// # coverage: DRAFT-WORKFLOW tour of the rubric NEW form (/platform/rubrics/new) —
// distinct from rubrics-create (which centers the form fields). After the form
...
// scroll through the form's step sections (Basic Information → Standard Groups →
// Standards) gives context. Every beat past the one page-ready assertion
...
// "Create Rubric" button is never clicked), no draft is switched into (Escape
...
// The step sections the rubric form renders, top->bottom. Each entry is
...
const STEP_SECTIONS: Array<{ stepId: string; heading: RegExp }> = [
  ...
  { stepId: "standard_groups", heading: /standard groups/i },
  { stepId: "standards", heading: /^standards$/i },
];

test.describe("demo: rubrics draft", () => {
  test("tours the rubric draft workflow in the SaveToolbar", async ({ page }) => {
    ...
    // BEAT 5 — brief context scroll top->bottom through the rubric form's step
