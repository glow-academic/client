...
// # coverage: full-page tour of the scenarios library — scroll the loaded grid
// top->bottom, surface a card's per-row affordances (Edit/View/Duplicate), then
// drive every list control on camera (search -> Persona/Simulation/Department
...
test.describe("demo: scenarios overview", () => {
  test("browse the scenarios library", async ({ page, demo, registry, request, runId }) => {
    await overviewDemo({ page, demo, registry, request, runId }, "scenario", async (p) => {
      ...
      const grid = p.getByTestId("scenarios-grid").first();
      if (await grid.isVisible().catch(() => false)) {
        await grid.scrollIntoViewIfNeeded().catch(() => undefined);
        await pauseForDemo(900);
      }

      const cards = p.getByTestId("scenario-card");
      ...
      await hoverLocatorIfVisible(p.getByTestId("btn-edit-scenario"));
      await hoverLocatorIfVisible(p.getByTestId("btn-view-scenario"));
      await hoverLocatorIfVisible(p.getByTestId("btn-duplicate-scenario"));
      ...
      // skipped cleanly: search (type -> settle -> clear), the Persona/
      // Simulation/Department faceted filters (open -> pick -> Reset), the card
      // View (column-visibility) toggle, pagination (Next -> Prev), and the
      // page-size selector. All read-only / reversible.
