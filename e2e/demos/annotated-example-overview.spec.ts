...
// # coverage: full-page tour of a real graded /attempt/{id} review. A
// completed+graded attempt opens on its RUBRIC SCORECARD by default (the server
// sets show_results so AttemptChat auto-swaps to RubricView). We tour the
// scorecard (the annotated grade), flip to the conversation transcript via the
// rubric view-toggle and scroll it, then exercise the Objectives + Documents
// header toggles, ending back on the scorecard. Read-only throughout: every
// header control is a reversible view-mode swap, nothing is sent/confirmed, the
// chat input is never used.
//
// Resolution gotcha (mirrors annotated-example-excellent): the top-scored
...
test.describe("demo: annotated-example overview", () => {
  test("tour a graded attempt: scorecard, transcript, and view toggles", async ({
    page,
    demo,
    request,
  }) => {
    ...
    // ONE hard page-ready assertion: header `timer` (present for any view) OR
    // the transcript container. Everything after is guarded.
    // Header `timer` pill renders for any attempt view; anchor on it alone (a
    // message-bearing attempt also mounts the transcript container, so an
    // `.or()` over both would multi-match under strict mode).
    ...
    await saveDemoVideo(page, "annotated-example-overview");
  });
});
