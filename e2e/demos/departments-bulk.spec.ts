...
// coverage: tour the WHOLE departments management surface top->bottom — scroll
// the populated card grid, type into the name search and clear it, open each
// faceted picker (Profile / Settings / Logins) and the View-columns menu then
...
// never click the confirm button (btn-confirm-bulk-delete / btn-confirm-delete).
...
async function selectCard(page: Page, index: number): Promise<boolean> {
  const checkbox = page
    .getByRole("checkbox", { name: /^Select department /i })
    .nth(index);
...
test.describe("demo: departments bulk", () => {
  test("tour the departments grid, filters, and bulk-select", async ({ page }) => {
    test.setTimeout(120_000);

    // Open on the clean, unselected departments card grid.
    await page.goto("/platform/departments");
    await expectAuthenticated(page);

    // ONE real page-ready assertion: the card grid is the feature container.
    await expect(page.getByTestId("departments-grid")).toBeVisible({ timeout: 30_000 });
    ...
    const search = page.getByTestId("departments-search");
    ...
    await peekPicker(page, /^Profile$/);
    await peekPicker(page, /^Settings$/);
    await peekPicker(page, /^Logins$/);
    ...
    // Departments is a CARD GRID that often fits on one page, so the pagination
    ...
    // isEnabled() on a never-attached locator waits the full 120s test timeout
    // (actionTimeout defaults to 0), which is what hung this demo.
    ...
    const bulkDelete = page
      .getByTestId("departments-toolbar")
    ...
    if (haveBulkAction && (await bulkDelete.isEnabled().catch(() => false))) {
      ...
      const dialog = page.getByTestId("dialog-bulk-delete-department");
    ...
    await expect(page.getByTestId("departments-grid")).toBeVisible({ timeout: 15_000 });
    await pauseForDemo(1_300);
