...
// coverage: tour the WHOLE parameters management surface top->bottom — scroll the
// populated card grid, type into the name search and clear it, open each faceted
// picker (Scenario / Field / Department) and the View-columns menu then dismiss
// with Escape, walk pagination next/prev, then bulk-select two cards to swap in
// the selection toolbar, OPEN-and-CANCEL the bulk-EDIT dialog AND the bulk-DELETE
// confirm, and Unselect All so the clip ends on the clean, populated grid. Every
// beat past the single page-ready assertion is guarded so a thin/empty library
// just skips it. HARD RULE: non-destructive — we open the edit dialog + delete
// confirm then back out; we never click the apply/confirm buttons
// (btn-apply-bulk-edit / btn-confirm-bulk-delete / btn-confirm-delete).
...
async function selectCard(page: Page, index: number): Promise<boolean> {
  const checkbox = page
    .getByRole("checkbox", { name: /^Select parameter /i })
    .nth(index);
...
test.describe("demo: parameters bulk", () => {
  test("tour the parameters grid, filters, and bulk-select", async ({ page }) => {
    test.setTimeout(120_000);

    // Open on the clean, unselected parameters card grid.
    await page.goto("/management/parameters");
    await expectAuthenticated(page);

    // ONE real page-ready assertion: the card grid is the feature container.
    await expect(page.getByTestId("parameters-grid")).toBeVisible({ timeout: 30_000 });
    ...
    const search = page.getByTestId("parameters-search");
    ...
    await peekPicker(page, /^Scenario$/);
    await peekPicker(page, /^Field$/);
    ...
    // Parameters is a CARD GRID that may fit on one page, so the pagination
    ...
    // (actionTimeout defaults to 0), which would hang the demo.
    ...
    const haveSelection = firstSelected && secondSelected;

    if (haveSelection) {
      const toolbar = page.getByTestId("parameters-toolbar");

      // BEAT 8: OPEN the bulk-EDIT dialog to show the affordance — then back out.
      // NON-DESTRUCTIVE: we never click the apply button.
      const bulkEdit = toolbar.getByRole("button", { name: /^Edit \d+/ }).first();
      if (
        (await bulkEdit.isVisible().catch(() => false)) &&
        (await bulkEdit.isEnabled().catch(() => false))
      ) {
        await bulkEdit.scrollIntoViewIfNeeded().catch(() => undefined);
        await pauseForDemo(900);
        await bulkEdit.click().catch(() => undefined);
        const editDialog = page.getByTestId("dialog-bulk-edit");
        if (await editDialog.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      // BEAT 9: reveal the destructive bulk action. The selection toolbar shows
      // a "Delete N of M" (or "Delete N matching") button once cards are held.
      const bulkDelete = toolbar.getByRole("button", { name: /^Delete \d+/ }).first();
      if (
        ...
        // BEAT 10: OPEN the bulk-delete confirm to show the affordance — then
        // back out. NON-DESTRUCTIVE: we never click the confirm button.
        await bulkDelete.click().catch(() => undefined);
        const dialog = page.getByTestId("dialog-bulk-delete-parameter");
      ...
      // BEAT 11: clear the selection so the clip ends on a clean, populated grid
      // — never a half-selected or open-dialog state.
    ...
    await expect(page.getByTestId("parameters-grid")).toBeVisible({ timeout: 15_000 });
    await pauseForDemo(1_300);
