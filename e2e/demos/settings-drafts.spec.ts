// # coverage: DRAFT-WORKFLOW tour of the settings NEW form (/settings/new) —
// distinct from settings-create (which centers the form fields). After the form
// settles, this clip CENTERS on the SaveToolbar draft workflow: it opens the
// "Drafts picker" trigger to reveal the saved-drafts list, types into the
// draft-search (pressSequentially so the typing shows) to filter the list then
// clears it, hovers a draft entry to surface it, and reveals the autosave toggle
// (draft-autosave-toggle) WITHOUT flipping it — then Escape. A brief top->bottom
// scroll through the settings form's step sections (Basic → Color → Logins →
// Systems → MCP → Thresholds → Providers → Auths) gives context. Every beat past
// the one page-ready assertion (artifact-form) is guarded (isVisible().catch /
// scrollToText no-ops on an absent target), so an empty draft list (picker still
// shows) or an unmounted section is a clean no-op. Strictly NON-DESTRUCTIVE:
// nothing is submitted (the "Create Setting" button is never clicked), no draft
// is switched into (Escape only), and the autosave toggle is only revealed,
// never flipped.
...
// The step sections the settings form renders, top->bottom. Each entry is
...
const STEP_SECTIONS: Array<{ stepId: string; heading: RegExp }> = [
  ...
  { stepId: "color", heading: /^color$/i },
  { stepId: "logins", heading: /^logins$/i },
  { stepId: "systems", heading: /^systems$/i },
  { stepId: "mcp", heading: /^mcp$/i },
  { stepId: "thresholds", heading: /^thresholds$/i },
  { stepId: "provider", heading: /^providers$/i },
  { stepId: "auth", heading: /^auths$/i },
];

test.describe("demo: settings drafts", () => {
  test("tours the settings draft workflow in the SaveToolbar", async ({ page }) => {
    ...
    if (await draftTrigger.isVisible().catch(() => false)) {
      ...
      if (await draftSearch.isVisible().catch(() => false)) {
        ...
        await draftSearch.pressSequentially("config", { delay: 55 }).catch(() => undefined);
        await pauseForDemo(1_300);
    ...
    // BEAT 5 — brief context scroll top->bottom through the settings form's step
    ...
    // End held back at the top of the form (Basic) so the clip closes on the
    // setting draft's substantive content.
