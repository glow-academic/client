...
// coverage: full-page tour of the providers library (LLM-provider card grid)
// top->bottom — orient on the management toolbar, scroll the populated cards,
// type into the "Search providers..." box (live re-query) then clear it, open
// EACH faceted picker popover (Model / Status / Department) and Escape, open the
// "View" column-visibility menu and Escape, walk pagination next/prev, reveal the
// additive "Import CSV" dialog (open, hold, Escape), then exercise the BULK path
// non-destructively: click a card to flip the toolbar into selection mode, OPEN
// the bulk-delete confirm and CANCEL, OPEN the bulk-edit dialog and CANCEL, then
// Unselect All so the clip ends on a clean, populated grid.
//
...
// the single page-ready assertion (`providers-grid`), a missing/absent control
...
test.describe("demo: providers bulk", () => {
  test("tour the providers grid, filters, search + reveal bulk delete/edit & CSV import (non-destructive)", async ({
    page,
  }) => {
    test.setTimeout(120_000);

    // Open on the providers library (clean, unselected).
    await page.goto("/intelligence/providers");
    await expectAuthenticated(page);

    // ONE real page-ready assertion: the card grid is the feature container.
    await expect(page.getByTestId("providers-grid")).toBeVisible({ timeout: 40_000 });

    // Trim the skeleton lead-in so the clip never opens on the shimmer fade-in.
    await settleLoaded(page);

    // BEAT 1 — orient on the management toolbar (search + filters + actions).
    const toolbar = page.getByTestId("providers-toolbar").first();
    ...
    // BEAT 2 — scroll the populated provider cards top->bottom so the whole
    // grid reads, then return to the top.
    ...
    // BEAT 3 — type into the "Search providers..." box so the typing + re-query
    // shows, let it react, then clear it back to the full grid (read-only).
    const search = page.getByTestId("input-search-providers").first();
    if (await search.isVisible().catch(() => false)) {
      ...
      // Re-query after clear can re-fetch the list; settle past any skeleton.
      await settleLoaded(page);
    ...
    await peekFilter(page, /^Model$/);
    await peekFilter(page, /^Status$/);
    await peekFilter(page, /^Department$/);

    // BEAT 5 — open the "View" column-visibility menu to show the toggle
    ...
    // hold on the second page, then back. Gate isEnabled() behind a visible +
    // short-timeout check so an absent/single-page paginator no-ops fast (a bare
    // isEnabled() on a never-attached locator would wait the full test timeout).
    ...
    if (
      ...
      // Re-query on page change; settle past any list skeleton before holding.
      await settleLoaded(page);
      const prevPage = page
    ...
    // BEAT 8 — flip into selection mode by clicking a card body (the card's
    // onClick toggles its selection ring), swapping the toolbar in for the
    // bulk-action bar (Delete / Edit / Select Page / Unselect All). Guarded:
    ...
    if (selected) {
      // BEAT 9 — OPEN the bulk-delete confirm to show the destructive
      // affordance, hold on it, then CANCEL. NON-DESTRUCTIVE:
      // btn-confirm-bulk-delete is never clicked. The toolbar button reads
      // "Delete N of M" (or "Delete N matching" under all-matching mode).
      const bulkDelete = page
        .getByTestId("providers-toolbar")
      ...
      if (
        ...
        const dialog = page.getByTestId("dialog-bulk-delete-provider").first();
    ...
    await expect(page.getByTestId("providers-grid")).toBeVisible({ timeout: 15_000 });
    await pauseForDemo(1_300);
