#!/usr/bin/env python3
"""Generate .env from install-config.yaml.

No external dependencies — uses only Python stdlib.
Parses the specific YAML format used by install-config.yaml (flat/two-level nesting).

Usage:
    python3 scripts/generate-env.py                              # reads install-config.yaml
...
def generate_env(config_path: str | None, interactive: bool, output_path: str) -> None:
    ...
    lines = [
        ...
        f'NEXT_PUBLIC_CAMPUS_EMAIL="{values["NEXT_PUBLIC_CAMPUS_EMAIL"]}"',
        "",
        "# Database",
        ...
        f'AUTH_MICROSOFT_ENTRA_ID_SECRET="{values["AUTH_MICROSOFT_ENTRA_ID_SECRET"]}"',
        "",
        "# AI Providers",
