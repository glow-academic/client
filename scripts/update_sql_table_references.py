...
REGULAR_ARTIFACTS = [
    ...
    'eval', 'field', 'model', 'parameter', 'persona',
    'profile', 'provider', 'rubric', 'scenario', 'setting',
    'simulation', 'tool'
]
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='v4',
        help='SQL version directory (default: v4)'
    )
    ...
    for old_name, new_name in SPECIAL_PLURAL_ARTIFACTS.items():
        ...
        for sql_file in sql_files:
            updated, repl_count = update_file(sql_file, old_name, new_name, args.dry_run)
            if updated:
                count += 1
                replacements += repl_count
        if count > 0:
            print(f"  {old_name} → {new_name}: {count} files ({replacements} replacements)")
            total_updated += count
    ...
    for artifact in REGULAR_ARTIFACTS:
        ...
        replacements = 0
        for sql_file in sql_files:
            updated, repl_count = update_file(sql_file, artifact, new_name, args.dry_run)
            if updated:
                count += 1
                replacements += repl_count
        if count > 0:
            print(f"  {artifact} → {new_name}: {count} files ({replacements} replacements)")
            total_updated += count
            total_replacements += replacements
    ...
    for resource in RESOURCES:
        ...
        replacements = 0
        for sql_file in sql_files:
            updated, repl_count = update_file(sql_file, resource, new_name, args.dry_run)
            if updated:
                count += 1
                replacements += repl_count
        if count > 0:
            print(f"  {resource} → {new_name}: {count} files ({replacements} replacements)")
            total_updated += count
            total_replacements += replacements
