...
def prompt_module_selection(label: str, items: list[str]) -> str | list[str]:
    ...
    print(f"    [a] All (recommended)  [s] Select individually  [n] None")
    while True:
...
def write_config(data: dict) -> str:
    ...
    lines.append(f"    admin_password: {yaml_scalar(data.get('keycloak_password', 'admin'))}")
    lines.append(f"    realm: {yaml_scalar(data.get('keycloak_realm', 'master'))}")
    lines.append(f"    client_id: {yaml_scalar(data.get('keycloak_client_id', 'glow-client'))}")
    lines.append("  microsoft:")
    lines.append(f"    client_id: {yaml_scalar(data.get('microsoft_client_id', ''))}")
    lines.append(f"    client_secret: {yaml_scalar(data.get('microsoft_client_secret', ''))}")
    lines.append("")
...
def main() -> None:
    ...
    data["microsoft_client_id"] = prompt("Microsoft Entra client ID (blank to skip)", "")
        ...
        if setup_type:
            ...
            if setup_dir.is_dir():
                ...
                for cat_dir in sorted(setup_dir.iterdir()):
                    ...
                    if not items:
                        continue
                    result = prompt_module_selection(f"  {setup_type}/{cat_name}", items)
                    if result:
