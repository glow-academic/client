...
class Classes(_Base, table=True):
    ...
    term: str = Field(sa_column=Column('term', Enum('fall', 'spring', 'summer', name='class_term'), server_default='fall'))
...
class Cohorts(_Base, table=True):
    ...
    profile_ids: list = Field(sa_column=Column('profile_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
...
class Models(_Base, table=True):
    ...
    active: bool = Field(sa_column=Column('active', Boolean, server_default=text('true')))
    model_type: str = Field(sa_column=Column('model_type', Enum('ttt', 'tts', 'stt', name='model_type'), server_default='ttt'))
...
class Documents(_Base, table=True):
    ...
    type: str = Field(sa_column=Column('type', Enum('homework', 'project', 'quiz', 'midterm', 'lab', 'lecture', 'syllabus', name='document_type'), server_default='homework'))
...
class Profiles(_Base, table=True):
    ...
    created_at: datetime = Field(sa_column=Column('created_at', DateTime(True), default_factory=datetime.utcnow))
    role: str = Field(sa_column=Column('role', Enum('admin', 'instructional', 'instructor', 'ta', name='profile_role'), server_default='ta'))
    class_ids: list = Field(sa_column=Column('class_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
...
class Simulations(_Base, table=True):
    ...
    title: str = Field(sa_column=Column('title', Text))
    active: bool = Field(sa_column=Column('active', Boolean, server_default=text('true')))
    scenario_ids: list = Field(sa_column=Column('scenario_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
    cohort_ids: list = Field(sa_column=Column('cohort_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
...
class Dashboards(_Base, table=True):
    ...
    created_at: datetime = Field(sa_column=Column('created_at', DateTime(True), default_factory=datetime.utcnow))
    updated_at: datetime = Field(sa_column=Column('updated_at', DateTime(True), default_factory=datetime.utcnow))
    header_component_ids: list = Field(sa_column=Column('header_component_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
    primary_component_ids: list = Field(sa_column=Column('primary_component_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
    secondary_component_ids: list = Field(sa_column=Column('secondary_component_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
    footer_component_ids: list = Field(sa_column=Column('footer_component_ids', ARRAY(Uuid(as_uuid=True)), default_factory=list))
    auto_scroll: bool = Field(sa_column=Column('auto_scroll', Boolean, server_default=text('false')))
    show_indicators: bool = Field(sa_column=Column('show_indicators', Boolean, server_default=text('true')))
    header_components: int = Field(sa_column=Column('header_components', Integer, server_default=text('3')))
