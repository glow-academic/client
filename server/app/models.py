...
class Accounts(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="accounts_pkey"),)
    ...
    refresh_token: Optional[str] = Field(
        default=None, sa_column=Column("refresh_token", Text)
    )
    access_token: Optional[str] = Field(
        default=None, sa_column=Column("access_token", Text)
    )
    expires_at: Optional[int] = Field(
        default=None, sa_column=Column("expires_at", BigInteger)
    )
    ...
    session_state: Optional[str] = Field(
        default=None, sa_column=Column("session_state", Text)
    )
    token_type: Optional[str] = Field(
        default=None, sa_column=Column("token_type", Text)
    )


class AppLogs(_Base, table=True):
    __tablename__ = "app_logs"
    __table_args__ = (PrimaryKeyConstraint("id", name="app_logs_pkey"),)
    ...
    context: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column("context", JSONB)
    )
...
class Classes(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="classes_pkey"),)
    ...
    term: str = Field(
        sa_column=Column(
            "term", Enum("fall", "spring", "summer", name="class_term"), default=r"fall"
        )
    )
    description: str = Field(sa_column=Column("description", Text))
    default_class: bool = Field(
        sa_column=Column("default_class", Boolean, default=False)
    )
...
class Cohorts(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="cohorts_pkey"),)
    ...
    profile_ids: List[uuid.UUID] = Field(
        default_factory=list, sa_column=Column("profile_ids", ARRAY(Uuid(as_uuid=True)))
    )
...
class Components(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="components_pkey"),)
    ...
    layout: Dict[str, Any] = Field(
        default_factory=dict, sa_column=Column("layout", JSONB)
    )
    stat: bool = Field(sa_column=Column("stat", Boolean, default=False))
    default_component: bool = Field(
        sa_column=Column("default_component", Boolean, default=False)
    )


class Models(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="models_pkey"),)
    ...
    name: str = Field(sa_column=Column("name", Text))
    description: str = Field(sa_column=Column("description", Text))
    provider_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("provider_id", Uuid(as_uuid=True))
    )
    active: bool = Field(sa_column=Column("active", Boolean, default=True))
    model_type: str = Field(
        sa_column=Column(
            "model_type", Enum("ttt", "tts", "stt", name="model_type"), default=r"ttt"
        )
    )
...
class Providers(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="providers_pkey"),)
...
class Rubrics(_Base, table=True):
    ...
    default_rubric: bool = Field(
        sa_column=Column("default_rubric", Boolean, default=False)
    )
    ...
    simulation_chat_grades: List["SimulationChatGrades"] = Relationship(
        back_populates="rubric"
    )


class Sessions(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="sessions_pkey"),)
...
class Users(_Base, table=True):
    ...
    emailVerified: Optional[datetime] = Field(
        default=None, sa_column=Column("emailVerified", DateTime(True))
    )
...
class Agents(_Base, table=True):
    ...
    default_agent: bool = Field(
        sa_column=Column("default_agent", Boolean, default=False)
    )
    ...
    model_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("model_id", Uuid(as_uuid=True))
    )
    stt_model_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("stt_model_id", Uuid(as_uuid=True))
    )
    tts_model_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("tts_model_id", Uuid(as_uuid=True))
    )
    reasoning: Optional[str] = Field(
        default=None,
        sa_column=Column(
            "reasoning", Enum("low", "medium", "high", name="reasoning_effort")
        ),
    )
...
class Documents(_Base, table=True):
    ...
    class_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("class_id", Uuid(as_uuid=True))
    )
    type: str = Field(
        sa_column=Column(
            "type",
...
class Profiles(_Base, table=True):
    __table_args__ = (
        ForeignKeyConstraint(
            ["user_id"], ["users.id"], ondelete="CASCADE", name="profiles_user_id_fkey"
        ),
    ...
    last_login: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        sa_column=Column("last_login", DateTime(True)),
    )
    ...
    role: str = Field(
        sa_column=Column(
            "role",
            Enum("admin", "instructional", "instructor", "ta", name="profile_role"),
            default=r"ta",
        )
    )
    class_ids: List[uuid.UUID] = Field(
        default_factory=list, sa_column=Column("class_ids", ARRAY(Uuid(as_uuid=True)))
    )
    active: bool = Field(sa_column=Column("active", Boolean, default=False))
    last_active: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        sa_column=Column("last_active", DateTime(True)),
    )
    ...
    simulation_attempts: List["SimulationAttempts"] = Relationship(
...
class Simulations(_Base, table=True):
    ...
    scenario_ids: List[uuid.UUID] = Field(
        ...
        sa_column=Column("scenario_ids", ARRAY(Uuid(as_uuid=True))),
    )
    cohort_ids: List[uuid.UUID] = Field(
        default_factory=list, sa_column=Column("cohort_ids", ARRAY(Uuid(as_uuid=True)))
    )
    rubric_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("rubric_id", Uuid(as_uuid=True))
    )
    default_simulation: bool = Field(
        sa_column=Column("default_simulation", Boolean, default=False)
    )
    time_limit: Optional[int] = Field(
        default=None, sa_column=Column("time_limit", Integer)
    )
...
class AppFeedback(_Base, table=True):
    ...
    type: str = Field(
        sa_column=Column(
            "type", Enum("feature", "bug", "question", "other", name="feedback_type")
        )
    )
    created_at: Optional[datetime] = Field(
        default_factory=lambda: datetime.now(timezone.utc),
...
class AssistantChats(_Base, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["profile_id"], ["profiles.id"], name="assistant_chats_profile_id_fkey"
        ),
    ...
    title: str = Field(sa_column=Column("title", Text))
    profile_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("profile_id", Uuid(as_uuid=True))
    )
    ...
    assistant_tool_calls: List["AssistantToolCalls"] = Relationship(
        back_populates="chat"
    )
...
class Dashboards(_Base, table=True):
    ...
    header_component_ids: List[uuid.UUID] = Field(
        default_factory=list,
        sa_column=Column("header_component_ids", ARRAY(Uuid(as_uuid=True))),
    )
    primary_component_ids: List[uuid.UUID] = Field(
        default_factory=list,
        sa_column=Column("primary_component_ids", ARRAY(Uuid(as_uuid=True))),
    )
    secondary_component_ids: List[uuid.UUID] = Field(
        default_factory=list,
        sa_column=Column("secondary_component_ids", ARRAY(Uuid(as_uuid=True))),
    )
    footer_component_ids: List[uuid.UUID] = Field(
        default_factory=list,
        sa_column=Column("footer_component_ids", ARRAY(Uuid(as_uuid=True))),
    )
    auto_scroll: bool = Field(sa_column=Column("auto_scroll", Boolean, default=False))
    show_indicators: bool = Field(
        sa_column=Column("show_indicators", Boolean, default=True)
    )
    header_components: int = Field(
        sa_column=Column("header_components", Integer, default=3)
    )
    main_split: float = Field(sa_column=Column("main_split", Double(53), default=0.65))
    footer_split: float = Field(
        sa_column=Column("footer_split", Double(53), default=0.5)
    )
    profile_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("profile_id", Uuid(as_uuid=True))
    )
...
class Events(_Base, table=True):
    ...
    schedule_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("schedule_id", Uuid(as_uuid=True))
    )
...
class Scenarios(_Base, table=True):
    ...
    name: str = Field(sa_column=Column("name", Text))
    description: str = Field(sa_column=Column("description", Text))
    default_scenario: bool = Field(
        sa_column=Column("default_scenario", Boolean, default=False)
    )
    generated: bool = Field(sa_column=Column("generated", Boolean, default=False))
    agent_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("agent_id", Uuid(as_uuid=True))
    )
    class_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("class_id", Uuid(as_uuid=True))
    )
    crowdedness: Optional[int] = Field(
        default=None, sa_column=Column("crowdedness", Integer)
    )
    intensity: Optional[int] = Field(
        default=None, sa_column=Column("intensity", Integer)
    )
    seniority: Optional[str] = Field(
        default=None,
        sa_column=Column(
            "seniority",
            Enum("freshman", "sophomore", "junior", "senior", name="seniority_levels"),
        ),
    )
    location: Optional[str] = Field(
        default=None,
        sa_column=Column("location", Enum("lawson", "haas", "dsai", name="locations")),
    )
    tod: Optional[str] = Field(
        default=None,
        sa_column=Column(
            "tod",
            Enum(
    ...
    urgency: Optional[str] = Field(
        default=None,
        sa_column=Column("urgency", Enum("hour", "day", "days", name="urgency_type")),
    )
    documents: Optional[List[uuid.UUID]] = Field(
        default=None, sa_column=Column("documents", ARRAY(Uuid(as_uuid=True)))
    )
...
class SimulationAttempts(_Base, table=True):
    ...
    simulation_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("simulation_id", Uuid(as_uuid=True))
    )
    profile_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("profile_id", Uuid(as_uuid=True))
    )
    ...
    simulation: Optional["Simulations"] = Relationship(
        back_populates="simulation_attempts"
    )
...
class Standards(_Base, table=True):
    ...
    description: str = Field(sa_column=Column("description", Text))
    points: int = Field(sa_column=Column("points", Integer))
    standard_group_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("standard_group_id", Uuid(as_uuid=True))
    )

    standard_group: Optional["StandardGroups"] = Relationship(
        back_populates="standards"
    )
    simulation_chat_feedbacks: List["SimulationChatFeedbacks"] = Relationship(
...
class AssistantMessages(_Base, table=True):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["chat_id"], ["assistant_chats.id"], name="assistant_messages_chat_id_fkey"
        ),
    ...
    role: str = Field(
        sa_column=Column(
            "role", Enum("user", "assistant", name="assistant_message_type")
        )
    )
...
class AssistantToolCalls(_Base, table=True):
    ...
    tool_type: str = Field(
        sa_column=Column(
            "tool_type",
            Enum("create", "read", "update", "delete", name="assistant_tool_type"),
        )
    )
    ...
    chat: Optional["AssistantChats"] = Relationship(
        back_populates="assistant_tool_calls"
    )
...
class SimulationChats(_Base, table=True):
    ...
    title: str = Field(sa_column=Column("title", Text))
    scenario_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("scenario_id", Uuid(as_uuid=True))
    )
    attempt_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("attempt_id", Uuid(as_uuid=True))
    )
    completed: bool = Field(sa_column=Column("completed", Boolean, default=False))
    completed_at: Optional[datetime] = Field(
        default=None, sa_column=Column("completed_at", DateTime(True))
    ...
    attempt: Optional["SimulationAttempts"] = Relationship(
        back_populates="simulation_chats"
    )
    scenario: Optional["Scenarios"] = Relationship(back_populates="simulation_chats")
    simulation_chat_grades: List["SimulationChatGrades"] = Relationship(
        back_populates="simulation_chat"
    )
    simulation_messages: List["SimulationMessages"] = Relationship(
        back_populates="chat"
    )
    simulation_sketches: List["SimulationSketches"] = Relationship(
        back_populates="chat"
    )
...
class SimulationChatGrades(_Base, table=True):
    ...
    simulation_chat_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("simulation_chat_id", Uuid(as_uuid=True))
    )

    rubric: Optional["Rubrics"] = Relationship(back_populates="simulation_chat_grades")
    simulation_chat: Optional["SimulationChats"] = Relationship(
        back_populates="simulation_chat_grades"
    )
    simulation_chat_feedbacks: List["SimulationChatFeedbacks"] = Relationship(
        back_populates="simulation_chat_grade"
    )
...
class SimulationMessages(_Base, table=True):
    ...
    type: str = Field(
        sa_column=Column(
            "type", Enum("query", "response", name="simulation_message_type")
        )
    )
    completed: bool = Field(sa_column=Column("completed", Boolean, default=False))
    file_path: Optional[str] = Field(default=None, sa_column=Column("file_path", Text))

    chat: Optional["SimulationChats"] = Relationship(
        back_populates="simulation_messages"
    )
...
class SimulationSketches(_Base, table=True):
    ...
    chat: Optional["SimulationChats"] = Relationship(
        back_populates="simulation_sketches"
    )
...
class SimulationChatFeedbacks(_Base, table=True):
    ...
    standard_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("standard_id", Uuid(as_uuid=True))
    )
    simulation_chat_grade_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("simulation_chat_grade_id", Uuid(as_uuid=True))
    )
    total: int = Field(sa_column=Column("total", Integer))
    feedback: Optional[str] = Field(default=None, sa_column=Column("feedback", Text))

    simulation_chat_grade: Optional["SimulationChatGrades"] = Relationship(
        back_populates="simulation_chat_feedbacks"
    )
    standard: Optional["Standards"] = Relationship(
        back_populates="simulation_chat_feedbacks"
    )
