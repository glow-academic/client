...
class Cohorts(_Base, table=True):
    ...
    simulation_ids: List[uuid.UUID] = Field(
        ...
        sa_column=Column("simulation_ids", ARRAY(Uuid(as_uuid=True))),
    )
...
class Documents(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="documents_pkey"),)
...
class Parameters(_Base, table=True):
    __table_args__ = (PrimaryKeyConstraint("id", name="parameters_pkey"),)
...
class ParameterItems(_Base, table=True):
    ...
    parameter_id: Mapped[uuid.UUID] = Field(
        sa_column=Column("parameter_id", Uuid(as_uuid=True))
    )
...
class Personas(_Base, table=True):
    __table_args__ = (
        ForeignKeyConstraint(
            ["model_id"], ["models.id"], name="personas_model_id_fkey"
        ),
    ...
    system_prompt: str = Field(sa_column=Column("system_prompt", Text))
    temperature: int = Field(sa_column=Column("temperature", Integer))
    default_persona: bool = Field(
        sa_column=Column("default_persona", Boolean, default=False)
    )
...
class Profiles(_Base, table=True):
    ...
    default_profile: bool = Field(
        sa_column=Column("default_profile", Boolean, default=False)
    )
    active: bool = Field(sa_column=Column("active", Boolean, default=False))
...
class Simulations(_Base, table=True):
    ...
    practice_simulation: bool = Field(
        sa_column=Column("practice_simulation", Boolean, default=False)
    )
...
class Scenarios(_Base, table=True):
    ...
    active: bool = Field(sa_column=Column("active", Boolean, default=True))
    persona_id: Optional[uuid.UUID] = Field(
        default=None, sa_column=Column("persona_id", Uuid(as_uuid=True))
    )
    parameter_item_ids: Optional[List[uuid.UUID]] = Field(
        default=None, sa_column=Column("parameter_item_ids", ARRAY(Uuid(as_uuid=True)))
    )
