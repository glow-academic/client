...
class Agents(_Base, table=True):
    ...
    agent_type: str = Field(
        sa_column=Column(
            "agent_type",
            Enum("default", "student", "ta", name="agent_type"),
            server_default=text("'student'::agent_type"),
        )
    )
...
class Classes(_Base, table=True):
    ...
    term: str = Field(
        sa_column=Column(
            "term",
            Enum("fall", "spring", "summer", name="class_term"),
            server_default=text("'fall'::class_term"),
        )
    )
    description: str = Field(sa_column=Column("description", Text))
...
class Rubrics(_Base, table=True):
    ...
    rubric_grades: List["RubricGrades"] = Relationship(back_populates="rubric")
...
class Users(_Base, table=True):
    ...
    viewed_intro: bool = Field(
        sa_column=Column("viewed_intro", Boolean, server_default=text("false"))
    )
    created_at: datetime = Field(
        sa_column=Column("created_at", DateTime(True), server_default=text("now()"))
    )
    role: str = Field(
        sa_column=Column(
            "role",
            Enum("admin", "instructional", "instructor", "ta", name="user_role"),
            server_default=text("'ta'::user_role"),
        )
    )
    name: str = Field(sa_column=Column("name", Text))
    username: str = Field(sa_column=Column("username", Text))
    password: str = Field(sa_column=Column("password", Text))
    class_ids: list = Field(
        sa_column=Column(
            "class_ids", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
        )
    )
...
class Documents(_Base, table=True):
    ...
    type: str = Field(
        sa_column=Column(
            ...
            server_default=text("'homework'::document_type"),
        )
    )
    classified: bool = Field(
        sa_column=Column("classified", Boolean, server_default=text("false"))
    )
...
class Evals(_Base, table=True):
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["class_id"], ["classes.id"], ondelete="CASCADE", name="evals_class_id_fkey"
        ),
    ...
    scenario_ids: list = Field(
        sa_column=Column(
            "scenario_ids", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
        )
    )
    agent_ids: list = Field(
        sa_column=Column(
            "agent_ids", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
        )
    )
    eval_type: str = Field(
        sa_column=Column(
            "eval_type",
            Enum("student", "ta", name="eval_type"),
            server_default=text("'student'::eval_type"),
        )
    )
    ...
    rubric_ids: list = Field(
        sa_column=Column(
            "rubric_ids", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
        )
    )
...
class RubricGrades(_Base, table=True):
    ...
    rubric: Optional["Rubrics"] = Relationship(back_populates="rubric_grades")
...
class Scenarios(_Base, table=True):
    ...
    seniority: str = Field(
        sa_column=Column(
            ...
            server_default=text("'freshman'::seniority_levels"),
        )
    )
...
class Simulations(_Base, table=True):
    ...
    documents: list = Field(
        sa_column=Column(
            "documents", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
        )
    )
    active: bool = Field(
        sa_column=Column("active", Boolean, server_default=text("true"))
    )
    scenario_ids: list = Field(
        sa_column=Column(
            "scenario_ids", ARRAY(Uuid()), server_default=text("ARRAY[]::uuid[]")
...
class Topics(_Base, table=True):
    ...
    prerequisite: bool = Field(
        sa_column=Column("prerequisite", Boolean, server_default=text("false"))
    )
    class_id: UUID = Field(sa_column=Column("class_id", Uuid))
...
class Attempts(_Base, table=True):
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["user_id"], ["users.id"], ondelete="CASCADE", name="attempts_user_id_fkey"
        ),
...
class EvalRuns(_Base, table=True):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["eval_id"], ["evals.id"], ondelete="CASCADE", name="eval_runs_eval_id_fkey"
        ),
        ForeignKeyConstraint(
            ["query_agent_id"],
            ["agents.id"],
...
class SimulationChats(_Base, table=True):
    ...
    completed: bool = Field(
        sa_column=Column("completed", Boolean, server_default=text("false"))
    )
    completed_at: Optional[datetime] = Field(
        default=None, sa_column=Column("completed_at", DateTime(True))
    )

    attempt: Optional["Attempts"] = Relationship(back_populates="simulation_chats")
...
class StandardGrades(_Base, table=True):
    ...
    rubric_grade_id: UUID = Field(sa_column=Column("rubric_grade_id", Uuid))
