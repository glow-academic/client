"""Values GET endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/v5/sql/queries/resources/values/get_values_complete.sql"

router = APIRouter()


async def get_values_internal(
    conn: asyncpg.Connection,
    ids: list[UUID],
    bypass_cache: bool = False,
) -> list[QGetValuesV4Item]:
    """Internal function to fetch values by IDs."""
    if not ids:
        return []
...
@router.post(
    "/values/get",
    response_model=GetValuesApiResponse,
)
async def get_values(
    request: GetValuesApiRequest,
    ...
    try:
        items = await get_values_internal(conn, request.ids, bypass_cache)
