...
SQL_PATH = "app/v5/sql/queries/resources/link_scenarios_complete.sql"
...
class LinkScenariosSqlParams(BaseModel):
    resource_id: UUID
    group_id: UUID
    tool_id: UUID
...
class LinkScenariosSqlRow(BaseModel):
    scenarios_id: UUID | None = None


async def link_scenarios_internal(
    ...
    result_row = LinkScenariosSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.scenarios_id:
        raise ValueError(f"Failed to link scenario: {resource_id}")
    return result_row.scenarios_id
