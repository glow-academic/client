...
SQL_PATH = "app/v5/sql/queries/resources/models/get_models_complete.sql"

router = APIRouter()


async def get_models_internal(
    conn: asyncpg.Connection,
    ids: list[UUID],
    bypass_cache: bool = False,
) -> list[QGetModelsV4Item]:
    """Internal function to fetch models by IDs.
    ...
    # Execute SQL
    params = GetModelsSqlParams(ids=ids)
    result = cast(
        GetModelsSqlRow,
...
async def get_models(
    ...
    try:
        items = await get_models_internal(conn, request.ids, bypass_cache)
