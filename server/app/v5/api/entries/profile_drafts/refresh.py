...
MV_NAME = "profile_drafts_mv"

router = APIRouter()


async def refresh_profile_drafts_internal(
    ...
    await invalidate_tags(["entries", "profile_drafts"])
    return {
        "success": True,
...
async def refresh_profile_drafts(
    ...
    try:
        result = await refresh_profile_drafts_internal(conn)
