...
from app.v5.sql.types import (
    ...
    GetImagesEntriesSqlParams,
    GetImagesEntriesSqlRow,
    load_sql_query,
)
...
SQL_PATH = "app/v5/sql/queries/entries/images/get_images_entries_complete.sql"

router = APIRouter()


async def get_images_entries_internal(
    conn: asyncpg.Connection,
    ids: list[UUID],
    bypass_cache: bool = False,
) -> list[dict]:
    """Internal function to fetch images entries by IDs."""
    if not ids:
        return []
    ...
    params = GetImagesEntriesSqlParams(ids=ids)
    result = cast(
        GetImagesEntriesSqlRow,
