...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_search_persona_drafts_entries_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_search_persona_drafts_entries_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION public.api_search_persona_drafts_entries_v4(
