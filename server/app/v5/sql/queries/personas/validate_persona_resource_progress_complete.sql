-- Validate persona resource progress - no-op function for validation only
-- Validates artifact_type and resource_type for persona progress events
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_validate_persona_resource_progress_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_validate_persona_resource_progress_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_validate_persona_resource_progress_v4_%'
...
CREATE OR REPLACE FUNCTION api_validate_persona_resource_progress_v4(
    profile_id uuid,
    group_id uuid,
    resource_type text,
    artifact_type text
