"""Handler for generate_video WebSocket event."""
...
async def _generate_video_impl(sid: str, data: GenerateVideoPayload) -> None:
    ...
    try:
        logger.info(f"Received generate_video request from {sid} with data: {data}")
        ...
        logger.error(f"Error in generate_video for {sid}: {str(e)}", exc_info=True)
...
async def generate_video(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler"""
    try:
        validated = GenerateVideoPayload(**data)
        await _generate_video_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in generate_video for {sid}: {e}")
        await video_generation_error(
            VideoGenerationErrorPayload(
