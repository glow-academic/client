        ...
        async with pool.acquire() as conn:
            ...
            if needs_generation:
                try:
                    ...
                    if not department_id:
                        raise ValueError(
                            "Cannot generate scenario: no department_id available"
                        )
                    ...
                    def tool_use_behavior(
                        ...
                        completed_required = all(
                            scenario_progress.get(tool, False)
                    ...
                    final_profile_id = (
                        attempt_profile_uuid
                        if attempt_profile_uuid
                        else context["default_guest_profile_id"]
                    )
                    ...
                    sql = load_sql(
                        "sql/v3/scenarios/insert_scenario_problem_statement.sql"
                    )
                    await conn.fetchrow(
                        sql,
                        new_scenario_id,
                    ...
                    logger.info(
                        f"Created problem statement for child scenario {new_scenario_id}"
                    )
                    ...
                    logger.info(
                        f"Created scenario_tree edge: parent={scenario_id} -> child={new_scenario_id}"
                    )
                    ...
                    if persona_id:
                        sql = load_sql(
                            "sql/v3/scenarios/insert_scenario_persona_link.sql"
                        )
                        await conn.execute(sql, new_scenario_id, persona_id, True)
                        logger.info(
                            f"Linked persona {persona_id} to child scenario {new_scenario_id}"
                        )

                    # Link documents
                    if doc_ids:
                        sql = load_sql(
                            "sql/v3/scenarios/insert_scenario_document_link.sql"
                        )
                        for doc_id in doc_ids:
                            await conn.execute(sql, new_scenario_id, doc_id, True)
                        logger.info(
                            f"Linked {len(doc_ids)} document(s) to child scenario {new_scenario_id}"
                        )
                    ...
                    if param_ids:
                        ...
                        logger.info(
                            f"Linked {len(param_ids)} parameter item(s) to child scenario {new_scenario_id}"
                        )
                    ...
                    logger.info(
                        f"Linked department {department_id} to child scenario {new_scenario_id}"
                    )
