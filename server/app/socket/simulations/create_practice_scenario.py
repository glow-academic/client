...
async def create_practice_scenario_error(
    payload: CreatePracticeScenarioErrorPayload, room: str
) -> None:
    await sio.emit("create_practice_scenario_error", payload.model_dump(), room=room)


async def _create_practice_scenario_impl(
    ...
    try:
        logger.info(
            f"Received create_practice_scenario request from {sid} with data: {data}"
        )
...
async def create_practice_scenario(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler"""
    try:
        validated = CreatePracticeScenarioPayload(**data)
        await _create_practice_scenario_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in create_practice_scenario for {sid}: {e}")
        await create_practice_scenario_error(
            CreatePracticeScenarioErrorPayload(
