"""Handler for voice_response_done WebSocket event."""
...
class VoiceResponseDonePayload(BaseModel):
    """Client-to-server payload for voice_response_done."""
...
async def _voice_response_done_impl(
    ...
    try:
        logger.info(
            f"Received voice_response_done from {sid} for chat {data.chat_id}, "
            f"response_id={data.response_id}, event_id={data.event_id}"
        )

        chat_id = data.chat_id
        if not chat_id:
            logger.warning(f"Missing chat_id in voice_response_done from {sid}")
            return
        ...
        async with pool.acquire() as conn:
            ...
            # Emit simulation_run_complete to signal that the response is done
...
async def voice_response_done(sid: str, data: dict[str, Any]) -> None:
