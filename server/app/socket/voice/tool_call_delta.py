"""Handler for voice_tool_call_delta WebSocket event."""
...
class VoiceToolCallDeltaPayload(BaseModel):
    """Client-to-server payload for voice_tool_call_delta."""
...
async def _voice_tool_call_delta_impl(
    ...
    try:
        ...
        logger.error(
            f"Error in voice_tool_call_delta for {sid}: {str(e)}", exc_info=True
        )
        await voice_tool_call_error(
...
async def voice_tool_call_delta(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler."""
    try:
        validated = VoiceToolCallDeltaPayload(**data)
        await _voice_tool_call_delta_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in voice_tool_call_delta for {sid}: {e}")
        await voice_tool_call_error(
            VoiceToolCallErrorPayload(
