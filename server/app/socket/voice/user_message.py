"""Handler for voice_user_message WebSocket event."""
...
class VoiceUserMessagePayload(BaseModel):
    """Client-to-server payload for voice_user_message."""
...
async def voice_user_message_error(
    payload: VoiceUserMessageErrorPayload, room: str
) -> None:
    await sio.emit("voice_user_message_error", payload.model_dump(), room=room)


async def _voice_user_message_impl(sid: str, data: VoiceUserMessagePayload) -> None:
    ...
    try:
        logger.info(
            f"Received voice_user_message from {sid}: chat_id={data.chat_id}, message_length={len(data.message)}"
        )
        ...
        logger.error(f"Invalid UUID format in voice_user_message for {sid}: {e}")
        await voice_user_message_error(
            VoiceUserMessageErrorPayload(
...
async def voice_user_message(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler."""
    try:
        validated = VoiceUserMessagePayload(**data)
        await _voice_user_message_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in voice_user_message for {sid}: {e}")
        await voice_user_message_error(
            VoiceUserMessageErrorPayload(
