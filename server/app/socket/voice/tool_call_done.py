"""Handler for voice_tool_call_done WebSocket event."""

import json
import uuid
from typing import Any

from app.main import get_pool, get_simulation_tool_calls_dict, sio
...
class VoiceToolCallDonePayload(BaseModel):
    """Client-to-server payload for voice_tool_call_done."""
...
async def _voice_tool_call_done_impl(
    ...
    try:
        logger.info(
            f"Received voice_tool_call_done from {sid}: call_id={data.call_id}, "
            f"chat_id={data.chat_id}, arguments_length={len(data.arguments)}"
        )
        ...
        async with pool.acquire() as conn:
            ...
            if (
                ...
                logger.warning(
                    ...
                    "chat_id={chat_id}. This may happen if delta events were missed."
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Falling back to complete tool call processing for "
                        f"call_id={call_id}"
                    )
            ...
            logger.info(
                f"Matched persona '{persona_name}' to {persona_display_name} "
                f"(ID: {persona_id})"
            )
        ...
        logger.error(
            f"Error in voice_tool_call_done for {sid}: {str(e)}", exc_info=True
        )
        await voice_tool_call_error(
...
async def voice_tool_call_done(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler."""
    try:
        validated = VoiceToolCallDonePayload(**data)
        await _voice_tool_call_done_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in voice_tool_call_done for {sid}: {e}")
        await voice_tool_call_error(
            VoiceToolCallErrorPayload(
