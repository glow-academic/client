"""Handler for voice_transcript_ready WebSocket event."""
...
class VoiceTranscriptReadyPayload(BaseModel):
    """Client-to-server payload for voice_transcript_ready."""
...
async def voice_transcript_ready_emit(
    payload: VoiceTranscriptReadyPayload, room: str
) -> None:
    """Emit voice_transcript_ready event to room (server-to-client)."""
    await sio.emit("voice_transcript_ready", payload.model_dump(), room=room)


async def _voice_transcript_ready_impl(
    ...
    try:
        logger.info(
            f"Received voice_transcript_ready from {sid}: chat_id={data.chat_id}, "
            f"transcript_length={len(data.transcript)}, item_id={data.item_id}"
        )

        chat_id = data.chat_id
        if not chat_id:
            logger.warning(f"Missing chat_id in voice_transcript_ready from {sid}")
            return

        transcript = data.transcript
        if not transcript or not transcript.strip():
            logger.warning(f"Empty transcript in voice_transcript_ready from {sid}")
            return

        room = f"simulation_{chat_id}"

        # First, relay the event for UI updates (so optimistic message shows transcript immediately)
        await voice_transcript_ready_emit(data, room)
        ...
        async with pool.acquire() as conn:
            ...
            logger.info(
                f"Created user message {user_message['id']} for chat {chat_id} via voice_transcript_ready"
            )

    except ValueError as e:
        logger.error(f"Invalid UUID format in voice_transcript_ready for {sid}: {e}")
    except Exception as e:
        logger.error(f"Error handling voice_transcript_ready: {e}", exc_info=True)


@sio.event  # type: ignore
async def voice_transcript_ready(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler."""
    try:
        validated = VoiceTranscriptReadyPayload(**data)
        await _voice_transcript_ready_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in voice_transcript_ready for {sid}: {e}")
