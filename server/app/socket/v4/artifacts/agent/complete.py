"""Agent completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.agent.save import save_agent_internal
...
from app.socket.v4.artifacts.agent.types import AgentGenerationCompleteEvent
...
async def _handle_agent_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle agent generation run completion.
    ...
    3. If all agents done: emits agent_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save agent if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_agent_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        agent_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save agent: {str(e)}")

        # Emit agent_generation_complete
        event = AgentGenerationCompleteEvent(
            artifact_type="agent",
...
async def agent_generation_complete_api(
    request: AgentGenerationCompleteEvent,
) -> dict[str, bool]:
    """Server-to-client event: Agent generation completed."""
    return {"success": True}
