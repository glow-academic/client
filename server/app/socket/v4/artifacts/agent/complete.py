"""Agent completion handler - listens to artifact_generation_complete events and emits granular agent events."""
...
SQL_PATH = "app/sql/v4/agents/get_agent_resource_ids_by_group_id_complete.sql"


@internal_sio.on("resource_complete")  # type: ignore
async def handle_agent_artifact_complete(data: dict[str, Any]) -> None:
    """Handle resource_complete internal event - filter by agent artifact_type and emit granular event."""
    ...
    if not resource_id_str:
        # Emit resource_error - the agents error handler will pick it up and emit agent_generation_error to client
