...
SQL_PATH = "app/sql/v4/queries/agents/validate_agent_resource_error_complete.sql"


@internal_sio.on("generate_call_error")  # type: ignore
async def handle_agents_error(data: dict[str, Any]) -> None:
    """Handle generate_*_error event - filter by agent artifact_type and emit agent-specific event."""
    ...
    error_message = data.get("error_message") or data.get(
        "message", "An error occurred during agent generation"
    )

    # Emit agent-specific error event with all fields from internal event
    await sio.emit(
...
@server_router.post("/agent_generation_error")
async def agent_generation_error_api(
    request: dict[str, Any],
) -> dict[str, bool]:
    """Server-to-client event: agent generation error."""
    _ = request
    return {"ok": True}
