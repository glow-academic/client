...
This module handles all business logic for agent generation:
...
from app.api.v4.artifacts.agent.types import GetAgentWebsocketResponse
...
def _build_agent_jinja_context(
    response: GetAgentWebsocketResponse,
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_agent_internal's ID resolution).
...
async def _agent_generate_impl(
    ...
    """Handle agent generation with all business logic."""
    try:
        ...
        # Step 1: Fetch agent data (includes pre-fetched config resources)
        ...
        agent_jinja_context = _build_agent_jinja_context(result)

        # Step 3: Check rate limit (the only thing still in SQL)
        async with get_db_connection() as conn:
            context_params = GetAgentGenerationContextSqlParams(
            ...
            if requests_per_day is not None and runs_today >= requests_per_day:
                ...
                logger.error(
                    f"Agent generation rate limit exceeded - "
        ...
        async with get_db_connection() as conn:
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Agent generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare agent generation: Unknown error",
                        artifact_type="agent",
            ...
            if config_id:
                ...
                config_view = {}
            draft_agent_view = (
                result.views.draft_agent.model_dump(mode="json")
                if result.views and result.views.draft_agent
                else {}
            )
            jinja_context["views"] = {
                "config": config_view,
                "draft_agent": draft_agent_view,
            }
            ...
            # Emit agent_generation_started to client
            await sio.emit(
...
@server_router.post("/agent_generation_started")
async def agent_generation_started_api(
    request: AgentGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Agent generation started.
