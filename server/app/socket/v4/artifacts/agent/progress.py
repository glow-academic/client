"""Agent progress handler - listens to resource_progress events and emits agent-specific events."""
...
SQL_PATH = "app/sql/v4/queries/agents/validate_agent_resource_progress_complete.sql"


@internal_sio.on("resource_progress")  # type: ignore
async def handle_agents_progress(data: dict[str, Any]) -> None:
    """Handle resource_progress internal event - filter by agent artifact_type and emit agent-specific event."""
    ...
    try:
        async with get_db_connection() as conn:
            params = ValidateAgentResourceProgressSqlParams(
    ...
    # Emit agent-specific progress event with all fields from internal event
    await sio.emit(
        "agent_generation_progress",
        {
