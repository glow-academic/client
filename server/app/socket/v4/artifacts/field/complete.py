"""Field completion handler - listens to generate_call_complete events and emits granular field events."""
...
from app.socket.v4.artifacts.field.types import FieldGenerationCompleteEvent
...
async def handle_field_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by field artifact_type and emit granular event."""
    ...
    if event_type == "text_complete":
        await _handle_field_text_complete(sid, data)
        return

    # Handle run complete - save final assistant content + update tokens
    if event_type == "run_complete":
        await _handle_field_run_complete(sid, data)
        return
    ...
    try:
        async with get_db_connection() as conn:
            if resource_type == "names":
                name_items = await get_names_internal(conn, [resource_id])
                event.name_resource = name_items[0] if name_items else None
            elif resource_type == "descriptions":
                desc_items = await get_descriptions_internal(conn, [resource_id])
                event.description_resource = desc_items[0] if desc_items else None
            elif resource_type == "flags":
                flag_items = await get_flags_internal(conn, [resource_id])
                event.flag_resource = flag_items[0] if flag_items else None
...
async def _handle_field_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle field text generation completion - save assistant message."""
    run_id = data.get("run_id")
    ...
    try:
        ...
        logger.exception(f"Failed to save field text message: {str(e)}")


async def _handle_field_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle field generation run completion - save assistant output if present."""
    run_id = data.get("run_id")
    ...
    try:
        ...
        logger.exception(f"Failed to save field run complete: {str(e)}")
...
async def field_generation_complete_api(
    request: FieldGenerationCompleteEvent,
) -> dict[str, bool]:
    """Server-to-client event: Field generation completed.
