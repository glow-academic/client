"""Field completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.field.save import save_field_internal
...
async def handle_field_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by field artifact_type."""
...
async def _handle_field_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle field generation run completion.
    ...
    3. If all agents done: emits field_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save field if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_field_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        field_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save field: {str(e)}")

        # Emit field_generation_complete
        event = FieldGenerationCompleteEvent(
            ...
            run_id=run_id,
            success=True,
            message="Field generation completed",
            field_id=field_id,
        )
