"""Health completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_health_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by health artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_health_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_health_run_complete(sid, data)
        return
...
async def _handle_health_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle health text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save health text message: {str(e)}")


async def _handle_health_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle health generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits health_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save health run complete: {str(e)}")
    ...
    if is_complete:
        # Emit health_generation_complete
        event = HealthGenerationCompleteEvent(
            artifact_type="health",
            group_id=group_id_str or "",
            resource_type="health",
            run_id=run_id,
            success=True,
            message="Health generation completed",
        )
...
async def health_generation_complete_api(
    ...
    Emitted when all agents have finished generating health resources.
    """
    return {"success": True}
