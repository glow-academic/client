"""Health generation router - unified handler for all health resource types.

This module handles all business logic for health generation:
...
from app.api.v4.artifacts.health.get import get_health_websocket
from app.api.v4.artifacts.health.types import GetHealthWebsocketResponse
...
from app.socket.v4.artifacts.health.types import (
    HEALTH_RESOURCE_TYPES,
    GenerateHealthPayload,
)
from app.socket.v4.artifacts.types import (
    GenerateErrorApiRequest,
    HealthGenerationStartedEvent,
)
...
def _build_health_jinja_context(
    response: GetHealthWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_health_internal's ID resolution).
...
async def _health_generate_impl(
    sid: str,
    data: GenerateHealthPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle health generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches health bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if HEALTH_RESOURCE_TYPES is populated)
        if HEALTH_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in HEALTH_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch health bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_health_websocket(
            pool=pool,
        ...
        health_jinja_context = _build_health_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Health generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare health generation: {error_msg}",
                    artifact_type="health",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareHealthGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Health generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare health generation: Unknown error",
                        artifact_type="health",
            ...
            jinja_context = health_jinja_context
            ...
            # Emit health_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate health resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate health resources: {str(e)}",
                artifact_type="health",
                group_id=None,
                resource_type="health",
            ),
...
async def health_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle health_generate event (client-to-server)."""
    try:
        payload = GenerateHealthPayload(**data)
        ...
        await _health_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("health_generate")  # type: ignore
async def health_generate_internal(data: dict[str, Any]) -> None:
    """Handle health_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def health_generation_started_api(
    ...
    Emitted when health generation begins, listing resource types being generated.
    """
    return {"success": True}
