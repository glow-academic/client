"""Health progress handler - emits percentage-based progress as resources complete.

Listens on generate_call_complete, filters for health artifact with tool_result events
that have a resource_id (successful resource creation), and emits health_generation_progress
...
from app.socket.v4.artifacts.health.types import HealthGenerationProgressEvent
...
async def handle_health_resource_progress(data: dict[str, Any]) -> None:
    """Track resource completions and emit health progress percentage."""
    ...
    event = HealthGenerationProgressEvent(
...
async def health_generation_progress_api(
    request: HealthGenerationProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Health generation progress.

    Emitted as individual resources complete during health generation.
