"""Eval generation router - unified handler for all eval resource types.

This module handles all business logic for eval generation:
...
- Eval-specific: rubrics alias normalization and agent resolution fallbacks
...
from app.api.v4.artifacts.eval.types import GetEvalWebsocketResponse
...
from app.socket.v4.artifacts.eval.types import (
    EVAL_RESOURCE_TYPES,
    GenerateEvalPayload,
)
...
def _resolve_agent_for_resource(
    resource_type: str, resource_agent_ids: dict[str, uuid.UUID | None]
) -> uuid.UUID | None:
    """Resolve agent_id for an eval resource type with fallback logic."""
    ...
    if resource_type in {"run_positions", "group_positions"}:
        return resource_agent_ids.get("agents") or resource_agent_ids.get("rubrics")
    return None
...
async def _eval_generate_impl(
    ...
    try:
        ...
        # Step 1: Fetch pre-hydrated eval data
        ...
        # Uses eval-specific resolve_agent_for_resource with fallback logic
        agent_groups: dict[uuid.UUID, list[str]] = {}
        for rt in resource_types:
            aid = _resolve_agent_for_resource(rt, resource_agent_ids)
            if aid is not None:
        ...
        if not agent_resource or not model_resource or not provider_resource:
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message="Incomplete eval generation configuration (agents/models/providers)",
                    artifact_type="eval",
                    group_id=None,
                    resource_type="eval",
                ),
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareEvalGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                await emit_to_internal(
                    ...
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare eval generation",
                        artifact_type="eval",
            ...
            draft_view = {}
            if result.views and result.views.draft_eval:
                draft_view = result.views.draft_eval.model_dump(mode="json")
            jinja_context["views"] = {
                "config": config_view,
                "draft_eval": draft_view,
            }
            ...
            # Emit eval_generation_started to client
            await sio.emit(
                "eval_generation_started",
                EvalGenerationStartedEvent(
                    artifact_type="eval",
...
@internal_sio.on("eval_generate")  # type: ignore
async def eval_generate_internal(data: dict[str, Any]) -> None:
    """Handle eval_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
@server_router.post("/eval_generation_started")
async def eval_generation_started_api(
    request: EvalGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Eval generation started.
