"""Eval completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.eval.save import save_eval_internal
...
async def handle_eval_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by eval artifact_type."""
...
async def _handle_eval_run_complete(sid: str, data: dict[str, Any]) -> None:
    ...
    3. If all agents done: emits eval_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save eval if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_eval_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        eval_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save eval: {str(e)}")

        # Emit eval_generation_complete
