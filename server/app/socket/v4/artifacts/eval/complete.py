"""Eval completion handler - emits eval-specific generation complete events."""
...
async def handle_eval_artifact_complete(data: dict[str, Any]) -> None:
    ...
    try:
        async with get_db_connection() as conn:
            request_payload = GetEvalApiRequest.model_validate(
                {"eval_id": uuid.UUID(artifact_id_str)}
            )
    ...
    values = getattr(result, "agent_ids", None) or []
...
async def eval_generation_complete_api(request: dict[str, Any]) -> dict[str, bool]:
    _ = request
    return {"ok": True}
