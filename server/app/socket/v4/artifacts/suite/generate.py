"""Suite generation router - unified handler for all suite resource types.

This module handles all business logic for suite generation:
...
from app.api.v4.artifacts.suite.get import get_suite_websocket
from app.api.v4.artifacts.suite.types import GetSuiteWebsocketResponse
...
from app.socket.v4.artifacts.suite.types import (
    SUITE_RESOURCE_TYPES,
...
async def _suite_generate_impl(
    sid: str,
    data: GenerateSuitePayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle suite generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches suite bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if SUITE_RESOURCE_TYPES is populated)
        if SUITE_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in SUITE_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch suite bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Suite generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare suite generation: {error_msg}",
                    artifact_type="suite",
        ...
        async with get_db_connection() as conn:
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Suite generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare suite generation: Unknown error",
                        artifact_type="suite",
        ...
        logger.exception(f"Failed to generate suite resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate suite resources: {str(e)}",
                artifact_type="suite",
                group_id=None,
                resource_type="suite",
            ),
...
async def suite_generation_started_api(
    ...
    Emitted when suite generation begins, listing resource types being generated.
    """
    return {"success": True}
