"""Suite completion handler - handles run/text completion and multi-agent coordination.
...
async def _handle_suite_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle suite text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save suite text message: {str(e)}")


async def _handle_suite_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle suite generation run completion.
    ...
    try:
        ...
        logger.exception(f"Failed to save suite run complete: {str(e)}")
    ...
    if is_complete:
        ...
        event = SuiteGenerationCompleteEvent(
            ...
            run_id=run_id,
            success=True,
            message="Suite generation completed",
        )
