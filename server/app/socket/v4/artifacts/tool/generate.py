"""Tool generation handler - routes tool generation through unified generate pipeline."""
...
class GenerateToolPayload(GetToolApiRequest):
    """Request to generate tool resources."""
...
async def _generate_tool_impl(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            request_payload = GetToolApiRequest.model_validate(data.model_dump())
        ...
        await emit_generation_error(
            sid=sid,
            artifact_type="tool",
            message=f"Failed to generate tool: {str(e)}",
            resource_id=str(data.tool_id) if data.tool_id else None,
            resource_type="tool",
        )
...
async def tool_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle tool_generate client event."""
    try:
        ...
        if not profile_id_str:
            await emit_generation_error(
                ...
                resource_id=str(payload.tool_id) if payload.tool_id else None,
                resource_type="tool",
            )
            return

        await _generate_tool_impl(sid, payload, uuid.UUID(profile_id_str))
    except Exception as e:
        await emit_generation_error(
            sid=sid,
            artifact_type="tool",
            message=f"Invalid request: {str(e)}",
            resource_id=str(data.get("tool_id")) if data.get("tool_id") else None,
            resource_type="tool",
        )
