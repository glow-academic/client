"""Tool generation router - unified handler for all tool resource types.
...
from app.api.v4.artifacts.tool.types import GetToolWebsocketResponse
...
def _build_tool_jinja_context(
    response: GetToolWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    _ = resource_types
...
async def _tool_generate_impl(
    ...
    try:
        ...
        # Step 1: Fetch pre-hydrated tool data
        ...
        if not agent_resource or not model_resource or not provider_resource:
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message="Incomplete tool generation configuration (agents/models/providers)",
                    artifact_type="tool",
                    group_id=None,
                    resource_type="tool",
                ),
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Tool generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare tool generation: {error_msg}",
                    artifact_type="tool",
        ...
        tool_jinja_context = _build_tool_jinja_context(result, resource_types)
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareToolGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                await emit_to_internal(
                    ...
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare tool generation",
                        artifact_type="tool",
            ...
            jinja_context = tool_jinja_context
            if config_id:
            ...
            draft_view = {}
            if result.views and result.views.draft_tool:
                draft_view = result.views.draft_tool.model_dump(mode="json")
            jinja_context["views"] = {
                "config": config_view,
                "draft_tool": draft_view,
            }
            ...
            # Emit tool_generation_started to client
            await sio.emit(
...
@server_router.post("/tool_generation_started")
async def tool_generation_started_api(
    request: ToolGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Tool generation started.
