"""Scenario error handler - listens to generate_*_error events and emits scenario-specific events."""
...
SQL_PATH = "app/sql/v4/queries/scenarios/validate_scenario_resource_error_complete.sql"


@internal_sio.on("generate_call_error")  # type: ignore
async def handle_scenarios_error(data: dict[str, Any]) -> None:
    """Handle generate_*_error event - filter by scenario artifact_type and emit scenario-specific event."""
    ...
    # Emit scenario-specific error event with all fields from internal event
    await sio.emit(
