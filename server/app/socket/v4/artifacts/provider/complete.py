"""Provider completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_provider_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by provider artifact_type."""
...
async def _handle_provider_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle provider generation run completion.
    ...
    3. If all agents done: emits provider_generation_complete
    ...
    if is_complete:
        # All agents finished - emit provider_generation_complete
        event = ProviderGenerationCompleteEvent(
            ...
            run_id=run_id,
            success=True,
            message="Provider generation completed",
        )
