"""Dashboard progress handler - emits percentage-based progress as resources complete.

Listens on generate_call_complete, filters for dashboard artifact with tool_result events
that have a resource_id (successful resource creation), and emits dashboard_generation_progress
...
from app.socket.v4.artifacts.dashboard.types import DashboardGenerationProgressEvent
...
async def handle_dashboard_resource_progress(data: dict[str, Any]) -> None:
    """Track resource completions and emit dashboard progress percentage."""
    ...
    event = DashboardGenerationProgressEvent(
...
async def dashboard_generation_progress_api(
    request: DashboardGenerationProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Dashboard generation progress.

    Emitted as individual resources complete during dashboard generation.
