"""Dashboard generation router - unified handler for all dashboard resource types.

This module handles all business logic for dashboard generation:
...
from app.api.v4.artifacts.dashboard.get import get_dashboard_websocket
from app.api.v4.artifacts.dashboard.types import GetDashboardWebsocketResponse
...
from app.socket.v4.artifacts.dashboard.types import (
    DASHBOARD_RESOURCE_TYPES,
    GenerateDashboardPayload,
)
from app.socket.v4.artifacts.types import (
    DashboardGenerationStartedEvent,
...
def _build_dashboard_jinja_context(
    response: GetDashboardWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_dashboard_internal's ID resolution).
...
async def _dashboard_generate_impl(
    sid: str,
    data: GenerateDashboardPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle dashboard generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches dashboard bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if DASHBOARD_RESOURCE_TYPES is populated)
        if DASHBOARD_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in DASHBOARD_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch dashboard bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_dashboard_websocket(
            pool=pool,
        ...
        dashboard_jinja_context = _build_dashboard_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Dashboard generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare dashboard generation: {error_msg}",
                    artifact_type="dashboard",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareDashboardGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Dashboard generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare dashboard generation: Unknown error",
                        artifact_type="dashboard",
            ...
            jinja_context = dashboard_jinja_context
            ...
            # Emit dashboard_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate dashboard resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate dashboard resources: {str(e)}",
                artifact_type="dashboard",
                group_id=None,
                resource_type="dashboard",
            ),
...
async def dashboard_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle dashboard_generate event (client-to-server)."""
    try:
        payload = GenerateDashboardPayload(**data)
        ...
        await _dashboard_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("dashboard_generate")  # type: ignore
async def dashboard_generate_internal(data: dict[str, Any]) -> None:
    """Handle dashboard_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def dashboard_generation_started_api(
    ...
    Emitted when dashboard generation begins, listing resource types being generated.
    """
    return {"success": True}
