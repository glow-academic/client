"""Dashboard completion handler - handles run/text completion and multi-agent coordination.
...
from app.socket.v4.artifacts.dashboard.types import (
    DashboardGenerationCompleteEvent,
)
...
async def handle_dashboard_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by dashboard artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_dashboard_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_dashboard_run_complete(sid, data)
        return
...
async def _handle_dashboard_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle dashboard text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save dashboard text message: {str(e)}")


async def _handle_dashboard_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle dashboard generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits dashboard_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save dashboard run complete: {str(e)}")
    ...
    if is_complete:
        # Emit dashboard_generation_complete
        event = DashboardGenerationCompleteEvent(
...
async def dashboard_generation_complete_api(
    ...
    Emitted when all agents have finished generating dashboard resources.
    """
    return {"success": True}
