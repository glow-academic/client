"""Simulation progress handler - listens to resource_progress events and emits simulation-specific events."""
...
SQL_PATH = "app/sql/v4/simulations/validate_simulation_resource_progress_complete.sql"


@internal_sio.on("resource_progress")  # type: ignore
async def handle_simulations_progress(data: dict[str, Any]) -> None:
    """Handle resource_progress internal event - filter by simulation artifact_type and emit simulation-specific event."""
    ...
    # Emit simulation-specific progress event with all fields from internal event
    await sio.emit(
