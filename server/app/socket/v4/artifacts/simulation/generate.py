...
This module handles all business logic for simulation generation:
...
from app.sql.types import (
    ...
    GetAgentToolsSqlRow,
    GetSimulationGenerationContextSqlParams,
    GetSimulationGenerationContextSqlRow,
    PrepareSimulationGenerationSqlParams,
    PrepareSimulationGenerationSqlRow,
)
...
def _build_simulation_jinja_context(
    ...
    Resources are the current selections (from get_simulation_internal's ID resolution).
...
async def _simulation_generate_impl(
    ...
    """Handle simulation generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches simulation data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Step 1: Fetch simulation data (includes pre-fetched config resources)
        ...
        async with get_db_connection() as conn:
            context_params = GetSimulationGenerationContextSqlParams(
            ...
            if requests_per_day is not None and runs_today >= requests_per_day:
                ...
                logger.error(
                    f"Simulation generation rate limit exceeded - "
                    f"profile_id={profile_id}, agent_id={agent_id}, "
                    f"reason: {error_msg}"
                )
                await _emit_generation_error(
                    sid,
                    message=f"Failed to prepare simulation generation: {error_msg}",
                    group_id=result.group_id,
                )
                return
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareSimulationGenerationSqlParams(
                p_profile_id=profile_id,
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Simulation generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await _emit_generation_error(
                    sid,
                    message="Failed to prepare simulation generation: Unknown error",
            ...
            if config_id:
                ...
                config_view = {}
            draft_simulation_view = (
                result.views.draft_simulation.model_dump(mode="json")
                if result.views and result.views.draft_simulation
                else {}
            )
            jinja_context["views"] = {
                "config": config_view,
                "draft_simulation": draft_simulation_view,
            }
            ...
            # Emit simulation_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate simulation resources: {str(error)}")
        await _emit_generation_error(
            sid,
...
@server_router.post("/simulation_generation_started")
async def simulation_generation_started_api(
    request: SimulationGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Simulation generation started.
