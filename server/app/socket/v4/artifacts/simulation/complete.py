"""Simulation completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_simulation_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by simulation artifact_type."""
    if data.get("artifact_type") != "simulation":
        return
...
async def _handle_simulation_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle simulation generation run completion.
    ...
    3. If all agents done: emits simulation_generation_complete
