...
Caches run_id -> {chat_id, message_id} so that progress events can
look up the chat/message context without a DB query per delta.
...
def set_run_context(run_id: str, chat_id: str, message_id: str) -> None:
    """Cache run context when a run is created."""
    _run_contexts[run_id] = RunContext(chat_id=chat_id, message_id=message_id)
