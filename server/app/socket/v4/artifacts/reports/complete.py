"""Reports completion handler - handles run/text completion and multi-agent coordination.
...
from app.socket.v4.artifacts.reports.types import (
    ReportsGenerationCompleteEvent,
)
...
async def handle_reports_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by reports artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_reports_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_reports_run_complete(sid, data)
        return
...
async def _handle_reports_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle reports text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save reports text message: {str(e)}")


async def _handle_reports_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle reports generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits reports_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save reports run complete: {str(e)}")
    ...
    if is_complete:
        # Emit reports_generation_complete
        event = ReportsGenerationCompleteEvent(
