"""Reports generation router - unified handler for all reports resource types.

This module handles all business logic for reports generation:
...
from app.api.v4.artifacts.reports.get import get_reports_websocket
from app.api.v4.artifacts.reports.types import GetReportsWebsocketResponse
...
from app.socket.v4.artifacts.reports.types import (
    REPORTS_RESOURCE_TYPES,
    GenerateReportsPayload,
)
from app.socket.v4.artifacts.types import (
    GenerateErrorApiRequest,
    ReportsGenerationStartedEvent,
)
...
def _build_reports_jinja_context(
    response: GetReportsWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_reports_internal's ID resolution).
...
async def _reports_generate_impl(
    sid: str,
    data: GenerateReportsPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle reports generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches reports bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if REPORTS_RESOURCE_TYPES is populated)
        if REPORTS_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in REPORTS_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch reports bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_reports_websocket(
            pool=pool,
        ...
        reports_jinja_context = _build_reports_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Reports generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare reports generation: {error_msg}",
                    artifact_type="reports",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareReportsGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Reports generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare reports generation: Unknown error",
                        artifact_type="reports",
            ...
            jinja_context = reports_jinja_context
            ...
            # Emit reports_generation_started to client
            await sio.emit(
                "reports_generation_started",
                {
                    "artifact_type": "reports",
        ...
        logger.exception(f"Failed to generate reports resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate reports resources: {str(e)}",
                artifact_type="reports",
                group_id=None,
                resource_type="reports",
            ),
...
async def reports_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle reports_generate event (client-to-server)."""
    try:
        payload = GenerateReportsPayload(**data)
        ...
        await _reports_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("reports_generate")  # type: ignore
async def reports_generate_internal(data: dict[str, Any]) -> None:
    """Handle reports_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def reports_generation_started_api(
    ...
    Emitted when reports generation begins, listing resource types being generated.
    """
    return {"success": True}
