"""Test generation router - unified handler for all test entry types.

This module handles all business logic for test generation:
...
Follows the persona/generate.py and attempt/generate.py pattern.
"""
...
def _build_test_jinja_context(
    response: GetTestWebsocketResponse, entry_types: list[str]
) -> dict[str, Any]:
...
async def _test_generate_impl(
    sid: str, data: GenerateTestPayload, profile_id: uuid.UUID
) -> None:
    """Handle test generation with all business logic.

    This function:
    1. Validates entry_types against TEST_ENTRY_TYPES
    2. Fetches test data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        invalid_types = [et for et in entry_types if et not in TEST_ENTRY_TYPES]
        if invalid_types:
            await emit_to_internal(
                "generate_call_error",
                GenerateErrorApiRequest(
        ...
        test_jinja_context = _build_test_jinja_context(result, entry_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Test generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare test generation: {error_msg}",
                    artifact_type="test",
        ...
        async with get_db_connection() as conn:
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Test generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare test generation: Unknown error",
                        artifact_type="test",
            ...
            jinja_context = test_jinja_context
            ...
            # Emit test_generation_started to client
            await sio.emit(
            ...
            await internal_sio.emit(
                "generate_artifact",
                {
                    "sid": sid,
                    "artifact_type": "test",
                    "resource_type": entry_types[0] if entry_types else "test",
                    "run_id": str(run_id),
        ...
        logger.exception(f"Failed to generate test resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate test resources: {str(e)}",
                artifact_type="test",
                group_id=None,
                resource_type="test",
            ),
...
async def test_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle test_generate event (client-to-server)."""
    try:
        payload = GenerateTestPayload(**data)
        ...
        await _test_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("test_generate")  # type: ignore
async def test_generate_internal(data: dict[str, Any]) -> None:
    """Handle test_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
@client_router.post("/test/generate")
async def test_generate_api(
    request: GenerateTestPayload,
) -> dict[str, bool]:
    """Client-to-server event: test_generate.
...
@server_router.post("/test/generation_started")
async def test_generation_started_api(
    request: TestGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Test generation started.
