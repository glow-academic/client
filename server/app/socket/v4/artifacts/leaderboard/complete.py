"""Leaderboard completion handler - handles run/text completion and multi-agent coordination.
...
from app.socket.v4.artifacts.leaderboard.types import (
    LeaderboardGenerationCompleteEvent,
)
...
async def handle_leaderboard_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by leaderboard artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_leaderboard_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_leaderboard_run_complete(sid, data)
        return
...
async def _handle_leaderboard_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle leaderboard text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save leaderboard text message: {str(e)}")


async def _handle_leaderboard_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle leaderboard generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits leaderboard_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save leaderboard run complete: {str(e)}")
    ...
    if is_complete:
        # Emit leaderboard_generation_complete
        event = LeaderboardGenerationCompleteEvent(
            artifact_type="leaderboard",
            group_id=group_id_str or "",
            resource_type="leaderboard",
            run_id=run_id,
            success=True,
            message="Leaderboard generation completed",
        )

        await sio.emit(
            "leaderboard_generation_complete",
...
async def leaderboard_generation_complete_api(
    ...
    Emitted when all agents have finished generating leaderboard resources.
    """
    return {"success": True}
