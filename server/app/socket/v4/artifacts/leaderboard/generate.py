"""Leaderboard generation router - unified handler for all leaderboard resource types.

This module handles all business logic for leaderboard generation:
...
from app.api.v4.artifacts.leaderboard.get import get_leaderboard_websocket
from app.api.v4.artifacts.leaderboard.types import GetLeaderboardWebsocketResponse
...
from app.socket.v4.artifacts.leaderboard.types import (
    LEADERBOARD_RESOURCE_TYPES,
    GenerateLeaderboardPayload,
)
from app.socket.v4.artifacts.types import (
    GenerateErrorApiRequest,
    LeaderboardGenerationStartedEvent,
)
...
# SQL paths
SQL_PATH_PREPARE = "app/sql/v4/queries/generate/leaderboard/prepare_leaderboard_generation_complete.sql"
...
def _build_leaderboard_jinja_context(
    response: GetLeaderboardWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_leaderboard_internal's ID resolution).
...
async def _leaderboard_generate_impl(
    sid: str,
    data: GenerateLeaderboardPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle leaderboard generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches leaderboard bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if LEADERBOARD_RESOURCE_TYPES is populated)
        if LEADERBOARD_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in LEADERBOARD_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch leaderboard bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_leaderboard_websocket(
            pool=pool,
        ...
        leaderboard_jinja_context = _build_leaderboard_jinja_context(
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Leaderboard generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare leaderboard generation: {error_msg}",
                    artifact_type="leaderboard",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareLeaderboardGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Leaderboard generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare leaderboard generation: Unknown error",
                        artifact_type="leaderboard",
            ...
            jinja_context = leaderboard_jinja_context
            ...
            # Emit leaderboard_generation_started to client
            await sio.emit(
                "leaderboard_generation_started",
                {
                    "artifact_type": "leaderboard",
        ...
        logger.exception(f"Failed to generate leaderboard resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate leaderboard resources: {str(e)}",
                artifact_type="leaderboard",
                group_id=None,
                resource_type="leaderboard",
            ),
...
async def leaderboard_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle leaderboard_generate event (client-to-server)."""
    try:
        payload = GenerateLeaderboardPayload(**data)
        ...
        await _leaderboard_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("leaderboard_generate")  # type: ignore
async def leaderboard_generate_internal(data: dict[str, Any]) -> None:
    """Handle leaderboard_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def leaderboard_generation_started_api(
    ...
    Emitted when leaderboard generation begins, listing resource types being generated.
    """
    return {"success": True}
