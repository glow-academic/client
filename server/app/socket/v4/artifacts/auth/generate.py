...
from app.api.v4.artifacts.auth.types import GetAuthWebsocketResponse
    ...
    try:
        ...
        if not data.draft_id:
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message="draft_id is required for auth generation",
                    artifact_type="auth",
                    group_id=None,
                    resource_type="auth",
                ),
        ...
        # Step 1: Fetch pre-hydrated auth data
        ...
        if not agent_resource or not model_resource or not provider_resource:
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message="Incomplete auth generation configuration (agents/models/providers)",
                    artifact_type="auth",
                    group_id=None,
                    resource_type="auth",
                ),
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareAuthGenerationSqlParams(
                p_profile_id=profile_id,
            ...
            if not prepare_row.run_id:
                await emit_to_internal(
                    ...
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare auth generation",
                        artifact_type="auth",
            ...
            draft_view = {}
            if result.views and result.views.draft_auth:
                draft_view = result.views.draft_auth.model_dump(mode="json")
            jinja_context["views"] = {
                "config": config_view,
                "draft_auth": draft_view,
            }
            ...
            # Emit auth_generation_started to client
            await sio.emit(
                "auth_generation_started",
                AuthGenerationStartedEvent(
                    artifact_type="auth",
...
@server_router.post("/auth_generation_started")
async def auth_generation_started_api(
    request: AuthGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Auth generation started.
