"""Setting completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.setting.save import save_setting_internal
...
async def handle_setting_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by setting artifact_type."""
...
async def _handle_setting_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle setting generation run completion.
    ...
    3. If all agents done: emits setting_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save setting if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_setting_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        setting_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save setting: {str(e)}")

        # Emit setting_generation_complete
        event = SettingGenerationCompleteEvent(
            ...
            run_id=run_id,
            success=True,
            message="Setting generation completed",
