"""Setting completion handler - listens to generate_call_complete events and emits granular setting events."""
...
from app.socket.v4.artifacts.setting.types import SettingGenerationCompleteEvent
...
async def handle_setting_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by setting artifact_type and emit granular event."""

    artifact_type = data.get("artifact_type")
    if artifact_type != "setting":
        return
    ...
    if event_type == "text_complete":
        await _handle_setting_text_complete(sid, data)
        return

    # Handle run complete - save final assistant content + update tokens
    if event_type == "run_complete":
        await _handle_setting_run_complete(sid, data)
        return
    ...
    event = SettingGenerationCompleteEvent(
        artifact_type="setting",
        group_id=group_id_str,
        resource_type=resource_type,
    ...
    try:
        async with get_db_connection() as conn:
            if resource_type == "names":
                ...
                event.color_resources = items if items else None
                ...
                items = await get_profiles_internal(conn, [resource_id])
                event.profile_resources = items if items else None
            elif resource_type == "auths":
                items = await get_auths_internal(conn, [resource_id])
                event.auth_resources = items if items else None
            elif resource_type == "provider_keys":
                items = await get_provider_keys_internal(conn, [resource_id])
                event.provider_key_resources = items if items else None
            elif resource_type == "auth_item_keys":
                ...
                event.role_route_resources = items if items else None
...
async def _handle_setting_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle setting text generation completion - save assistant message."""
    run_id = data.get("run_id")
    ...
    try:
        ...
        logger.exception(f"Failed to save setting text message: {str(e)}")


async def _handle_setting_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle setting generation run completion - save assistant output if present."""
    run_id = data.get("run_id")
    ...
    try:
        ...
        logger.exception(f"Failed to save setting run complete: {str(e)}")
...
async def setting_generation_complete_api(
    request: SettingGenerationCompleteEvent,
) -> dict[str, bool]:
    """Server-to-client event: Setting generation completed.
