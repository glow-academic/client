"""Group completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_group_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by group artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_group_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_group_run_complete(sid, data)
        return
...
async def _handle_group_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle group text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save group text message: {str(e)}")


async def _handle_group_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle group generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits group_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save group run complete: {str(e)}")
    ...
    if is_complete:
        # Emit group_generation_complete
        event = GroupGenerationCompleteEvent(
            artifact_type="group",
            group_id=group_id_str or "",
            resource_type="group",
            run_id=run_id,
            success=True,
            message="Group generation completed",
        )

        await sio.emit(
            "group_generation_complete",
...
async def group_generation_complete_api(
    ...
    Emitted when all agents have finished generating group resources.
    """
    return {"success": True}
