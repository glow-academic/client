"""Group progress handler - emits percentage-based progress as resources complete.

Listens on generate_call_complete, filters for group artifact with tool_result events
that have a resource_id (successful resource creation), and emits group_generation_progress
...
from app.socket.v4.artifacts.group.types import GroupGenerationProgressEvent
...
async def handle_group_resource_progress(data: dict[str, Any]) -> None:
    """Track resource completions and emit group progress percentage."""
    ...
    event = GroupGenerationProgressEvent(
...
async def group_generation_progress_api(
    request: GroupGenerationProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Group generation progress.

    Emitted as individual resources complete during group generation.
