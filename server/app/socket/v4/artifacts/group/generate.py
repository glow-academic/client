"""Group generation router - unified handler for all group resource types.

This module handles all business logic for group generation:
...
from app.api.v4.artifacts.group.get import get_group_websocket
from app.api.v4.artifacts.group.types import GetGroupWebsocketResponse
...
from app.socket.v4.artifacts.group.types import (
    GROUP_RESOURCE_TYPES,
    GenerateGroupPayload,
)
from app.socket.v4.artifacts.types import (
    GenerateErrorApiRequest,
    GroupGenerationStartedEvent,
)
...
def _build_group_jinja_context(
    response: GetGroupWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_group_internal's ID resolution).
...
async def _group_generate_impl(
    sid: str,
    data: GenerateGroupPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle group generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches group bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if GROUP_RESOURCE_TYPES is populated)
        if GROUP_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in GROUP_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch group bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_group_websocket(
            pool=pool,
        ...
        group_jinja_context = _build_group_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Group generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare group generation: {error_msg}",
                    artifact_type="group",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareGroupGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Group generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare group generation: Unknown error",
                        artifact_type="group",
            ...
            jinja_context = group_jinja_context
            ...
            # Emit group_generation_started to client
            await sio.emit(
                "group_generation_started",
                {
                    "artifact_type": "group",
        ...
        logger.exception(f"Failed to generate group resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate group resources: {str(e)}",
                artifact_type="group",
                group_id=None,
                resource_type="group",
            ),
...
async def group_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle group_generate event (client-to-server)."""
    try:
        payload = GenerateGroupPayload(**data)
        ...
        await _group_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("group_generate")  # type: ignore
async def group_generate_internal(data: dict[str, Any]) -> None:
    """Handle group_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def group_generation_started_api(
    ...
    Emitted when group generation begins, listing resource types being generated.
    """
    return {"success": True}
