"""Artifact generation permission helpers.

Business logic for validating artifact generation prerequisites.
SQL fetches raw data, Python applies the rules.

This module provides shared permission validation that can be used
by any artifact type (persona, simulation, etc.).
"""
...
class GenerationContext:
    ...
    This dataclass holds the prerequisite data needed to validate
    whether generation can proceed. SQL queries should populate this
...
def validate_generation_access(ctx: GenerationContext) -> tuple[bool, list[str]]:
    """Validate all prerequisites for artifact generation.

    Checks:
    1. Agent exists
    2. Agent is active
    3. Agent has model configured
    4. Model has provider configured
    5. Provider has API key
    6. Rate limit not exceeded

    Args:
        ctx: GenerationContext populated from SQL
...
def format_generation_error(failures: list[str]) -> str:
    ...
    Returns:
        Semicolon-separated error message, or "Unknown error" if empty
    """
    if not failures:
