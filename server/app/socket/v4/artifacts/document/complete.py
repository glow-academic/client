"""Document completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.document.save import save_document_internal
...
async def handle_document_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by document artifact_type."""
...
async def _handle_document_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle document generation run completion.
    ...
    3. If all agents done: emits document_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save document if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_document_internal(
                        conn=conn,
                ...
                logger.exception(f"Failed to auto-save document: {str(e)}")

        # Emit document_generation_complete
