"""Cohort progress handler - listens to resource_progress events and emits cohort-specific events."""
...
from app.sql.types import (ValidateCohortResourceProgressSqlParams,
...
SQL_PATH = "app/sql/v4/queries/cohorts/validate_cohort_resource_progress_complete.sql"


@internal_sio.on("resource_progress")  # type: ignore
async def handle_cohorts_progress(data: dict[str, Any]) -> None:
    """Handle resource_progress internal event - filter by cohort artifact_type and emit cohort-specific event."""
    ...
    try:
        async with get_db_connection() as conn:
            params = ValidateCohortResourceProgressSqlParams(
    ...
    # Emit cohort-specific progress event with all fields from internal event
    await sio.emit(
        "cohort_generation_progress",
        {
