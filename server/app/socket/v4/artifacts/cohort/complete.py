"""Cohort completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.cohort.save import save_cohort_internal
...
async def _handle_cohort_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle cohort generation run completion.
    ...
    3. If all agents done: emits cohort_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save cohort if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_cohort_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        cohort_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save cohort: {str(e)}")

        # Emit cohort_generation_complete
...
async def cohort_generation_complete_api(
    ...
    """Server-to-client event: Cohort generation completed."""
    return {"success": True}
