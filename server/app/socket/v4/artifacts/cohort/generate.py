...
This module handles all business logic for cohort generation:
...
from app.socket.v4.artifacts.cohort.types import (
    COHORT_RESOURCE_TYPES,
    GenerateCohortPayload,
)
...
def _build_cohort_jinja_context(
    response: GetCohortWebsocketResponse,
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_cohort_internal's ID resolution).
...
async def _cohort_generate_impl(
    ...
    """Handle cohort generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches cohort data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        if not data.draft_id:
            await _emit_generation_error(
                sid, message="Draft ID is required for cohort generation"
            )
            return
        ...
        # Step 1: Fetch cohort data (includes pre-fetched config resources)
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareCohortGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Cohort generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await _emit_generation_error(
                    sid,
                    message="Failed to prepare cohort generation: Unknown error",
            ...
            jinja_context = cohort_jinja_context
            ...
            if config_id:
                ...
                config_view = {}
            draft_cohort_view = (
                result.views.draft_cohort.model_dump(mode="json")
                if result.views and result.views.draft_cohort
                else {}
            )
            jinja_context["views"] = {
                "config": config_view,
                "draft_cohort": draft_cohort_view,
            }
            ...
            # Emit cohort_generation_started to client
            await sio.emit(
                "cohort_generation_started",
                {
                    "artifact_type": "cohort",
        ...
        logger.exception(f"Failed to generate cohort resources: {str(e)}")
        await _emit_generation_error(
            sid,
            message=f"Failed to generate cohort resources: {str(e)}",
            resource_types=data.resource_types
...
async def cohort_generate(sid: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        await _emit_generation_error(sid, message=f"Invalid request: {str(e)}")
...
@server_router.post("/cohort_generation_started")
async def cohort_generation_started_api(
    request: CohortGenerationStartedEvent,
) -> dict[str, bool]:
    """Server-to-client event: Cohort generation started.
