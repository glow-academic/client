"""Session completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_session_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by session artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_session_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_session_run_complete(sid, data)
        return
...
async def _handle_session_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle session text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save session text message: {str(e)}")


async def _handle_session_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle session generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits session_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save session run complete: {str(e)}")
    ...
    if is_complete:
        # Emit session_generation_complete
        event = SessionGenerationCompleteEvent(
...
async def session_generation_complete_api(
    ...
    Emitted when all agents have finished generating session resources.
    """
    return {"success": True}
