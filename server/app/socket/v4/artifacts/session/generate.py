"""Session generation router - unified handler for all session resource types.

This module handles all business logic for session generation:
...
from app.api.v4.artifacts.session.get import get_session_websocket
from app.api.v4.artifacts.session.types import GetSessionWebsocketResponse
...
from app.socket.v4.artifacts.session.types import (
    SESSION_RESOURCE_TYPES,
...
def _build_session_jinja_context(
    response: GetSessionWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_session_internal's ID resolution).
...
async def _session_generate_impl(
    sid: str,
    data: GenerateSessionPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle session generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches session bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if SESSION_RESOURCE_TYPES is populated)
        if SESSION_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in SESSION_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch session bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_session_websocket(
            pool=pool,
        ...
        session_jinja_context = _build_session_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Session generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare session generation: {error_msg}",
                    artifact_type="session",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareSessionGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Session generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare session generation: Unknown error",
                        artifact_type="session",
            ...
            jinja_context = session_jinja_context
            ...
            # Emit session_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate session resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate session resources: {str(e)}",
                artifact_type="session",
                group_id=None,
                resource_type="session",
            ),
...
async def session_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle session_generate event (client-to-server)."""
    try:
        payload = GenerateSessionPayload(**data)
        ...
        await _session_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("session_generate")  # type: ignore
async def session_generate_internal(data: dict[str, Any]) -> None:
    """Handle session_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def session_generation_started_api(
    ...
    Emitted when session generation begins, listing resource types being generated.
    """
    return {"success": True}
