"""Activity progress handler - emits percentage-based progress as resources complete.

Listens on generate_call_complete, filters for activity artifact with tool_result events
that have a resource_id (successful resource creation), and emits activity_generation_progress
...
from app.socket.v4.artifacts.activity.types import ActivityGenerationProgressEvent
...
async def handle_activity_resource_progress(data: dict[str, Any]) -> None:
    """Track resource completions and emit activity progress percentage."""
    ...
    event = ActivityGenerationProgressEvent(
...
async def activity_generation_progress_api(
    request: ActivityGenerationProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Activity generation progress.

    Emitted as individual resources complete during activity generation.
