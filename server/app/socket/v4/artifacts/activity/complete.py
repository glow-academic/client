"""Activity completion handler - handles run/text completion and multi-agent coordination.
...
from app.socket.v4.artifacts.activity.types import (
    ActivityGenerationCompleteEvent,
)
...
async def handle_activity_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by activity artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_activity_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_activity_run_complete(sid, data)
        return
...
async def _handle_activity_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle activity text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save activity text message: {str(e)}")


async def _handle_activity_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle activity generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits activity_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save activity run complete: {str(e)}")
    ...
    if is_complete:
        # Emit activity_generation_complete
        event = ActivityGenerationCompleteEvent(
...
async def activity_generation_complete_api(
    ...
    Emitted when all agents have finished generating activity resources.
    """
    return {"success": True}
