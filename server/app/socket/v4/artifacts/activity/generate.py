"""Activity generation router - unified handler for all activity resource types.

This module handles all business logic for activity generation:
...
from app.api.v4.artifacts.activity.get import get_activity_websocket
from app.api.v4.artifacts.activity.types import GetActivityWebsocketResponse
...
from app.socket.v4.artifacts.activity.types import (
    ACTIVITY_RESOURCE_TYPES,
    GenerateActivityPayload,
)
from app.socket.v4.artifacts.types import (
    ActivityGenerationStartedEvent,
...
def _build_activity_jinja_context(
    response: GetActivityWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_activity_internal's ID resolution).
...
async def _activity_generate_impl(
    sid: str,
    data: GenerateActivityPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle activity generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches activity bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if ACTIVITY_RESOURCE_TYPES is populated)
        if ACTIVITY_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in ACTIVITY_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch activity bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_activity_websocket(
            pool=pool,
        ...
        activity_jinja_context = _build_activity_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Activity generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare activity generation: {error_msg}",
                    artifact_type="activity",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareActivityGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Activity generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare activity generation: Unknown error",
                        artifact_type="activity",
            ...
            jinja_context = activity_jinja_context
            ...
            # Emit activity_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate activity resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate activity resources: {str(e)}",
                artifact_type="activity",
                group_id=None,
                resource_type="activity",
            ),
...
async def activity_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle activity_generate event (client-to-server)."""
    try:
        payload = GenerateActivityPayload(**data)
        ...
        await _activity_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("activity_generate")  # type: ignore
async def activity_generate_internal(data: dict[str, Any]) -> None:
    """Handle activity_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def activity_generation_started_api(
    ...
    Emitted when activity generation begins, listing resource types being generated.
    """
    return {"success": True}
