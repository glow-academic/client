"""Parameter completion handler - emits parameter-specific generation complete events."""
...
async def handle_parameter_artifact_complete(data: dict[str, Any]) -> None:
    ...
    try:
        async with get_db_connection() as conn:
            request_payload = GetParameterApiRequest.model_validate(
                {"parameter_id": uuid.UUID(artifact_id_str)}
            )
    ...
    values = getattr(result, "field_ids", None) or []
...
async def parameter_generation_complete_api(request: dict[str, Any]) -> dict[str, bool]:
    _ = request
    return {"ok": True}
