...
from app.api.v4.artifacts.parameter.save import save_parameter_internal
...
async def _handle_parameter_run_complete(sid: str, data: dict[str, Any]) -> None:
    ...
    if is_complete:
        # All agents finished - auto-save parameter if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_parameter_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        parameter_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save parameter: {str(e)}")

        # Emit parameter_generation_complete
