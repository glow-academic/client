"""Parameter completion handler - listens to generate_call_complete events and emits granular parameter events."""
...
async def handle_parameter_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by parameter artifact_type and emit granular event."""
    ...
    try:
        async with get_db_connection() as conn:
            ...
            elif resource_type in ("fields", "parameter_fields"):
...
async def _handle_parameter_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle parameter generation run completion - save assistant output if present."""
    run_id = data.get("run_id")
