"""Profile completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.profile.save import save_profile_internal
...
async def handle_profile_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by profile artifact_type."""
    if data.get("artifact_type") != "profile":
        return
...
async def _handle_profile_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle profile generation run completion.
    ...
    3. If all agents done: emits profile_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save profile if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_profile_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        saved_profile_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save profile: {str(e)}")

        # Emit profile_generation_complete
