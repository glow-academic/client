"""Pricing progress handler - emits percentage-based progress as resources complete.

Listens on generate_call_complete, filters for pricing artifact with tool_result events
that have a resource_id (successful resource creation), and emits pricing_generation_progress
...
from app.socket.v4.artifacts.pricing.types import PricingGenerationProgressEvent
...
async def handle_pricing_resource_progress(data: dict[str, Any]) -> None:
    """Track resource completions and emit pricing progress percentage."""
    ...
    event = PricingGenerationProgressEvent(
...
async def pricing_generation_progress_api(
    request: PricingGenerationProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Pricing generation progress.

    Emitted as individual resources complete during pricing generation.
