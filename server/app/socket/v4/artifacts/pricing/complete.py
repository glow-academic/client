"""Pricing completion handler - handles run/text completion and multi-agent coordination.
...
from app.socket.v4.artifacts.pricing.types import (
    PricingGenerationCompleteEvent,
)
...
async def handle_pricing_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by pricing artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_pricing_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_pricing_run_complete(sid, data)
        return
...
async def _handle_pricing_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle pricing text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save pricing text message: {str(e)}")


async def _handle_pricing_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle pricing generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits pricing_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save pricing run complete: {str(e)}")
    ...
    if is_complete:
        # Emit pricing_generation_complete
        event = PricingGenerationCompleteEvent(
            artifact_type="pricing",
            group_id=group_id_str or "",
            resource_type="pricing",
            run_id=run_id,
            success=True,
            message="Pricing generation completed",
        )

        await sio.emit(
            "pricing_generation_complete",
