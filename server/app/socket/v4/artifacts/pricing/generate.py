"""Pricing generation router - unified handler for all pricing resource types.

This module handles all business logic for pricing generation:
...
from app.api.v4.artifacts.pricing.get import get_pricing_websocket
from app.api.v4.artifacts.pricing.types import GetPricingWebsocketResponse
...
from app.socket.v4.artifacts.pricing.types import (
    PRICING_RESOURCE_TYPES,
    GeneratePricingPayload,
)
from app.socket.v4.artifacts.types import (
    GenerateErrorApiRequest,
    PricingGenerationStartedEvent,
)
...
def _build_pricing_jinja_context(
    response: GetPricingWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_pricing_internal's ID resolution).
...
async def _pricing_generate_impl(
    sid: str,
    data: GeneratePricingPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle pricing generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches pricing bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if PRICING_RESOURCE_TYPES is populated)
        if PRICING_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in PRICING_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch pricing bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_pricing_websocket(
            pool=pool,
        ...
        pricing_jinja_context = _build_pricing_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Pricing generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare pricing generation: {error_msg}",
                    artifact_type="pricing",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PreparePricingGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Pricing generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare pricing generation: Unknown error",
                        artifact_type="pricing",
            ...
            jinja_context = pricing_jinja_context
            ...
            # Emit pricing_generation_started to client
            await sio.emit(
                "pricing_generation_started",
                {
                    "artifact_type": "pricing",
        ...
        logger.exception(f"Failed to generate pricing resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate pricing resources: {str(e)}",
                artifact_type="pricing",
                group_id=None,
                resource_type="pricing",
            ),
...
async def pricing_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle pricing_generate event (client-to-server)."""
    try:
        payload = GeneratePricingPayload(**data)
        ...
        await _pricing_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("pricing_generate")  # type: ignore
async def pricing_generate_internal(data: dict[str, Any]) -> None:
    """Handle pricing_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def pricing_generation_started_api(
    ...
    Emitted when pricing generation begins, listing resource types being generated.
    """
    return {"success": True}
