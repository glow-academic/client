"""Record completion handler - handles run/text completion and multi-agent coordination.
...
async def handle_record_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by record artifact_type."""
    ...
    if event_type == "text_complete":
        await _handle_record_text_complete(sid, data)
        return

    # Handle run complete - coordinate multi-agent, emit completion
    if event_type == "run_complete":
        await _handle_record_run_complete(sid, data)
        return
...
async def _handle_record_text_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle record text generation completion - save assistant message."""
    ...
    try:
        ...
        logger.exception(f"Failed to save record text message: {str(e)}")


async def _handle_record_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle record generation run completion.

    Coordinates multi-agent completion via generation_tracker:
    1. Records this agent's completion
    2. If all agents done: emits record_generation_complete
    ...
    try:
        ...
        logger.exception(f"Failed to save record run complete: {str(e)}")
    ...
    if is_complete:
        # Emit record_generation_complete
        event = RecordGenerationCompleteEvent(
            artifact_type="record",
            group_id=group_id_str or "",
            resource_type="record",
            run_id=run_id,
            success=True,
            message="Record generation completed",
        )
...
async def record_generation_complete_api(
    ...
    Emitted when all agents have finished generating record resources.
    """
    return {"success": True}
