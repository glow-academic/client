"""Record generation router - unified handler for all record resource types.

This module handles all business logic for record generation:
...
from app.api.v4.artifacts.record.get import get_record_websocket
from app.api.v4.artifacts.record.types import GetRecordWebsocketResponse
...
from app.socket.v4.artifacts.record.types import (
...
def _build_record_jinja_context(
    response: GetRecordWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
    """Build Jinja context with resources as top-level variables.

    Resources are the current selections (from get_record_internal's ID resolution).
...
async def _record_generate_impl(
    sid: str,
    data: GenerateRecordPayload,
    profile_id: uuid.UUID,
) -> None:
    """Handle record generation with all business logic.

    This function:
    1. Validates resource_types and resolves agent_id from domain mappings
    2. Fetches record bundle data via internal function (includes pre-fetched config resources)
    ...
    try:
        ...
        # Validate resource_types against allowed list (if RECORD_RESOURCE_TYPES is populated)
        if RECORD_RESOURCE_TYPES:
            invalid_types = [
                rt for rt in resource_types if rt not in RECORD_RESOURCE_TYPES
            ]
            if invalid_types:
                await emit_to_internal(
                    "generate_call_error",
        ...
        # Step 1: Fetch record bundle data (includes pre-fetched config resources)
        pool = get_pool()
        if not pool:
            raise RuntimeError("Database pool not initialized")

        result = await get_record_websocket(
            pool=pool,
        ...
        record_jinja_context = _build_record_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Record generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare record generation: {error_msg}",
                    artifact_type="record",
        ...
        async with get_db_connection() as conn:
            ...
            prepare_params = PrepareRecordGenerationSqlParams(
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Record generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare record generation: Unknown error",
                        artifact_type="record",
            ...
            jinja_context = record_jinja_context
            ...
            # Emit record_generation_started to client
            await sio.emit(
        ...
        logger.exception(f"Failed to generate record resources: {str(e)}")
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
                sid=sid,
                error_message=f"Failed to generate record resources: {str(e)}",
                artifact_type="record",
                group_id=None,
                resource_type="record",
            ),
...
async def record_generate(sid: str, data: dict[str, Any]) -> None:
    """Handle record_generate event (client-to-server)."""
    try:
        payload = GenerateRecordPayload(**data)
        ...
        await _record_generate_impl(sid, payload, profile_id)
    except Exception as e:
        await emit_to_internal(
            "generate_call_error",
            GenerateErrorApiRequest(
...
@internal_sio.on("record_generate")  # type: ignore
async def record_generate_internal(data: dict[str, Any]) -> None:
    """Handle record_generate event from internal bus (server-to-server)."""
    try:
        sid = data.get("sid", "")
        if not sid:
            return
...
async def record_generation_started_api(
    ...
    Emitted when record generation begins, listing resource types being generated.
    """
    return {"success": True}
