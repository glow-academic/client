"""Practice generation handler — canonical pattern with independent config chain.
...
3. Emit practice_generation_started with attempt_id
4. Call get_practice_websocket() for config + resources (independent config chain)
...
from app.api.v4.artifacts.practice.get import get_practice_websocket
from app.api.v4.artifacts.practice.types import GetPracticeWebsocketResponse
...
def _build_practice_jinja_context(
    response: GetPracticeWebsocketResponse, resource_types: list[str]
) -> dict[str, Any]:
...
async def _practice_generate_impl(
    ...
    """Handle practice generation with full canonical pattern."""
    try:
        ...
        # Emit practice_generation_started to client
        ...
        result = await get_practice_websocket(
            pool=pool,
        ...
        practice_jinja_context = _build_practice_jinja_context(result, resource_types)
        ...
        if requests_per_day is not None and runs_today >= requests_per_day:
            ...
            logger.error(
                f"Practice generation rate limit exceeded - "
            ...
            await emit_to_internal(
                ...
                GenerateErrorApiRequest(
                    sid=sid,
                    error_message=f"Failed to prepare practice generation: {error_msg}",
                    artifact_type="practice",
        ...
        async with get_db_connection() as conn:
            ...
            if not prepare_row.run_id:
                logger.error(
                    f"Practice generation preparation failed unexpectedly - "
                    f"profile_id={profile_id}, agent_id={agent_id}"
                )
                await emit_to_internal(
                    "generate_call_error",
                    GenerateErrorApiRequest(
                        sid=sid,
                        error_message="Failed to prepare practice generation: Unknown error",
                        artifact_type="practice",
            ...
            jinja_context = practice_jinja_context
