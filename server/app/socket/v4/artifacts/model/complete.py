"""Model completion handler - handles run/text completion and multi-agent coordination.
...
from app.api.v4.artifacts.model.save import save_model_internal
...
async def handle_model_artifact_complete(data: dict[str, Any]) -> None:
    """Handle generate_call_complete and generate_text_complete events - filter by model artifact_type."""
...
async def _handle_model_run_complete(sid: str, data: dict[str, Any]) -> None:
    """Handle model generation run completion.
    ...
    3. If all agents done: emits model_generation_complete
    ...
    if is_complete:
        # All agents finished - auto-save model if save=True (default)
        ...
        if should_save and profile_id_str and group_id_str:
            try:
                ...
                async with get_db_connection() as conn:
                    saved_id = await save_model_internal(
                        conn=conn,
                    ...
                    if saved_id:
                        model_id = str(saved_id)
            except Exception as e:
                logger.exception(f"Failed to auto-save model: {str(e)}")

        # Emit model_generation_complete
