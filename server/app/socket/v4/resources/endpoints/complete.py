"""Endpoints resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle endpoints generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch endpoints/{resource_id}: {e}")
        return

    event = EndpointsGenerationCompleteEvent(
...
async def endpoints_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting endpoints."""
