"""Rubrics resource error handler."""
...
from app.socket.v4.resources.rubrics.types import RubricsGenerationErrorEvent
...
async def handle_error(data: dict[str, Any]) -> None:
    """Rubrics generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return

    event = RubricsGenerationErrorEvent(
...
async def rubrics_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting rubrics."""
...
async def rubrics_generation_error_api(
    request: RubricsGenerationErrorEvent,
) -> dict[str, bool]:
