"""Rubricsubrics resource completion handler."""
...
from app.socket.v4.resources.rubrics.types import RubricsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle rubrics generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
...
async def rubrics_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting rubrics."""
...
async def rubrics_generation_complete_api(
    request: RubricsGenerationEvent,
) -> dict[str, bool]:
