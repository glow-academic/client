"""ScenarioFlags resource completion handler."""
...
from app.socket.v4.resources.scenario_flags.types import (
    ScenarioFlagsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle scenario_flags generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_scenario_flags_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch scenario_flags/{resource_id}: {e}")
        return

    event = ScenarioFlagsGenerationCompleteEvent(
    ...
    await sio.emit(
        "scenario_flags_generation_complete",
...
async def scenario_flags_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting scenario_flags."""
