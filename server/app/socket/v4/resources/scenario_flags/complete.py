...
from app.socket.v4.resources.scenario_flags.types import ScenarioFlagsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle scenario_flags generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = ScenarioFlagsGenerationEvent(
...
async def scenario_flags_generation_complete_api(
    request: ScenarioFlagsGenerationEvent,
) -> dict[str, bool]:
