"""Questions resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle questions generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch questions/{resource_id}: {e}")
        return

    event = QuestionsGenerationCompleteEvent(
