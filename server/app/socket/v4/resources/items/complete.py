"""Itemstems resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle items generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
...
async def items_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting items."""
