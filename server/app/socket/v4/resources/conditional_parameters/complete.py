"""ConditionalParameters resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle conditional_parameters generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_conditional_parameters_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch conditional_parameters/{resource_id}: {e}")
        return

    event = ConditionalParametersGenerationCompleteEvent(
    ...
    await sio.emit(
        "conditional_parameters_generation_complete",
...
async def conditional_parameters_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting conditional_parameters."""
