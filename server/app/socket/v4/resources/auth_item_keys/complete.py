"""AuthItemKeys resource completion handler."""
...
from app.socket.v4.resources.auth_item_keys.types import (
    AuthItemKeysGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle auth_item_keys generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch auth_item_keys/{resource_id}: {e}")
        return

    event = AuthItemKeysGenerationCompleteEvent(
    ...
    await sio.emit(
        "auth_item_keys_generation_complete",
...
async def auth_item_keys_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting auth_item_keys."""
