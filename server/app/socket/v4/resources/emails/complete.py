"""Emails resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle emails generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch emails/{resource_id}: {e}")
        return

    event = EmailsGenerationCompleteEvent(
...
async def emails_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting emails."""
