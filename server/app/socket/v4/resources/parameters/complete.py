"""Parametersarameters resource completion handler."""
...
from app.socket.v4.resources.parameters.types import ParametersGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle parameters generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = ParametersGenerationEvent(
...
async def parameters_generation_complete_api(
    request: ParametersGenerationEvent,
) -> dict[str, bool]:
