"""Personasersonas resource completion handler."""
...
from app.socket.v4.resources.personas.types import PersonasGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle personas generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
...
async def personas_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting personas."""
...
async def personas_generation_complete_api(
    request: PersonasGenerationEvent,
) -> dict[str, bool]:
