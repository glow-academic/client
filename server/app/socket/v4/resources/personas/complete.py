"""Personas resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle personas generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch personas/{resource_id}: {e}")
        return

    event = PersonasGenerationCompleteEvent(
