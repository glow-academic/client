"""Simulations resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle simulations generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch simulations/{resource_id}: {e}")
        return

    event = SimulationsGenerationCompleteEvent(
