"""Simulationsimulations resource completion handler."""
...
from app.socket.v4.resources.simulations.types import SimulationsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle simulations generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = SimulationsGenerationEvent(
    ...
    await sio.emit(
        "simulations_generation_complete",
...
async def simulations_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting simulations."""
...
async def simulations_generation_complete_api(
    request: SimulationsGenerationEvent,
) -> dict[str, bool]:
