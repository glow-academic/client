"""ScenarioPositions resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle scenario_positions generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_scenario_positions_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch scenario_positions/{resource_id}: {e}")
        return

    event = ScenarioPositionsGenerationCompleteEvent(
...
async def scenario_positions_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting scenario_positions."""
