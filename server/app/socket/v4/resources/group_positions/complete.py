...
from app.socket.v4.resources.group_positions.types import GroupPositionsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle group_positions generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = GroupPositionsGenerationEvent(
...
async def group_positions_generation_complete_api(
    request: GroupPositionsGenerationEvent,
) -> dict[str, bool]:
