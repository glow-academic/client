...
from app.socket.v4.resources.videos.types import VideosGenerationErrorEvent
...
async def handle_error(data: dict[str, Any]) -> None:
    """Videos generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return

    event = VideosGenerationErrorEvent(
...
async def videos_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting videos."""
...
async def videos_generation_error_api(
    request: VideosGenerationErrorEvent,
) -> dict[str, bool]:
