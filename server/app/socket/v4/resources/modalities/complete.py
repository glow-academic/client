"""Modalitiesodalities resource completion handler."""
...
from app.socket.v4.resources.modalities.types import ModalitiesGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle modalities generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = ModalitiesGenerationEvent(
    ...
    await sio.emit(
        "modalities_generation_complete",
...
async def modalities_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting modalities."""
...
async def modalities_generation_complete_api(
    request: ModalitiesGenerationEvent,
) -> dict[str, bool]:
