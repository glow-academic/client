"""Modalities resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle modalities generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch modalities/{resource_id}: {e}")
        return

    event = ModalitiesGenerationCompleteEvent(
