...
from app.socket.v4.resources.arg_positions.types import ArgPositionsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle arg_positions generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = ArgPositionsGenerationEvent(
...
async def arg_positions_generation_complete_api(
    request: ArgPositionsGenerationEvent,
) -> dict[str, bool]:
