"""ArgPositions resource completion handler."""
...
from app.socket.v4.resources.arg_positions.types import (
    ArgPositionsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle arg_positions generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch arg_positions/{resource_id}: {e}")
        return

    event = ArgPositionsGenerationCompleteEvent(
    ...
    await sio.emit(
        "arg_positions_generation_complete",
...
async def arg_positions_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting arg_positions."""
