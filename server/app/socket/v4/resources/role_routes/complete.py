"""RoleRoutes resource completion handler."""
...
from app.socket.v4.resources.role_routes.types import RoleRoutesGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle role_routes generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = RoleRoutesGenerationEvent(
    ...
    await sio.emit(
        "role_routes_generation_complete",
...
async def role_routes_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting role_routes."""
...
async def role_routes_generation_complete_api(
    request: RoleRoutesGenerationEvent,
) -> dict[str, bool]:
