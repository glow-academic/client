...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle role_routes generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch role_routes/{resource_id}: {e}")
        return

    event = RoleRoutesGenerationCompleteEvent(
