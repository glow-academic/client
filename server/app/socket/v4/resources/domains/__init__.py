"""Domains resource socket events - OpenAPI schema endpoints."""

from fastapi import APIRouter

from app.socket.v4.resources.domains.types import DomainsGenerationCompleteEvent
from app.socket.v4.resources.types import (
    ResourceErrorEvent,
    ResourceProgressEvent,
    ResourceStartEvent,
)

server_router = APIRouter()


@server_router.post("/domains_generation_complete")
async def domains_generation_complete_api(
    request: DomainsGenerationCompleteEvent,
) -> dict[str, bool]:
    """Server-to-client event: Domains generation completed."""
    return {"success": True}


@server_router.post("/domains_generation_started")
async def domains_generation_started_api(
    request: ResourceStartEvent,
) -> dict[str, bool]:
    """Server-to-client event: Domains generation started."""
    return {"success": True}


@server_router.post("/domains_generation_progress")
async def domains_generation_progress_api(
    request: ResourceProgressEvent,
) -> dict[str, bool]:
    """Server-to-client event: Domains generation progress."""
    return {"success": True}


@server_router.post("/domains_generation_error")
async def domains_generation_error_api(
    request: ResourceErrorEvent,
) -> dict[str, bool]:
    """Server-to-client event: Domains generation error."""
    return {"success": True}
