"""StandardGroups resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle standard_groups generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_standard_groups_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch standard_groups/{resource_id}: {e}")
        return

    event = StandardGroupsGenerationCompleteEvent(
    ...
    await sio.emit(
        "standard_groups_generation_complete",
...
async def standard_groups_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting standard_groups."""
