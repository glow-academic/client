"""ScenarioTimeLimits resource completion handler."""
...
from app.socket.v4.resources.scenario_time_limits.types import (
    ScenarioTimeLimitsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle scenario_time_limits generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_scenario_time_limits_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch scenario_time_limits/{resource_id}: {e}")
        return

    event = ScenarioTimeLimitsGenerationCompleteEvent(
...
async def scenario_time_limits_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting scenario_time_limits."""
    if data.get("event_type") != "tool_result":
        return
    if resolve_resource_type(data) != "scenario_time_limits":
        return
