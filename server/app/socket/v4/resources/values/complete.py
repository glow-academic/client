"""Values resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle values generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch values/{resource_id}: {e}")
        return

    event = ValuesGenerationCompleteEvent(
...
async def values_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting values."""
