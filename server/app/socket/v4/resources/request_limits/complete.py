"""RequestLimits resource completion handler."""
...
from app.socket.v4.resources.request_limits.types import (
    RequestLimitsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle request_limits generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch request_limits/{resource_id}: {e}")
        return

    event = RequestLimitsGenerationCompleteEvent(
    ...
    await sio.emit(
        "request_limits_generation_complete",
...
async def request_limits_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting request_limits."""
