"""RunRubrics resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle run_rubrics generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch run_rubrics/{resource_id}: {e}")
        return

    event = RunRubricsGenerationCompleteEvent(
    ...
    await sio.emit(
        "run_rubrics_generation_complete",
...
async def run_rubrics_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting run_rubrics."""
