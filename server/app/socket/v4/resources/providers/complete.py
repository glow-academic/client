"""Providers resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle providers generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_providers_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch providers/{resource_id}: {e}")
        return

    event = ProvidersGenerationCompleteEvent(
...
async def providers_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting providers."""
