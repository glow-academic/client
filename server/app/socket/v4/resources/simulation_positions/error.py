"""SimulationPositions resource error handler."""
...
from app.socket.v4.resources.simulation_positions.types import SimulationPositionsGenerationErrorEvent
...
async def handle_error(data: dict[str, Any]) -> None:
    """SimulationPositions generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
    ...
    await sio.emit(
        "simulation_positions_generation_error",
...
async def simulation_positions_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting simulation_positions."""
