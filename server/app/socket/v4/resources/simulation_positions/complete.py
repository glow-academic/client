"""SimulationPositions resource completion handler."""
...
from app.api.v4.resources.simulation_positions.get import get_simulation_positions_internal
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle simulation_positions generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch simulation_positions/{resource_id}: {e}")
        return

    event = SimulationPositionsGenerationCompleteEvent(
...
async def simulation_positions_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting simulation_positions."""
