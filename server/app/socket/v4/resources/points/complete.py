"""Points resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle points generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_points_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch points/{resource_id}: {e}")
        return

    event = PointsGenerationCompleteEvent(
...
async def points_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting points."""
    if data.get("event_type") != "tool_result":
        return
    if resolve_resource_type(data) != "points":
        return
