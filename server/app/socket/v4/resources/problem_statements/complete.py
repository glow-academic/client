"""ProblemStatements resource completion handler."""
...
from app.socket.v4.resources.problem_statements.types import (
    ProblemStatementsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle problem_statements generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch problem_statements/{resource_id}: {e}")
        return

    event = ProblemStatementsGenerationCompleteEvent(
    ...
    await sio.emit(
        "problem_statements_generation_complete",
...
async def problem_statements_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting problem_statements."""
