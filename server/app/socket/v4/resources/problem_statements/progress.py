"""ProblemStatements resource progress handler."""
...
from app.socket.v4.resources.problem_statements.types import ProblemStatementsGenerationProgressEvent
...
async def handle_progress(data: dict[str, Any]) -> None:
    """ProblemStatements generation progress - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
    ...
    await sio.emit(
        "problem_statements_generation_progress",
...
async def problem_statements_call_progress_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_delta events targeting problem_statements."""
