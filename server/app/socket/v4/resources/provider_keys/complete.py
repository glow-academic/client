"""ProviderKeys resource completion handler."""
...
from app.socket.v4.resources.provider_keys.types import (
    ProviderKeysGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle provider_keys generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch provider_keys/{resource_id}: {e}")
        return

    event = ProviderKeysGenerationCompleteEvent(
    ...
    await sio.emit(
        "provider_keys_generation_complete",
...
async def provider_keys_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting provider_keys."""
