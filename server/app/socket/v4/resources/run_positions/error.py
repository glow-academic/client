"""RunPositions resource error handler."""
...
async def handle_error(data: dict[str, Any]) -> None:
    """RunPositions generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
    ...
    await sio.emit(
        "run_positions_generation_error",
...
async def run_positions_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting run_positions."""
