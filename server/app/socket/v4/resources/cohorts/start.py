...
from app.socket.v4.resources.cohorts.types import CohortsGenerationStartedEvent
...
async def handle_start(data: dict[str, Any]) -> None:
    ...
    event = CohortsGenerationStartedEvent(
...
async def cohorts_call_start_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_start events targeting cohorts."""
...
async def cohorts_generation_started_api(
    request: CohortsGenerationStartedEvent,
) -> dict[str, bool]:
