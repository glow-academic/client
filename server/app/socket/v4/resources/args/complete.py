"""Argsrgs resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle args generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
...
async def args_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting args."""
...
@server_router.post("/args_generation_complete")
async def args_generation_complete_api(
    request: ArgsGenerationEvent,
) -> dict[str, bool]:
    """Server-to-client event: Args generation completed."""
    return {"success": True}
