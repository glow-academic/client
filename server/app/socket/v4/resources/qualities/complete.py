"""Qualitiesualities resource completion handler."""
...
from app.socket.v4.resources.qualities.types import QualitiesGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle qualities generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    ...
    event = QualitiesGenerationEvent(
...
async def qualities_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting qualities."""
...
async def qualities_generation_complete_api(
    request: QualitiesGenerationEvent,
) -> dict[str, bool]:
