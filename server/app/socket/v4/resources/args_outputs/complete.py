"""ArgsOutputs resource completion handler."""
...
from app.socket.v4.resources.args_outputs.types import (
    ArgsOutputsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle args_outputs generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch args_outputs/{resource_id}: {e}")
        return

    event = ArgsOutputsGenerationCompleteEvent(
    ...
    await sio.emit(
        "args_outputs_generation_complete",
...
async def args_outputs_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting args_outputs."""
