"""ArgsOutputs resource start handler."""
...
async def handle_start(data: dict[str, Any]) -> None:
    """ArgsOutputs generation started - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
    ...
    await sio.emit(
        "args_outputs_generation_started",
...
async def args_outputs_call_start_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_start events targeting args_outputs."""
    if data.get("event_type") != "tool_call_start":
        return
    if resolve_resource_type(data) != "args_outputs":
        return
