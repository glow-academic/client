"""GroupRubrics resource completion handler."""
...
from app.socket.v4.resources.group_rubrics.types import (
    GroupRubricsGenerationCompleteEvent,
)
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle group_rubrics generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch group_rubrics/{resource_id}: {e}")
        return

    event = GroupRubricsGenerationCompleteEvent(
    ...
    await sio.emit(
        "group_rubrics_generation_complete",
...
async def group_rubrics_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting group_rubrics."""
