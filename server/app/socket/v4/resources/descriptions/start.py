"""Descriptions resource start handler."""
...
from app.socket.v4.resources.descriptions.types import DescriptionsGenerationStartedEvent
...
async def handle_start(data: dict[str, Any]) -> None:
    """Descriptions generation started - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
    ...
    await sio.emit(
        "descriptions_generation_started",
...
async def descriptions_call_start_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_start events targeting descriptions."""
