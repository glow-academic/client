"""Descriptions resource completion handler."""
...
from app.socket.v4.resources.descriptions.types import DescriptionsGenerationCompleteEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle descriptions generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch descriptions/{resource_id}: {e}")
        return

    event = DescriptionsGenerationCompleteEvent(
    ...
    await sio.emit(
        "descriptions_generation_complete",
...
async def descriptions_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting descriptions."""
