"""Images resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle images generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch images/{resource_id}: {e}")
        return

    event = ImagesGenerationCompleteEvent(
