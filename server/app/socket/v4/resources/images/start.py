...
async def handle_start(data: dict[str, Any]) -> None:
    """Images generation started - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def images_call_start_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_start events targeting images."""
