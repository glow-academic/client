"""Documents resource error handler."""
...
from app.socket.v4.resources.documents.types import DocumentsGenerationErrorEvent
...
async def handle_error(data: dict[str, Any]) -> None:
    """Documents generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return

    event = DocumentsGenerationErrorEvent(
...
async def documents_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting documents."""
...
async def documents_generation_error_api(
    request: DocumentsGenerationErrorEvent,
) -> dict[str, bool]:
