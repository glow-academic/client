"""Keys resource completion handler."""
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle keys generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        ...
        logger.exception(f"Failed to fetch keys/{resource_id}: {e}")
        return

    event = KeysGenerationCompleteEvent(
...
async def keys_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting keys."""
