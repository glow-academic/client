"""Groups resource completion handler."""
...
from app.api.v4.resources.groups.get import get_groups_internal
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle groups generation complete - hydrate and emit typed event."""
    sid = data.get("sid", "")
    ...
    try:
        async with get_db_connection() as conn:
            items = await get_groups_internal(conn, [resource_id])
            if not items:
                return
            item = items[0]
            resource_data = (
        ...
        logger.exception(f"Failed to fetch groups/{resource_id}: {e}")
        return

    event = GroupsGenerationCompleteEvent(
...
async def groups_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting groups."""
