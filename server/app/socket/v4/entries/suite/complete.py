"""Suite entry completion handler."""
...
from app.socket.v4.entries.suite.types import SuiteGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle suite generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def suite_call_complete_listener(data: dict[str, Any]) -> None:
    """Listen for tool_result events targeting suite."""
    if data.get("event_type") != "tool_result":
        return
    if resolve_entry_type(data) != "suite":
        return
