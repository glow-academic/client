...
async def handle_progress(data: dict[str, Any]) -> None:
    """Health generation progress - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def health_call_progress_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_delta events targeting health."""
