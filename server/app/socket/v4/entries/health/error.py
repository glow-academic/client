...
async def handle_error(data: dict[str, Any]) -> None:
    """Health generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def health_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting health."""
