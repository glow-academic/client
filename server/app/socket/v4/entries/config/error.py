...
async def handle_error(data: dict[str, Any]) -> None:
    """Config generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def config_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting config."""
...
@server_router.post("/config_generation_error")
async def config_generation_error_api(
    request: ConfigGenerationEvent,
) -> dict[str, bool]:
    """Server-to-client event: Config generation error."""
    return {"success": True}
