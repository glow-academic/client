...
async def handle_error(data: dict[str, Any]) -> None:
    """Chat generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def chat_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting chat."""
