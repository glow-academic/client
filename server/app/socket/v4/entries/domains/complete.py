"""Domains entry completion handler."""
...
from app.socket.v4.entries.domains.types import DomainsGenerationEvent
...
async def handle_complete(data: dict[str, Any]) -> None:
    """Handle domains generation complete - emit typed event from tool result."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def domains_call_complete_listener(data: dict[str, Any]) -> None:
    ...
    if data.get("event_type") != "tool_result":
        return
    if resolve_entry_type(data) != "domains":
        return
