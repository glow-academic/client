"""Training entry progress handler."""
...
async def handle_progress(data: dict[str, Any]) -> None:
    """Training generation progress - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def training_call_progress_listener(data: dict[str, Any]) -> None:
    """Listen for tool_call_delta events targeting training."""
