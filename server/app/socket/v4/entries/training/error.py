...
async def handle_error(data: dict[str, Any]) -> None:
    """Training generation error - emit typed event."""
    sid = data.get("sid", "")
    if not sid:
        return
...
async def training_call_error_listener(data: dict[str, Any]) -> None:
    """Listen for error events targeting training."""
