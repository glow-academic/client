"""Handler for hint_eval_start WebSocket event - eval-specific logic for hint agent."""
...
SQL_PATH = "app/sql/v4/agents/agents_hint_hint_eval_start_complete.sql"


async def _hint_eval_impl(
    sid: str,
    data: AgentsHintHintEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsHintHintEvalStartSqlParams(
                **data.model_dump(),
            ...
            await internal_sio.emit(
                "benchmark_eval_complete",
                {
                    ...
                    "tool_id": None,
                    "success": True,
                    "message": "Hint eval completed successfully",
                    "sid": sid,
                },
            )
    except RuntimeError:
...
async def hint_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsHintHintEvalStartApiRequest,
        handler=_hint_eval_impl,  # type: ignore[arg-type]
