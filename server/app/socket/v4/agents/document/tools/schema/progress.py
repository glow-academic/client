"""Handler for document_tool_schema_progress - all types auto-generated from SQL."""

import uuid
from typing import Any, cast

from fastapi import APIRouter

from app.infra.v4.websocket.get_db_connection import get_db_connection
from app.infra.v4.websocket.handler_wrapper import handle_internal_event
from app.infra.v4.websocket.openapi_helpers import register_server_endpoint
from app.infra.v4.websocket.typed_emit import emit_to_client
from app.main import get_internal_sio
from app.sql.types import (
    DocumentToolSchemaProgressUpdateApiRequest,
    DocumentToolSchemaProgressUpdateSqlParams,
    DocumentToolSchemaProgressUpdateSqlRow,
)
from utils.sql_helper import execute_sql_typed

internal_sio = get_internal_sio()
server_router = APIRouter()

SQL_PATH = "app/sql/v4/documents/tools/schema/document_tool_schema_progress_update_complete.sql"


async def _document_tool_schema_progress_impl(
    sid: str,
    data: DocumentToolSchemaProgressUpdateApiRequest,
    profile_id: uuid.UUID,
    group_id: uuid.UUID | None = None,
) -> None:
    """Handle schema tool progress - SQL updates arguments, then emit to client."""
    try:
        async with get_db_connection() as conn:
            # Call SQL - SQL handles tool_call lookup and argument accumulation
            # Get arguments_delta from data (may be arguments_raw or arguments_delta)
            arguments_delta = (
                getattr(data, "arguments_delta", None)
                or getattr(data, "arguments_raw", "")
                or ""
            )

            params = DocumentToolSchemaProgressUpdateSqlParams(
                run_id=uuid.UUID(data.run_id),
                tool_call_id=data.tool_call_id,
                call_id=data.call_id,
                arguments_delta=arguments_delta,  # Delta - SQL accumulates
                progress_type=data.progress_type,
                document_id=uuid.UUID(data.document_id) if data.document_id else None,
            )

            result = cast(
                DocumentToolSchemaProgressUpdateSqlRow,
                await execute_sql_typed(conn, SQL_PATH, params=params),
            )

            # Emit to client with accumulated arguments
            await emit_to_client(
                "documents_progress",
                {
                    "type": data.progress_type,
                    "document_id": data.document_id,
                    "tool_type": "schema",  # Stable enum
                    "tool_call_id": result.tool_call_id,
                    "arguments_raw": result.arguments_raw,  # Accumulated by SQL
                },
                room=sid,
            )

    except RuntimeError:
        await emit_to_client(
            "documents_error",
            {"success": False, "message": "Database connection unavailable"},
            room=sid,
        )
    except Exception as e:
        await internal_sio.emit(
            "document_tool_schema_error",
            {"sid": sid, "success": False, "message": str(e)},
        )


@internal_sio.on("document_tool_schema_progress")  # type: ignore
async def document_tool_schema_progress_internal(
    data: dict[str, Any],
) -> None:
    """Handle document_tool_schema_progress event."""
    await handle_internal_event(
        data=data,
        request_type=DocumentToolSchemaProgressUpdateApiRequest,
        handler=_document_tool_schema_progress_impl,  # type: ignore[arg-type]
        error_event_name="document_tool_schema_error",
        error_response_type=None,  # Auto-generated if error SQL exists
    )


register_server_endpoint(
    server_router,
    "/document_tool_schema_progress",
    DocumentToolSchemaProgressUpdateApiRequest,
    "Progress update for schema tool (tool_type='schema')",
)
