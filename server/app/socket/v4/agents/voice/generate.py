...
async def _simulation_voice_generate_impl(
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            rows = await conn.fetch(function_call_sql, simulation_agent_id)
            agent_tools_config = [
            ...
            sql_messages = load_sql(
                "app/sql/v4/simulations/get_simulation_messages.sql"
            )
            ...
            try:
                await log_websocket_activity(
                    ...
                    endpoint="/socket/v4/simulations/voice/generate",
                    error=False,
                )
            except Exception:
                pass
    ...
    try:
        ...
        async with get_db_connection() as conn:
            ...
            if profile_id_uuid:
                sql_get_key = load_sql(
                    "app/sql/v4/settings/get_key_id_for_model_with_profile.sql"
                )
                ...
                sql_get_key = load_sql(
                    "app/sql/v4/settings/get_key_id_for_model_default.sql"
                )
                key_id_row = await conn.fetchrow(
                    sql_get_key,
                    model_id_uuid,
            ...
            sql_get_or_create_run = load_sql(
                "app/sql/v4/simulations/get_or_create_run_for_chat.sql"
            )
        ...
        try:
            await log_websocket_activity(
                ...
                context={"error": str(e)},
                endpoint="/socket/v4/simulations/voice/start",
                error=True,
            )
        except Exception:
            pass
