...
SQL_PATH = "app/sql/v4/agents/agents_scenario_tools_statement_statement_eval_start_complete.sql"


async def _statement_eval_impl(
    sid: str,
    data: AgentsScenarioToolsStatementStatementEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsScenarioToolsStatementStatementEvalStartSqlParams(
                **data.model_dump(),
            ...
            await emit_to_internal(
                "benchmark_eval_complete",
                {
                    ...
                    "success": True,
                    "message": "Statement eval completed successfully",
                },
                sid=sid,
            )
    except RuntimeError:
...
async def statement_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsScenarioToolsStatementStatementEvalStartApiRequest,
        handler=_statement_eval_impl,  # type: ignore[arg-type]
...
register_client_endpoint(
    ...
    "/eval",
    AgentsScenarioToolsStatementStatementEvalStartApiRequest,
    "Execute statement tool for eval",
)
