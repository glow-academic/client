"""Attempt message handler.

Handles: attempt_message — send a user message in an attempt chat.

Flow: Fetch → validate → prepare SQL → sync events → build chat history →
compose with _generate_impl (pre-created run_id skips prepare step).
"""
...
async def _attempt_message_impl(
    ...
    try:
        ...
        # Step 9: Compose with _generate_impl (pre-created run_id skips prepare)
        resource_types = data.resource_types or ["contents", "hints"]
