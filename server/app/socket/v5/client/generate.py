"""Unified generation handler for ALL draft artifact types.

Replaces the per-artifact ``{artifact}_generate`` handlers in v4 with a single
``generate`` event that dispatches via the artifact registry.

The 16-step flow is identical to every v4 handler, but parameterised by
``ArtifactGenerateConfig`` so no business logic is duplicated.
"""
...
async def _generate_impl(
    ...
    """Unified generate implementation — parameterised by registry config."""
    ...
    try:
        ...
        if config.requires_draft and not data.draft_id:
            await _emit_error(
                sid,
        ...
        if artifact_type == "profile" and not artifact_id and data.staff_id:
            artifact_id = uuid.UUID(data.staff_id)
        ...
        # For non-pool artifacts, still get pool for later use
        if not pool:
            pool = get_pool()

        # Result is dynamically typed per artifact — we use Any to access
        # the shared interface (resources, views, resource_agent_ids, group_id).
        result: Any = await config.fetcher(profile_id, artifact_id, data.draft_id, pool)
        ...
        async with get_db_connection() as conn:
            ...
            # Extract draft view using registry's draft_view_key
...
async def generate(sid: str, data: dict[str, Any]) -> None:
    ...
    Expects ``data.artifact_type`` to dispatch via the registry.
    """
