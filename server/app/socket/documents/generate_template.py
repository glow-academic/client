"""Handler for generate_document_template WebSocket event."""
...
async def _generate_document_template_impl(
    ...
    try:
        logger.info(
            f"Received generate_document_template request from {sid} with data: {data}"
        )
        ...
        logger.error(
            f"Error in generate_document_template for {sid}: {str(e)}", exc_info=True
        )
        await document_template_generation_error(
            DocumentTemplateGenerationErrorPayload(
...
async def generate_document_template(sid: str, data: dict[str, Any]) -> None:
    """Wrapper that validates payload before calling actual handler"""
    try:
        validated = GenerateDocumentTemplatePayload(**data)
        await _generate_document_template_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in generate_document_template for {sid}: {e}")
        await document_template_generation_error(
            DocumentTemplateGenerationErrorPayload(
