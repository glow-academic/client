...
class ClassifyEvalStartPayload(BaseModel):
    """Request to execute classify agent for eval."""
...
class ClassifyEvalCompletePayload(BaseModel):
    """Response indicating classify eval completed."""
...
async def _classify_eval_impl(sid: str, data: ClassifyEvalStartPayload) -> None:
    """Handle classify_eval_start requests via WebSocket."""
    try:
        logger.info(f"Received classify_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "classify_eval_complete",
                ClassifyEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Classify eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in classify_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "classify_eval_complete",
            ClassifyEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def classify_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        validated = ClassifyEvalStartPayload(**data)
        sid = data.get("sid", "internal")
        await _classify_eval_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in classify_eval_internal: {e}")


@server_router.post("/eval", response_model=dict[str, bool])
async def classify_eval_api(request: ClassifyEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute classify agent for eval."""
    return {"success": True}
