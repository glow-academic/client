...
SQL_PATH = "app/sql/v3/agents/agents_rubric_tools_rubric_rubric_eval_start_complete.sql"


async def _rubric_eval_impl(
    sid: str,
    data: AgentsRubricToolsRubricRubricEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsRubricToolsRubricRubricEvalStartSqlParams(
                **data.model_dump(),
...
async def rubric_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsRubricToolsRubricRubricEvalStartApiRequest,
