...
class DocumentEvalStartPayload(BaseModel):
    """Request to execute document agent for eval."""
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                DocumentEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Document eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "document_eval_complete",
            DocumentEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def document_eval_api(request: DocumentEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute document agent for eval."""
    return {"success": True}
