...
class AudioEvalStartPayload(BaseModel):
    """Request to execute audio agent for eval."""
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "audio_eval_complete",
                AudioEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Audio eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "audio_eval_complete",
            AudioEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def audio_eval_api(request: AudioEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute audio agent for eval."""
    return {"success": True}
