...
async def _scenario_eval_impl(sid: str, data: ScenarioEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received scenario_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                ScenarioEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Scenario eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in scenario_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "scenario_eval_complete",
            ScenarioEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def scenario_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error(f"Validation error in scenario_eval_internal: {e}")
