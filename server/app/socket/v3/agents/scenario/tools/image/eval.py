...
from app.sql.types import (
    ImageEvalStartApiRequest,
    ImageEvalStartSqlParams,
    ImageEvalStartSqlRow,
)

internal_sio = get_internal_sio()
server_router = APIRouter()

SQL_PATH = "app/sql/v3/agents/agents_scenario_tools_image_image_eval_start_complete.sql"


async def _image_eval_impl(
    sid: str,
    data: ImageEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = ImageEvalStartSqlParams(
                **data.model_dump(),
...
async def image_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=ImageEvalStartApiRequest,
