...
SQL_PATH = "app/sql/v3/agents/agents_scenario_tools_title_title_eval_start_complete.sql"


async def _title_eval_impl(
    sid: str,
    data: AgentsScenarioToolsTitleTitleEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsScenarioToolsTitleTitleEvalStartSqlParams(
                **data.model_dump(),
...
async def title_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsScenarioToolsTitleTitleEvalStartApiRequest,
