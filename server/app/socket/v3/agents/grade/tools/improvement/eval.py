...
from app.sql.types import (
    ImprovementEvalStartApiRequest,
    ImprovementEvalStartSqlParams,
    ImprovementEvalStartSqlRow,
)

internal_sio = get_internal_sio()
server_router = APIRouter()

SQL_PATH = "app/sql/v3/agents/agents_grade_tools_improvement_improvement_eval_start_complete.sql"


async def _improvement_eval_impl(
    sid: str,
    data: ImprovementEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = ImprovementEvalStartSqlParams(
                **data.model_dump(),
            ...
            await emit_to_internal(
                "benchmark_eval_complete",
                {
                    ...
                    "success": True,
                    "message": "Improvement eval completed successfully",
                },
                sid=sid,
            )
    except RuntimeError:
...
async def improvement_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=ImprovementEvalStartApiRequest,
        handler=_improvement_eval_impl,  # type: ignore[arg-type]
...
register_client_endpoint(
    ...
    "/eval",
    ImprovementEvalStartApiRequest,
    "Execute improvement tool for eval",
)
