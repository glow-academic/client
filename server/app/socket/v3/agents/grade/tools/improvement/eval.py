...
async def _improvement_eval_impl(sid: str, data: ImprovementEvalStartPayload) -> None:
    ...
    try:
        logger.info(
            f"Received improvement_eval_start request from {sid} with data: {data}"
        )
        ...
        logger.error(f"Error in improvement_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "improvement_eval_complete",
            ImprovementEvalCompletePayload(
                test_id=data.test_id,
...
async def improvement_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error(f"Validation error in improvement_eval_internal: {e}")
