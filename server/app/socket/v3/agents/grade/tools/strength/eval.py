...
from app.sql.types import (
    StrengthEvalStartApiRequest,
    StrengthEvalStartSqlParams,
    StrengthEvalStartSqlRow,
)

internal_sio = get_internal_sio()
server_router = APIRouter()

SQL_PATH = "app/sql/v3/agents/agents_grade_tools_strength_strength_eval_start_complete.sql"


async def _strength_eval_impl(
    sid: str,
    data: StrengthEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = StrengthEvalStartSqlParams(
                **data.model_dump(),
...
async def strength_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=StrengthEvalStartApiRequest,
