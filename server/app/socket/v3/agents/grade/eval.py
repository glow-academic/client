"""Handler for grade_eval_start WebSocket event - eval-specific logic for grade agent."""
...
class GradeEvalStartPayload(BaseModel):
    """Request to execute grade agent for eval."""
...
async def _grade_eval_impl(sid: str, data: GradeEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received grade_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "grade_eval_complete",
                GradeEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Grade eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in grade_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "grade_eval_complete",
            GradeEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
