...
class MemberEvalStartPayload(BaseModel):
    """Request to execute member agent for eval."""
...
class MemberEvalCompletePayload(BaseModel):
    """Response indicating member eval completed."""
...
async def _member_eval_impl(sid: str, data: MemberEvalStartPayload) -> None:
    """Handle member_eval_start requests via WebSocket."""
    try:
        logger.info(f"Received member_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "member_eval_complete",
                MemberEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Member eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in member_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "member_eval_complete",
            MemberEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def member_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        validated = MemberEvalStartPayload(**data)
        sid = data.get("sid", "internal")
        await _member_eval_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in member_eval_internal: {e}")


@server_router.post("/eval", response_model=dict[str, bool])
async def member_eval_api(request: MemberEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute member agent for eval."""
    return {"success": True}
