...
class VideoEvalStartPayload(BaseModel):
    """Request to execute video agent for eval."""
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "video_eval_complete",
                VideoEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Video eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "video_eval_complete",
            VideoEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def video_eval_api(request: VideoEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute video agent for eval."""
    return {"success": True}
