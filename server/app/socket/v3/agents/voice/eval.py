...
from app.sql.types import (
    VoiceEvalStartApiRequest,
    VoiceEvalStartSqlParams,
    VoiceEvalStartSqlRow,
)

internal_sio = get_internal_sio()
server_router = APIRouter()

SQL_PATH = "app/sql/v3/agents/voice/voice_eval_start_complete.sql"


async def _voice_eval_impl(
    sid: str,
    data: VoiceEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = VoiceEvalStartSqlParams(
                **data.model_dump(),
...
async def voice_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=VoiceEvalStartApiRequest,
