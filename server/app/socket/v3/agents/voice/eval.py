...
class VoiceEvalStartPayload(BaseModel):
    """Request to execute voice agent for eval."""
...
class VoiceEvalCompletePayload(BaseModel):
    """Response indicating voice eval completed."""
...
async def _voice_eval_impl(sid: str, data: VoiceEvalStartPayload) -> None:
    """Handle voice_eval_start requests via WebSocket."""
    try:
        logger.info(f"Received voice_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "voice_eval_complete",
                VoiceEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Voice eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in voice_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "voice_eval_complete",
            VoiceEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def voice_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        validated = VoiceEvalStartPayload(**data)
        sid = data.get("sid", "internal")
        await _voice_eval_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in voice_eval_internal: {e}")


@server_router.post("/eval", response_model=dict[str, bool])
async def voice_eval_api(request: VoiceEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute voice agent for eval."""
    return {"success": True}
