...
SQL_PATH = "app/sql/v3/agents/agents_image_tools_debug_debug_eval_start_complete.sql"


async def _debug_eval_impl(
    sid: str,
    data: AgentsImageToolsDebugDebugEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsImageToolsDebugDebugEvalStartSqlParams(
                **data.model_dump(),
...
async def debug_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsImageToolsDebugDebugEvalStartApiRequest,
