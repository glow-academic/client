"""Handler for image_tool_title_eval_start WebSocket event - eval-specific logic for title tool."""
...
class ImageTitleEvalStartPayload(BaseModel):
...
class ImageTitleEvalCompletePayload(BaseModel):
...
async def _image_tool_title_eval_start_impl(
    sid: str, data: ImageTitleEvalStartPayload
) -> None:
    """Handle image_tool_title_eval_start requests via WebSocket."""
    try:
        logger.info(
            f"Received image_tool_title_eval_start request from {sid} with data: {data}"
        )
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "image_tool_title_eval_complete",
                ImageTitleEvalCompletePayload(
                    test_id=test_id,
                    tool_id=tool_id,
                    success=True,
...
@internal_sio.on("image_tool_title_eval_start")  # type: ignore
async def title_eval_internal(data: dict[str, Any]) -> None:
    """Handle image_tool_title_eval_start event from internal bus."""
    try:
        validated = ImageTitleEvalStartPayload(**data)
        sid = data.get("sid", "internal")
        await _image_tool_title_eval_start_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in title_eval_internal: {e}")


@server_router.post("/eval", response_model=dict[str, bool])
async def title_eval_api(request: ImageTitleEvalStartPayload) -> dict[str, bool]:
