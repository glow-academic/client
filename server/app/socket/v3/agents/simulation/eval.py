...
class SimulationEvalStartPayload(BaseModel):
    """Request to execute simulation agent for eval."""
...
class SimulationEvalCompletePayload(BaseModel):
    """Response indicating simulation eval completed."""
...
async def _simulation_eval_impl(sid: str, data: SimulationEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received simulation_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                SimulationEvalCompletePayload(test_id=test_id, agent_id=agent_id, success=True, message="Simulation eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "simulation_eval_complete",
            SimulationEvalCompletePayload(test_id=data.test_id, agent_id=data.agent_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def simulation_eval_api(request: SimulationEvalStartPayload) -> dict[str, bool]:
    """Internal event: Execute simulation agent for eval."""
    return {"success": True}
