...
async def debug_info_tool_complete(
    ...
    logger.info(
        f"[debug_info_complete] Emitting complete event: "
        f"room={room}"
    )
...
async def debug_info_tool_error(
    payload: DebugInfoToolErrorPayload, room: str
) -> None:
...
async def _debug_info_impl(sid: str, data: dict[str, Any]) -> str | None:
    ...
    logger.info(
        f"[debug_info] Handler received event: sid={sid}"
    )
    ...
    try:
        ...
        logger.error(
            f"Error in debug_info for {sid}: {str(e)}", exc_info=True
        )
        await debug_info_tool_error(
            DebugInfoToolErrorPayload(
                success=False,
