...
class TitleEvalStartPayload(BaseModel):
...
class TitleEvalCompletePayload(BaseModel):
...
async def _title_eval_impl(sid: str, data: TitleEvalStartPayload) -> None:
    """Handle title_eval_start requests via WebSocket."""
    try:
        logger.info(f"Received title_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                "title_eval_complete",
                TitleEvalCompletePayload(test_id=test_id, tool_id=tool_id, success=True, message="Title eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "title_eval_complete",
            TitleEvalCompletePayload(test_id=data.test_id, tool_id=data.tool_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def title_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        validated = TitleEvalStartPayload(**data)
        sid = data.get("sid", "internal")
        await _title_eval_impl(sid, validated)
    except ValidationError as e:
        logger.error(f"Validation error in title_eval_internal: {e}")


@server_router.post("/eval", response_model=dict[str, bool])
async def title_eval_api(request: TitleEvalStartPayload) -> dict[str, bool]:
