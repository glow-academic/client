...
async def _question_eval_impl(sid: str, data: QuestionEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received question_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                QuestionEvalCompletePayload(test_id=test_id, tool_id=tool_id, success=True, message="Question eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "question_eval_complete",
            QuestionEvalCompletePayload(test_id=data.test_id, tool_id=data.tool_id, success=False, message=str(e)),
            sid=sid,
        )
