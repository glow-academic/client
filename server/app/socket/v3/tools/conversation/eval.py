...
async def _conversation_eval_impl(sid: str, data: ConversationEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received conversation_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                ConversationEvalCompletePayload(test_id=test_id, tool_id=tool_id, success=True, message="Conversation eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "conversation_eval_complete",
            ConversationEvalCompletePayload(test_id=data.test_id, tool_id=data.tool_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def conversation_eval_api(request: ConversationEvalStartPayload) -> dict[str, bool]:
