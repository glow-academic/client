...
async def _document_eval_impl(sid: str, data: DocumentEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received document_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                DocumentEvalCompletePayload(test_id=test_id, tool_id=tool_id, success=True, message="Document eval completed"),
                sid=sid,
            )
    except Exception as e:
        logger.error(f"Error in document_eval for {sid}: {str(e)}", exc_info=True)
        await emit_to_internal(
            "document_eval_complete",
            DocumentEvalCompletePayload(test_id=data.test_id, tool_id=data.tool_id, success=False, message=str(e)),
            sid=sid,
        )
...
async def document_eval_internal(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.error(f"Validation error in document_eval_internal: {e}")
