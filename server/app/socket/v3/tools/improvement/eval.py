...
async def _improvement_eval_impl(sid: str, data: ImprovementEvalStartPayload) -> None:
    ...
    try:
        logger.info(f"Received improvement_eval_start request from {sid} with data: {data}")
        ...
        async with pool.acquire() as conn:
            ...
            await emit_to_internal(
                ...
                ImprovementEvalCompletePayload(test_id=test_id, tool_id=tool_id, success=True, message="Improvement eval completed"),
                sid=sid,
            )
        ...
        await emit_to_internal(
            "improvement_eval_complete",
            ImprovementEvalCompletePayload(test_id=data.test_id, tool_id=data.tool_id, success=False, message=str(e)),
            sid=sid,
        )
