"""Handler for rubric_title_progress - handles incremental updates for create_title tool calls."""
...
class RubricTitleProgressPayload(BaseModel):
    """Rubric title tool progress event."""
...
class RubricTitleProgressErrorPayload(BaseModel):
    """Error response for rubric title progress."""
...
async def _rubric_title_progress_impl(
    sid: str,
    data: RubricTitleProgressPayload,
    profile_id: uuid.UUID,
    group_id: uuid.UUID | None = None,
) -> None:
    """Handle rubric_title_progress - tracks progress and emits to client."""
    try:
...
@internal_sio.on("rubric_title_progress")  # type: ignore
async def rubric_title_progress_internal(
    data: dict[str, Any],
) -> None:
    """Handle rubric_title_progress event from internal bus."""
    await handle_internal_event(
        data=data,
        request_type=RubricTitleProgressPayload,
        handler=_rubric_title_progress_impl,  # type: ignore[arg-type]
        error_event_name="rubric_title_error",
        error_response_type=RubricTitleProgressErrorPayload,
    )
