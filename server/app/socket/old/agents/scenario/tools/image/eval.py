...
SQL_PATH = "app/sql/v4/agents/agents_scenario_tools_image_image_eval_start_complete.sql"


async def _image_eval_impl(
    sid: str,
    data: AgentsScenarioToolsImageImageEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsScenarioToolsImageImageEvalStartSqlParams(
                **data.model_dump(),
            ...
            await emit_to_internal(
                "benchmark_eval_complete",
                {
                    ...
                    "success": True,
                    "message": "Image eval completed successfully",
                },
                sid=sid,
            )
    except RuntimeError:
...
async def image_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsScenarioToolsImageImageEvalStartApiRequest,
        handler=_image_eval_impl,  # type: ignore[arg-type]
