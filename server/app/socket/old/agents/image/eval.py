...
SQL_PATH = "app/sql/v4/agents/agents_image_image_eval_start_complete.sql"


async def _image_eval_impl(
    sid: str,
    data: AgentsImageImageEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsImageImageEvalStartSqlParams(
                **data.model_dump(),
...
async def image_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsImageImageEvalStartApiRequest,
