"""Handler for document_eval_start WebSocket event - eval-specific logic for document agent."""
...
SQL_PATH = "app/sql/v4/agents/agents_document_document_eval_start_complete.sql"


async def _document_eval_impl(
    sid: str,
    data: AgentsDocumentDocumentEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsDocumentDocumentEvalStartSqlParams(
                **data.model_dump(),
            ...
            await internal_sio.emit(
                "benchmark_eval_complete",
                {
                    ...
                    "tool_id": None,
                    "success": True,
                    "message": "Document eval completed successfully",
                    "sid": sid,
                },
            )
    except RuntimeError:
...
async def document_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsDocumentDocumentEvalStartApiRequest,
        handler=_document_eval_impl,  # type: ignore[arg-type]
...
register_client_endpoint(
    ...
    "/eval",
    AgentsDocumentDocumentEvalStartApiRequest,
    "Execute document agent for eval",
)
