...
SQL_PATH = "app/sql/v4/agents/agents_video_tools_debug_debug_eval_start_complete.sql"


async def _debug_eval_impl(
    sid: str,
    data: AgentsVideoToolsDebugDebugEvalStartApiRequest,
    ...
    try:
        async with get_db_connection() as conn:
            params = AgentsVideoToolsDebugDebugEvalStartSqlParams(
                **data.model_dump(),
...
async def debug_eval_internal(data: dict[str, Any]) -> None:
    ...
    await handle_internal_event(
        data=data,
        request_type=AgentsVideoToolsDebugDebugEvalStartApiRequest,
