"""Handler for member_speak_complete - finalizes DB and emits to client."""
...
SQL_PATH_FINALIZE = "app/sql/v4/member/tools/member_tool_complete_finalize_complete.sql"


class MemberSpeakCompletePayload(BaseModel):
    """Member speak tool complete event."""
...
class MemberSpeakCompleteErrorPayload(BaseModel):
    """Error response for member speak complete."""
...
class MemberSpeakMessageCompletePayload(BaseModel):
    """Member speak message completed."""
...
async def member_speak_message_complete(
    payload: MemberSpeakMessageCompletePayload, room: str
) -> None:
    await sio.emit("member_speak_message_complete", payload.model_dump(), room=room)
...
async def _member_speak_complete_impl(
    sid: str,
    data: MemberSpeakCompletePayload,
    profile_id: uuid.UUID,
    group_id: uuid.UUID | None = None,
) -> None:
    """Handle member_speak_complete - finalizes DB and emits to client."""
    try:
        ...
        async with get_db_connection() as conn:
            ...
            await member_speak_message_complete(
                MemberSpeakMessageCompletePayload(
...
@internal_sio.on("member_speak_complete")  # type: ignore
async def member_speak_complete_internal(
    data: dict[str, Any],
) -> None:
    """Handle member_speak_complete event from internal bus."""
    await handle_internal_event(
        data=data,
        request_type=MemberSpeakCompletePayload,
        handler=_member_speak_complete_impl,
        error_event_name="member_speak_error",
        error_response_type=MemberSpeakCompleteErrorPayload,
    )
...
register_server_endpoint(
    ...
    MemberSpeakCompletePayload,
    "Member speak tool completed successfully",
)
