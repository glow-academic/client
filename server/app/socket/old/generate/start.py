...
async def _generate_start_impl(
    ...
    try:
        async with get_db_connection() as conn:
            ...
            # Optionally route to artifacts/generate.py (new unified handler)
            # For now, keep old handlers working, but allow routing to artifacts
            use_artifacts = True  # Feature flag - set to False to use old handlers
            
            if use_artifacts:
                # Route to unified artifacts handler
                await internal_sio.emit("generate_artifact", modality_payload)
            else:
                # Legacy routing to old modality handlers
