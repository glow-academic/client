...
async def test_started_output(data: dict[str, Any]) -> None:
    sid = data.get("sid", "")
    rooms = data.get("rooms") or ([sid] if sid else [])
    call_id = data.get("call_id")
    if call_id:
        append_call_event(UUID(call_id), "test_started", data, UPLOAD_FOLDER)
    for room in rooms:
        await sio.emit("test_started", data, room=room)
