...
async def generate_error_output(data: dict[str, Any]) -> None:
    sid = data.get("sid", "")
    rooms = data.get("rooms") or ([sid] if sid else [])
    call_id = data.get("call_id")
    if call_id:
        append_call_event(UUID(call_id), "generate_error", data, UPLOAD_FOLDER)
    for room in rooms:
        await sio.emit("generate_error", data, room=room)
