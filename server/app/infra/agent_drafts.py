"""Agent drafts list logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (profiles_id)
  2. search_agent_drafts — declarative filter by profile ownership
"""
...
async def list_agent_drafts_client(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. search_agent_drafts(profile_ids=[profiles_id]) → entries
    ...
    async with pool.acquire() as conn:
        drafts = await search_agent_drafts(
            conn,
