"""Auth drafts list logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (profiles_id)
  2. search_auth_drafts — declarative filter by profile ownership
"""
...
async def list_auth_drafts_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. search_auth_drafts(profile_ids=[profiles_id]) → entries
    ...
    async with pool.acquire() as conn:
        drafts = await search_auth_drafts(
            conn,
