...
Core duplicate function that composes existing black-box tools:
  ...
  3. get_rubrics — fetch original with all junction IDs
  4. create_name — new name resource ("{name} Copy")
  5. search_flags — find inactive flag (rubric_active, value=false)
  6. create_rubric — new artifact with original's IDs + new name + inactive flag
...
from app.infra.rubric_permissions import compute_can_duplicate
...
async def duplicate_rubric_client(
    ...
    *,
    ...
    Flow:
      ...
      2. compute_can_duplicate -> permission check
      3. get_rubrics -> fetch original with all junctions
      4. create_name("{name} Copy") -> new name resource
      5. search_flags -> find inactive flag (rubric_active, value=false)
      6. create_rubric -> new artifact with original IDs + inactive flag
      7. invalidate_tags
    """
    ...
    # -- Step 3: Fetch original rubric with all junctions -----------------------
    ...
    # -- Step 5: Find inactive flag (rubric_active, value=false) ----------------
    ...
    # -- Step 6: Create new rubric artifact with inactive flag ------------------
