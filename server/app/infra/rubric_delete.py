...
Core delete function that composes existing black-box tools:
  ...
  2. resolve_rubric_permissions_context — per-item exists, departments, usage
  3. compute_can_delete — permission check
  4. delete_rubrics — bulk delete tool
...
from app.routes.v5.api.main.rubric.permissions import compute_can_delete
...
async def delete_rubric_client(
    ...
    *,
    ...
    Flow:
      ...
      2. Per-item: resolve_rubric_permissions_context -> exists, departments, usage
      3. Per-item: compute_can_delete -> permission check (fail fast)
      4. Fetch names for result messages
      5. Single transaction: delete_rubrics -> bulk delete
      6. invalidate_tags
    """
    ...
    results = [
        DeleteRubricResult(
            success=True,
            rubric_id=pid,
            message=f"Rubric '{name_map.get(pid, 'Unknown')}' deleted successfully",
        )
        for pid in result.deleted_ids
    ]

    return DeleteRubricApiResponse(results=results)
