"""Department drafts list logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (profiles_id)
  2. search_department_drafts — declarative filter by profile ownership
"""
...
async def list_department_drafts_client(
    ...
    *,
    ...
    Flow:
      ...
      2. search_department_drafts(profile_ids=[profiles_id]) → entries
    ...
    async with pool.acquire() as conn:
        drafts = await search_department_drafts(
            conn,
