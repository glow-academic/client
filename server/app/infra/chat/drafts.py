"""Chat drafts list logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (profiles_id)
  2. search_chat_drafts — declarative filter by profile ownership
"""
...
async def list_chat_drafts_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. search_chat_drafts(profile_ids=[profiles_id]) → entries
    ...
    async with pool.acquire() as conn:
        drafts = await search_chat_drafts(
            conn,
