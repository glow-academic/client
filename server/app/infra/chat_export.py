...
async def export_chat_client(
    ...
    *,
    ...
    personas_str = PIPE.join(
        getattr(p, "name", "") or ""
        for p in (resources.get("personas", None) and resources["personas"].selected or [])
    )
    documents_str = PIPE.join(
        getattr(d, "name", "") or ""
        for d in (resources.get("documents", None) and resources["documents"].selected or [])
    )
    scenarios_str = PIPE.join(
        getattr(s, "name", "") or ""
        for s in (resources.get("scenarios", None) and resources["scenarios"].selected or [])
    )
    ...
    parameter_fields_str = PIPE.join(
        getattr(pf, "name", "") or ""
        for pf in (resources.get("parameter_fields", None) and resources["parameter_fields"].selected or [])
    )
    questions_str = PIPE.join(
        getattr(q, "question", "") or getattr(q, "name", "") or ""
        for q in (resources.get("questions", None) and resources["questions"].selected or [])
    )
    options_str = PIPE.join(
        getattr(o, "option", "") or getattr(o, "name", "") or ""
        for o in (resources.get("options", None) and resources["options"].selected or [])
    )
    ...
    problem_statements_str = PIPE.join(
        ...
        for ps in (resources.get("problem_statements", None) and resources["problem_statements"].selected or [])
    )
    objectives_str = PIPE.join(
        getattr(o, "objective", "") or getattr(o, "name", "") or ""
        for o in (resources.get("objectives", None) and resources["objectives"].selected or [])
    )
