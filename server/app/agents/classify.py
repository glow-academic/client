...
from app.models import Chats, Rubrics, Classes, Documents
                ...
                for doc_num in doc_numbers:
                    if doc_num in document_mapping:
                        ...
                        if document.type != new_type:
                            ...
                            classified_count += 1
                            logger.info(f"Updated document '{document.name}' to type '{new_type}'")

        session.commit()
        
        logger.info(f"Successfully classified {classified_count} documents for class {class_data.name}")
