...
from app.schemas.profile import (CohortItem, CohortsData, DepartmentItem,
                                 ProfileContextRequest, ProfileContextResponse,
                                 ProfileItem, SimulationContextItem,
                                 SimulationsData)
...
class ProfileService(BaseService):
    ...
    async def get_simulatable_profiles(self, profile_id: str) -> list[ProfileItem]:
    ...
    async def authorize_emulation(
        ...
        simulatable_profiles = await self.get_simulatable_profiles(requester_profile_id)
        ...
        if actual_profile_id != effective_profile_id:
            ...
            if not allowed:
                raise PermissionError(
                    reason
                    or "You do not have permission to view this profile's context"
                )
