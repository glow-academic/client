...
class ScenarioService:
    ...
    async def randomly_fill_scenario_attributes(
        ...
        if not existing_param_ids:
            ...
            if active_parameters:
                ...
                for param in active_parameters:
                    query, params = self.queries.get_parameter_items_by_parameter(str(param['id']))
            ...
            query, params = self.queries.get_parameter_items_batch([str(pid) for pid in existing_param_ids])
            ...
            for param_id in active_param_ids:
                if param_id in existing_items_by_param:
                    ...
                    query, params = self.queries.get_parameter_items_by_parameter(str(param_id))
                    param_items = await self.conn.fetch(query, *params)
                    if param_items:
                        selected_item = random.choice(param_items)
                        scenario_parameter_item_ids.append(selected_item['id'])
        ...
        await self.conn.execute(
            self.queries.insert_scenario_tree_edge(),
    ...
    async def suggest_randomized_sections(
        ...
        *,
        ...
        if document_ids:
            query, params = self.queries.get_documents_by_ids([str(d) for d in document_ids])
        ...
        if "documents" in targets_set:
            ...
            def score_doc(doc: Dict[str, Any]) -> float:
                score = 0.0
                name_overlap = context_tokens.intersection(set(tokenize(doc.get('name') or "")))
                ...
                try:
                    ...
                    if file_path:
                        doc_text = read_document_content_for_similarity(file_path)
                        doc_text = doc_text[:5000]
                        sim = fuzz.token_set_ratio(context_text, normalize_text(doc_text))  # 0..100
            ...
            if active_documents:
                ...
                chosen_tag = weighted_choice(tag_scores) or (tag_scores[0][0] if tag_scores else "__untagged__")
                candidates = tag_to_docs.get(chosen_tag, [])
                ...
                selected = weighted_sample_without_replacement(cand_docs, cand_scores, 1)
                ...
                for param in active_parameters:
                    ...
                    if param_is_default:
                        ...
                        def score_item(it: Dict[str, Any]) -> float:
                            ...
                            p_tokens = set(tokenize(param.get('name'))) | set(tokenize(param.get('description')))
                            ...
                            sim_all: float = float(
                                fuzz.token_set_ratio(
                                    context_text, normalize_text(f"{it.get('name')} {it.get('description')} {it.get('value')}")
                                )
                            )
    ...
    async def search_scenarios(
        ...
        persona_query, persona_params = self.queries.get_scenario_personas_batch(scenario_ids)
        ...
        try:
            ...
            for sim in result["simulations"]:
                simulation_list.append({
                    ...
                    "created_at": sim["created_at"] if sim.get("created_at") else None,
                })
