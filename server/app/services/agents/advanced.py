...
async def run_advanced_agent_parallel(
    ...
    for agent_key, group_chats in agent_groups.items():
        ...
        for turn in range(eval_obj.max_turns):
            ...
            current_agent = query_agent if current_speaker == 'query' else response_agent
            ...
            for i, (chat_id, response) in enumerate(zip(chat_states.keys(), responses)):
                ...
                state['current_speaker'] = 'response' if current_speaker == 'query' else 'query'
    ...
    if matches and len(matches) >= expected_count:
        ...
        if len(non_empty_lines) >= expected_count:
            ...
            if non_empty_lines:
                ...
                outputs = [f"{base_response} (Response {i+1})" for i in range(expected_count)]
            else:
                outputs = [f"I understand your question {i+1}. Let me help you with that." for i in range(expected_count)]
    
    # Ensure we have exactly the expected number of outputs
    while len(outputs) < expected_count:
        outputs.append("I apologize, but I need more information to provide a complete response.")
    
    return outputs[:expected_count]

class AdvancedAgent:
    def __init__(self, agent_name: str, agent_prompt: str, agent_type: str, temperature: float = 0.0, use_parallel_output: bool = False):
