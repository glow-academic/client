...
async def run_grading_agent(
    ...
    """
    This function is used to run the grading agent.
    ...
    if test_data:
        ...
        result = await Runner.run(grading_agent.agent(), input=input_items)
...
class GradingAgent:
    def __init__(self):
        ...
        self.system_prompt = ("Your purpose is to grade a conversation between a student and a GTA, grading them on the following metrics (from 1-5): listening, course objectives, time management, and adaptability. "
        "A rubric will be provided to you to help guide the responses. "
        "Output a JSON object with the following fields: listen, obj, time, adapt. You should also provide feedback for each metric to let the GTA know how to improve. "
        "Please keep this feedback concise, not more than 1-2 sentences. "
        "The feedback should be specific and measureable, like 'Recursion was explained incorrectly' or 'Ask more questions about min heaps, don't just give the definition'. "
        "You can use listen_fb, obj_fb, time_fb, and adapt_fb to provide feedback for each metric. "
        "You will only be evaluating when the conversation is ended by the user, indicated by them clicking on a button in the UI, so don't assume that the conversation is always complete. "
        "if you feel the conversation, don't give a score necessarily, but just say you don't have enough information to evaluate the conversation. "
        "If the conversation is ended, but doesn't feel complete, but there is enough information to evaluate, still don't give a score, but provide feedback on what the GTA could have done better to complete the conversation, and still mention that a score will only be given when an compelted conversation is had. ")
