"""Department search-profile endpoint - search profiles for adding to a department."""
...
class DepartmentSearchProfileRequest(BaseModel):
    """Request for searching profiles to add to a department."""

    departmentId: str | None = None  # Optional for new departments
    query: str | None = (
...
class DepartmentSearchProfileResponse(BaseModel):
    """Response for department search-profile endpoint."""
...
@router.post("/search-profile", response_model=DepartmentSearchProfileResponse)
async def department_search_profile(
    request: DepartmentSearchProfileRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> DepartmentSearchProfileResponse:
    """Search profiles for adding to a department (excludes profiles already in department if departmentId provided)."""
    ...
    if cached:
        ...
        return DepartmentSearchProfileResponse.model_validate(cached["data"])
    ...
    try:
        ...
        # Department exclusion filter - exclude profiles already in this department (only if departmentId provided)
        dept_exclusion = ""
        if request.departmentId:
            params.append(request.departmentId)
            dept_exclusion = f"AND NOT EXISTS (SELECT 1 FROM profile_departments pd2 WHERE pd2.profile_id = p.id AND pd2.department_id = ${param_idx}::uuid AND pd2.active = true)"
            param_idx += 1
        ...
        if not result:
            ...
            response_data = DepartmentSearchProfileResponse(
                staff=[],
                department_mapping={},
                cohort_mapping={},
            )
        ...
        handle_route_error(
            ...
            operation="department_search_profile",
