...
async def export_reports(
    ...
    try:
        # Load SQL template
        sql_template = load_sql("sql/v3/reports/export.sql")
        ...
        # Add additional filters for profileIds, simulationIds, scenarioIds
        param_counter = len(params) + 1
        profile_ids_filter = ""
        simulation_ids_filter = ""
        scenario_ids_filter = ""

        if request.profileIds and len(request.profileIds) > 0:
            profile_ids_filter = f" AND p.id = ANY(${param_counter}::uuid[])"
            params.append(request.profileIds)
            param_counter += 1
        else:
            profile_ids_filter = ""

        if request.simulationIds and len(request.simulationIds) > 0:
            simulation_ids_filter = f" AND a.simulation_id = ANY(${param_counter}::uuid[])"
            params.append(request.simulationIds)
            param_counter += 1
        else:
            simulation_ids_filter = ""

        if request.scenarioIds and len(request.scenarioIds) > 0:
            scenario_ids_filter = f" AND a.scenario_id = ANY(${param_counter}::uuid[])"
            params.append(request.scenarioIds)
            param_counter += 1
        else:
            scenario_ids_filter = ""
        ...
        sql_query = sql_query.replace("{PROFILE_IDS_FILTER}", profile_ids_filter)
        sql_query = sql_query.replace("{SIMULATION_IDS_FILTER}", simulation_ids_filter)
        sql_query = sql_query.replace("{SCENARIO_IDS_FILTER}", scenario_ids_filter)
        ...
        # Get simulation mapping for Brightspace format
        simulation_mapping: dict[str, dict[str, str]] = {}
        if request.brightspaceFormat:
            sim_mapping_query = """
                SELECT jsonb_object_agg(
                    sim.id::text,
                    jsonb_build_object('name', sim.title, 'description', COALESCE(sim.description, ''))
                )
                FROM simulations sim
                WHERE sim.active = true AND sim.practice_simulation = true
            """
            sim_mapping_result = await conn.fetchval(sim_mapping_query)
            if sim_mapping_result:
                if isinstance(sim_mapping_result, str):
                    simulation_mapping = json.loads(sim_mapping_result)
                else:
                    simulation_mapping = sim_mapping_result
