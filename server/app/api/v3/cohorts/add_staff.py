"""Cohort add-staff endpoint - search staff for adding to a cohort."""
...
class CohortAddStaffRequest(BaseModel):
    """Request for searching staff to add to a cohort."""
...
class CohortAddStaffResponse(BaseModel):
    """Response for cohort add-staff endpoint."""
...
@router.post("/add-staff", response_model=CohortAddStaffResponse)
async def cohort_add_staff(
    request: CohortAddStaffRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CohortAddStaffResponse:
    """Search staff for adding to a cohort (excludes profiles already in cohort)."""
    ...
    if cached:
        ...
        return CohortAddStaffResponse.model_validate(cached["data"])

    try:
        # Build dynamic SQL query
        # Start with current_profile_id (used in CTEs)
        params: list[Any] = [request.profileId, request.cohortId]
        param_idx = 3  # $1 is current_profile_id, $2 is cohort_id
        ...
        if not result:
            ...
            response_data = CohortAddStaffResponse(
                staff=[],
                department_mapping={},
                cohort_mapping={},
            )
