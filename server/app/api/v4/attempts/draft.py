"""Attempt draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/attempts/patch_attempt_draft_complete.sql"
...
@router.patch(
    "/draft",
    response_model=PatchAttemptDraftApiResponse,
    dependencies=[
        audit_activity(
            "attempt.draft.patched",
            "{{ actor.name }} saved attempt draft",
        )
    ],
)
async def patch_attempt_draft(
    request: PatchAttemptDraftApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> PatchAttemptDraftApiResponse:
    """Patch attempt draft (creates if not exists)."""
    tags = ["attempts", "drafts"]  # From router tags
    ...
    try:
        ...
        async with conn.transaction():
            ...
            params = PatchAttemptDraftSqlParams(
            ...
            if not result or result.draft_id is None:
                raise ValueError("Failed to patch attempt draft")
        ...
        api_response = PatchAttemptDraftApiResponse.model_validate(result.model_dump())
