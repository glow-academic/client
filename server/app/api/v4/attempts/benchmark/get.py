...
SQL_PATH = "app/sql/v4/queries/attempts/get_eval_attempt_complete.sql"
...
async def get_benchmark_attempt(
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetEvalAttemptApiResponse.model_validate(cached["data"])
    ...
    try:
        ...
        params = GetEvalAttemptSqlParams(**request.model_dump(), profile_id=profile_id)
        ...
        if not result.attempt_exists:
            raise HTTPException(
                status_code=404, detail=f"Eval attempt not found: {request.attempt_id}"
            )
        ...
        api_response = GetEvalAttemptApiResponse.model_validate(result.model_dump())
