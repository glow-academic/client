"""Model draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/models/patch_model_draft_complete.sql"
...
@router.patch(
    ...
    dependencies=[
        audit_activity(
            "models.draft.patched",
            "{{ actor.name }} saved model draft",
        )
    ],
)
async def patch_model_draft(
    ...
    """Patch model draft (creates if not exists)."""
    tags = ["models", "drafts"]  # From router tags
