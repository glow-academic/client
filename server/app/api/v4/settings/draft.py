"""Settings draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/settings/patch_settings_draft_complete.sql"
...
@router.patch(
    "/draft",
    response_model=PatchSettingsDraftApiResponse,
    dependencies=[
        audit_activity(
            "settings.draft.patched",
            "{{ actor.name }} saved settings draft",
        )
    ],
)
async def patch_settings_draft(
    request: PatchSettingsDraftApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> PatchSettingsDraftApiResponse:
    """Patch settings draft (creates if not exists)."""
    tags = ["settings", "drafts"]  # From router tags
    ...
    try:
        ...
        async with conn.transaction():
            ...
            params = PatchSettingsDraftSqlParams(
            ...
            if not result or result.draft_id is None:
                raise ValueError("Failed to patch settings draft")
        ...
        api_response = PatchSettingsDraftApiResponse.model_validate(result.model_dump())
        ...
        handle_route_error(
            ...
            operation="patch_settings_draft",
            sql_query=sql_query,
