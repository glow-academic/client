...
SQL_PATH = "app/sql/v4/queries/views/analytics/simulation_facts/get_analytics_simulation_facts_view_complete.sql"

router = APIRouter()


async def get_simulation_facts_internal(
    ...
    """Internal function for fetching simulation facts data.
    ...
    Returns:
        GetSimulationFactsResponse with items, total_count, and filter options
    """
    ...
    cache_key_val = cache_key(
        "views/analytics/simulation_facts/get",
        {
    ...
    if not bypass_cache:
        ...
        if cached:
            return GetSimulationFactsResponse.model_validate(cached)
    ...
    items: list[SimulationFactsItem] = []
...
@router.post(
    "/get",
    response_model=GetSimulationFactsResponse,
    dependencies=[
        audit_activity(
            "views.analytics.simulation_facts.get",
            "{{ actor.name }} fetched simulation facts data",
        )
    ],
)
async def get_simulation_facts(
    request: GetSimulationFactsRequest,
    ...
    try:
        result = await get_simulation_facts_internal(
            conn=conn,
        ...
        handle_route_error(
            ...
            operation="views_analytics_simulation_facts_get",
            sql_query=sql_query,
