...
async def refresh_strengths_view(
    ...
    """Refresh the attempt_strength_mv materialized view concurrently."""
    ...
    try:
        ...
        return RefreshResponse(
            success=True,
            method="concurrent",
            message=f"Refreshed attempt_strength_mv in {duration_ms}ms",
            duration_ms=duration_ms,
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to refresh attempt_strength_mv: {str(e)}",
        ) from e
