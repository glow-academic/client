...
MV_SQL_PATH = (
    ...
    / "sql/v4/views/simulation/attempt_strength_mv.sql"
)
...
async def recreate_strengths_view(
    ...
    """Recreate the attempt_strength_mv materialized view."""
    ...
    try:
        ...
        return RefreshResponse(
            success=True,
            method="recreate",
            message=f"Recreated attempt_strength_mv in {duration_ms}ms",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to recreate attempt_strength_mv: {str(e)}",
        ) from e
