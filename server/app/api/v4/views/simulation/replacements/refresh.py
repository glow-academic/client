...
async def refresh_replacements_view(
    ...
    """Refresh the mv_attempt_replacement materialized view concurrently."""
    tags = ["views", "simulation", "replacements"]
    try:
        start_time = time.time()
        await conn.execute(
            "REFRESH MATERIALIZED VIEW CONCURRENTLY mv_attempt_replacement"
        )
        ...
        return RefreshResponse(
            success=True,
            method="concurrent",
            message=f"Refreshed mv_attempt_replacement in {duration_ms}ms",
            duration_ms=duration_ms,
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to refresh mv_attempt_replacement: {str(e)}",
        ) from e
