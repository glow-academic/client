...
MV_SQL_PATH = (
    ...
    / "sql/v4/views/simulation/mv_simulation_replacements.sql"
)
...
async def recreate_replacements_view(
    ...
    """Recreate the mv_simulation_replacements materialized view."""
    ...
    try:
        ...
        return RefreshResponse(
            success=True,
            method="recreate",
            message=f"Recreated mv_simulation_replacements in {duration_ms}ms",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to recreate mv_simulation_replacements: {str(e)}",
        ) from e
