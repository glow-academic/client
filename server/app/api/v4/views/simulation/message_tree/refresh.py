...
async def refresh_message_tree_view(
    ...
    """Refresh the attempt_message_tree_mv materialized view concurrently."""
    ...
    try:
        ...
        return RefreshResponse(
            success=True,
            method="concurrent",
            message=f"Refreshed attempt_message_tree_mv in {duration_ms}ms",
            duration_ms=duration_ms,
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to refresh attempt_message_tree_mv: {str(e)}",
        ) from e
