"""Recreate endpoint for simulation chats view."""
...
@router.post(
    ...
    dependencies=[
        audit_activity(
            "views.simulation.chats.recreate",
            "{{ actor.name }} recreated simulation chats view",
        )
    ],
)
async def recreate_chats_view(
    ...
    """Recreate the mv_simulation_chats materialized view."""
    ...
    try:
        ...
        return RefreshResponse(
            success=True,
            method="recreate",
            message=f"Recreated mv_simulation_chats in {duration_ms}ms",
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to recreate mv_simulation_chats: {str(e)}",
        ) from e
