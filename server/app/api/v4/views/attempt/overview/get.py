"""Get endpoint for attempt overview view."""
...
from app.api.v4.views.attempt.overview.types import (
    GetOverviewRequest,
    GetOverviewResponse,
    OverviewViewItem,
    StandardGroupItem,
...
SQL_PATH = (
    "app/sql/v4/queries/views/attempt/overview/get_attempt_overview_view_complete.sql"
)

router = APIRouter()


async def get_attempt_overview_internal(
    ...
    from app.sql.types import (
        GetAttemptOverviewViewSqlParams,
    )

    cache_key_val = cache_key(
        "views/attempt/overview/get",
        {
    ...
    # Execute SQL query
    params = GetAttemptOverviewViewSqlParams(
        start_date=start_date,
        end_date=end_date,
...
@router.post(
    ...
    dependencies=[
        audit_activity(
            "views.attempt.overview.get",
            "{{ actor.name }} fetched attempt overview data",
        )
    ],
)
async def get_overview(
    ...
    """Get attempt overview data from the materialized view.
    ...
    try:
        ...
        result = await get_attempt_overview_internal(
            conn=conn,
        ...
        handle_route_error(
            ...
            operation="views_attempt_overview_get",
            sql_query=sql_query,
