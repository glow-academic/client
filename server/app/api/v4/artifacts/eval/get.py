"""Eval get endpoint - v4 API following DHH principles.
Unified endpoint that handles both new (eval_id = NULL) and detail (eval_id provided).
"""
...
SQL_PATH = "app/sql/v4/evals/get_eval_complete.sql"
...
async def get_eval(
    ...
    if cached:
        ...
        return GetEvalApiResponse.model_validate(cached["data"])
    ...
    try:
        ...
        available_model_runs_search = request.available_model_runs_search
        available_model_runs_agent_ids = request.available_model_runs_agent_ids
        available_model_runs_page = request.available_model_runs_page
        available_model_runs_page_size = request.available_model_runs_page_size
        draft_id = request.draft_id
        eval_id = request.eval_id  # Can be NULL for new mode
        ...
        if result.actor_name:
            ...
            # Only add eval to audit context if eval_id was provided (detail mode)
            if eval_id and result.name_resource and result.name_resource.name:
                audit_ctx["eval"] = {
        ...
        response_data = GetEvalApiResponse.model_validate(
            {
                **result.model_dump(),
