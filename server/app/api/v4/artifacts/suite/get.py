"""Get endpoint for suite artifact — internal + websocket layers."""
...
from app.api.v4.artifacts.suite.types import (
...
# Suite has no domain resources for agent resolution
SUITE_BUNDLE_RESOURCES: set[str] = set()


@dataclass
class SuiteInternalData:
    """Internal data from core suite fetching (cacheable layer)."""
...
async def get_suite_internal(
    ...
    """Fetch config chain for suite artifact.

    Returns a SuiteInternalData dataclass consumed by the websocket wrapper.
    """
    ...
    agent_ids, _create_tool_ids, _link_tool_ids = resolve_agents_for_artifact(
        settings_data.agent_tool_entries, SUITE_BUNDLE_RESOURCES
    )
