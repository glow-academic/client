...
def _extract_tool_websocket_context(result: GetToolSqlRow) -> dict[str, Any]:
...
async def get_tool_internal(
    ...
    """Internal SQL fetch layer for tool get endpoint."""
    return cast(
        GetToolSqlRow,
...
def get_tool_websocket(result: GetToolSqlRow) -> dict[str, Any]:
    """Websocket wrapper layer for tool generation context."""
    return _extract_tool_websocket_context(result)


def get_tool_client(result: GetToolSqlRow) -> GetToolApiResponse:
    """Client/BFF wrapper layer for tool get response."""
    payload = result.model_dump()
    if "generation_context" in payload:
        payload["generation_context"] = get_tool_websocket(result)
    return GetToolApiResponse.model_validate(payload)
...
async def get_tool(
    ...
    try:
        ...
        result = await get_tool_internal(conn, params)
        ...
        response_data = get_tool_client(result)
