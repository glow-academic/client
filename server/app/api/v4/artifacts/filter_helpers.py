"""Shared MV-backed filter option queries for analytics artifacts (mv_chats)."""
...
async def fetch_cohort_filter_options(
    ...
    """Fetch cohort filter options from mv_chats."""
...
async def fetch_department_filter_options(
    ...
    """Fetch department filter options from mv_chats."""
...
async def fetch_date_range_from_mv(
    ...
    """Fetch date range from mv_chats."""
    if not accessible_department_ids:
        return (None, None)
    async with pool.acquire() as c:
