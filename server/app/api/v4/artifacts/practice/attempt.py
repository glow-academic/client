...
@router.post(
    ...
    dependencies=[
        audit_activity(
            "practice.attempt", "{{ actor.name }} viewed practice attempt detail"
        )
    ],
)
async def practice_attempt_detail(
    request: GetPracticeAttemptDetailRequest,
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetPracticeAttemptDetailResponse.model_validate(cached["data"])
