        ...
        if all_run_ids:
            async with pool.acquire() as c:
                rows = await c.fetch(
                    ...
                    LEFT JOIN agent_runs_junction arj ON arj.run_id = re.id AND arj.active = true
                    LEFT JOIN agent_names_junction anj ON anj.agent_id = arj.agent_id AND anj.active = true
