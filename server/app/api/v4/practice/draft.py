"""Practice draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/practice/patch_practice_draft_complete.sql"
...
@router.patch(
    "/draft",
    response_model=PatchPracticeDraftApiResponse,
    dependencies=[
        audit_activity(
            "practice.draft.patched",
            "{{ actor.name }} saved practice draft",
        )
    ],
)
async def patch_practice_draft(
    request: PatchPracticeDraftApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> PatchPracticeDraftApiResponse:
    """Patch practice draft (creates if not exists)."""
    tags = ["practice", "drafts"]  # From router tags
    ...
    try:
        ...
        async with conn.transaction():
            ...
            params = PatchPracticeDraftSqlParams(
            ...
            if not result or result.draft_id is None:
                raise ValueError("Failed to patch practice draft")
        ...
        api_response = PatchPracticeDraftApiResponse.model_validate(result.model_dump())
        ...
        handle_route_error(
            ...
            operation="patch_practice_draft",
            sql_query=sql_query,
