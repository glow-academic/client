"""Parameter draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/parameters/patch_parameter_draft_complete.sql"
...
@router.patch(
    ...
    dependencies=[
        audit_activity(
            "parameter.draft.patched",
            "{{ actor.name }} saved parameter draft",
        )
    ],
)
async def patch_parameter_draft(
    ...
    """Patch parameter draft (creates if not exists)."""
    tags = ["parameters", "drafts"]  # From router tags
