"""Department draft endpoint - v4 API following DHH principles."""
...
@router.patch(
    ...
    dependencies=[
        audit_activity(
            ...
            "{{ actor.name }} saved department draft",
        )
    ],
)
async def patch_department_draft(
    ...
    """Patch department draft (creates if not exists)."""
    tags = ["departments", "drafts"]  # From router tags
    ...
    try:
        ...
        api_response = PatchDepartmentDraftApiResponse.model_validate(
            result.model_dump()
        )
