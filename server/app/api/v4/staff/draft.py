"""Staff draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/staff/patch_staff_draft_complete.sql"
...
@router.patch(
    "/draft",
    response_model=PatchStaffDraftApiResponse,
    dependencies=[
        audit_activity(
            "staff.draft.patched",
            "{{ actor.name }} saved staff draft",
        )
    ],
)
async def patch_staff_draft(
    request: PatchStaffDraftApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> PatchStaffDraftApiResponse:
    """Patch staff draft (creates if not exists)."""
    tags = ["staff", "drafts"]  # From router tags
    ...
    try:
        ...
        async with conn.transaction():
            ...
            params = PatchStaffDraftSqlParams(
            ...
            if not result or result.draft_id is None:
                raise ValueError("Failed to patch staff draft")
        ...
        api_response = PatchStaffDraftApiResponse.model_validate(result.model_dump())
