"""Key draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/keys/patch_key_draft_complete.sql"
...
@router.patch(
    "/draft",
    response_model=PatchKeyDraftApiResponse,
    dependencies=[
        audit_activity(
            "key.draft.patched",
            "{{ actor.name }} saved key draft",
        )
    ],
)
async def patch_key_draft(
    request: PatchKeyDraftApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> PatchKeyDraftApiResponse:
    """Patch key draft (creates if not exists)."""
    tags = ["keys", "drafts"]  # From router tags
    ...
    try:
        ...
        async with conn.transaction():
            ...
            if not result or result.draft_id is None:
                raise ValueError("Failed to patch key draft")
        ...
        api_response = PatchKeyDraftApiResponse.model_validate(result.model_dump())
