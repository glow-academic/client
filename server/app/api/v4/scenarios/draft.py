"""Scenario draft endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/scenario/patch_scenario_draft_complete.sql"
...
@router.patch(
    ...
    dependencies=[
        audit_activity(
            "scenario.draft.patched",
            "{{ actor.name }} saved scenario draft",
        )
    ],
)
async def patch_scenario_draft(
    ...
    """Patch scenario draft (creates if not exists)."""
    tags = ["scenarios", "drafts"]  # From router tags
