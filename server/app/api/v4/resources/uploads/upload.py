...
@router.options("/uploads/upload/{upload_id}")
...
async def tus_creation(request: Request) -> Response:
    ...
    if app_prefix:
        location = f"/{app_prefix}/api/v4/resources/uploads/upload/{upload_id}"
    else:
        location = f"/api/v4/resources/uploads/upload/{upload_id}"
...
@router.head("/uploads/upload/{upload_id}")
