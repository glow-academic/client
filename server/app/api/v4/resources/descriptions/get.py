"""Descriptions GET endpoint - v4 API following DHH principles."""
...
SQL_PATH = "app/sql/v4/queries/resources/descriptions/get_descriptions_complete.sql"
...
async def get_descriptions_internal(
    ...
    if not bypass_cache:
        ...
        if cached:
            return [QGetDescriptionsV4Item.model_validate(item) for item in cached.get("items", [])]

    # Execute SQL
    params = GetDescriptionsSqlParams(ids=ids, search=search)
    result = cast(
        GetDescriptionsSqlRow,
...
@router.post(
    "/descriptions/get",
    response_model=GetDescriptionsApiResponse,
)
async def get_descriptions(
    request: GetDescriptionsApiRequest,
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetDescriptionsApiResponse.model_validate(cached)
    ...
    try:
        ...
        # Execute SQL
        params = GetDescriptionsSqlParams(ids=request.ids, search=request.search)
        ...
        response_data = GetDescriptionsApiResponse(items=items)
