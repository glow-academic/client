...
SQL_PATH = "app/sql/v4/queries/resources/link_descriptions_complete.sql"
...
async def link_descriptions_internal(
    ...
    result_row = LinkDescriptionsSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.description_id:
        raise ValueError(f"Failed to link description: {resource_id}")
    return result_row.description_id
