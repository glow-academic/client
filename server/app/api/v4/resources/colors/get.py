...
async def get_colors_internal(
    ...
    # Execute SQL
    params = GetColorsSqlParams(ids=ids, search=search)
    result = cast(
        GetColorsSqlRow,
...
async def get_colors(
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetColorsApiResponse.model_validate(cached)
    ...
    try:
        ...
        # Execute SQL
        params = GetColorsSqlParams(ids=request.ids, search=request.search)
        ...
        response_data = GetColorsApiResponse(items=items)
