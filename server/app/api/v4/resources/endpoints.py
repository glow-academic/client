...
SQL_PATH = "app/sql/v4/resources/endpoints_complete.sql"
...
@router.post(
    "/endpoints",
    response_model=EndpointsApiResponse,
    dependencies=[
        audit_activity(
            "endpoints.created",
            "{{ actor.name }} created endpoints",
        )
    ],
)
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                endpoints={"id": str(result.endpoints_id)},
            )
