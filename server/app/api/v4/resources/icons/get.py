...
SQL_PATH = "app/sql/v4/queries/resources/icons/get_icons_complete.sql"
...
async def get_icons_internal(
    ...
    if not bypass_cache:
        ...
        if cached:
            return [QGetIconsV4Item.model_validate(item) for item in cached.get("items", [])]

    # Execute SQL
    params = GetIconsSqlParams(ids=ids, search=search)
    result = cast(
        GetIconsSqlRow,
...
async def get_icons(
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetIconsApiResponse.model_validate(cached)
    ...
    try:
        ...
        # Execute SQL
        params = GetIconsSqlParams(ids=request.ids, search=request.search)
        ...
        response_data = GetIconsApiResponse(items=items)
