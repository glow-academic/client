...
from app.api.v4.resources.objectives.types import SearchObjectivesParams
...
SQL_PATH = "app/sql/v4/queries/resources/objectives/search_objectives_complete.sql"
...
@router.post(
    "/objectives/search",
    response_model=SearchObjectivesApiResponse,
)
async def search_objectives(
    request: SearchObjectivesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> SearchObjectivesApiResponse:
    """Search objectives resources."""
    tags = ["resources", "objectives"]
    bypass_cache = http_request.headers.get("X-Bypass-Cache") == "1"

    try:
        items = await search_objectives_internal(
            conn,
            request.search,
            request.limit_count,
        ...
        return SearchObjectivesApiResponse(items=items)
