...
SQL_PATH = (
    "app/sql/v4/queries/entries/test_feedback/search_test_feedback_entries_complete.sql"
)
...
@router.post(
    ...
    response_model=SearchTestFeedbackEntriesApiResponse,
)
async def search_test_feedback_entries(
    request: SearchTestFeedbackEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> SearchTestFeedbackEntriesApiResponse:
    """Search test_feedback entries."""
    tags = ["entries", "test_feedback"]
    bypass_cache = http_request.headers.get("X-Bypass-Cache") == "1"

    try:
        items = await search_test_feedback_entries_internal(
            conn,
            request.search,
            request.limit_count,
        ...
        return SearchTestFeedbackEntriesApiResponse(items=items)
        ...
        handle_route_error(
            ...
            operation="search_test_feedback_entries",
