...
@router.post(
    "/args_values/create",
    response_model=CreateArgsValuesEntriesApiResponse,
    dependencies=[
        audit_activity(
            "args_values.created",
            "{{ actor.name }} created args_values entry",
        )
    ],
)
async def create_args_values_entry(
    request: CreateArgsValuesEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateArgsValuesEntriesApiResponse:
    """Create args_values entry."""
    tags = ["entries", "args_values"]
    ...
    try:
        ...
        api_response = await create_args_values_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            args_values={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_args_values_entry",
            sql_query=sql_query,
            sql_params=None,
