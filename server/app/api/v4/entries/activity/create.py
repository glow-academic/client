...
@router.post(
    "/activity/create",
    response_model=CreateActivityEntriesApiResponse,
    dependencies=[
        audit_activity(
            "activity.created",
            "{{ actor.name }} created activity entry",
        )
    ],
)
async def create_activity_entry(
    request: CreateActivityEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_activity_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            activity={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_activity_entry",
            sql_query=sql_query,
            sql_params=None,
