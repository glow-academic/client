...
@router.post(
    "/health/create",
    response_model=CreateHealthEntriesApiResponse,
    dependencies=[
        audit_activity(
            "health.created",
            "{{ actor.name }} created health entry",
        )
    ],
)
async def create_health_entry(
    request: CreateHealthEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_health_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            health={"id": str(api_response.id)},
        )
