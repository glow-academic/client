...
@router.post(
    "/auth_drafts/create",
    response_model=CreateAuthDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "auth_drafts.created",
            "{{ actor.name }} created auth_drafts entry",
        )
    ],
)
async def create_auth_drafts_entry(
    request: CreateAuthDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateAuthDraftsEntriesApiResponse:
    """Create auth_drafts entry."""
    tags = ["entries", "auth_drafts"]
    ...
    try:
        ...
        api_response = await create_auth_drafts_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            auth_drafts={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_auth_drafts_entry",
            sql_query=sql_query,
            sql_params=None,
