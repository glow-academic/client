...
@router.post(
    "/runs/create",
    response_model=CreateRunsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "runs.created",
            "{{ actor.name }} created runs entry",
        )
    ],
)
async def create_runs_entry(
    request: CreateRunsEntriesApiRequest,
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                runs={"id": str(result.id)},
            )

        api_response = CreateRunsEntriesApiResponse.model_validate(result.model_dump())
