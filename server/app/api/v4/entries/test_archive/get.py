...
from app.sql.types import (
    ...
    GetTestArchiveEntriesSqlParams,
    GetTestArchiveEntriesSqlRow,
    load_sql_query,
)
...
SQL_PATH = (
    "app/sql/v4/queries/entries/test_archive/get_test_archive_entries_complete.sql"
)

router = APIRouter()


async def get_test_archive_entries_internal(
    conn: asyncpg.Connection,
    ids: list[UUID],
    bypass_cache: bool = False,
) -> list[dict]:
    """Internal function to fetch test_archive entries by IDs."""
    if not ids:
        return []
    ...
    params = GetTestArchiveEntriesSqlParams(ids=ids)
    result = cast(
        GetTestArchiveEntriesSqlRow,
