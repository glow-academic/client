...
async def get_attempt_chat_entries_internal(
    ...
    """Internal function to fetch attempt_chat entries by IDs."""
    if not ids:
        return []
    ...
    params = GetAttemptChatEntriesSqlParams(ids=ids)
    result = cast(
        GetAttemptChatEntriesSqlRow,
...
CHATS_VIEW_SQL_PATH = "app/sql/v4/queries/views/chat/get_chat_view_complete.sql"
...
async def get_chats_internal(
    ...
    result = await execute_sql_typed(conn, CHATS_VIEW_SQL_PATH, params=params)
