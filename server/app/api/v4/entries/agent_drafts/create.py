...
@router.post(
    "/agent_drafts/create",
    response_model=CreateAgentDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "agent_drafts.created",
            "{{ actor.name }} created agent_drafts entry",
        )
    ],
)
async def create_agent_drafts_entry(
    request: CreateAgentDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateAgentDraftsEntriesApiResponse:
    """Create agent_drafts entry."""
    tags = ["entries", "agent_drafts"]
    ...
    try:
        ...
        api_response = await create_agent_drafts_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            agent_drafts={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_agent_drafts_entry",
            sql_query=sql_query,
            sql_params=None,
