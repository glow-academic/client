...
@router.post(
    "/audios/create",
    response_model=CreateAudiosEntriesApiResponse,
    dependencies=[
        audit_activity(
            "audios.created",
            "{{ actor.name }} created audios entry",
        )
    ],
)
async def create_audios_entry(
    request: CreateAudiosEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_audios_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            audios={"id": str(api_response.id)},
        )
