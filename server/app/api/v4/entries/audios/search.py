...
SQL_PATH = "app/sql/v4/queries/entries/audios/search_audios_entries_complete.sql"
...
@router.post(
    "/audios/search",
    response_model=SearchAudiosEntriesApiResponse,
)
async def search_audios_entries(
    request: SearchAudiosEntriesApiRequest,
    ...
    try:
        items = await search_audios_entries_internal(
            conn,
            request.search,
            request.limit_count,
        ...
        return SearchAudiosEntriesApiResponse(items=items)
        ...
        handle_route_error(
            ...
            operation="search_audios_entries",
