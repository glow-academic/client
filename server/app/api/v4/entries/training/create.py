...
@router.post(
    "/training/create",
    response_model=CreateTrainingEntriesApiResponse,
    dependencies=[
        audit_activity(
            "training.created",
            "{{ actor.name }} created training entry",
        )
    ],
)
async def create_training_entry(
    request: CreateTrainingEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_training_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            training={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_training_entry",
            sql_query=sql_query,
            sql_params=None,
