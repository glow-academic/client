...
@router.post(
    "/training_drafts/create",
    response_model=CreateTrainingDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "training_drafts.created",
            "{{ actor.name }} created training_drafts entry",
        )
    ],
)
async def create_training_drafts_entry(
    request: CreateTrainingDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateTrainingDraftsEntriesApiResponse:
    """Create training_drafts entry."""
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                training_drafts={"id": str(result.id)},
            )

        api_response = CreateTrainingDraftsEntriesApiResponse.model_validate(
            result.model_dump()
        )
        ...
        handle_route_error(
            ...
            operation="create_training_drafts_entry",
            sql_query=sql_query,
