...
@router.post(
    "/bindings/create",
    response_model=CreateBindingsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "bindings.created",
            "{{ actor.name }} created bindings entry",
        )
    ],
)
async def create_bindings_entry(
    request: CreateBindingsEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_bindings_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            bindings={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_bindings_entry",
            sql_query=sql_query,
            sql_params=None,
