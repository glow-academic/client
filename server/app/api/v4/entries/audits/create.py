...
@router.post(
    "/audits/create",
    response_model=CreateAuditsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "audits.created",
            "{{ actor.name }} created audits entry",
        )
    ],
)
async def create_audits_entry(
    request: CreateAuditsEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_audits_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            audits={"id": str(api_response.id)},
        )
