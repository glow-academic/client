...
@router.post(
    "/suite/create",
    response_model=CreateSuiteEntriesApiResponse,
    dependencies=[
        audit_activity(
            "suite.created",
            "{{ actor.name }} created suite entry",
        )
    ],
)
async def create_suite_entry(
    request: CreateSuiteEntriesApiRequest,
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                suite={"id": str(result.id)},
            )

        api_response = CreateSuiteEntriesApiResponse.model_validate(result.model_dump())
