...
@router.post(
    "/persona/create",
    response_model=CreatePersonaEntriesApiResponse,
    dependencies=[
        audit_activity(
            "persona.created",
            "{{ actor.name }} created persona entry",
        )
    ],
)
async def create_persona_entry(
    request: CreatePersonaEntriesApiRequest,
    ...
    try:
        ...
        api_response = CreatePersonaEntriesApiResponse.model_validate(
            result.model_dump()
        )
