...
@router.post(
    "/practice_training/create",
    response_model=CreatePracticeTrainingEntriesApiResponse,
    dependencies=[
        audit_activity(
            "practice_training.created",
            "{{ actor.name }} created practice_training entry",
        )
    ],
)
async def create_practice_training_entry(
    request: CreatePracticeTrainingEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreatePracticeTrainingEntriesApiResponse:
    """Create practice_training entry."""
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                practice_training={"id": str(result.id)},
            )

        api_response = CreatePracticeTrainingEntriesApiResponse.model_validate(
            result.model_dump()
        )
        ...
        handle_route_error(
            ...
            operation="create_practice_training_entry",
            sql_query=sql_query,
