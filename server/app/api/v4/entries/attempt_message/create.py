"""Attempt Message entry CREATE endpoint."""
...
SQL_PATH = "app/sql/v4/queries/entries/attempt_message/create_attempt_message_entries_complete.sql"
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                attempt_message={"id": str(result.id)},
            )

        api_response = CreateAttemptMessageEntriesApiResponse.model_validate(
            result.model_dump()
        )
