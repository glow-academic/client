...
@router.post(
    "/cohort_drafts/create",
    response_model=CreateCohortDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "cohort_drafts.created",
            "{{ actor.name }} created cohort_drafts entry",
        )
    ],
)
async def create_cohort_drafts_entry(
    request: CreateCohortDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateCohortDraftsEntriesApiResponse:
    """Create cohort_drafts entry."""
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                cohort_drafts={"id": str(result.id)},
            )

        api_response = CreateCohortDraftsEntriesApiResponse.model_validate(
            result.model_dump()
        )
        ...
        handle_route_error(
            ...
            operation="create_cohort_drafts_entry",
            sql_query=sql_query,
