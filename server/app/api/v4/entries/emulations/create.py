...
@router.post(
    "/emulations/create",
    response_model=CreateEmulationsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "emulations.created",
            "{{ actor.name }} created emulations entry",
        )
    ],
)
async def create_emulations_entry(
    request: CreateEmulationsEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_emulations_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            emulations={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_emulations_entry",
            sql_query=sql_query,
            sql_params=None,
