...
@router.post(
    "/messages/create",
    response_model=CreateMessagesEntriesApiResponse,
    dependencies=[
        audit_activity(
            "messages.created",
            "{{ actor.name }} created messages entry",
        )
    ],
)
async def create_messages_entry(
    request: CreateMessagesEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_messages_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            messages={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_messages_entry",
            sql_query=sql_query,
            sql_params=None,
