...
@router.post(
    ...
    dependencies=[
        audit_activity(
            ...
            "{{ actor.name }} created attempt_replacement entry",
        )
    ],
)
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                attempt_replacement={"id": str(result.id)},
            )

        api_response = CreateAttemptReplacementEntriesApiResponse.model_validate(
            result.model_dump()
        )
