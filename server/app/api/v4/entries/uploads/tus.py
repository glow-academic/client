"""TUS protocol endpoints - v4 API following DHH principles."""
...
@router.options("/uploads/tus/{upload_id}")
...
async def tus_creation(request: Request) -> Response:
    ...
    if app_prefix:
        location = f"/{app_prefix}/api/v4/entries/uploads/tus/{upload_id}"
    else:
        location = f"/api/v4/entries/uploads/tus/{upload_id}"
...
@router.head("/uploads/tus/{upload_id}")
...
@router.post(
    "/uploads/upload/{upload_id}/finalize",
