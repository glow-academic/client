...
@router.post(
    "/sessions/create",
    response_model=CreateSessionsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "sessions.created",
            "{{ actor.name }} created sessions entry",
        )
    ],
)
async def create_sessions_entry(
    request: CreateSessionsEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_sessions_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            sessions={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_sessions_entry",
            sql_query=sql_query,
            sql_params=None,
