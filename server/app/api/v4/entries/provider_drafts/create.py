...
@router.post(
    "/provider_drafts/create",
    response_model=CreateProviderDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "provider_drafts.created",
            "{{ actor.name }} created provider_drafts entry",
        )
    ],
)
async def create_provider_drafts_entry(
    request: CreateProviderDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateProviderDraftsEntriesApiResponse:
    """Create provider_drafts entry."""
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                provider_drafts={"id": str(result.id)},
            )

        api_response = CreateProviderDraftsEntriesApiResponse.model_validate(
            result.model_dump()
        )
        ...
        handle_route_error(
            ...
            operation="create_provider_drafts_entry",
            sql_query=sql_query,
