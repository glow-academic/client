...
@router.post(
    "/field_drafts/create",
    response_model=CreateFieldDraftsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "field_drafts.created",
            "{{ actor.name }} created field_drafts entry",
        )
    ],
)
async def create_field_drafts_entry(
    request: CreateFieldDraftsEntriesApiRequest,
    http_request: Request,
    response: Response,
    conn: Annotated[asyncpg.Connection, Depends(get_db)],
) -> CreateFieldDraftsEntriesApiResponse:
    """Create field_drafts entry."""
    tags = ["entries", "field_drafts"]
    ...
    try:
        ...
        api_response = await create_field_drafts_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            field_drafts={"id": str(api_response.id)},
        )
        ...
        handle_route_error(
            ...
            operation="create_field_drafts_entry",
            sql_query=sql_query,
            sql_params=None,
