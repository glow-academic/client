...
@router.post(
    "/groups/create",
    response_model=CreateGroupsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "groups.created",
            "{{ actor.name }} created groups entry",
        )
    ],
)
async def create_groups_entry(
    request: CreateGroupsEntriesApiRequest,
    ...
    try:
        ...
        api_response = await create_groups_entry_internal(conn, request_dict, mcp)

        audit_set(
            http_request,
            actor={"id": profile_id},
            groups={"id": str(api_response.id)},
        )
