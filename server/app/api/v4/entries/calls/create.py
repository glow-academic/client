...
@router.post(
    "/calls/create",
    response_model=CreateCallsEntriesApiResponse,
    dependencies=[
        audit_activity(
            "calls.created",
            "{{ actor.name }} created calls entry",
        )
    ],
)
async def create_calls_entry(
    request: CreateCallsEntriesApiRequest,
    ...
    try:
        ...
        async with conn.transaction():
            ...
            audit_set(
                http_request,
                actor={"id": profile_id},
                calls={"id": str(result.id)},
            )

        api_response = CreateCallsEntriesApiResponse.model_validate(result.model_dump())
