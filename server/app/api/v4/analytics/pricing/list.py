"""Pricing list endpoint - POST /analytics/pricing/list"""
...
SQL_PATH = "app/sql/v4/queries/pricing/get_pricing_runs_complete.sql"
...
@router.post(
    "/list",
    response_model=GetPricingRunsApiResponse,
    dependencies=[
        audit_activity("pricing.list", "{{ actor.name }} viewed pricing list")
    ],
)
async def get_pricing_list(
    request: GetPricingRunsApiRequest,
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return GetPricingRunsApiResponse.model_validate(cached["data"])
    ...
    try:
        ...
        api_response = GetPricingRunsApiResponse.model_validate(result.model_dump())
