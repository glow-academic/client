...
class RedisRequestStorage(RequestStorage):
    ...
    async def set(self, key: str, field: str, value: Any) -> None:
        ...
        try:
            ...
            logger.error(
                f"Redis set error for key {key}, field {field}: {e}", exc_info=True
            )
    ...
    async def delete(self, key: str, field: str | None = None) -> None:
        ...
        try:
            ...
            logger.error(
                f"Redis delete error for key {key}, field {field}: {e}", exc_info=True
            )
...
def create_request_storage(
    redis_client: Any | None, ttl_seconds: int = 3600
) -> RequestStorage:  # type: ignore[return]
