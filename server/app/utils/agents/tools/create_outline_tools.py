...
from app.main import get_outline_storage, get_question_storage
...
def create_outline_tools(
    ...
    if not profile_id or not primary_id:
        logger.warning(
            "profile_id and primary_id required for outline/question storage"
        )
    ...
    async def set_outline(
        ...
        outline: str = Field(description="The detailed outline content for the video"),
        ...
        await storage.set(storage_key, "name", name)
        await storage.set(storage_key, "outline", outline)
        if question_timestamps is not None:
            # Ensure question_timestamps is a dict
            if isinstance(question_timestamps, dict):
                timestamps_dict = question_timestamps
            elif isinstance(question_timestamps, str):
                # Try to parse if it's a string
                timestamps_dict = json.loads(question_timestamps)
            else:
                timestamps_dict = question_timestamps
            await storage.set(storage_key, "question_timestamps", timestamps_dict)
        await storage.set(storage_key, "outline_progress", True)
        ...
        if question_timestamps:
            if isinstance(question_timestamps, dict):
                logger.info(
                    f"✓ Set question timestamps for {len(question_timestamps)} question(s)"
                )
    ...
    async def set_video_name(
        ...
        await storage.set(storage_key, "video_name", video_name)
        await storage.set(storage_key, "video_name_progress", True)
    ...
    if include_questions:
        ...
        logger.info(
            f"Created {len(tools)} outline and question tools (multiple choice and multi-select)"
        )
    else:
