...
def create_outline_tools(
    ...
    if not profile_id or not primary_id:
        logger.warning("profile_id and primary_id required for outline/question storage")
    ...
    async def set_outline(
        ...
        if question_timestamps:
            if isinstance(question_timestamps, dict):
                logger.info(f"✓ Set question timestamps for {len(question_timestamps)} question(s)")
