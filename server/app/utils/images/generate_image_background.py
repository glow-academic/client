    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            try:
                ...
                logger.error(f"Image generation failed for {image_id}: {e}", exc_info=True)
                await _emit_image_error(
...
async def _emit_image_complete(
    ...
    from app.socket.scenarios.generate_ai import scenario_image_generation_complete
    from app.socket.scenarios.generate_ai import ScenarioImageGenerationCompletePayload
    
    if not room:
        logger.warning(f"No room specified for image {image_id}, cannot emit WebSocket event")
        return
...
async def _emit_image_error(
    ...
    from app.main import get_image_generation_storage
    from app.socket.scenarios.generate_ai import scenario_image_generation_error
    from app.socket.scenarios.generate_ai import ScenarioImageGenerationErrorPayload
