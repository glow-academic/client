...
def _sql_path_to_route_name(sql_path: str) -> str | None:
    ...
    Handles paths of arbitrary depth:
        "app/sql/queries/agents/create_agent_complete.sql" -> "create_agent"
        "app/sql/queries/documents/tools/html/document_tool_html_complete_complete.sql" -> "document_tool_html_complete"
        "tests/sql/integration/queries/infra/activity/insert_test_profile.sql" -> "insert_test_profile"
    ...
    """
    app_sql_prefix = "app/sql/queries/"
    tests_sql_prefix = "tests/sql/integration/queries/"

    # Pattern: app/sql/queries/[resource]/[operation]_complete.sql
    # Pattern: app/sql/queries/infrastructure/infrastructure_[category]_[operation]_complete.sql -> infra_[category]_[operation]
    # Pattern: app/sql/queries/[any]/[path]/[operation]_complete.sql (arbitrary depth)
    ...
    # Pattern: tests/sql/integration/queries/infra/[resource]/[operation].sql
    ...
    # Pattern: tests/sql/integration/queries/socket/[operation].sql (allow nested folders)
    ...
    # Pattern: tests/sql/integration/queries/api/[resource]/test_[operation]_v4_complete.sql
    if sql_path.startswith(f"{tests_sql_prefix}api/"):
        ...
        if operation.endswith("_v4"):
            operation = operation[: -len("_v4")]
        return operation.replace("-", "_")

    # Pattern: tests/sql/integration/queries/helpers/test_[operation]_v4_complete.sql
    ...
    # Pattern: tests/sql/integration/queries/conftest/test_[operation]_v4_complete.sql
...
def _sort_sql_files(sql_file: Path, server_root: Path) -> tuple[int, str]:
    ...
    analytics_dependent_paths = [
        "app/sql/queries/dashboard/get_dashboard_bundle_complete.sql",
        "app/sql/queries/home/get_home_overview_complete.sql",
        "app/sql/queries/leaderboard/get_leaderboard_bundle_complete.sql",
        "app/sql/queries/practice/get_practice_overview_complete.sql",
        "app/sql/queries/reports/get_per_simulation_metrics_complete.sql",
        "app/sql/queries/reports/get_reports_bundle_complete.sql",
        "app/sql/queries/reports/get_reports_overview_complete.sql",
    ]
    ...
    if sql_path == "app/sql/queries/profile/get_profile_complete.sql":
        return (14, sql_path)

    if sql_path == "app/sql/queries/profile/get_profile_access_complete.sql":
        return (15, sql_path)
...
def is_types_file_complete(
    ...
    test_sql_dir = server_root / "tests" / "sql" / "integration"
...
async def generate_types_for_sql_file(
    ...
    try:
        ...
        from app.sql.sql_introspect import introspect_sql_file
        from app.sql.sql_typegen import generate_types_file
...
async def compile_sql_types(
    ...
    if server_root is None:
        ...
        # compile_types.py is at: server/app/sql/compile_types.py
        # We need to go up 2 levels: sql -> app -> server
    ...
    if incremental_mode:
        ...
        for file_path_str in sql_files or []:
            ...
            try:
                ...
                if not (
                    rel_str.startswith("app/sql/")
                    or rel_str.startswith("tests/sql/integration/")
                ):
                    print(
                        f"⚠️  File must be in app/sql/ or tests/sql/integration/: {rel_str}"
                    )
                    continue
            except ValueError:
