...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_upload_file_info_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_upload_file_info_v3(%s)', r.sig);
    END LOOP;
END $$;

-- Create function that returns upload file info with existence check
CREATE OR REPLACE FUNCTION api_get_upload_file_info_v3(
    upload_id uuid,
    profile_id uuid
)
RETURNS TABLE (
