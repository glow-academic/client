...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_home_history_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_home_history_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_home_history_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_home_history_v3_attempt_history_row AS (
    attempt_id uuid,
    date timestamptz,
    profile_id uuid,
    profile_name text,
...
CREATE TYPE types.q_get_home_history_v3_option AS (
    value text,
    label text,
    count int
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_home_history_v3(
...
RETURNS TABLE (
    actor_name text,
    data types.q_get_home_history_v3_attempt_history_row[],
    total_count int,
    page int,
    page_size int,
    total_pages int,
    profile_options types.q_get_home_history_v3_option[],
    simulation_options types.q_get_home_history_v3_option[],
    scenario_options types.q_get_home_history_v3_option[]
)
LANGUAGE sql
STABLE
AS $$
...
history_rows_agg AS (
    SELECT COALESCE(ARRAY_AGG(
        ...
        )::types.q_get_home_history_v3_attempt_history_row
        ORDER BY pr.rn
    ), '{}'::types.q_get_home_history_v3_attempt_history_row[]) as data
...
profile_options_agg AS (
    SELECT COALESCE(ARRAY_AGG((poc.profile_id::text, poc.profile_name, poc.count)::types.q_get_home_history_v3_option), '{}'::types.q_get_home_history_v3_option[]) as profile_options
    FROM profile_options_cte poc
),
simulation_options_agg AS (
    SELECT COALESCE(ARRAY_AGG((soc.simulation_id::text, soc.simulation_name, soc.count)::types.q_get_home_history_v3_option), '{}'::types.q_get_home_history_v3_option[]) as simulation_options
    FROM simulation_options_cte soc
),
scenario_options_agg AS (
    SELECT COALESCE(ARRAY_AGG((scoc.scenario_id::text, scoc.scenario_title, scoc.count)::types.q_get_home_history_v3_option), '{}'::types.q_get_home_history_v3_option[]) as scenario_options
...
SELECT 
    ...
    COALESCE((SELECT data FROM history_rows_agg), '{}'::types.q_get_home_history_v3_attempt_history_row[]) as data,
    pi.total_count,
    pi.page,
    pi.page_size,
    pi.total_pages,
    COALESCE((SELECT profile_options FROM profile_options_agg), '{}'::types.q_get_home_history_v3_option[]) as profile_options,
    COALESCE((SELECT simulation_options FROM simulation_options_agg), '{}'::types.q_get_home_history_v3_option[]) as simulation_options,
    COALESCE((SELECT scenario_options FROM scenario_options_agg), '{}'::types.q_get_home_history_v3_option[]) as scenario_options
