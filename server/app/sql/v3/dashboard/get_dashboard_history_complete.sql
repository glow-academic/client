...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_dashboard_history_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_dashboard_history_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_dashboard_history_v3_%'
...
CREATE TYPE types.q_get_dashboard_history_v3_attempt_history_row AS (
    attempt_id uuid,
    date timestamptz,
    profile_id uuid,
    profile_name text,
...
CREATE TYPE types.q_get_dashboard_history_v3_profile_option AS (
    value text,
    label text,
    count int
);
...
CREATE TYPE types.q_get_dashboard_history_v3_simulation_option AS (
    value text,
    label text,
    count int
);
...
CREATE TYPE types.q_get_dashboard_history_v3_scenario_option AS (
    value text,
    label text,
    count int
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_dashboard_history_v3(
...
RETURNS TABLE (
    data types.q_get_dashboard_history_v3_attempt_history_row[],
    total_count bigint,
    archived_count bigint,
    unarchived_count bigint,
    profile_options types.q_get_dashboard_history_v3_profile_option[],
    simulation_options types.q_get_dashboard_history_v3_simulation_option[],
    scenario_options types.q_get_dashboard_history_v3_scenario_option[]
)
LANGUAGE sql
STABLE
AS $$
...
history_attempts AS (
    ...
    WHERE sa.created_at >= (SELECT start_date FROM params)
      ...
      AND (
        (cardinality((SELECT simulation_filters FROM params)::text[]) = 0) AND sim.practice_simulation = FALSE
        OR
        (cardinality((SELECT simulation_filters FROM params)::text[]) > 0 AND (
          ('general' = ANY((SELECT simulation_filters FROM params)::text[]) AND sim.practice_simulation = FALSE) OR
          ('practice' = ANY((SELECT simulation_filters FROM params)::text[]) AND sim.practice_simulation = TRUE) OR
...
data_agg AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_dashboard_history_v3_attempt_history_row
            ORDER BY 
                CASE 
...
profile_options_agg AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (profile_id::text, profile_name, count)::types.q_get_dashboard_history_v3_profile_option
            ORDER BY profile_name
        ),
        '{}'::types.q_get_dashboard_history_v3_profile_option[]
    ) AS profile_options
...
simulation_options_agg AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (simulation_id::text, simulation_name, count)::types.q_get_dashboard_history_v3_simulation_option
            ORDER BY simulation_name
        ),
        '{}'::types.q_get_dashboard_history_v3_simulation_option[]
...
scenario_options_agg AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (scenario_id::text, scenario_title, count)::types.q_get_dashboard_history_v3_scenario_option
            ORDER BY scenario_title
        ),
        '{}'::types.q_get_dashboard_history_v3_scenario_option[]
    ) AS scenario_options
