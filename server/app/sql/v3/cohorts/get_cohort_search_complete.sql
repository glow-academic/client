...
DROP FUNCTION IF EXISTS api_get_cohort_search_v3(uuid, uuid, text, uuid[], int) CASCADE;

-- 2) Drop types WITHOUT CASCADE (reuse staff_item from new endpoint)
DROP TYPE IF EXISTS types.q_get_cohort_search_v3_cohort;
DROP TYPE IF EXISTS types.q_get_cohort_search_v3_department;

-- 3) Recreate types (reuse staff_item from new endpoint)
CREATE TYPE types.q_get_cohort_search_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_cohort_search_v3_department AS (
    department_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_cohort_search_v3(
    p_profile_id uuid,
...
RETURNS TABLE (
    staff types.q_get_cohort_new_v3_staff_item[],
    cohorts types.q_get_cohort_search_v3_cohort[],
    departments types.q_get_cohort_search_v3_department[],
    actor_name text
)
LANGUAGE sql
STABLE
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.cohort_id, cd.name, cd.description)::types.q_get_cohort_search_v3_cohort
            ORDER BY cd.name
        ) FROM cohorts_data cd),
        '{}'::types.q_get_cohort_search_v3_cohort[]
    ) as cohorts,
    -- Aggregate departments separately
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_get_cohort_search_v3_department
            ORDER BY dd.name
        ) FROM departments_data dd),
        '{}'::types.q_get_cohort_search_v3_department[]
    ) as departments,
    up.actor_name
