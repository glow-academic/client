...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_cohort_new_v3_%'
...
CREATE TYPE types.q_get_cohort_new_v3_staff_item AS (
    profile_id uuid,
    first_name text,
    last_name text,
    emails text[],
...
CREATE TYPE types.q_get_cohort_new_v3_profile AS (
    profile_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_cohort_new_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_cohort_new_v3_department_for_staff AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_cohort_new_v3_department AS (
    department_id uuid,
    name text,
    description text,
    simulation_ids text[],
...
CREATE OR REPLACE FUNCTION api_get_cohort_new_v3(
    profile_id uuid
)
RETURNS TABLE (
    title text,
    ...
    profiles types.q_get_cohort_new_v3_profile[],
    staff types.q_get_cohort_new_v3_staff_item[],
    cohorts types.q_get_cohort_new_v3_cohort[],
    departments_for_staff types.q_get_cohort_new_v3_department_for_staff[],
    departments types.q_get_cohort_new_v3_department[],
...
cohort_staff AS (
    SELECT DISTINCT ON (p.id)
        ...
        CASE 
            ...
            WHEN ups.role = profile_role.superadmin THEN true
            WHEN ups.role = profile_role.admin AND p.role IN (profile_role.instructional, profile_role.member, profile_role.guest) THEN true
            WHEN ups.role = profile_role.instructional AND p.role IN (profile_role.member, profile_role.guest) THEN true
            WHEN ups.role = profile_role.member AND p.role = profile_role.guest THEN true
            ELSE false
        END as can_edit,
        CASE 
        ...
        CASE 
            WHEN p.id = (SELECT profile_id FROM params) THEN false
            WHEN ups.role = profile_role.superadmin THEN true
            WHEN ups.role = profile_role.admin AND p.role IN (profile_role.admin, profile_role.instructional, profile_role.member, profile_role.guest) THEN true
            WHEN ups.role = profile_role.instructional AND p.role IN (profile_role.instructional, profile_role.member, profile_role.guest) THEN true
            WHEN ups.role = profile_role.member AND p.role IN (profile_role.member, profile_role.guest) THEN true
            WHEN ups.role = profile_role.guest AND p.role = profile_role.guest THEN true
            ELSE false
        END as can_remove
    FROM params x
    ...
    WHERE (
        ups.role = profile_role.superadmin OR
        (ups.role = profile_role.admin AND p.role IN (profile_role.admin, profile_role.instructional, profile_role.member, profile_role.guest)) OR
        (ups.role = profile_role.instructional AND p.role IN (profile_role.instructional, profile_role.member, profile_role.guest)) OR
        (ups.role = profile_role.member AND p.role IN (profile_role.member, profile_role.guest)) OR
        (ups.role = profile_role.guest AND p.role = profile_role.guest)
    )
...
SELECT 
    ...
    (SELECT COALESCE(
        ARRAY_AGG(
            (css.simulation_id::text, css.name, css.description, css.time_limit, css.active, css.position, css.usage_count, css.success_rate, css.last_used, CASE WHEN css.usage_count = 0 THEN true ELSE false END)::types.q_get_cohort_detail_v3_simulation
    ...
    (SELECT COALESCE(
        ARRAY_AGG(
            (cmd.cohort_id, cmd.name, cmd.description)::types.q_get_cohort_new_v3_cohort
        ),
        '{}'::types.q_get_cohort_new_v3_cohort[]
    )
     FROM cohort_mapping_data cmd) as cohorts,
    (SELECT COALESCE(
        ARRAY_AGG(
            (dmfsd.department_id, dmfsd.name, dmfsd.description)::types.q_get_cohort_new_v3_department_for_staff
        ),
        '{}'::types.q_get_cohort_new_v3_department_for_staff[]
    )
     FROM department_mapping_for_staff_data dmfsd) as departments_for_staff,
    (SELECT COALESCE(
        ARRAY_AGG(
            (dmd.department_id, dmd.name, dmd.description, dmd.simulation_ids, dmd.staff_ids)::types.q_get_cohort_new_v3_department
        ),
        '{}'::types.q_get_cohort_new_v3_department[]
    )
