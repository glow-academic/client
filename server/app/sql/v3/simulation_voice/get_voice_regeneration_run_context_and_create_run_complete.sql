...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_voice_regeneration_run_context_and_create_run_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_voice_regeneration_run_context_and_create_run_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_get_voice_regeneration_run_context_and_create_run_v3_%'
...
CREATE TYPE types.i_get_voice_regeneration_run_context_and_create_run_v3_document AS (
    id text,
    name text,
    url text,
    type text,
...
CREATE TYPE types.i_get_voice_regeneration_run_context_and_create_run_v3_msg AS (
    role text,
    content text
);
...
CREATE OR REPLACE FUNCTION socket_get_voice_regeneration_run_context_and_create_run_v3(
    chat_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    agent_id uuid,
    voice_agent_id uuid,
    documents types.i_get_voice_regeneration_run_context_and_create_run_v3_document[],
    run_id text,
    previous_messages types.i_get_voice_regeneration_run_context_and_create_run_v3_msg[]
)
LANGUAGE sql
VOLATILE
AS $$
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.i_get_voice_regeneration_run_context_and_create_run_v3_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.i_get_voice_regeneration_run_context_and_create_run_v3_msg[]
...
documents_data AS (
    ...
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (d.id::text, d.name, COALESCE(u.file_path, ''), COALESCE(u.mime_type, ''), d.created_at)::types.i_get_voice_regeneration_run_context_and_create_run_v3_document
                ORDER BY d.created_at
            ) FILTER (WHERE d.id IS NOT NULL AND sd.active = true AND d.active = true),
            ARRAY[]::types.i_get_voice_regeneration_run_context_and_create_run_v3_document[]
        ) as documents
    FROM params p
    JOIN chats sc ON sc.id = p.chat_id
    JOIN attempt_chats ac ON ac.chat_id = sc.id
...
SELECT 
    ...
    COALESCE(dd.documents, ARRAY[]::types.i_get_voice_regeneration_run_context_and_create_run_v3_document[]) as documents,
