...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_auth_list_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_auth_list_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_auth_list_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_auth_list_v3_auth_item AS (
    auth_item_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_auth_list_v3_auth AS (
    auth_id uuid,
    name text,
    description text,
    active boolean,
    ...
    num_items integer,
    sample_items types.q_get_auth_list_v3_auth_item[],
    can_edit boolean,
    can_delete boolean,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_auth_list_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    auths types.q_get_auth_list_v3_auth[]
)
LANGUAGE sql
STABLE
AS $$
...
auth_sample_items AS (
    SELECT 
        ai.auth_id,
        ARRAY_AGG(
            (ai.id, ai.name, ai.description)::types.q_get_auth_list_v3_auth_item
            ORDER BY ai.name
        ) as sample_items
    FROM (
