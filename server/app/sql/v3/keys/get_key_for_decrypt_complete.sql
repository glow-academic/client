...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_key_for_decrypt_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_key_for_decrypt_v3(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Recreate function (no types to drop for decrypt)
CREATE OR REPLACE FUNCTION api_get_key_for_decrypt_v3(
    key_id uuid,
    profile_id uuid
)
RETURNS TABLE (
