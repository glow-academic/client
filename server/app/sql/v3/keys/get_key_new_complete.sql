...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_key_new_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_key_new_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_key_new_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_key_new_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_key_new_v3_model AS (
    model_id uuid,
    name text,
    description text,
    provider text,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_key_new_v3(profile_id uuid)
RETURNS TABLE (
    key_id text,
    name text,
    key_masked text,
    ...
    can_edit boolean,
    departments types.q_get_key_new_v3_department[],
    models types.q_get_key_new_v3_model[],
    actor_name text
)
LANGUAGE sql
STABLE
...
SELECT 
    ...
    COALESCE(
        ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_get_key_new_v3_department
            ORDER BY dd.name
        ) FILTER (WHERE dd.department_id IS NOT NULL),
        '{}'::types.q_get_key_new_v3_department[]
    ) as departments,
    '{}'::types.q_get_key_new_v3_model[] as models,
