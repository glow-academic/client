...
DROP FUNCTION IF EXISTS api_list_fields_v3(uuid);

-- 2) Drop types WITHOUT CASCADE
DROP TYPE IF EXISTS types.q_list_fields_v3_field;
DROP TYPE IF EXISTS types.q_list_fields_v3_parameter;
DROP TYPE IF EXISTS types.q_list_fields_v3_department;
DROP TYPE IF EXISTS types.q_list_fields_v3_option;

-- 3) Recreate types
CREATE TYPE types.q_list_fields_v3_field AS (
    field_id uuid,
    name text,
    description text,
    active boolean,
...
CREATE TYPE types.q_list_fields_v3_parameter AS (
    parameter_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_fields_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_fields_v3_option AS (
    value text,
    label text
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_list_fields_v3(
    profile_id uuid
)
RETURNS TABLE (
    actor_name text,
    fields types.q_list_fields_v3_field[],
    parameters types.q_list_fields_v3_parameter[],
    departments types.q_list_fields_v3_department[],
    parameter_options types.q_list_fields_v3_option[],
    department_options types.q_list_fields_v3_option[]
)
LANGUAGE sql
STABLE
AS $$
        ...
        ARRAY_AGG(
            (p.parameter_id::text,
             CASE 
                 ...
                 ELSE p.name
             END)::types.q_list_fields_v3_option
            ORDER BY p.name
...
department_options_data AS (
    SELECT 
        ARRAY_AGG(
            (d.department_id::text, d.name)::types.q_list_fields_v3_option
            ORDER BY d.name
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (fd.field_id, fd.name, fd.description, fd.active, fd.created_at, fd.updated_at, fd.department_ids, fd.parameter_ids, fd.conditional_parameter_ids, fd.can_edit, fd.can_delete, fd.can_duplicate)::types.q_list_fields_v3_field
            ORDER BY fd.name
        ) FROM fields_data fd),
        '{}'::types.q_list_fields_v3_field[]
    ) as fields,
    -- Aggregate parameters
    COALESCE(
        (SELECT ARRAY_AGG(
            (pd.parameter_id, pd.name, pd.description)::types.q_list_fields_v3_parameter
            ORDER BY pd.name
        ) FROM parameter_data pd),
        '{}'::types.q_list_fields_v3_parameter[]
    ) as parameters,
    -- Aggregate departments
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_list_fields_v3_department
            ORDER BY dd.name
        ) FROM department_data dd),
        '{}'::types.q_list_fields_v3_department[]
    ) as departments,
    -- Parameter options (for UI filtering)
    COALESCE((SELECT options FROM parameter_options_data), '{}'::types.q_list_fields_v3_option[]) as parameter_options,
    -- Department options (for UI filtering)
    COALESCE((SELECT options FROM department_options_data), '{}'::types.q_list_fields_v3_option[]) as department_options
