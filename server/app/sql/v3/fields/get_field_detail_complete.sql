...
DROP FUNCTION IF EXISTS api_get_field_detail_v3(uuid, uuid);

-- 2) Drop types WITHOUT CASCADE
DROP TYPE IF EXISTS types.q_get_field_detail_v3_department;
DROP TYPE IF EXISTS types.q_get_field_detail_v3_parameter;

-- 3) Recreate types
CREATE TYPE types.q_get_field_detail_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_field_detail_v3_parameter AS (
    parameter_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_field_detail_v3(
    field_id uuid,
    profile_id uuid
)
RETURNS TABLE (
    ...
    departments types.q_get_field_detail_v3_department[],
    valid_department_ids text[],
    parameters types.q_get_field_detail_v3_parameter[],
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (vdd.department_id, vdd.name, vdd.description)::types.q_get_field_detail_v3_department
            ORDER BY vdd.name
        ) FROM valid_departments_data vdd),
        '{}'::types.q_get_field_detail_v3_department[]
    ) as departments,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (vpd.parameter_id, vpd.name, vpd.description)::types.q_get_field_detail_v3_parameter
            ORDER BY vpd.name
        ) FROM valid_parameters_data vpd),
        '{}'::types.q_get_field_detail_v3_parameter[]
    ) as parameters,
    -- Valid parameter IDs
