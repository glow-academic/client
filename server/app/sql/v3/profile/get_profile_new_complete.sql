...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_profile_new_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_profile_new_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_profile_new_v3_%'
...
CREATE TYPE types.q_get_profile_new_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_profile_new_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_profile_new_v3(
    profile_id uuid
)
RETURNS TABLE (
    first_name text,
    ...
    role_options text[],
    departments types.q_get_profile_new_v3_department[],
    cohorts types.q_get_profile_new_v3_cohort[],
    actor_name text
)
LANGUAGE sql
STABLE
...
valid_departments_agg AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (vdd.department_id, vdd.name, vdd.description)::types.q_get_profile_new_v3_department
                ORDER BY vdd.name
            ) FILTER (WHERE vdd.department_id IS NOT NULL),
            '{}'::types.q_get_profile_new_v3_department[]
        ) as departments
...
all_cohorts_agg AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (acd.cohort_id, acd.name, acd.description)::types.q_get_profile_new_v3_cohort
                ORDER BY acd.name
            ) FILTER (WHERE acd.cohort_id IS NOT NULL),
            '{}'::types.q_get_profile_new_v3_cohort[]
        ) as cohorts
...
SELECT 
    ...
    -- Mappings (now arrays)
    COALESCE(vda.departments, '{}'::types.q_get_profile_new_v3_department[]) as departments,
    COALESCE(aca.cohorts, '{}'::types.q_get_profile_new_v3_cohort[]) as cohorts,
    -- Actor name
    ap.actor_name
