...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_profile_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_profile_v3(%s)', r.sig);
    END LOOP;
END $$;
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION api_create_profile_v3(
    profile_id uuid,
    first_name text,
    last_name text,
