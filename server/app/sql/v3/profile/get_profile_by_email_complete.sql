...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_profile_by_email_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_profile_by_email_v3(%s)', r.sig);
    END LOOP;
END $$;
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION api_get_profile_by_email_v3(
    email text,
