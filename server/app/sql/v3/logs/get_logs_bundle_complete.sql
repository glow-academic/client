...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_logs_bundle_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_logs_bundle_v3(%s)', r.sig);
    END LOOP;
END $$;
...
DROP TYPE IF EXISTS types.q_get_logs_bundle_v3_health_kpis;
DROP TYPE IF EXISTS types.q_get_logs_bundle_v3_health_kpi;
DROP TYPE IF EXISTS types.q_get_logs_bundle_v3_trend_data;
DROP TYPE IF EXISTS types.q_get_logs_bundle_v3_metrics_data_point;

-- 3) Recreate types
CREATE TYPE types.q_get_logs_bundle_v3_trend_data AS (
    date text,
    value float,
    latency float,
    count int
...
CREATE TYPE types.q_get_logs_bundle_v3_health_kpi AS (
    ok boolean,
    latency_ms float,
    error text,
    trend types.q_get_logs_bundle_v3_trend_data[]
);

CREATE TYPE types.q_get_logs_bundle_v3_health_kpis AS (
    websocket types.q_get_logs_bundle_v3_health_kpi,
    redis types.q_get_logs_bundle_v3_health_kpi,
    document types.q_get_logs_bundle_v3_health_kpi,
    database types.q_get_logs_bundle_v3_health_kpi,
    authentication types.q_get_logs_bundle_v3_health_kpi
);

CREATE TYPE types.q_get_logs_bundle_v3_metrics_data_point AS (
    date text,
    cpu_percent float,
    latency_ms float,
    memory_bytes bigint,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_logs_bundle_v3(profile_id uuid DEFAULT NULL)
RETURNS TABLE (
    actor_name text,
    health_kpis types.q_get_logs_bundle_v3_health_kpis,
    metrics types.q_get_logs_bundle_v3_metrics_data_point[]
)
LANGUAGE sql
STABLE
AS $$
...
websocket_kpi AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (wt.date, wt.value, wt.latency, wt.count)::types.q_get_logs_bundle_v3_trend_data
                ORDER BY wt.date
            ) FROM websocket_trend wt),
            '{}'::types.q_get_logs_bundle_v3_trend_data[]
        ) as trend
),
...
redis_kpi AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (rt.date, rt.value, rt.latency, rt.count)::types.q_get_logs_bundle_v3_trend_data
                ORDER BY rt.date
            ) FROM redis_trend rt),
            '{}'::types.q_get_logs_bundle_v3_trend_data[]
        ) as trend
...
document_kpi AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (dt.date, dt.value, dt.latency, dt.count)::types.q_get_logs_bundle_v3_trend_data
                ORDER BY dt.date
...
database_kpi AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (dbt.date, dbt.value, dbt.latency, dbt.count)::types.q_get_logs_bundle_v3_trend_data
                ORDER BY dbt.date
...
authentication_kpi AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (at.date, at.value, at.latency, at.count)::types.q_get_logs_bundle_v3_trend_data
                ORDER BY at.date
...
health_kpis_row AS (
    SELECT 
        (
            (SELECT (ok, latency_ms, error, trend)::types.q_get_logs_bundle_v3_health_kpi FROM websocket_kpi LIMIT 1),
            (SELECT (ok, latency_ms, error, trend)::types.q_get_logs_bundle_v3_health_kpi FROM redis_kpi LIMIT 1),
            (SELECT (ok, latency_ms, error, trend)::types.q_get_logs_bundle_v3_health_kpi FROM document_kpi LIMIT 1),
            (SELECT (ok, latency_ms, error, trend)::types.q_get_logs_bundle_v3_health_kpi FROM database_kpi LIMIT 1),
            (SELECT (ok, latency_ms, error, trend)::types.q_get_logs_bundle_v3_health_kpi FROM authentication_kpi LIMIT 1)
        )::types.q_get_logs_bundle_v3_health_kpis as health_kpis
),
actor_profile AS (
    ...
    COALESCE(
        ARRAY_AGG(
            (
                ...
                mt.max_errors_total,
                mt.sample_count
            )::types.q_get_logs_bundle_v3_metrics_data_point
