...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_model_new_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_model_new_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_model_new_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_model_new_v3_provider AS (
    provider_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_model_new_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_model_new_v3_model AS (
    model_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_model_new_v3_key AS (
    key_id uuid,
    name text,
    description text,
    key_masked text,
...
CREATE TYPE types.q_get_model_new_v3_unit AS (
    unit_id uuid,
    name text,
    unit_category text,
    value int
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_model_new_v3(profile_id uuid)
RETURNS TABLE (
    valid_provider_ids uuid[],
    providers types.q_get_model_new_v3_provider[],
    valid_department_ids uuid[],
    departments types.q_get_model_new_v3_department[],
    valid_model_ids uuid[],
    models types.q_get_model_new_v3_model[],
    valid_key_ids uuid[],
    keys types.q_get_model_new_v3_key[],
    units types.q_get_model_new_v3_unit[],
    user_role text,
...
providers_aggregated AS (
    SELECT 
        ...
        ARRAY_AGG((p.id, p.name, COALESCE(p.description, ''))::types.q_get_model_new_v3_provider ORDER BY p.name) as providers
    FROM providers p
    WHERE p.active = true
),
departments_aggregated AS (
    SELECT 
        ...
        ARRAY_AGG((udd.id, udd.name, COALESCE(udd.description, ''))::types.q_get_model_new_v3_department ORDER BY udd.name) as departments
    FROM user_departments_data udd
),
models_aggregated AS (
    SELECT 
        ARRAY_AGG(vm.model_id ORDER BY vm.name) as valid_model_ids,
        ARRAY_AGG((vm.model_id, vm.name, vm.description)::types.q_get_model_new_v3_model ORDER BY vm.name) as models
...
units_aggregated AS (
    SELECT 
        ARRAY_AGG(
            (aud.unit_id, aud.name, aud.unit_category, aud.value)::types.q_get_model_new_v3_unit
...
SELECT 
    ...
    COALESCE(pa.providers, '{}'::types.q_get_model_new_v3_provider[]) as providers,
    COALESCE(da.valid_department_ids, ARRAY[]::uuid[]) as valid_department_ids,
    COALESCE(da.departments, '{}'::types.q_get_model_new_v3_department[]) as departments,
    COALESCE(ma.valid_model_ids, ARRAY[]::uuid[]) as valid_model_ids,
    COALESCE(ma.models, '{}'::types.q_get_model_new_v3_model[]) as models,
    COALESCE(ka.valid_key_ids, ARRAY[]::uuid[]) as valid_key_ids,
    COALESCE(ka.keys, '{}'::types.q_get_model_new_v3_key[]) as keys,
    COALESCE(ua.units, '{}'::types.q_get_model_new_v3_unit[]) as units,
