...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_model_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_model_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_create_model_v3_%' OR typname LIKE 'q_create_model_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.i_create_model_v3_pricing AS (
    pricing_type text,
    unit_id uuid,
    price float
);

CREATE TYPE types.i_create_model_v3_temperature_bounds AS (
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_create_model_v3(
    provider_id uuid,
    name text,
    description text,
    active boolean,
    ...
    temperature_bounds types.i_create_model_v3_temperature_bounds DEFAULT NULL,
    pricing types.i_create_model_v3_pricing[] DEFAULT ARRAY[]::types.i_create_model_v3_pricing[],
