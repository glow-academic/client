...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_list_models_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_list_models_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_list_models_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_list_models_v3_model AS (
    model_id uuid,
    name text,
    description text,
    active boolean,
...
CREATE TYPE types.q_list_models_v3_provider_option AS (
    value text,
    label text
);

CREATE TYPE types.q_list_models_v3_status_option AS (
    value text,
    label text
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_list_models_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    models types.q_list_models_v3_model[],
    provider_options types.q_list_models_v3_provider_option[],
    status_options types.q_list_models_v3_status_option[]
)
LANGUAGE sql
STABLE
AS $$
...
provider_options_aggregated AS (
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (value, label)::types.q_list_models_v3_provider_option
                ORDER BY label
            ),
            '{}'::types.q_list_models_v3_provider_option[]
        ) as provider_options
...
SELECT 
    ...
    ARRAY[
        ('true', 'Active')::types.q_list_models_v3_status_option,
        ('false', 'Inactive')::types.q_list_models_v3_status_option
    ] as status_options
