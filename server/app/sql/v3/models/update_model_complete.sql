...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_update_model_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_update_model_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_update_model_v3_%' OR typname LIKE 'q_update_model_v3_%'
...
CREATE TYPE types.i_update_model_v3_pricing AS (
    pricing_type text,
    unit_id uuid,
    price float
);

CREATE TYPE types.i_update_model_v3_temperature_bounds AS (
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_update_model_v3(
    model_id uuid,
    provider_id uuid,
    name text,
    description text,
    ...
    temperature_bounds types.i_update_model_v3_temperature_bounds DEFAULT NULL,
    pricing types.i_update_model_v3_pricing[] DEFAULT ARRAY[]::types.i_update_model_v3_pricing[],
...
BEGIN
    ...
    IF NOT validate_department_update_permissions(
        ...
        ARRAY(SELECT department_id::text FROM model_departments WHERE model_id = api_update_model_v3.model_id AND active = true),
        ARRAY(SELECT department_id::text FROM profile_departments WHERE profile_id = api_update_model_v3.profile_id AND active = true)
    ) THEN
    ...
    -- Update model
    UPDATE models SET
        provider_id = api_update_model_v3.provider_id,
        name = api_update_model_v3.name,
        description = api_update_model_v3.description,
        active = api_update_model_v3.active,
        value = api_update_model_v3.value,
        updated_at = NOW()
    WHERE id = api_update_model_v3.model_id
    RETURNING name INTO updated_model_name;

    -- Deactivate all existing department links
    UPDATE model_departments
    SET active = false, updated_at = NOW()
    WHERE model_id = api_update_model_v3.model_id AND active = true;
    ...
    IF array_length(department_ids, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, dept_id, true, NOW(), NOW()
    ...
    IF base_url IS NOT NULL AND TRIM(base_url) != '' THEN
        ...
        VALUES (api_update_model_v3.model_id, base_url, true, NOW(), NOW())
    ...
    ELSE
        ...
        SET active = false, updated_at = NOW()
        WHERE model_id = api_update_model_v3.model_id;
    END IF;
    ...
    IF temperature_bounds IS NOT NULL THEN
        ...
        IF temperature_bounds.bounds_type = 'range' THEN
            ...
            VALUES 
                (api_update_model_v3.model_id, temperature_bounds.lower_bound, false, true),
                (api_update_model_v3.model_id, temperature_bounds.upper_bound, true, true)
        ...
        ELSIF temperature_bounds.bounds_type = 'values' AND array_length(temperature_bounds.values_array, 1) > 0 THEN
            INSERT INTO model_temperature_levels (model_id, temperature, is_upper, active)
            SELECT api_update_model_v3.model_id, temp_val, false, true
    ...
    IF array_length(pricing, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, p.pricing_type::pricing_type, p.unit_id, p.price, true
    ...
    IF array_length(final_input_mods, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, mod::modality_type, true, true
    ...
    IF array_length(final_output_mods, 1) > 0 THEN
        INSERT INTO model_modalities (model_id, modality, is_input, active)
        SELECT api_update_model_v3.model_id, mod::modality_type, false, true
    ...
    IF array_length(reasoning_levels, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, level::reasoning_effort, true
    ...
    IF array_length(final_voices, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, voice::voice, true
    ...
    IF array_length(qualities, 1) > 0 THEN
        ...
        SELECT api_update_model_v3.model_id, quality::quality, true
