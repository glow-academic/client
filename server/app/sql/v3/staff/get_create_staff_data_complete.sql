...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_create_staff_data_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_create_staff_data_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_create_staff_data_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_create_staff_data_v3_staff AS (
    profile_id uuid,
    first_name text,
    last_name text,
    emails text[],
...
CREATE TYPE types.q_get_create_staff_data_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_create_staff_data_v3_department AS (
    department_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_create_staff_data_v3(
    department_ids uuid[],
    profile_id uuid
)
RETURNS TABLE (
    actor_name text,
    staff types.q_get_create_staff_data_v3_staff[],
    cohorts types.q_get_create_staff_data_v3_cohort[],
    departments types.q_get_create_staff_data_v3_department[],
    role_options text[]
)
LANGUAGE sql
STABLE
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (sr.profile_id, sr.first_name, sr.last_name, sr.emails, sr.primary_email, sr.name, sr.role, sr.active, sr.last_active, sr.cohort_ids, sr.department_ids, sr.primary_department_id, sr.requests_per_day, sr.total_requests, sr.requests_in_last_day)::types.q_get_create_staff_data_v3_staff
            ORDER BY sr.last_name, sr.first_name
        )
        FROM staff_rows sr),
        '{}'::types.q_get_create_staff_data_v3_staff[]
    ) as staff,
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.cohort_id, cd.name, cd.description)::types.q_get_create_staff_data_v3_cohort
            ORDER BY cd.name
        )
        FROM cohorts_data cd),
        '{}'::types.q_get_create_staff_data_v3_cohort[]
    ) as cohorts,
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_get_create_staff_data_v3_department
            ORDER BY dd.name
        )
        FROM departments_data dd),
        '{}'::types.q_get_create_staff_data_v3_department[]
    ) as departments,
