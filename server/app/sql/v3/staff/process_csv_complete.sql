...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_process_csv_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_process_csv_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname = 'q_process_csv_v3_processed_row'
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname = 'q_process_csv_v3_csv_row_error'
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE (typname LIKE 'q_process_csv_v3_column_mapping' OR typname LIKE 'i_process_csv_v3_column_mapping')
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
        EXECUTE format('DROP TYPE IF EXISTS types.%I', r.typname);
    END LOOP;
...
CREATE TYPE types.i_process_csv_v3_column_mapping AS (
    csv_column text,
    target_field text  -- NULL if not mapped
);

-- Output type for column mapping (for consistency)
CREATE TYPE types.q_process_csv_v3_column_mapping AS (
    csv_column text,
    target_field text  -- NULL if not mapped
);

CREATE TYPE types.q_process_csv_v3_csv_row_error AS (
    row_index integer,
    field text,
    message text
);

CREATE TYPE types.q_process_csv_v3_processed_row AS (
    row_index integer,
    ...
    department_ids text[],
    cohort_ids text[],
    errors types.q_process_csv_v3_csv_row_error[]
);
...
CREATE OR REPLACE FUNCTION api_process_csv_v3(
    csv_content text,  -- Raw CSV content from frontend
    column_mappings types.i_process_csv_v3_column_mapping[],  -- Column mappings from frontend
    profile_id uuid
)
RETURNS TABLE (
    success boolean,
    headers text[],
    rows types.q_process_csv_v3_processed_row[],
    actor_name text
)
LANGUAGE sql
STABLE
...
WITH params AS (
    SELECT 
        ...
        COALESCE(column_mappings, ARRAY[]::types.i_process_csv_v3_column_mapping[]) AS column_mappings,
...
SELECT 
    ...
    ARRAY[]::types.q_process_csv_v3_processed_row[] as rows,  -- Populated by Python after parsing
