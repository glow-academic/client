...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_upsert_staff_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_upsert_staff_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_upsert_staff_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.i_upsert_staff_v3_profile AS (
    first_name text,
    last_name text,
...
CREATE OR REPLACE FUNCTION api_upsert_staff_v3(
    profiles types.i_upsert_staff_v3_profile[],  -- Array of profiles to upsert
...
WITH params AS (
    SELECT 
        COALESCE(profiles, ARRAY[]::types.i_upsert_staff_v3_profile[]) AS profiles,
...
role_validation AS (
    ...
    SELECT 
        ...
        CASE 
            WHEN cur.role = 'superadmin' AND pe.role IN ('superadmin', 'admin', 'instructional', 'member', 'guest') THEN true
            WHEN cur.role = 'admin' AND pe.role IN ('instructional', 'member', 'guest') THEN true
            WHEN cur.role = 'instructional' AND pe.role IN ('member', 'guest') THEN true
            WHEN cur.role = 'member' AND pe.role = 'guest' THEN true
            ELSE false
        END as can_assign
