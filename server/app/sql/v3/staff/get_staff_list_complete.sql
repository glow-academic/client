...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_list_staff_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_list_staff_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_list_staff_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_list_staff_v3_staff AS (
    profile_id uuid,
    first_name text,
    last_name text,
    emails text[],
...
CREATE TYPE types.q_list_staff_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_staff_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_staff_v3_trend_data AS (
    date date,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_list_staff_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    current_user_role text,
    staff types.q_list_staff_v3_staff[],
    cohorts types.q_list_staff_v3_cohort[],
    departments types.q_list_staff_v3_department[],
    trend_data_active types.q_list_staff_v3_trend_data[],
    trend_data_admin types.q_list_staff_v3_trend_data[],
    trend_data_instructional types.q_list_staff_v3_trend_data[],
    trend_data_member types.q_list_staff_v3_trend_data[],
    trend_data_total_requests types.q_list_staff_v3_trend_data[],
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.cohort_id, cd.name, cd.description)::types.q_list_staff_v3_cohort
            ORDER BY cd.name
        )
        FROM cohorts_data cd),
        '{}'::types.q_list_staff_v3_cohort[]
    ) as cohorts,
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_list_staff_v3_department
            ORDER BY dd.name
        )
        FROM departments_data dd),
        '{}'::types.q_list_staff_v3_department[]
    ) as departments,
    COALESCE(
        (SELECT ARRAY_AGG(
            (active_trend.date, active_trend.count::double precision, 0)::types.q_list_staff_v3_trend_data
        ...
        '{}'::types.q_list_staff_v3_trend_data[]
    ) as trend_data_active,
    COALESCE(
        (SELECT ARRAY_AGG(
            (auc.date, auc.cumulative_count::double precision, auc.daily_count)::types.q_list_staff_v3_trend_data
            ORDER BY auc.date
        )
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (iuc.date, iuc.cumulative_count::double precision, iuc.daily_count)::types.q_list_staff_v3_trend_data
            ORDER BY iuc.date
        )
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (muc.date, muc.cumulative_count::double precision, muc.daily_count)::types.q_list_staff_v3_trend_data
            ORDER BY muc.date
        )
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (trc.date, trc.cumulative_count::double precision, trc.daily_count)::types.q_list_staff_v3_trend_data
            ORDER BY trc.date
        )
