...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_bulk_archive_attempts_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_bulk_archive_attempts_v3(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Recreate function
CREATE OR REPLACE FUNCTION api_bulk_archive_attempts_v3(
    archived boolean,
    profile_id uuid,
...
BEGIN
    ...
    IF v_use_attempt_ids_mode THEN
        ...
        WITH update_attempts AS (
            ...
            SET archived = api_bulk_archive_attempts_v3.archived
            WHERE id = ANY(attempt_ids)
              AND archived != api_bulk_archive_attempts_v3.archived
            RETURNING id
        )
