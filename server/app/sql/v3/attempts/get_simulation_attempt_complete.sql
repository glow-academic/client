...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_simulation_attempt_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_simulation_attempt_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        ...
        FOR r IN 
            SELECT typname 
            FROM pg_type 
            WHERE typname LIKE 'q_get_simulation_attempt_v3_%'
...
-- 3) Recreate types
-- Core types
CREATE TYPE types.q_get_simulation_attempt_v3_attempt AS (
    id uuid,
...
CREATE TYPE types.q_get_simulation_attempt_v3_simulation AS (
    id uuid,
    title text,
    description text,
    department_id uuid,
...
CREATE TYPE types.q_get_simulation_attempt_v3_attempt_profile AS (
    profile_id uuid,
    attempt_id uuid,
    active boolean
);

CREATE TYPE types.q_get_simulation_attempt_v3_timer AS (
    elapsed int,
    "limit" int,
    exceeded boolean,
    formatted text
...
CREATE TYPE types.q_get_simulation_attempt_v3_aggregated_results AS (
    total_score float,
...
CREATE TYPE types.q_get_simulation_attempt_v3_message_feedback_replace AS (
    section text,
    replace text
);

CREATE TYPE types.q_get_simulation_attempt_v3_message_feedback_highlight AS (
    section text
);

CREATE TYPE types.q_get_simulation_attempt_v3_message_feedback AS (
    id uuid,
    name text,
    description text,
    type text,
    replaces types.q_get_simulation_attempt_v3_message_feedback_replace[],
    highlights types.q_get_simulation_attempt_v3_message_feedback_highlight[]
);

CREATE TYPE types.q_get_simulation_attempt_v3_message AS (
    id uuid,
    created_at timestamptz,
    updated_at timestamptz,
    chat_id uuid,
    ...
    completed boolean,
    persona_id uuid,
    feedbacks types.q_get_simulation_attempt_v3_message_feedback[]
);

-- Hint types
CREATE TYPE types.q_get_simulation_attempt_v3_hint AS (
...
CREATE TYPE types.q_get_simulation_attempt_v3_hints_by_message AS (
    message_id uuid,
    hints types.q_get_simulation_attempt_v3_hint[]
);

-- Grade types
CREATE TYPE types.q_get_simulation_attempt_v3_grade AS (
    id uuid,
    created_at timestamptz,
...
CREATE TYPE types.q_get_simulation_attempt_v3_standard_achievement AS (
    standard_id uuid,
    achieved boolean
);

CREATE TYPE types.q_get_simulation_attempt_v3_standard_pass AS (
    standard_id uuid,
    passed boolean
);

CREATE TYPE types.q_get_simulation_attempt_v3_standard_feedback AS (
    standard_id uuid,
    feedback text
);

CREATE TYPE types.q_get_simulation_attempt_v3_grading_state AS (
    achieved_standards types.q_get_simulation_attempt_v3_standard_achievement[],
    passed_standards types.q_get_simulation_attempt_v3_standard_pass[],
    grade_description text,
    feedback_by_standard_id types.q_get_simulation_attempt_v3_standard_feedback[]
);

-- Dynamic rubric types (convert dicts to arrays)
CREATE TYPE types.q_get_simulation_attempt_v3_skill_score AS (
    skill_name text,
    score float
);

CREATE TYPE types.q_get_simulation_attempt_v3_skill_feedback AS (
    skill_name text,
    feedback text
);

CREATE TYPE types.q_get_simulation_attempt_v3_dynamic_rubric AS (
    chat_id uuid,
    score float,
    passed boolean,
    time_taken float,
    skill_scores types.q_get_simulation_attempt_v3_skill_score[],
    skill_feedbacks types.q_get_simulation_attempt_v3_skill_feedback[],
    total_possible_points float
);

-- Previous chat types
CREATE TYPE types.q_get_simulation_attempt_v3_previous_chat AS (
    chat_id uuid,
    attempt_id uuid,
    score float,
    passed boolean,
...
-- Persona types
CREATE TYPE types.q_get_simulation_attempt_v3_persona AS (
    id uuid,
    name text,
    icon text,
    color text
...
-- Scenario types
CREATE TYPE types.q_get_simulation_attempt_v3_scenario AS (
    id uuid,
    name text,
...
-- Video types
CREATE TYPE types.q_get_simulation_attempt_v3_option AS (
    id uuid,
    option_text text,
    type text,
    is_correct boolean
...
CREATE TYPE types.q_get_simulation_attempt_v3_question AS (
    id uuid,
    question_text text,
    type text,
    allow_multiple boolean,
    times int[],
    options types.q_get_simulation_attempt_v3_option[]
);

CREATE TYPE types.q_get_simulation_attempt_v3_video_document AS (
    id uuid,
    name text,
    description text,
    extension text,
...
CREATE TYPE types.q_get_simulation_attempt_v3_video AS (
    id uuid,
    title text,
    length_seconds int,
    upload_id uuid,
    video_documents types.q_get_simulation_attempt_v3_video_document[],
    questions types.q_get_simulation_attempt_v3_question[],
    show_image boolean
);

-- Quiz types
CREATE TYPE types.q_get_simulation_attempt_v3_quiz_response AS (
    question_id uuid,
    option_id uuid,
    completed boolean,
    created_at timestamptz
...
CREATE TYPE types.q_get_simulation_attempt_v3_quiz AS (
    id uuid,
    completed boolean,
    responses types.q_get_simulation_attempt_v3_quiz_response[]
);

-- Chat types
CREATE TYPE types.q_get_simulation_attempt_v3_chat AS (
    id uuid,
    created_at timestamptz,
    updated_at timestamptz,
    title text,
...
CREATE TYPE types.q_get_simulation_attempt_v3_chat_data AS (
    chat types.q_get_simulation_attempt_v3_chat,
    scenario types.q_get_simulation_attempt_v3_scenario,
    messages types.q_get_simulation_attempt_v3_message[],
    hints types.q_get_simulation_attempt_v3_hints_by_message[],
    grade types.q_get_simulation_attempt_v3_grade,
    grading_state types.q_get_simulation_attempt_v3_grading_state,
    dynamic_rubric types.q_get_simulation_attempt_v3_dynamic_rubric,
    previous_chats types.q_get_simulation_attempt_v3_previous_chat[],
    personas types.q_get_simulation_attempt_v3_persona[],
    content_type text,
    video types.q_get_simulation_attempt_v3_video,
    quiz types.q_get_simulation_attempt_v3_quiz
);

-- Document types
CREATE TYPE types.q_get_simulation_attempt_v3_scenario_document AS (
    document_id uuid,
    name text,
    type text,
    updated_at timestamptz,
...
CREATE TYPE types.q_get_simulation_attempt_v3_standard_group_mapping AS (
...
CREATE TYPE types.q_get_simulation_attempt_v3_standard_mapping AS (
    standard_id uuid,
    name text,
    description text,
    points float
...
CREATE TYPE types.q_get_simulation_attempt_v3_standard_group_standards AS (
    standard_group_id uuid,
    standard_ids text[]
);

CREATE TYPE types.q_get_simulation_attempt_v3_rubric_structure AS (
    standard_groups types.q_get_simulation_attempt_v3_standard_group_standards[],
    standard_groups_mapping types.q_get_simulation_attempt_v3_standard_group_mapping[],
    standards_mapping types.q_get_simulation_attempt_v3_standard_mapping[]
);

-- Continuation option types
CREATE TYPE types.q_get_simulation_attempt_v3_continuation_option AS (
    scenario_id uuid,
    position int,
    scenario_name text,
...
CREATE TYPE types.q_get_simulation_attempt_v3_available_continuation_options AS (
    next_sequential_options types.q_get_simulation_attempt_v3_continuation_option[],
    has_options boolean
);

-- All simulation scenario types
CREATE TYPE types.q_get_simulation_attempt_v3_all_simulation_scenario AS (
    id uuid,
    name text,
    problem_statement text,
    department_id uuid,
...
CREATE TYPE types.q_get_simulation_attempt_v3_feedback AS (
    id uuid,
    created_at timestamptz,
    standard_id uuid,
    grade_id uuid,
...
CREATE TYPE types.q_get_simulation_attempt_v3_standard AS (
    id uuid,
    name text,
    points float,
    standard_group_id uuid
...
CREATE TYPE types.q_get_simulation_attempt_v3_role_check AS (
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_simulation_attempt_v3(
    attempt_id uuid,
    profile_id uuid
)
RETURNS TABLE (
    ...
    actor_name text,
    access_denied boolean,
    attempt types.q_get_simulation_attempt_v3_attempt,
    simulation types.q_get_simulation_attempt_v3_simulation,
    attempt_profiles types.q_get_simulation_attempt_v3_attempt_profile[],
    chats types.q_get_simulation_attempt_v3_chat_data[],
    scenario_documents types.q_get_simulation_attempt_v3_scenario_document[],
    aggregated_results types.q_get_simulation_attempt_v3_aggregated_results,
    timer types.q_get_simulation_attempt_v3_timer,
    ...
    is_active boolean,
    rubric_structure types.q_get_simulation_attempt_v3_rubric_structure,
    all_simulation_scenarios types.q_get_simulation_attempt_v3_all_simulation_scenario[],
    available_continuation_options types.q_get_simulation_attempt_v3_available_continuation_options
)
LANGUAGE sql
STABLE
AS $$
...
attempt_profiles_data AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (ap.profile_id, ap.attempt_id, ap.active)::types.q_get_simulation_attempt_v3_attempt_profile
            ORDER BY ap.created_at
        ),
        '{}'::types.q_get_simulation_attempt_v3_attempt_profile[]
    ) as attempt_profiles
    FROM params x
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (q.id, q.question_text, 'choice'::text, q.allow_multiple,
                 ...
                 COALESCE(
                     (SELECT ARRAY_AGG(
                         (o.id, o.option_text, o.type::text, CASE WHEN qa.option_id IS NOT NULL THEN true ELSE false END)::types.q_get_simulation_attempt_v3_option
                         ORDER BY o.id
                     )
                     ...
                     '{}'::types.q_get_simulation_attempt_v3_option[]
                 )
                )::types.q_get_simulation_attempt_v3_question
...
quiz_responses_grouped AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (qr.question_id, qr.option_id, qr.response_completed, qr.response_created_at)::types.q_get_simulation_attempt_v3_quiz_response
                ORDER BY qr.response_created_at
            ),
            '{}'::types.q_get_simulation_attempt_v3_quiz_response[]
        ) as responses
            ...
            ARRAY_AGG(
                (pwg.chat_id, pwg.attempt_id, pwg.score::float, pwg.passed, pwg.created_at, pwg.title, 
                 ...
                 CASE 
                     ...
                     ELSE NULL::float
                 END
                )::types.q_get_simulation_attempt_v3_previous_chat
...
all_simulation_scenarios_with_previous_chats AS (
    SELECT 
        ...
        )::types.q_get_simulation_attempt_v3_scenario as scenario_data,
        COALESCE(pcf.previous_chats, '{}'::types.q_get_simulation_attempt_v3_previous_chat[]) as previous_chats
...
message_feedbacks_data AS (
    SELECT 
        ...
        (mf.id, mf.name, mf.description, mf.type::text,
         COALESCE(
             (SELECT ARRAY_AGG((mfr.section, mfr.replace)::types.q_get_simulation_attempt_v3_message_feedback_replace ORDER BY mfr.idx)
                        FROM message_feedback_replace mfr
                        WHERE mfr.message_feedback_id = mf.id),
             '{}'::types.q_get_simulation_attempt_v3_message_feedback_replace[]
         ),
         COALESCE(
             (SELECT ARRAY_AGG((mfh.section)::types.q_get_simulation_attempt_v3_message_feedback_highlight ORDER BY mfh.idx)
                        FROM message_feedback_highlight mfh
                        WHERE mfh.message_feedback_id = mf.id),
             '{}'::types.q_get_simulation_attempt_v3_message_feedback_highlight[]
                    )
        )::types.q_get_simulation_attempt_v3_message_feedback as feedback_data
...
message_feedbacks_grouped AS (
    SELECT 
        ...
        COALESCE(
            ...
            '{}'::types.q_get_simulation_attempt_v3_message_feedback[]
        ) as feedbacks
        ...
        COALESCE(
            ARRAY_AGG(
                (mwt.id, mwt.created_at, mwt.updated_at, mwt.chat_id, mwt.content, mwt.type, mwt.completed, mwt.persona_id,
                 COALESCE(mfg.feedbacks, '{}'::types.q_get_simulation_attempt_v3_message_feedback[])
                )::types.q_get_simulation_attempt_v3_message
...
personas_per_chat AS (
    SELECT 
        cb.id as chat_id,
        COALESCE(
            ARRAY_AGG((p.id, p.name, p.icon, p.color)::types.q_get_simulation_attempt_v3_persona ORDER BY p.name)
            FILTER (WHERE p.id IS NOT NULL),
            '{}'::types.q_get_simulation_attempt_v3_persona[]
        ) as personas
    FROM chats_base cb
        ...
        COALESCE(
            ARRAY_AGG(
                (m.id,
                 COALESCE(
                     (SELECT ARRAY_AGG(
                         (sh.simulation_message_id, sh.hint, sh.idx, sh.created_at)::types.q_get_simulation_attempt_v3_hint
                         ORDER BY sh.idx
                     )
...
feedbacks_grouped AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (scf.id, scf.created_at, scf.standard_id, scf.grade_id, scf.total::float, scf.feedback)::types.q_get_simulation_attempt_v3_feedback
            ),
            '{}'::types.q_get_simulation_attempt_v3_feedback[]
        ) as feedbacks
    FROM feedbacks scf
...
rubric_standards_grouped AS (
    SELECT 
        ...
        ARRAY_AGG(
            (s.id, s.name, s.points, s.standard_group_id)::types.q_get_simulation_attempt_v3_standard
        ) as standards_list
    FROM standards s
...
standards_mapping_merged AS (
    SELECT 
        ARRAY_AGG(
            (s.id, s.name, COALESCE(s.description, ''), s.points)::types.q_get_simulation_attempt_v3_standard_mapping
    ...
    SELECT 
        CASE 
            WHEN EXISTS (SELECT 1 FROM rubric_standard_groups) THEN
                (COALESCE(
                    (SELECT ARRAY_AGG(
                        (rsg.id, rsgroup.standard_ids)::types.q_get_simulation_attempt_v3_standard_group_standards
                    )
                    FROM rubric_standard_groups rsg
                    LEFT JOIN rubric_standards_grouped rsgroup ON rsgroup.standard_group_id = rsg.id),
                    '{}'::types.q_get_simulation_attempt_v3_standard_group_standards[]
                ),
                COALESCE(
                    (SELECT ARRAY_AGG(
                        (rsg.id, rsg.name, COALESCE(rsg.description, ''), rsg.points, rsg.pass_points)::types.q_get_simulation_attempt_v3_standard_group_mapping
                    )
                    FROM rubric_standard_groups rsg),
                    '{}'::types.q_get_simulation_attempt_v3_standard_group_mapping[]
                ),
                COALESCE(smm.standards_mapping, '{}'::types.q_get_simulation_attempt_v3_standard_mapping[])
                )::types.q_get_simulation_attempt_v3_rubric_structure
            ELSE NULL
...
scenario_documents_data AS (
    SELECT COALESCE(
        ARRAY_AGG(DISTINCT
            ...
            )::types.q_get_simulation_attempt_v3_scenario_document
        ),
        '{}'::types.q_get_simulation_attempt_v3_scenario_document[]
...
skill_scores_per_chat AS (
    ...
    CROSS JOIN LATERAL unnest(COALESCE(fg.feedbacks, '{}'::types.q_get_simulation_attempt_v3_feedback[])) fb
        ...
        CASE 
            WHEN gd.grade IS NOT NULL AND EXISTS (SELECT 1 FROM rubric_standard_groups) THEN
                (gd.chat_id,
                 ...
                 COALESCE(
                     (SELECT ARRAY_AGG(
                         (sspc.group_name, ROUND((sspc.avg_score / sspc.max_points) * 5)::float)::types.q_get_simulation_attempt_v3_skill_score
                     )
                     FROM skill_scores_per_chat sspc
                     WHERE sspc.chat_id = gd.chat_id),
                     '{}'::types.q_get_simulation_attempt_v3_skill_score[]
                 ),
                 COALESCE(
                     (SELECT ARRAY_AGG(
                         (sspc.short_name, sspc.feedbacks_text)::types.q_get_simulation_attempt_v3_skill_feedback
                     )
                     FROM skill_scores_per_chat sspc
                     WHERE sspc.chat_id = gd.chat_id),
                     '{}'::types.q_get_simulation_attempt_v3_skill_feedback[]
                 ),
                 COALESCE(
                ...
                )::types.q_get_simulation_attempt_v3_dynamic_rubric
            ELSE NULL
    ...
    SELECT 
        ...
        CASE 
            WHEN gd.grade IS NOT NULL AND EXISTS (SELECT 1 FROM rubric_standard_groups) THEN
                (                COALESCE(
                    (SELECT ARRAY_AGG(
                        (fb.standard_id, true)::types.q_get_simulation_attempt_v3_standard_achievement
                    )
                    FROM feedbacks_grouped fg2
                    CROSS JOIN LATERAL unnest(COALESCE(fg2.feedbacks, '{}'::types.q_get_simulation_attempt_v3_feedback[])) fb
                    WHERE fg2.grade_id = (gd.grade).id),
                    '{}'::types.q_get_simulation_attempt_v3_standard_achievement[]
                ),
                COALESCE(
                    (SELECT ARRAY_AGG(
                        (fb.standard_id, (fb.total >= mspgc.pass_points)::boolean)::types.q_get_simulation_attempt_v3_standard_pass
                    )
                    FROM feedbacks_grouped fg3
                    CROSS JOIN LATERAL unnest(COALESCE(fg3.feedbacks, '{}'::types.q_get_simulation_attempt_v3_feedback[])) fb
                    JOIN standards s ON s.id = fb.standard_id
                    ...
                    '{}'::types.q_get_simulation_attempt_v3_standard_pass[]
                ),
                COALESCE((gd.grade).description, ''),
                COALESCE(
                    (SELECT ARRAY_AGG(
                        (fb.standard_id, fb.feedback)::types.q_get_simulation_attempt_v3_standard_feedback
                    )
                    FROM feedbacks_grouped fg4
                    CROSS JOIN LATERAL unnest(COALESCE(fg4.feedbacks, '{}'::types.q_get_simulation_attempt_v3_feedback[])) fb
                    WHERE fg4.grade_id = (gd.grade).id),
                    '{}'::types.q_get_simulation_attempt_v3_standard_feedback[]
                )
                )::types.q_get_simulation_attempt_v3_grading_state
            ELSE NULL
    ...
    SELECT 
        ...
        ((cb.id, cb.created_at, cb.updated_at, cb.title, cb.scenario_id,
          ...
          CASE 
              ...
              ELSE NULL 
          END,
          cb.trace_id, cb.document_ids)::types.q_get_simulation_attempt_v3_chat,
         sd.scenario_data,
         COALESCE(mg.messages, '{}'::types.q_get_simulation_attempt_v3_message[]),
         COALESCE(hd.hints, '{}'::types.q_get_simulation_attempt_v3_hints_by_message[]),
         gd.grade,
         COALESCE(gspc.grading_state, NULL::types.q_get_simulation_attempt_v3_grading_state),
         drpc.dynamic_rubric,
         COALESCE(
         ...
         COALESCE(ppc.personas, '{}'::types.q_get_simulation_attempt_v3_persona[]),
         'scenario'::text,
         NULL::types.q_get_simulation_attempt_v3_video,
         NULL::types.q_get_simulation_attempt_v3_quiz
        )::types.q_get_simulation_attempt_v3_chat_data as chat_data,
        cb.completed,
        cb.created_at,
        gd.grade
    FROM chats_base cb
        ...
        CASE 
            WHEN COUNT(*) FILTER (WHERE completed = true AND grade IS NOT NULL) > 0 THEN
                (COALESCE(SUM((grade).score::numeric) FILTER (WHERE completed = true AND grade IS NOT NULL), 0)::float,
                 ...
                 COUNT(*)::int
                )::types.q_get_simulation_attempt_v3_aggregated_results
            ELSE NULL
    ...
    SELECT 
        (etc.total_elapsed::int,
         ...
         CASE 
             ...
             ELSE ''
         END
        )::types.q_get_simulation_attempt_v3_timer as timer
    FROM attempt_base ab
...
available_continuation_options AS (
    ...
    valid_next_scenarios AS (
        ...
        WHERE ssl.position >= csp.current_position
          AND ab.sim_practice_simulation = false
          AND array_length(COALESCE(asswpc.previous_chats, '{}'::types.q_get_simulation_attempt_v3_previous_chat[]), 1) > 0
        ORDER BY ssl.position
    ),
    scenario_options_expanded AS (
        ...
        CROSS JOIN LATERAL unnest(COALESCE(vns.previous_chats, '{}'::types.q_get_simulation_attempt_v3_previous_chat[])) pc
    )
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (soe.scenario_id, soe.position, soe.scenario_name, soe.previous_chat_id, soe.title, soe.score, soe.percentage, soe.time_taken)::types.q_get_simulation_attempt_v3_continuation_option
                ORDER BY soe.position, soe.score DESC NULLS LAST
            ),
            '{}'::types.q_get_simulation_attempt_v3_continuation_option[]
...
SELECT 
    ...
    (ab.id, ab.created_at, ab.simulation_id, ab.infinite_mode, ab.archived, cap.profile_id)::types.q_get_simulation_attempt_v3_attempt as attempt,
    (ab.sim_id, ab.sim_title, ab.sim_description, ab.sim_department_id, ab.sim_active, false, ab.sim_practice_simulation, 
     ...
     ab.sim_time_limit, ab.sim_rubric_id, ab.sim_created_at, ab.sim_updated_at)::types.q_get_simulation_attempt_v3_simulation as simulation,
    COALESCE(apd.attempt_profiles, '{}'::types.q_get_simulation_attempt_v3_attempt_profile[]) as attempt_profiles,
    COALESCE(
        (SELECT ARRAY_AGG(uc.chat_data ORDER BY uc.position, uc.created_at)
         FROM unified_content uc),
        '{}'::types.q_get_simulation_attempt_v3_chat_data[]
    ) as chats,
    COALESCE(sdd.scenario_documents, '{}'::types.q_get_simulation_attempt_v3_scenario_document[]) as scenario_documents,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            ((asswpc.scenario_data).id, (asswpc.scenario_data).name, (asswpc.scenario_data).problem_statement,
             ...
             (asswpc.scenario_data).objectives, asswpc.previous_chats)::types.q_get_simulation_attempt_v3_all_simulation_scenario
            ORDER BY asswpc.position
        )
        FROM all_simulation_scenarios_with_previous_chats asswpc),
        '{}'::types.q_get_simulation_attempt_v3_all_simulation_scenario[]
    ) as all_simulation_scenarios,
    COALESCE(
        (SELECT (aco.next_sequential_options, aco.has_options)::types.q_get_simulation_attempt_v3_available_continuation_options
         FROM available_continuation_options aco),
        (ARRAY[]::types.q_get_simulation_attempt_v3_continuation_option[], false)::types.q_get_simulation_attempt_v3_available_continuation_options
