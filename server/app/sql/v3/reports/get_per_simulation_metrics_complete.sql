...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_per_simulation_metrics_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_per_simulation_metrics_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_per_simulation_metrics_v3_%'
...
CREATE TYPE types.q_per_simulation_metrics_v3_metric AS (
    profile_id uuid,
    simulation_id uuid,
    average_score float,
    highest_score float,
...
-- 4) Create function
CREATE OR REPLACE FUNCTION api_get_per_simulation_metrics_v3(
    start_date text,
    end_date text,
    profile_id uuid,
...
RETURNS TABLE (
    metrics types.q_per_simulation_metrics_v3_metric[]
)
LANGUAGE sql
STABLE
AS $$
...
SELECT 
    COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_per_simulation_metrics_v3_metric
            ORDER BY sm.profile_id, sm.simulation_id
        ),
        ARRAY[]::types.q_per_simulation_metrics_v3_metric[]
    ) AS metrics
