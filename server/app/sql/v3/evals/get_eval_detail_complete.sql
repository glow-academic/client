...
departments_array AS (
    SELECT COALESCE(
        ARRAY_AGG((vd.id, vd.name, vd.description)::types.q_get_eval_detail_v3_department),
...
eval_agents_array AS (
    SELECT COALESCE(
        ARRAY_AGG((vea.id, vea.name, vea.description, vea.roles)::types.q_get_eval_detail_v3_agent),
        '{}'::types.q_get_eval_detail_v3_agent[]
    ) as eval_agents,
...
agents_array AS (
    SELECT COALESCE(
        ARRAY_AGG((vae.id, vae.name, vae.description, vae.roles)::types.q_get_eval_detail_v3_agent),
        '{}'::types.q_get_eval_detail_v3_agent[]
    ) as agents,
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(
                ...
                )::types.q_get_eval_detail_v3_available_model_run
            ),
            '{}'::types.q_get_eval_detail_v3_available_model_run[]
