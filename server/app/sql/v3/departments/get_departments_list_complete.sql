...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_list_departments_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_list_departments_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_list_departments_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_list_departments_v3_department AS (
    department_id uuid,
    title text,
    description text,
    active boolean,
...
CREATE TYPE types.q_list_departments_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_departments_v3_profile AS (
    profile_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_list_departments_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    departments types.q_list_departments_v3_department[],
    cohorts types.q_list_departments_v3_cohort[],
    profiles types.q_list_departments_v3_profile[]
)
LANGUAGE sql
STABLE
AS $$
        ...
        COALESCE(
            ARRAY_AGG(
                (dd.id, dd.title, dd.description, dd.active, dd.updated_at,
                 ...
                 dd.can_edit, dd.can_delete, dd.can_duplicate)::types.q_list_departments_v3_department
                ORDER BY dd.title
            ),
            '{}'::types.q_list_departments_v3_department[]
        ) as departments
...
cohorts_agg AS (
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (cd.cohort_id, cd.name, cd.description)::types.q_list_departments_v3_cohort
                ORDER BY cd.name
            ),
            '{}'::types.q_list_departments_v3_cohort[]
        ) as cohorts
    FROM (
...
profiles_agg AS (
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (pd.profile_id, pd.name, pd.description)::types.q_list_departments_v3_profile
                ORDER BY pd.name
            ),
            '{}'::types.q_list_departments_v3_profile[]
        ) as profiles
    FROM (
