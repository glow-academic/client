...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_duplicate_scenario_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_duplicate_scenario_v3(%s)', r.sig);
    END LOOP;
END $$;
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION api_duplicate_scenario_v3(
    scenario_id uuid,
    profile_id uuid
)
RETURNS TABLE (
