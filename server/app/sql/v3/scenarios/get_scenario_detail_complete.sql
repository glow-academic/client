...
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_field;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_department;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_agent;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_simulation;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_objective;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_problem_statement;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_scenario_image;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_scenario_video;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_question;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_question_option;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_objective_with_departments;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_document_detail;
DROP TYPE IF EXISTS types.q_get_scenario_detail_v3_parameter_detail;
...
CREATE TYPE types.q_get_scenario_detail_v3_document AS (
    document_id uuid,
    name text,
    description text,
    file_path text,
...
CREATE TYPE types.q_get_scenario_detail_v3_parameter AS (
    parameter_id uuid,
    name text,
    description text,
...
CREATE TYPE types.q_get_scenario_detail_v3_field AS (
    field_id uuid,
    name text,
    description text,
    parameter_id uuid,
...
CREATE TYPE types.q_get_scenario_detail_v3_department AS (
    department_id uuid,
    name text,
    description text,
    persona_ids uuid[],
...
CREATE TYPE types.q_get_scenario_detail_v3_agent AS (
    agent_id uuid,
    name text,
    description text,
    roles text[]
...
CREATE TYPE types.q_get_scenario_detail_v3_simulation AS (
    simulation_id uuid,
    name text,
    description text,
    time_limit bigint,
...
CREATE TYPE types.q_get_scenario_detail_v3_objective AS (
    objective_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_scenario_detail_v3_problem_statement AS (
...
CREATE TYPE types.q_get_scenario_detail_v3_scenario_image AS (
    upload_id uuid,
    name text,
    file_path text,
    mime_type text,
...
CREATE TYPE types.q_get_scenario_detail_v3_scenario_video AS (
    id uuid,
    name text,
...
CREATE TYPE types.q_get_scenario_detail_v3_question_option AS (
    id uuid,
    option_text text,
    type text,
    is_correct boolean
...
CREATE TYPE types.q_get_scenario_detail_v3_question AS (
    id uuid,
    question_text text,
    allow_multiple boolean,
    active boolean,
    options types.q_get_scenario_detail_v3_question_option[],
    times integer[]
);

CREATE TYPE types.q_get_scenario_detail_v3_objective_with_departments AS (
    objective text,
    department_ids uuid[]
);

CREATE TYPE types.q_get_scenario_detail_v3_document_detail AS (
    document_id uuid,
    name text,
    updated_at timestamptz,
    extension text,
...
CREATE TYPE types.q_get_scenario_detail_v3_parameter_detail AS (
    parameter_id uuid,
    field_ids uuid[],
    valid_field_ids uuid[]
);

CREATE TYPE types.q_get_scenario_detail_v3_field_range AS (
    parameter_id uuid,
    min_count integer,
    max_count integer
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_scenario_detail_v3(
    scenario_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    parameters types.q_get_scenario_detail_v3_parameter[],
    fields types.q_get_scenario_detail_v3_field[],
    departments types.q_get_scenario_detail_v3_department[],
    agents types.q_get_scenario_detail_v3_agent[],
    simulations types.q_get_scenario_detail_v3_simulation[],
    objectives types.q_get_scenario_detail_v3_objective[],
    problem_statements types.q_get_scenario_detail_v3_problem_statement[],
    scenario_images types.q_get_scenario_detail_v3_scenario_image[],
    scenario_videos types.q_get_scenario_detail_v3_scenario_video[],
    questions types.q_get_scenario_detail_v3_question[],
    objectives_history types.q_get_scenario_detail_v3_objective_with_departments[],
    document_details types.q_get_scenario_detail_v3_document_detail[],
    parameters_detail types.q_get_scenario_detail_v3_parameter_detail[]
)
LANGUAGE sql
STABLE
AS $$
...
scenario_field_ranges_data AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG((sfr.parameter_id, sfr.min_count, sfr.max_count)::types.q_get_scenario_detail_v3_field_range ORDER BY sfr.parameter_id),
            ARRAY[]::types.q_get_scenario_detail_v3_field_range[]
        ) as field_ranges
    FROM scenario_core sc
...
SELECT 
    ...
    COALESCE((
        SELECT ARRAY_AGG(ROW(vpa.persona_id, vpa.name, vpa.description, vpa.color, vpa.icon, vpa.image_model, vpa.parameter_ids, vpa.field_ids, vpa.example)::types.q_get_scenario_detail_v3_persona ORDER BY vpa.name)
        FROM valid_personas_array vpa
    ), '{}'::types.q_get_scenario_detail_v3_persona[]) as personas,
    COALESCE((
        SELECT ARRAY_AGG((ada.document_id, ada.name, ada.description, ada.file_path, ada.mime_type, ada.parameter_ids, ada.field_ids, ada.parent_document_id)::types.q_get_scenario_detail_v3_document ORDER BY ada.name)
        FROM all_documents_array ada
    ), '{}'::types.q_get_scenario_detail_v3_document[]) as documents,
    COALESCE((
        SELECT ARRAY_AGG(ROW(apa.id, apa.name, apa.description, apa.document_parameter, apa.persona_parameter, apa.scenario_parameter, apa.video_parameter)::types.q_get_scenario_detail_v3_parameter ORDER BY apa.name)
        FROM all_parameters_array apa
    ), '{}'::types.q_get_scenario_detail_v3_parameter[]) as parameters,
    COALESCE((
        SELECT ARRAY_AGG((afa.field_id, afa.name, afa.description, afa.parameter_id, afa.parameter_name, afa.conditional_parameter_ids)::types.q_get_scenario_detail_v3_field ORDER BY afa.parameter_name, afa.name)
        FROM all_fields_array afa
    ), '{}'::types.q_get_scenario_detail_v3_field[]) as fields,
    COALESCE((
        SELECT ARRAY_AGG((ada2.department_id, ada2.name, ada2.description, ada2.persona_ids, ada2.document_ids, ada2.parameter_ids, ada2.field_ids)::types.q_get_scenario_detail_v3_department ORDER BY ada2.name)
        FROM all_departments_array ada2
    ), '{}'::types.q_get_scenario_detail_v3_department[]) as departments,
    COALESCE((
        SELECT ARRAY_AGG((vaa.agent_id, vaa.name, vaa.description, vaa.roles)::types.q_get_scenario_detail_v3_agent ORDER BY vaa.name)
        FROM valid_agents_array vaa
    ), '{}'::types.q_get_scenario_detail_v3_agent[]) as agents,
    COALESCE((
        SELECT ARRAY_AGG((sda.simulation_id, sda.name, sda.description, sda.time_limit, sda.department_ids)::types.q_get_scenario_detail_v3_simulation ORDER BY sda.name)
        FROM simulation_data sda
    ), '{}'::types.q_get_scenario_detail_v3_simulation[]) as simulations,
    COALESCE((
        SELECT ARRAY_AGG((soa.objective_id, soa.name, soa.description)::types.q_get_scenario_detail_v3_objective ORDER BY soa.sort_order, soa.idx, soa.created_at DESC)
        FROM scenario_objectives_array soa
    ), '{}'::types.q_get_scenario_detail_v3_objective[]) as objectives,
    COALESCE((
        SELECT ARRAY_AGG((psa.problem_statement_id, psa.name, psa.problem_statement, psa.created_at, psa.updated_at)::types.q_get_scenario_detail_v3_problem_statement ORDER BY psa.sort_order, psa.problem_statement_id)
        FROM problem_statements_array psa
    ), '{}'::types.q_get_scenario_detail_v3_problem_statement[]) as problem_statements,
    COALESCE((
        SELECT ARRAY_AGG((sia.upload_id, sia.name, sia.file_path, sia.mime_type, sia.active, sia.created_at, sia.updated_at)::types.q_get_scenario_detail_v3_scenario_image ORDER BY sia.sort_order, sia.created_at DESC)
        FROM scenario_images_array sia
    ), '{}'::types.q_get_scenario_detail_v3_scenario_image[]) as scenario_images,
    COALESCE((
        SELECT ARRAY_AGG((sva.id, sva.name, sva.length_seconds, sva.completed, sva.active, sva.image_enabled, sva.file_path, sva.mime_type, sva.upload_id)::types.q_get_scenario_detail_v3_scenario_video ORDER BY sva.sort_order, sva.created_at DESC)
        FROM scenario_videos_array sva
    ), '{}'::types.q_get_scenario_detail_v3_scenario_video[]) as scenario_videos,
    COALESCE((
        SELECT ARRAY_AGG(
            ROW(
                sqa.id,
                ...
                sqa.active,
                COALESCE((
                    SELECT ARRAY_AGG(ROW(qoa.id, qoa.option_text, qoa.type, qoa.is_correct)::types.q_get_scenario_detail_v3_question_option ORDER BY qoa.id)
                    FROM question_options_array qoa
                    WHERE qoa.question_id = sqa.id
                ), '{}'::types.q_get_scenario_detail_v3_question_option[]),
                COALESCE((
                    SELECT ARRAY_AGG(qta.time ORDER BY qta.time)
                    FROM question_times_array qta
                    WHERE qta.question_id = sqa.id
                ), ARRAY[]::integer[])
            )::types.q_get_scenario_detail_v3_question
            ORDER BY sqa.sort_order, sqa.created_at DESC
        )
        FROM scenario_questions_array sqa
    ), '{}'::types.q_get_scenario_detail_v3_question[]) as questions,
    COALESCE((
        SELECT ARRAY_AGG((owda.objective, owda.department_ids)::types.q_get_scenario_detail_v3_objective_with_departments ORDER BY owda.objective)
        FROM objectives_with_departments_array owda
    ), '{}'::types.q_get_scenario_detail_v3_objective_with_departments[]) as objectives_history,
    COALESCE((
        SELECT ARRAY_AGG((dda.document_id, dda.name, dda.updated_at, dda.extension, dda.scenario_ids, dda.can_edit, dda.can_delete, dda.active, dda.department_ids, dda.file_path, dda.mime_type, dda.upload_id, dda.field_ids, dda.is_template, dda.parent_document_id)::types.q_get_scenario_detail_v3_document_detail ORDER BY dda.name)
        FROM document_details_array dda
    ), '{}'::types.q_get_scenario_detail_v3_document_detail[]) as document_details,
    COALESCE((
        SELECT ARRAY_AGG((apd.param_id, apd.selected_items, apd.valid_items)::types.q_get_scenario_detail_v3_parameter_detail ORDER BY apd.param_id)
        FROM all_parameters_data apd
    ), '{}'::types.q_get_scenario_detail_v3_parameter_detail[]) as parameters_detail
FROM scenario_core sc
CROSS JOIN user_profile up
