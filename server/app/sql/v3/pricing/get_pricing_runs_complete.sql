...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_pricing_runs_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_pricing_runs_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_pricing_runs_v3_group_run'
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_pricing_runs_v3_run_summary'
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
        EXECUTE format('DROP TYPE IF EXISTS types.%I', r.typname);
    END LOOP;
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_pricing_runs_v3_%'
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
        EXECUTE format('DROP TYPE IF EXISTS types.%I', r.typname);
    END LOOP;
...
-- 3) Recreate types
CREATE TYPE types.q_get_pricing_runs_v3_debug_info AS (
    id uuid,
    created_at timestamptz,
    content text
);

CREATE TYPE types.q_get_pricing_runs_v3_run_summary AS (
    run_id uuid,
    created_at timestamptz,
    input_tokens int,
    output_tokens int,
    ...
    agent_id uuid,
    persona_id uuid,
    debug_info types.q_get_pricing_runs_v3_debug_info[]
);

CREATE TYPE types.q_get_pricing_runs_v3_group_run AS (
    group_id uuid,
    created_at timestamptz,
    run_count int,
    ...
    total_cost numeric,
    runs types.q_get_pricing_runs_v3_run_summary[]
);

CREATE TYPE types.q_get_pricing_runs_v3_filter_option AS (
    value text,
    label text,
    count int
);

CREATE TYPE types.q_get_pricing_runs_v3_model AS (
    model_id uuid,
    name text,
    description text,
    input_ppm numeric,
...
CREATE TYPE types.q_get_pricing_runs_v3_profile AS (
    profile_id uuid,
    name text
);

CREATE TYPE types.q_get_pricing_runs_v3_agent AS (
    agent_id uuid,
    name text
);

CREATE TYPE types.q_get_pricing_runs_v3_persona AS (
    persona_id uuid,
    name text
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_pricing_runs_v3(
    start_date text,
    end_date text,
    department_ids uuid[],
    profile_id uuid,
...
RETURNS TABLE (
    actor_name text,
    group_runs types.q_get_pricing_runs_v3_group_run[],
    total_count bigint,
    page int,
    page_size int,
    total_pages int,
    model_options types.q_get_pricing_runs_v3_filter_option[],
    profile_options types.q_get_pricing_runs_v3_filter_option[],
    actor_options types.q_get_pricing_runs_v3_filter_option[],
    models types.q_get_pricing_runs_v3_model[],
    profiles types.q_get_pricing_runs_v3_profile[],
    agents types.q_get_pricing_runs_v3_agent[],
    personas types.q_get_pricing_runs_v3_persona[]
)
LANGUAGE sql
STABLE
AS $$
...
runs_with_debug AS (
    SELECT
        ...
        COALESCE(
            ARRAY_AGG(
                (di.id, di.created_at, di.content)::types.q_get_pricing_runs_v3_debug_info
                ORDER BY di.created_at
            ) FILTER (WHERE di.id IS NOT NULL),
            '{}'::types.q_get_pricing_runs_v3_debug_info[]
        ) as debug_info
    FROM runs_base mrb
...
groups_with_runs AS (
    SELECT
        ...
        ARRAY_AGG(
            (mrf.run_id, mrf.created_at, mrf.input_tokens, mrf.output_tokens, mrf.run_cost, mrf.model_id, mrf.profile_id, mrf.agent_id, mrf.persona_id, mrf.debug_info)::types.q_get_pricing_runs_v3_run_summary
...
model_pricing_aggregated AS (
    ...
    SELECT 
        mrf.model_id,
        COALESCE(SUM(CASE WHEN mp.pricing_type = pricing_type.input THEN mp.price * (1000000.0 / u.value) ELSE 0 END), 0.0) as input_ppm,
        COALESCE(SUM(CASE WHEN mp.pricing_type = pricing_type.output THEN mp.price * (1000000.0 / u.value) ELSE 0 END), 0.0) as output_ppm
    FROM (SELECT DISTINCT model_id FROM runs_filtered WHERE model_id IS NOT NULL) mrf
    LEFT JOIN model_pricing mp ON mp.model_id = mrf.model_id AND mp.active = true AND mp.pricing_type IN (pricing_type.input, pricing_type.output)
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (pg.group_id, pg.created_at, pg.run_count, pg.total_input_tokens, pg.total_output_tokens, pg.total_cost, pg.runs)::types.q_get_pricing_runs_v3_group_run
        ) FROM paginated_groups pg),
        '{}'::types.q_get_pricing_runs_v3_group_run[]
    ) as group_runs,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (moc.model_id::text, moc.model_name, moc.count)::types.q_get_pricing_runs_v3_filter_option
        ) FROM model_options_cte moc),
        '{}'::types.q_get_pricing_runs_v3_filter_option[]
    ) as model_options,
    COALESCE(
        (SELECT ARRAY_AGG(
            (poc.profile_id::text, poc.profile_name, poc.count)::types.q_get_pricing_runs_v3_filter_option
        ) FROM profile_options_cte poc),
        '{}'::types.q_get_pricing_runs_v3_filter_option[]
    ) as profile_options,
    COALESCE(
        (SELECT ARRAY_AGG(
            (aoc.actor_id::text, aoc.actor_name, aoc.count)::types.q_get_pricing_runs_v3_filter_option
        ) FROM actor_options_cte aoc),
        '{}'::types.q_get_pricing_runs_v3_filter_option[]
    ) as actor_options,
    COALESCE(
        (SELECT ARRAY_AGG(
            DISTINCT (m.id, m.name, COALESCE(m.description, ''), COALESCE(mpa.input_ppm, 0.0), COALESCE(mpa.output_ppm, 0.0))::types.q_get_pricing_runs_v3_model
        ) 
        FROM (SELECT DISTINCT model_id FROM runs_filtered WHERE model_id IS NOT NULL) mrf
        JOIN models m ON m.id = mrf.model_id
        LEFT JOIN model_pricing_aggregated mpa ON mpa.model_id = m.id),
        '{}'::types.q_get_pricing_runs_v3_model[]
    ) as models,
    COALESCE(
        (SELECT ARRAY_AGG(
            DISTINCT (p.id, p.first_name || ' ' || p.last_name)::types.q_get_pricing_runs_v3_profile
        )
        ...
        '{}'::types.q_get_pricing_runs_v3_profile[]
    ) as profiles,
    COALESCE(
        (SELECT ARRAY_AGG(
            DISTINCT (a.id, a.name)::types.q_get_pricing_runs_v3_agent
        )
        FROM (SELECT DISTINCT agent_id FROM runs_filtered WHERE agent_id IS NOT NULL) agt
        JOIN agents a ON a.id = agt.agent_id),
        '{}'::types.q_get_pricing_runs_v3_agent[]
    ) as agents,
    COALESCE(
        (SELECT ARRAY_AGG(
            DISTINCT (per.id, per.name)::types.q_get_pricing_runs_v3_persona
        )
        ...
        '{}'::types.q_get_pricing_runs_v3_persona[]
    ) as personas
FROM params
LIMIT 1
$$;
