...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_scenario_run_context_and_create_run_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_scenario_run_context_and_create_run_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_get_scenario_run_context_and_create_run_v3_%'
...
CREATE TYPE types.i_get_scenario_run_context_and_create_run_v3_document AS (
    id text,
    name text,
    file_path text,
    mime_type text,
...
CREATE TYPE types.i_get_scenario_run_context_and_create_run_v3_document_template AS (
    document_id text,
    document_name text,
...
CREATE TYPE types.i_get_scenario_run_context_and_create_run_v3_parameter_item AS (
    item_name text,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION socket_get_scenario_run_context_and_create_run_v3(
    department_id uuid,
    profile_id uuid,
    agent_id uuid,
...
RETURNS TABLE (
    ...
    documents types.i_get_scenario_run_context_and_create_run_v3_document[],
    document_templates types.i_get_scenario_run_context_and_create_run_v3_document_template[],
    parameter_items types.i_get_scenario_run_context_and_create_run_v3_parameter_item[],
    profile_id text,
    req_per_day integer,
...
context_data AS (
    ...
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (d.id::text, d.name, COALESCE(u.file_path, template_u.file_path), COALESCE(u.mime_type, template_u.mime_type), d.template, t.args)::types.i_get_scenario_run_context_and_create_run_v3_document
                ORDER BY array_position(p.document_ids, d.id)
            )::types.i_get_scenario_run_context_and_create_run_v3_document[]
            FROM documents d
            ...
            ARRAY[]::types.i_get_scenario_run_context_and_create_run_v3_document[]
        ) as documents,
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (d.id::text, d.name, COALESCE(d.description, ''), d.classify_agent_id::text, d.document_agent_id::text, t.args, t.upload_id::text, u.file_path)::types.i_get_scenario_run_context_and_create_run_v3_document_template
                ORDER BY array_position(p.document_ids, d.id)
            )::types.i_get_scenario_run_context_and_create_run_v3_document_template[]
            FROM documents d
            ...
            ARRAY[]::types.i_get_scenario_run_context_and_create_run_v3_document_template[]
        ) as document_templates,
        
        -- Parameter items data (aggregated as composite type array with parameter info)
        COALESCE(
            (SELECT ARRAY_AGG(
                (f.name, f.description, pa.name, pa.description)::types.i_get_scenario_run_context_and_create_run_v3_parameter_item
                ORDER BY array_position(p.parameter_item_ids, f.id)
            )::types.i_get_scenario_run_context_and_create_run_v3_parameter_item[]
            FROM fields f
            ...
            ARRAY[]::types.i_get_scenario_run_context_and_create_run_v3_parameter_item[]
        ) as parameter_items,
