...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_scenario_new_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_scenario_new_v3(%s)', r.sig);
    END LOOP;
END $$;
...
-- 3) Recreate types
CREATE TYPE types.q_get_scenario_new_v3_field_param_filter AS (
    parameter_id uuid,
...
CREATE OR REPLACE FUNCTION api_get_scenario_new_v3(
    ...
    field_show_selected_by_param types.q_get_scenario_new_v3_field_param_filter[] DEFAULT ARRAY[]::types.q_get_scenario_new_v3_field_param_filter[]
)
RETURNS TABLE (
    actor_name text,
    user_role text,
...
WITH params AS (
    SELECT 
        ...
        COALESCE(field_show_selected_by_param, ARRAY[]::types.q_get_scenario_new_v3_field_param_filter[]) AS field_show_selected_by_param
),
params_single AS (
