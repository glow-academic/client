...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_randomize_scenario_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_randomize_scenario_v3(%s)', r.sig);
    END LOOP;
END $$;
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION api_randomize_scenario_v3(
    scenario_id uuid,
    profile_id uuid,
    randomize_type text,
    department_ids uuid[],
...
BEGIN
    ...
    WITH user_departments AS (
        ...
        WHERE pd.profile_id = api_randomize_scenario_v3.profile_id 
          AND pd.active = true 
          AND d.active = true
    ),
    ...
    filtered_personas AS (
        ...
        AND (
            ...
            (COALESCE(array_length(api_randomize_scenario_v3.department_ids, 1), 0) = 0 
             OR EXISTS (
                 SELECT 1 FROM user_departments ud 
                 WHERE ud.id = ANY(api_randomize_scenario_v3.department_ids)
            ...
            OR (COALESCE(array_length(api_randomize_scenario_v3.department_ids, 1), 0) = 0
        ...
        AND (COALESCE(array_length(api_randomize_scenario_v3.persona_ids, 1), 0) = 0
             OR p.id = ANY(api_randomize_scenario_v3.persona_ids))
    ),
    ...
    filtered_documents AS (
        ...
        AND (COALESCE(array_length(api_randomize_scenario_v3.document_ids, 1), 0) = 0
             OR d.id = ANY(api_randomize_scenario_v3.document_ids))
    ),
    ...
    filtered_parameters AS (
        ...
        AND (COALESCE(array_length(api_randomize_scenario_v3.parameter_ids, 1), 0) = 0
             OR p.id = ANY(api_randomize_scenario_v3.parameter_ids))
    ),
    ...
    filtered_fields AS (
        ...
        AND (COALESCE(array_length(api_randomize_scenario_v3.field_ids, 1), 0) = 0
             OR f.id = ANY(api_randomize_scenario_v3.field_ids))
        AND (COALESCE(array_length(api_randomize_scenario_v3.parameter_ids, 1), 0) = 0
             OR pf.parameter_id = ANY(api_randomize_scenario_v3.parameter_ids))
    )
    SELECT 
    ...
    SELECT 
        COALESCE((SELECT min_count FROM scenario_persona_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 1),
        COALESCE((SELECT max_count FROM scenario_persona_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 3),
        COALESCE((SELECT min_count FROM scenario_document_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 0),
        COALESCE((SELECT max_count FROM scenario_document_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 3),
        COALESCE((SELECT min_count FROM scenario_parameter_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 0),
        COALESCE((SELECT max_count FROM scenario_parameter_ranges WHERE scenario_id = api_randomize_scenario_v3.scenario_id), 3)
    INTO 
        allowed_persona_min,
        allowed_persona_max,
    ...
    final_persona_min := COALESCE(api_randomize_scenario_v3.persona_min, allowed_persona_min);
    final_persona_max := COALESCE(api_randomize_scenario_v3.persona_max, allowed_persona_min);
    ...
    final_document_min := COALESCE(api_randomize_scenario_v3.document_min, allowed_document_min);
    final_document_max := COALESCE(api_randomize_scenario_v3.document_max, allowed_document_min);
    ...
    final_parameter_min := COALESCE(api_randomize_scenario_v3.parameter_selection_min, allowed_parameter_min);
    final_parameter_max := COALESCE(api_randomize_scenario_v3.parameter_selection_max, allowed_parameter_max);
