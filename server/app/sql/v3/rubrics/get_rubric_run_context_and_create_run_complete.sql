...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_rubric_run_context_and_create_run_v3_%'
...
-- 3) Recreate function (no composite types needed for this query - returns simple columns)
...
AS $$
WITH params AS (
    SELECT department_id AS department_id, profile_id AS profile_id, rubric_agent_id AS rubric_agent_id
),
best_agent AS (
