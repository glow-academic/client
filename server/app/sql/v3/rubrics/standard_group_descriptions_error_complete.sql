...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_standard_group_descriptions_error_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_standard_group_descriptions_error_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_standard_group_descriptions_error_v3_%'
...
CREATE OR REPLACE FUNCTION socket_standard_group_descriptions_error_v3(
    success boolean,
    message text,
    profile_id uuid,
