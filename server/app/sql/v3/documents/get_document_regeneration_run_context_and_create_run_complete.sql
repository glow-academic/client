-- Get all data needed to run document regeneration agent AND create run in single atomic transaction
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_document_regeneration_run_context_and_create_run_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_document_regeneration_run_context_and_create_run_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_document_regen_run_context_create_run_v3_%'
...
CREATE TYPE types.i_document_regen_run_context_create_run_v3_msg AS (
    role text,
    content text
);
...
CREATE OR REPLACE FUNCTION socket_get_document_regeneration_run_context_and_create_run_v3(
    department_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    group_id uuid,
    trace_id text,
    previous_messages types.i_document_regen_run_context_create_run_v3_msg[]  -- All messages from all previous runs in group
)
LANGUAGE sql
VOLATILE
AS $$
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.i_document_regen_run_context_create_run_v3_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.i_document_regen_run_context_create_run_v3_msg[]
    ) as previous_messages
    FROM previous_messages_all_runs
),
best_agent AS (
    -- Use the provided document_agent_id directly (UI handles filtering and selection)
