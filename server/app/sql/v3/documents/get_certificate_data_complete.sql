...
DROP FUNCTION IF EXISTS api_get_certificate_data_v3(uuid);

-- 2) Drop types WITHOUT CASCADE (drop dependent type first)
-- If any other object depends on them, this will ERROR and stop the migration (good)
DROP TYPE IF EXISTS types.q_get_certificate_data_v3_cohort;  -- Drop first (depends on simulation)
DROP TYPE IF EXISTS types.q_get_certificate_data_v3_simulation;

-- 3) Recreate types
CREATE TYPE types.q_get_certificate_data_v3_simulation AS (
    name text,
    score int,
    passed boolean
);

CREATE TYPE types.q_get_certificate_data_v3_cohort AS (
    name text,
    passed boolean,
    simulations types.q_get_certificate_data_v3_simulation[]
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_certificate_data_v3(
    profile_id uuid
)
RETURNS TABLE (
    profile_name text,
    actor_name text,
    cohorts types.q_get_certificate_data_v3_cohort[]
)
LANGUAGE sql
STABLE
AS $$
...
cohort_simulation_data AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (sr.simulation_title, sr.score, sr.passed)::types.q_get_certificate_data_v3_simulation
                ORDER BY sr.simulation_title
            ),
            '{}'::types.q_get_certificate_data_v3_simulation[]
        ) AS simulations,
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.name, cd.passed, cd.simulations)::types.q_get_certificate_data_v3_cohort
            ORDER BY cd.name
        )
        FROM cohort_data cd),
        '{}'::types.q_get_certificate_data_v3_cohort[]
    ) AS cohorts
