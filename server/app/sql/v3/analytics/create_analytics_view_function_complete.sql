...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_analytics_view_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_analytics_view_v3(%s)', r.sig);
    END LOOP;
END $$;

-- Create function that returns actor_name and response fields
CREATE OR REPLACE FUNCTION api_create_analytics_view_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    success boolean,
    message text,
