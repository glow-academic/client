...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_classify_debug_eval_start_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_classify_debug_eval_start_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_classify_debug_eval_start_v3_%'
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION socket_classify_debug_eval_start_v3(
    profile_id uuid,
    test_id uuid,
    attempt_id uuid,
    eval_id uuid,
