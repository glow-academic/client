...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_member_regeneration_run_context_and_create_run_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_member_regeneration_run_context_and_create_run_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_member_regen_run_context_create_run_v3_%'
...
CREATE TYPE types.i_member_regen_run_context_create_run_v3_msg AS (
    role text,
    content text
);

CREATE TYPE types.i_member_regen_run_context_create_run_v3_document AS (
    id text,
    name text,
    file_path text,
    mime_type text
...
CREATE OR REPLACE FUNCTION socket_get_member_regeneration_run_context_and_create_run_v3(
    chat_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    documents types.i_member_regen_run_context_create_run_v3_document[],
    run_id text,
    group_id uuid,
    previous_messages types.i_member_regen_run_context_create_run_v3_msg[]
)
LANGUAGE sql
VOLATILE
AS $$
...
previous_messages_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.i_member_regen_run_context_create_run_v3_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.i_member_regen_run_context_create_run_v3_msg[]
...
documents_data AS (
    SELECT 
        sc.id as chat_id,
        ARRAY_AGG(
            (d.id::text, d.name, u.file_path, u.mime_type)::types.i_member_regen_run_context_create_run_v3_document
            ORDER BY d.id
...
SELECT 
    ...
    COALESCE(dd.documents, ARRAY[]::types.i_member_regen_run_context_create_run_v3_document[]) as documents,
