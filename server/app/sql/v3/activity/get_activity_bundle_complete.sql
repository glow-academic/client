...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_activity_bundle_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_activity_bundle_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_activity_bundle_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_activity_bundle_v3_chart_data_point AS (
    date date,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_activity_bundle_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    ...
    chart_data types.q_get_activity_bundle_v3_chart_data_point[]
)
LANGUAGE sql
STABLE
AS $$
...
SELECT 
    ...
    (SELECT COUNT(*) FROM feedback)::bigint as total_feedback_count,
    ...
    COALESCE(
        ARRAY_AGG(
            (cd.date, cd.active_profiles_count::integer, cd.feedback_count::integer, cd.activity_count::integer, cd.error_count::integer)::types.q_get_activity_bundle_v3_chart_data_point
            ORDER BY cd.date
        ),
        '{}'::types.q_get_activity_bundle_v3_chart_data_point[]
    ) as chart_data
