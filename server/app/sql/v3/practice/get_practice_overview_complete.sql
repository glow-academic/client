...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_practice_overview_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_practice_overview_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_practice_overview_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_practice_overview_v3_practice_simulation AS (
...
CREATE TYPE types.q_get_practice_overview_v3_standard_group AS (
...
CREATE TYPE types.q_get_practice_overview_v3_standard AS (
    standard_id uuid,
    name text,
    description text,
    points int
...
CREATE TYPE types.q_get_practice_overview_v3_simulation AS (
    simulation_id uuid,
    name text,
    description text,
    time_limit int,
...
CREATE TYPE types.q_get_practice_overview_v3_persona AS (
    persona_id uuid,
    name text,
    description text,
    color text,
...
CREATE TYPE types.q_get_practice_overview_v3_scenario AS (
    scenario_id uuid,
    name text,
    description text,
    persona_ids text[]
...
CREATE TYPE types.q_get_practice_overview_v3_parameter AS (
    parameter_id uuid,
    name text,
    description text,
...
CREATE TYPE types.q_get_practice_overview_v3_field AS (
    field_id uuid,
    name text,
    description text,
    parameter_id uuid,
...
CREATE TYPE types.q_get_practice_overview_v3_department AS (
    department_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_practice_overview_v3(
    profile_id uuid,
...
RETURNS TABLE (
    ...
    mode text,  -- 'practice'
    has_data boolean,
    items types.q_get_practice_overview_v3_practice_simulation[],
    standard_groups types.q_get_practice_overview_v3_standard_group[],
    standards types.q_get_practice_overview_v3_standard[],
    simulations types.q_get_practice_overview_v3_simulation[],
    personas types.q_get_practice_overview_v3_persona[],
    scenarios types.q_get_practice_overview_v3_scenario[],
    parameters types.q_get_practice_overview_v3_parameter[],
    fields types.q_get_practice_overview_v3_field[],
    departments types.q_get_practice_overview_v3_department[],
...
standard_groups_array AS (
    SELECT 
        (sg.id, sg.name, sg.description, sg.points, sg.pass_points)::types.q_get_practice_overview_v3_standard_group AS standard_group
...
standards_array AS (
    SELECT 
        (st.id, st.name, st.description, st.points)::types.q_get_practice_overview_v3_standard AS standard
    FROM standards st
...
items_rows AS (
    SELECT
        ...
        )::types.q_get_practice_overview_v3_practice_simulation AS item,
...
simulations_array AS (
    SELECT 
        (sim.id, sim.title, COALESCE(sim.description, ''), sim.time_limit, COALESCE(sim.department_ids, ARRAY[]::text[]))::types.q_get_practice_overview_v3_simulation AS simulation
...
personas_array AS (
    SELECT 
        (p.id, p.name, p.description, COALESCE(p.color, ''), COALESCE(p.icon, ''))::types.q_get_practice_overview_v3_persona AS persona
    FROM persona_data p
),
...
scenarios_array AS (
    SELECT 
        (s.id, s.name, s.description, s.persona_ids)::types.q_get_practice_overview_v3_scenario AS scenario
...
parameters_array AS (
    SELECT 
        (par.id, par.name, par.description, par.document_parameter, par.persona_parameter)::types.q_get_practice_overview_v3_parameter AS parameter
...
fields_array AS (
    SELECT 
        (pi.id, pi.name, pi.description, pi.parameter_id, pi.parameter_name)::types.q_get_practice_overview_v3_field AS field
...
departments_array AS (
    SELECT 
        (d.id, d.name, d.description)::types.q_get_practice_overview_v3_department AS department
...
items_agg AS (
    SELECT COALESCE(
        ...
        '{}'::types.q_get_practice_overview_v3_practice_simulation[]
    ) as items
    FROM items_rows ir
),
-- Aggregate mappings separately
standard_groups_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT standard_group), '{}'::types.q_get_practice_overview_v3_standard_group[]) as standard_groups
    FROM standard_groups_array
),
standards_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT standard), '{}'::types.q_get_practice_overview_v3_standard[]) as standards
    FROM standards_array
),
simulations_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT simulation), '{}'::types.q_get_practice_overview_v3_simulation[]) as simulations
    FROM simulations_array
),
personas_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT persona), '{}'::types.q_get_practice_overview_v3_persona[]) as personas
    FROM personas_array
),
scenarios_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT scenario), '{}'::types.q_get_practice_overview_v3_scenario[]) as scenarios
    FROM scenarios_array
),
parameters_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT parameter), '{}'::types.q_get_practice_overview_v3_parameter[]) as parameters
    FROM parameters_array
),
fields_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT field), '{}'::types.q_get_practice_overview_v3_field[]) as fields
    FROM fields_array
),
departments_agg AS (
    SELECT COALESCE(ARRAY_AGG(DISTINCT department), '{}'::types.q_get_practice_overview_v3_department[]) as departments
...
SELECT 
    ...
    COALESCE((SELECT items FROM items_agg), '{}'::types.q_get_practice_overview_v3_practice_simulation[]) as items,
    COALESCE((SELECT standard_groups FROM standard_groups_agg), '{}'::types.q_get_practice_overview_v3_standard_group[]) as standard_groups,
    COALESCE((SELECT standards FROM standards_agg), '{}'::types.q_get_practice_overview_v3_standard[]) as standards,
    COALESCE((SELECT simulations FROM simulations_agg), '{}'::types.q_get_practice_overview_v3_simulation[]) as simulations,
    COALESCE((SELECT personas FROM personas_agg), '{}'::types.q_get_practice_overview_v3_persona[]) as personas,
    COALESCE((SELECT scenarios FROM scenarios_agg), '{}'::types.q_get_practice_overview_v3_scenario[]) as scenarios,
    COALESCE((SELECT parameters FROM parameters_agg), '{}'::types.q_get_practice_overview_v3_parameter[]) as parameters,
    COALESCE((SELECT fields FROM fields_agg), '{}'::types.q_get_practice_overview_v3_field[]) as fields,
    COALESCE((SELECT departments FROM departments_agg), '{}'::types.q_get_practice_overview_v3_department[]) as departments,
