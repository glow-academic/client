...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_settings_list_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_settings_list_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_settings_list_v3_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_settings_list_v3_setting AS (
    settings_id uuid,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_settings_list_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    settings types.q_get_settings_list_v3_setting[]
)
LANGUAGE sql
STABLE
AS $$
...
SELECT 
    ...
    COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_settings_list_v3_setting
