...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_update_settings_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_update_settings_v3(%s)', r.sig);
    END LOOP;
END $$;
...
DECLARE
    ...
    type_order text[] := ARRAY[
        'i_update_settings_v3_auth_key',
        'i_update_settings_v3_auth_value',
        'i_update_settings_v3_auth_key_item',
        'i_update_settings_v3_auth_value_item',
        'i_update_settings_v3_provider_key',
        'i_update_settings_v3_provider_enabled',
        'i_update_settings_v3_auth_enabled'
    ];
    type_name text;
BEGIN
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_update_settings_v3_%'
...
CREATE TYPE types.i_update_settings_v3_provider_key AS (
    provider_id uuid,
    key_id uuid
);

CREATE TYPE types.i_update_settings_v3_auth_key_item AS (
    auth_item_id uuid,
    key_id uuid
);

CREATE TYPE types.i_update_settings_v3_auth_key AS (
    auth_id uuid,
    items types.i_update_settings_v3_auth_key_item[]
);

CREATE TYPE types.i_update_settings_v3_provider_enabled AS (
    provider_id uuid,
    enabled boolean
);

CREATE TYPE types.i_update_settings_v3_auth_enabled AS (
    auth_id uuid,
    enabled boolean
);

CREATE TYPE types.i_update_settings_v3_auth_value_item AS (
    auth_item_id uuid,
    value text
);

CREATE TYPE types.i_update_settings_v3_auth_value AS (
    auth_id uuid,
    items types.i_update_settings_v3_auth_value_item[]
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_update_settings_v3(
    name text,
    description text,
    primary_color text,
    accent text,
    ...
    profile_id uuid,
    provider_keys types.i_update_settings_v3_provider_key[] DEFAULT ARRAY[]::types.i_update_settings_v3_provider_key[],
    auth_keys types.i_update_settings_v3_auth_key[] DEFAULT ARRAY[]::types.i_update_settings_v3_auth_key[],
    default_admin_profile_id uuid DEFAULT NULL,
    default_guest_profile_id uuid DEFAULT NULL,
    provider_enabled types.i_update_settings_v3_provider_enabled[] DEFAULT ARRAY[]::types.i_update_settings_v3_provider_enabled[],
    auth_enabled types.i_update_settings_v3_auth_enabled[] DEFAULT ARRAY[]::types.i_update_settings_v3_auth_enabled[],
    auth_values types.i_update_settings_v3_auth_value[] DEFAULT ARRAY[]::types.i_update_settings_v3_auth_value[],
