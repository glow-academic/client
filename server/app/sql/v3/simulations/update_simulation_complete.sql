...
DROP FUNCTION IF EXISTS api_update_simulation_v3(uuid, text, text, boolean, boolean, uuid[], uuid[], boolean[], uuid[], boolean[], uuid[], int[], boolean[], boolean[], boolean[], boolean[], boolean[], uuid, uuid, uuid, uuid, uuid, uuid);

-- 2) Recreate function
CREATE OR REPLACE FUNCTION api_update_simulation_v3(
    simulation_id uuid,
    title text,
    description text,
    active boolean,
...
WITH params AS (
    SELECT 
        ...
        grade_text_agent_id AS grade_text_agent_id,
        grade_voice_agent_id AS grade_voice_agent_id,
...
update_simulation AS (
    UPDATE simulations SET
        ...
        grade_text_agent_id = COALESCE(x.grade_text_agent_id, simulations.grade_text_agent_id),
        grade_voice_agent_id = x.grade_voice_agent_id,
