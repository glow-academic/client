...
DROP TYPE IF EXISTS types.q_get_simulation_detail_v3_scenario_mapping;
...
CREATE TYPE types.q_get_simulation_detail_v3_scenario_mapping AS (
    scenario_id uuid,
    name text,
    description text,
    persona_ids uuid[],
...
RETURNS TABLE (
    ...
    scenarios_full types.q_get_simulation_detail_v3_scenario_mapping[],
...
scenarios_full_data AS (
    SELECT 
        ARRAY_AGG(
            ...
            )::types.q_get_simulation_detail_v3_scenario_mapping
            ORDER BY vsl.name
        ) as scenarios_full
...
SELECT 
    ...
    COALESCE(sfd.scenarios_full, ARRAY[]::types.q_get_simulation_detail_v3_scenario_mapping[]) as scenarios_full,
