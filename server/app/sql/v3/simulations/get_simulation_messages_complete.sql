...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_simulation_messages_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_simulation_messages_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_simulation_messages_v3_%'
...
CREATE TYPE types.q_get_simulation_messages_v3_message AS (
    id text,
    chat_id text,
    role text,
    content text,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION socket_get_simulation_messages_v3(
    chat_id uuid
)
RETURNS TABLE (
    messages types.q_get_simulation_messages_v3_message[]
)
LANGUAGE sql
STABLE
AS $$
...
SELECT 
    ARRAY_AGG(
        (dm.id, dm.chat_id, dm.role, dm.content, dm.created_at, dm.completed, dm.updated_at, dm.audio, dm.upload_id)::types.q_get_simulation_messages_v3_message
