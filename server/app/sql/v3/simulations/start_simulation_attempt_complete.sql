...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_start_simulation_attempt_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_start_simulation_attempt_v3(%s)', r.sig);
    END LOOP;
END $$;
...
CREATE TYPE types.q_start_simulation_attempt_v3_document AS (
    id text,
    name text,
    file_path text,
    mime_type text
...
CREATE TYPE types.q_start_simulation_attempt_v3_parameter_item AS (
    id text,
    name text,
    description text,
    parameter_id text,
...
CREATE TYPE types.q_start_simulation_attempt_v3_simulation_data AS (
    id text,
    title text,
    description text,
    active boolean,
...
CREATE TYPE types.q_start_simulation_attempt_v3_scenario_metadata AS (
    persona_id text,
    persona_name text,
    ...
    documents types.q_start_simulation_attempt_v3_document[],
    parameter_items types.q_start_simulation_attempt_v3_parameter_item[],
    active boolean,
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION socket_start_simulation_attempt_v3(
    simulation_id uuid,
...
RETURNS TABLE (
    ...
    video_id text,
    trace_id text,
    simulation_data types.q_start_simulation_attempt_v3_simulation_data,
    scenario_metadata types.q_start_simulation_attempt_v3_scenario_metadata
)
LANGUAGE sql
VOLATILE
AS $$
...
SELECT 
    ...
    (sd.id::text, sd.title, sd.description, sd.active, sd.practice_simulation, sd.rubric_id::text)::types.q_start_simulation_attempt_v3_simulation_data as simulation_data,
    ...
    CASE 
        WHEN ctc.content_type = 'scenario' THEN
            (
                ...
                COALESCE(
                    (SELECT ARRAY_AGG(
                        (dd.document_id::text, dd.name, dd.file_path, dd.mime_type)::types.q_start_simulation_attempt_v3_document
                        ORDER BY dd.document_id
                    ) FROM document_data dd WHERE dd.scenario_id = sfd.scenario_id),
                    '{}'::types.q_start_simulation_attempt_v3_document[]
                ),
                COALESCE(
                    (SELECT ARRAY_AGG(
                        (pid.field_id::text, pid.name, pid.description, pid.parameter_id::text, pid.parameter_name)::types.q_start_simulation_attempt_v3_parameter_item
                        ORDER BY pid.field_id
                    ) FROM parameter_item_data pid WHERE pid.scenario_id = sfd.scenario_id),
                    '{}'::types.q_start_simulation_attempt_v3_parameter_item[]
                ),
                sfd.active,
                sfd.default_scenario,
                sfd.generated
            )::types.q_start_simulation_attempt_v3_scenario_metadata
        ELSE NULL::types.q_start_simulation_attempt_v3_scenario_metadata
