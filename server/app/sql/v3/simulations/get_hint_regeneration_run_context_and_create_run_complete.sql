...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_hint_regeneration_run_context_and_create_run_v3'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_hint_regeneration_run_context_and_create_run_v3(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_hint_regen_run_context_create_run_v3_%'
...
CREATE TYPE types.i_hint_regen_run_context_create_run_v3_msg AS (
    role text,
    content text
);

CREATE TYPE types.i_hint_regen_run_context_create_run_v3_document AS (
    document_id uuid,
    name text,
    file_path text,
    mime_type text
...
CREATE OR REPLACE FUNCTION socket_get_hint_regeneration_run_context_and_create_run_v3(
    message_id uuid,
    chat_id uuid,
    department_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    documents types.i_hint_regen_run_context_create_run_v3_document[],
    run_id text,
    group_id uuid,
    previous_messages types.i_hint_regen_run_context_create_run_v3_msg[]  -- All messages from all previous runs in group
)
LANGUAGE sql
VOLATILE
AS $$
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.i_hint_regen_run_context_create_run_v3_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.i_hint_regen_run_context_create_run_v3_msg[]
...
profile_rate_limit AS (
    ...
    CROSS JOIN params p_params
    WHERE p.id = p_params.profile_id
),
runs_today AS (
...
context_data AS (
    SELECT 
        ...
        COALESCE(
            (SELECT ARRAY_AGG(
                (dd.document_id, dd.name, dd.file_path, dd.mime_type)::types.i_hint_regen_run_context_create_run_v3_document
                ORDER BY dd.document_id
            ) FROM document_data dd),
            '{}'::types.i_hint_regen_run_context_create_run_v3_document[]
        ) as documents
