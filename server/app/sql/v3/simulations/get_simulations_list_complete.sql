...
CREATE TYPE types.q_list_simulations_v3_document AS (
    document_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_simulations_v3_field AS (
    field_id uuid,
    name text,
    description text,
    parameter_id uuid,
...
CREATE TYPE types.q_list_simulations_v3_scenario AS (
    scenario_id uuid,
    name text,
    description text,
    active boolean,
    persona_ids text[],
    persona_mapping types.q_list_simulations_v3_persona[],
    document_mapping types.q_list_simulations_v3_document[],
    parameter_item_mapping types.q_list_simulations_v3_field[],
    parameter_item_ids text[],
    document_ids text[]
);

CREATE TYPE types.q_list_simulations_v3_simulation AS (
    simulation_id uuid,
    name text,
    description text,
    department_ids text[],
...
CREATE TYPE types.q_list_simulations_v3_rubric AS (
    rubric_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_simulations_v3_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_simulations_v3_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_list_simulations_v3_option AS (
    value text,
    label text
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_list_simulations_v3(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    simulations types.q_list_simulations_v3_simulation[],
    scenarios types.q_list_simulations_v3_scenario[],
    rubrics types.q_list_simulations_v3_rubric[],
    departments types.q_list_simulations_v3_department[],
    cohorts types.q_list_simulations_v3_cohort[],
    rubric_options types.q_list_simulations_v3_option[],
    cohort_options types.q_list_simulations_v3_option[],
    department_options types.q_list_simulations_v3_option[]
)
LANGUAGE sql
STABLE
AS $$
...
scenario_persona_mapping AS (
    SELECT 
        sp.scenario_id,
        ARRAY_AGG(
            (pd.persona_id, pd.name, pd.description, pd.color, pd.icon, pd.image_model)::types.q_list_simulations_v3_persona
            ORDER BY pd.name
        ) as personas
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            ...
            )::types.q_list_simulations_v3_simulation
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (sbd.scenario_id, sbd.name, sbd.description, sbd.active, sbd.persona_ids,
             COALESCE(spm.personas, ARRAY[]::types.q_list_simulations_v3_persona[]),
             ARRAY[]::types.q_list_simulations_v3_document[],
             ARRAY[]::types.q_list_simulations_v3_field[],
             sbd.parameter_item_ids,
             sbd.document_ids
            )::types.q_list_simulations_v3_scenario
            ORDER BY sbd.name
        ) FROM scenario_base_data sbd
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (rd.rubric_id, rd.name, rd.description)::types.q_list_simulations_v3_rubric
            ORDER BY rd.name
        ) FROM rubric_data rd),
        '{}'::types.q_list_simulations_v3_rubric[]
    ) as rubrics,
    -- Aggregate departments
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_list_simulations_v3_department
            ORDER BY dd.name
        ) FROM department_data dd),
        '{}'::types.q_list_simulations_v3_department[]
    ) as departments,
    -- Aggregate cohorts
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.cohort_id, cd.name, cd.description)::types.q_list_simulations_v3_cohort
            ORDER BY cd.name
        ) FROM cohort_data cd),
        '{}'::types.q_list_simulations_v3_cohort[]
    ) as cohorts,
    -- Rubric options
    COALESCE(
        (SELECT ARRAY_AGG(
            (rd.rubric_id::text, rd.name)::types.q_list_simulations_v3_option
            ORDER BY rd.name
        ) FROM rubric_data rd),
        '{}'::types.q_list_simulations_v3_option[]
    ) as rubric_options,
    -- Cohort options
    COALESCE(
        (SELECT ARRAY_AGG(
            (cd.cohort_id::text, cd.name)::types.q_list_simulations_v3_option
            ORDER BY cd.name
        ) FROM cohort_data cd),
        '{}'::types.q_list_simulations_v3_option[]
    ) as cohort_options,
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.department_id::text, dd.name)::types.q_list_simulations_v3_option
            ORDER BY dd.name
        ) FROM department_data dd),
        '{}'::types.q_list_simulations_v3_option[]
