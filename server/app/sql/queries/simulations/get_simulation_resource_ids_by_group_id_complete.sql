...
AS $$
DECLARE
    valid_simulation_resource_types text[] := ARRAY['names', 'descriptions', 'departments', 'flags', 'scenarios', 'scenario_flags', 'scenario_positions', 'scenario_rubrics', 'scenario_time_limits'];
BEGIN
    ...
    SELECT
        ...
        CASE WHEN resource_type = 'scenarios' THEN ARRAY[resources_id] ELSE ARRAY[]::uuid[] END as scenario_ids,
        CASE WHEN resource_type = 'scenario_flags' THEN ARRAY[resources_id] ELSE ARRAY[]::uuid[] END as scenario_flag_ids,
        CASE WHEN resource_type = 'scenario_positions' THEN ARRAY[resources_id] ELSE ARRAY[]::uuid[] END as scenario_position_ids,
        CASE WHEN resource_type = 'scenario_rubrics' THEN ARRAY[resources_id] ELSE ARRAY[]::uuid[] END as scenario_rubric_ids,
        CASE WHEN resource_type = 'scenario_time_limits' THEN ARRAY[resources_id] ELSE ARRAY[]::uuid[] END as scenario_time_limit_ids;
END;
$$;
