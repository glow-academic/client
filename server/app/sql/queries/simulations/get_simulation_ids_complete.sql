...
-- Get names_id (from draft first, then simulation)
name_data AS (
    SELECT COALESCE(
        (SELECT nd.names_id FROM simulation_drafts_names_connection nd WHERE nd.draft_id = (SELECT p_draft_id FROM params) LIMIT 1),
        (SELECT sn.names_id FROM simulation_names_junction sn WHERE sn.simulation_id = (SELECT p_simulation_id FROM params) LIMIT 1)
    ) as names_id
),
-- Get descriptions_id (from draft first, then simulation)
description_data AS (
    SELECT COALESCE(
        ...
        (SELECT sd.descriptions_id FROM simulation_descriptions_junction sd WHERE sd.simulation_id = (SELECT p_simulation_id FROM params) LIMIT 1)
    ) as descriptions_id
),
...
scenario_flag_data AS (
    SELECT COALESCE(
        ...
        (SELECT ARRAY_AGG(ssf.scenario_flags_id) FROM simulation_scenario_flags_junction ssf JOIN scenario_flags_resource sfr ON sfr.id = ssf.scenario_flags_id JOIN flags_resource f ON f.id = sfr.flag_id WHERE ssf.simulation_id = (SELECT p_simulation_id FROM params) AND f.value = true),
        ARRAY[]::uuid[]
...
scenario_position_data AS (
    SELECT COALESCE(
        ...
        (SELECT ARRAY_AGG(ssp.scenario_positions_id) FROM simulation_scenario_positions_junction ssp WHERE ssp.simulation_id = (SELECT p_simulation_id FROM params) AND ssp.active = true),
        ARRAY[]::uuid[]
...
scenario_rubric_data AS (
    SELECT COALESCE(
        ...
        (SELECT ARRAY_AGG(ssr.scenario_rubrics_id) FROM simulation_scenario_rubrics_junction ssr WHERE ssr.simulation_id = (SELECT p_simulation_id FROM params) AND ssr.active = true),
        ARRAY[]::uuid[]
...
scenario_time_limit_data AS (
    SELECT COALESCE(
        ...
        (SELECT ARRAY_AGG(sstl.scenario_time_limits_id) FROM simulation_scenario_time_limits_junction sstl WHERE sstl.simulation_id = (SELECT p_simulation_id FROM params) AND sstl.active = true),
        ARRAY[]::uuid[]
    ) as scenario_time_limit_ids
)
SELECT
    (SELECT names_id FROM name_data),
    (SELECT descriptions_id FROM description_data),
