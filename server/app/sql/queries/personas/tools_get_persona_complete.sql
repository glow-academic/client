-- Tools layer: Persona ID Fetching
-- Returns all resource IDs for a persona (junction IDs only, no suggestions)
-- Independent copy of get_persona_ids_complete.sql for tools layer evolution
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'tools_get_persona_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS tools_get_persona_v4(%s)', r.sig);
    END LOOP;
END $$;

-- Create function
CREATE OR REPLACE FUNCTION tools_get_persona_v4(
    profile_id uuid,
