...
sim_data AS (
    ...
    SELECT DISTINCT
        ...
        EXISTS (SELECT 1 FROM simulation_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.simulation_id = s.id AND f.name = 'practice' AND f.value = TRUE)
    FROM simulation_artifact s
    JOIN cohort_simulations_junction cs ON cs.simulations_id = s.id
    ...
    JOIN profiles_resource pr_sim ON pr_sim.id = ppj_sim.profile_id AND pr_sim.id = ANY(cr.profile_ids)
    LEFT JOIN (
        SELECT
            sd.simulation_id,
...
simulations_aggregated AS (
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (s.id, (SELECT n.name FROM simulation_names_junction sn JOIN names_resource n ON sn.names_id = n.id WHERE sn.simulation_id = s.id LIMIT 1), (SELECT (SELECT d.description FROM document_descriptions_junction dd JOIN descriptions_resource d ON dd.descriptions_id = d.id WHERE dd.document_id = d.id LIMIT 1) FROM scenario_descriptions_junction sd JOIN descriptions_resource d ON sd.descriptions_id = d.id WHERE sd.scenario_id = s.id LIMIT 1), s.department_ids, s.time_limit, EXISTS (SELECT 1 FROM simulation_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.simulation_id = s.id AND f.name = 'simulation_active' AND f.value = TRUE), EXISTS (SELECT 1 FROM simulation_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.simulation_id = s.id AND f.name = 'practice' AND f.value = TRUE))::types.q_get_profile_context_v4_simulation
                ORDER BY (SELECT n.name FROM simulation_names_junction sn JOIN names_resource n ON sn.names_id = n.id WHERE sn.simulation_id = s.id LIMIT 1)
            ),
...
earliest_attempt AS (
    ...
    JOIN profile_departments_junction pd_all ON pd_all.departments_id = pd_effective.departments_id
    ...
    JOIN all_attempt_profiles aap ON aap.profile_id = ppj.profile_id
...
settings_resolution AS (
    ...
    settings_auths_data AS (
        ...
        SELECT 
            ARRAY_AGG(a.id::text ORDER BY (SELECT n.name FROM auth_names_junction an JOIN names_resource n ON an.names_id = n.id WHERE an.auth_id = a.id LIMIT 1)) as auth_ids,
            COALESCE(
                ARRAY_AGG(
                    (a.id, (SELECT n.name FROM auth_names_junction an JOIN names_resource n ON an.names_id = n.id WHERE an.auth_id = a.id LIMIT 1), COALESCE((SELECT d.description FROM auth_descriptions_junction ad JOIN descriptions_resource d ON ad.descriptions_id = d.id WHERE ad.auth_id = a.id LIMIT 1), ''), (SELECT s.value FROM auth_slugs_junction as_j JOIN slugs_resource s ON s.id = as_j.slugs_id WHERE as_j.auth_id = a.id LIMIT 1))::types.q_get_profile_context_v4_auth
        ...
        JOIN setting_auths_junction sa ON sa.setting_id = ss.settings_id AND sa.active = true
        JOIN auths_resource a ON a.id = sa.auths_id AND EXISTS (SELECT 1 FROM auth_flags_junction af JOIN flags_resource f ON af.flags_id = f.id WHERE af.auth_id = a.id AND f.name = 'auth_active' AND f.value = true)
    ),
    ...
    SELECT 
        ...
        EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'setting_active' AND f.value = TRUE) as settings_active,
        (SELECT n.name FROM setting_names_junction sn JOIN names_resource n ON sn.names_id = n.id WHERE sn.setting_id = s.id LIMIT 1) as settings_name,
        (SELECT d.description FROM setting_descriptions_junction sd JOIN descriptions_resource d ON sd.descriptions_id = d.id WHERE sd.setting_id = s.id LIMIT 1) as settings_description,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'primary'::color_type LIMIT 1) as primary_color,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'accent'::color_type LIMIT 1) as accent,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'background'::color_type LIMIT 1) as background,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'surface'::color_type LIMIT 1) as surface,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'success'::color_type LIMIT 1) as success,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'warning'::color_type LIMIT 1) as warning,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'error'::color_type LIMIT 1) as error,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'sidebar_background'::color_type LIMIT 1) as sidebar_background,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'sidebar_primary'::color_type LIMIT 1) as sidebar_primary,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'chart1'::color_type LIMIT 1) as chart1,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'chart2'::color_type LIMIT 1) as chart2,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'chart3'::color_type LIMIT 1) as chart3,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'chart4'::color_type LIMIT 1) as chart4,
        (SELECT c.hex_code FROM setting_colors_junction sc JOIN colors_resource c ON sc.colors_id = c.id WHERE sc.setting_id = s.id AND c.type = 'chart5'::color_type LIMIT 1) as chart5,
        EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'guest_login_enabled' AND f.value = TRUE) as settings_guest_login_enabled,
        (SELECT t.value FROM setting_thresholds_junction st JOIN thresholds_resource t ON st.thresholds_id = t.id WHERE st.setting_id = s.id AND t.type = 'success'::threshold_type LIMIT 1) as success_threshold,
        (SELECT t.value FROM setting_thresholds_junction st JOIN thresholds_resource t ON st.thresholds_id = t.id WHERE st.setting_id = s.id AND t.type = 'warning'::threshold_type LIMIT 1) as warning_threshold,
        (SELECT t.value FROM setting_thresholds_junction st JOIN thresholds_resource t ON st.thresholds_id = t.id WHERE st.setting_id = s.id AND t.type = 'danger'::threshold_type LIMIT 1) as danger_threshold,
...
simulation_ids_computed AS (
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(s.id::text ORDER BY (SELECT n.name FROM simulation_names_junction sn JOIN names_resource n ON sn.names_id = n.id WHERE sn.simulation_id = s.id LIMIT 1)),
            ARRAY[]::text[]
        ) as simulation_ids
    FROM sim_data s
),
drafts_data AS (
    ...
    JOIN (SELECT id, 'agent'::text as artifact, version, session_id, created_at as updated_at FROM agent_drafts_entry WHERE active = true
     ...
     UNION ALL SELECT id, 'department', version, session_id, created_at FROM department_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'document', version, session_id, created_at FROM document_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'eval', version, session_id, created_at FROM eval_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'field', version, session_id, created_at FROM field_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'model', version, session_id, created_at FROM model_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'parameter', version, session_id, created_at FROM parameter_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'persona', version, session_id, created_at FROM persona_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'profile', version, session_id, created_at FROM profile_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'provider', version, session_id, created_at FROM provider_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'rubric', version, session_id, created_at FROM rubric_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'scenario', version, session_id, created_at FROM scenario_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'setting', version, session_id, created_at FROM setting_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'simulation', version, session_id, created_at FROM simulation_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'suite', version, session_id, created_at FROM invocation_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'tool', version, session_id, created_at FROM tool_drafts_entry WHERE active = true
     UNION ALL SELECT id, 'training', version, session_id, created_at FROM chat_drafts_entry WHERE active = true) d ON d.session_id = psc.session_id
