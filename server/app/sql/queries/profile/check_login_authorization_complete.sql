...
default_settings AS (
    ...
    SELECT 
        s.id as settings_id,
        EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'guest_login_enabled' AND f.value = TRUE) as guest_login_enabled
...
active_departments_count AS (
    ...
    WHERE EXISTS (SELECT 1 FROM department_flags_junction df JOIN flags_resource f ON df.flags_id = f.id WHERE df.department_id = department_artifact.id AND f.name = 'department_active' AND f.value = true)
),
...
department_auth_providers_count AS (
    ...
    JOIN setting_artifact s ON s.id = ds.settings_id AND EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'setting_active' AND f.value = TRUE)
    JOIN setting_auths_junction sa ON sa.setting_id = s.id AND sa.active = true
