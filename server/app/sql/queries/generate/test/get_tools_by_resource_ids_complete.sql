-- Get tools by tools_resource IDs (skips agent_tools_junction).
-- Used by test run.py to resolve tools from bundle department's tool_ids.
-- Same composite return type as get_agent_entry_tools.
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'socket_get_tools_by_resource_ids_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_tools_by_resource_ids_v4(%s)', r.sig);
    END LOOP;
END $$;
...
CREATE OR REPLACE FUNCTION socket_get_tools_by_resource_ids_v4(
    p_tool_resource_ids uuid[]
)
RETURNS TABLE (
