-- Prepare dashboard generation: mutations only (group/run/config creation)
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'socket_prepare_dashboard_generation_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_prepare_dashboard_generation_v4(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Create the function (mutations only)
CREATE OR REPLACE FUNCTION socket_prepare_dashboard_generation_v4(
    p_profile_id uuid,
