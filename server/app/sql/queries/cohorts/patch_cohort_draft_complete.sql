...
-- Ensure cohort draft composite types exist before function creation.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
BEGIN
    ...
    ON CONFLICT ON CONSTRAINT cohort_drafts_profiles_connection_pkey DO UPDATE
    SET version = v_new_version;
    ...
    INSERT INTO cohort_drafts_profile_personas_connection (draft_id, profile_personas_id, version)
    SELECT v_draft_id, ppid, v_new_version
    FROM UNNEST(v_profile_persona_ids) AS ppid
    ON CONFLICT ON CONSTRAINT cohort_drafts_profile_personas_connection_pkey DO UPDATE
    SET version = v_new_version;
    ...
    IF (
        ...
        IF v_name_id IS NOT NULL AND (names).create_tool_id IS NOT NULL THEN
            ...
            VALUES (v_call_id, 'cohort_draft_create_names_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_name_id IS NOT NULL AND (names).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_names_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_description_id IS NOT NULL AND (descriptions).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_descriptions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_description_id IS NOT NULL AND (descriptions).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_descriptions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_active_flag_id IS NOT NULL AND (flags).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_active_flag_id IS NOT NULL AND (flags).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_department_ids, 1), 0) > 0 AND (departments).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_departments_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_department_ids, 1), 0) > 0 AND (departments).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_departments_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_ids, 1), 0) > 0 AND (simulations).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_simulations_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_ids, 1), 0) > 0 AND (simulations).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_simulations_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_position_ids, 1), 0) > 0 AND (simulation_positions).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_simulation_positions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_position_ids, 1), 0) > 0 AND (simulation_positions).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_simulation_positions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_availability_ids, 1), 0) > 0 AND (simulation_availability).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_simulation_availability_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_simulation_availability_ids, 1), 0) > 0 AND (simulation_availability).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_simulation_availability_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_profile_ids, 1), 0) > 0 AND (profiles).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_profiles_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_profile_ids, 1), 0) > 0 AND (profiles).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_profiles_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_profile_persona_ids, 1), 0) > 0 AND (profile_personas).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_create_profile_personas_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_profile_persona_ids, 1), 0) > 0 AND (profile_personas).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'cohort_draft_link_profile_personas_' || v_call_id::text, v_run_id, NOW());
