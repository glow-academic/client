-- Refresh Benchmark Materialized Views - API Endpoint
-- Refreshes benchmark-related MVs.
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_refresh_mv_benchmark_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_refresh_mv_benchmark_v4(%s)', r.sig);
    END LOOP;
END $$;
...
CREATE OR REPLACE FUNCTION api_refresh_mv_benchmark_v4(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    success boolean,
    message text,
...
BEGIN
    -- Refresh benchmark MVs (all independent, no ordering required)
    ...
    SELECT COALESCE(
        (SELECT n.name FROM profile_names_junction pn JOIN names_resource n ON pn.names_id = n.id WHERE pn.profile_id = api_refresh_mv_benchmark_v4.profile_id LIMIT 1),
        'System'
    ) INTO actor_name_val
    FROM profile_artifact
    WHERE id = api_refresh_mv_benchmark_v4.profile_id;

    -- Return success response
    RETURN QUERY SELECT
        COALESCE(actor_name_val, 'System')::text as actor_name,
        true::boolean as success,
        format('Refreshed %s benchmark materialized views', array_length(refreshed, 1))::text as message,
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN QUERY SELECT
            ...
            format('Failed to refresh benchmark MVs: %s (completed: %s)', SQLERRM, array_to_string(refreshed, ', '))::text as message,
