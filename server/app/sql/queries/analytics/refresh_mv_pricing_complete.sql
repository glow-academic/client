...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_refresh_mv_pricing_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_refresh_mv_pricing_v4(%s)', r.sig);
    END LOOP;
END $$;
...
CREATE OR REPLACE FUNCTION api_refresh_mv_pricing_v4(profile_id uuid)
RETURNS TABLE (
    actor_name text,
    success boolean,
    message text,
...
BEGIN
    ...
    SELECT COALESCE(
        (SELECT n.name FROM profile_names_junction pn JOIN names_resource n ON pn.names_id = n.id WHERE pn.profile_id = api_refresh_mv_pricing_v4.profile_id LIMIT 1),
        'System'
    ) INTO actor_name_val
    FROM profile_artifact
    WHERE id = api_refresh_mv_pricing_v4.profile_id;
