...
FROM (
    ...
    WHERE r.active = true
      ...
      AND (COALESCE(array_length(reasoning_level_ids, 1), 0) = 0 OR r.id = ANY(reasoning_level_ids))
      -- Artifact boolean filters (each filters to resources linked to at least one of that artifact type)
      AND (NOT agent OR EXISTS (SELECT 1 FROM agent_reasoning_levels_junction arj WHERE arj.reasoning_levels_id = r.id AND arj.active = true))
      AND (NOT model OR EXISTS (SELECT 1 FROM model_reasoning_levels_junction j WHERE j.reasoning_levels_id = r.id AND j.active = true))
