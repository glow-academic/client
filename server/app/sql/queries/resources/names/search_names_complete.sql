...
FROM (
    ...
    WHERE n.name IS NOT NULL
      ...
      AND (NOT agent OR EXISTS (SELECT 1 FROM agent_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT auth OR EXISTS (SELECT 1 FROM auth_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT cohort OR EXISTS (SELECT 1 FROM cohort_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT department OR EXISTS (SELECT 1 FROM department_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT document OR EXISTS (SELECT 1 FROM document_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT eval OR EXISTS (SELECT 1 FROM eval_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT field OR EXISTS (SELECT 1 FROM field_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT model OR EXISTS (SELECT 1 FROM model_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT parameter OR EXISTS (SELECT 1 FROM parameter_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT persona OR EXISTS (SELECT 1 FROM persona_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT profile OR EXISTS (SELECT 1 FROM profile_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT provider OR EXISTS (SELECT 1 FROM provider_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT rubric OR EXISTS (SELECT 1 FROM rubric_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT scenario OR EXISTS (SELECT 1 FROM scenario_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT setting OR EXISTS (SELECT 1 FROM setting_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT simulation OR EXISTS (SELECT 1 FROM simulation_names_junction j WHERE j.names_id = n.id AND j.active = true))
      AND (NOT tool OR EXISTS (SELECT 1 FROM tool_names_junction j WHERE j.names_id = n.id AND j.active = true))
    ORDER BY n.name
    LIMIT limit_count
    OFFSET offset_count
) q;
