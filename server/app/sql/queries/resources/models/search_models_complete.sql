...
FROM (
    ...
    WHERE m.name IS NOT NULL
      ...
      AND (NOT agent OR EXISTS (SELECT 1 FROM agent_models_junction amj WHERE amj.models_id = m.id AND amj.active = true))
      AND (NOT model OR EXISTS (SELECT 1 FROM model_models_junction j WHERE j.models_id = m.id AND j.active = true))
    ORDER BY m.name
    LIMIT limit_count
    OFFSET offset_count
) q;
