...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_link_names_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_link_names_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_link_names_v4.group_id, NOW(), NOW());

        v_call_id := uuidv7();
        INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
        VALUES (v_call_id, 'names_' || v_call_id::text, v_run_id, NOW());
