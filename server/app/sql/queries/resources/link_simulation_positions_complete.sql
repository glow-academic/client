-- Link simulation_positions resource (tool call tracking for non-creatable resources)
-- Records a tool call linking an existing simulation_positions resource to a run/call chain
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_link_simulation_positions_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_link_simulation_positions_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_link_simulation_positions_v4(
    resources_id uuid DEFAULT NULL,
    group_id uuid DEFAULT NULL,
    tool_id uuid DEFAULT NULL
)
RETURNS TABLE (
    simulation_positions_id uuid
)
LANGUAGE plpgsql
VOLATILE
AS $$
...
BEGIN
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_link_simulation_positions_v4.group_id, NOW(), NOW());
        ...
        VALUES (api_link_simulation_positions_v4.tool_id, v_call_id);

        -- Link resource to call
        INSERT INTO simulation_positions_calls_connection (simulation_positions_id, call_id)
        VALUES (api_link_simulation_positions_v4.resources_id, v_call_id);
    END IF;

    RETURN QUERY SELECT api_link_simulation_positions_v4.resources_id;
END;
$$;
