...
BEGIN
    ...
    IF template IS NOT NULL THEN
        ...
        IF v_instruction_id IS NOT NULL THEN
            RETURN QUERY SELECT v_instruction_id, NULL::uuid;
            RETURN;
        END IF;
    END IF;
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_create_instructions_v4.group_id, NOW(), NOW());
        ...
        -- Link tool to call
        INSERT INTO tools_calls_connection (tools_id, call_id) VALUES (api_create_instructions_v4.tool_id, v_call_id);
    ...
    RETURN QUERY SELECT v_instruction_id, v_call_id;
END;
$$;
