-- Search points resources
-- Parameters: search (text), limit_count (int), offset_count (int), exclude_ids (uuid[])
-- Returns: items (array of points resources)

-- Drop function if exists (handles signature variations)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN
        SELECT oidvectortypes(proargtypes) as sig
        FROM pg_proc
        WHERE proname = 'api_search_points_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_search_points_v4(%s)', r.sig);
    END LOOP;
END $$;

-- Create function
CREATE OR REPLACE FUNCTION api_search_points_v4(
    search text DEFAULT NULL,
    limit_count int DEFAULT 20,
    offset_count int DEFAULT 0,
    exclude_ids uuid[] DEFAULT ARRAY[]::uuid[],
    -- Artifact boolean filters: when true, only return resources linked to that artifact type
    rubric boolean DEFAULT false
)
RETURNS TABLE (
    items types.q_get_points_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
SELECT COALESCE(
    ARRAY_AGG(
        (q.id, q.value, q.generated)::types.q_get_points_v4_item
        ORDER BY q.id
    ),
    ARRAY[]::types.q_get_points_v4_item[]
) as items
FROM (
    SELECT p.id, p.value, COALESCE(p.generated, false) AS generated
    FROM points_resource p
    WHERE p.active = true
      AND (exclude_ids IS NULL OR NOT (p.id = ANY(exclude_ids)))
      -- Artifact boolean filters (each filters to resources linked to at least one of that artifact type)
      AND (NOT rubric OR EXISTS (SELECT 1 FROM rubric_points_junction j WHERE j.points_id = p.id AND j.active = true))
    ORDER BY p.id
    LIMIT limit_count
    OFFSET offset_count
) q;
$$;
