...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_create_items_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_items_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_items_v4(
...
DECLARE
    ...
    v_call_id uuid;
BEGIN
    -- INSERT INTO items_resource table
    INSERT INTO items_resource(active, mcp)
    VALUES (true, mcp)
    RETURNING id INTO v_items_id;
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_create_items_v4.group_id, NOW(), NOW());

        -- Create call record
        v_call_id := uuidv7();
        INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
        VALUES (v_call_id, 'items_' || v_call_id::text, v_run_id, NOW());
    ...
    RETURN QUERY SELECT v_items_id;
END;
$$;
