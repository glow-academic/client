...
FROM (
    ...
    WHERE d.description IS NOT NULL
      ...
      AND (NOT agent OR EXISTS (SELECT 1 FROM agent_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT auth OR EXISTS (SELECT 1 FROM auth_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT cohort OR EXISTS (SELECT 1 FROM cohort_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT department OR EXISTS (SELECT 1 FROM department_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT document OR EXISTS (SELECT 1 FROM document_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT eval OR EXISTS (SELECT 1 FROM eval_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT field OR EXISTS (SELECT 1 FROM field_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT model OR EXISTS (SELECT 1 FROM model_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT parameter OR EXISTS (SELECT 1 FROM parameter_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT persona OR EXISTS (SELECT 1 FROM persona_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT provider OR EXISTS (SELECT 1 FROM provider_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT rubric OR EXISTS (SELECT 1 FROM rubric_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT scenario OR EXISTS (SELECT 1 FROM scenario_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT setting OR EXISTS (SELECT 1 FROM setting_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT simulation OR EXISTS (SELECT 1 FROM simulation_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
      AND (NOT tool OR EXISTS (SELECT 1 FROM tool_descriptions_junction j WHERE j.descriptions_id = d.id AND j.active = true))
