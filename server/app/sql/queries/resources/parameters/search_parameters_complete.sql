...
FROM (
    ...
    WHERE p.active = true
      ...
      AND (NOT document OR EXISTS (SELECT 1 FROM document_parameter_fields_junction j JOIN parameter_fields_resource pfr ON pfr.id = j.parameter_fields_id WHERE pfr.parameter_id = p.id AND j.active = true))
      AND (NOT parameter OR EXISTS (SELECT 1 FROM parameter_parameters_junction j WHERE j.parameter_id = p.id AND j.active = true))
      AND (NOT persona OR EXISTS (SELECT 1 FROM persona_parameter_fields_junction j JOIN parameter_fields_resource pfr ON pfr.id = j.parameter_fields_id WHERE pfr.parameter_id = p.id AND j.active = true))
      AND (NOT scenario OR EXISTS (SELECT 1 FROM scenario_parameter_fields_junction j JOIN parameter_fields_resource pfr ON pfr.id = j.parameter_fields_id WHERE pfr.parameter_id = p.id AND j.active = true))
    ORDER BY p.name
    LIMIT limit_count
    OFFSET offset_count
) q;
