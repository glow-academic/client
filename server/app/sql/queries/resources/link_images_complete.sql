-- Link images resource (tool call tracking for non-creatable resources)
-- Records a tool call linking an existing images resource to a run/call chain
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_link_images_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_link_images_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_link_images_v4(
...
BEGIN
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (api_link_images_v4.resources_id, v_call_id);
    END IF;

    RETURN QUERY SELECT api_link_images_v4.resources_id;
END;
$$;
