-- Link descriptions resource (tool call tracking for selecting existing resources)
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_link_descriptions_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_link_descriptions_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_link_descriptions_v4(
...
BEGIN
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_link_descriptions_v4.group_id, NOW(), NOW());
        ...
        VALUES (api_link_descriptions_v4.tool_id, v_call_id);

        INSERT INTO descriptions_calls_connection (descriptions_id, call_id)
        VALUES (api_link_descriptions_v4.resources_id, v_call_id);
    END IF;

    RETURN QUERY SELECT api_link_descriptions_v4.resources_id;
END;
$$;
