...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_create_run_positions_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_run_positions_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_create_run_positions_v4.group_id, NOW(), NOW());

        -- Create call record
        v_call_id := uuidv7();
        INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
        VALUES (v_call_id, 'run_positions_' || v_call_id::text, v_run_id, NOW());

        -- Link tool to call
        INSERT INTO tools_calls_connection (tools_id, call_id) VALUES (api_create_run_positions_v4.tool_id, v_call_id);

        -- Link resource to call
        INSERT INTO run_positions_calls_connection (run_positions_id, call_id)
        VALUES (v_run_positions_id, v_call_id);
    END IF;

    RETURN QUERY SELECT v_run_positions_id, v_call_id;
END;
$$;
