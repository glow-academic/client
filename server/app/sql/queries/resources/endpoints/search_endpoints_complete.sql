...
-- Returns: items (array of endpoints resources)
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_search_endpoints_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_search_endpoints_v4(%s)', r.sig);
    END LOOP;
END $$;

-- Create function
CREATE OR REPLACE FUNCTION api_search_endpoints_v4(
...
SELECT COALESCE(
    ARRAY_AGG(
        (q.id, q.base_url, q.generated)::types.q_get_endpoints_v4_item
        ORDER BY q.id
    ),
    ARRAY[]::types.q_get_endpoints_v4_item[]
) as items
FROM (
    SELECT r.id, r.base_url, COALESCE(r.generated, false) AS generated
    FROM endpoints_resource r
    WHERE r.active = true
