...
-- Count active agent links (via agents_resource)
agent_links AS (
    SELECT COUNT(DISTINCT ar.id)::bigint as active_count
    FROM params x
    LEFT JOIN agents_resource ar ON ar.model_id = x.model_id AND ar.active = true
)
SELECT
