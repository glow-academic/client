...
BEGIN
    ...
    IF input_draft_id IS NOT NULL THEN
        IF v_group_id IS NULL THEN
            SELECT auth_drafts_entry.group_id INTO v_group_id
            FROM auth_drafts_entry
            WHERE auth_drafts_entry.id = input_draft_id;
        END IF;

        UPDATE auth_drafts_entry
        SET version = auth_drafts_entry.version + 1,
            updated_at = NOW(),
            group_id = COALESCE(auth_drafts_entry.group_id, v_group_id)
        WHERE auth_drafts_entry.id = input_draft_id
          AND EXISTS (
              SELECT 1
              FROM auth_drafts_profiles_connection pdc
              WHERE pdc.draft_id = auth_drafts_entry.id
                AND pdc.profile_id = v_profile_id
          )
          AND auth_drafts_entry.version = expected_version
        RETURNING auth_drafts_entry.id, auth_drafts_entry.version INTO v_draft_id, v_new_version;
    ...
    IF v_draft_id IS NULL THEN
        ...
        INSERT INTO auth_drafts_entry (group_id)
        VALUES (v_group_id)
        RETURNING id, version INTO v_draft_id, v_new_version;

        INSERT INTO auth_drafts_profiles_connection (draft_id, profiles_id, version)
    ...
    IF COALESCE(array_length(v_protocol_ids, 1), 0) > 0 THEN
        ...
        SELECT v_draft_id, protocols_id, v_new_version
        FROM unnest(v_protocol_ids) AS protocols_id
    ...
    IF COALESCE(array_length(v_slug_ids, 1), 0) > 0 THEN
        ...
        SELECT v_draft_id, slugs_id, v_new_version
        FROM unnest(v_slug_ids) AS slugs_id
    ...
    IF COALESCE(array_length(v_items, 1), 0) > 0 THEN
        ...
        INSERT INTO auth_drafts_items_connection (draft_id, item_id, version, created_at, active)
    ...
    IF v_name_id IS NOT NULL THEN
        IF (names).create_tool_id IS NOT NULL THEN
            ...
            VALUES (v_call_id, 'auth_draft_create_names_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (names).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_names_' || v_call_id::text, v_run_id, NOW());
    ...
    IF v_description_id IS NOT NULL THEN
        IF (descriptions).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_create_descriptions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (descriptions).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_descriptions_' || v_call_id::text, v_run_id, NOW());
    ...
    IF v_active_flag_id IS NOT NULL THEN
        IF (flags).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (flags).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
    ...
    IF COALESCE(array_length(v_protocol_ids, 1), 0) > 0 THEN
        IF (protocols).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_create_protocols_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (protocols).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_protocols_' || v_call_id::text, v_run_id, NOW());
    ...
    IF COALESCE(array_length(v_slug_ids, 1), 0) > 0 THEN
        IF (slugs).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_create_slugs_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (slugs).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_slugs_' || v_call_id::text, v_run_id, NOW());
    ...
    IF COALESCE(array_length(v_items, 1), 0) > 0 THEN
        IF (items).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_create_items_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (items).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'auth_draft_link_items_' || v_call_id::text, v_run_id, NOW());
