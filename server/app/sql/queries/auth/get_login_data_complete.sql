...
dept_settings AS (
    SELECT DISTINCT s.id as settings_id, EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'guest_login_enabled' AND f.value = TRUE) as guest_login_enabled
...
default_settings AS (
    SELECT s.id as settings_id, EXISTS (SELECT 1 FROM setting_flags_junction sf JOIN flags_resource f ON sf.flags_id = f.id WHERE sf.setting_id = s.id AND f.name = 'guest_login_enabled' AND f.value = TRUE) as guest_login_enabled
    FROM setting_artifact s
...
dept_auths AS (
    ...
    JOIN setting_auths_junction sa ON sa.auths_id = a.id AND sa.active = true
    JOIN dept_settings ds ON ds.settings_id = sa.setting_id
    WHERE EXISTS (SELECT 1 FROM auth_flags_junction af JOIN flags_resource f ON af.flags_id = f.id WHERE af.auth_id = a.id AND f.name = 'auth_active' AND f.value = true)
),
...
default_auths AS (
    ...
    FROM auths_resource a
    JOIN setting_auths_junction sa ON sa.auths_id = a.id AND sa.active = true
    JOIN default_settings ds ON ds.settings_id = sa.setting_id
    WHERE EXISTS (SELECT 1 FROM auth_flags_junction af JOIN flags_resource f ON af.flags_id = f.id WHERE af.auth_id = a.id AND f.name = 'auth_active' AND f.value = true)
),
