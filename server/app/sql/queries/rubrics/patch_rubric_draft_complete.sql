        ...
        IF v_draft_id IS NOT NULL THEN
            ...
            IF v_group_id IS NOT NULL AND (
                ...
                IF v_name_id IS NOT NULL AND (names).create_tool_id IS NOT NULL THEN
                    ...
                    VALUES (v_call_id, 'rubric_draft_create_names_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_name_id IS NOT NULL AND (names).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_names_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_description_id IS NOT NULL AND (descriptions).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_descriptions_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_description_id IS NOT NULL AND (descriptions).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_descriptions_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_active_flag_id IS NOT NULL AND (flags).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_active_flag_id IS NOT NULL AND (flags).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_department_ids, 1), 0) > 0 AND (departments).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_departments_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_department_ids, 1), 0) > 0 AND (departments).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_departments_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_total_points_id IS NOT NULL AND (points).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_points_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_total_points_id IS NOT NULL AND (points).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_points_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_pass_points_id IS NOT NULL AND (pass_points).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_pass_points_' || v_call_id::text, v_run_id, NOW());
                ...
                IF v_pass_points_id IS NOT NULL AND (pass_points).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_pass_points_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_standard_group_ids, 1), 0) > 0 AND (standard_groups).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_standard_groups_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_standard_group_ids, 1), 0) > 0 AND (standard_groups).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_standard_groups_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_standard_ids, 1), 0) > 0 AND (standards).create_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_create_standards_' || v_call_id::text, v_run_id, NOW());
                ...
                IF COALESCE(array_length(v_standard_ids, 1), 0) > 0 AND (standards).link_tool_id IS NOT NULL THEN
                    v_call_id := uuidv7();
                    INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                    VALUES (v_call_id, 'rubric_draft_link_standards_' || v_call_id::text, v_run_id, NOW());
