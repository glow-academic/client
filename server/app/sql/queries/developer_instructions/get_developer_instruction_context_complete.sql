...
scenario_parameters_data AS (
                ...
                COALESCE(
                    jsonb_agg(
                        jsonb_build_object(
                            ...
                            'active', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'parameter_active' AND f.value = TRUE),
                            'document_parameter', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'document_parameter' AND f.value = TRUE),
                            'persona_parameter', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'persona_parameter' AND f.value = TRUE),
                            'scenario_parameter', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'scenario_parameter' AND f.value = TRUE),
                            'video_parameter', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'video_parameter' AND f.value = TRUE),
                            'simulation_parameter', EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'simulation_parameter' AND f.value = TRUE),
    ...
    WHERE p_scenario_id IS NOT NULL
      AND EXISTS (SELECT 1 FROM parameter_flags_junction pf JOIN flags_resource f ON pf.flags_id = f.id WHERE pf.parameter_id = p.id AND f.name = 'parameter_active' AND f.value = true)
...
scenario_fields_data AS (
                ...
                COALESCE(
                    jsonb_agg(
                        jsonb_build_object(
                            ...
                            'active', EXISTS (SELECT 1 FROM field_flags_junction ff JOIN flags_resource fl ON ff.flags_id = fl.id WHERE ff.field_id = f.id AND fl.name = 'field_active' AND fl.value = TRUE),
    ...
    WHERE (p_scenario_id IS NOT NULL AND spf.scenario_id = p_scenario_id)
      AND spf.active = true
      AND EXISTS (SELECT 1 FROM field_flags_junction ff JOIN flags_resource fl ON ff.flags_id = fl.id WHERE ff.field_id = f.id AND fl.name = 'field_active' AND fl.value = true)
      AND EXISTS (SELECT 1 FROM agent_has_scenario_artifact WHERE has_scenario = true)
),
                ...
                COALESCE(
                    jsonb_agg(
                        jsonb_build_object(
                            ...
                            'active', EXISTS (SELECT 1 FROM document_flags_junction df JOIN flags_resource f ON df.flags_id = f.id WHERE df.document_id = d.id AND f.name = 'document_active' AND f.value = TRUE),
