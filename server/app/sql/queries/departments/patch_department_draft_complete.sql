...
BEGIN
    ...
    IF input_draft_id IS NOT NULL THEN
        IF v_group_id IS NULL THEN
            SELECT department_drafts_entry.group_id INTO v_group_id
            FROM department_drafts_entry
            WHERE department_drafts_entry.id = input_draft_id;
        END IF;

        UPDATE department_drafts_entry
        SET version = department_drafts_entry.version + 1,
            updated_at = NOW(),
            group_id = COALESCE(department_drafts_entry.group_id, v_group_id)
        WHERE department_drafts_entry.id = input_draft_id
          AND EXISTS (
              SELECT 1
    ...
    IF v_draft_id IS NULL THEN
        ...
        INSERT INTO department_drafts_entry (group_id)
        VALUES (v_group_id)
        RETURNING id, version INTO v_draft_id, v_new_version;

        INSERT INTO department_drafts_profiles_connection (draft_id, profiles_id, version)
    ...
    IF v_run_id IS NOT NULL AND v_name_id IS NOT NULL THEN
        IF (names).create_tool_id IS NOT NULL THEN
            ...
            VALUES (v_call_id, 'department_draft_create_names_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (names).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_link_names_' || v_call_id::text, v_run_id, NOW());
    ...
    IF v_run_id IS NOT NULL AND v_description_id IS NOT NULL THEN
        IF (descriptions).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_create_descriptions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (descriptions).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_link_descriptions_' || v_call_id::text, v_run_id, NOW());
    ...
    IF v_run_id IS NOT NULL AND v_active_flag_id IS NOT NULL THEN
        IF (flags).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (flags).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
    ...
    IF v_run_id IS NOT NULL AND COALESCE(array_length(v_settings_ids, 1), 0) > 0 THEN
        IF (settings).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_create_settings_' || v_call_id::text, v_run_id, NOW());
        ...
        IF (settings).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
            VALUES (v_call_id, 'department_draft_link_settings_' || v_call_id::text, v_run_id, NOW());
