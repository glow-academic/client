...
AS $$
    ...
    actor_info AS (
        SELECT 
            COALESCE(
                (SELECT n.name FROM profile_names_junction pn JOIN names_resource n ON pn.names_id = n.id WHERE pn.profile_id = profile_artifact.id LIMIT 1) || ' ' ||
                (SELECT n2.name FROM profile_names_junction pn2 JOIN names_resource n2 ON pn2.names_id = n2.id WHERE pn2.profile_id = profile_artifact.id LIMIT 1),
                ''
            ) as actor_name
