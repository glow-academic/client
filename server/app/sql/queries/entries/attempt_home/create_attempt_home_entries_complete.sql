...
BEGIN
    ...
    VALUES (api_create_attempt_home_entry_v4.session_id, true, api_create_attempt_home_entry_v4.mcp)
    RETURNING texts_entry.id INTO v_text_id;

    -- Link upload to text entry
    IF api_create_attempt_home_entry_v4.upload_id IS NOT NULL THEN
        INSERT INTO text_uploads_entry (text_id, upload_id)
        VALUES (v_text_id, api_create_attempt_home_entry_v4.upload_id);
    END IF;
    ...
    VALUES (api_create_attempt_home_entry_v4.run_id, 'assistant', true, api_create_attempt_home_entry_v4.mcp)
    ...
    IF api_create_attempt_home_entry_v4.upload_id IS NOT NULL THEN
        ...
        VALUES (v_message_id, api_create_attempt_home_entry_v4.upload_id);
    END IF;
