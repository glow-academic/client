-- Get simulation_drafts entries by IDs from simulation_drafts_entry
...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_simulation_drafts_entries_v4_item%'
...
CREATE TYPE types.q_get_simulation_drafts_entries_v4_item AS (
    draft_id uuid,
...
RETURNS TABLE (
    items types.q_get_simulation_drafts_entries_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
    WITH mv_data AS (
        SELECT mv.*
        FROM simulation_drafts_entry mv
    ...
    items_agg AS (
        SELECT COALESCE(
            ARRAY_AGG(
                ...
                )::types.q_get_simulation_drafts_entries_v4_item
                ORDER BY updated_at DESC
            ),
            ARRAY[]::types.q_get_simulation_drafts_entries_v4_item[]
        ) AS items
        FROM mv_data
    )
