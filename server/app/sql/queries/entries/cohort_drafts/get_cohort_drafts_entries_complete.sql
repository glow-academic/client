-- Get cohort_drafts entries by IDs from cohort_drafts_entry
...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_cohort_drafts_entries_v4_item%'
...
CREATE TYPE types.q_get_cohort_drafts_entries_v4_item AS (
    draft_id uuid,
...
RETURNS TABLE (
    items types.q_get_cohort_drafts_entries_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
    ...
    items_agg AS (
        SELECT COALESCE(
            ...
            ARRAY[]::types.q_get_cohort_drafts_entries_v4_item[]
        ) AS items
        FROM mv_data
    )
