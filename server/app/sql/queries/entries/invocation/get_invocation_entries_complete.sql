-- Get invocation entries by IDs from invocation_mv
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_get_invocation_entries_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_invocation_entries_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION public.api_get_invocation_entries_v4(
    ids uuid[]
) RETURNS TABLE(
    items jsonb
)
...
BEGIN
    ...
    WHERE m.invocation_entry_id = ANY(ids);
END;
$$;
