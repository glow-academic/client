-- Create test_invocation entry with strongly-typed params
...
BEGIN
    ...
    IF api_create_test_invocation_entry_v4.upload_id IS NOT NULL THEN
        INSERT INTO text_uploads_entry (text_id, upload_id)
        VALUES (v_text_id, api_create_test_invocation_entry_v4.upload_id);
    END IF;

    -- 2. Create call record
    v_call_id := uuidv7();
    INSERT INTO calls_entry (id, run_id, external_call_id)
    VALUES (v_call_id, api_create_test_invocation_entry_v4.run_id, 'test_invocation_' || v_call_id::text);

    -- 3. Link tool to call
    IF api_create_test_invocation_entry_v4.tool_id IS NOT NULL THEN
        INSERT INTO tools_calls_connection (tools_id, call_id)
        VALUES (api_create_test_invocation_entry_v4.tool_id, v_call_id);
    END IF;
    ...
    VALUES (api_create_test_invocation_entry_v4.run_id, 'assistant', true, api_create_test_invocation_entry_v4.mcp)
    ...
    IF api_create_test_invocation_entry_v4.upload_id IS NOT NULL THEN
        ...
        VALUES (v_message_id, api_create_test_invocation_entry_v4.upload_id);
    END IF;
