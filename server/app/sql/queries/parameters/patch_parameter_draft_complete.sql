...
BEGIN
    ...
    IF input_draft_id IS NOT NULL THEN
        ...
        FROM parameter_drafts_entry vde
        ...
        UPDATE parameter_drafts_entry
        SET version = parameter_drafts_entry.version + 1,
            updated_at = NOW(),
            group_id = COALESCE(parameter_drafts_entry.group_id, v_group_id)
        WHERE id = input_draft_id
          AND EXISTS (
              SELECT 1
              FROM parameter_drafts_profiles_connection pdc
              WHERE pdc.draft_id = parameter_drafts_entry.id
                AND pdc.profiles_id = v_profiles_resource_id
          )
          AND parameter_drafts_entry.version = expected_version
    ...
    IF v_draft_id IS NULL THEN
        ...
        INSERT INTO parameter_drafts_entry (group_id)
        VALUES (v_group_id)
        RETURNING id, version INTO v_draft_id, v_new_version;

        INSERT INTO parameter_drafts_profiles_connection (draft_id, profiles_id, version)
        VALUES (v_draft_id, v_profiles_resource_id, v_new_version);
    END IF;

    -- Replace draft links
    DELETE FROM parameter_drafts_names_connection WHERE draft_id = v_draft_id;
    DELETE FROM parameter_drafts_descriptions_connection WHERE draft_id = v_draft_id;
    DELETE FROM parameter_drafts_flags_connection WHERE draft_id = v_draft_id;
    DELETE FROM parameter_drafts_departments_connection WHERE draft_id = v_draft_id;
    DELETE FROM parameter_drafts_fields_connection WHERE draft_id = v_draft_id;

    IF v_name_id IS NOT NULL THEN
        INSERT INTO parameter_drafts_names_connection (draft_id, names_id, version)
    ...
    IF v_description_id IS NOT NULL THEN
        INSERT INTO parameter_drafts_descriptions_connection (draft_id, descriptions_id, version)
    ...
    INSERT INTO parameter_drafts_flags_connection (draft_id, flags_id, version)
    ...
    INSERT INTO parameter_drafts_departments_connection (draft_id, departments_id, version)
    ...
    INSERT INTO parameter_drafts_fields_connection (draft_id, fields_id, version)
    SELECT v_draft_id, fid, v_new_version
    ...
    IF v_group_id IS NOT NULL THEN
        ...
        IF v_name_id IS NOT NULL THEN
            IF (names).create_tool_id IS NOT NULL THEN
                ...
                VALUES (v_call_id, 'parameter_draft_create_names_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (names).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_link_names_' || v_call_id::text, v_run_id, NOW());
        ...
        IF v_description_id IS NOT NULL THEN
            IF (descriptions).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_create_descriptions_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (descriptions).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_link_descriptions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_flag_ids, 1), 0) > 0 THEN
            IF (flags).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (flags).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_department_ids, 1), 0) > 0 THEN
            IF (departments).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_create_departments_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (departments).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_link_departments_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(v_field_ids, 1), 0) > 0 THEN
            IF (fields).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_create_fields_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (fields).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'parameter_draft_link_fields_' || v_call_id::text, v_run_id, NOW());
