-- Patch eval draft (create if not exists, patch if exists)
...
BEGIN
    ...
    WITH defaults AS (
        SELECT jsonb_build_object(
            ...
            'model_run_ids', jsonb_build_array(),
            'group_ids', jsonb_build_array(),
            'rubric_grade_agent_pairs', jsonb_build_array(),
            'rubric_grade_agent_active_states', jsonb_build_object(),
            'rubric_grade_agent_positions', jsonb_build_array()
        ) AS d
    ),
    payload AS (
    ...
    SELECT 'evals'::draft_resource_type, p_profile_id, p
    FROM payload, params
