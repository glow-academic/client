...
rubrics_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (r.id, r.name, COALESCE(r.description, ''), r.points, r.pass_points
...
departments_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (d.id, d.title, COALESCE(d.description, '')
    ...
    WHERE d.active = true AND d.id IS NOT NULL
),
...
agents_array AS (
    ...
    WHERE a.active = true AND a.id IS NOT NULL
),
