...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_benchmark_bundle_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_benchmark_bundle_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_benchmark_bundle_v4_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_benchmark_bundle_v4_eval AS (
    eval_id uuid,
    name text,
    description text,
    rubric_id uuid,
...
CREATE TYPE types.q_get_benchmark_bundle_v4_eval_attempt AS (
    attempt_id uuid,
    eval_id uuid,
    eval_name text,
...
CREATE TYPE types.q_get_benchmark_bundle_v4_rubric AS (
    rubric_id uuid,
    name text,
    description text,
    points int,
...
CREATE TYPE types.q_get_benchmark_bundle_v4_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_benchmark_bundle_v4_agent AS (
    agent_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_benchmark_bundle_v4_standard_group AS (
...
CREATE TYPE types.q_get_benchmark_bundle_v4_standard AS (
    standard_id uuid,
    name text,
    description text,
    points int
...
CREATE TYPE types.q_get_benchmark_bundle_v4_rubric_standard_group AS (
    rubric_id uuid,
    standard_group_id uuid,
    standard_ids uuid[]
);

CREATE TYPE types.q_get_benchmark_bundle_v4_rubric_option AS (
    value text,
    label text
);

CREATE TYPE types.q_get_benchmark_bundle_v4_department_option AS (
    value text,
    label text
);

CREATE TYPE types.q_get_benchmark_bundle_v4_agent_option AS (
    value text,
    label text
);

-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_benchmark_bundle_v4(
    profile_id uuid,
...
RETURNS TABLE (
    actor_name text,
    evals types.q_get_benchmark_bundle_v4_eval[],
    attempts types.q_get_benchmark_bundle_v4_eval_attempt[],
    rubrics types.q_get_benchmark_bundle_v4_rubric[],
    departments types.q_get_benchmark_bundle_v4_department[],
    agents types.q_get_benchmark_bundle_v4_agent[],
    standard_groups types.q_get_benchmark_bundle_v4_standard_group[],
    standards types.q_get_benchmark_bundle_v4_standard[],
    rubric_standard_groups types.q_get_benchmark_bundle_v4_rubric_standard_group[],
    rubric_options types.q_get_benchmark_bundle_v4_rubric_option[],
    department_options types.q_get_benchmark_bundle_v4_department_option[],
    agent_options types.q_get_benchmark_bundle_v4_agent_option[],
    total_count bigint,
    page int,
    page_size int,
    total_pages int
...
attempts_with_eval AS (
    ...
    WHERE 
        ...
        (p.eval_ids IS NULL OR p.eval_ids = ARRAY[]::uuid[] OR ea.eval_id = ANY(p.eval_ids))
        -- Filter by archived if provided
        AND (p.archived IS NULL OR ea.archived = p.archived)
),
-- Get eval departments for access control (for attempts)
...
filtered_attempts AS (
    ...
    CROSS JOIN params p
    WHERE 
        (p.status IS NULL OR p.status = '' OR aas.status = p.status)
        ...
        AND (
            NOT EXISTS (SELECT 1 FROM attempt_eval_departments aed WHERE aed.eval_id = aas.eval_id)
            OR EXISTS (
                SELECT 1 
                FROM attempt_eval_departments aed
                JOIN user_departments ud ON ud.department_id::text = ANY(aed.department_ids)
                WHERE aed.eval_id = aas.eval_id
            )
            OR EXISTS (SELECT 1 FROM user_profile WHERE role IN ('admin', 'superadmin'))
        )
),
-- Apply pagination using ROW_NUMBER() to avoid OFFSET variable issue
numbered_attempts AS (
    SELECT 
        fa.*,
        ROW_NUMBER() OVER (ORDER BY fa.attempt_created_at DESC) as rn
...
evals_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_eval
...
attempts_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_eval_attempt
            ORDER BY pa.attempt_created_at DESC
        ),
        '{}'::types.q_get_benchmark_bundle_v4_eval_attempt[]
    ) as attempts
...
rubrics_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_rubric
...
departments_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            (d.id, (SELECT n.name FROM department_names dn JOIN names n ON dn.name_id = n.id WHERE dn.department_id = d.id LIMIT 1), COALESCE((SELECT d.description FROM document_descriptions dd JOIN descriptions d ON dd.description_id = d.id WHERE dd.document_id = d.id LIMIT 1), '')
            )::types.q_get_benchmark_bundle_v4_department
    ...
    WHERE EXISTS (SELECT 1 FROM document_flags df JOIN flags fl ON df.flag_id = fl.id WHERE df.document_id = d.id AND fl.name = 'active' AND df.type = 'active'::type_document_flags AND df.value = true) AND d.id IS NOT NULL
),
...
standard_groups_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_standard_group
            ORDER BY sg.name
        ),
        '{}'::types.q_get_benchmark_bundle_v4_standard_group[]
    ) as standard_groups
...
standards_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_standard
            ORDER BY st.name
        ),
        '{}'::types.q_get_benchmark_bundle_v4_standard[]
    ) as standards
...
rubric_standard_groups_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_rubric_standard_group
...
rubric_options_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_rubric_option
            ORDER BY (SELECT n.name FROM rubric_names rn JOIN names n ON rn.name_id = n.id WHERE rn.rubric_id = r.id LIMIT 1)
        ),
        '{}'::types.q_get_benchmark_bundle_v4_rubric_option[]
    ) as rubric_options
...
department_options_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_department_option
            ORDER BY (SELECT n.name FROM department_names dn JOIN names n ON dn.name_id = n.id WHERE dn.department_id = d.id LIMIT 1)
        ),
        '{}'::types.q_get_benchmark_bundle_v4_department_option[]
...
agent_options_array AS (
    SELECT COALESCE(
        ARRAY_AGG(
            ...
            )::types.q_get_benchmark_bundle_v4_agent_option
            ORDER BY (SELECT n.name FROM agent_names an JOIN names n ON an.name_id = n.id WHERE an.agent_id = a.id LIMIT 1)
        ),
        '{}'::types.q_get_benchmark_bundle_v4_agent_option[]
    ) as agent_options
...
SELECT 
    ...
    (SELECT attempts FROM attempts_array) as attempts,
    ...
    COALESCE((SELECT count FROM total_count), 0)::bigint as total_count,
    p.page::int as page,
