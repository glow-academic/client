...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_benchmark_error_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_benchmark_error_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_benchmark_error_v4_%'
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION socket_benchmark_error_v4(
    profile_id uuid,
    error_message text,
    attempt_id uuid,
