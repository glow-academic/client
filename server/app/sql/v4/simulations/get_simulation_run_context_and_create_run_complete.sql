...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_simulation_run_context_and_create_run_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_simulation_run_context_and_create_run_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_simulation_run_context_and_create_run_v4_%'
...
CREATE TYPE types.q_get_simulation_run_context_and_create_run_v4_document AS (
    id text,
    name text,
    file_path text,
    mime_type text
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION socket_get_simulation_run_context_and_create_run_v4(
    chat_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    documents types.q_get_simulation_run_context_and_create_run_v4_document[],
    run_id text,
    group_id uuid
)
LANGUAGE sql
...
active_settings AS (
    ...
    SELECT 
        COALESCE(
            ...
            (SELECT id FROM setting_artifact s WHERE EXISTS (SELECT 1 FROM setting_flags sf JOIN flags_resource f ON sf.flag_id = f.id WHERE sf.setting_id = s.id AND f.name = 'active' AND sf.value = TRUE) LIMIT 1)  -- Last resort
        ) as settings_id
),
...
LEFT JOIN reasoning_levels_resource rl ON rl.id = mrl.reasoning_level_id AND rl.active = true
    ...
    LEFT JOIN agents_resource a_voice ON a_voice.id = NULL::uuid AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = TRUE)
...
SELECT 
    ...
    COALESCE(dd.documents, ARRAY[]::types.q_get_simulation_run_context_and_create_run_v4_document[]) as documents,
