...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_simulation_run_context_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_simulation_run_context_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_simulation_run_context_v4_%'
...
CREATE TYPE types.q_get_simulation_run_context_v4_document AS (
    id text,
    name text,
    file_path text,
    mime_type text
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION socket_get_simulation_run_context_v4(
    chat_id uuid
)
RETURNS TABLE (
    chat_id text,
    ...
    documents types.q_get_simulation_run_context_v4_document[]
)
LANGUAGE sql
VOLATILE
AS $$
...
SELECT 
    ...
    COALESCE(
        (SELECT ARRAY_AGG(
            (dd.document_id::text, dd.name, dd.file_path, dd.mime_type)::types.q_get_simulation_run_context_v4_document
            ORDER BY dd.document_id
        ) FROM document_data dd WHERE dd.chat_id::text = cd.chat_id),
        '{}'::types.q_get_simulation_run_context_v4_document[]
    ) as documents
