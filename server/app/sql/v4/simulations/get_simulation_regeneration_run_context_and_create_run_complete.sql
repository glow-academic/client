...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_sim_regen_run_context_create_run_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_sim_regen_run_context_create_run_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_sim_regen_run_context_create_run_v4_%'
...
CREATE TYPE types.q_get_sim_regen_run_context_create_run_v4_document AS (
    id text,
    name text,
    file_path text,
    mime_type text
...
CREATE TYPE types.q_get_sim_regen_run_context_create_run_v4_msg AS (
    role text,
    content text
);
...
CREATE OR REPLACE FUNCTION socket_get_sim_regen_run_context_create_run_v4(
    chat_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    documents types.q_get_sim_regen_run_context_create_run_v4_document[],
    run_id text,
    group_id uuid,
    previous_messages types.q_get_sim_regen_run_context_create_run_v4_msg[]
)
LANGUAGE sql
VOLATILE
AS $$
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.q_get_sim_regen_run_context_create_run_v4_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.q_get_sim_regen_run_context_create_run_v4_msg[]
...
context_data AS (
    ...
    LEFT JOIN LATERAL (
        SELECT sp.persona_id, (SELECT n.name FROM persona_names pn JOIN names_resource n ON pn.name_id = n.id WHERE pn.persona_id = sp.persona_id LIMIT 1) as persona_name
...
LEFT JOIN reasoning_levels_resource rl ON rl.id = mrl.reasoning_level_id AND rl.active = true
    ...
    LEFT JOIN agents_resource a_voice ON a_voice.id = adom_voice.agent_id 
    ...
    INNER JOIN models_resource m_voice ON m_voice.id = am_voice.model_id
...
documents_data AS (
    ...
    SELECT 
        sc.id as chat_id,
        ARRAY_AGG(
            (d.id::text, (SELECT n.name FROM document_names dn JOIN names_resource n ON dn.name_id = n.id WHERE dn.document_id = d.id LIMIT 1), u.file_path, u.mime_type)::types.q_get_sim_regen_run_context_create_run_v4_document
            ORDER BY d.id
...
SELECT 
    ...
    COALESCE(dd.documents, ARRAY[]::types.q_get_sim_regen_run_context_create_run_v4_document[]) as documents,
