...
settings_auths_with_items AS (
    ...
    SELECT 
        ...
        EXISTS (SELECT 1 FROM agent_flags af WHERE af.agent_id = a.id AND af.type = 'active'::type_agent_flags AND af.value = TRUE) AS active,
        COALESCE(
            ARRAY_AGG(
...
SELECT 
    s.id as settings_id,
    s.created_at,
    EXISTS (SELECT 1 FROM setting_flags sf WHERE sf.setting_id = s.id AND sf.type = 'active'::type_setting_flags AND sf.value = TRUE),
