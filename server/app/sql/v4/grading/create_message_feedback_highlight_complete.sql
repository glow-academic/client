...
-- Note: message_feedback_id now REFERENCES strengths_resource(id)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_create_message_feedback_highlight_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_create_message_feedback_highlight_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_create_message_feedback_highlight_v4_%'
...
CREATE TYPE types.i_create_message_feedback_highlight_v4_highlight AS (
    section text
);
