...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_grading_regeneration_run_context_and_create_run_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_grading_regeneration_run_context_and_create_run_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_grading_regen_run_context_create_run_v4_%'
...
DROP TYPE IF EXISTS types.q_get_grading_regen_run_context_create_run_v4_standard_group CASCADE;
CREATE TYPE types.q_get_grading_regen_run_context_create_run_v4_standard_group AS (
    id text,
    name text,
    short_name text,
    description text,
...
DROP TYPE IF EXISTS types.q_get_grading_regen_run_context_create_run_v4_standard CASCADE;
CREATE TYPE types.q_get_grading_regen_run_context_create_run_v4_standard AS (
    id text,
    name text,
    description text,
    points integer,
...
CREATE TYPE types.q_get_grading_regen_run_context_create_run_v4_msg AS (
    role text,
    content text
);
...
CREATE OR REPLACE FUNCTION socket_get_grading_regeneration_run_context_and_create_run_v4(
    chat_id uuid,
    department_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    standard_groups types.q_get_grading_regen_run_context_create_run_v4_standard_group[],
    standards types.q_get_grading_regen_run_context_create_run_v4_standard[],
    -- Previous messages (from all previous runs in group)
    previous_messages types.q_get_grading_regen_run_context_create_run_v4_msg[],
    -- Agent data
    agent_id text,
    agent_name text,
    system_prompt text,
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.q_get_grading_regen_run_context_create_run_v4_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.q_get_grading_regen_run_context_create_run_v4_msg[]
...
standard_groups_aggregated AS (
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (sgd.id::text, sgd.name, sgd.short_name, COALESCE(sgd.description, ''), sgd.points, sgd.pass_points, sgd.rubric_id::text)::types.q_get_grading_regen_run_context_create_run_v4_standard_group
                ORDER BY sgd.name
            ),
            ARRAY[]::types.q_get_grading_regen_run_context_create_run_v4_standard_group[]
        ) as standard_groups
...
standards_aggregated AS (
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (std.id::text, std.name, COALESCE(std.description, ''), std.points, std.standard_group_id::text)::types.q_get_grading_regen_run_context_create_run_v4_standard
                ORDER BY std.name
            ),
            ARRAY[]::types.q_get_grading_regen_run_context_create_run_v4_standard[]
        ) as standards
