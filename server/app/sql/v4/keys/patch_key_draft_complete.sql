-- Patch key draft (create if not exists, patch if exists)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_patch_key_draft_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_patch_key_draft_v4(%s)', r.sig);
    END LOOP;
END $$;
...
CREATE OR REPLACE FUNCTION api_patch_key_draft_v4(
    profile_id uuid,
    patch text,
...
BEGIN
    ...
    SELECT 'keys'::draft_resource_type, p_profile_id, p
    FROM payload, params
