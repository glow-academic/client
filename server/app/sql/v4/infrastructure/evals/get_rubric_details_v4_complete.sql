...
AS $$
    SELECT r.id, 
           ...
           (SELECT p.value FROM rubric_points rp JOIN points_resource p ON p.id = rp.point_id WHERE rp.rubric_id = r.id AND rp.type = 'total'::point_type LIMIT 1)::integer as points,
           (SELECT p.value FROM rubric_points rp JOIN points_resource p ON p.id = rp.point_id WHERE rp.rubric_id = r.id AND rp.type = 'pass'::point_type LIMIT 1)::integer as pass_points
