...
image_model_check AS (
    ...
    WHERE mm.model_id = (SELECT model_id FROM params) AND mr.modality = 'image' AND mm.type = 'output'::type_model_modalities AND mm.active = true
),
...
providers_data AS (
    -- Get all providers FROM providers_resource resource table
    SELECT DISTINCT
        p.id as provider_id,
        n.name as name,
...
model_pricing_data AS (
    ...
    WHERE mp.model_id = (SELECT model_id FROM params) AND mp.active = true AND pr.active = true AND u.active = true
...
model_modalities_data AS (
    ...
    FROM model_modalities mm
    JOIN modalities_resource mr ON mr.id = mm.modality_id
    WHERE mm.model_id = (SELECT model_id FROM params) AND mm.active = true AND mr.active = true
),
...
model_qualities_data AS (
    ...
    WHERE mq.model_id = (SELECT model_id FROM params) AND mq.active = true AND qr.active = true
),
