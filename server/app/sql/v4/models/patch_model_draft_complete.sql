-- Patch model draft (create if not exists, patch if exists)
...
BEGIN
    ...
    WITH defaults AS (
        SELECT jsonb_build_object(
            ...
            'description', '',
            'active', true,
            'provider_id', NULL::text,
            'value', '',
            'department_ids', jsonb_build_array(),
            'custom_model', false,
            'base_url', '',
            'enable_modalities', true,
            'enable_temperature', false,
            'enable_pricing', false,
            'enable_voices', false,
            'enable_reasoning_levels', false,
            'enable_qualities', false,
            'temperature_bounds', jsonb_build_object(
                'type', 'range',
                'lower', 0.0,
                'upper', 1.0
            ),
            'pricing', jsonb_build_object(),
            'selected_pricing_types', jsonb_build_array(),
            'modalities', jsonb_build_object(
                'input', jsonb_build_array('text'),
                'output', jsonb_build_array('text')
            ),
            'reasoning_levels', jsonb_build_array(),
    ...
    SELECT 'models'::draft_resource_type, p_profile_id, p
    FROM payload, params
