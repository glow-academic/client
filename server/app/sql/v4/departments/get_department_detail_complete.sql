...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_department_detail_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_department_detail_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_department_detail_v4_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_department_detail_v4_setting AS (
    settings_id uuid,
...
CREATE TYPE types.q_get_department_detail_v4_cohort AS (
    cohort_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_department_detail_v4_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_department_detail_v4_model AS (
    model_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_department_detail_v4_key AS (
    key_id uuid,
    name text,
    description text,
    key_masked text,
...
CREATE TYPE types.q_get_department_detail_v4_model_key AS (
    model_id uuid,
    key_id uuid
);
...
RETURNS TABLE (
    ...
    valid_key_ids uuid[],
    actor_name text,
    settings types.q_get_department_detail_v4_setting[],
    cohorts types.q_get_department_detail_v4_cohort[],
...
SELECT 
    ...
    COALESCE(
        ...
        (SELECT n.name FROM department_names dn JOIN names n ON dn.name_id = n.id WHERE dn.department_id = d.id LIMIT 1)::text
    ) as title,
    COALESCE(
    ...
    COALESCE(
        ...
        EXISTS (SELECT 1 FROM department_flags df JOIN flags fl ON df.flag_id = fl.id WHERE df.department_id = d.id AND fl.name = 'active' AND df.type = 'active'::type_department_flags AND df.value = TRUE)::boolean
    ) as active,
    CASE 
    ...
    COALESCE(
        ARRAY_AGG(
            (sd.settings_id, sd.created_at, sd.active, sd.department_ids)::types.q_get_department_detail_v4_setting
            ORDER BY sd.created_at DESC
        ) FILTER (WHERE uda.has_access = true AND sd.settings_id IS NOT NULL),
        '{}'::types.q_get_department_detail_v4_setting[]
    ) as settings,
    COALESCE(
        ARRAY_AGG(
            (cd.cohort_id, cd.name, cd.description)::types.q_get_department_detail_v4_cohort
            ORDER BY cd.name
        ) FILTER (WHERE uda.has_access = true AND cd.cohort_id IS NOT NULL),
        '{}'::types.q_get_department_detail_v4_cohort[]
    ) as cohorts,
    COALESCE(
        ARRAY_AGG(
            (dd.department_id, dd.name, dd.description)::types.q_get_department_detail_v4_department
            ORDER BY dd.name
        ) FILTER (WHERE uda.has_access = true AND dd.department_id IS NOT NULL),
        '{}'::types.q_get_department_detail_v4_department[]
    ) as departments,
    COALESCE(
        ARRAY_AGG(
            (dm.model_id, dm.name, dm.description)::types.q_get_department_detail_v4_model
            ORDER BY dm.name
        ) FILTER (WHERE uda.has_access = true AND dm.model_id IS NOT NULL),
        '{}'::types.q_get_department_detail_v4_model[]
    ) as models,
    COALESCE(
        ARRAY_AGG(
            (kd.key_id, kd.name, kd.description, kd.key_masked, kd.active)::types.q_get_department_detail_v4_key
            ORDER BY kd.name
        ) FILTER (WHERE uda.has_access = true AND kd.key_id IS NOT NULL),
        '{}'::types.q_get_department_detail_v4_key[]
    ) as keys,
    COALESCE(
        ARRAY_AGG(
            (mka.model_id, mka.key_id)::types.q_get_department_detail_v4_model_key
