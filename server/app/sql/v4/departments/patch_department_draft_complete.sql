...
BEGIN
    ...
    IF input_draft_id IS NOT NULL THEN
        ...
        IF v_draft_id IS NOT NULL THEN
            ...
            DELETE FROM draft_settings WHERE draft_settings.draft_id = v_draft_id;
            ...
            IF setting_ids IS NOT NULL THEN
                DELETE FROM draft_settings WHERE draft_settings.draft_id = v_draft_id;
                INSERT INTO draft_settings (draft_id, settings_id, version)
                SELECT v_draft_id, setting_id, v_new_version
                FROM UNNEST(setting_ids) as setting_id
                ON CONFLICT ON CONSTRAINT draft_settings_pkey DO UPDATE
                SET version = v_new_version,
                    updated_at = now();
            END IF;
    ...
    VALUES ('department'::artifacts, v_profile_id, v_group_id)
    RETURNING id, version INTO v_draft_id, v_new_version;
