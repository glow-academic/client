-- Document agent eval start handler
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_document_eval_start_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_document_eval_start_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_document_eval_start_v4_%'
...
-- 3) Recreate function
CREATE OR REPLACE FUNCTION socket_document_eval_start_v4(
    profile_id uuid,
    test_id uuid,
    attempt_id uuid,
    eval_id uuid,
