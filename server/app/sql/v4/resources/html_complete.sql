...
CREATE OR REPLACE FUNCTION api_create_html_v4(agent_id uuid,
    group_id uuid,
    name text,
    description text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_html_v4.agent_id
      AND rt.resource = 'html'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource html', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_html_v4.agent_id 
    ...
    -- Check if html already exists (match on name)
    SELECT r.id INTO v_html_id
    FROM html_resource r
    WHERE r.name = api_create_html_v4.name
    LIMIT 1;
    ...
    -- INSERT INTO html_resource table (always insert, never update)
    INSERT INTO html_resource(name, description, active, call_id, mcp)
    ...
    VALUES (
        api_create_html_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_html_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
