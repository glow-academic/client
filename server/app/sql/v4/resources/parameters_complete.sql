...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM parameter WHERE id = v_artifact_id) THEN
    ...
    v_params_jsonb := jsonb_build_object('parameter_id', parameter_id);
    ...
    -- INSERT into parameters table (always insert, never update)
    -- Create resource with new unique id and parameter_id FK
    INSERT INTO parameters(id, parameter_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_parameters_v4.group_id, NOW(), NOW())
    ...
    SELECT 
        api_create_parameters_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_parameters_v4.group_id), -1) + 1,
        NOW(),
        NOW();
