...
CREATE OR REPLACE FUNCTION api_create_times_v4(agent_id uuid,
    group_id uuid,
    time_taken numeric,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_times_v4.agent_id
      AND rt.resource = 'times'::resources
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource times', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_times_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('time_taken', time_taken);
    ...
    -- INSERT INTO times_resource table (always insert, never update)
    INSERT INTO times_resource(time_taken, active, call_id, mcp)
    VALUES (time_taken, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_times_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_times_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
