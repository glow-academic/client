...
BEGIN
    ...
    WHERE at.agent_id = api_create_responses_v4.agent_id
      AND rt.resource = 'responses'::resources
      AND at.active = true
    ...
    v_params_jsonb := jsonb_build_object('option_id', option_id, 'question_id', question_id);
    ...
    -- INSERT INTO responses_resource table (always insert, never update)
    INSERT INTO responses_resource(option_id, question_id, active, call_id, mcp)
    ...
    VALUES (
        api_create_responses_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_responses_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
