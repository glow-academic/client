...
BEGIN
    ...
    WHERE at.agent_id = api_create_questions_v4.agent_id
      AND rt.resource = 'questions'::resources
      AND at.active = true
      AND t.active = true
    LIMIT 1;
    ...
    v_params_jsonb := jsonb_build_object('question_text', question_text, 'allow_multiple', allow_multiple, 'time_value', time_value);
    ...
    FOR v_arg_key, v_arg_value IN
        SELECT 
            ...
            CASE sf.name
                WHEN 'question_text' THEN v_params_jsonb->>'question_text'
                WHEN 'allow_multiple' THEN v_params_jsonb->>'allow_multiple'
                WHEN 'time' THEN v_params_jsonb->>'time_value'
    ...
    INSERT INTO questions(question_text, allow_multiple, time, active, call_id)
    VALUES (question_text, allow_multiple, time_value, true, v_call_id)
    ...
    VALUES (
        api_create_questions_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_questions_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
