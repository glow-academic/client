...
-- Get or create operation (returns existing ID if model_id + group_id already exists)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), model_id (uuid, required, third), mcp (boolean, optional, fourth)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_models_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_models_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_models_v4(agent_id uuid,
    group_id uuid,
    model_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided model_id as artifact_id
    v_artifact_id := api_create_models_v4.model_id;
    
    -- Validate that model artifact exists
    IF NOT EXISTS (SELECT 1 FROM model_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Model artifact % does not exist', v_artifact_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_models_v4.agent_id
      AND rt.resource = 'models'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource models', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_models_v4.agent_id 
    ...
    -- Check if models already exists (match on model_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM models_resource r
    WHERE r.model_id = v_artifact_id
      AND r.group_id = api_create_models_v4.group_id
    LIMIT 1;
    ...
    -- INSERT INTO models_resource table (always insert, never update)
    -- INSERT INTO models_resource table (always insert, never update)
    -- Create resource with new unique id and model_id FK
    INSERT INTO models_resource(id, model_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_models_v4.group_id, NOW(), NOW())
    ...
    SELECT 
        api_create_models_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_models_v4.group_id), -1) + 1,
        NOW(),
        NOW();
