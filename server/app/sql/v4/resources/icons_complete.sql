...
CREATE OR REPLACE FUNCTION api_create_icons_v4(agent_id uuid,
    group_id uuid,
    name text,
    description text,
    value numeric,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_icons_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource icons', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_icons_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('name', name, 'description', description, 'value', value);
    ...
    -- INSERT INTO icons_resource table (always insert, never update)
    INSERT INTO icons_resource(name, description, value, active, call_id, mcp)
    VALUES (name, description, value, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_icons_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_icons_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
