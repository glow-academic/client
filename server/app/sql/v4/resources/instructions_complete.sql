...
CREATE OR REPLACE FUNCTION api_create_instructions_v4(agent_id uuid,
    group_id uuid,
    template text,
...
BEGIN
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_instructions_v4.agent_id 
    ...
    INSERT INTO instructions(template, active, call_id, mcp)
    ...
    VALUES (
        api_create_instructions_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_instructions_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
