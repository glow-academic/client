-- Create groups_rubric_grade_agents resource
...
-- Returns: groups_rubric_grade_agents_id (uuid)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_groups_rubric_grade_agents_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_groups_rubric_grade_agents_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_groups_rubric_grade_agents_v4(agent_id uuid,
    group_id uuid,
    mcp boolean DEFAULT false)
RETURNS TABLE (
    groups_rubric_grade_agents_id uuid
)
LANGUAGE plpgsql
VOLATILE
AS $$
DECLARE
    v_groups_rubric_grade_agents_id uuid;
    v_call_id uuid;
    v_tool_id uuid;
    v_template_id uuid;
...
BEGIN
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource groups_rubric_grade_agents', agent_id;
    END IF;
    ...
    VALUES (
        v_call_id,
        'groups_rubric_grade_agents_' || v_call_id::text,
        v_tool_id,
        v_template_id,
        v_arguments_raw,
        true,
    ...
    -- INSERT INTO groups_rubric_grade_agents_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO groups_rubric_grade_agents_resource(active, call_id, mcp)
    VALUES (true, v_call_id, mcp)
    RETURNING id INTO v_groups_rubric_grade_agents_id;
    ...
    SELECT 
        api_create_groups_rubric_grade_agents_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_groups_rubric_grade_agents_v4.group_id), -1) + 1,
        NOW(),
        NOW();
    
    RETURN QUERY SELECT v_groups_rubric_grade_agents_id;
END;
$$;
