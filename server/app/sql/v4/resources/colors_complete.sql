...
BEGIN
    ...
    INSERT INTO colors(name, description, hex_code, active, call_id, mcp)
    ...
    VALUES (
        api_create_colors_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_colors_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
