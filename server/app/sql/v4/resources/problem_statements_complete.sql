...
BEGIN
    ...
    v_params_jsonb := jsonb_build_object('name', name, 'problem_statement', problem_statement);
    ...
    SELECT 
        group_id,
        v_run_id,
        COALESCE((SELECT MAX(idx) FROM group_runs WHERE group_id = api_create_problem_statements_v4.group_id), -1) + 1,
        NOW(),
        NOW();
