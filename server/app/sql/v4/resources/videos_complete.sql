...
BEGIN
    ...
    WHERE at.agent_id = api_create_videos_v4.agent_id
      AND rt.resource = 'videos'::resources
      AND at.active = true
    ...
    v_params_jsonb := jsonb_build_object('name', name, 'length_seconds', length_seconds, 'description', description);
    ...
    INSERT INTO videos_resource(name, length_seconds, description, active, call_id, mcp)
    ...
    VALUES (
        api_create_videos_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_videos_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
