...
BEGIN
    ...
    WHERE at.agent_id = api_create_evals_v4.agent_id
      AND rt.resource = 'evals'::resources
      AND at.active = true
      AND t.active = true
    LIMIT 1;
    ...
    v_params_jsonb := jsonb_build_object('eval_id', eval_id);
    ...
    INSERT INTO evals_resource(id, eval_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
    ...
    SELECT 
        api_create_evals_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_evals_v4.group_id), -1) + 1,
        NOW(),
        NOW();
