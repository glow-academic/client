-- Create debug_info_entry resource
...
CREATE OR REPLACE FUNCTION api_create_debug_info_v4(agent_id uuid,
    group_id uuid,
    content text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_debug_info_v4.agent_id
      AND rt.resource = 'debug_info_entry'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource debug_info_entry', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_debug_info_v4.agent_id 
    ...
    -- Check if debug_info_entry already exists (match on content)
    SELECT r.id INTO v_debug_info_id
    FROM debug_info_entry r
    ...
    -- INSERT INTO debug_info_entry table (always insert, never update)
    INSERT INTO debug_info_entry(content, active, call_id, mcp)
    ...
    VALUES (
        api_create_debug_info_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_debug_info_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
