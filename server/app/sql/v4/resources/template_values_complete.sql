...
-- Parameters: agent_id (uuid, required, first), template_id uuid, schema_field_id uuid, string_value text, number_value text, boolean_value text
...
BEGIN
    ...
    INTO v_tool_id, v_template_id_from_tool, v_schema_id
    FROM agent_tools at
    ...
    WHERE at.agent_id = api_create_template_values_v4.agent_id
    ...
    v_params_jsonb := jsonb_build_object('template_id', template_id, 'schema_field_id', schema_field_id, 'string_value', string_value, 'number_value', number_value, 'boolean_value', boolean_value);
    ...
    VALUES (
        v_call_id,
        'template_values_' || v_call_id::text,
        v_tool_id,
    ...
    -- INSERT INTO template_values_resource table (always insert, never update)
    INSERT INTO template_values_resource(template_id, schema_field_id, string_value, number_value, boolean_value, active, call_id, mcp)
    ...
    VALUES (
        api_create_template_values_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_template_values_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
