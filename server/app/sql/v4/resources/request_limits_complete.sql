...
CREATE OR REPLACE FUNCTION api_create_request_limits_v4(agent_id uuid,
    group_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_request_limits_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource request_limits', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_request_limits_v4.agent_id 
    ...
    VALUES (
        v_call_id,
        'request_limits_' || v_call_id::text,
        v_tool_id,
        v_template_id,
        v_arguments_raw,
        true,
    ...
    INSERT INTO request_limits_resource(active, call_id, mcp)
    ...
    SELECT 
        api_create_request_limits_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_request_limits_v4.group_id), -1) + 1,
        NOW(),
        NOW();
