...
BEGIN
    ...
    v_params_jsonb := jsonb_build_object('upload_id', upload_id::text);
    ...
    SELECT 
        api_create_uploads_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_uploads_v4.group_id), -1) + 1,
        NOW(),
        NOW();
