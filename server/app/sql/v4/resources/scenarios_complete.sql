...
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), scenario_id (uuid, required, third), mcp (boolean, optional, fourth)
...
CREATE OR REPLACE FUNCTION api_create_scenarios_v4(agent_id uuid,
    group_id uuid,
    scenario_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided scenario_id as artifact_id
    v_artifact_id := api_create_scenarios_v4.scenario_id;
    
    -- Validate that scenario artifact exists
    IF NOT EXISTS (SELECT 1 FROM scenario WHERE id = v_artifact_id) THEN
    ...
    WHERE at.agent_id = api_create_scenarios_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource scenarios', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_scenarios_v4.agent_id 
    ...
    -- INSERT into scenarios table (always insert, never update)
    -- Create resource with new unique id and scenario_id FK
    INSERT INTO scenarios(id, scenario_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
    ...
    SELECT 
        api_create_scenarios_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_scenarios_v4.group_id), -1) + 1,
        NOW(),
        NOW();
