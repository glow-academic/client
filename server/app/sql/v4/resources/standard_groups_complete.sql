...
BEGIN
    ...
    v_params_jsonb := jsonb_build_object('name', name, 'short_name', short_name, 'description', description, 'points', points, 'pass_points', pass_points);
    ...
    SELECT 
        group_id,
        v_run_id,
        COALESCE((SELECT MAX(idx) FROM group_runs WHERE group_id = api_create_standard_groups_v4.group_id), -1) + 1,
        NOW(),
        NOW();
