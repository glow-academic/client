...
CREATE OR REPLACE FUNCTION api_create_feedbacks_v4(agent_id uuid,
    group_id uuid,
    total numeric,
    feedback text,
    standard_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_feedbacks_v4.agent_id
      AND rt.resource = 'feedbacks'::resources
      AND at.active = true
      AND t.active = true
    LIMIT 1;
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource feedbacks', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_feedbacks_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('total', total, 'feedback', feedback, 'standard_id', standard_id);
    ...
    -- INSERT INTO feedbacks_resource table (always insert, never update)
    INSERT INTO feedbacks_resource(total, feedback, standard_id, active, call_id, mcp)
    VALUES (total, feedback, standard_id, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_feedbacks_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_feedbacks_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
