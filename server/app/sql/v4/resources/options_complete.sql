...
BEGIN
    ...
    WHERE at.agent_id = api_create_options_v4.agent_id
      AND rt.resource = 'options'::resources
      AND at.active = true
      AND t.active = true
    LIMIT 1;
    ...
    v_params_jsonb := jsonb_build_object('option_text', option_text, 'is_correct', is_correct);
    ...
    INSERT INTO options(option_text, is_correct, active, call_id)
    VALUES (option_text, is_correct, true, v_call_id)
    ...
    VALUES (
        api_create_options_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_options_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
