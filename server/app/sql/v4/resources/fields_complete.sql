...
-- Get or create operation (returns existing ID if field_id + group_id already exists)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_fields_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_fields_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_fields_v4(agent_id uuid,
    group_id uuid,
    field_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided field_id as artifact_id
    v_artifact_id := api_create_fields_v4.field_id;
    
    -- Validate that field artifact exists
    IF NOT EXISTS (SELECT 1 FROM field_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Field artifact % does not exist', v_artifact_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_fields_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource fields', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_fields_v4.agent_id 
    ...
    -- Check if fields already exists (match on field_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM fields_resource r
    WHERE r.field_id = v_artifact_id
      AND r.group_id = api_create_fields_v4.group_id
    LIMIT 1;
    ...
    -- INSERT INTO fields_resource table (always insert, never update)
    -- Create resource with new unique id and field_id FK
    INSERT INTO fields_resource(id, field_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_fields_v4.group_id, NOW(), NOW())
    ...
    SELECT 
        api_create_fields_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_fields_v4.group_id), -1) + 1,
        NOW(),
        NOW();
