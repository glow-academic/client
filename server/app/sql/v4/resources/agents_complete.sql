...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM agent WHERE id = v_artifact_id) THEN
    ...
    v_params_jsonb := jsonb_build_object('artifact_agent_id', artifact_agent_id);
    ...
    -- INSERT into agents table (always insert, never update)
    -- Create resource with new unique id and agent_id FK
    INSERT INTO agents(id, agent_id, active, generated, mcp, call_id, group_id, created_at, updated_at)
