...
CREATE OR REPLACE FUNCTION api_create_improvements_v4(agent_id uuid,
    group_id uuid,
    name text,
    description text,
    message_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_improvements_v4.agent_id
      AND rt.resource = 'improvements'::resources
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource improvements', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_improvements_v4.agent_id 
    ...
    -- Check if improvements already exists (match on name + message_id)
    SELECT r.id INTO v_improvement_id
    FROM improvements_resource r
    WHERE r.name = api_create_improvements_v4.name
      AND r.message_id = api_create_improvements_v4.message_id
    LIMIT 1;

    IF v_improvement_id IS NOT NULL THEN
    ...
    -- INSERT INTO improvements_resource table (always insert, never update)
    INSERT INTO improvements_resource(name, description, message_id, active, call_id, mcp)
    VALUES (name, description, message_id, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_improvements_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_improvements_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
