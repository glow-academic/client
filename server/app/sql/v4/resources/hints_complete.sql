...
CREATE OR REPLACE FUNCTION api_create_hints_v4(agent_id uuid,
    group_id uuid,
    hint text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_hints_v4.agent_id
      AND rt.resource = 'hints'::resources
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource hints', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_hints_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('hint', hint);
    ...
    -- INSERT INTO hints_resource table (always insert, never update)
    INSERT INTO hints_resource(hint, active, call_id, mcp)
    VALUES (hint, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_hints_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_hints_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
