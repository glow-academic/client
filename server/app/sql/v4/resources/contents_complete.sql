...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_contents_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_contents_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_contents_v4(agent_id uuid,
    group_id uuid,
    content text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_contents_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource contents', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_contents_v4.agent_id 
    ...
    -- Check if contents already exists (match on content)
    SELECT r.id INTO v_contents_id
    FROM contents_resource r
    WHERE r.content = api_create_contents_v4.content
    LIMIT 1;

    IF v_contents_id IS NOT NULL THEN
        RETURN QUERY SELECT v_contents_id;
        RETURN;
    END IF;
    ...
    RETURNING id INTO v_contents_id;
    ...
    VALUES (
        api_create_contents_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_contents_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
