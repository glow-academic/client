...
-- Parameters: agent_id (uuid, required, first), name text
...
BEGIN
    ...
    WHERE at.agent_id = api_create_templates_v4.agent_id
      AND rt.resource = 'templates'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource templates', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags_junction 
            WHERE agent_id = api_create_templates_v4.agent_id 
    ...
    -- Check if templates already exists (match on name)
    SELECT r.id INTO v_template_id
    FROM templates_resource r
    WHERE r.name = api_create_templates_v4.name
    LIMIT 1;
    ...
    VALUES (
        api_create_templates_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_templates_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
