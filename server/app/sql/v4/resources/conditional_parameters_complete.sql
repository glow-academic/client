-- Create conditional_parameters resource
...
-- Returns: conditional_parameters_id (uuid)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_conditional_parameters_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_conditional_parameters_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_conditional_parameters_v4(agent_id uuid,
    group_id uuid,
    mcp boolean DEFAULT false)
RETURNS TABLE (
    conditional_parameters_id uuid
)
LANGUAGE plpgsql
VOLATILE
AS $$
DECLARE
    v_conditional_parameters_id uuid;
    v_call_id uuid;
    v_tool_id uuid;
    v_template_id uuid;
...
BEGIN
    ...
    WHERE at.agent_id = api_create_conditional_parameters_v4.agent_id
      AND rt.resource = 'conditional_parameters'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource conditional_parameters', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_conditional_parameters_v4.agent_id 
    ...
    VALUES (
        v_call_id,
        'conditional_parameters_' || v_call_id::text,
        v_tool_id,
        v_template_id,
        v_arguments_raw,
        true,
    ...
    -- INSERT INTO conditional_parameters_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO conditional_parameters_resource(active, call_id, mcp)
    VALUES (true, v_call_id, mcp)
    RETURNING id INTO v_conditional_parameters_id;
    ...
    SELECT 
        api_create_conditional_parameters_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_conditional_parameters_v4.group_id), -1) + 1,
        NOW(),
        NOW();
    
    RETURN QUERY SELECT v_conditional_parameters_id;
END;
$$;
