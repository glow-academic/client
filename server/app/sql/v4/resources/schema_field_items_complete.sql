...
-- Parameters: agent_id (uuid, required, first), schema_field_id uuid, item_schema_id uuid
...
BEGIN
    ...
    WHERE at.agent_id = api_create_schema_field_items_v4.agent_id
    ...
    v_params_jsonb := jsonb_build_object('schema_field_id', schema_field_id, 'item_schema_id', item_schema_id);
    ...
    VALUES (
        v_call_id,
        'schema_field_items_' || v_call_id::text,
        v_tool_id,
        v_template_id,
        v_arguments_raw,
        true,
    ...
    -- INSERT INTO schema_field_items_resource table (always insert, never update)
    INSERT INTO schema_field_items_resource(schema_field_id, item_schema_id, active, call_id, mcp)
    ...
    VALUES (
        api_create_schema_field_items_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_schema_field_items_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
