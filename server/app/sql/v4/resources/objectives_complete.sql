...
CREATE OR REPLACE FUNCTION api_create_objectives_v4(agent_id uuid,
    group_id uuid,
    objective text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_objectives_v4.agent_id
      AND rt.resource = 'objectives'::resources
      AND at.active = true
      AND t.active = true
    LIMIT 1;
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource objectives', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_objectives_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('objective', objective);
    ...
    -- INSERT INTO objectives_resource table (always insert, never update)
    INSERT INTO objectives_resource(objective, active, call_id, mcp)
    VALUES (objective, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_objectives_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_objectives_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
