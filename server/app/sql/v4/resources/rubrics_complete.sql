-- Create rubrics resource
-- Get or create operation (returns existing ID if rubric_id + group_id already exists)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), rubric_id (uuid, required, third), mcp (boolean, optional, fourth)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_rubrics_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_rubrics_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_rubrics_v4(agent_id uuid,
    group_id uuid,
    rubric_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided rubric_id as artifact_id
    v_artifact_id := api_create_rubrics_v4.rubric_id;
    
    -- Validate that rubric artifact exists
    IF NOT EXISTS (SELECT 1 FROM rubric_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Rubric artifact % does not exist', v_artifact_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_rubrics_v4.agent_id
      AND rt.resource = 'rubrics'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource rubrics', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_rubrics_v4.agent_id 
    ...
    -- Check if rubrics already exists (match on rubric_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM rubrics_resource r
    WHERE r.rubric_id = v_artifact_id
      AND r.group_id = api_create_rubrics_v4.group_id
    LIMIT 1;
    ...
    SELECT 
        api_create_rubrics_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_rubrics_v4.group_id), -1) + 1,
        NOW(),
        NOW();
