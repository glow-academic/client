...
CREATE OR REPLACE FUNCTION api_create_flags_v4(agent_id uuid,
    group_id uuid,
    name text,
    description text,
    icon_id uuid,
...
BEGIN
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource flags', agent_id;
    END IF;
    ...
    v_params_jsonb := jsonb_build_object('name', name, 'description', description, 'icon_id', icon_id);
    ...
    INSERT INTO flags(name, description, icon_id, active, call_id, mcp)
    ...
    VALUES (
        api_create_flags_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_flags_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
