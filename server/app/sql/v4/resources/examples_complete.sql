...
-- Parameters: agent_id (uuid, required, first), example (text)
...
CREATE OR REPLACE FUNCTION api_create_examples_v4(agent_id uuid,
    group_id uuid,
    example text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_examples_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource examples', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_examples_v4.agent_id 
    ...
    -- Check if examples already exists (match on example)
    SELECT r.id INTO v_example_id
    FROM examples_resource r
    WHERE r.example = api_create_examples_v4.example
    LIMIT 1;
    ...
    INSERT INTO examples_resource(example, active, call_id, mcp)
    VALUES (example, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_examples_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_examples_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
