...
-- Parameters: agent_id (uuid, required, first), template_id uuid, schema_field_id uuid, item_template_id uuid, position_value integer
...
CREATE OR REPLACE FUNCTION api_create_template_array_items_v4(agent_id uuid,
    group_id uuid,
    template_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_template_array_items_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource template_array_items', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_template_array_items_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('template_id', template_id, 'schema_field_id', schema_field_id, 'item_template_id', item_template_id, 'position_value', position_value);
    ...
    VALUES (
        v_call_id,
        'template_array_items_' || v_call_id::text,
        v_tool_id,
    ...
    -- INSERT INTO template_array_items_resource table (always insert, never update)
    INSERT INTO template_array_items_resource(template_id, schema_field_id, item_template_id, "position", active, call_id, mcp)
    VALUES (template_id, schema_field_id, item_template_id, position_value, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_template_array_items_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_template_array_items_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
