...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM key WHERE id = v_artifact_id) THEN
    ...
    v_params_jsonb := jsonb_build_object('key_id', key_id);
    ...
    SELECT 
        api_create_keys_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_keys_v4.group_id), -1) + 1,
        NOW(),
        NOW();
