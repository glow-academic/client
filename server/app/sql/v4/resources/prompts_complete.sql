...
BEGIN
    ...
    v_params_jsonb := jsonb_build_object('system_prompt', system_prompt, 'name', name, 'description', description);
    ...
    INSERT INTO prompts(system_prompt, name, description, active, call_id)
    VALUES (system_prompt, name, description, true, v_call_id)
    ...
    VALUES (
        api_create_prompts_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_prompts_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
