-- Create reasoning_levels resource
...
-- Returns: reasoning_levels_id (uuid)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_reasoning_levels_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_reasoning_levels_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_reasoning_levels_v4(agent_id uuid,
    group_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_reasoning_levels_v4.agent_id
      AND rt.resource = 'reasoning_levels'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource reasoning_levels', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_reasoning_levels_v4.agent_id 
    ...
    VALUES (
        v_call_id,
        'reasoning_levels_' || v_call_id::text,
        v_tool_id,
        v_template_id,
        v_arguments_raw,
        true,
    ...
    -- INSERT INTO reasoning_levels_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO reasoning_levels_resource(active, call_id, mcp)
    VALUES (true, v_call_id, mcp)
    RETURNING id INTO v_reasoning_levels_id;
    ...
    SELECT 
        api_create_reasoning_levels_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_reasoning_levels_v4.group_id), -1) + 1,
        NOW(),
        NOW();
    
    RETURN QUERY SELECT v_reasoning_levels_id;
END;
$$;
