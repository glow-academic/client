-- Create ranges resource
-- Always INSERT operation (preserves all information)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), min_count (integer), max_count (integer), mcp (boolean, optional, fifth)
-- Returns: range_id (uuid)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_ranges_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_ranges_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_ranges_v4(agent_id uuid,
    group_id uuid,
    min_count integer,
    max_count integer,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_ranges_v4.agent_id
      AND rt.resource = 'ranges'::resources
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource ranges', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_ranges_v4.agent_id 
    ...
    v_params_jsonb := jsonb_build_object('min_count', min_count, 'max_count', max_count);
    ...
    -- INSERT INTO ranges_resource table (always insert, never update)
    INSERT INTO ranges_resource(min_count, max_count, active, call_id, mcp)
    VALUES (min_count, max_count, true, v_call_id, mcp)
    ...
    SELECT 
        api_create_ranges_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_ranges_v4.group_id), -1) + 1,
        NOW(),
        NOW();
    
    RETURN QUERY SELECT v_range_id;
END;
$$;
