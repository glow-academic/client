...
-- Parameters: agent_id (uuid, required, first)
...
BEGIN
    ...
    WHERE at.agent_id = api_create_schemas_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource schemas', agent_id;
    END IF;
    ...
    INSERT INTO schemas(active, call_id)
    VALUES (true, v_call_id)
    ...
    VALUES (
        api_create_schemas_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_schemas_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
