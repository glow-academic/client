...
-- Get or create operation (returns existing ID if end_reason already exists)
-- Parameters: agent_id (uuid, required, first), end_reason text
...
CREATE OR REPLACE FUNCTION api_create_conversations_v4(agent_id uuid,
    group_id uuid,
    end_reason text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_conversations_v4.agent_id
      AND rt.resource = 'conversations'::resources
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource conversations', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags 
            WHERE agent_id = api_create_conversations_v4.agent_id 
    ...
    -- Check if conversations already exists (match on end_reason)
    SELECT r.id INTO v_conversation_id
    FROM conversations_resource r
    WHERE r.end_reason = api_create_conversations_v4.end_reason
    LIMIT 1;
    ...
    -- INSERT INTO conversations_resource table (always insert, never update)
    INSERT INTO conversations_resource(end_reason, active, call_id, mcp)
    VALUES (end_reason, true, v_call_id, mcp)
    ...
    VALUES (
        api_create_conversations_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_conversations_v4.group_id), -1) + 1,
        NOW(),
        NOW()
    );
