...
BEGIN
    ...
    WHERE at.agent_id = api_create_texts_v4.agent_id
      AND rt.resource = 'texts'::resources
      AND at.active = true
    ...
    -- INSERT INTO texts_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO texts_resource(active, call_id, mcp)
    VALUES (true, v_call_id, mcp)
    RETURNING id INTO v_texts_id;
    ...
    SELECT 
        api_create_texts_v4.group_id,
        v_run_id,
        COALESCE((SELECT MAX(gr.idx) FROM group_runs gr WHERE gr.group_id = api_create_texts_v4.group_id), -1) + 1,
        NOW(),
        NOW();
