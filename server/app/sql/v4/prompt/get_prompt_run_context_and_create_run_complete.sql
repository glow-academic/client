...
dept_specific_settings_with_keys AS (
    ...
    WHERE EXISTS (SELECT 1 FROM scenario_flags sf JOIN flags_resource f ON sf.flag_id = f.id WHERE sf.scenario_id = s.id AND f.name = 'active' AND sf.value = true) AND sd.active = true
    LIMIT 1
),
...
context_data AS (
    ...
    JOIN agents_resource a ON a.id = pa.agent_id AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a.id AND f.name = 'active' AND af.value = true)
