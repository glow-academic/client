...
dept_specific_settings_with_keys AS (
    ...
    WHERE EXISTS (SELECT 1 FROM scenario_flags sf WHERE sf.scenario_id = s.id AND sf.type = 'active'::type_scenario_flags AND sf.value = true) AND sd.active = true
    LIMIT 1
),
...
context_data AS (
    ...
    JOIN agents_resource a ON a.id = pa.agent_id AND EXISTS (SELECT 1 FROM agent_flags af WHERE af.agent_id = a.id AND af.type = 'active'::type_agent_flags AND af.value = true)
...
LEFT JOIN reasoning_levels_resource rl ON rl.id = mrl.reasoning_level_id AND rl.active = true
    ...
    GROUP BY sc.id, sc.title, g.trace_id,
             ...
             m.id, (SELECT v.value FROM model_values mv JOIN values_resource v ON mv.value_id = v.id WHERE mv.model_id = m.id LIMIT 1), n_prov.name, k.key, e.base_url, pa.agent_id, act_s.settings_id,
             -- Other fields
             ap.profile_id,
