-- Finalize prompt tool call - marks tool_call and message as completed
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_prompt_tool_complete_finalize_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_prompt_tool_complete_finalize_v4(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Recreate function
CREATE OR REPLACE FUNCTION socket_prompt_tool_complete_finalize_v4(
    chat_id uuid,
    run_id uuid,
    tool_call_id text,
    call_id text,
