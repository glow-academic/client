...
fields_data AS (
    SELECT 
        ...
        EXISTS (SELECT 1 FROM field_flags ff JOIN flags_resource fl ON ff.flag_id = fl.id WHERE ff.field_id = f.id AND fl.name = 'field_active' AND ff.value = TRUE) as active,
        f.created_at,
        f.updated_at,
