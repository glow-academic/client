-- Patch parameter draft (create if not exists, patch if exists)
...
BEGIN
    ...
    WITH defaults AS (
        SELECT jsonb_build_object(
            ...
            'field_active_states', jsonb_build_object(),
            'field_default_states', jsonb_build_object()
        ) AS d
    ),
    payload AS (
    ...
    SELECT 'parameters'::draft_resource_type, p_profile_id, p
    FROM payload, params
