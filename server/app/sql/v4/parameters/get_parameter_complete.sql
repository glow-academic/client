...
parameter_exists_check AS (
    SELECT 
        CASE 
            ...
            ELSE EXISTS(SELECT 1 FROM parameter WHERE id = (SELECT parameter_id FROM params))::boolean
...
parameter_departments_aggregated AS (
    ...
    FROM (
        ...
        FROM params x
        JOIN fields f ON (SELECT pf.parameter_id FROM parameter_fields pf WHERE pf.field_id = f.id LIMIT 1) = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND EXISTS (SELECT 1 FROM field_flags ff JOIN flags fl ON ff.flag_id = fl.id WHERE ff.field_id = f.id AND fl.name = 'active' AND ff.type = 'active'::type_field_flags AND ff.value = true)
...
parameter_data AS (
    SELECT 
        (SELECT n.name FROM parameter_names pn JOIN names n ON pn.name_id = n.id WHERE pn.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) LIMIT 1),
        (SELECT d.description FROM parameter_descriptions pd JOIN descriptions d ON pd.description_id = d.id WHERE pd.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) LIMIT 1),
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'active' AND paf.type = 'active'::type_parameter_flags AND paf.value = TRUE) as active,
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'simulation_parameter' AND paf.type = 'simulation_parameter'::type_parameter_flags AND paf.value = TRUE) as simulation_parameter,
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'document_parameter' AND paf.type = 'document_parameter'::type_parameter_flags AND paf.value = TRUE) as document_parameter,
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'persona_parameter' AND paf.type = 'persona_parameter'::type_parameter_flags AND paf.value = TRUE) as persona_parameter,
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'scenario_parameter' AND paf.type = 'scenario_parameter'::type_parameter_flags AND paf.value = TRUE) as scenario_parameter,
        EXISTS (SELECT 1 FROM parameter_flags paf JOIN flags fl ON paf.flag_id = fl.id WHERE paf.parameter_id = COALESCE(x.parameter_id, (SELECT id FROM default_parameter LIMIT 1)) AND fl.name = 'video_parameter' AND paf.type = 'video_parameter'::type_parameter_flags AND paf.value = TRUE) as video_parameter,
...
all_fields_with_usage AS (
    ...
    WHERE EXISTS (SELECT 1 FROM field_flags ff JOIN flags fl ON ff.flag_id = fl.id WHERE ff.field_id = f.id AND fl.name = 'active' AND ff.type = 'active'::type_field_flags AND ff.value = true)
    GROUP BY f.id, (SELECT n.name FROM field_names fn JOIN names n ON fn.name_id = n.id WHERE fn.field_id = f.id LIMIT 1), (SELECT d.description FROM field_descriptions fd JOIN descriptions d ON fd.description_id = d.id WHERE fd.field_id = f.id LIMIT 1), afd.department_ids, f.generated
),
-- Parameter items from parameter (fields connected to parameter)
...
filtered_personas AS (
    ...
    FROM params x
    JOIN persona p ON EXISTS (SELECT 1 FROM persona_flags pf JOIN flags fl ON pf.flag_id = fl.id WHERE pf.persona_id = p.id AND fl.name = 'active' AND pf.type = 'active'::type_persona_flags AND pf.value = true)
...
filtered_documents AS (
    SELECT DISTINCT 
        ...
        COALESCE((SELECT d2.description FROM document_descriptions dd JOIN descriptions d2 ON dd.description_id = d2.id WHERE dd.document_id = d.id LIMIT 1), '') as description,
        COALESCE(d.generated, false) as generated
    FROM params x
    JOIN document d ON EXISTS (SELECT 1 FROM document_flags df JOIN flags fl ON df.flag_id = fl.id WHERE df.document_id = d.id AND fl.name = 'active' AND df.type = 'active'::type_document_flags AND df.value = true)
            ...
            ELSE COALESCE(
                (SELECT ARRAY_AGG(pf.field_id ORDER BY pf.created_at)
                 ...
                 WHERE pf.parameter_id = (SELECT parameter_id FROM params)
                   AND EXISTS (SELECT 1 FROM field_flags ff JOIN flags fl ON ff.flag_id = fl.id WHERE ff.field_id = pf.field_id AND fl.name = 'active' AND ff.type = 'active'::type_field_flags AND ff.value = true)),
                ARRAY[]::uuid[]
            )
        END as field_ids
    FROM params
