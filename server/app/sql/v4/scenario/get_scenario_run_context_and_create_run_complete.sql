...
context_data AS (
    ...
    SELECT 
        ...
        COALESCE(
            ...
            AND EXISTS (SELECT 1 FROM parameter_flags pf JOIN flags_resource f ON pf.flag_id = f.id WHERE pf.parameter_id = pa.id AND f.name = 'active' AND pf.value = TRUE)
            ),
