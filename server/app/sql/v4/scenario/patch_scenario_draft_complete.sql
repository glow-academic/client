-- Patch scenario draft (create if not exists, patch if exists)
...
BEGIN
    ...
    WITH defaults AS (
        SELECT jsonb_build_object(
            ...
            'persona_ids', jsonb_build_array(),
            'document_ids', jsonb_build_array(),
            'template_document_ids', jsonb_build_array(),
            'parameter_ids', jsonb_build_array(),
            'field_ids', jsonb_build_array(),
            'image_ids', jsonb_build_array(),
            'objective_ids', jsonb_build_array(),
            'problem_statement_ids', jsonb_build_array(),
            'use_image', false,
            'use_video', false,
            'use_objectives', true,
            'use_questions', false,
            ...
            'active', true,
            'randomize', NULL,
            'randomize_personas', NULL,
            'randomize_documents', NULL,
            'randomize_parameters', NULL,
            'field_show_selected', jsonb_build_object(),
            'field_ranges', jsonb_build_object(),
            'randomize_parameter_items', jsonb_build_object(),
            'persona_min', NULL,
            'persona_max', NULL,
            'document_min', NULL,
            'document_max', NULL,
            'parameter_selection_min', NULL,
            'parameter_selection_max', NULL
        ) AS d
    ),
    payload AS (
