...
link_persona_range AS (
    ...
    SELECT ns.id, gpri.range_id, 'persona'::type_scenario_ranges, false, false, NOW(), NOW()
    FROM new_scenario ns
...
link_document_range AS (
    INSERT INTO scenario_ranges (scenario_id, range_id, type, generated, mcp, created_at, updated_at)
    SELECT ns.id, gdri.range_id, 'document'::type_scenario_ranges, false, false, NOW(), NOW()
    FROM new_scenario ns
...
link_parameter_range AS (
    INSERT INTO scenario_ranges (scenario_id, range_id, type, generated, mcp, created_at, updated_at)
    SELECT ns.id, gpri.range_id, 'parameter'::type_scenario_ranges, false, false, NOW(), NOW()
    FROM new_scenario ns
...
link_field_range AS (
    ...
    -- We create one field range per scenario, not per parameter
    INSERT INTO scenario_ranges (scenario_id, range_id, type, generated, mcp, created_at, updated_at)
    SELECT ns.id, gfri.range_id, 'field'::type_scenario_ranges, false, false, NOW(), NOW()
    FROM new_scenario ns
