-- Patch auth draft (create if not exists, patch if exists)
...
BEGIN
    ...
    WITH defaults AS (
        SELECT jsonb_build_object(
            ...
            'description', '',
            'active', false,
            'authItemIds', jsonb_build_array(),
            'authItemActiveStates', jsonb_build_object(),
            'authItemEncryptedStates', jsonb_build_object(),
            'authItemData', jsonb_build_object()
        ) AS d
    ),
    payload AS (
    ...
    SELECT 'auth'::draft_resource_type, p_profile_id, p
    FROM payload, params
