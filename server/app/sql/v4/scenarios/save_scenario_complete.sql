...
-- 3) Recreate types
CREATE TYPE types.q_save_scenario_v4_parameter AS (
    parameter_id uuid,
    field_ids uuid[]
);

CREATE TYPE types.q_save_scenario_v4_question_timestamp AS (
    question_id uuid,
    video_id uuid,
    timestamps numeric[]
);

CREATE TYPE types.q_save_scenario_v4_upload_image AS (
    upload_id uuid,
    name text
);
...
CREATE OR REPLACE FUNCTION api_save_scenario_v4(
    ...
    document_ids text[],
    objective_ids text[],
    parameters types.q_save_scenario_v4_parameter[],
    profile_id uuid,
    ...
    question_timestamps types.q_save_scenario_v4_question_timestamp[] DEFAULT ARRAY[]::types.q_save_scenario_v4_question_timestamp[],
...
WITH raw_params AS (
    SELECT
        input_scenario_id AS input_scenario_id,
        name AS name,
...
-- Conditional: Check scenario existence if input_scenario_id provided
scenario_exists_check AS (
    SELECT 
        CASE 
            WHEN (SELECT input_scenario_id FROM raw_params) IS NULL THEN NULL::boolean
            ELSE EXISTS(SELECT 1 FROM scenario_artifact WHERE id = (SELECT input_scenario_id FROM raw_params LIMIT 1))::boolean
...
parameter_preprocessing AS (
    ...
    FROM raw_params rp
    CROSS JOIN UNNEST(COALESCE(rp.parameters, ARRAY[]::types.q_save_scenario_v4_parameter[])) as param
...
upload_images_preprocessing AS (
    SELECT
        COALESCE(
            ARRAY_AGG(
                (upload_id, image_name)::types.q_save_scenario_v4_upload_image
                ORDER BY idx
            ),
            '{}'::types.q_save_scenario_v4_upload_image[]
        ) as upload_images
    FROM raw_params rp
    CROSS JOIN LATERAL (
        SELECT 
...
params AS (
    SELECT
        rp.input_scenario_id AS input_scenario_id,
        rp.name AS name,
        ...
        COALESCE(uip.upload_images, ARRAY[]::types.q_save_scenario_v4_upload_image[]) AS upload_images,
        ...
        COALESCE(rp.question_timestamps, ARRAY[]::types.q_save_scenario_v4_question_timestamp[]) AS question_timestamps,
        rp.run_id AS run_id,
...
validate_permissions AS (
    SELECT 
        CASE 
            WHEN (SELECT input_scenario_id FROM params) IS NULL THEN
            ...
            ELSE
                -- Validate update permissions (check scenario exists and user has access)
                (SELECT 
                    CASE 
                        WHEN NOT EXISTS(SELECT 1 FROM scenario_artifact WHERE id = (SELECT input_scenario_id FROM params LIMIT 1)) THEN false
                        ELSE true  -- TODO: Add proper department access check
                    END
                )
...
new_scenario AS (
    ...
    SELECT NOW(), NOW()
    FROM params
    WHERE input_scenario_id IS NULL
    RETURNING id
),
scenario_updated AS (
    UPDATE scenario_artifact sa
    SET updated_at = NOW()
    FROM params p
    WHERE sa.id = p.input_scenario_id
      AND p.input_scenario_id IS NOT NULL
    RETURNING sa.id
),
scenario_id_resolved AS (
    SELECT 
        COALESCE(
            (SELECT id FROM new_scenario LIMIT 1),
            (SELECT id FROM scenario_updated LIMIT 1),
            (SELECT input_scenario_id FROM params LIMIT 1)
        ) as scenario_id
),
link_name AS (
    -- Link name to scenario
    INSERT INTO scenario_names (scenario_id, name_id, created_at, updated_at)
    SELECT sir.scenario_id, gocn.name_id, NOW(), NOW()
...
link_description AS (
    ...
    SELECT sir.scenario_id, gocd.description_id, NOW(), NOW()
    FROM scenario_id_resolved sir
...
link_tree_edge AS (
    ...
    SELECT sir.scenario_id, sir.scenario_id, true
    FROM scenario_id_resolved sir
),
