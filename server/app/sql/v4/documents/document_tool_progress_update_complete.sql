-- UPDATE document_artifact tool call progress - creates/updates tool_call and accumulates arguments
-- Handles all document agent tools (create_title, generate_html, generate_schema)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_document_tool_progress_update_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_document_tool_progress_update_v4(%s)', r.sig);
    END LOOP;
END $$;
