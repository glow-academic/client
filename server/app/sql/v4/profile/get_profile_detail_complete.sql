...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_profile_detail_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_profile_detail_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_profile_detail_v4_%'
...
-- 3) Recreate types
CREATE TYPE types.q_get_profile_detail_v4_department AS (
    department_id uuid,
    name text,
    description text
);

CREATE TYPE types.q_get_profile_detail_v4_cohort AS (
    cohort_id uuid,
    name text,
    description text
);
...
-- 4) Recreate function
CREATE OR REPLACE FUNCTION api_get_profile_detail_v4(
...
RETURNS TABLE (
    ...
    departments types.q_get_profile_detail_v4_department[],
    cohorts types.q_get_profile_detail_v4_cohort[],
    actor_name text
)
LANGUAGE sql
STABLE
...
target_profile AS (
    ...
    GROUP BY p.id, p.role, EXISTS (SELECT 1 FROM profile_flags pf JOIN flags fl ON pf.flag_id = fl.id WHERE pf.profile_id = p.id AND fl.name = 'active' AND pf.type = 'active'::type_profile_flags AND pf.value = TRUE), prl.requests_per_day
),
...
target_profile_departments AS (
    SELECT 
        ARRAY_AGG(pd.department_id ORDER BY (SELECT n.name FROM department_names dn JOIN names n ON dn.name_id = n.id WHERE dn.department_id = d.id LIMIT 1)) as department_ids,
...
valid_departments_agg AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (vdd.department_id, vdd.name, vdd.description)::types.q_get_profile_detail_v4_department
                ORDER BY vdd.name
            ) FILTER (WHERE vdd.department_id IS NOT NULL),
            '{}'::types.q_get_profile_detail_v4_department[]
        ) as departments
...
all_cohorts_agg AS (
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (acd.cohort_id, acd.name, acd.description)::types.q_get_profile_detail_v4_cohort
                ORDER BY acd.name
            ) FILTER (WHERE acd.cohort_id IS NOT NULL),
            '{}'::types.q_get_profile_detail_v4_cohort[]
        ) as cohorts
...
SELECT 
    ...
    COALESCE(vda.departments, '{}'::types.q_get_profile_detail_v4_department[]) as departments,
    COALESCE(aca.cohorts, '{}'::types.q_get_profile_detail_v4_cohort[]) as cohorts,
    -- Actor name
    ap.actor_name
