-- Get cohort resource IDs by group_id - no-op function for validation and mapping
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_get_cohort_resource_ids_by_group_id_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_cohort_resource_ids_by_group_id_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'q_get_cohort_resource_ids_by_group_id_v4_%'
...
CREATE OR REPLACE FUNCTION api_get_cohort_resource_ids_by_group_id_v4(
    profile_id uuid,
    group_id uuid,
    resource_id uuid,
    resource_type text,
