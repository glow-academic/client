...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_rubric_run_context_and_create_run_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_rubric_run_context_and_create_run_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_get_rubric_run_context_and_create_run_v4_%'
...
CREATE TYPE types.i_get_rubric_run_context_and_create_run_v4_standard_group AS (
    id text,
    name text,
    description text,
    points integer,
...
CREATE TYPE types.i_get_rubric_run_context_and_create_run_v4_standard AS (
    id text,
    name text,
    points integer,
...
CREATE OR REPLACE FUNCTION socket_get_rubric_run_context_and_create_run_v4(
    department_id uuid,
    profile_id uuid,
    rubric_agent_id uuid,
    group_id uuid DEFAULT NULL,
    rubric_id uuid DEFAULT NULL,
    standard_groups types.i_get_rubric_run_context_and_create_run_v4_standard_group[] DEFAULT NULL,
    standards types.i_get_rubric_run_context_and_create_run_v4_standard[] DEFAULT NULL
)
RETURNS TABLE (
    agent_id text,
    agent_name text,
