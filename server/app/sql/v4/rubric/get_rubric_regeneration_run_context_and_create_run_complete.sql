...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'socket_get_rubric_regeneration_run_context_and_create_run_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_rubric_regeneration_run_context_and_create_run_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_rubric_regen_run_context_create_run_v4_%'
...
CREATE TYPE types.i_rubric_regen_run_context_create_run_v4_sg AS (
    id text,
    name text,
    description text,
    points integer,
...
CREATE TYPE types.i_rubric_regen_run_context_create_run_v4_std AS (
    id text,
    name text,
    points integer,
...
CREATE TYPE types.i_rubric_regen_run_context_create_run_v4_msg AS (
    role text,
    content text
);
...
CREATE OR REPLACE FUNCTION socket_get_rubric_regeneration_run_context_and_create_run_v4(
    department_id uuid,
    profile_id uuid,
...
RETURNS TABLE (
    ...
    group_id uuid,
    trace_id text,
    standard_groups types.i_rubric_regen_run_context_create_run_v4_sg[],  -- Array of standard groups FROM rubric_artifact
    standards types.i_rubric_regen_run_context_create_run_v4_std[],  -- Array of standards FROM rubric_artifact
    previous_messages types.i_rubric_regen_run_context_create_run_v4_msg[]  -- All messages from all previous runs in group
)
LANGUAGE sql
VOLATILE
AS $$
...
rubric_structure AS (
    ...
    SELECT 
        COALESCE(
            ARRAY_AGG(
                (sg.id::text, sg.name, sg.description, sg.points, sg.pass_points)::types.i_rubric_regen_run_context_create_run_v4_sg
                ORDER BY rsg.position, sg.created_at
            ),
            '{}'::types.i_rubric_regen_run_context_create_run_v4_sg[]
        ) as standard_groups,
        COALESCE(
            ARRAY_AGG(
                (s.id::text, (SELECT n.name FROM scenario_names sn JOIN names_resource n ON sn.name_id = n.id WHERE sn.scenario_id = s.id LIMIT 1), s.points, s.standard_group_id::text)::types.i_rubric_regen_run_context_create_run_v4_std
                ORDER BY rsg.position, s.created_at
            ),
            '{}'::types.i_rubric_regen_run_context_create_run_v4_std[]
        ) as standards
    FROM params p
...
previous_messages_array AS (
    ...
    SELECT COALESCE(
        ARRAY_AGG(
            (role, content)::types.i_rubric_regen_run_context_create_run_v4_msg
            ORDER BY run_idx, created_at
        ),
        '{}'::types.i_rubric_regen_run_context_create_run_v4_msg[]
