...
target_profile_departments AS (
    ...
    WHERE pd.profile_id = (SELECT target_profile_id FROM params) AND pd.active = true AND EXISTS (SELECT 1 FROM document_flags df JOIN flags fl ON df.flag_id = fl.id WHERE df.document_id = d.id AND fl.name = 'active' AND df.type = 'active'::type_document_flags AND df.value = true)
),
all_cohort_ids AS (
...
departments_data AS (
    ...
    LEFT JOIN departments d ON d.id = pd.department_id AND EXISTS (SELECT 1 FROM document_flags df JOIN flags fl ON df.flag_id = fl.id WHERE df.document_id = d.id AND fl.name = 'active' AND df.type = 'active'::type_document_flags AND df.value = true)
