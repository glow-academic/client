...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_bulk_create_staff_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_bulk_create_staff_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname LIKE 'i_bulk_create_staff_v4_%'
...
-- 3) Recreate types
CREATE TYPE types.i_bulk_create_staff_v4_profile AS (
    first_name text,
    last_name text,
...
CREATE OR REPLACE FUNCTION api_bulk_create_staff_v4(
    profiles types.i_bulk_create_staff_v4_profile[],
...
WITH params AS (
    SELECT 
        COALESCE(profiles, ARRAY[]::types.i_bulk_create_staff_v4_profile[]) AS profiles,
...
first_names_resources AS (
    ...
    SELECT DISTINCT pd.first_name, NOW(), NOW()
    FROM profiles_data pd
    WHERE pd.first_name IS NOT NULL AND pd.first_name != ''
...
last_names_resources AS (
    INSERT INTO names_resource (name, created_at, updated_at)
    SELECT DISTINCT pd.last_name, NOW(), NOW()
    FROM profiles_data pd
    WHERE pd.last_name IS NOT NULL AND pd.last_name != ''
    ON CONFLICT (name) DO UPDATE SET updated_at = NOW()
    RETURNING id as name_id, name
),
profile_insert AS (
    -- Insert all profiles without first_name, last_name, active, role columns
...
link_profile_first_names AS (
    ...
    JOIN profiles_data pd ON pd.profile_id = pi.id
