-- Get context data for tool generation validation
-- This SQL fetches RAW DATA only - no business logic
-- Python applies the validation rules in permissions.py
-- Identical structure to persona generation context (agent/model/provider queries are artifact-agnostic)
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'socket_get_tool_generation_context_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_tool_generation_context_v4(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Create the function
CREATE OR REPLACE FUNCTION socket_get_tool_generation_context_v4(
    p_profile_id uuid,
    p_agent_id uuid
)
RETURNS TABLE (
...
-- Get active settings (for API key lookup)
