...
BEGIN
    ...
    IF p_scenario_id IS NULL THEN
        SELECT sc.id INTO v_scenario_id
        FROM simulation_scenarios_junction ss
        JOIN scenario_artifact sc ON sc.id = ss.scenario_id
        LEFT JOIN scenario_positions_junction sp ON sp.scenario_id = sc.id
        LEFT JOIN scenario_positions_resource spr ON spr.id = sp.position_id
        WHERE ss.simulation_id = p_simulation_id
          AND ss.active = true
        ORDER BY COALESCE(spr.position, 0) ASC
        LIMIT 1;
    ELSE
    ...
    -- Link attempt to simulation
    INSERT INTO simulation_attempts_simulations_connection (simulation_id, attempt_id, active)
    VALUES (p_simulation_id, v_attempt_id, true);

    -- Link attempt to profile
    INSERT INTO simulation_attempts_profiles_connection (profile_id, attempt_id, active)
    VALUES (p_profile_id, v_attempt_id, true);
    ...
    -- Link chat to scenario if we have one
    IF v_scenario_id IS NOT NULL THEN
        INSERT INTO simulation_chats_scenarios_connection (scenario_id, chat_id, active)
        VALUES (v_scenario_id, v_chat_id, true);
    END IF;

    RETURN QUERY SELECT v_attempt_id, v_chat_id, v_scenario_id;
END;
$$;
