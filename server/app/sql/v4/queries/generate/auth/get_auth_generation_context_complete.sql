-- Get rate limit context for auth generation validation
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'socket_get_auth_generation_context_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS socket_get_auth_generation_context_v4(%s)', r.sig);
    END LOOP;
END $$;

-- 2) Create the function (rate limit only)
CREATE OR REPLACE FUNCTION socket_get_auth_generation_context_v4(
    p_profile_id uuid
)
RETURNS TABLE (
