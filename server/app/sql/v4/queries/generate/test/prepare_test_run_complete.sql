...
BEGIN
    ...
    -- Create config_entry linked to run
    INSERT INTO config_entry (created_at, updated_at, generated, mcp, active, run_id)
    VALUES (NOW(), NOW(), true, false, true, v_run_id)
    RETURNING id INTO v_config_id;

    -- Link agent to config (if provided)
    ...
    -- Link model to config (if provided)
    ...
    -- Link provider to config (if provided)
