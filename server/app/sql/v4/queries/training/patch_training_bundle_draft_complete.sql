...
DO $$
BEGIN
    DROP TYPE IF EXISTS types.training_bundle_multi_resource_action CASCADE;
    CREATE TYPE types.training_bundle_multi_resource_action AS (
        resource_ids uuid[],
        create_tool_id uuid,
        link_tool_id uuid
    );
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_patch_training_bundle_draft_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_patch_training_bundle_draft_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_patch_training_bundle_draft_v4(
    profile_id uuid,
    input_draft_id uuid DEFAULT NULL,
    group_id uuid DEFAULT NULL,
    departments types.training_bundle_multi_resource_action DEFAULT NULL,
    personas types.training_bundle_multi_resource_action DEFAULT NULL,
    documents types.training_bundle_multi_resource_action DEFAULT NULL,
    parameter_fields types.training_bundle_multi_resource_action DEFAULT NULL,
    parameters types.training_bundle_multi_resource_action DEFAULT NULL,
    fields types.training_bundle_multi_resource_action DEFAULT NULL,
    questions types.training_bundle_multi_resource_action DEFAULT NULL,
...
BEGIN
    ...
    IF input_draft_id IS NOT NULL THEN
        ...
        IF v_draft_id IS NOT NULL THEN
            ...
            IF option_ids IS NOT NULL THEN
                INSERT INTO options_drafts_connection (draft_id, options_id, version)
    ...
    IF v_draft_id IS NULL THEN
        ...
        VALUES ('training'::artifact_type, v_group_id)
        RETURNING id, version INTO v_draft_id, v_new_version;
    ...
    IF v_group_id IS NOT NULL THEN
        ...
        IF COALESCE(array_length(option_ids, 1), 0) > 0 THEN
            IF (options).create_tool_id IS NOT NULL THEN
                ...
                INSERT INTO tool_calls_junction (tool_id, call_id) VALUES ((options).create_tool_id, v_call_id);
            ...
            IF (options).link_tool_id IS NOT NULL THEN
                ...
                INSERT INTO tool_calls_junction (tool_id, call_id) VALUES ((options).link_tool_id, v_call_id);
                INSERT INTO options_calls_connection (options_id, call_id)
                SELECT x.option_id, v_call_id FROM UNNEST(option_ids) AS x(option_id);
            END IF;
        END IF;
