...
BEGIN
    ...
    VALUES (true, api_create_test_archive_entry_v4.mcp)
    RETURNING texts_entry.id INTO v_text_id;

    -- Link upload to text entry
    IF api_create_test_archive_entry_v4.upload_id IS NOT NULL THEN
        INSERT INTO text_uploads_entry (text_id, upload_id)
        VALUES (v_text_id, api_create_test_archive_entry_v4.upload_id);
    END IF;
