-- Create test_completion entry with strongly-typed params
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_create_test_completion_entry_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_test_completion_entry_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    VALUES (true, api_create_test_completion_entry_v4.mcp)
    RETURNING texts_entry.id INTO v_text_id;

    -- Link upload to text entry
    IF api_create_test_completion_entry_v4.upload_id IS NOT NULL THEN
        INSERT INTO text_uploads_entry (text_id, upload_id)
        VALUES (v_text_id, api_create_test_completion_entry_v4.upload_id);
    END IF;

    -- 2. Create call record
    v_call_id := uuidv7();
    INSERT INTO calls_entry (id, run_id, external_call_id)
    VALUES (v_call_id, api_create_test_completion_entry_v4.run_id, 'test_completion_' || v_call_id::text);

    -- 3. Link tool to call
    IF api_create_test_completion_entry_v4.tool_id IS NOT NULL THEN
        INSERT INTO tools_calls_connection (tools_id, call_id)
        VALUES (api_create_test_completion_entry_v4.tool_id, v_call_id);
    END IF;
    ...
    VALUES (api_create_test_completion_entry_v4.run_id, v_call_id, 'assistant', v_text_id, true, api_create_test_completion_entry_v4.mcp)
