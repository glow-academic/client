...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_suite_drafts_entries_v4_item%'
...
CREATE TYPE types.q_get_suite_drafts_entries_v4_item AS (
    draft_id uuid,
...
RETURNS TABLE (
    items types.q_get_suite_drafts_entries_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
        ...
        SELECT COALESCE(
            ARRAY_AGG(
                (
                    ...
                    tool_ids,
                    voice_ids
                )::types.q_get_suite_drafts_entries_v4_item
                ORDER BY updated_at DESC
            ),
            ARRAY[]::types.q_get_suite_drafts_entries_v4_item[]
        ) AS items
        FROM mv_data
    )
