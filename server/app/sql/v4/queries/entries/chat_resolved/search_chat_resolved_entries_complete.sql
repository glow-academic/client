-- Search chat_resolved entries from chat_resolved_mv
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_search_chat_resolved_entries_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_search_chat_resolved_entries_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION public.api_search_chat_resolved_entries_v4(
    search text DEFAULT NULL,
    limit_count integer DEFAULT 20,
    offset_count integer DEFAULT 0,
    attempt_id uuid DEFAULT NULL,
    group_id uuid DEFAULT NULL,
    chat_resolved_id uuid DEFAULT NULL,
