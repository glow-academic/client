...
-- Look up scenario time limit for this simulation+scenario
scenario_time_limit AS (
    SELECT COALESCE(stlr.time_limit_seconds, 0) as time_limit_seconds
    FROM chosen_scenario_id csi
    CROSS JOIN params p
    LEFT JOIN scenario_time_limits_resource stlr ON stlr.scenario_id = csi.scenarios_resource_id AND stlr.active = true
    LEFT JOIN simulation_scenario_time_limits_junction sstlj ON sstlj.scenario_time_limit_id = stlr.id
        AND sstlj.simulation_id = p.simulation_id AND sstlj.active = true
    LIMIT 1
),
...
new_chat AS (
    INSERT INTO simulation_chats_entry (
        created_at, title, updated_at, attempt_id, time_limit_seconds
    )
    SELECT
        ng.created_at,
    ...
    CROSS JOIN content_type_check ctc
    CROSS JOIN scenario_time_limit stl
    WHERE ctc.content_type = 'scenario'
