...
new_group AS (
    ...
    SELECT now(), now(), (SELECT id FROM sessions_entry WHERE sessions_entry.profile_id = socket_start_simulation_attempt_v4.profile_id AND sessions_entry.active = true ORDER BY created_at DESC LIMIT 1)
    FROM new_attempt na
