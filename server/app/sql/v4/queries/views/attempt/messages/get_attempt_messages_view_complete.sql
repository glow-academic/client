...
CREATE TYPE types.q_get_attempt_messages_view_v4_highlight AS (
    section text,
    idx int
);

-- Replacement type (nested under improvement)
CREATE TYPE types.q_get_attempt_messages_view_v4_replacement AS (
    section text,
    replace_text text,
    idx int
);
...
CREATE TYPE types.q_get_attempt_messages_view_v4_strength AS (
    name text,
    description text,
    highlights types.q_get_attempt_messages_view_v4_highlight[]
);

-- Improvement type (id/message_id removed - implied by parent message)
CREATE TYPE types.q_get_attempt_messages_view_v4_improvement AS (
    name text,
    description text,
    replacements types.q_get_attempt_messages_view_v4_replacement[]
);

-- Hint type (message_id removed - implied by parent message)
CREATE TYPE types.q_get_attempt_messages_view_v4_hint AS (
    hint text,
    idx int
);

-- Content type (id removed, only persona_id - metadata fetched via handler)
CREATE TYPE types.q_get_attempt_messages_view_v4_content AS (
    content text,
...
CREATE TYPE types.q_get_attempt_messages_view_v4_item AS (
    ...
    contents types.q_get_attempt_messages_view_v4_content[],

    -- Strengths and improvements (message_id implied)
    strengths types.q_get_attempt_messages_view_v4_strength[],
...
AS $$
            ...
            COALESCE(
                ARRAY_AGG(
                    (
                        ...
                        (c).persona_id,
                        (c).created_at
                    )::types.q_get_attempt_messages_view_v4_content
                    ORDER BY (c).created_at
                ) FILTER (WHERE (c).content IS NOT NULL),
                ARRAY[]::types.q_get_attempt_messages_view_v4_content[]
            ) AS contents
        FROM mv_data mv
                ...
                ARRAY_AGG(
                    (
                        ...
                        (
                            SELECT COALESCE(
                                ARRAY_AGG(
                                    ((h).section, (h).idx)::types.q_get_attempt_messages_view_v4_highlight
                                    ORDER BY (h).idx
                                ),
                                ARRAY[]::types.q_get_attempt_messages_view_v4_highlight[]
                            )
                            FROM unnest((s).highlights) AS h
                        )
                    )::types.q_get_attempt_messages_view_v4_strength
                ) FILTER (WHERE (s).name IS NOT NULL),
                ARRAY[]::types.q_get_attempt_messages_view_v4_strength[]
            ) AS strengths
        FROM mv_data mv
                ...
                ARRAY_AGG(
                    (
                        ...
                        (
                            SELECT COALESCE(
                                ARRAY_AGG(
                                    ((r).section, (r).replace_text, (r).idx)::types.q_get_attempt_messages_view_v4_replacement
                                    ORDER BY (r).idx
                                ),
                                ARRAY[]::types.q_get_attempt_messages_view_v4_replacement[]
                            )
                            FROM unnest((i).replacements) AS r
                        )
                    )::types.q_get_attempt_messages_view_v4_improvement
                ) FILTER (WHERE (i).name IS NOT NULL),
                ARRAY[]::types.q_get_attempt_messages_view_v4_improvement[]
            ) AS improvements
        FROM mv_data mv
    ...
    hints_transformed AS (
        SELECT
            ...
            COALESCE(
                ARRAY_AGG(
                    ((h).hint, (h).idx)::types.q_get_attempt_messages_view_v4_hint
                    ORDER BY (h).idx
                ) FILTER (WHERE (h).hint IS NOT NULL),
                ARRAY[]::types.q_get_attempt_messages_view_v4_hint[]
            ) AS hints
        FROM mv_data mv
