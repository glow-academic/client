-- ============================================================================
-- Query: get_home_context_view
-- Purpose: IDs-first home context for artifact hydration (thin MV filter layer)
-- Section: VIEWS/HOME/CONTEXT
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_get_home_context_view_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_home_context_view_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_home_context_view_v4_%'
...
CREATE OR REPLACE FUNCTION api_get_home_context_view_v4(
    profile_id_filter uuid
)
RETURNS TABLE (
    items types.q_get_home_context_view_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
...
SELECT
    COALESCE(
        ...
        ARRAY[]::types.q_get_home_context_view_v4_item[]
    ) AS items;
$$;
