...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_get_video_list_view_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_video_list_view_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_video_list_view_v4_%'
        ...
        SELECT COALESCE(
            ARRAY_AGG(
                (
                    ...
                    size,
                    created_at
                )::types.q_get_video_list_view_v4_item
                ORDER BY created_at DESC
            ),
            ARRAY[]::types.q_get_video_list_view_v4_item[]
        ) AS items
        FROM sorted
    )
    SELECT
