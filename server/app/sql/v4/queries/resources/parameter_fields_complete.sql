...
CREATE OR REPLACE FUNCTION api_create_parameter_fields_v4(agent_id uuid,
    group_id uuid,
    field_id uuid,
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM fields_resource WHERE id = api_create_parameter_fields_v4.field_id) THEN
        RAISE EXCEPTION 'Fields resource % does not exist', api_create_parameter_fields_v4.field_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_parameter_fields_v4.agent_id
      AND rt.resource = 'parameter_fields'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource parameter_fields', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_parameter_fields_v4.agent_id
              AND value = true
        ) THEN
    ...
    -- Check if parameter_fields already exists (match on field_id)
    SELECT r.id INTO v_resource_id
    FROM parameter_fields_resource r
    WHERE r.field_id = api_create_parameter_fields_v4.field_id
    LIMIT 1;
    ...
    VALUES (
        v_call_id,
        'parameter_fields_' || v_call_id::text,
        v_template_id,
        v_arguments_raw,
        true,
        NOW(),
    ...
    -- INSERT INTO parameter_fields_resource table (always insert, never update)
    INSERT INTO parameter_fields_resource(id, field_id, active, generated, call_id, created_at, updated_at)
    VALUES (uuidv7(), api_create_parameter_fields_v4.field_id, true, true, v_call_id, NOW(), NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_parameter_fields_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_parameter_fields_v4.agent_id, v_run_id);
