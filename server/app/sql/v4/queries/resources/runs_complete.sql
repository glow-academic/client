...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_runs_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_runs_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_runs_v4(agent_id uuid,
    group_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_runs_v4.agent_id
      AND rt.resource = 'runs'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource runs_entry', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_runs_v4.agent_id 
    ...
    -- INSERT INTO runs_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO runs_resource(active, call_id, mcp)
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_runs_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_runs_v4.agent_id, v_run_id);
