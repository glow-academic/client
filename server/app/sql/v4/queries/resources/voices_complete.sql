...
-- Parameters: voice (text), mcp (boolean), group_id (uuid, optional), tool_id (uuid, optional)
...
CREATE OR REPLACE FUNCTION api_create_voices_v4(
...
DECLARE
    ...
    v_call_id uuid;
BEGIN
    -- INSERT INTO voices_resource table
    INSERT INTO voices_resource(voice, active, mcp, generated)
    VALUES (
        api_create_voices_v4.voice,
        true,
        api_create_voices_v4.mcp,
        api_create_voices_v4.mcp
    )
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, api_create_voices_v4.group_id, NOW(), NOW());

        -- Create call record
        v_call_id := uuidv7();
        INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
        VALUES (v_call_id, 'voices_' || v_call_id::text, v_run_id, true, NOW(), NOW());

        -- Link tool to call
        INSERT INTO tools_calls_connection (tools_id, call_id) VALUES (api_create_voices_v4.tool_id, v_call_id);
