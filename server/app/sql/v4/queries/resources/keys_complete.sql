...
-- Get or create operation (returns existing ID if key_id + group_id already exists)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), key_id (uuid, required, third), mcp (boolean, optional, fourth)
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_keys_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_keys_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_keys_v4(agent_id uuid,
    group_id uuid,
    key_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided key_id as artifact_id
    ...
    WHERE at.agent_id = api_create_keys_v4.agent_id
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource keys', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_keys_v4.agent_id 
    ...
    -- Check if keys already exists (match on key_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM keys_resource r
    WHERE r.key_id = v_artifact_id
      AND r.group_id = api_create_keys_v4.group_id
    LIMIT 1;
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_keys_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_keys_v4.agent_id, v_run_id);
