...
CREATE OR REPLACE FUNCTION api_create_thresholds_v4(agent_id uuid,
    group_id uuid,
    value numeric,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_thresholds_v4.agent_id
      AND rt.resource = 'thresholds'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource thresholds', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags_junction 
            WHERE agent_id = api_create_thresholds_v4.agent_id 
    ...
    -- INSERT INTO thresholds_resource table (always insert, never update)
    INSERT INTO thresholds_resource(value, active, call_id, mcp)
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_thresholds_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_thresholds_v4.agent_id, v_run_id);
