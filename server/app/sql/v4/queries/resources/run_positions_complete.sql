...
CREATE OR REPLACE FUNCTION api_create_run_positions_v4(agent_id uuid,
    group_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_run_positions_v4.agent_id
      AND rt.resource = 'run_positions'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource run_positions', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_run_positions_v4.agent_id 
    ...
    -- INSERT INTO run_positions_resource table (always insert, never update)
    -- Note: Column names and values need to be adjusted based on actual table schema
    INSERT INTO run_positions_resource(active, call_id, mcp)
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_run_positions_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_run_positions_v4.agent_id, v_run_id);
    ...
    RETURN QUERY SELECT v_run_positions_id;
END;
$$;
