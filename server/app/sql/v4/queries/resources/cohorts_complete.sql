-- Create cohorts resource
-- Get or create operation (returns existing ID if cohort_id + group_id already exists)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), cohort_id (uuid, required, third), mcp (boolean, optional, fourth)
...
CREATE OR REPLACE FUNCTION api_create_cohorts_v4(agent_id uuid,
    group_id uuid,
    cohort_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided cohort_id as artifact_id
    v_artifact_id := api_create_cohorts_v4.cohort_id;
    
    -- Validate that cohort artifact exists
    IF NOT EXISTS (SELECT 1 FROM cohort_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Cohort artifact % does not exist', v_artifact_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_cohorts_v4.agent_id
      AND rt.resource = 'cohorts'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource cohorts', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_cohorts_v4.agent_id 
    ...
    -- Check if cohorts already exists (match on cohort_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM cohorts_resource r
    WHERE r.cohort_id = v_artifact_id
      AND r.group_id = api_create_cohorts_v4.group_id
    LIMIT 1;
    ...
    -- INSERT INTO cohorts_resource table (always insert, never update)
    -- Create resource with new unique id and cohort_id FK
    INSERT INTO cohorts_resource(id, cohort_id, active, generated, mcp, call_id, group_id, created_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_cohorts_v4.group_id, NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_cohorts_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_cohorts_v4.agent_id, v_run_id);
