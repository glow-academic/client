...
BEGIN
    FOR r IN
        ...
        FROM pg_proc
        WHERE proname = 'api_get_thresholds_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_get_thresholds_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    FOR r IN
        SELECT typname
        FROM pg_type
        WHERE typname LIKE 'q_get_thresholds_v4_%'
...
-- Create composite type for threshold item
CREATE TYPE types.q_get_thresholds_v4_item AS (
    id uuid,
    value integer,
    generated boolean
);
...
-- Create function
CREATE OR REPLACE FUNCTION api_get_thresholds_v4(
...
SELECT COALESCE(
    ARRAY_AGG(
        (r.id, r.value, COALESCE(r.generated, false))::types.q_get_thresholds_v4_item
