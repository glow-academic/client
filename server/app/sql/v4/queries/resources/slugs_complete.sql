...
CREATE OR REPLACE FUNCTION api_create_slugs_v4(
...
DECLARE
    ...
    v_call_id uuid;
BEGIN
    -- INSERT INTO slugs_resource table
    ...
    IF tool_id IS NOT NULL AND group_id IS NOT NULL THEN
        ...
        VALUES (v_run_id, 0, 0, 0, api_create_slugs_v4.group_id, NOW(), NOW());

        -- Create call record
        v_call_id := uuidv7();
        INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
        VALUES (v_call_id, 'slugs_' || v_call_id::text, v_run_id, true, NOW(), NOW());

        -- Link tool to call
        INSERT INTO tools_calls_connection (tools_id, call_id) VALUES (api_create_slugs_v4.tool_id, v_call_id);

        -- Link resource to call
        INSERT INTO slugs_calls_connection (slugs_id, call_id)
        VALUES (v_slugs_id, v_call_id);
    END IF;
