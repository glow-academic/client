...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_profiles_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_profiles_v4(%s)', r.sig);
    END LOOP;
END $$;

CREATE OR REPLACE FUNCTION api_create_profiles_v4(agent_id uuid,
    group_id uuid,
    profile_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided profile_id as artifact_id
    v_artifact_id := api_create_profiles_v4.profile_id;
    
    -- Validate that profile artifact exists
    IF NOT EXISTS (SELECT 1 FROM profile_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Profile artifact % does not exist', v_artifact_id;
    END IF;
    ...
    WHERE at.agent_id = api_create_profiles_v4.agent_id
      AND rt.resource = 'profiles'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource profiles', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_profiles_v4.agent_id 
    ...
    -- INSERT INTO profiles_resource table (always insert, never update)
    -- INSERT INTO profiles_resource table (always insert, never update)
    -- Create resource with new unique id and profile_id FK
    INSERT INTO profiles_resource(id, profile_id, active, generated, mcp, call_id, group_id, created_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_profiles_v4.group_id, NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_profiles_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_profiles_v4.agent_id, v_run_id);
