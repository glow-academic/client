...
-- Parameters: agent_id (uuid, required, first), system_prompt text, name text, description text
...
CREATE OR REPLACE FUNCTION api_create_prompts_v4(agent_id uuid,
    group_id uuid,
    system_prompt text,
    name text,
    description text,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_prompts_v4.agent_id
      AND rt.resource = 'prompts'::resource_type
      AND at.active = true
    ...
    -- INSERT INTO prompts_resource table (always insert, never update)
    INSERT INTO prompts_resource(system_prompt, name, description, active, call_id, mcp)
    VALUES (system_prompt, name, description, true, v_call_id, mcp)
    ...
    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_prompts_v4.agent_id, v_run_id);
