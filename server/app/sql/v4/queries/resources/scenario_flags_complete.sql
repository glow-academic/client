-- Create scenario_flags_junction resource
...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_create_scenario_flags_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_create_scenario_flags_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    WHERE at.agent_id = api_create_scenario_flags_v4.agent_id
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_scenario_flags_v4.agent_id 
    ...
    -- Check if scenario_flags_junction already exists (match on scenario_id + flag_id)
    ...
    VALUES (
        v_call_id,
        'scenario_flags_' || v_call_id::text,
        v_template_id,
        v_arguments_raw,
        true,
        NOW(),
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_scenario_flags_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_scenario_flags_v4.agent_id, v_run_id);
