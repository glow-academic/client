-- Search role_routes resources
-- Parameters: search (text), limit_count (int), offset_count (int), exclude_ids (uuid[])
-- Returns: items (array of role_routes resources)

-- Drop function if exists (handles signature variations)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN
        SELECT oidvectortypes(proargtypes) as sig
        FROM pg_proc
        WHERE proname = 'api_search_role_routes_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_search_role_routes_v4(%s)', r.sig);
    END LOOP;
END $$;

-- Create function
CREATE OR REPLACE FUNCTION api_search_role_routes_v4(
    search text DEFAULT NULL,
    limit_count int DEFAULT 20,
    offset_count int DEFAULT 0,
    exclude_ids uuid[] DEFAULT ARRAY[]::uuid[],
    role_ids uuid[] DEFAULT ARRAY[]::uuid[],
    route_ids uuid[] DEFAULT ARRAY[]::uuid[],
    -- Artifact boolean filters: when true, only return resources linked to that artifact type
    setting boolean DEFAULT false
)
RETURNS TABLE (
    items types.q_get_role_routes_v4_item[]
)
LANGUAGE sql
STABLE
AS $$
SELECT COALESCE(
    ARRAY_AGG(
        (q.id, q.role_id, q.route_id, q.generated)::types.q_get_role_routes_v4_item
        ORDER BY q.id
    ),
    ARRAY[]::types.q_get_role_routes_v4_item[]
) as items
FROM (
    SELECT r.id, r.role_id, r.route_id, COALESCE(r.generated, false) AS generated
    FROM role_routes_resource r
    WHERE r.active = true
      AND (exclude_ids IS NULL OR NOT (r.id = ANY(exclude_ids)))
      AND (COALESCE(array_length(role_ids, 1), 0) = 0 OR r.role_id = ANY(role_ids))
      AND (COALESCE(array_length(route_ids, 1), 0) = 0 OR r.route_id = ANY(route_ids))
      -- Artifact boolean filters (each filters to resources linked to at least one of that artifact type)
      AND (NOT setting OR EXISTS (SELECT 1 FROM setting_role_routes_junction j WHERE j.role_routes_id = r.id AND j.active = true))
    ORDER BY r.id
    LIMIT limit_count
    OFFSET offset_count
) q;
$$;
