...
-- Get or create operation (returns existing ID if simulation_id + group_id already exists)
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), simulation_id (uuid, required, third), mcp (boolean, optional, fourth)
...
CREATE OR REPLACE FUNCTION api_create_simulations_v4(agent_id uuid,
    group_id uuid,
    simulation_id uuid,
...
DECLARE
    ...
    v_run_id uuid;
BEGIN
    -- Use provided simulation_id as artifact_id
    v_artifact_id := api_create_simulations_v4.simulation_id;
    
    -- Validate that simulation artifact exists
    IF NOT EXISTS (SELECT 1 FROM simulation_artifact WHERE id = v_artifact_id) THEN
        RAISE EXCEPTION 'Simulation artifact % does not exist', v_artifact_id;
    END IF;
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource simulations', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_simulations_v4.agent_id 
    ...
    -- INSERT INTO simulations_resource table (always insert, never update)
    -- INSERT INTO simulations_resource table (always insert, never update)
    -- Create resource with new unique id and simulation_id FK
    INSERT INTO simulations_resource(id, simulation_id, active, generated, mcp, call_id, group_id, created_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_simulations_v4.group_id, NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_simulations_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_simulations_v4.agent_id, v_run_id);
