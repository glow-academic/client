...
CREATE OR REPLACE FUNCTION api_create_group_positions_v4(agent_id uuid,
    group_id uuid,
...
BEGIN
    ...
    WHERE at.agent_id = api_create_group_positions_v4.agent_id
      AND rt.resource = 'group_positions'::resource_type
      AND at.active = true
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource group_positions', agent_id;
    END IF;
    ...
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            WHERE agent_id = api_create_group_positions_v4.agent_id 
    ...
    VALUES (
        v_call_id,
        'group_positions_' || v_call_id::text,
        v_template_id,
        v_arguments_raw,
        true,
        NOW(),
    ...
    INSERT INTO group_positions_resource(active, call_id, mcp)
    ...
    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_group_positions_v4.agent_id, v_run_id);
