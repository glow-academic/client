...
-- Parameters: agent_id (uuid, required, first), group_id (uuid, required, second), description (text)
...
CREATE OR REPLACE FUNCTION api_create_descriptions_v4(agent_id uuid,
    group_id uuid,
    description text,
...
BEGIN
    ...
    IF v_tool_id IS NULL THEN
        RAISE EXCEPTION 'Agent % does not have tool for resource descriptions', agent_id;
    END IF;
    -- Validate agent has mcp flag when mcp=true
    IF mcp = true AND agent_id IS NOT NULL THEN
        IF NOT EXISTS (
            SELECT 1 FROM agent_flags_junction 
            WHERE agent_id = api_create_descriptions_v4.agent_id 
    ...
    -- Check if descriptions already exists (match on description)
    SELECT r.id INTO v_description_id
    FROM descriptions_resource r
    WHERE r.description = api_create_descriptions_v4.description
    LIMIT 1;
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_descriptions_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_descriptions_v4.agent_id, v_run_id);
