...
-- Get or create operation (returns existing ID if agent_id + group_id already exists)
...
BEGIN
    ...
    WHERE at.agent_id = api_create_agents_v4.agent_id
      AND rt.resource = 'agents'::resource_type
      AND at.active = true
    ...
    -- Check if agents already exists (match on agent_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM agents_resource r
    WHERE r.agent_id = v_artifact_id
      AND r.group_id = api_create_agents_v4.group_id
    LIMIT 1;
    ...
    INSERT INTO agents_resource(id, agent_id, active, generated, mcp, call_id, group_id, created_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_agents_v4.group_id, NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_agents_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_agents_v4.agent_id, v_run_id);
