...
-- Get or create operation (returns existing ID if setting_id + group_id already exists)
...
BEGIN
    ...
    WHERE at.agent_id = api_create_settings_v4.agent_id
      AND rt.resource = 'settings'::resource_type
      AND at.active = true
    ...
    -- Check if settings already exists (match on setting_id + group_id)
    SELECT r.id INTO v_resource_id
    FROM settings_resource r
    WHERE r.setting_id = v_artifact_id
      AND r.group_id = api_create_settings_v4.group_id
    LIMIT 1;
    ...
    -- INSERT INTO settings_resource table (always insert, never update)
    -- INSERT INTO settings_resource table (always insert, never update)
    -- Create resource with new unique id and setting_id FK
    INSERT INTO settings_resource(id, setting_id, active, generated, mcp, call_id, group_id, created_at)
    VALUES (uuidv7(), v_artifact_id, true, true, mcp, v_call_id, api_create_settings_v4.group_id, NOW())
    ...
    VALUES (v_run_id, 0, 0, 0, api_create_settings_v4.group_id, NOW(), NOW());

    -- Link agent to run
    INSERT INTO agent_runs_junction (agent_id, run_id) VALUES (api_create_settings_v4.agent_id, v_run_id);
