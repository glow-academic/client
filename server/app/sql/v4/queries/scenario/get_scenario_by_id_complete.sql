...
SELECT 
    ...
    (SELECT st.parent_id FROM scenario_tree_junction st WHERE st.child_id = s.id AND st.parent_id != s.id LIMIT 1) as root_scenario_id,
    (SELECT st.parent_id FROM scenario_tree_junction st WHERE st.child_id = s.id AND st.parent_id != s.id LIMIT 1) as parent_scenario_id,
    s.created_at,
    s.updated_at,
