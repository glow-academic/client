...
SELECT 
    ...
    (SELECT sr.parent_id FROM scenario_scenarios_junction ssj JOIN scenarios_resource sr ON sr.id = ssj.scenarios_id WHERE ssj.scenario_id = s.id LIMIT 1) as root_scenario_id,
    (SELECT sr.parent_id FROM scenario_scenarios_junction ssj JOIN scenarios_resource sr ON sr.id = ssj.scenarios_id WHERE ssj.scenario_id = s.id LIMIT 1) as parent_scenario_id,
    s.created_at,
    s.updated_at,
