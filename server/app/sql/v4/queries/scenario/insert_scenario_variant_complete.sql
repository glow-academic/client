...
SELECT 
    ...
    p.images_enabled,
    ''::text as description,
    (SELECT st.parent_id FROM scenario_tree_junction st WHERE st.child_id = ns.id AND st.parent_id != ns.id LIMIT 1) as root_scenario_id,
    (SELECT st.parent_id FROM scenario_tree_junction st WHERE st.child_id = ns.id AND st.parent_id != ns.id LIMIT 1) as parent_scenario_id,
