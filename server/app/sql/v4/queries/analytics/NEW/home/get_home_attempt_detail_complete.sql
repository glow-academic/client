-- Get home attempt detail using three-MV architecture
-- Three queries for parallel execution:
-- 1. api_get_home_attempt_v4 - Attempt-level data + resource joins
-- 2. api_get_home_attempt_chats_v4 - Chat-level data + resource joins
-- 3. api_get_home_attempt_messages_v4 - Message-level data (arrays already denormalized)
--
-- Dependencies: mv_home_attempts, mv_home_chats, mv_home_messages
...
DECLARE
    ...
    func_names text[] := ARRAY[
        'api_get_home_attempt_v4',
        'api_get_home_attempt_chats_v4',
        'api_get_home_attempt_messages_v4'
    ];
    func_name text;
BEGIN
...
CREATE OR REPLACE FUNCTION api_get_home_attempt_v4(
    attempt_id uuid,
    profile_id uuid
)
RETURNS TABLE (
...
attempt_exists_check AS (
    SELECT EXISTS(
        SELECT 1 FROM mv_home_attempts WHERE attempt_id = (SELECT attempt_id FROM params)
...
CREATE OR REPLACE FUNCTION api_get_home_attempt_chats_v4(
    attempt_id uuid
)
RETURNS TABLE (
    -- Chat data (from MV)
...
FROM mv_home_chats c
WHERE c.attempt_id = attempt_id
ORDER BY c.chat_position
$$;

-- ============================================================================
-- Query 3: Messages data (arrays already denormalized in MV)
-- ============================================================================

CREATE OR REPLACE FUNCTION api_get_home_attempt_messages_v4(
    attempt_id uuid
)
RETURNS TABLE (
