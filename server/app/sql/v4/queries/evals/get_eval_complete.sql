...
RETURNS TABLE (
    ...
    -- Single-select resources: view_groups_entry flag
    groups_flag_id uuid,
...
active_flag_resource_data AS (
    SELECT 
        ...
        (
            SELECT ROW(f.id, f.name, f.description, f.icon, COALESCE(f.generated, false))::types.q_get_eval_v4_flag_resource 
            FROM (
...
-- Flag resource data for view_groups_entry flag
groups_flag_resource_data AS (
    SELECT 
        COALESCE(
            (SELECT df.flags_id FROM flags_drafts_connection df JOIN flags_resource f ON df.flags_id = f.id WHERE df.draft_id = (SELECT draft_id FROM params) AND f.name = '' LIMIT 1),
        ...
        (
            ...
            FROM (
                ...
                FROM flags_drafts_connection df 
                JOIN flags_resource f ON df.flags_id = f.id 
                WHERE df.draft_id = (SELECT draft_id FROM params) AND f.name = ''
                UNION ALL
                SELECT f.id, f.name, f.description, f.icon, COALESCE(f.generated, false) as generated, 2 as priority
                FROM eval_flags_junction ef 
                JOIN flags_resource f ON ef.flag_id = f.id 
                WHERE ef.eval_id = (SELECT eval_id FROM params) AND f.name = '' AND ef.value = TRUE
            ) f
            ORDER BY priority
            LIMIT 1
...
-- Agent selection for 'flags' resource (for active, dynamic, view_groups_entry flags)
...
user_department_ids_for_rubrics AS (
    SELECT ARRAY_AGG(ddj.department_id) as ids
    FROM params x
    JOIN departments_resource d ON EXISTS (SELECT 1 FROM department_flags_junction df JOIN flags_resource f ON df.flag_id = f.id WHERE df.department_id = d.id AND f.name = 'department_active' AND df.value = true)
    JOIN department_departments_junction ddj ON ddj.departments_id = d.id
    JOIN profile_departments_junction pd ON ddj.department_id = pd.department_id AND pd.profile_id = x.profile_id AND pd.active = true
),
...
-- Available model view_runs_entry query (adapted from get_eval_detail_complete.sql)
...
-- Available view_groups_entry query (filtered by group_search)
groups_base AS (
    SELECT
        g.id as group_id,
        g.created_at,
    ...
    LEFT JOIN view_runs_entry r ON r.group_id = g.id
