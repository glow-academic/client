...
BEGIN
    ...
    IF (
        ...
        IF COALESCE(array_length(v_profile_ids, 1), 0) > 0 AND (profiles).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'cohort_draft_create_profiles_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF COALESCE(array_length(v_profile_ids, 1), 0) > 0 AND (profiles).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'cohort_draft_link_profiles_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF COALESCE(array_length(v_profile_persona_ids, 1), 0) > 0 AND (profile_personas).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'cohort_draft_create_profile_personas_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF COALESCE(array_length(v_profile_persona_ids, 1), 0) > 0 AND (profile_personas).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'cohort_draft_link_profile_personas_' || v_call_id::text, v_run_id, true, NOW(), NOW());
