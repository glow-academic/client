...
BEGIN
    FOR r IN 
        ...
        FROM pg_proc 
        WHERE proname = 'api_process_document_csv_v4'
          AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS api_process_document_csv_v4(%s)', r.sig);
    END LOOP;
END $$;
...
BEGIN
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname = 'q_process_document_csv_v4_processed_row'
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE typname = 'q_process_document_csv_v4_csv_row_error'
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
    ...
    FOR r IN 
        SELECT typname 
        FROM pg_type 
        WHERE (typname LIKE 'q_process_document_csv_v4_column_mapping' OR typname LIKE 'i_process_document_csv_v4_column_mapping')
          AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'types')
    LOOP
        EXECUTE format('DROP TYPE IF EXISTS types.%I', r.typname);
    END LOOP;
...
CREATE TYPE types.i_process_document_csv_v4_column_mapping AS (
    csv_column text,
    target_field text  -- NULL if not mapped
);

-- Output type for column mapping (for consistency)
CREATE TYPE types.q_process_document_csv_v4_column_mapping AS (
    csv_column text,
    target_field text  -- NULL if not mapped
);

CREATE TYPE types.q_process_document_csv_v4_csv_row_error AS (
    row_index integer,
    field text,
    message text
);
...
CREATE TYPE types.q_process_document_csv_v4_processed_row AS (
    ...
    department_ids text[],
    cohort_ids text[],
    errors types.q_process_document_csv_v4_csv_row_error[]
);
...
CREATE OR REPLACE FUNCTION api_process_document_csv_v4(
    csv_content text,  -- Raw CSV content from frontend
    column_mappings types.i_process_document_csv_v4_column_mapping[],  -- Column mappings from frontend
    profile_id uuid
)
RETURNS TABLE (
    success boolean,
