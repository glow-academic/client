...
BEGIN
    ...
    IF v_run_id IS NOT NULL AND v_name_id IS NOT NULL THEN
        IF (names).create_tool_id IS NOT NULL THEN
            ...
            VALUES (v_call_id, 'department_create_names_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF (names).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_link_names_' || v_call_id::text, v_run_id, true, NOW(), NOW());
    ...
    IF v_run_id IS NOT NULL AND v_description_id IS NOT NULL THEN
        IF (descriptions).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_create_descriptions_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF (descriptions).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_link_descriptions_' || v_call_id::text, v_run_id, true, NOW(), NOW());
    ...
    IF v_run_id IS NOT NULL AND v_active_flag_id IS NOT NULL THEN
        IF (flags).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_create_flags_' || v_call_id::text, v_run_id, true, NOW(), NOW());
        ...
        IF (flags).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_link_flags_' || v_call_id::text, v_run_id, true, NOW(), NOW());
    ...
    IF v_run_id IS NOT NULL AND COALESCE(array_length(v_settings_ids, 1), 0) > 0 THEN
        IF (settings).create_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_create_settings_' || v_call_id::text, v_run_id, true, NOW(), NOW());
            INSERT INTO tool_calls_junction (tool_id, call_id) VALUES ((settings).create_tool_id, v_call_id);
        ...
        IF (settings).link_tool_id IS NOT NULL THEN
            v_call_id := uuidv7();
            INSERT INTO calls_entry (id, external_call_id, run_id, completed, created_at, updated_at)
            VALUES (v_call_id, 'department_link_settings_' || v_call_id::text, v_run_id, true, NOW(), NOW());
            INSERT INTO tool_calls_junction (tool_id, call_id) VALUES ((settings).link_tool_id, v_call_id);
            INSERT INTO settings_calls_connection (settings_id, call_id)
            SELECT setting_id, v_call_id FROM unnest(v_settings_ids) AS setting_id;
        END IF;
    END IF;
