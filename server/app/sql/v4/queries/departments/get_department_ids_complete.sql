...
BEGIN
    ...
    CREATE TYPE department_candidate_agent AS (
        agent_id uuid,
        agent_name text,
        tool_resources text[],
...
domain_ids_data AS (
    SELECT
        ...
        (SELECT id FROM domains_resource WHERE resource = 'settings'::resource_type AND active = true LIMIT 1) as settings_domain_id
)
SELECT
    ...
    -- Candidate agents (for Python-side agent scoring)
    (SELECT COALESCE(
        ARRAY_AGG(ROW(ca.agent_id, ca.agent_name, ca.tool_resources, ca.create_tool_ids, ca.link_tool_ids, ca.department_ids, ca.updated_at, ca.is_mcp)::department_candidate_agent),
        ARRAY[]::department_candidate_agent[]
    ) FROM candidate_agents_data ca) as candidate_agents,

    -- Tools existence
    tec.names_has_tools,
    tec.settings_has_tools,

    -- Domain IDs
    did.name_domain_id,
    did.description_domain_id,
    did.flag_domain_id,
    did.settings_domain_id
FROM params x
