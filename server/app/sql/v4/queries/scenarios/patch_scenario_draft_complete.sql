...
-- Ensure scenario draft composite types exist before function creation.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
    ...
    IF input_draft_id IS NOT NULL THEN
        ...
        IF v_draft_id IS NOT NULL THEN
            ...
            IF active_flag_id IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_flags_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
            ...
            IF department_ids IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_departments_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
            ...
            IF persona_ids IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_personas_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
            ...
            IF document_ids IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_documents_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
            ...
            IF image_ids IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_images_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
            ...
            IF video_ids IS NOT NULL THEN
                ...
                ON CONFLICT ON CONSTRAINT scenario_drafts_videos_connection_pkey DO UPDATE
                SET version = v_new_version;
            END IF;
    ...
    IF v_group_id IS NOT NULL THEN
        ...
        IF COALESCE(array_length(flag_ids, 1), 0) > 0 THEN
            IF (flags).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_flags_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (flags).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_flags_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(department_ids, 1), 0) > 0 THEN
            IF (departments).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_departments_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (departments).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_departments_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(persona_ids, 1), 0) > 0 THEN
            IF (personas).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_personas_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (personas).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_personas_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(document_ids, 1), 0) > 0 THEN
            IF (documents).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_documents_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (documents).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_documents_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(parameter_ids, 1), 0) > 0 THEN
            IF (parameters).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_parameters_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (parameters).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_parameters_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(parameter_field_ids, 1), 0) > 0 THEN
            IF (parameter_fields).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_parameter_fields_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (parameter_fields).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_parameter_fields_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(image_ids, 1), 0) > 0 THEN
            IF (images).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_images_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (images).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_images_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(video_ids, 1), 0) > 0 THEN
            IF (videos).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_videos_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (videos).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_videos_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(question_ids, 1), 0) > 0 THEN
            IF (questions).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_questions_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (questions).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_questions_' || v_call_id::text, v_run_id, NOW());
        ...
        IF COALESCE(array_length(option_ids, 1), 0) > 0 THEN
            IF (options).create_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_create_options_' || v_call_id::text, v_run_id, NOW());
            ...
            IF (options).link_tool_id IS NOT NULL THEN
                v_call_id := uuidv7();
                INSERT INTO calls_entry (id, external_call_id, run_id, created_at)
                VALUES (v_call_id, 'scenario_draft_link_options_' || v_call_id::text, v_run_id, NOW());
