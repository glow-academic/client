...
scenario_options_cte AS (
    ...
    JOIN scenarios_resource s ON s.id = acs.scenarios_id
...
attempt_scenario_ids AS (
    SELECT DISTINCT
        sc.attempt_id,
        ARRAY_AGG(DISTINCT ssj_scj.scenario_id) FILTER (WHERE ssj_scj.scenario_id IS NOT NULL) AS scenario_ids
    FROM all_chats sc
...
score_for_sort AS (
    ...
    LEFT JOIN scenario_rubrics_resource srr ON srr.scenario_id = ssj_scj.scenario_id
...
searchable_rows AS (
    ...
    WHERE
        ...
        OR EXISTS (
            ...
            JOIN all_chat_scenarios acs_scj ON acs_scj.chat_id = sc.id
            JOIN scenario_scenarios_junction ssj_scj ON ssj_scj.scenarios_id = acs_scj.scenarios_id
            JOIN scenario_personas_junction sp ON sp.scenario_id = ssj_scj.scenario_id AND sp.active = TRUE
...
history_chat_rollup AS (
    SELECT
        ...
        array_agg(DISTINCT ssj_scj.scenario_id) FILTER (WHERE ssj_scj.scenario_id IS NOT NULL) AS scenario_ids_seen
    FROM all_chats sc
    LEFT JOIN all_chat_scenarios acs_scj ON acs_scj.chat_id = sc.id
    LEFT JOIN scenario_scenarios_junction ssj_scj ON ssj_scj.scenarios_id = acs_scj.scenarios_id
...
history_grade_rollup AS (
    SELECT
        ...
        SUM(CASE WHEN hcg.score IS NOT NULL AND (SELECT p.value FROM scenario_rubrics_resource srr JOIN rubric_points_junction rp ON rp.rubric_id = srr.rubric_id AND rp.type = 'total'::point_type JOIN points_resource p ON p.id = rp.point_id WHERE srr.scenario_id = ssj_scj.scenario_id LIMIT 1) > 0
            THEN TRUNC(hcg.score / (SELECT p.value FROM scenario_rubrics_resource srr JOIN rubric_points_junction rp ON rp.rubric_id = srr.rubric_id AND rp.type = 'total'::point_type JOIN points_resource p ON p.id = rp.point_id WHERE srr.scenario_id = ssj_scj.scenario_id LIMIT 1)::numeric * 100.0, 2)
...
history_personas AS (
    ...
    JOIN all_chat_scenarios acs_scj ON acs_scj.chat_id = sc.id
    JOIN scenario_scenarios_junction ssj_scj ON ssj_scj.scenarios_id = acs_scj.scenarios_id
    LEFT JOIN scenario_personas_junction sp ON sp.scenario_id = ssj_scj.scenario_id AND sp.active = TRUE
    WHERE sc.attempt_id IN (SELECT attempt_id FROM paginated_ids)
    GROUP BY sc.attempt_id
),
...
history_first_scenario AS (
    SELECT DISTINCT ON (sc.attempt_id)
        sc.attempt_id,
        ssj_scj.scenario_id AS practice_scenario_id
    FROM all_chats sc
    JOIN all_chat_scenarios acs_scj ON acs_scj.chat_id = sc.id
    JOIN scenario_scenarios_junction ssj_scj ON ssj_scj.scenarios_id = acs_scj.scenarios_id
    WHERE sc.attempt_id IN (SELECT attempt_id FROM paginated_ids)
