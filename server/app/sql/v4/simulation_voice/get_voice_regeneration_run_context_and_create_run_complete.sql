...
documents_data AS (
    ...
    SELECT 
        ...
        COALESCE(
            ARRAY_AGG(
                (d.id::text, (SELECT n.name FROM document_names dn JOIN names_resource n ON dn.name_id = n.id WHERE dn.document_id = d.id LIMIT 1), COALESCE(u.file_path, ''), COALESCE(u.mime_type, ''), d.created_at)::types.i_get_voice_regeneration_run_context_and_create_run_v4_document
                ORDER BY d.created_at
            ) FILTER (WHERE d.id IS NOT NULL AND sd.active = true AND EXISTS (SELECT 1 FROM document_flags df JOIN flags_resource f ON df.flag_id = f.id WHERE df.document_id = d.id AND f.name = 'active' AND df.value = true)),
...
context_data AS (
    ...
    SELECT 
        ...
        (SELECT p_persona.id FROM scenario_personas sp 
         JOIN personas_resource p_persona ON p_persona.id = sp.persona_id 
         WHERE sp.scenario_id = s.id AND sp.active = true 
           AND EXISTS (SELECT 1 FROM persona_flags pf JOIN flags_resource f ON pf.flag_id = f.id WHERE pf.persona_id = p_persona.id AND f.name = 'active' AND pf.value = true)
         LIMIT 1) as persona_id,
        (SELECT (SELECT n.name FROM persona_names pn JOIN names_resource n ON pn.name_id = n.id WHERE pn.persona_id = p_persona.id LIMIT 1) FROM scenario_personas sp 
         JOIN personas_resource p_persona ON p_persona.id = sp.persona_id 
         WHERE sp.scenario_id = s.id AND sp.active = true 
           AND EXISTS (SELECT 1 FROM persona_flags pf JOIN flags_resource f ON pf.flag_id = f.id WHERE pf.persona_id = p_persona.id AND f.name = 'active' AND pf.value = true)
        ...
        (SELECT m_voice.id FROM agent_artifact a_voice 
        ...
        (SELECT m_voice.value FROM agent_artifact a_voice 
         JOIN agent_models am_voice ON am_voice.agent_id = a_voice.id
         JOIN models_resource m_voice ON m_voice.id = am_voice.model_id
    ...
    LEFT JOIN endpoints_resource e_voice ON e_voice.id = me_voice_j.endpoint_id AND e_voice.active = true
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_custom_model,
        (SELECT p_voice_prov.id::text FROM agent_artifact a_voice 
         JOIN agent_models am_voice ON am_voice.agent_id = a_voice.id
         JOIN models_resource m_voice ON m_voice.id = am_voice.model_id
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_provider_id,
        (SELECT n_voice_prov.name FROM agent_artifact a_voice 
         JOIN agent_models am_voice ON am_voice.agent_id = a_voice.id
         JOIN models_resource m_voice ON m_voice.id = am_voice.model_id
         LEFT JOIN model_providers mp_voice ON mp_voice.model_id = m_voice.id
         LEFT JOIN providers_resource p_voice_prov ON p_voice_prov.id = mp_voice.providers_id
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_provider,
        (SELECT e_voice.base_url FROM agent_artifact a_voice 
         JOIN agent_models am_voice ON am_voice.agent_id = a_voice.id
         JOIN models_resource m_voice ON m_voice.id = am_voice.model_id
         LEFT JOIN model_endpoints me_voice_j ON me_voice_j.model_id = m_voice.id
    LEFT JOIN endpoints_resource e_voice ON e_voice.id = me_voice_j.endpoint_id AND e_voice.active = true
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_base_url,
        ...
        (SELECT kr_voice.key FROM agent_artifact a_voice 
         ...
         LEFT JOIN keys_resource kr_voice ON kr_voice.id = spk_voice.key_id 
             AND EXISTS (SELECT 1 FROM key_flags kf JOIN flags_resource f ON kf.flag_id = f.id WHERE kf.key_id = kr_voice.id AND f.name = 'active' AND kf.value = true)
        ...
        (SELECT COALESCE(pr_prompt_voice_dept.system_prompt, pr_prompt_voice_default.system_prompt, '')
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_system_prompt,
        (SELECT COALESCE(tl_voice.temperature, 0.0)
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_temperature,
        (SELECT rl_voice.reasoning_level
         ...
         WHERE false 
           AND EXISTS (SELECT 1 FROM agent_flags af JOIN flags_resource f ON af.flag_id = f.id WHERE af.agent_id = a_voice.id AND f.name = 'active' AND af.value = true)) as voice_reasoning,
    ...
    LEFT JOIN setting_artifact st ON st.id = ast.settings_id
