...
SELECT
    ...
    mc.scenario_id AS scenario_id,
    mc.scenario_id AS scenarios_id,
...
LEFT JOIN mv_attempt_chats mc ON mc.chat_id = c.id
LEFT JOIN scenario_names_junction snj ON snj.scenario_id = mc.scenario_id
LEFT JOIN names_resource sn ON sn.id = snj.name_id
LEFT JOIN scenario_departments_junction sd
       ON sd.scenario_id = mc.scenario_id
      AND sd.active = TRUE
LEFT JOIN scenario_problem_statements_junction sps
       ON sps.scenario_id = mc.scenario_id
      AND sps.active = TRUE
LEFT JOIN problem_statements_resource ps ON ps.id = sps.problem_statement_id
LEFT JOIN scenario_personas_junction spj ON spj.scenario_id = mc.scenario_id
...
LEFT JOIN LATERAL (
    ...
    WHERE sfj2.scenario_id = mc.scenario_id
