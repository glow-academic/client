...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_suite'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS mv_suite CASCADE;

CREATE MATERIALIZED VIEW mv_suite AS
WITH
department_agg AS (
    SELECT
        bbdc.suite_id,
...
CREATE UNIQUE INDEX mv_suite_pk
    ON mv_suite (suite_entry_id);

CREATE INDEX mv_suite_benchmark_id_idx
    ON mv_suite (benchmark_id);

CREATE INDEX mv_suite_department_ids_gin_idx
    ON mv_suite USING GIN (department_ids);

REFRESH MATERIALIZED VIEW mv_suite;
