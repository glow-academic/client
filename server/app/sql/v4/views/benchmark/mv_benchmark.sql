-- Materialized View: mv_benchmark
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_benchmark'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS mv_benchmark CASCADE;

CREATE MATERIALIZED VIEW mv_benchmark AS
WITH
...
CREATE UNIQUE INDEX mv_benchmark_pk
    ON mv_benchmark (benchmark_id);

CREATE INDEX mv_benchmark_use_groups_idx
    ON mv_benchmark (use_groups);

CREATE INDEX mv_benchmark_dynamic_idx
    ON mv_benchmark (dynamic);

CREATE INDEX mv_benchmark_eval_ids_gin_idx
    ON mv_benchmark USING GIN (eval_ids);

CREATE INDEX mv_benchmark_department_ids_gin_idx
    ON mv_benchmark USING GIN (department_ids);

CREATE INDEX mv_benchmark_profile_ids_gin_idx
    ON mv_benchmark USING GIN (profile_ids);

REFRESH MATERIALIZED VIEW mv_benchmark;
