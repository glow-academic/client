-- Materialized View: test_invocations_mv
...
-- parallel fetching. Follows attempt_chats_mv pattern: resource-ID-rich
-- via bundle_snapshot, grade data denormalized. Feedbacks fetched separately
-- via test_feedbacks_mv using grade_id.
--
...
-- ============================================================================
-- Step 1: Drop all indexes on test_invocations_mv materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'test_invocations_mv'
    LOOP
...
-- ============================================================================
-- Step 2: Drop test_invocations_mv materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS test_invocations_mv CASCADE;

-- ============================================================================
-- Step 3: Create test_invocations_mv Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW test_invocations_mv AS
WITH
...
-- Analogous to subbundle_snapshot in attempt_chats_mv
-- ============================================================================
bundle_snapshot AS (
    SELECT
...
CREATE UNIQUE INDEX test_invocations_mv_pk
    ON test_invocations_mv (invocation_id);
...
CREATE INDEX test_invocations_mv_test_id_idx
    ON test_invocations_mv (test_id);

-- Completion status for filtering
CREATE INDEX test_invocations_mv_completed_idx
    ON test_invocations_mv (invocation_completed);

-- Timestamp for sorting
CREATE INDEX test_invocations_mv_created_at_idx
    ON test_invocations_mv (invocation_created_at DESC);

-- Group ID for filtering
CREATE INDEX test_invocations_mv_group_id_idx
    ON test_invocations_mv (group_id)
    WHERE group_id IS NOT NULL;

-- Bundle department ID for filtering
CREATE INDEX test_invocations_mv_bbd_id_idx
    ON test_invocations_mv (suite_department_id)
    WHERE suite_department_id IS NOT NULL;

-- Invocation run IDs for filtering by execution run
CREATE INDEX test_invocations_mv_invocation_run_ids_gin
    ON test_invocations_mv USING GIN (invocation_run_ids);

-- Configured run IDs for filtering by template run
CREATE INDEX test_invocations_mv_run_ids_gin
    ON test_invocations_mv USING GIN (run_ids);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW test_invocations_mv;
