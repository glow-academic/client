-- Materialized View: mv_test_invocations
...
-- via mv_test_feedbacks using grade_id.
--
...
-- ============================================================================
-- Step 1: Drop all indexes on mv_test_invocations materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_test_invocations'
    LOOP
...
-- ============================================================================
-- Step 2: Drop mv_test_invocations materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_test_invocations CASCADE;

-- ============================================================================
-- Step 3: Create mv_test_invocations Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_test_invocations AS
WITH
...
CREATE UNIQUE INDEX mv_test_invocation_pk
    ON mv_test_invocations (invocation_id);
...
CREATE INDEX mv_test_invocation_test_id_idx
    ON mv_test_invocations (test_id);

-- Completion status for filtering
CREATE INDEX mv_test_invocation_completed_idx
    ON mv_test_invocations (invocation_completed);

-- Timestamp for sorting
CREATE INDEX mv_test_invocation_created_at_idx
    ON mv_test_invocations (invocation_created_at DESC);

-- Group ID for filtering
CREATE INDEX mv_test_invocation_group_id_idx
    ON mv_test_invocations (group_id)
    WHERE group_id IS NOT NULL;

-- Bundle department ID for filtering
CREATE INDEX mv_test_invocation_bbd_id_idx
    ON mv_test_invocations (suite_department_id)
    WHERE suite_department_id IS NOT NULL;

-- Invocation run IDs for filtering by execution run
CREATE INDEX mv_test_invocation_invocation_run_ids_gin
    ON mv_test_invocations USING GIN (invocation_run_ids);

-- Configured run IDs for filtering by template run
CREATE INDEX mv_test_invocation_run_ids_gin
    ON mv_test_invocations USING GIN (run_ids);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW mv_test_invocations;
