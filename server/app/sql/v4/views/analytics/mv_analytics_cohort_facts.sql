-- Materialized View: mv_cohort_facts
-- Cohort section fact table for dashboard analytics.
--
-- Grain: One row per chat
-- Filter: None at MV level - all data included (filtering done at query time)
--
-- Purpose: Supports three cohort section widgets:
--   1. Cohort Progress (GROUP BY cohort_id, attempt_date, simulation_id)
--   2. Persona Performance (GROUP BY persona_id, cohort_id)
--   3. Attempt Improvement (GROUP BY attempt_number, simulation_id, cohort_id)
--
-- Section: ANALYTICS (cohort section)
--
-- Dependencies: Uses entry tables only (self-contained, no MV dependencies)

-- ============================================================================
-- Step 1: Drop all indexes on mv_cohort_facts materialized view (if it exists)
-- ============================================================================
...
-- ============================================================================
-- Step 2: Drop mv_cohort_facts materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_cohort_facts CASCADE;

-- ============================================================================
-- Step 3: Create mv_cohort_facts Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_cohort_facts AS
WITH
latest_grade AS (
...
CREATE UNIQUE INDEX mv_cohort_facts_pk
    ON mv_cohort_facts (chat_id);
...
CREATE INDEX mv_cohort_facts_profile_id_idx
    ON mv_cohort_facts (profile_id);

CREATE INDEX mv_cohort_facts_persona_id_idx
    ON mv_cohort_facts (persona_id)
    WHERE persona_id IS NOT NULL;

CREATE INDEX mv_cohort_facts_attempt_id_idx
    ON mv_cohort_facts (attempt_id);

-- Time index
CREATE INDEX mv_cohort_facts_attempt_date_idx
    ON mv_cohort_facts (attempt_date DESC);

-- Flag indexes
CREATE INDEX mv_cohort_facts_attempt_type_idx
    ON mv_cohort_facts (attempt_type);

CREATE INDEX mv_cohort_facts_is_archived_idx
    ON mv_cohort_facts (is_archived);

-- Composite indexes for common query patterns

-- Cohort progress: cohort + date + simulation
CREATE INDEX mv_cohort_facts_cohort_date_sim_idx
    ON mv_cohort_facts (cohort_id, attempt_date DESC, simulation_id)
    WHERE cohort_id IS NOT NULL;

-- Persona performance: persona + cohort
CREATE INDEX mv_cohort_facts_persona_cohort_idx
    ON mv_cohort_facts (persona_id, cohort_id)
    WHERE persona_id IS NOT NULL;

-- Attempt improvement: profile + simulation + attempt_number
CREATE INDEX mv_cohort_facts_profile_sim_attempt_idx
    ON mv_cohort_facts (profile_id, simulation_id, attempt_number);

-- Default filter: non-archived general
CREATE INDEX mv_cohort_facts_default_idx
    ON mv_cohort_facts (cohort_id, attempt_date DESC)
    WHERE attempt_type = 'general' AND is_archived = FALSE;

-- Profile + type + archived (common dashboard filter combo)
CREATE INDEX mv_cohort_facts_profile_type_archived_idx
    ON mv_cohort_facts (profile_id, attempt_type, is_archived, attempt_date DESC);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW mv_cohort_facts;
