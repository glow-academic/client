-- Materialized View: mv_scenario_facts
-- Scenario/footer section fact table for dashboard analytics.
--
...
-- ============================================================================
-- Step 1: Drop all indexes on mv_scenario_facts materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_scenario_facts'
    LOOP
...
-- ============================================================================
-- Step 2: Drop mv_scenario_facts materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_scenario_facts CASCADE;

-- ============================================================================
-- Step 3: Create mv_scenario_facts Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_scenario_facts AS
WITH
latest_grade AS (
...
CREATE UNIQUE INDEX mv_scenario_facts_pk
    ON mv_scenario_facts (chat_id);
...
-- Resource ID indexes
CREATE INDEX mv_scenario_facts_simulation_id_idx
    ON mv_scenario_facts (simulation_id);

CREATE INDEX mv_scenario_facts_scenario_id_idx
    ON mv_scenario_facts (scenario_id)
    WHERE scenario_id IS NOT NULL;

CREATE INDEX mv_scenario_facts_profile_id_idx
    ON mv_scenario_facts (profile_id);

CREATE INDEX mv_scenario_facts_cohort_id_idx
    ON mv_scenario_facts (cohort_id)
    WHERE cohort_id IS NOT NULL;

CREATE INDEX mv_scenario_facts_department_id_idx
    ON mv_scenario_facts (department_id)
    WHERE department_id IS NOT NULL;

CREATE INDEX mv_scenario_facts_persona_id_idx
    ON mv_scenario_facts (persona_id)
    WHERE persona_id IS NOT NULL;

CREATE INDEX mv_scenario_facts_attempt_id_idx
    ON mv_scenario_facts (attempt_id);

-- Time index
CREATE INDEX mv_scenario_facts_attempt_date_idx
    ON mv_scenario_facts (attempt_date DESC);

-- Flag indexes
CREATE INDEX mv_scenario_facts_attempt_type_idx
    ON mv_scenario_facts (attempt_type);

CREATE INDEX mv_scenario_facts_is_archived_idx
    ON mv_scenario_facts (is_archived);

-- Composite indexes for widget query patterns

-- Widget 1 (Simulation Performance): GROUP BY (simulation_id, scenario_id)
CREATE INDEX mv_scenario_facts_sim_scenario_idx
    ON mv_scenario_facts (simulation_id, scenario_id);

-- Widget 2 (Simulation Composition): GROUP BY attempt_id -> simulation_id
CREATE INDEX mv_scenario_facts_sim_attempt_idx
    ON mv_scenario_facts (simulation_id, attempt_id);

-- Widget 3/4 (Scenario Performance/Stats): scenario_id + date for daily grouping
CREATE INDEX mv_scenario_facts_scenario_date_idx
    ON mv_scenario_facts (scenario_id, attempt_date DESC)
    WHERE scenario_id IS NOT NULL;

-- Default filter: non-archived general
CREATE INDEX mv_scenario_facts_default_idx
    ON mv_scenario_facts (simulation_id, attempt_date DESC)
    WHERE attempt_type = 'general' AND is_archived = FALSE;

-- Profile + type + archived (common dashboard filter combo)
CREATE INDEX mv_scenario_facts_profile_type_archived_idx
    ON mv_scenario_facts (profile_id, attempt_type, is_archived, attempt_date DESC);

-- GIN index on document_ids for array queries
CREATE INDEX mv_scenario_facts_document_ids_gin_idx
    ON mv_scenario_facts USING GIN (document_ids);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW mv_scenario_facts;
