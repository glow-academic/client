-- Materialized View: mv_home_chat_facts
-- Base fact table for HOME section analytics.
--
-- Grain: One row per chat
-- Filter: attempt_type = 'general' AND is_archived = FALSE
--
-- This MV is INDEPENDENT - it does not depend on any other MVs.
-- Section: HOME (overview, history, certificate, attempt complete)
--
-- Dependencies: Only uses _entry and _connection tables (no _resource or _junction tables)
-- ============================================================================
-- Step 1: Drop all indexes on mv_home_chat_facts materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_home_chat_facts'
    LOOP
...
-- ============================================================================
-- Step 2: Drop mv_home_chat_facts materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_home_chat_facts CASCADE;

-- ============================================================================
-- Step 3: Create mv_home_chat_facts Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_home_chat_facts AS
WITH
...
SELECT
    ...
    -- Flags (attempt_type and is_archived filtered out - always 'general' and FALSE)
...
CREATE UNIQUE INDEX mv_home_chat_facts_pk
    ON mv_home_chat_facts (chat_id);
...
-- Entry ID indexes
CREATE INDEX mv_home_chat_facts_attempt_id_idx
    ON mv_home_chat_facts (attempt_id);

CREATE INDEX mv_home_chat_facts_grade_id_idx
    ON mv_home_chat_facts (grade_id)
    WHERE grade_id IS NOT NULL;

-- Resource ID indexes for filtering
CREATE INDEX mv_home_chat_facts_simulation_id_idx
    ON mv_home_chat_facts (simulation_id);

CREATE INDEX mv_home_chat_facts_profile_id_idx
    ON mv_home_chat_facts (profile_id);

CREATE INDEX mv_home_chat_facts_cohort_id_idx
    ON mv_home_chat_facts (cohort_id)
    WHERE cohort_id IS NOT NULL;

CREATE INDEX mv_home_chat_facts_department_id_idx
    ON mv_home_chat_facts (department_id)
    WHERE department_id IS NOT NULL;

CREATE INDEX mv_home_chat_facts_role_id_idx
    ON mv_home_chat_facts (role_id)
    WHERE role_id IS NOT NULL;

CREATE INDEX mv_home_chat_facts_scenario_id_idx
    ON mv_home_chat_facts (scenario_id);

CREATE INDEX mv_home_chat_facts_persona_id_idx
    ON mv_home_chat_facts (persona_id)
    WHERE persona_id IS NOT NULL;

CREATE INDEX mv_home_chat_facts_rubric_id_idx
    ON mv_home_chat_facts (rubric_id)
    WHERE rubric_id IS NOT NULL;

-- Timestamp indexes for date range filtering
CREATE INDEX mv_home_chat_facts_attempt_created_at_idx
    ON mv_home_chat_facts (attempt_created_at);

CREATE INDEX mv_home_chat_facts_chat_created_at_idx
    ON mv_home_chat_facts (chat_created_at);

CREATE INDEX mv_home_chat_facts_grade_created_at_idx
    ON mv_home_chat_facts (grade_created_at)
    WHERE grade_created_at IS NOT NULL;

-- Flag indexes
CREATE INDEX mv_home_chat_facts_completed_idx
    ON mv_home_chat_facts (completed);

CREATE INDEX mv_home_chat_facts_passed_idx
    ON mv_home_chat_facts (passed)
    WHERE passed IS NOT NULL;

-- Composite indexes for common query patterns
CREATE INDEX mv_home_chat_facts_profile_simulation_idx
    ON mv_home_chat_facts (profile_id, simulation_id);

CREATE INDEX mv_home_chat_facts_profile_cohort_idx
    ON mv_home_chat_facts (profile_id, cohort_id)
    WHERE cohort_id IS NOT NULL;

CREATE INDEX mv_home_chat_facts_profile_attempt_created_idx
    ON mv_home_chat_facts (profile_id, attempt_created_at DESC);

CREATE INDEX mv_home_chat_facts_cohort_simulation_idx
    ON mv_home_chat_facts (cohort_id, simulation_id)
    WHERE cohort_id IS NOT NULL;

-- Grade percent index for score-based filtering and sorting
CREATE INDEX mv_home_chat_facts_grade_percent_idx
    ON mv_home_chat_facts (grade_percent DESC NULLS LAST)
    WHERE grade_percent IS NOT NULL;

-- Composite index for profile + grade percent
CREATE INDEX mv_home_chat_facts_profile_grade_percent_idx
    ON mv_home_chat_facts (profile_id, grade_percent DESC NULLS LAST)
    WHERE grade_percent IS NOT NULL;

-- GIN index for message_time_taken_seconds array
CREATE INDEX mv_home_chat_facts_message_time_taken_gin
    ON mv_home_chat_facts USING GIN (message_time_taken_seconds);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW mv_home_chat_facts;
