-- Materialized View: mv_home_attempt_chats
-- Attempt complete view for HOME section - chat detail for single attempt.
--
-- Grain: One row per chat (for single attempt detail view)
-- Purpose: When viewing a completed general attempt, show all chats
--
-- Section: HOME
-- Source: Same as mv_home_chat_facts (identical structure for single-attempt queries)
--
-- Note: This MV is identical to mv_home_chat_facts. The primary key on chat_id
-- and index on attempt_id enables efficient single-attempt lookups.
-- Could be consolidated with mv_home_chat_facts if duplication is undesirable.
--
-- ============================================================================
-- Step 1: Drop all indexes on mv_home_attempt_chats materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_home_attempt_chats'
    LOOP
...
-- ============================================================================
-- Step 2: Drop mv_home_attempt_chats materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_home_attempt_chats CASCADE;

-- ============================================================================
-- Step 3: Create mv_home_attempt_chats Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_home_attempt_chats AS
WITH
...
CREATE UNIQUE INDEX mv_home_attempt_chats_pk
    ON mv_home_attempt_chats (chat_id);
...
CREATE INDEX mv_home_attempt_chats_attempt_id_idx
    ON mv_home_attempt_chats (attempt_id);

-- Profile filtering
CREATE INDEX mv_home_attempt_chats_profile_id_idx
    ON mv_home_attempt_chats (profile_id);

-- Simulation filtering
CREATE INDEX mv_home_attempt_chats_simulation_id_idx
    ON mv_home_attempt_chats (simulation_id);

-- Scenario filtering
CREATE INDEX mv_home_attempt_chats_scenario_id_idx
    ON mv_home_attempt_chats (scenario_id);

-- Persona filtering
CREATE INDEX mv_home_attempt_chats_persona_id_idx
    ON mv_home_attempt_chats (persona_id)
    WHERE persona_id IS NOT NULL;

-- Time-based sorting
CREATE INDEX mv_home_attempt_chats_chat_created_at_idx
    ON mv_home_attempt_chats (chat_created_at);

-- Composite: attempt + chat time for ordered chat list
CREATE INDEX mv_home_attempt_chats_attempt_chat_created_idx
    ON mv_home_attempt_chats (attempt_id, chat_created_at);

-- Completion status
CREATE INDEX mv_home_attempt_chats_completed_idx
    ON mv_home_attempt_chats (completed);

-- Pass status
CREATE INDEX mv_home_attempt_chats_passed_idx
    ON mv_home_attempt_chats (passed)
...
REFRESH MATERIALIZED VIEW mv_home_attempt_chats;
