...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_practice'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS mv_practice CASCADE;

CREATE MATERIALIZED VIEW mv_practice AS
WITH
...
CREATE UNIQUE INDEX mv_practice_pk
    ON mv_practice (practice_id);

CREATE INDEX mv_practice_simulation_ids_gin_idx
    ON mv_practice USING GIN (simulation_ids);

CREATE INDEX mv_practice_cohort_ids_gin_idx
    ON mv_practice USING GIN (cohort_ids);

CREATE INDEX mv_practice_profile_ids_gin_idx
    ON mv_practice USING GIN (profile_ids);

CREATE INDEX mv_practice_scenario_ids_gin_idx
    ON mv_practice USING GIN (scenario_ids);

REFRESH MATERIALIZED VIEW mv_practice;
