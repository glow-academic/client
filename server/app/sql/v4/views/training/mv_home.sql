...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_home'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS mv_home CASCADE;

CREATE MATERIALIZED VIEW mv_home AS
WITH
...
CREATE UNIQUE INDEX mv_home_pk
    ON mv_home (home_id);

CREATE INDEX mv_home_simulation_ids_gin_idx
    ON mv_home USING GIN (simulation_ids);

CREATE INDEX mv_home_cohort_ids_gin_idx
    ON mv_home USING GIN (cohort_ids);

CREATE INDEX mv_home_profile_ids_gin_idx
    ON mv_home USING GIN (profile_ids);

CREATE INDEX mv_home_scenario_ids_gin_idx
    ON mv_home USING GIN (scenario_ids);

REFRESH MATERIALIZED VIEW mv_home;
