-- Materialized View: mv_chats
-- Unified chat-grain fact table consolidating mv_profile_facts, mv_simulation_facts,
-- mv_scenario_facts, mv_scenario_facts, and mv_attempt_chats.
--
...
-- ============================================================================
-- Step 1: Drop all indexes on mv_chats materialized view (if it exists)
-- ============================================================================
...
-- ============================================================================
-- Step 2: Drop mv_chats materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS mv_chats CASCADE;

-- ============================================================================
-- Step 3: Create mv_chats Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW mv_chats AS
WITH
latest_grade AS (
...
CREATE UNIQUE INDEX mv_chats_pk
    ON mv_chats (chat_id);
...
-- Resource ID indexes
CREATE INDEX mv_chats_profile_id_idx
    ON mv_chats (profile_id);

CREATE INDEX mv_chats_cohort_id_idx
    ON mv_chats (cohort_id)
    WHERE cohort_id IS NOT NULL;

CREATE INDEX mv_chats_department_id_idx
    ON mv_chats (department_id)
    WHERE department_id IS NOT NULL;

CREATE INDEX mv_chats_simulation_id_idx
    ON mv_chats (simulation_id);

CREATE INDEX mv_chats_scenario_id_idx
    ON mv_chats (scenario_id)
    WHERE scenario_id IS NOT NULL;

CREATE INDEX mv_chats_user_persona_id_idx
    ON mv_chats (user_persona_id)
    WHERE user_persona_id IS NOT NULL;

CREATE INDEX mv_chats_rubric_id_idx
    ON mv_chats (rubric_id)
    WHERE rubric_id IS NOT NULL;

CREATE INDEX mv_chats_attempt_id_idx
    ON mv_chats (attempt_id);

CREATE INDEX mv_chats_training_department_id_idx
    ON mv_chats (training_department_id)
    WHERE training_department_id IS NOT NULL;

-- Time index
CREATE INDEX mv_chats_attempt_date_idx
    ON mv_chats (attempt_date DESC);

-- Flag indexes
CREATE INDEX mv_chats_attempt_type_idx
    ON mv_chats (attempt_type);

CREATE INDEX mv_chats_is_archived_idx
    ON mv_chats (is_archived);

-- Composite indexes for common query patterns

-- Profile aggregation: profile + date (header, leaderboard, reports)
CREATE INDEX mv_chats_profile_date_idx
    ON mv_chats (profile_id, attempt_date DESC);

-- Dashboard filter combo: profile + type + archived
CREATE INDEX mv_chats_profile_type_archived_idx
    ON mv_chats (profile_id, attempt_type, is_archived, attempt_date DESC);

-- Default filter: non-archived general
CREATE INDEX mv_chats_default_idx
    ON mv_chats (profile_id, attempt_date DESC)
    WHERE attempt_type = 'general' AND is_archived = FALSE;

-- Grade-based queries (for avg_score, highest_score rankings)
CREATE INDEX mv_chats_grade_idx
    ON mv_chats (grade_score DESC NULLS LAST)
    WHERE grade_score IS NOT NULL;

-- Cohort progress: cohort + date + simulation (secondary section)
CREATE INDEX mv_chats_cohort_date_sim_idx
    ON mv_chats (cohort_id, attempt_date DESC, simulation_id)
    WHERE cohort_id IS NOT NULL;

-- Persona performance: persona + cohort (secondary section)
CREATE INDEX mv_chats_persona_cohort_idx
    ON mv_chats (user_persona_id, cohort_id)
    WHERE user_persona_id IS NOT NULL;

-- Attempt improvement: profile + simulation + attempt_number (secondary section)
CREATE INDEX mv_chats_profile_sim_attempt_idx
    ON mv_chats (profile_id, simulation_id, attempt_number);

-- Simulation + scenario (footer section)
CREATE INDEX mv_chats_sim_scenario_idx
    ON mv_chats (simulation_id, scenario_id);

-- Scenario + date (footer section)
CREATE INDEX mv_chats_scenario_date_idx
    ON mv_chats (scenario_id, attempt_date DESC)
    WHERE scenario_id IS NOT NULL;

-- Rubric indexes (primary section)
CREATE INDEX mv_chats_rubric_chat_idx
    ON mv_chats (rubric_id, chat_id)
    WHERE rubric_id IS NOT NULL;
...
REFRESH MATERIALIZED VIEW mv_chats;
