-- Materialized View: attempt_chats_mv
-- Unified chat-grain fact table consolidating profile_facts_mv, simulation_facts_mv,
-- scenario_facts_mv, scenario_facts_mv, and attempt_chats_mv.
--
...
-- ============================================================================
-- Step 1: Drop all indexes on attempt_chats_mv materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'attempt_chats_mv'
    LOOP
...
-- ============================================================================
-- Step 2: Drop attempt_chats_mv materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS attempt_chats_mv CASCADE;

-- ============================================================================
-- Step 3: Create attempt_chats_mv Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW attempt_chats_mv AS
WITH
latest_grade AS (
...
CREATE UNIQUE INDEX attempt_chats_mv_pk
    ON attempt_chats_mv (chat_id);
...
-- Resource ID indexes
CREATE INDEX attempt_chats_mv_profile_id_idx
    ON attempt_chats_mv (profile_id);

CREATE INDEX attempt_chats_mv_cohort_id_idx
    ON attempt_chats_mv (cohort_id)
    WHERE cohort_id IS NOT NULL;

CREATE INDEX attempt_chats_mv_department_id_idx
    ON attempt_chats_mv (department_id)
    WHERE department_id IS NOT NULL;

CREATE INDEX attempt_chats_mv_simulation_id_idx
    ON attempt_chats_mv (simulation_id);

CREATE INDEX attempt_chats_mv_scenario_id_idx
    ON attempt_chats_mv (scenario_id)
    WHERE scenario_id IS NOT NULL;

CREATE INDEX attempt_chats_mv_user_persona_id_idx
    ON attempt_chats_mv (user_persona_id)
    WHERE user_persona_id IS NOT NULL;

CREATE INDEX attempt_chats_mv_rubric_id_idx
    ON attempt_chats_mv (rubric_id)
    WHERE rubric_id IS NOT NULL;

CREATE INDEX attempt_chats_mv_attempt_id_idx
    ON attempt_chats_mv (attempt_id);

CREATE INDEX attempt_chats_mv_training_department_id_idx
    ON attempt_chats_mv (training_department_id)
    WHERE training_department_id IS NOT NULL;

-- Time index
CREATE INDEX attempt_chats_mv_attempt_date_idx
    ON attempt_chats_mv (attempt_date DESC);

-- Flag indexes
CREATE INDEX attempt_chats_mv_attempt_type_idx
    ON attempt_chats_mv (attempt_type);

CREATE INDEX attempt_chats_mv_is_archived_idx
    ON attempt_chats_mv (is_archived);

-- Composite indexes for common query patterns

-- Profile aggregation: profile + date (header, leaderboard, reports)
CREATE INDEX attempt_chats_mv_profile_date_idx
    ON attempt_chats_mv (profile_id, attempt_date DESC);

-- Dashboard filter combo: profile + type + archived
CREATE INDEX attempt_chats_mv_profile_type_archived_idx
    ON attempt_chats_mv (profile_id, attempt_type, is_archived, attempt_date DESC);

-- Default filter: non-archived general
CREATE INDEX attempt_chats_mv_default_idx
    ON attempt_chats_mv (profile_id, attempt_date DESC)
    WHERE attempt_type = 'general' AND is_archived = FALSE;

-- Grade-based queries (for avg_score, highest_score rankings)
CREATE INDEX attempt_chats_mv_grade_idx
    ON attempt_chats_mv (grade_score DESC NULLS LAST)
    WHERE grade_score IS NOT NULL;

-- Cohort progress: cohort + date + simulation (secondary section)
CREATE INDEX attempt_chats_mv_cohort_date_sim_idx
    ON attempt_chats_mv (cohort_id, attempt_date DESC, simulation_id)
    WHERE cohort_id IS NOT NULL;

-- Persona performance: persona + cohort (secondary section)
CREATE INDEX attempt_chats_mv_persona_cohort_idx
    ON attempt_chats_mv (user_persona_id, cohort_id)
    WHERE user_persona_id IS NOT NULL;

-- Attempt improvement: profile + simulation + attempt_number (secondary section)
CREATE INDEX attempt_chats_mv_profile_sim_attempt_idx
    ON attempt_chats_mv (profile_id, simulation_id, attempt_number);

-- Simulation + scenario (footer section)
CREATE INDEX attempt_chats_mv_sim_scenario_idx
    ON attempt_chats_mv (simulation_id, scenario_id);

-- Scenario + date (footer section)
CREATE INDEX attempt_chats_mv_scenario_date_idx
    ON attempt_chats_mv (scenario_id, attempt_date DESC)
    WHERE scenario_id IS NOT NULL;

-- Rubric indexes (primary section)
CREATE INDEX attempt_chats_mv_rubric_chat_idx
    ON attempt_chats_mv (rubric_id, chat_id)
    WHERE rubric_id IS NOT NULL;
...
REFRESH MATERIALIZED VIEW attempt_chats_mv;
