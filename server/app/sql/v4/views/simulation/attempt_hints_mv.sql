-- Materialized View: attempt_hints_mv
...
-- ============================================================================
-- Step 1: Drop all indexes on attempt_hints_mv materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'attempt_hints_mv'
    LOOP
...
-- ============================================================================
-- Step 2: Drop attempt_hints_mv materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS attempt_hints_mv CASCADE;

-- ============================================================================
-- Step 3: Create attempt_hints_mv Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW attempt_hints_mv AS
SELECT
    h.id AS hint_id,
    h.message_id,
    h.hint,
...
CREATE UNIQUE INDEX attempt_hints_mv_pk
    ON attempt_hints_mv (hint_id);
...
CREATE INDEX attempt_hints_mv_message_id_idx
    ON attempt_hints_mv (message_id);

-- Composite: message + idx for ordered access
CREATE INDEX attempt_hints_mv_message_idx_idx
    ON attempt_hints_mv (message_id, idx);

-- ============================================================================
-- Step 6: Refresh Materialized View with Data
-- ============================================================================

REFRESH MATERIALIZED VIEW attempt_hints_mv;
