-- Materialized View: mv_draft_eval
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'mv_draft_eval'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS mv_draft_eval CASCADE;

CREATE MATERIALIZED VIEW mv_draft_eval AS
WITH draft_links AS (
...
WHERE d.artifact = 'eval'::artifact_type
GROUP BY d.id, d.created_at, d.updated_at, d.version, d.generated, d.mcp, d.active
WITH NO DATA;

CREATE UNIQUE INDEX mv_draft_eval_pk ON mv_draft_eval (draft_id);

REFRESH MATERIALIZED VIEW mv_draft_eval;
