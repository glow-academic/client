...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'rubric_drafts_mv'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS rubric_drafts_mv CASCADE;

CREATE MATERIALIZED VIEW rubric_drafts_mv AS
WITH draft_links AS (
...
CREATE UNIQUE INDEX rubric_drafts_mv_pk ON rubric_drafts_mv (draft_id);

REFRESH MATERIALIZED VIEW rubric_drafts_mv;
