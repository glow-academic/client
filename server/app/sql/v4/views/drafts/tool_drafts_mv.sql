...
WITH draft_links AS (
    ...
    UNION ALL SELECT draft_id, 'entries'::text AS resource_type, entries_id AS resource_id FROM tool_drafts_entries_connection WHERE active = true
    ...
    UNION ALL SELECT draft_id, 'resources'::text AS resource_type, resources_id AS resource_id FROM tool_drafts_resources_connection WHERE active = true
...
SELECT
    ...
    COALESCE(array_agg(DISTINCT l.resource_id) FILTER (WHERE l.resource_type = 'entries'), ARRAY[]::uuid[]) AS entry_ids,
    ...
    COALESCE(array_agg(DISTINCT l.resource_id) FILTER (WHERE l.resource_type = 'resources'), ARRAY[]::uuid[]) AS resource_ids,
