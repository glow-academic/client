-- Materialized View: draft_simulation_mv
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'draft_simulation_mv'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS draft_simulation_mv CASCADE;

CREATE MATERIALIZED VIEW draft_simulation_mv AS
WITH draft_links AS (
...
CREATE UNIQUE INDEX draft_simulation_mv_pk ON draft_simulation_mv (draft_id);

REFRESH MATERIALIZED VIEW draft_simulation_mv;
