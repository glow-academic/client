-- Materialized View: draft_parameter_mv
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'draft_parameter_mv'
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I', r.indexname);
    END LOOP;
END $$;

DROP MATERIALIZED VIEW IF EXISTS draft_parameter_mv CASCADE;

CREATE MATERIALIZED VIEW draft_parameter_mv AS
WITH draft_links AS (
...
CREATE UNIQUE INDEX draft_parameter_mv_pk ON draft_parameter_mv (draft_id);

REFRESH MATERIALIZED VIEW draft_parameter_mv;
