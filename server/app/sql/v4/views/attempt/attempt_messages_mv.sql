-- Materialized View: attempt_messages_mv
...
-- ============================================================================
-- Step 1: Drop all indexes on attempt_messages_mv materialized view (if it exists)
-- ============================================================================
...
BEGIN
    FOR r IN
        ...
        WHERE schemaname = 'public'
          AND tablename = 'attempt_messages_mv'
    LOOP
...
-- ============================================================================
-- Step 2: Drop attempt_messages_mv materialized view if it exists
-- ============================================================================

DROP MATERIALIZED VIEW IF EXISTS attempt_messages_mv CASCADE;

-- ============================================================================
-- Step 3: Create attempt_messages_mv Materialized View
-- ============================================================================

CREATE MATERIALIZED VIEW attempt_messages_mv AS
WITH
...
CREATE UNIQUE INDEX attempt_messages_mv_pk
    ON attempt_messages_mv (message_id);
...
CREATE INDEX attempt_messages_mv_chat_id_idx
    ON attempt_messages_mv (chat_id);

-- Attempt ID for parallel lookup
CREATE INDEX attempt_messages_mv_attempt_id_idx
    ON attempt_messages_mv (attempt_id);

-- Composite: attempt + chat for ordering (position derived in service layer)
CREATE INDEX attempt_messages_mv_attempt_chat_idx
    ON attempt_messages_mv (attempt_id, chat_id);

-- Message type for filtering
CREATE INDEX attempt_messages_mv_type_idx
    ON attempt_messages_mv (type);

-- Created at for ordering (position derived from this)
CREATE INDEX attempt_messages_mv_created_at_idx
    ON attempt_messages_mv (created_at);

-- Runs ID for filtering by run
CREATE INDEX attempt_messages_mv_runs_id_idx
    ON attempt_messages_mv (runs_id)
...
REFRESH MATERIALIZED VIEW attempt_messages_mv;
