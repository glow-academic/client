...
history_attempts AS (
    ...
    WHERE sa.created_at >= p.start_date
      ...
      AND (
        (cardinality(p.simulation_filters) = 0) AND NOT EXISTS (SELECT 1 FROM simulation_flags sf WHERE sf.simulation_id = sim.id AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)
        OR
        (cardinality(p.simulation_filters) > 0 AND (
          ('general' = ANY(p.simulation_filters) AND NOT EXISTS (SELECT 1 FROM simulation_flags sf WHERE sf.simulation_id = sim.id AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)) OR
          ('practice' = ANY(p.simulation_filters) AND EXISTS (SELECT 1 FROM simulation_flags sf WHERE sf.simulation_id = sim.id AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)) OR
...
history_attempt_cohorts AS (
    ...
    LEFT JOIN cohort_artifact c ON c.id = cp.cohort_id AND EXISTS (SELECT 1 FROM cohort_flags cf WHERE cf.cohort_id = c.id AND cf.type = 'active'::type_cohort_flags AND cf.value = TRUE)
            ...
            SUM(
                CASE 
                    WHEN sc.completed AND hcg.chat_id IS NOT NULL THEN
                        (SELECT COALESCE(t.time_taken, 0) FROM grade_artifact scg 
                    ...
                    WHEN sc.completed THEN
                        EXTRACT(EPOCH FROM (
                            (SELECT scg.created_at FROM grade_artifact scg 
                             JOIN run_artifact r ON r.id = scg.run_id
                             JOIN group_runs gr ON gr.run_id = r.id
                             JOIN grade_groups gg ON gg.group_id = gr.group_id
                             JOIN chat_artifact c ON c.id = gg.chat_id
