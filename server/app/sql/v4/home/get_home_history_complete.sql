...
history_attempts AS (
    ...
    WHERE sa.created_at >= p.start_date
      ...
      AND (
        (cardinality(p.simulation_filters) = 0) AND NOT EXISTS (SELECT 1 FROM simulation_flags sf JOIN flags fl ON sf.flag_id = fl.id WHERE sf.simulation_id = sim.id AND fl.name = 'practice' AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)
        OR
        (cardinality(p.simulation_filters) > 0 AND (
          ('general' = ANY(p.simulation_filters) AND NOT EXISTS (SELECT 1 FROM simulation_flags sf JOIN flags fl ON sf.flag_id = fl.id WHERE sf.simulation_id = sim.id AND fl.name = 'practice' AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)) OR
          ('practice' = ANY(p.simulation_filters) AND EXISTS (SELECT 1 FROM simulation_flags sf JOIN flags fl ON sf.flag_id = fl.id WHERE sf.simulation_id = sim.id AND fl.name = 'practice' AND sf.type = 'practice'::type_simulation_flags AND sf.value = TRUE)) OR
...
profile_options_cte AS (
    ...
    JOIN profile p ON p.id = haf.profile_id
...
simulation_options_cte AS (
    SELECT 
        ...
        COUNT(DISTINCT haf.attempt_id) AS count
    FROM history_attempts_filtered haf
    JOIN simulation s ON s.id = haf.simulation_id
    GROUP BY haf.simulation_id, (SELECT n.name FROM simulation_names sn JOIN names n ON sn.name_id = n.id WHERE sn.simulation_id = s.id LIMIT 1)
            ...
            SUM(
                CASE 
                    WHEN sc.completed AND hcg.chat_id IS NOT NULL THEN
                        (SELECT COALESCE(t.time_taken, 0) FROM grade scg 
                    ...
                    WHEN sc.completed THEN
                        EXTRACT(EPOCH FROM (
                            (SELECT scg.created_at FROM grade scg 
                             JOIN run r ON r.id = scg.run_id
                             JOIN group_runs gr ON gr.run_id = r.id
                             JOIN grade_groups gg ON gg.group_id = gr.group_id
                             JOIN chat c ON c.id = gg.chat_id
...
history_scenario_ids AS (
    SELECT
        s.id AS simulation_id,
        ARRAY_AGG(ss.scenario_id ORDER BY COALESCE((SELECT sp.value FROM scenario_positions sp WHERE sp.simulation_id = ss.simulation_id AND sp.scenario_id = ss.scenario_id LIMIT 1), 999999))::uuid[] AS scenario_ids_assigned
    FROM simulation s
    LEFT JOIN simulation_scenarios ss ON ss.simulation_id = s.id
    WHERE s.id IN (SELECT simulation_id FROM history_attempts_final)
    GROUP BY s.id
...
attempt_joined AS (
    ...
    JOIN simulation s ON s.id = ar.simulation_id
    ...
    JOIN profile p ON p.id = ar.profile_id
),
...
user_profile AS (
    ...
    FROM resolve_profile_id rpi
    JOIN profile ON profile.id = rpi.resolved_profile_id
),
-- Calculate page info
page_info AS (
    SELECT 
