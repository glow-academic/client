...
CREATE TYPE types.q_get_tool_v4_tool AS (
    tool_id uuid,
    name text,
    description text,
    active boolean,
...
-- 4) Recreate function (skeleton - returns empty/default values)
...
AS $$
WITH params AS (
    SELECT tool_id AS tool_id, profile_id AS profile_id, draft_id AS draft_id
),
user_profile AS (
    SELECT 
        COALESCE(
...
SELECT 
    ...
    CASE WHEN (SELECT tool_id FROM params) IS NULL THEN false ELSE false END as tool_exists,
    ''::text as name,
    ''::text as description,
