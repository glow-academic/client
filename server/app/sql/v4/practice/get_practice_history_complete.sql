...
history_attempts AS (
    ...
    WHERE EXISTS (SELECT 1 FROM simulation_flags sf JOIN flags_resource f ON sf.flag_id = f.id WHERE sf.simulation_id = sim.id AND f.name = 'practice' AND sf.value = TRUE)
      AND sa.profile_id = (SELECT resolved_profile_id FROM resolve_profile_id)
...
profile_options_cte AS (
    ...
    GROUP BY ha.profile_id, (SELECT n.name FROM profile_names pn JOIN names_resource n ON pn.name_id = n.id WHERE pn.profile_id = p.id LIMIT 1), (SELECT n2.name FROM profile_names pn2 JOIN names_resource n2 ON pn2.name_id = n2.id WHERE pn2.profile_id = p.id LIMIT 1)
...
simulation_options_cte AS (
    ...
    GROUP BY ha.simulation_id, (SELECT n.name FROM simulation_names sn JOIN names_resource n ON sn.name_id = n.id WHERE sn.simulation_id = s.id LIMIT 1)
...
scenario_options_cte AS (
    SELECT
        ...
        COUNT(DISTINCT ha.attempt_id) AS count
    FROM history_attempts ha
    JOIN chats sc ON sc.attempt_id = ha.attempt_id
