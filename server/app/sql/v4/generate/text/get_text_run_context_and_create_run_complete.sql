...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            EXECUTE format('DROP FUNCTION IF EXISTS socket_get_hint_run_context_and_create_run_v4(%s)', func_sig);
...
BEGIN
    ...
    LOOP
        BEGIN
            -- Try to get signature, but handle missing types gracefully
            SELECT pg_get_function_identity_arguments(r.oid) INTO func_sig;
            EXECUTE format('DROP FUNCTION IF EXISTS socket_get_text_run_context_and_create_run_v4(%s)', func_sig);
        EXCEPTION WHEN OTHERS THEN
            -- Function might reference missing types, try dropping by OID with CASCADE
            BEGIN
                EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', r.oid::regprocedure);
...
BEGIN
    ...
    LOOP
        BEGIN
            -- Try to get signature, but handle missing types gracefully
            SELECT pg_get_function_identity_arguments(r.oid) INTO func_sig;
            EXECUTE format('DROP FUNCTION IF EXISTS socket_get_generation_run_context_and_create_run_v4(%s) CASCADE', func_sig);
        EXCEPTION WHEN OTHERS THEN
            -- Function might reference missing types, try dropping by OID with CASCADE
            BEGIN
                EXECUTE format('DROP FUNCTION IF EXISTS %s CASCADE', r.oid::regprocedure);
