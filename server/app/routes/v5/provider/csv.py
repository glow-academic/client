"""Provider CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
class ParseProviderCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateProviderItem]
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateProviderItem:
    kwargs: dict[str, Any] = {}
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in PROVIDER_IMPORT_FIELDS)}",
            )
        ...
        return ParseProviderCsvApiResponse(
