"""Department CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
class ParseDepartmentCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateDepartmentItem]
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateDepartmentItem:
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ParseDepartmentCsvApiResponse)
async def parse_department_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in DEPARTMENT_IMPORT_FIELDS)}",
            )

        mapped_fields = [field_map[idx]["key"] for idx in sorted(field_map.keys())]

        required_keys = {f["key"] for f in DEPARTMENT_IMPORT_FIELDS if f.get("required")}
        ...
        return ParseDepartmentCsvApiResponse(
        ...
        handle_route_error(
            ...
            operation="parse_department_csv",
