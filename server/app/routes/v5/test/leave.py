...
class TestLeaveRequest(BaseModel):
    sid: str
    invocation_id: UUID


class TestLeaveResponse(BaseModel):
    success: bool


@router.post("/leave", response_model=TestLeaveResponse)
async def test_leave(
    request: TestLeaveRequest,
    ...
    await leave_entity(request.sid, "test", request.invocation_id)
    return TestLeaveResponse(success=True)
