"""Cohort CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateCohortItem:
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ParseCohortCsvApiResponse)
async def parse_cohort_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in COHORT_IMPORT_FIELDS)}",
            )
