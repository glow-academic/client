...
async def search_rubrics(
    ...
    *,
    ...
    if simulation_rubric is not None:
        extra_conditions.append(("{alias}.simulation_rubric = ${idx}::boolean", simulation_rubric))
    if video_rubric is not None:
        extra_conditions.append(("{alias}.video_rubric = ${idx}::boolean", video_rubric))
    if standard_group_ids:
        extra_conditions.append(
            ("(COALESCE(array_length(${idx}::uuid[], 1), 0) = 0 OR {alias}.standard_group_ids && ${idx}::uuid[])", standard_group_ids)
        )
