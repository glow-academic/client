"""documents/link internal — reusable data-access layer."""
...
SQL_PATH = "app/sql/queries/resources/link_documents_complete.sql"
...
async def link_documents_internal(
    ...
    result_row = LinkDocumentsSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.documents_id:
        raise ValueError(f"Failed to link documents: {resource_id}")
    return result_row.documents_id
