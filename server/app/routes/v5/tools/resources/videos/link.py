"""videos/link internal — reusable data-access layer."""
...
SQL_PATH = "app/sql/queries/resources/link_videos_complete.sql"
...
async def link_videos_internal(
    ...
    result_row = LinkVideosSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.videos_id:
        raise ValueError(f"Failed to link videos: {resource_id}")
    return result_row.videos_id
