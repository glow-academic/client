"""profiles/link internal — reusable data-access layer."""
...
SQL_PATH = "app/sql/queries/resources/link_profiles_complete.sql"
...
async def link_profiles_internal(
    ...
    result_row = LinkProfilesSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.profiles_id:
        raise ValueError(f"Failed to link profile: {resource_id}")
    return result_row.profiles_id
