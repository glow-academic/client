"""options/link internal — reusable data-access layer."""
...
SQL_PATH = "app/sql/queries/resources/link_options_complete.sql"
...
async def link_options_internal(
    ...
    result_row = LinkOptionsSqlRow.model_validate(
        result.model_dump() if hasattr(result, "model_dump") else result
    )
    if not result_row.options_id:
        raise ValueError(f"Failed to link options: {resource_id}")
    return result_row.options_id
