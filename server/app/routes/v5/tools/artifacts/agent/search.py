"""Agent artifact SEARCH — returns matching agent IDs."""
    ...
    *,
    ...
    if search:
        ...
        conditions.append(
            f"("
            f"EXISTS ("
            f"SELECT 1 FROM agent_names_junction nj "
            ...
            f") OR EXISTS ("
            f"SELECT 1 FROM agent_descriptions_junction dj "
    ...
    order_join = (
        f"LEFT JOIN agent_names_junction pnj ON pnj.{OWNER_COL} = a.id AND pnj.active = true "
