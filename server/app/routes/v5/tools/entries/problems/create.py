"""problems/create internal — reusable data-access layer."""
...
from app.routes.v5.api.entries.problems.types import (
    ...
    CreateProblemsEntrySqlParams,
    CreateProblemsEntrySqlRow,
)
from app.utils.cache.invalidate_tags import invalidate_tags
from app.utils.sql_helper import execute_sql_typed
from app.utils.storage.file_writer import write_text_file

SQL_PATH = "app/sql/queries/entries/problems/create_problems_entries_complete.sql"
...
async def create_problems_entry_internal(
    ...
    """Internal function to create problems entry.
    ...
    async with conn.transaction():
        ...
        request_dict["upload_id"] = await write_text_file(
            conn, None, "Created problems entry"
        )
        request_dict["tool_id"] = tool_id
        if run_id is not None:
            request_dict["run_id"] = run_id

        params = CreateProblemsEntrySqlParams(**request_dict)
        ...
        if not result or not result.id:
            raise ValueError("Failed to create problems entry")
    ...
    return CreateProblemsEntryResponse.model_validate(result.model_dump())
