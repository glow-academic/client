"""attempt_chat/create internal — reusable data-access layer."""
...
SQL_PATH = (
    "app/sql/queries/entries/attempt_chat/create_attempt_chat_entries_complete.sql"
)

ENTRY_TYPE = "attempt_chats"
...
async def create_attempt_chat_entry_internal(
    ...
    async with conn.transaction():
        ...
        request_dict["upload_id"] = await write_text_file(
            conn, None, "Created attempt chat entry"
        )
        request_dict["tool_id"] = tool_id
        if run_id is not None:
            request_dict["run_id"] = run_id

        params = CreateAttemptChatEntrySqlParams(**request_dict)
