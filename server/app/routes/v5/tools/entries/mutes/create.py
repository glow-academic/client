"""mutes/create internal — reusable data-access layer."""
...
from app.routes.v5.api.entries.mutes.types import (
...
SQL_PATH = "app/sql/queries/entries/mutes/create_mutes_entries_complete.sql"
...
async def create_mutes_entry_internal(
    ...
    async with conn.transaction():
        ...
        request_dict["upload_id"] = await write_text_file(
            conn, None, "Created mutes entry"
        )
        request_dict["tool_id"] = tool_id
        if run_id is not None:
            request_dict["run_id"] = run_id

        params = CreateMutesEntrySqlParams(**request_dict)
