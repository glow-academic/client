...
from app.sql.types import (
    ...
    GetSuiteDraftsEntriesSqlParams,
    QGetSuiteDraftsEntriesV4Item,
    load_sql_query,
)
...
SQL_PATH = (
    "app/sql/queries/entries/suite_drafts/get_suite_drafts_entries_complete.sql"
)

router = APIRouter()


async def get_suite_drafts_entries_internal(
    conn: asyncpg.Connection,
    ids: list[UUID],
    bypass_cache: bool = False,
) -> list[QGetSuiteDraftsEntriesV4Item]:
    """Internal function to fetch suite_drafts entries by IDs."""
    if not ids:
        return []
    ...
    if not bypass_cache:
        ...
        if cached:
            return [
                QGetSuiteDraftsEntriesV4Item.model_validate(item)
                for item in cached.get("items", [])
            ]

    params = GetSuiteDraftsEntriesSqlParams(ids=ids)
    result = await execute_sql_typed(conn, SQL_PATH, params=params)

    items: list[QGetSuiteDraftsEntriesV4Item] = (
