"""Department import endpoint — parses a CSV file and creates departments in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical department fields, and delegates to the existing create_department_impl.
"""
...
from app.infra.department.search import DEPARTMENT_IMPORT_FIELDS
...
class ImportDepartmentApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""

    upload_id: UUID


class ImportDepartmentApiResponse(CreateDepartmentApiResponse):
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in DEPARTMENT_IMPORT_FIELDS:
...
def _row_to_item(
    ...
    """Convert a single CSV row to a CreateDepartmentItem using the field map."""
    kwargs: dict[str, Any] = {}
    ...
    return CreateDepartmentItem(**kwargs)
...
@router.post("/csv", response_model=ImportDepartmentApiResponse)
async def import_departments(
    request: ImportDepartmentApiRequest,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched department import fields. "
                f"Expected columns: {', '.join(f['label'] for f in DEPARTMENT_IMPORT_FIELDS)}",
            )
        ...
        required_keys = {
            f["key"] for f in DEPARTMENT_IMPORT_FIELDS if f.get("required")
        }
        ...
        # 3. Convert rows to CreateDepartmentItem objects
        ...
        return ImportDepartmentApiResponse(
