"""Simulation import endpoint — parses a CSV file and creates simulations in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical simulation fields, and delegates to the existing create_simulation_impl.
"""
...
from app.routes.v5.api.main.simulation.types import CreateSimulationApiResponse
...
class ImportSimulationApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""

    upload_id: UUID


class ImportSimulationApiResponse(CreateSimulationApiResponse):
...
def _row_to_item(
    ...
    """Convert a single CSV row to a CreateSimulationItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportSimulationApiResponse)
async def import_simulations(
    request: ImportSimulationApiRequest,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched simulation import fields. "
                f"Expected columns: {', '.join(f['label'] for f in SIMULATION_IMPORT_FIELDS)}",
            )
        ...
        required_keys = {
            f["key"] for f in SIMULATION_IMPORT_FIELDS if f.get("required")
        }
        ...
        # 3. Convert rows to CreateSimulationItem objects
        items = [_row_to_item(row, field_map) for row in data_rows]

        # 4. Delegate to existing create logic
        async def _runner() -> CreateSimulationApiResponse:
            return await create_simulation_impl(
                pool,
                redis,
        ...
        return ImportSimulationApiResponse(
