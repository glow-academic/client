"""Eval import endpoint — parses a CSV file and creates evals in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical eval fields, and delegates to the existing create_eval_impl.
"""
...
from app.infra.eval.create import (
...
class ImportEvalApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""

    upload_id: UUID


class ImportEvalApiResponse(CreateEvalApiResponse):
...
def _row_to_item(
    ...
    """Convert a single CSV row to a CreateEvalItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportEvalApiResponse)
async def import_evals(
    request: ImportEvalApiRequest,
    http_request: Request,
    response: Response,
) -> ImportEvalApiResponse:
    """Import evals from a CSV file.
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched eval import fields. "
                f"Expected columns: {', '.join(f['label'] for f in EVAL_IMPORT_FIELDS)}",
            )
        ...
        # 3. Convert rows to CreateEvalItem objects
