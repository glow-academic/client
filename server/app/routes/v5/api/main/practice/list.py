"""Practice list endpoint — paginated attempt history.

Data sources:
  - attempt_mv: paginated attempts (filtered by profile, practice=True)
...
async def list_practice_internal(
    pool: asyncpg.Pool,
...
@router.post("/list", response_model=ListPracticeResponse)
async def practice_list(
    ...
    if not bypass_cache:
        ...
        if cached:
            ...
            return ListPracticeResponse.model_validate(cached["data"])
    ...
    try:
        ...
        api_response = await list_practice_internal(
            pool=pool,
            request=request,
