"""Rubric import endpoint — parses a CSV file and creates rubrics in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical rubric fields, and delegates to the existing create_rubric_impl.
"""
...
from app.infra.rubric.create import (
    CreateRubricItem,
...
class ImportRubricApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""
    upload_id: UUID


class ImportRubricApiResponse(CreateRubricApiResponse):
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateRubricItem:
    """Convert a single CSV row to a CreateRubricItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportRubricApiResponse)
async def import_rubrics(
    request: ImportRubricApiRequest,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched rubric import fields. "
                f"Expected columns: {', '.join(f['label'] for f in RUBRIC_IMPORT_FIELDS)}",
            )
        ...
        # 3. Convert rows to CreateRubricItem objects
        ...
        return ImportRubricApiResponse(
