"""Auth draft endpoint - handles autosave for all auth resources.
Uses access check SQL + Python permission logic before executing draft patch.
Now supports protocol_ids and slug_ids.
"""
...
from app.routes.v5.api.main.auth.permissions import compute_can_draft
...
# SQL paths — reuse duplicate access check (just needs user_role)
ACCESS_CHECK_SQL_PATH = "app/sql/queries/auth/check_auth_duplicate_access_complete.sql"
SQL_PATH = "app/sql/queries/auth/patch_auth_draft_complete.sql"
...
async def patch_auth_draft(
    ...
    """Patch auth draft - accepts resource IDs and creates/updates draft."""
    tags = ["auth", "drafts"]
