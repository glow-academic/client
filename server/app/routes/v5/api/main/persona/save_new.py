...
Uses composable infra layers:
  ...
  2. resolve_persona_permissions_context — access check (fail fast)
  3. _resolve_values — raw value → ID via create/search tools
  4. create_persona / update_persona artifact tools — junction writes
  5. create_persona resource tool — denormalized snapshot
"""
...
async def _create_denormalized_snapshot(
    ...
    *,
    ...
    names, descriptions, colors, icons, instructions = await _hydrate_singles(
        conn, redis, name_id, description_id, color_id, icon_id, instructions_id,
    )

    name_val = names[0].name if names else ""
    desc_val = descriptions[0].description if descriptions else ""
    color_val = colors[0].hex_code if colors else ""
    icon_val = icons[0].value if icons else ""
    instructions_val = instructions[0].template if instructions else ""

    example_vals: list[str] = []
    if example_ids:
        from app.routes.v5.tools.resources.examples.get import get_examples
        examples = await get_examples(conn, example_ids, redis, bypass_cache=True)
        example_vals = [e.example for e in examples]
...
async def _hydrate_singles(conn, redis, name_id, description_id, color_id, icon_id, instructions_id):
    """Fetch single-select resources in parallel."""
    import asyncio
...
async def save_persona_client(
    ...
    *,
    ...
    for idx, item in enumerate(items):
        item_errors = await _resolve_values(
