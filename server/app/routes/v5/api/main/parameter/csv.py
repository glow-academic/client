"""Parameter CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
class ParseParameterCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateParameterItem]
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateParameterItem:
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ParseParameterCsvApiResponse)
async def parse_parameter_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in PARAMETER_IMPORT_FIELDS)}",
            )
        ...
        return ParseParameterCsvApiResponse(
