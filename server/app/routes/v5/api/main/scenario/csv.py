"""Scenario import endpoint — parses a CSV file and creates scenarios in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical scenario fields, and delegates to the existing create_scenario_impl.
"""
...
from app.infra.scenario.create import (
    CreateScenarioItem,
    create_scenario_impl,
)
from app.infra.scenario.search import SCENARIO_IMPORT_FIELDS
from app.routes.v5.api.main.scenario.types import CreateScenarioApiResponse
...
class ImportScenarioApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""

    upload_id: UUID


class ImportScenarioApiResponse(CreateScenarioApiResponse):
...
def _build_field_map(headers: list[str]) -> dict[int, Any]:
    ...
    for field in SCENARIO_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, Any]) -> CreateScenarioItem:
    """Convert a single CSV row to a CreateScenarioItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportScenarioApiResponse)
async def import_scenarios(
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched scenario import fields. "
                f"Expected columns: {', '.join(f.label for f in SCENARIO_IMPORT_FIELDS)}",
            )

        mapped_fields = [field_map[idx].key for idx in sorted(field_map.keys())]

        # Check required fields are mapped
        required_keys = {f.key for f in SCENARIO_IMPORT_FIELDS if f.required}
        ...
        # 3. Convert rows to CreateScenarioItem objects
        ...
        return ImportScenarioApiResponse(
