"""Tool import endpoint — parses a CSV file and creates tools in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical tool fields, and delegates to the existing create_tool_impl.
"""
...
from app.infra.tool.create import (
    CreateToolApiResponse,
    CreateToolItem,
    create_tool_impl,
)
from app.infra.tool.search import TOOL_IMPORT_FIELDS
...
class ImportToolApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""
    upload_id: UUID


class ImportToolApiResponse(CreateToolApiResponse):
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in TOOL_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateToolItem:
    """Convert a single CSV row to a CreateToolItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportToolApiResponse)
async def import_tools(
    request: ImportToolApiRequest,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched tool import fields. "
                f"Expected columns: {', '.join(f['label'] for f in TOOL_IMPORT_FIELDS)}",
            )

        mapped_fields = [field_map[idx]["key"] for idx in sorted(field_map.keys())]

        # Check required fields are mapped
        required_keys = {f["key"] for f in TOOL_IMPORT_FIELDS if f.get("required")}
        ...
        # 3. Convert rows to CreateToolItem objects
