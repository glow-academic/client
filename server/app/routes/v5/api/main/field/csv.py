"""Field CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
class ParseFieldCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateFieldItem]
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in FIELD_IMPORT_FIELDS)}",
            )
        ...
        return ParseFieldCsvApiResponse(
