"""Field import endpoint — parses a CSV file and creates fields in one step.

Takes an upload_id (from a TUS-uploaded CSV), reads the file, auto-maps columns
to canonical field fields, and delegates to the existing create_field_impl.
"""
...
from app.infra.field.create import (
    CreateFieldApiResponse,
    CreateFieldItem,
    create_field_impl,
)
from app.infra.field.search import FIELD_IMPORT_FIELDS
...
class ImportFieldApiRequest(BaseModel):
    """Request model — just the upload_id of a TUS-uploaded CSV."""

    upload_id: UUID


class ImportFieldApiResponse(CreateFieldApiResponse):
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in FIELD_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateFieldItem:
    """Convert a single CSV row to a CreateFieldItem using the field map."""
    kwargs: dict[str, Any] = {}
...
@router.post("/csv", response_model=ImportFieldApiResponse)
async def import_fields(
    request: ImportFieldApiRequest,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                status_code=400,
                detail="No CSV columns matched field import fields. "
                f"Expected columns: {', '.join(f['label'] for f in FIELD_IMPORT_FIELDS)}",
            )

        mapped_fields = [field_map[idx]["key"] for idx in sorted(field_map.keys())]

        # Check required fields are mapped
        required_keys = {f["key"] for f in FIELD_IMPORT_FIELDS if f.get("required")}
        ...
        # 3. Convert rows to CreateFieldItem objects
