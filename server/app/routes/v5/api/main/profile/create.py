"""Profile create endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.profile.create.
"""
...
@router.post("/create", response_model=CreateProfileApiResponse)
async def create_profile(
    ...
    try:
        ...
        async def _runner() -> CreateProfileApiResponse:
            return await create_profile_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            session_id=session_id,
            operation="create",
            arguments={"profiles": [item.model_dump(mode="json") for item in request.profiles]},
