...
GAPs / TODOs:
  ...
  - TODO: Wire resolve_websocket_context as alternative to registry fetcher
          once all artifact types support it.
"""
...
async def generate_prepare_handler_new(data: dict[str, Any]) -> None:
    ...
    try:
        ...
        # --- Step 3: Fetch via registry fetcher ---
        pool = get_pool()
        if not pool:
        ...
        # --- Step 4: Resolve systems → agents → models → providers ---
        ...
        # --- Step 5: Build agent groups (pure) ---
        ...
        if not agent_groups:
            await _emit_error(sid, "No agent found for the requested resource types", artifact_type)
            return
        ...
        for agent_group_id, agent_resource_types in agent_groups.items():
            ...
            # Fetch prompt + instructions for this agent
            ...
            async def _fetch_instructions(ar: Any) -> list[str]:
                ...
                if not iids:
                    return []
                async with pool.acquire() as c:
                    instructions = await get_instructions(c, iids, redis)
