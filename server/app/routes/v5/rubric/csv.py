"""Rubric CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
class ParseRubricCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateRubricItem]
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in RUBRIC_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateRubricItem:
    kwargs: dict[str, Any] = {}
    for col_idx, field_def in field_map.items():
        ...
        if field_type == "boolean":
            ...
            kwargs[key] = raw
    return CreateRubricItem(**kwargs)


@router.post("/csv", response_model=ParseRubricCsvApiResponse)
async def parse_rubric_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in RUBRIC_IMPORT_FIELDS)}",
            )

        mapped_fields = [field_map[idx]["key"] for idx in sorted(field_map.keys())]

        required_keys = {f["key"] for f in RUBRIC_IMPORT_FIELDS if f.get("required")}
        ...
        return ParseRubricCsvApiResponse(
