"""Setting artifact SEARCH — returns matching setting IDs."""
...
async def search_settings(
    ...
    *,
    ...
    """Search setting artifacts by filters. Returns (IDs, total_count)."""
    conditions: list[str] = []
    params: list[object] = []
    idx = 1
    ...
    if search:
        conditions.append(
            f"("
            f"EXISTS ("
            f"SELECT 1 FROM setting_names_junction nj "
            ...
            f") OR EXISTS ("
            f"SELECT 1 FROM setting_descriptions_junction dj "
    ...
    if department_ids:
        idx = add_junction_filter(
            ...
            params,
            idx,
            junction_table="setting_departments_junction",
            owner_col=OWNER_COL,
    ...
    # Order by name
    order_join = (
        f"LEFT JOIN setting_names_junction pnj ON pnj.{OWNER_COL} = a.id AND pnj.active = true "
