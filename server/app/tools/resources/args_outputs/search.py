"""Args Outputs SEARCH — reusable data-access layer."""
...
from app.tools.resources.args_outputs.types import GetArgOutputResponse
...
async def search_args_outputs(
    ...
    *,
    ...
    """Search args_outputs with optional artifact/draft filters."""
    if limit_count <= 0:
        return []
    ...
    key = cache_key(
        "/v5/resources/args_outputs/search",
        {
            ...
            "args_ids": [str(i) for i in (args_ids or [])],
            **artifact_filters,
        },
    )
    ...
    ids = await search_resource_ids(
        ...
        additional_search_columns=["template"],
    ...
    items = await get_args_outputs(conn, ids, redis, bypass_cache=True)
