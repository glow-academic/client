...
class MockProfile:
    def __init__(self, id, first_name, last_name, alias, created_at, role="student"):
        self.id, self.first_name, self.last_name, self.alias, self.created_at, self.role = id, first_name, last_name, alias, created_at, role
...
class MockChat:
    def __init__(self, id, attempt_id, scenario_id, created_at, title, completed=True, completed_at=None):
        self.id, self.attempt_id, self.scenario_id, self.created_at, self.title, self.completed, self.completed_at = id, attempt_id, scenario_id, created_at, title, completed, completed_at

class MockMessage:
     def __init__(self, chat_id, type, content, created_at, completed=True):
        self.chat_id, self.type, self.content, self.created_at, self.completed = chat_id, type, content, created_at, completed

class MockGrade:
    def __init__(self, id, score, passed, time_taken, created_at):
        self.id, self.score, self.passed, self.time_taken, self.created_at = id, score, passed, time_taken, created_at
...
class TestStudentSimReport:
    ...
    def test_success_with_full_data(self, mock_get_session, mock_db_session):
        ...
        # 1. Mock Profile
        mock_profile = MockProfile(profile_id, "Detailed", "Student", "detail", datetime.now())
        ...
        mock_chat = MockChat(uuid.uuid4(), mock_attempt.id, scenario_id, datetime.now(), "Chat")
