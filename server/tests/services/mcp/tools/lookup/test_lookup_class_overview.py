...
class TestClassOverview:
    ...
    def test_class_overview_success(self, mock_get_session):
        ...
        mock_profiles = [MockProfile(profile_id, "John", "Doe", "jdoe", class_ids=[class_id])]
    ...
    def test_class_overview_multiple_students(self, mock_get_session):
        ...
        mock_class = MockClass(class_id, "Test Class", "CS101")
        mock_profiles = [
            MockProfile(uuid.uuid4(), "John", "Doe", "jdoe", class_ids=[class_id]),
            MockProfile(uuid.uuid4(), "Jane", "Smith", "jsmith", class_ids=[class_id]),
            MockProfile(uuid.uuid4(), "Bob", "Johnson", "bjohnson", class_ids=[class_id])
        ]
        mock_scenarios = []
    ...
    def test_class_overview_array_filtering(self, mock_get_session):
        ...
        mock_class = MockClass(class_id, "Test Class", "CS101")
        # Create profiles with different class_ids arrays
        mock_profiles = [
            MockProfile(uuid.uuid4(), "John", "Doe", "jdoe", class_ids=[class_id]),  # Should be included
            MockProfile(uuid.uuid4(), "Jane", "Smith", "jsmith", class_ids=[other_class_id]),  # Should be excluded
            MockProfile(uuid.uuid4(), "Bob", "Johnson", "bjohnson", class_ids=[class_id, other_class_id]),  # Should be included
            MockProfile(uuid.uuid4(), "Alice", "Brown", "abrown", class_ids=[]),  # Should be excluded
        ]
        mock_scenarios = []
        ...
        # Should only include profiles that have class_id in their class_ids array
