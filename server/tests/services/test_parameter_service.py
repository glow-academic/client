...
@pytest.mark.asyncio
async def test_get_parameters_list(db: asyncpg.Connection, disable_cache: None) -> None:
    ...
    # Create filters
    filters = ParametersFilters(departmentIds=[dept_id], profileId=profile_id)
...
async def test_get_parameters_list_empty_departments(
    ...
    filters = ParametersFilters(departmentIds=[], profileId=profile_id)
...
async def test_get_parameter_detail_optimized(
    ...
    # Create request
    request = ParameterDetailRequest(parameterId=parameter_id, profileId=profile_id)
    ...
    if result.department_id:
        assert len(result.department_mapping) > 0, (
            "department_mapping should be populated when parameter has department"
        )
...
async def test_get_parameter_detail_not_found(
    ...
    request = ParameterDetailRequest(
        parameterId="00000000-0000-0000-0000-000000000000", profileId=profile_id
    )
...
async def test_parameter_can_duplicate_permissions(
    ...
    for parameter in resp_superadmin.parameters:
        assert parameter.can_duplicate is True, (
            "Superadmin should be able to duplicate parameters"
        )
    ...
    if admin_id:
        ...
        for parameter in resp_admin.parameters:
            assert parameter.can_duplicate is True, (
                "Admin should be able to duplicate parameters"
            )
    ...
    if instructional_id:
        ...
        for parameter in resp_instructional.parameters:
            assert parameter.can_duplicate is False, (
                "Instructional should NOT be able to duplicate parameters"
            )


@pytest.mark.asyncio
async def test_get_parameter_detail_default_consolidated(
    db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test getting default parameter detail with consolidated query (1 query instead of 2)."""
    # Setup - Get test profile ID
    profile_id = await get_test_profile_id(db)

    # Create request
    from app.schemas.parameters import ParameterDetailDefaultRequest

    request = ParameterDetailDefaultRequest(profileId=profile_id)

    # Execute - Call the service method
    svc = ParameterService(db)
    result = await svc.get_parameter_detail_default(request)

    # Assert - Check basic structure
    assert result is not None
    assert hasattr(result, "name")
    assert hasattr(result, "description")
    assert hasattr(result, "numerical")
    assert hasattr(result, "active")
    assert hasattr(result, "default_parameter")
