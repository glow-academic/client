...
class TestRunAdvancedAgentParallel:
    ...
    async def test_run_advanced_agent_parallel_success(
        self,
        mock_parallel_turn,
        mock_evaluate,
        mock_session,
        sample_eval_obj,
        sample_eval_run_chat_map,
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_advanced_agent_parallel_empty_input(
        self, mock_session, sample_eval_obj
    ):
...
class TestRunParallelAgentTurn:
    ...
    async def test_run_parallel_agent_turn_success(
        self, mock_agent_class, mock_runner, mock_session, sample_agent, sample_scenario
    ):
    ...
    async def test_run_parallel_agent_turn_fallback_parsing(
        self,
        mock_parse,
        mock_agent_class,
        mock_runner,
    ...
    @patch("app.services.agents.advanced.Runner")
    @patch("app.services.agents.advanced.AdvancedAgent")
    async def test_run_parallel_agent_turn_error_handling(
        self, mock_agent_class, mock_runner, mock_session, sample_agent, sample_scenario
    ):
...
class TestRunAdvancedAgent:
    ...
    async def test_run_advanced_agent_success(
        self, mock_handle_chat, mock_session, sample_eval_chat
    ):
        ...
        async for token in run_advanced_agent(
            chat_id=sample_eval_chat.id, input_text="Test input", session=mock_session
        ):
            tokens.append(token)
    ...
    async def test_run_advanced_agent_chat_not_found(self, mock_session):
        ...
        with pytest.raises(ValueError, match="Chat not found with ID"):
            async for token in run_advanced_agent(
                chat_id=str(uuid4()), input_text="Test input", session=mock_session
            ):
                pass
...
class TestAdvancedAgent:
    ...
    def test_advanced_agent_default_init(self, mock_get_gemini):
        ...
        )  # Should use agent_prompt directly

    @patch("app.services.agents.advanced.get_gemini")
    @patch("app.services.agents.advanced.Agent")
    def test_advanced_agent_agent_method_without_parallel(
        self, mock_agent_class, mock_get_gemini
    ):
    ...
    @patch("app.services.agents.advanced.get_gemini")
    @patch("app.services.agents.advanced.Agent")
    def test_advanced_agent_agent_method_with_parallel(
        self, mock_agent_class, mock_get_gemini
    ):
