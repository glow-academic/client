...
class MockPersona:
    def __init__(self, id, name, system_prompt, temperature, model_id, reasoning, description="Test persona"):
        self.id = id
        self.name = name
    ...
    async def test_run_scenario_agent_success(self, mock_session):
        ...
        with patch('app.services.agents.collection.scenario.Runner.run', return_value=mock_result):
            with patch('app.services.agents.generic.decrypt_api_key', return_value="decrypted_key"):
                with patch('app.services.agents.collection.scenario.trace') as mock_trace:
