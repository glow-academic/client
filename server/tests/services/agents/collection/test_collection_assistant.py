"""
Tests for app.agents.collection.assistant
"""
...
class MockAssistantToolCall:
    def __init__(
        self, id, chat_id, tool_name, tool_type, tool_arguments, created_at=None
    ):
        self.id = id
        self.chat_id = chat_id
...
class TestRun_Assistant_Agent:
    ...
    async def test_run_assistant_agent_success(self, mock_session):
        ...
        mock_agent = MockAgent(
            agent_id, "Assistant", "You are helpful", 0.7, model_id, "medium"
        )
        ...
        with patch(
            "app.agents.collection.assistant.Runner.run_streamed",
            return_value=mock_result,
        ):
            with patch(
                "app.agents.collection.assistant.MCPServerStreamableHttp"
            ) as mock_mcp:
                ...
                with patch(
                    ...
                    with patch(
                        "app.agents.collection.assistant.trace"
                    ) as mock_trace:
...
class TestCancel_Assistant_Run:
    ...
    def test_cancel_assistant_run_invalid_uuid(self, mock_session):
        ...
        with patch(
            "app.main.cancel_active_run", side_effect=ValueError("Invalid UUID")
        ):
