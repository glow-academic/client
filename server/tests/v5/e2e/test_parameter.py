"""E2E skeleton: Parameter artifact lifecycle (/management/parameters)."""
...
def test_parameter_lifecycle(page: Page, base_url: str) -> None:
    ...
    try:
        ...
        defaults = post_json(
            request,
            "/api/v5/artifacts/parameters/new",
        ...
        param_name = generate_unique_name("E2E Param")
        name_input.fill(param_name)
        description_input = page.get_by_test_id("input-parameter-description")
        description_input.fill("Parameter created via E2E lifecycle test.")
        ...
        param_card = (
            page.get_by_test_id("parameter-card").filter(has_text=param_name).first
        )
        expect(param_card).to_be_visible()
        param_id = param_card.get_attribute("data-parameter-id")
        if param_id:
