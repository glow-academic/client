...
from app.v5.api.socket.client.connect import connect, disconnect
...
async def test_disconnect_with_profile_success(
    ...
    profile_result = await execute_sql_typed(
        conn=db,
        sql_path="tests/v5/sql/integration/queries/socket/helpers/test_get_profile_by_id_v4_complete.sql",
...
async def test_disconnect_removes_active_connections(
    ...
    from app.v5.infra.websocket.set_active_connection import set_active_connection
    ...
    from app.v5.infra.websocket.get_active_connection import get_active_connection
