"""Integration tests for resource instructions create endpoint."""

import httpx
import pytest
from tests.seed_helpers import TEST_SUPERADMIN_PROFILE_ID

pytestmark = pytest.mark.asyncio

BYPASS_CACHE_HEADERS = {"X-Bypass-Cache": "1"}


class TestCreateInstructions:
    """Tests for POST /api/v5/resources/instructions endpoint."""

    async def test_create_instructions_success(self, client: httpx.AsyncClient) -> None:
        """CREATE with valid data returns created ID."""
        # Arrange
        headers = {
            **BYPASS_CACHE_HEADERS,
            "X-Profile-Id": TEST_SUPERADMIN_PROFILE_ID,
        }

        # Act
        response = await client.post(
            "/api/v5/resources/instructions",
            json={"template": "Test instruction template for integration tests"},
            headers=headers,
        )

        # Assert
        assert response.status_code == 200
        data = response.json()
        assert data["instruction_id"] is not None

    async def test_create_instructions_no_profile(
        self, client: httpx.AsyncClient
    ) -> None:
        """CREATE without X-Profile-Id returns 401."""
        # Act
        response = await client.post(
            "/api/v5/resources/instructions",
            json={"template": "Test template"},
            headers=BYPASS_CACHE_HEADERS,
        )

        # Assert
        assert response.status_code == 401
