...
class TestGetModels:
    """Tests for GET /api/v5/resources/models/get endpoint."""

    async def test_get_models_returns_items(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_models_empty_ids(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_models_nonexistent_id(self, client: httpx.AsyncClient) -> None:
...
class TestGetModelsInternal:
    """Tests for get_models_internal function."""
    ...
    async def test_returns_items_for_valid_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Arrange
        search_items = await search_models_internal(db, bypass_cache=True)
        assert len(search_items) > 0
        seed_ids = [item.id for item in search_items[:2]]

        # Act
        items = await get_models_internal(db, seed_ids, bypass_cache=True)
    ...
    async def test_returns_empty_for_empty_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await get_models_internal(db, [], bypass_cache=True)
