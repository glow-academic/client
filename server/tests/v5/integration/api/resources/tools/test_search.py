...
class TestSearchTools:
    """Tests for POST /api/v5/resources/tools/search endpoint."""
    ...
    async def test_search_tools_with_limit(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_search_tools_with_offset(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_search_tools_with_search_text(
...
class TestSearchToolsInternal:
    """Tests for search_tools_internal function."""

    async def test_returns_items(self, db: asyncpg.Connection) -> None:
        """Internal function returns items with no filters."""
        # Act
        items = await search_tools_internal(db, bypass_cache=True)
    ...
    async def test_respects_limit(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await search_tools_internal(db, limit_count=1, bypass_cache=True)
    ...
    async def test_returns_empty_for_zero_limit(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await search_tools_internal(db, limit_count=0, bypass_cache=True)
