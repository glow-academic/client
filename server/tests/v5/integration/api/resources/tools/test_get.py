...
class TestGetTools:
    """Tests for GET /api/v5/resources/tools/get endpoint."""

    async def test_get_tools_returns_items(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_tools_empty_ids(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_tools_nonexistent_id(self, client: httpx.AsyncClient) -> None:
...
class TestGetToolsInternal:
    """Tests for get_tools_internal function."""
    ...
    async def test_returns_items_for_valid_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Arrange
        search_items = await search_tools_internal(db, bypass_cache=True)
        assert len(search_items) > 0
        seed_ids = [item.id for item in search_items[:2]]

        # Act
        items = await get_tools_internal(db, seed_ids, bypass_cache=True)
    ...
    async def test_returns_empty_for_empty_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await get_tools_internal(db, [], bypass_cache=True)
