...
SQL types, views, and functions are now bootstrapped once per session by
...
async def seed_setting_data() -> None:
    """Seed test data needed by setting artifact tests.
...
async def db(seed_setting_data: None) -> AsyncGenerator[asyncpg.Connection, None]:
