...
from tests.seed_ids import SEED_DEPARTMENT_ARTIFACT_ID
...
async def test_returns_base_columns(conn):
    items = await get_departments(conn, [SEED_DEPARTMENT_ARTIFACT_ID])

    assert len(items) == 1
    p = items[0]
    assert p.id == SEED_DEPARTMENT_ARTIFACT_ID
...
async def test_returns_empty_for_unknown_id(conn):
    ...
    items = await get_departments(conn, [nonexistent_id()])
    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_departments(conn, [])
    assert items == []


async def test_fetches_name_ids_when_requested(conn):
    items = await get_departments(conn, [SEED_DEPARTMENT_ARTIFACT_ID], names=True)
...
async def test_no_junctions_when_all_false(conn):
    ...
    p = items[0]
    for field in [
        "name_ids", "description_ids", "flag_ids",
        "settings_ids", "department_ids",
    ]:
