"""Tests for search_settings — black-box using resource + artifact tools only."""
...
async def test_bare_search_returns_results(conn, redis_client):
    """A setting with a name should appear in an unfiltered search."""
...
async def test_department_filter(conn, redis_client):
    """Filter by department_ids returns only matching settings."""
...
async def test_exclude_ids(conn, redis_client):
    """Excluded settings should not appear in results."""
...
async def test_pagination(conn, redis_client):
    ...
    page1 = await search_settings(conn, search=f"page-{tag}", limit_count=2, offset_count=0)
    page2 = await search_settings(conn, search=f"page-{tag}", limit_count=2, offset_count=2)
    page3 = await search_settings(conn, search=f"page-{tag}", limit_count=2, offset_count=4)
...
async def test_active_only_false_includes_inactive(conn, redis_client):
    """active_only=False includes inactive settings."""
