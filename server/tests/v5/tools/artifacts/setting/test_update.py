"""Tests for update_setting — black-box using resource + artifact tools only."""
...
from app.tools.artifacts.setting.update import update_setting
...
async def _create_with_junctions(conn, redis_client):
    ...
    result = await create_setting(conn, name_id=n.id, department_ids=[d1.id, d2.id])
    return result.id, n.id, d1.id, d2.id


async def test_updates_mcp(conn, redis_client):
    result = await create_setting(conn)
    await update_setting(conn, result.id, mcp=True)
...
async def test_replaces_single_select_junction(conn, redis_client):
    ...
    await update_setting(conn, aid, name_id=new_name.id)
    items = await get_settings(conn, [aid], names=True)
...
async def test_keeps_unchanged_single_junction(conn, redis_client):
    ...
    await update_setting(conn, aid, name_id=name_id)
    items = await get_settings(conn, [aid], names=True)
...
async def test_deactivates_removed_multi_ids(conn, redis_client):
    ...
    await update_setting(conn, aid, department_ids=[d1])
    items = await get_settings(conn, [aid], departments=True)
...
async def test_adds_new_multi_ids(conn, redis_client):
    ...
    await update_setting(conn, aid, department_ids=[d1, d2, d3.id])
    items = await get_settings(conn, [aid], departments=True)
...
async def test_clears_all_multi_ids(conn, redis_client):
    aid, _, d1, d2 = await _create_with_junctions(conn, redis_client)
    await update_setting(conn, aid, department_ids=[])
    items = await get_settings(conn, [aid], departments=True)
...
async def test_updates_flags(conn, redis_client):
    ...
    result = await create_setting(conn, flag_ids=[f1.id])
    await update_setting(conn, result.id, flag_ids=[f2.id])
...
async def test_multi_none_means_no_change(conn, redis_client):
    aid, _, d1, d2 = await _create_with_junctions(conn, redis_client)
    await update_setting(conn, aid, department_ids=None)
    items = await get_settings(conn, [aid], departments=True)
