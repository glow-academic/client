...
async def test_hides_inactive_by_default(conn):
    created = await create_field(conn)
    await update_field(conn, created.id, active=False)
...
async def test_returns_inactive_when_active_filter_is_none(conn):
    ...
    items = await get_fields(conn, [created.id], active=None)
...
async def test_can_filter_for_only_inactive(conn):
    active_item = await create_field(conn)
    inactive_item = await create_field(conn)
    await update_field(conn, inactive_item.id, active=False)
