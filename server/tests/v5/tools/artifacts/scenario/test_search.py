...
async def test_bare_search_returns_results(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn)
    assert s.id in ids
...
async def test_text_search_filters_by_name(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, search=f"match-{tag}")
    assert s1.id in ids
...
async def test_text_search_filters_by_description(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, search=f"desc-{tag}")
    assert s1.id in ids
    assert s2.id not in ids
...
async def test_department_filter(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, department_ids=[d1.id])
    assert s1.id in ids
    assert s2.id not in ids
...
async def test_persona_filter(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, persona_ids=[p1_id])
    assert s1.id in ids
    assert s2.id not in ids
...
async def test_exclude_ids(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, exclude_ids=[s1.id])
...
async def test_pagination(conn, redis_client):
    ...
    page1, _total = await search_scenarios(
        conn, search=f"page-{tag}", limit_count=2, offset_count=0
    )
    page2, _total = await search_scenarios(
        conn, search=f"page-{tag}", limit_count=2, offset_count=2
    )
    page3, _total = await search_scenarios(
...
async def test_active_only_false_includes_inactive(conn, redis_client):
    ...
    ids, _total = await search_scenarios(conn, search=name.name, active_only=False)
    assert s.id in ids
