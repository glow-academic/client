...
async def test_pagination(conn, redis_client):
    ...
    page1 = await search_documents(conn, search=f"page-{tag}", limit_count=2, offset_count=0)
    page2 = await search_documents(conn, search=f"page-{tag}", limit_count=2, offset_count=2)
    page3 = await search_documents(conn, search=f"page-{tag}", limit_count=2, offset_count=4)
