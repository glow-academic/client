...
async def test_hard_delete_single(conn, redis_client):
    ...
    p = await create_document(conn, name_id=name.id)
    result = await delete_documents(conn, [p.id])
    assert p.id in result.deleted_ids
    got = await get_documents(conn, [p.id], active=None)
    assert len(got) == 0
...
async def test_soft_delete_sets_inactive(conn, redis_client):
    ...
    p = await create_document(conn, name_id=name.id)
    result = await delete_documents(conn, [p.id], soft=True)
    assert p.id in result.deleted_ids
    got = await get_documents(conn, [p.id], active=None)
    assert len(got) == 1
...
async def test_soft_delete_recoverable(conn, redis_client):
    ...
    p = await create_document(conn, name_id=name.id)
    await delete_documents(conn, [p.id], soft=True)
    row = await conn.fetchrow(
        "SELECT id, active FROM document_artifact WHERE id = $1", p.id
    )
...
async def test_hard_delete_cascades_junctions(conn, redis_client):
    ...
    p = await create_document(conn, name_id=name.id)
    await delete_documents(conn, [p.id])
    row = await conn.fetchrow(
        "SELECT 1 FROM document_names_junction WHERE document_id = $1", p.id
    )
    assert row is None
