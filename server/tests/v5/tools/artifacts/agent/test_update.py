...
async def _create_with_junctions(conn, redis_client):
    ...
    result = await create_agent(conn, name_id=n.id, department_ids=[d1.id, d2.id])
...
async def test_updates_mcp(conn, redis_client):
    ...
    await update_agent(conn, result.id, mcp=True)
...
async def test_replaces_single_select_junction(conn, redis_client):
    ...
    await update_agent(conn, aid, name_id=new_name.id)
...
async def test_keeps_unchanged_single_junction(conn, redis_client):
    ...
    await update_agent(conn, aid, name_id=name_id)
...
async def test_adds_new_multi_ids(conn, redis_client):
    ...
    await update_agent(conn, aid, department_ids=[d1, d2, d3.id])
...
async def test_updates_flags(conn, redis_client):
    ...
    result = await create_agent(conn, flag_ids={f1.id: True})

    await update_agent(conn, result.id, flag_ids={f1.id: False})
