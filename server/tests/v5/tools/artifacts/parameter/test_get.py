...
async def test_returns_base_columns(conn, redis_client):
    ...
    created = await create_parameter(conn, name_id=name.id)

    items = await get_parameters(conn, [created.id])
    ...
    assert p.field_ids is None
    assert p.parameter_ids is None


async def test_returns_empty_for_unknown_id(conn):

    items = await get_parameters(conn, [nonexistent_id()])
    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_parameters(conn, [])
    assert items == []
...
async def test_fetches_name_ids_when_requested(conn, redis_client):
    ...
    items = await get_parameters(conn, [created.id], names=True)
...
async def test_hides_inactive_by_default(conn):
    created = await create_parameter(conn)
    await update_parameter(conn, created.id, active=False)
...
async def test_returns_inactive_when_active_filter_is_none(conn):
    ...
    items = await get_parameters(conn, [created.id], active=None)
...
async def test_can_filter_for_only_inactive(conn):
    active_item = await create_parameter(conn)
    inactive_item = await create_parameter(conn)
    await update_parameter(conn, inactive_item.id, active=False)
