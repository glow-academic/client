...
from tests.seed_ids import SEED_PARAMETER_ARTIFACT_ID
...
async def test_returns_base_columns(conn):
    items = await get_parameters(conn, [SEED_PARAMETER_ARTIFACT_ID])

    assert len(items) == 1
    p = items[0]
    assert p.id == SEED_PARAMETER_ARTIFACT_ID
...
async def test_fetches_name_ids_when_requested(conn):
    items = await get_parameters(conn, [SEED_PARAMETER_ARTIFACT_ID], names=True)
