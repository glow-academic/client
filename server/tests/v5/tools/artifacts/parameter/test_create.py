...
async def test_creates_bare_artifact(conn, redis_client):
    result = await create_parameter(conn)
    assert result.id is not None

    items = await get_parameters(conn, [result.id])
    assert len(items) == 1
...
async def test_passes_mcp_flag(conn, redis_client):
    result = await create_parameter(conn, mcp=True)
...
async def test_links_single_select_junctions(conn, redis_client):
    ...
    result = await create_parameter(conn, name_id=name.id, description_id=desc.id)

    items = await get_parameters(conn, [result.id], names=True, descriptions=True)
    p = items[0]
...
async def test_links_multi_select_junctions(conn, redis_client):
    ...
    result = await create_parameter(conn, department_ids=[d1.id, d2.id])

    items = await get_parameters(conn, [result.id], departments=True)
...
async def test_links_flags_with_value(conn, redis_client):
    ...
    result = await create_parameter(conn, flag_ids=[f1.id, f2.id])

    items = await get_parameters(conn, [result.id], flags=True)
