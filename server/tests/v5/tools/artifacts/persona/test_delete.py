...
async def test_hard_delete_multiple(conn, redis_client):
    ...
    result = await delete_personas(conn, ids)
    assert set(result.deleted_ids) == set(ids)

    got = await get_personas(conn, ids)
    assert len(got) == 0
...
async def test_hard_delete_empty_list(conn, redis_client):
    ...
    result = await delete_personas(conn, [])
    assert result.deleted_ids == []
...
async def test_soft_delete_sets_inactive(conn, redis_client):
    ...
    result = await delete_personas(conn, [p.id], soft=True)
    assert p.id in result.deleted_ids

    # Still exists but inactive
    got = await get_personas(conn, [p.id])
    assert len(got) == 0  # get filters active=true by default
...
async def test_soft_delete_recoverable(conn, redis_client):
    ...
    await delete_personas(conn, [p.id], soft=True)

    # Verify it's still in DB, just inactive
    row = await conn.fetchrow(
        "SELECT id, active FROM persona_artifact WHERE id = $1", p.id
    )
...
async def test_hard_delete_cascades_junctions(conn, redis_client):
    ...
    await delete_personas(conn, [p.id])

    # Junction row should be gone
    row = await conn.fetchrow(
        "SELECT 1 FROM persona_names_junction WHERE persona_id = $1", p.id
    )
    assert row is None
