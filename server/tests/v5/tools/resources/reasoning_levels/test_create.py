"""Tests for create_reasoning_level."""

import pytest

from app.routes.v5.tools.resources.reasoning_levels.create import create_reasoning_level
from app.routes.v5.tools.resources.reasoning_levels.get import get_reasoning_levels

pytestmark = pytest.mark.asyncio


async def test_creates_new_reasoning_level(conn, redis_client):
    result = await create_reasoning_level(conn, "high", redis_client)

    assert result.reasoning_level == "high"
    assert result.active is True
    assert result.mcp is False


async def test_visible_via_get(conn, redis_client):
    result = await create_reasoning_level(conn, "medium", redis_client)

    items = await get_reasoning_levels(
        conn, [result.id], redis_client, bypass_cache=True
    )

    assert len(items) == 1
    assert items[0].id == result.id
    assert items[0].reasoning_level == "medium"


async def test_creates_second_row_for_same_value(conn, redis_client):
    first = await create_reasoning_level(conn, "low", redis_client)
    second = await create_reasoning_level(conn, "low", redis_client)

    assert first.id != second.id
    assert second.reasoning_level == "low"


async def test_sets_mcp_flag(conn, redis_client):
    result = await create_reasoning_level(conn, "high", redis_client, mcp=True)

    assert result.mcp is True
    assert result.generated is True
