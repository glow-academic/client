...
async def test_gets_created_request_limit(conn, redis_client):
    created = await create_request_limit(conn, 100, redis_client)

    items = await get_request_limits(conn, [created.id], redis_client)
...
async def test_cache_hit_skips_db(conn, redis_client):
    created = await create_request_limit(conn, 200, redis_client)
...
async def test_bypass_cache_skips_read_and_write(conn, redis_client):
    created = await create_request_limit(conn, 500, redis_client)

    items = await get_request_limits(conn, [created.id], redis_client, bypass_cache=True)
    assert len(items) == 1

    from app.utils.cache.cache_key import cache_key
    from app.utils.cache.get_cached import get_cached

    key = cache_key("/api/v5/resources/request_limits/get", {"ids": [str(created.id)]})
