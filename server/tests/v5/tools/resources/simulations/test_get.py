...
async def test_gets_created_simulation(conn, redis_client):
    ...
    assert items[0].name == "test-sim"
    assert items[0].description == "desc"
    assert items[0].department_ids == []
    assert items[0].practice is False
...
async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_simulations(conn, [], redis_client)

    assert items == []


async def test_cache_hit_skips_db(conn, redis_client):
    created = await create_simulation(conn, redis_client, name="test-sim-cache")
    ...
    items2 = await get_simulations(conn, [created.id], redis_client)
    assert len(items2) == 1
    assert items2[0].id == created.id


async def test_bypass_cache_skips_read_and_write(conn, redis_client):
    created = await create_simulation(conn, redis_client, name="test-sim-bypass")

    items = await get_simulations(conn, [created.id], redis_client, bypass_cache=True)
    assert len(items) == 1

    from app.utils.cache.cache_key import cache_key
    from app.utils.cache.get_cached import get_cached

    key = cache_key("/api/v5/resources/simulations/get", {"ids": [str(created.id)]})
