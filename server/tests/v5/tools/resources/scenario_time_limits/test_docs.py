...
from app.routes.v5.tools.resources.scenario_time_limits.docs import get_scenario_time_limits_docs
...
async def test_search_operation_has_params(conn):
    result = await get_scenario_time_limits_docs(conn)
    search_op = next(op for op in result.operations if op.name == "search_scenario_time_limits")
