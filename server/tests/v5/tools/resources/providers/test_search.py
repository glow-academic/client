...
async def test_returns_empty_for_no_match(conn, redis_client):
    items = await search_providers(conn, redis_client, search="zzz-no-match-zzz-" + uuid4().hex[:8])
...
async def test_respects_limit(conn, redis_client):
    for i in range(5):
        await create_provider(conn, name=f"limit-test-{uuid4().hex[:6]}", redis=redis_client)

    items = await search_providers(conn, redis_client, search="limit-test-", limit_count=2)

    assert len(items) <= 2


async def test_excludes_ids(conn, redis_client):
    a = await create_provider(conn, name=f"exclude-a-{uuid4().hex[:6]}", redis=redis_client)
    b = await create_provider(conn, name=f"exclude-b-{uuid4().hex[:6]}", redis=redis_client)
...
async def test_cache_hit(conn, redis_client):
    await create_provider(conn, name=f"cache-hit-{uuid4().hex[:6]}", redis=redis_client)
...
async def test_bypass_cache(conn, redis_client):
    await create_provider(conn, name=f"bypass-{uuid4().hex[:6]}", redis=redis_client)

    items = await search_providers(conn, redis_client, search="bypass-", bypass_cache=True)
