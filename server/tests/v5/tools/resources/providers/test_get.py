...
async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_providers(conn, [nonexistent_id()], redis_client)
...
async def test_cache_hit_skips_db(conn, redis_client):
    created = await create_provider(conn, "test-provider-cache-hit", redis=redis_client)
    ...
    items2 = await get_providers(conn, [created.id], redis_client)
    assert len(items2) == 1
    assert items2[0].name == "test-provider-cache-hit"


async def test_bypass_cache_skips_read_and_write(conn, redis_client):
    created = await create_provider(conn, "test-provider-bypass", redis=redis_client)

    items = await get_providers(conn, [created.id], redis_client, bypass_cache=True)
    assert len(items) == 1

    from app.utils.cache.cache_key import cache_key
    from app.utils.cache.get_cached import get_cached

    key = cache_key("/api/v5/resources/providers/get", {"ids": [str(created.id)]})
