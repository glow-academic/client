...
async def test_respects_limit(conn, redis_client):
    for i in range(5):
        await create_objective(
            conn, f"limit-test-obj-{uuid4().hex[:6]}", redis_client
        )
...
async def test_respects_offset(conn, redis_client):
    for i in range(3):
        await create_objective(
            conn, f"offset-test-obj-{uuid4().hex[:6]}", redis_client
        )
...
async def test_excludes_ids(conn, redis_client):
    a = await create_objective(
        conn, f"exclude-a-obj-{uuid4().hex[:6]}", redis_client
    )
    b = await create_objective(
        conn, f"exclude-b-obj-{uuid4().hex[:6]}", redis_client
    )
...
async def test_cache_hit(conn, redis_client):
    await create_objective(
        conn, f"cache-hit-obj-{uuid4().hex[:6]}", redis_client
    )
...
async def test_bypass_cache(conn, redis_client):
    await create_objective(
        conn, f"bypass-obj-{uuid4().hex[:6]}", redis_client
    )
