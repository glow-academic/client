...
async def test_cache_hit_skips_db(conn, redis_client):
    ...
    items2 = await get_temperature_levels(conn, [row_id], redis_client)
    assert len(items2) == 1
...
async def test_bypass_cache_skips_read_and_write(conn, redis_client):
    ...
    items = await get_temperature_levels(conn, [row_id], redis_client, bypass_cache=True)
    assert len(items) == 1

    from app.utils.cache.cache_key import cache_key
    from app.utils.cache.get_cached import get_cached

    key = cache_key("/api/v5/resources/temperature_levels/get", {"ids": [str(row_id)]})
