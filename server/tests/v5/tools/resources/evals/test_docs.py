"""Tests for get_evals_docs."""

import pytest

from app.routes.v5.tools.resources.evals.docs import get_evals_docs

pytestmark = pytest.mark.asyncio


async def test_returns_docs_response(conn):
    result = await get_evals_docs(conn)
...
async def test_includes_source_tables(conn):
    ...
    assert "evals_resource" in table_names
