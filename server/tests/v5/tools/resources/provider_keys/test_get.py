...
async def test_cache_hit_skips_db(conn, redis_client):
    ...
    key = await create_key(conn, redis_client, name="test-key-cache", key="sk-cache-123")
    from app.routes.v5.tools.resources.provider_keys.create import create_provider_key
...
async def test_bypass_cache_skips_read_and_write(conn, redis_client):
    ...
    key = await create_key(conn, redis_client, name="test-key-bypass", key="sk-bypass-123")
    from app.routes.v5.tools.resources.provider_keys.create import create_provider_key
