...
async def test_finds_created_name(conn, redis_client):
    await create_name(conn, "search-test-alpha", redis_client)

    items = await search_names(conn, redis_client, search="search-test-alpha")
...
async def test_search_is_case_insensitive(conn, redis_client):
    await create_name(conn, "CaseTest-Search", redis_client)

    items = await search_names(conn, redis_client, search="casetest-search")

    assert any(i.name == "CaseTest-Search" for i in items)
...
async def test_respects_offset(conn, redis_client):
    ...
    offset_items = await search_names(conn, redis_client, search="offset-test-", limit_count=10, offset_count=1)
...
async def test_returns_empty_for_zero_limit(conn, redis_client):
    items = await search_names(conn, redis_client, limit_count=0)
