...
async def test_returns_empty_for_no_match(conn, redis_client):
    items = await search_prompts(conn, redis_client, search="zzz-no-match-zzz-" + unique_tag())
...
async def test_respects_limit(conn, redis_client):
    for i in range(5):
        await create_prompt(conn, "sys", f"limit-test-{unique_tag()}", "desc", redis_client)
...
async def test_respects_offset(conn, redis_client):
    for i in range(3):
        await create_prompt(conn, "sys", f"offset-test-{unique_tag()}", "desc", redis_client)
...
async def test_excludes_ids(conn, redis_client):
    a = await create_prompt(conn, "sys", f"exclude-a-{unique_tag()}", "desc", redis_client)
    b = await create_prompt(conn, "sys", f"exclude-b-{unique_tag()}", "desc", redis_client)
