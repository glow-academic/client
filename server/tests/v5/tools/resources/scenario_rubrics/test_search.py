...
async def test_respects_offset(conn, redis_client):
    ...
    offset_items = await search_scenario_rubrics(conn, redis_client, limit_count=1000, offset_count=1)
...
async def test_excludes_ids(conn, redis_client):
    ...
    items = await search_scenario_rubrics(conn, redis_client, limit_count=1000, exclude_ids=[a.id])
...
async def test_bypass_cache(conn, redis_client):
    ...
    items = await search_scenario_rubrics(conn, redis_client, limit_count=1000, bypass_cache=True)
