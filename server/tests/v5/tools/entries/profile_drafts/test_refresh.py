"""Tests for refresh_profile_drafts."""
...
async def test_new_draft_appears_in_mv_after_refresh(conn, profile_id):
    ...
    result = await create_profile_draft(conn, group_id=group.id, session_id=session.id)

    row = await conn.fetchrow("SELECT id FROM profile_drafts_mv WHERE id = $1", result.id)
    assert row is None
