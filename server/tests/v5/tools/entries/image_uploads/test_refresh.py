...
async def _setup(conn):
    ...
    parent = await create_image(conn, session_id=session.id)
...
async def test_new_upload_appears_in_mv_after_refresh(conn):
    ...
    result = await create_image_upload(
        conn, image_id=parent.id, upload_id=upload.id, session_id=session.id
    )

    row = await conn.fetchrow("SELECT id FROM image_uploads_mv WHERE id = $1", result.id)
    assert row is None
