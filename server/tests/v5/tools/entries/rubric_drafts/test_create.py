"""Tests for rubric_drafts create."""

import pytest

from app.tools.v5.entries.groups.create import create_group
from app.tools.v5.entries.rubric_drafts.create import create_rubric_draft
...
async def test_create_returns_id(conn, profile_id):
    ...
    result = await create_rubric_draft(conn, group_id=group.id, session_id=session.id)
...
async def test_roundtrip_base_fields(conn, profile_id):
    ...
    items = await get_rubric_drafts(conn, [result.id])
