...
async def test_search_finds_created(conn, profile_id):
    ...
    items = await search_rubric_drafts(conn, group_id=group.id)
...
async def test_search_filters_by_group(conn, profile_id):
    ...
    items = await search_rubric_drafts(conn, group_id=nonexistent_id())
...
async def test_search_filters_by_session(conn, profile_id):
    ...
    items = await search_rubric_drafts(conn, session_id=session.id)
...
async def test_search_pagination(conn, profile_id):
    ...
    items = await search_rubric_drafts(conn, group_id=group.id, limit=1)
