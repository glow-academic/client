"""Tests for agent_drafts create wrapper."""

import pytest

from app.routes.v5.tools.entries.agent_drafts.create import create_agent_drafts
...
async def test_create_returns_id(conn):
    ...
    result = await create_agent_drafts(conn, group_id=group.id, session_id=session.id)
...
async def test_roundtrip(conn):
    ...
    items = await get_agent_drafts(conn, [result.id])
