...
async def test_search_finds_created(conn):
    ...
    items = await search_agent_drafts(conn, group_id=group.id)
...
async def test_search_filters_by_group(conn):
    session, group = await _setup(conn)
    await create_agent_draft(conn, group_id=group.id, session_id=session.id)

    items = await search_agent_drafts(conn, group_id=nonexistent_id())
...
async def test_search_filters_by_session(conn):
    ...
    items = await search_agent_drafts(conn, session_id=session.id)
...
async def test_search_pagination(conn):
    ...
    items = await search_agent_drafts(conn, group_id=group.id, limit=1)
