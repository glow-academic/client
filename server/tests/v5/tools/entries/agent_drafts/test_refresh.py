"""Tests for refresh_agent_drafts."""
...
async def test_new_draft_appears_in_mv_after_refresh(conn):
    ...
    result = await create_agent_draft(conn, group_id=group.id, session_id=session.id)

    row = await conn.fetchrow("SELECT id FROM agent_drafts_mv WHERE id = $1", result.id)
    assert row is None
