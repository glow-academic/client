"""Tests for create_attempt_highlight."""
...
async def _attempt_highlight(conn, **overrides):
    ...
    await create_attempt_message(
        conn, chat_id=attempt_chat.id, call_id=call2.id, message_id=msg_id
    )
    ...
    defaults = dict(
        strength_id=strength.id, call_id=call2.id, section="Good opening"
    )
...
async def test_returns_id(conn):
    result = await _attempt_highlight(conn)
...
async def test_visible_via_get_after_refresh(conn):
    ...
    items = await get_attempt_highlights(conn, [result.id])
    assert len(items) == 1


async def test_passes_mcp_flag(conn):
    result = await _attempt_highlight(conn, mcp=True)
