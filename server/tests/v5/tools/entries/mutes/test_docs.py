"""Tests for get_mutes_docs."""

import pytest

from app.routes.v5.tools.entries.mutes.docs import get_mutes_docs

pytestmark = pytest.mark.asyncio


async def test_returns_docs(conn):
    result = await get_mutes_docs(conn)

    assert result.name == "mutes"
...
async def test_includes_mv(conn):
    ...
    assert result.materialized_view.name == "mutes_mv"
...
async def test_includes_tables(conn):
    ...
    assert "mutes_entry" in table_names
...
async def test_includes_operations(conn):
    ...
    assert "create_mute" in op_names
    assert "refresh_mutes" in op_names
    assert "get_mutes" in op_names
    assert "search_mutes" in op_names


async def test_search_has_filters(conn):
    result = await get_mutes_docs(conn)

    search_op = next(op for op in result.operations if op.name == "search_mutes")
