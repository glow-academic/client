...
async def test_search_filters_by_message_id(conn):
    session, message, upload = await _deps(conn)
    await create_message_upload(
        conn, message_id=message.id, upload_id=upload.id, session_id=session.id
    )

    results = await search_message_uploads(conn, message_id=uuid4())
...
async def test_search_filters_by_upload_id(conn):
    ...
    results = await search_message_uploads(conn, upload_id=uuid4())

    assert len(results) == 0


async def test_search_pagination(conn):
    session, message, upload1 = await _deps(conn)
