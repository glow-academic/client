"""Tests for scenario_drafts search wrapper."""
...
async def test_filters_by_group(conn):
    session, group = await _setup(conn)
    await create_scenario_drafts(conn, group_id=group.id, session_id=session.id)

    items = await search_scenario_drafts(conn, group_id=uuid4())
