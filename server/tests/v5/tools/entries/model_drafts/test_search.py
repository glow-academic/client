"""Tests for model_drafts search wrapper."""

from uuid import uuid4

import pytest

from app.routes.v5.tools.entries.model_drafts.create import create_model_drafts
...
async def test_finds_created(conn):
    ...
    result = await create_model_drafts(conn, group_id=group.id, session_id=session.id)
...
async def test_filters_by_group(conn):
    session, group = await _setup(conn)
    await create_model_drafts(conn, group_id=group.id, session_id=session.id)
