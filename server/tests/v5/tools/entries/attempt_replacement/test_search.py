"""Tests for search_attempt_replacements."""
...
from app.tools.v5.entries.attempt_improvement.create import (
    create_attempt_improvement,
)
from app.tools.v5.entries.attempt_message.create import create_attempt_message
from app.tools.v5.entries.attempt_replacement.create import (
    create_attempt_replacement,
)
from app.tools.v5.entries.attempt_replacement.refresh import (
...
async def test_filters_by_improvement_id(conn, profile_id):
...
async def test_returns_all_without_filter(conn, profile_id):
    ...
    items = await search_attempt_replacements(conn)
