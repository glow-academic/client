...
async def test_search_filters_by_group(conn, profile_id):
    session, group = await _setup(conn, profile_id)
    await create_provider_draft(conn, group_id=group.id, session_id=session.id)

    items = await search_provider_drafts(conn, group_id=nonexistent_id())
...
async def test_search_filters_by_session(conn, profile_id):
    ...
    items = await search_provider_drafts(conn, session_id=session.id)
...
async def test_search_pagination(conn, profile_id):
    ...
    items = await search_provider_drafts(conn, group_id=group.id, limit=1)
