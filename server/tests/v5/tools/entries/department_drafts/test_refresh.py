"""Tests for refresh_department_drafts."""
...
async def test_new_draft_appears_in_mv_after_refresh(conn):
    ...
    result = await create_department_draft(conn, group_id=group.id, session_id=session.id)

    row = await conn.fetchrow("SELECT id FROM department_drafts_mv WHERE id = $1", result.id)
    assert row is None
