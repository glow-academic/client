"""Tests for get_department_drafts_docs."""
...
async def test_returns_docs_response(conn):
    ...
    assert result.name == "department_drafts"
...
async def test_includes_materialized_view(conn):
    ...
    assert result.materialized_view.name == "department_drafts_mv"
...
async def test_includes_source_tables(conn):
    ...
    assert "department_drafts_entry" in table_names
    assert "department_drafts_names_connection" in table_names
...
async def test_includes_all_operations(conn):
    ...
    assert "create_department_draft" in op_names
    assert "refresh_department_drafts" in op_names
    assert "get_department_drafts" in op_names
    assert "search_department_drafts" in op_names
