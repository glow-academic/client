...
async def test_returns_multiple(conn):
    session = await _session(conn)
    r1 = await create_problem(conn, session_id=session.id, type="bug")
    r2 = await create_problem(conn, session_id=session.id, type="feature")
    await refresh_problems(conn)
...
async def test_bypass_mv_returns_without_refresh(conn):
    session = await _session(conn)
    result = await create_problem(conn, session_id=session.id, type="other")
