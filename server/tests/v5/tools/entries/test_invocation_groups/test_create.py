...
from app.tools.v5.entries.test_invocation_groups.create import (
    create_test_invocation_groups,
)
from app.tools.v5.entries.test_invocation_groups.get import (
    get_test_invocation_groups,
)
from app.tools.v5.entries.test_invocation_groups.refresh import (
...
async def test_returns_id(conn, profile_id):
    result = await _test_invocation_groups(conn, profile_id)
...
async def test_passes_mcp_flag(conn, profile_id):
    result = await _test_invocation_groups(conn, profile_id, mcp=True)
