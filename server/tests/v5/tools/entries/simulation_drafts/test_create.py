"""Tests for simulation_drafts create."""
...
async def test_create_without_connections_returns_empty_lists(conn, profile_id):
    ...
    assert items[0].scenario_flag_ids == []
    assert items[0].scenario_position_ids == []
    assert items[0].scenario_rubric_ids == []
    assert items[0].scenario_time_limit_ids == []
    assert items[0].scenario_ids == []
