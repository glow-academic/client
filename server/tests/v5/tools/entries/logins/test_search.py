...
async def test_filters_by_profile(conn):
    ...
    items = await search_logins(conn, profile_id=SUPERADMIN_PROFILES_RESOURCE_ID)
...
async def test_pagination_limit(conn):
    ...
    items = await search_logins(conn, session_id=session.id, limit=1)
...
async def test_bypass_mv_finds_without_refresh(conn):
    ...
    items = await search_logins(conn, session_id=session.id, bypass_mv=True)
