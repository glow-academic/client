...
from app.tools.v5.entries.attempt_content.create import create_attempt_content
from app.tools.v5.entries.attempt_content.get import get_attempt_contents
from app.tools.v5.entries.attempt_content.refresh import refresh_attempt_content
...
async def test_returns_id(conn, profile_id):
    result = await _attempt_content(conn, profile_id)
...
async def test_passes_mcp_flag(conn, profile_id):
    result = await _attempt_content(conn, profile_id, mcp=True)
