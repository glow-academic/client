"""Tests for parameter_drafts create wrapper."""

import pytest

from app.routes.v5.tools.entries.parameter_drafts.create import create_parameter_drafts
...
async def test_create_returns_id(conn):
    ...
    result = await create_parameter_drafts(conn, group_id=group.id, session_id=session.id)
