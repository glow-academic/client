...
async def test_search_finds_created(conn, profile_id):
    ...
    items = await search_setting_drafts(conn, group_ids=[group.id])
...
async def test_search_filters_by_group(conn, profile_id):
    ...
    items = await search_setting_drafts(conn, group_ids=[nonexistent_id()])
...
async def test_search_pagination(conn, profile_id):
    ...
    items = await search_setting_drafts(conn, group_ids=[group.id], limit=1)
