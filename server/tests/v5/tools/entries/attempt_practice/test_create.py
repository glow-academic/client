...
async def _attempt_practice(conn, **overrides):
...
async def test_returns_ids(conn):
    result, attempt, practice = await _attempt_practice(conn)
...
async def test_row_exists(conn):
    result, _, _ = await _attempt_practice(conn)
...
async def test_passes_mcp_flag(conn):
    result, _, _ = await _attempt_practice(conn, mcp=True)
