...
-- Returns simulation_id for use in view_tests_entry
...
AS $$
    ...
    simulation_active_flag_link AS (
        ...
        SELECT ns.id, af.id, COALESCE(test_create_test_simulation_with_rubric_v4.active, true)
        FROM new_simulation ns, active_flag af
        RETURNING simulation_id
    ),
    simulation_practice_flag_link AS (
        INSERT INTO simulation_flags_junction (simulation_id, flag_id, value)
        SELECT ns.id, pf.id, COALESCE(test_create_test_simulation_with_rubric_v4.practice_simulation, false)
