...
AS $$
    ...
    WHERE EXISTS (SELECT 1 FROM eval_flags_junction ef JOIN flags_resource fl ON ef.flag_id = fl.id WHERE ef.eval_id = e.id AND fl.name = 'active'  AND ef.value = TRUE)
    LIMIT 1;
$$;