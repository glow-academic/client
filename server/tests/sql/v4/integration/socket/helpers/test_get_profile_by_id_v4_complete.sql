...
STABLE
AS $$
    SELECT EXISTS (SELECT 1 FROM profile_flags_junction pf JOIN flags_resource fl ON pf.flag_id = fl.id WHERE pf.profile_id = p.id AND fl.name = 'active'  AND pf.value = TRUE) as active
