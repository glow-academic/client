...
CREATE OR REPLACE FUNCTION test_create_test_rubric_v4(
    ...
    rubric_points_junction integer,
...
AS $$
    WITH new_rubric AS (
        INSERT INTO rubrics_resource(created_at)
        VALUES (NOW())
    ...
    rubric_flag_link AS (
        ...
        SELECT nr.id, af.id, rubric_active
        FROM new_rubric nr, active_flag af
        RETURNING rubric_id
    )
    SELECT 
        nr.id AS rubric_id,
        (SELECT n.name FROM rubric_names_junction rn JOIN names_resource n ON rn.name_id = n.id WHERE rn.rubric_id = nr.id LIMIT 1) AS name,
        (SELECT d.description FROM rubric_descriptions_junction rd JOIN descriptions_resource d ON rd.description_id = d.id WHERE rd.rubric_id = nr.id LIMIT 1) AS description,
        (SELECT p.value FROM rubric_points_junction rp JOIN points_resource p ON rp.point_id = p.id WHERE rp.rubric_id = nr.id AND rp.type = 'total'::point_type LIMIT 1) AS points,
        (SELECT p.value FROM rubric_points_junction rp JOIN points_resource p ON rp.point_id = p.id WHERE rp.rubric_id = nr.id AND rp.type = 'pass'::point_type LIMIT 1) AS pass_points,
        EXISTS (SELECT 1 FROM rubric_flags_junction rf JOIN flags_resource fl ON rf.flag_id = fl.id WHERE rf.rubric_id = nr.id AND fl.name = 'active'  AND rf.value = TRUE) AS active,
        nr.created_at
    FROM new_rubric nr;
$$;
