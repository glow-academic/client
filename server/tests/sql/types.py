"""SQL type definitions and registry - AUTO-GENERATED - DO NOT EDIT MANUALLY.

This file is automatically generated by sql-compile. All edits will be overwritten.
"""

from datetime import date, datetime
from typing import Any
from typing import TYPE_CHECKING, Literal, Type, TypeVar, overload, cast

from uuid import UUID

from pydantic import BaseModel
from pydantic import Field


# Type variables for generic return types
TInput = TypeVar("TInput", bound=BaseModel)
TOutput = TypeVar("TOutput", bound=BaseModel)

# Type alias for SQL strings loaded from files (semantic clarity)
SqlString = str

# Runtime fallback for missing generated types.
# This keeps server startup resilient when incremental SQL compilation misses some files.
_missing_type_cache: dict[str, type[BaseModel]] = {}

def _build_missing_type(name: str) -> type[BaseModel]:
    """Build a permissive placeholder model for missing generated types."""
    class _MissingSqlType(BaseModel):
        model_config = {"extra": "allow"}

        def to_tuple(self) -> tuple[Any, ...]:
            return tuple(self.model_dump().values())

    _MissingSqlType.__name__ = name
    return _MissingSqlType

def __getattr__(name: str) -> Any:
    """Resolve missing generated classes at import-time with placeholders."""
    if not (
        name.endswith(("SqlParams", "SqlRow", "ApiRequest", "ApiResponse"))
        or name.startswith("Q")
    ):
        raise AttributeError(f"module {__name__!r} has no attribute {name!r}")

    cached = _missing_type_cache.get(name)
    if cached is None:
        cached = _build_missing_type(name)
        _missing_type_cache[name] = cached
    return cached


# ============================================================================
# TYPE DEFINITIONS
# ============================================================================


# Generated from: create_test_profile

class CreateTestProfileSqlParams(BaseModel):

    profile_first_name: str
    profile_last_name: str
    profile_role: str | None = None
    profile_active: bool | None = True

    def to_tuple(self) -> tuple[Any, ...]:
        return (
            self.profile_first_name,
            self.profile_last_name,
            self.profile_role,
            self.profile_active,
        )

class CreateTestProfileSqlRow(BaseModel):

    profile_id: UUID | None = None
    first_name: str | None = None
    last_name: str | None = None
    role: str | None = None
    active: bool | None = None
    created_at: datetime | None = None

class CreateTestProfileApiRequest(BaseModel):

    profile_first_name: str
    profile_last_name: str
    profile_role: str | None = None
    profile_active: bool | None = True

class CreateTestProfileApiResponse(BaseModel):

    profile_id: UUID | None = None
    first_name: str | None = None
    last_name: str | None = None
    role: str | None = None
    active: bool | None = None
    created_at: datetime | None = None



# Generated from: get_or_create_test_profile

class GetOrCreateTestProfileSqlParams(BaseModel):

    email: str | None = None
    role: str | None = None
    first_name: str | None = None
    last_name: str | None = None

    def to_tuple(self) -> tuple[Any, ...]:
        return (
            self.email,
            self.role,
            self.first_name,
            self.last_name,
        )

class GetOrCreateTestProfileSqlRow(BaseModel):

    profile_id: UUID | None = None
    email: str | None = None
    role: str | None = None

class GetOrCreateTestProfileApiRequest(BaseModel):

    email: str | None = None
    role: str | None = None
    first_name: str | None = None
    last_name: str | None = None

class GetOrCreateTestProfileApiResponse(BaseModel):

    profile_id: UUID | None = None
    email: str | None = None
    role: str | None = None



# Generated from: test_create_test_guest_profile_v4_complete

class TestCreateTestGuestProfileV4CompleteSqlParams(BaseModel):

    email: str | None = None

    def to_tuple(self) -> tuple[Any, ...]:
        return (
            self.email,
        )

class TestCreateTestGuestProfileV4CompleteSqlRow(BaseModel):

    guest_id: UUID | None = None
    email: str | None = None

class TestCreateTestGuestProfileV4CompleteApiRequest(BaseModel):

    email: str | None = None

class TestCreateTestGuestProfileV4CompleteApiResponse(BaseModel):

    guest_id: UUID | None = None
    email: str | None = None



# Generated from: test_get_profile_activity_v4_complete

class TestGetProfileActivityV4CompleteSqlParams(BaseModel):

    profile_id: UUID

    def to_tuple(self) -> tuple[Any, ...]:
        return (
            self.profile_id,
        )

class TestGetProfileActivityV4CompleteSqlRow(BaseModel):

    last_active: datetime | None = None

class TestGetProfileActivityV4CompleteApiRequest(BaseModel):

    pass

class TestGetProfileActivityV4CompleteApiResponse(BaseModel):

    last_active: datetime | None = None



# Generated from: test_get_profile_by_id_v4_complete

class TestGetProfileByIdV4CompleteSqlParams(BaseModel):

    profile_id: UUID

    def to_tuple(self) -> tuple[Any, ...]:
        return (
            self.profile_id,
        )

class TestGetProfileByIdV4CompleteSqlRow(BaseModel):

    active: bool | None = None

class TestGetProfileByIdV4CompleteApiRequest(BaseModel):

    pass

class TestGetProfileByIdV4CompleteApiResponse(BaseModel):

    active: bool | None = None




# ============================================================================
# REGISTRY
# ============================================================================

_registry: dict[str, tuple[str, str, str, str]] = {
    "tests/sql/integration/queries/api/profile/test_create_test_profile_v4_complete.sql": (
        "CreateTestProfileSqlParams",
        "CreateTestProfileSqlRow",
        "CreateTestProfileApiRequest",
        "CreateTestProfileApiResponse",
    ),
    "tests/sql/integration/queries/helpers/test_get_or_create_test_profile_v4_complete.sql": (
        "GetOrCreateTestProfileSqlParams",
        "GetOrCreateTestProfileSqlRow",
        "GetOrCreateTestProfileApiRequest",
        "GetOrCreateTestProfileApiResponse",
    ),
    "tests/sql/integration/queries/socket/helpers/test_create_test_guest_profile_v4_complete.sql": (
        "TestCreateTestGuestProfileV4CompleteSqlParams",
        "TestCreateTestGuestProfileV4CompleteSqlRow",
        "TestCreateTestGuestProfileV4CompleteApiRequest",
        "TestCreateTestGuestProfileV4CompleteApiResponse",
    ),
    "tests/sql/integration/queries/socket/helpers/test_get_profile_activity_v4_complete.sql": (
        "TestGetProfileActivityV4CompleteSqlParams",
        "TestGetProfileActivityV4CompleteSqlRow",
        "TestGetProfileActivityV4CompleteApiRequest",
        "TestGetProfileActivityV4CompleteApiResponse",
    ),
    "tests/sql/integration/queries/socket/helpers/test_get_profile_by_id_v4_complete.sql": (
        "TestGetProfileByIdV4CompleteSqlParams",
        "TestGetProfileByIdV4CompleteSqlRow",
        "TestGetProfileByIdV4CompleteApiRequest",
        "TestGetProfileByIdV4CompleteApiResponse",
    ),
}


# ============================================================================
# HELPER FUNCTIONS
# ============================================================================

def get_sql_types(sql_path: str) -> tuple[Type[BaseModel], Type[BaseModel]]:
    """Get SQL input and output types for a SQL file path.
    
    Args:
        sql_path: SQL file path (e.g., "app/sql/queries/agents/get_agent_new_complete.sql")
    
    Returns:
        Tuple of (SqlParamsType, SqlRowType)
    
    Raises:
        ValueError: If no types are found for the SQL file path
    """
    if sql_path not in _registry:
        raise ValueError(f"No types found for SQL path: {sql_path}")
    
    sql_params_class, sql_row_class, _, _ = _registry[sql_path]
    
    # Get class from current module
    import sys
    current_module = sys.modules[__name__]
    sql_params_type = getattr(current_module, sql_params_class)
    sql_row_type = getattr(current_module, sql_row_class)
    
    return sql_params_type, sql_row_type


def get_api_types(sql_path: str) -> tuple[Type[BaseModel], Type[BaseModel]]:
    """Get API request and response types for a SQL file path.
    
    Args:
        sql_path: SQL file path (e.g., "app/sql/queries/agents/get_agent_new_complete.sql")
    
    Returns:
        Tuple of (ApiRequestType, ApiResponseType)
    
    Raises:
        ValueError: If no types are found for the SQL file path
    """
    if sql_path not in _registry:
        raise ValueError(f"No types found for SQL path: {sql_path}")
    
    _, _, api_request_class, api_response_class = _registry[sql_path]
    
    # Get class from current module
    import sys
    current_module = sys.modules[__name__]
    api_request_type = getattr(current_module, api_request_class)
    api_response_type = getattr(current_module, api_response_class)
    
    return api_request_type, api_response_type


# Overload declarations for load_sql_query() - provides strong type hints
# Auto-generated by sql-compile. Do not edit manually.
if TYPE_CHECKING:
    @overload
    def load_sql_query(
        file_path: Literal["tests/sql/integration/queries/api/profile/test_create_test_profile_v4_complete.sql"]
    ) -> SqlString: ...

    @overload
    def load_sql_query(
        file_path: Literal["tests/sql/integration/queries/helpers/test_get_or_create_test_profile_v4_complete.sql"]
    ) -> SqlString: ...

    @overload
    def load_sql_query(
        file_path: Literal["tests/sql/integration/queries/socket/helpers/test_create_test_guest_profile_v4_complete.sql"]
    ) -> SqlString: ...

    @overload
    def load_sql_query(
        file_path: Literal["tests/sql/integration/queries/socket/helpers/test_get_profile_activity_v4_complete.sql"]
    ) -> SqlString: ...

    @overload
    def load_sql_query(
        file_path: Literal["tests/sql/integration/queries/socket/helpers/test_get_profile_by_id_v4_complete.sql"]
    ) -> SqlString: ...

    @overload
    def load_sql_query(
        file_path: str
    ) -> SqlString: ...


def load_sql_query(
    file_path: str,
) -> SqlString:
    """Load SQL file content and return as string.

    Returns the SQL query string from the specified file path.
    Uses Literal overloads to provide strong type hints for file paths.

    Args:
        file_path: Relative path from server root (e.g., "tests/sql/integration/infra/activity/insert_test_profile.sql")

    Returns:
        SQL string with parameter placeholders ($1, $2, etc.)

    Example:
        ```python
        sql_query = load_sql_query("tests/sql/integration/infra/activity/insert_test_profile.sql")
        # sql_query is typed as SqlString
        ```
    """
    # Import here to avoid circular imports
    from app.utils.sql_helper import load_sql

    return load_sql(file_path)