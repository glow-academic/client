...
class TestStartAttempt:
    ...
    def test_start_attempt_success_with_user(
        self,
        sample_simulation,
        sample_scenario,
        sample_agent,
        ...
        mock_session.exec.side_effect = [
            MagicMock(
                one_or_none=MagicMock(return_value=sample_simulation)
            ),  # simulation query
            MagicMock(
                one_or_none=MagicMock(return_value=sample_scenario)
            ),  # scenario query
    ...
    def test_start_attempt_success_guest_mode(
        self, sample_simulation, sample_scenario, sample_agent, sample_class
    ):
    ...
    def test_start_attempt_empty_string_profile_id(
        self, sample_simulation, sample_scenario, sample_agent, sample_class
    ):
    ...
    def test_start_attempt_simulation_not_found(self):
        ...
        try:
            ...
            response = client.post(
                "/simulations/start",
                data={"simulation_id": str(uuid4()), "class_id": str(uuid4())},
            )
...
class TestContinueAttempt:
    ...
    def test_continue_attempt_success_with_next_scenario(
        self, mock_run_grade_agent, sample_simulation, sample_scenario, sample_agent
    ):
        ...
        mock_session.exec.side_effect = [
            ...
            MagicMock(
                one_or_none=MagicMock(return_value=mock_attempt)
            ),  # attempt query
            MagicMock(
                one_or_none=MagicMock(return_value=sample_simulation)
            ),  # simulation query
            MagicMock(
                one_or_none=MagicMock(return_value=next_scenario)
            ),  # next scenario query
            MagicMock(one_or_none=MagicMock(return_value=sample_agent)),  # agent query
        ]
    ...
    @patch("app.routes.simulations.run_grade_agent")
    def test_continue_attempt_success_no_more_scenarios(
        self, mock_run_grade_agent, sample_simulation
    ):
        ...
        try:
            ...
            )  # Should be completed since no more scenarios
        finally:
            # Clean up
            app.dependency_overrides.clear()
