...
class TestStartEval:
    ...
    def test_start_eval_success(
        self,
        mock_eval_runs_class,
        mock_select,
        mock_get_session,
        client,
        sample_eval_data,
        ...
        response = client.post(
            "/evals/start",
            data={"eval_id": str(sample_eval_data["id"]), "class_id": str(uuid4())},
        )
    ...
    def test_start_eval_not_found(self, mock_get_session, client):
        ...
        response = client.post(
            "/evals/start", data={"eval_id": str(uuid4()), "class_id": str(uuid4())}
        )
...
class TestRunMultipleEvals:
    ...
    def test_run_multiple_evals_success(
        self,
        mock_eval_chats_class,
        mock_parallel_agent,
        sample_eval_data,
        sample_agents_data,
        sample_scenarios_data,
        ...
        mock_session.exec.side_effect = [
            ...
            MagicMock(
                one=MagicMock(return_value=scenario_mock)
            ),  # scenario query (first)
            MagicMock(
                one=MagicMock(return_value=agent1_mock)
            ),  # query agent query (first)
            MagicMock(
                one=MagicMock(return_value=agent2_mock)
            ),  # response agent query (first)
            MagicMock(
                one=MagicMock(return_value=scenario_mock)
            ),  # scenario query (second)
            MagicMock(
                one=MagicMock(return_value=agent1_mock)
            ),  # query agent query (second)
            MagicMock(
                one=MagicMock(return_value=agent2_mock)
            ),  # response agent query (second)
        ]
        ...
        try:
            ...
            response = client.post(
                "/evals/run-multiple", data={"eval_id": str(sample_eval_data["id"])}
            )
    ...
    def test_run_multiple_evals_not_found(self):
        ...
        try:
            ...
            response = client.post(
                "/evals/run-multiple", data={"eval_id": str(uuid4())}
            )
...
class TestRunEval:
    ...
    def test_run_eval_success(
        ...
        async def mock_stream_events():
            ...
            yield MagicMock(
                type="raw_response_event", data=MagicMock(delta=" response")
            )
        ...
        response = client.post(
            "/evals/run", data={"eval_run_id": str(eval_run_data["id"])}
        )
    ...
    def test_run_eval_not_found(self, mock_get_session, client):
        ...
        response = client.post("/evals/run", data={"eval_run_id": str(uuid4())})
...
class TestGenerateNaturalOpening:
    ...
    def test_generate_student_opening(self, sample_scenarios_data, sample_agents_data):
        ...
        assert any(
            ...
            for word in ["trouble", "stuck", "help", "confused", "wrong"]
        )
    ...
    def test_generate_ta_opening(self, sample_scenarios_data, sample_agents_data):
        ...
        assert any(
            word in opening.lower()
            for word in ["help", "working", "questions", "together", "concern"]
        )
...
class TestRunAgentConversation:
    ...
    def test_run_agent_conversation_success(
        self,
        mock_generic_agent_class,
        mock_runner,
        sample_agents_data,
        sample_scenarios_data,
    ):
    ...
    @patch("app.routes.evals.Runner")
    @patch("app.services.agents.generic.GenericAgent")
    def test_run_agent_conversation_error(
        self,
        mock_generic_agent_class,
        mock_runner,
        sample_agents_data,
