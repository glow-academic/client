"""E2E tests for departments list SSR, search, filters, and pagination."""

from __future__ import annotations

import pytest
from playwright.sync_api import Page, expect

from server.tests.e2e.departments.helpers import (
    create_department_api,
    delete_department_api,
    fetch_departments_list,
    generate_unique_department_name,
)

ADMIN_PROFILE_ID = "6a2518eb-eba7-4650-aee0-d387c3fb8265"

pytestmark = [pytest.mark.e2e, pytest.mark.test_profile_id(ADMIN_PROFILE_ID)]


def test_departments_list_filters_and_empty_state(page: Page, base_url: str) -> None:
    """Ensure department list SSR renders and search/filter flows work."""
    page.goto(f"{base_url}/management/departments")
    page.wait_for_load_state("networkidle")

    grid = page.get_by_test_id("departments-grid")
    grid.wait_for(state="visible", timeout=15000)
    expect(grid).to_be_visible()

    cards = grid.get_by_test_id("department-card")
    initial_count = cards.count()
    assert initial_count > 0

    first_card = cards.first
    department_label = first_card.get_attribute("aria-label") or ""
    search_title = (
        department_label.replace("department card ", "").strip()
        or first_card.inner_text().splitlines()[0].strip()
    )

    search_input = page.get_by_test_id("departments-search")
    search_input.wait_for(state="visible", timeout=10000)
    search_input.fill(search_title)
    page.wait_for_timeout(250)
    filtered_count = cards.count()
    assert filtered_count <= initial_count
    assert (
        grid.get_by_test_id("department-card").filter(has_text=search_title).count() > 0
    )

    search_input.fill("")
    page.wait_for_timeout(250)
    assert cards.count() == initial_count

    toolbar = page.get_by_test_id("departments-toolbar")
    price_spent_button = toolbar.get_by_role("button", name="Price Spent")
    if price_spent_button.is_visible():
        price_spent_button.click()
        price_options = page.get_by_role("option")
        if price_options.count() > 1:
            option = price_options.nth(1)
            option.click()
            page.wait_for_timeout(250)
            assert cards.count() > 0
            price_spent_button.click()
            clear_option_locator = page.get_by_role("option").filter(
                has_text="Clear filters"
            )
            if clear_option_locator.count():
                clear_option_locator.first.click(force=True)
            else:
                page.get_by_role("option").nth(0).click()
            page.wait_for_timeout(250)
            assert cards.count() == initial_count
        else:
            price_spent_button.click()

    staff_count_button = toolbar.get_by_role("button", name="Staff Count")
    if staff_count_button.is_visible():
        staff_count_button.click()
        staff_options = page.get_by_role("option")
        if staff_options.count() > 1:
            option = staff_options.nth(1)
            option.click()
            page.wait_for_timeout(250)
            assert cards.count() > 0
            staff_count_button.click()
            clear_option_locator = page.get_by_role("option").filter(
                has_text="Clear filters"
            )
            if clear_option_locator.count():
                clear_option_locator.first.click(force=True)
            else:
                page.get_by_role("option").nth(0).click()
            page.wait_for_timeout(250)
            assert cards.count() == initial_count
        else:
            staff_count_button.click()

    search_input.fill("zzzz-no-match-zzzz")
    page.wait_for_timeout(250)
    expect(cards).to_have_count(0)
    expect(
        page.get_by_text("No departments match the current filters.")
    ).to_be_visible()

    search_input.fill("")
    page.wait_for_timeout(250)
    assert cards.count() == initial_count


def test_departments_pagination_persists_filters(page: Page, base_url: str) -> None:
    """Verify pagination works with filters applied."""
    created_department_ids: list[str] = []
    try:
        data = fetch_departments_list(
            page.context.request,
            profile_id=ADMIN_PROFILE_ID,
        )
        departments = data.get("departments", [])
        if len(departments) <= 12:
            needed = 13 - len(departments)
            for _ in range(needed):
                department_id = create_department_api(
                    page.context.request,
                    title=generate_unique_department_name("Pagination Department"),
                    description="Department created for pagination test",
                    profile_id=ADMIN_PROFILE_ID,
                    effective_profile_id=ADMIN_PROFILE_ID,
                )
                created_department_ids.append(department_id)
            page.goto(f"{base_url}/management/departments")
            page.wait_for_load_state("networkidle")

        page.goto(f"{base_url}/management/departments")
        page.wait_for_load_state("networkidle")

        next_button = page.get_by_role("button", name="Go to next page")
        if next_button.is_disabled():
            pytest.skip("Pagination controls unavailable after seeding departments")
        next_button.click()
        page.wait_for_timeout(250)

        page.reload()
        page.wait_for_load_state("networkidle")
        pagination_label = page.get_by_text("Page 2 of")
        expect(pagination_label).to_be_visible()

        prev_button = page.get_by_role("button", name="Go to previous page")
        prev_button.click()
        page.wait_for_timeout(250)
        expect(page.get_by_text("Page 1 of")).to_be_visible()
    finally:
        for department_id in created_department_ids:
            try:
                delete_department_api(
                    page.context.request,
                    department_id,
                    profile_id=ADMIN_PROFILE_ID,
                    effective_profile_id=ADMIN_PROFILE_ID,
                )
            except Exception:
                pass
