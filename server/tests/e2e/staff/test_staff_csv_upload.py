...
def test_staff_csv_upload_workflow(page: Page, base_url: str) -> None:
    ...
    try:
        ...
        if first_name_mapping.count() > 0:
            # Verify mapping is correct (should show "First Name" as destination)
            expect(first_name_mapping).to_be_visible()
        ...
        first_name_input = first_review_row.locator("input").first
...
def test_staff_csv_upload_with_validation_errors(page: Page, base_url: str) -> None:
    ...
    try:
        ...
        first_name_cell = first_row.locator("td").nth(1)  # First Name column
        ...
        if second_row.count() > 0:
            last_name_input = second_row.locator("input").nth(1)  # Last Name input
