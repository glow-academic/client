...
from server.tests.e2e.rubrics.helpers import (delete_rubric_api,
                                              fetch_rubric_detail,
...
def test_rubric_create_with_standard_groups(page: Page, base_url: str) -> None:
    ...
    try:
        ...
        create_category_text = page.get_by_text("Create a new evaluation category", exact=False)
        ...
        # Add 5 standards
        add_standard_button = page.get_by_role("button", name=re.compile("Add Standard", re.I))
        standards_data = [
            {"points": "5", "name": "Excellent Performance", "description": "Demonstrates exceptional understanding and application of concepts"},
            {"points": "4", "name": "Good Performance", "description": "Shows solid understanding with minor areas for improvement"},
            {"points": "3", "name": "Satisfactory Performance", "description": "Meets basic requirements and demonstrates adequate understanding"},
            {"points": "2", "name": "Needs Improvement", "description": "Shows some understanding but requires significant development"},
            {"points": "1", "name": "Unsatisfactory Performance", "description": "Does not meet minimum requirements and needs substantial support"},
        ]
        ...
        for i, standard_data in enumerate(standards_data):
            if add_standard_button.count():
                ...
                if standard_rows.count() > 0:
                    ...
                    row_points_input = new_standard_row.locator("input[type='number']").first
                    ...
                    if row_cells.count() >= 2:
                        ...
                        row_name_input = name_cell.locator("input[placeholder='Standard name']").first
        ...
        if table.count():
            ...
            if not create_button or not create_button.count():
                create_buttons = table_parent.get_by_role("button", name="Create", exact=True)
        ...
        if group_card.count():
            ...
            if card_header_area.count():
                ...
                if header_buttons.count() >= 1:
                    ...
                    cancel_button = page.get_by_role("button", name=re.compile("Cancel", re.I))
        ...
        assert standard_groups_detail is not None, "standard_groups_detail should not be None"
        assert standard_groups_mapping is not None, "standard_groups_mapping should not be None"
        ...
        if len(standard_group_ids) > 0:
            ...
            for group_id in standard_group_ids:
                assert group_id in standard_groups_detail, f"Group {group_id} should be in standard_groups_detail"
                group_detail = standard_groups_detail[group_id]
                assert "points" in group_detail, "Group detail should have points"
                assert "passPoints" in group_detail, "Group detail should have passPoints"
                assert "standard_ids" in group_detail, "Group detail should have standard_ids"
                ...
                if len(standard_ids) > 0:
                    assert len(standard_ids) == 5, f"Group should have 5 standards, got {len(standard_ids)}"

        # Verify the rubric name was updated
        assert detail.get("name") == f"{rubric_name} - Updated", "Rubric name should be updated"
