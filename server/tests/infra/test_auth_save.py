"""Tests for auth_save.save_auth_client — composable save with mocked tools.
...
from app.infra.auth_save import (
    resolve_auth_values,
    save_auth_client,
)
...
def _create_result(auth_id=None):
    """Fake CreateAuthResponse / UpdateAuthResponse."""
    r = MagicMock()
    r.id = auth_id or uuid4()
    return r
...
# ==============================================================================
# resolve_auth_values — unit tests
# ==============================================================================


class TestResolveAuthValues:
    async def test_passes_through_ids(self):
        """When IDs are already provided, no create/search calls happen."""
        item = SaveAuthItem(
            name_id=uuid4(),
        )
        errors = await resolve_auth_values(None, None, item, is_update=False)
        assert errors == []
    ...
    async def test_no_required_validation_on_update(self):
        ...
        errors = await resolve_auth_values(None, None, item, is_update=True)

        assert errors == []


# ==============================================================================
# save_auth_client — mocked end-to-end tests
# ==============================================================================


class TestSaveAuthClientCreate:
    async def test_create_success(self):
        ...
        with (
            ...
            patch(
                f"{MODULE}.create_auth_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(auth_id),
            ),
            patch(
            ...
            patch(f"{MODULE}.perform_keycloak_sync", new_callable=AsyncMock),
        ):
            result = await save_auth_client(
                conn,
                redis,
    ...
    async def test_create_permission_denied(self):
        ...
        with (
            ...
            await save_auth_client(conn, redis, profile_id=uuid4(), items=[item])

        assert exc_info.value.status_code == 403


class TestSaveAuthClientUpdate:
    async def test_update_success(self):
        """Full update flow with existing auth."""
        profile_id = uuid4()
        auth_id = uuid4()
        name_id = uuid4()
        snapshot_id = uuid4()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_auth_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(),
            ),
            patch(
                f"{MODULE}.update_auth_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(auth_id),
            ),
            patch(
        ...
        assert result.results[0].success is True
        assert result.results[0].auth_id == auth_id
        assert result.results[0].message == "Auth updated successfully"

    async def test_update_auth_not_found(self):
        """Update with non-existent auth -> 404."""
        item = SaveAuthItem(input_auth_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
...
class TestSaveAuthClientValidation:
    async def test_validation_errors_returned_without_mutation(self):
        ...
        item = SaveAuthItem()  # Missing required fields
    ...
    async def test_profile_not_found(self):
        ...
        with (
            ...
            await save_auth_client(conn, redis, profile_id=uuid4(), items=[])
