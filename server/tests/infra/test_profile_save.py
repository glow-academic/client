"""Integration tests for profile_save — real pool, real DB, no mocks.
...
from app.infra.profile_save import resolve_profile_values, save_profile_client
...
# ---------------------------------------------------------------------------
# resolve_profile_values  (takes conn or pool)
# ---------------------------------------------------------------------------
...
class TestResolveValues:
    async def test_passes_through_ids(self, pool, redis_client):
        ...
        async with pool.acquire() as conn:
            ...
            name = await create_name(conn, "Existing Profile", redis_client)
...
# ---------------------------------------------------------------------------
# save_profile_client -- create
# ---------------------------------------------------------------------------


class TestSaveProfileClientCreate:
    async def test_create_success(self, pool, redis_client, name_id):
        """Superadmin can create a profile with pre-resolved name."""
        result = await save_profile_client(
            pool,
            redis_client,
            profile_id=SUPERADMIN_PROFILE_ID,
            items=[SaveProfileItem(name_id=name_id)],
        )

        assert len(result.results) == 1
        assert result.results[0].success is True
        assert result.results[0].profile_id is not None

    async def test_create_with_raw_name(self, pool, redis_client):
        """Superadmin can create a profile with raw name (auto-resolved)."""
        result = await save_profile_client(
            pool,
            redis_client,
            profile_id=SUPERADMIN_PROFILE_ID,
            items=[SaveProfileItem(name="My New Profile")],
        )
    ...
    async def test_create_permission_denied(self, pool, redis_client, name_id):
        """Member cannot create profiles."""
        with pytest.raises(HTTPException) as exc_info:
            await save_profile_client(
                pool,
                redis_client,
...
# ---------------------------------------------------------------------------
# save_profile_client -- update
# ---------------------------------------------------------------------------


class TestSaveProfileClientUpdate:
    async def _create_profile(self, pool, redis_client) -> UUID:
        """Helper: create a profile to update later."""
        result = await save_profile_client(
            pool,
            redis_client,
            profile_id=SUPERADMIN_PROFILE_ID,
            items=[SaveProfileItem(name="Profile To Update")],
        )
        return result.results[0].profile_id

    async def test_update_success(self, pool, redis_client):
        """Update an existing profile's name."""
        target_profile_id = await self._create_profile(pool, redis_client)
    ...
    async def test_update_profile_not_found(self, pool, redis_client):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await save_profile_client(
                ...
                redis_client,
                profile_id=SUPERADMIN_PROFILE_ID,
                items=[SaveProfileItem(input_profile_id=fake_id)],
            )
...
# ---------------------------------------------------------------------------
# save_profile_client -- validation
# ---------------------------------------------------------------------------
