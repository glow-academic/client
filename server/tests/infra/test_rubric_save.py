"""Tests for rubric_save.save_rubric_client — composable save with mocked tools.
...
from app.infra.rubric_save import (
...
def _perms(*, exists=True, department_ids=None, active_simulation_count=0):
    """Fake RubricPermissionsContext."""
    p = MagicMock()
    p.exists = exists
    p.department_ids = department_ids or []
    p.active_simulation_count = active_simulation_count
    return p


def _create_result(rubric_id=None):
    """Fake CreateRubricResponse / UpdateRubricResponse."""
    r = MagicMock()
    r.id = rubric_id or uuid4()
    return r
...
# ======================================================================
# resolve_rubric_values -- unit tests
# ======================================================================
...
class TestResolveValues:
    async def test_passes_through_ids(self):
        ...
        errors = await resolve_rubric_values(None, None, item, is_update=False)
        assert errors == []
    ...
    async def test_no_required_validation_on_update(self):
        ...
        errors = await resolve_rubric_values(None, None, item, is_update=True)

        assert errors == []


# ======================================================================
# save_rubric_client -- mocked end-to-end tests
# ======================================================================
...
class TestSaveRubricClientCreate:
    async def test_create_success(self):
        ...
        with (
            ...
            patch(
                f"{MODULE}.create_rubric_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(rubric_id),
            ),
            patch(
    ...
    async def test_create_permission_denied(self):
        """Non-superadmin cannot create rubric."""
        item = SaveRubricItem(name_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            await save_rubric_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveRubricClientUpdate:
    async def test_update_success(self):
        """Full update flow with existing rubric."""
        profile_id = uuid4()
        rubric_id = uuid4()
        name_id = uuid4()
        snapshot_id = uuid4()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_rubric_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(),
            ),
            patch(
                f"{MODULE}.update_rubric_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(rubric_id),
            ),
            patch(
    ...
    async def test_update_rubric_not_found(self):
        """Update with non-existent rubric -> 404."""
        item = SaveRubricItem(input_rubric_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
    ...
    async def test_update_permission_denied(self):
        """Cannot edit when active simulations exist."""
        item = SaveRubricItem(input_rubric_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_rubric_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(active_simulation_count=1),
            ),
            pytest.raises(HTTPException) as exc_info,
        ):
            await save_rubric_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveRubricClientValidation:
    async def test_validation_errors_returned_without_mutation(self):
        ...
        item = SaveRubricItem()  # Missing required fields
    ...
    async def test_profile_not_found(self):
        ...
        with (
            ...
            await save_rubric_client(conn, redis, profile_id=uuid4(), items=[])
