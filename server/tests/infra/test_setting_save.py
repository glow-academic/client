"""Tests for setting_save.save_setting_client — composable save with mocked tools.
...
from app.infra.setting_save import (
...
def _perms(*, exists=True, department_ids=None):
    """Fake SettingPermissionsContext."""
    p = MagicMock()
    p.exists = exists
    p.department_ids = department_ids or []
    return p
...
def _create_result(setting_id=None):
    """Fake CreateSettingResponse / UpdateSettingResponse."""
    r = MagicMock()
    r.id = setting_id or uuid4()
    return r
...
# ==============================================================================
# resolve_setting_values — unit tests
# ==============================================================================


class TestResolveSettingValues:
    async def test_passes_through_ids(self):
        """When IDs are already provided, no create/search calls happen."""
        item = SaveSettingItem(
            name_id=uuid4(),
        )
        errors = await resolve_setting_values(None, None, item, is_update=False)
        assert errors == []
    ...
    async def test_no_required_validation_on_update(self):
        ...
        errors = await resolve_setting_values(None, None, item, is_update=True)

        assert errors == []


# ==============================================================================
# save_setting_client — mocked end-to-end tests
# ==============================================================================
...
class TestSaveSettingClientCreate:
    async def test_create_success(self):
        ...
        with (
            ...
            patch(
                f"{MODULE}.create_setting_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(setting_id),
            ),
            patch(
    ...
    async def test_create_permission_denied(self):
        ...
        item = SaveSettingItem(name_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            await save_setting_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveSettingClientUpdate:
    async def test_update_success(self):
        """Full update flow with existing setting."""
        profile_id = uuid4()
        setting_id = uuid4()
        name_id = uuid4()
        snapshot_id = uuid4()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_setting_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(),
            ),
            patch(
                f"{MODULE}.update_setting_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(setting_id),
            ),
            patch(
    ...
    async def test_update_setting_not_found(self):
        ...
        item = SaveSettingItem(input_setting_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
    ...
    async def test_update_permission_denied_wrong_departments(self):
        """User without matching departments cannot edit."""
        dept_id = uuid4()
        item = SaveSettingItem(input_setting_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            patch(
                f"{MODULE}.resolve_profile_identity_context",
                new_callable=AsyncMock,
                return_value=_profile(role="admin", department_ids=[]),
            ),
            patch(
                f"{MODULE}.resolve_setting_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(department_ids=[dept_id]),
            ),
            pytest.raises(HTTPException) as exc_info,
        ):
            await save_setting_client(conn, redis, profile_id=uuid4(), items=[item])
    ...
    async def test_update_superadmin_bypasses_department_check(self):
        ...
        with (
            patch(
                f"{MODULE}.resolve_profile_identity_context",
                new_callable=AsyncMock,
                return_value=_profile(role="superadmin", department_ids=[]),
            ),
            patch(
                f"{MODULE}.resolve_setting_permissions_context",
                new_callable=AsyncMock,
...
class TestSaveSettingClientValidation:
    async def test_validation_errors_returned_without_mutation(self):
        ...
        item = SaveSettingItem()  # Missing required fields
    ...
    async def test_profile_not_found(self):
        ...
        with (
            ...
            await save_setting_client(conn, redis, profile_id=uuid4(), items=[])
