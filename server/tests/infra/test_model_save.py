"""Tests for model_save.save_model_client — composable save with mocked tools.
...
from app.infra.model_save import (
...
def _perms(*, exists=True, department_ids=None, active_agent_count=0):
    """Fake ModelPermissionsContext."""
    p = MagicMock()
    p.exists = exists
    p.department_ids = department_ids or [uuid4()]
...
def _create_result(artifact_id=None):
    """Fake CreateModelResponse / UpdateModelResponse."""
    r = MagicMock()
...
# ===== resolve_model_values — unit tests =====
...
class TestResolveValues:
    async def test_passes_through_ids(self):
        ...
        errors = await resolve_model_values(None, None, item, is_update=False)
        assert errors == []

    async def test_creates_name_from_value(self):
        """Raw name value -> create_name -> sets name_id."""
        name_id = uuid4()
        item = SaveModelItem(name="Test Model")
    ...
    async def test_no_required_validation_on_update(self):
        ...
        errors = await resolve_model_values(None, None, item, is_update=True)

        assert errors == []


# ===== save_model_client — mocked end-to-end tests =====
...
class TestSaveModelClientCreate:
    async def test_create_success(self):
        ...
        item = SaveModelItem(name_id=name_id)
        ...
        with (
            ...
            patch(
                f"{MODULE}.create_model_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(model_id),
            ),
            patch(
    ...
    async def test_create_permission_denied(self):
        ...
        item = SaveModelItem(name_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            await save_model_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveModelClientUpdate:
    async def test_update_success(self):
        """Full update flow with existing model."""
        profile_id = uuid4()
        model_id = uuid4()
        name_id = uuid4()
        snapshot_id = uuid4()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_model_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(),
            ),
            patch(
                f"{MODULE}.update_model_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(model_id),
            ),
            patch(
    ...
    async def test_update_model_not_found(self):
        ...
        item = SaveModelItem(input_model_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
    ...
    async def test_update_permission_denied_active_agents(self):
        """Cannot edit when active agents exist and not superadmin."""
        item = SaveModelItem(input_model_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
...
class TestSaveModelClientValidation:
    async def test_validation_errors_returned_without_mutation(self):
        ...
        item = SaveModelItem()  # Missing required fields
    ...
    async def test_profile_not_found(self):
        ...
        with (
            ...
            await save_model_client(conn, redis, profile_id=uuid4(), items=[])
