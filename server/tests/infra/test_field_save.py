"""Tests for field_save.save_field_client — composable save with mocked tools.
...
from app.infra.field_save import (
...
def _create_result(entity_id=None):
    """Fake CreateFieldResponse / UpdateFieldResponse."""
    r = MagicMock()
...
# === resolve_field_values — unit tests ===
...
class TestResolveValues:
    async def test_passes_through_ids(self):
        ...
        errors = await resolve_field_values(None, None, item, is_update=False)
        assert errors == []
    ...
    async def test_no_required_validation_on_update(self):
        ...
        errors = await resolve_field_values(None, None, item, is_update=True)
...
# === save_field_client — mocked end-to-end tests ===
...
class TestSaveFieldClientCreate:
    async def test_create_success(self):
        ...
        with (
            ...
            patch(
                f"{MODULE}.create_field_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(field_id),
            ),
            patch(
    ...
    async def test_create_permission_denied(self):
        ...
        item = SaveFieldItem(name_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            await save_field_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveFieldClientUpdate:
    async def test_update_success(self):
        """Full update flow with existing field."""
        profile_id = uuid4()
        field_id = uuid4()
        name_id = uuid4()
        snapshot_id = uuid4()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_field_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(),
            ),
            patch(
                f"{MODULE}.update_field_artifact",
                new_callable=AsyncMock,
                return_value=_create_result(field_id),
            ),
            patch(
    ...
    async def test_update_field_not_found(self):
        ...
        item = SaveFieldItem(input_field_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
    ...
    async def test_update_permission_denied_default_field(self):
        """Cannot edit default field (no departments) as non-superadmin."""
        item = SaveFieldItem(input_field_id=uuid4())
        conn = AsyncMock()
        redis = AsyncMock()
        ...
        with (
            ...
            patch(
                f"{MODULE}.resolve_field_permissions_context",
                new_callable=AsyncMock,
                return_value=_perms(department_ids=[]),
            ),
            pytest.raises(HTTPException) as exc_info,
        ):
            await save_field_client(conn, redis, profile_id=uuid4(), items=[item])
...
class TestSaveFieldClientValidation:
    async def test_validation_errors_returned_without_mutation(self):
        ...
        item = SaveFieldItem()  # Missing required fields
    ...
    async def test_profile_not_found(self):
        ...
        with (
            ...
            await save_field_client(conn, redis, profile_id=uuid4(), items=[])
