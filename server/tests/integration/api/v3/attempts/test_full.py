...
async def test_get_attempt_full_previous_chats_with_child_scenarios(
    ...
    child_scenario_id = await _create_child_scenario(db, scenario_ids[0], "Child Variant")
    chat2_id = await _create_test_chat(db, child_scenario_id, attempt2_id, completed=True)
    ...
    scenario1_data = next((s for s in all_scenarios if s["id"] == scenario_ids[0]), None)
