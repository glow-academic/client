"""Integration tests for resource flags get endpoint."""
...
class TestGetFlags:
    """Tests for GET /api/v4/resources/flags/get endpoint."""

    async def test_get_flags_returns_items(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_flags_empty_ids(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_flags_nonexistent_id(self, client: httpx.AsyncClient) -> None:
...
class TestGetFlagsInternal:
    """Tests for get_flags_internal function."""
    ...
    async def test_returns_items_for_valid_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Arrange
        search_items = await search_flags_internal(db, bypass_cache=True)
        assert len(search_items) > 0
        seed_ids = [item.id for item in search_items[:2]]

        # Act
        items = await get_flags_internal(db, seed_ids, bypass_cache=True)
    ...
    async def test_returns_empty_for_empty_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await get_flags_internal(db, [], bypass_cache=True)
