...
class TestGetPrompts:
    """Tests for GET /api/v4/resources/prompts/get endpoint."""

    async def test_get_prompts_returns_items(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_get_prompts_empty_ids(self, client: httpx.AsyncClient) -> None:
...
class TestGetPromptsInternal:
    """Tests for get_prompts_internal function."""
    ...
    async def test_returns_items_for_valid_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Arrange
        search_items = await search_prompts_internal(db, bypass_cache=True)
        assert len(search_items) > 0
        seed_ids = [item.id for item in search_items[:2]]

        # Act
        items = await get_prompts_internal(db, seed_ids, bypass_cache=True)
    ...
    async def test_returns_empty_for_empty_ids(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await get_prompts_internal(db, [], bypass_cache=True)
