...
class TestSearchPrompts:
    """Tests for POST /api/v4/resources/prompts/search endpoint."""

    async def test_search_prompts_returns_items(
    ...
    async def test_search_prompts_with_limit(self, client: httpx.AsyncClient) -> None:
    ...
    async def test_search_prompts_with_search_text(
        self, client: httpx.AsyncClient
    ) -> None:
...
class TestSearchPromptsInternal:
    """Tests for search_prompts_internal function."""

    async def test_returns_items(self, db: asyncpg.Connection) -> None:
        """Internal function returns items with no filters."""
        # Act
        items = await search_prompts_internal(db, bypass_cache=True)
    ...
    async def test_respects_limit(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await search_prompts_internal(db, limit_count=1, bypass_cache=True)
    ...
    async def test_returns_empty_for_zero_limit(self, db: asyncpg.Connection) -> None:
        ...
        # Act
        items = await search_prompts_internal(db, limit_count=0, bypass_cache=True)
