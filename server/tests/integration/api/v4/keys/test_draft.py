"""Route tests for PATCH /api/v4/keys/draft endpoint."""
...
async def test_patch_key_draft_create(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test creating a new key draft."""
...
async def test_patch_key_draft_update(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test updating an existing key draft."""
    await get_superadmin_alias(db)
    ...
    create_response = await client.patch(
        "/api/v4/keys/draft",
        json={
            "patch": {"name": "Original Draft", "type": "api"},
            "expected_version": 0,
            "input_draft_id": None,
        },
    )
...
async def test_patch_key_draft_version_mismatch(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
