"""Route tests for PATCH /api/v4/auth/draft endpoint."""
...
async def test_patch_auth_draft_create(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test creating a new auth draft."""
    ...
    response = await client.patch(
        "/api/v4/auth/draft",
        json={
            "patch": {"name": "Draft Auth", "description": "Draft description"},
            "expected_version": 0,
            "input_draft_id": None,
        },
    )
...
async def test_patch_auth_draft_update(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test updating an existing auth draft."""
    await get_superadmin_alias(db)
