"""Route tests for PATCH /api/v4/artifacts/agents/draft endpoint."""
...
async def test_patch_agent_draft_create(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test creating a new agent draft."""
    ...
    response = await client.patch(
        ...
        json={
            "patch": {"name": "Draft Agent", "description": "Draft description"},
            "expected_version": 0,
            "input_draft_id": None,
        },
    )
...
async def test_patch_agent_draft_update(
    client: httpx.AsyncClient, db: asyncpg.Connection, disable_cache: None
) -> None:
    """Test updating an existing agent draft."""
    await get_superadmin_alias(db)
