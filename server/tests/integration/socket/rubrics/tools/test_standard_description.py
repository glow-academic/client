...
from app.socket.v3.tools.standard_description.call import (
...
async def test_rubric_tool_standard_description_success(
    ...
    events = mock_sio.get_events("rubrics_tools_standard_description_complete")
    assert len(events) == 1
...
async def test_rubric_tool_standard_description_internal_event(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test rubric_tool_standard_description via internal event."""
    # Arrange
    rubric_id = await db.fetchval(
        "INSERT INTO rubrics(name, description, points, pass_points, active) VALUES ('Test Rubric', 'Test', 100, 70, true) RETURNING id"
    )
...
async def test_rubric_tool_standard_description_missing_trace_id(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test rubric_tool_standard_description with missing trace_id."""
    # Arrange
    rubric_id = await db.fetchval(
        "INSERT INTO rubrics(name, description, points, pass_points, active) VALUES ('Test Rubric', 'Test', 100, 70, true) RETURNING id"
    )
