"""Integration tests for hint_generate WebSocket event."""
...
from app.socket.v4.agents.hint.generate import hint_generate

pytestmark = pytest.mark.asyncio


async def test_hint_generate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful hint_generate event."""
    # Arrange
    ...
    # Act
    await hint_generate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("hints_generate_error")
