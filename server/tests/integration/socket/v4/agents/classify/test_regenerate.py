"""Integration tests for classify_regenerate WebSocket event."""
...
from app.socket.v4.agents.classify.regenerate import classify_regenerate

pytestmark = pytest.mark.asyncio


async def test_classify_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful classify_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Classify Agent")

    data = {
        "department_id": str(department_id),
        "classify_agent_id": str(agent_id),
    }

    # Act
    await classify_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("classifys_regenerate_error")
