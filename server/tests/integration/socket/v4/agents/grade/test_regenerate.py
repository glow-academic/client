"""Integration tests for grade_regenerate WebSocket event."""
...
from app.socket.v4.agents.grade.regenerate import grade_regenerate

pytestmark = pytest.mark.asyncio


async def test_grade_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful grade_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Grade Agent")

    data = {
        "department_id": str(department_id),
        "grade_agent_id": str(agent_id),
    }

    # Act
    await grade_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("grades_regenerate_error")
