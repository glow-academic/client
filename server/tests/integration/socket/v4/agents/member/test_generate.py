"""Integration tests for member_generate WebSocket event."""
...
from app.socket.v4.agents.member.generate import member_generate

pytestmark = pytest.mark.asyncio


async def test_member_generate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful member_generate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Member Agent")

    data = {
        "department_id": str(department_id),
        "member_agent_id": str(agent_id),
    }

    # Act
    await member_generate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("members_generate_error")
