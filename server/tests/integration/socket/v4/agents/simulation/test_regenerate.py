"""Integration tests for simulation_regenerate WebSocket event."""
...
from app.socket.v4.agents.simulation.regenerate import simulation_regenerate

pytestmark = pytest.mark.asyncio


async def test_simulation_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful simulation_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Simulation Agent")

    data = {
        "department_id": str(department_id),
        "simulation_agent_id": str(agent_id),
    }

    # Act
    await simulation_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("simulations_regenerate_error")
