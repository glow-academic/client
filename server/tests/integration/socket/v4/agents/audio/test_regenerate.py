"""Integration tests for audio_regenerate WebSocket event."""
...
from app.socket.v4.agents.audio.regenerate import audio_regenerate

pytestmark = pytest.mark.asyncio


async def test_audio_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful audio_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Audio Agent")

    data = {
        "department_id": str(department_id),
        "audio_agent_id": str(agent_id),
    }

    # Act
    await audio_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("audios_regenerate_error")
