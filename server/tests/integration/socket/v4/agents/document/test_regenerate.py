"""Integration tests for document_regenerate WebSocket event."""
...
from app.socket.v4.agents.document.regenerate import document_regenerate

pytestmark = pytest.mark.asyncio


async def test_document_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful document_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Document Agent")

    data = {
        "department_id": str(department_id),
        "document_agent_id": str(agent_id),
    }

    # Act
    await document_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("documents_regenerate_error")
