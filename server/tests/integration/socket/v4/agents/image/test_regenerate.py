"""Integration tests for image_regenerate WebSocket event."""
...
from app.socket.v4.agents.image.regenerate import image_regenerate

pytestmark = pytest.mark.asyncio


async def test_image_regenerate_success(
    db: asyncpg.Connection, mock_sio: MockSocketIO, mock_internal_sio: MockInternalBus
) -> None:
    """Test successful image_regenerate event."""
    # Arrange
    ...
    agent_id = await get_or_create_test_agent(db, name="Image Agent")

    data = {
        "department_id": str(department_id),
        "image_agent_id": str(agent_id),
    }

    # Act
    await image_regenerate(sid, data)

    # Assert - verify handler completes
    # AI generation uses mocked Runner
    error_events = mock_sio.get_events("images_regenerate_error")
