...
-- Parameters: $1=agent_id (uuid), $2=department_id (uuid)
-- Returns: agent, model, provider, and profile data for audio grading agent
WITH profile_from_department AS (
    -- Get a profile from the department (for rate limiting and settings)
    SELECT pd.profile_id
    FROM profile_departments pd
    WHERE pd.department_id = $2::uuid
      AND pd.active = true
    LIMIT 1
),
