-- Get all data needed to run classification agent for uploads
-- Parameters: $1=department_id (uuid, nullable), $2=profile_id (uuid)
...
SELECT 
    ...
    COALESCE(
        (SELECT profile_id::text FROM params),
        dg.guest_profile_id
    ) as profile_id,
