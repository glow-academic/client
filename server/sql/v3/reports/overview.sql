    ...
    WHERE p.active = true
      AND (
          ...
          OR EXISTS (
              ...
              WHERE pf.parameter_id = p.id AND fd.department_id = ANY($7::uuid[])
          )
          OR NOT EXISTS (
              SELECT 1 
              FROM fields f2
              JOIN parameter_fields pf2 ON pf2.field_id = f2.id AND pf2.active = true
              JOIN field_departments fd2 ON fd2.field_id = f2.id AND fd2.active = true
              WHERE pf2.parameter_id = p.id
          )
      )
),
    ...
    'secondary', json_build_object(
        ...
        'cohortPerformance', json_build_object(
            'cohortData', COALESCE((SELECT json_agg(json_build_object(
                'id', cohort_id::text,
                'name', cohort_name,
                -- Profile_id is always present for reports - always use attempt-based pass rate
                'passRate', ROUND(
                    CASE 
