...
-- Parameters: $1=parameter_ids[] (uuid array, optional - if empty/null, get all document/policy parameters), $2=profile_id (uuid)
...
parameter_items_data AS (
    ...
    GROUP BY pi.id, pi.name, pi.description, pi.value, pi.parameter_id, p.id, p.name, p.document_parameter, p.policy_parameter
)
SELECT 
    id::text,
    name,
