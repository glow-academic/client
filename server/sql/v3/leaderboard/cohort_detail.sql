-- Leaderboard cohort detail query - assumes single cohortId
-- Parameters (in order):
-- $1, $2: start_date (datetime), end_date (datetime)
-- $3: cohort_id (uuid, required, non-null)
-- $4: roles (profile_role[])
-- $5: sim_filters (text[], optional) - ["general", "practice", "archived"]
...
filt AS (
    ...
    WHERE a.attempt_created_at >= $1 
      ...
      -- Role filtering
      AND ($4::profile_role[] IS NULL OR cardinality($4::profile_role[]) = 0 OR a.profile_role = ANY($4::profile_role[]))
      -- Filter by simulations linked to the specific cohort
      AND a.simulation_id IN (
        SELECT DISTINCT s.id
        FROM simulations s
        WHERE s.active = TRUE
          AND (
            -- Simulations linked to the specific cohort
            EXISTS (
              SELECT 1 
              FROM cohort_simulations cs 
              WHERE cs.simulation_id = s.id 
                AND cs.cohort_id = $3::uuid
                AND cs.active = TRUE
            )
            OR
            -- Practice simulations without cohorts (always included)
...
profile_stats AS (
    ...
    -- Department filtering at profile level
    WHERE (
        ...
        OR EXISTS (
            ...
            WHERE pd.profile_id = p.id 
              AND pd.department_id = ANY($6::uuid[])
              AND pd.active = TRUE
        )
        OR NOT EXISTS (
            SELECT 1 
