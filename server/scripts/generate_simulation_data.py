...
from app.models import Rubrics, Simulations
...
def fetch_source_data(session: Session) -> dict:
    ...
    if not simulations:
        raise ValueError(
            "No simulations with linked scenarios found. Please seed the database first."
        )
    ...
    print(
        f"  > Using {len(ta_user_ids)} hardcoded TA profile IDs (guaranteed to exist in fresh database)"
    )
    ...
    print(
        f"  > Loaded default rubric '{default_rubric.name}' with {len(standards_by_rubric)} standard groups."
    )
    ...
    try:
        ...
        for sim in simulations:
            for _ in range(chats_per_sim):
                ...
                attempt_rows.append(
                    f"({q(attempt_id)}, {q(created_ts)}, {q(ta_id)}, {q(sim.id)})"
                )
                ...
                for i in range(2):
                    q_time = chat_start_time + datetime.timedelta(
                        seconds=i * 20 + random.randint(1, 5)
                    )
                    message_rows.append(
                ...
                grade_rows.append(
                    f"({q(grade_id)}, {q(created_ts)}, {str(score >= 17).lower()}, {score}, {random.randint(180, 600)}, {q(rubric_id)}, {q(chat_id)})"
                )
                for std_id, pts in feedback_data:
                    fb_rows.append(
                        f"({q(uuid.uuid4())}, {q(created_ts)}, {q(std_id)}, {q(grade_id)}, {pts}, {q(f'Performance rated {pts}/5.')})"
                    )
