...
def main() -> None:
    ...
    if utils_dir.exists():
        ...
        for py_file in utils_dir.rglob("*.py"):
            ...
            rel_path = py_file.relative_to(utils_dir)
            test_dir = tests_dir / "utils" / rel_path.parent
            # Make the test filename unique to avoid import conflicts
            test_filename = (
                f"test_{rel_path.parent.name.lower()}_{rel_path.stem}.py"
                if rel_path.parent.name
                else f"test_{rel_path.stem}_utils.py"  # Add _utils suffix for utils directory
            )
            test_file = test_dir / test_filename
            expected_test_files.add(test_file.resolve())
            ...
            if not test_file.exists():
                module_path = str(rel_path.with_suffix("")).replace("/", ".")
                test_content = generate_service_test(
                    f"utils.{module_path}", source_functions
                )
                test_file.write_text(test_content)
                stats["created"] += 1
                print(f"    ✨ Created new test file: {test_file}")
