...
def write_registry_file(
    ...
    for _, sql_path, module_path, sql_params_class, sql_row_class, api_request_class, api_response_class in sorted(registry_entries, key=lambda x: x[1]):
        ...
        lines.append(f'        file_path: Literal["{sql_path}"]')
        lines.append(f"    ) -> tuple[Type[{sql_params_class}], Type[{sql_row_class}]]: ...")
        lines.append("")
    ...
    lines.append('    """Load typed input/output classes for a SQL file.')
    lines.append("")
    lines.append("    Returns the generated Pydantic model classes for input parameters and output rows.")
    lines.append("    Types are loaded from the registry generated by sql-compile.")
    lines.append("")
    ...
    lines.append('        ```python')
    if registry_type == "app":
        lines.append('        InputType, OutputType = load_sql_typed("app/sql/v3/agents/get_agent_new_complete.sql")')
        lines.append('        sql_query = load_sql_query("app/sql/v3/agents/get_agent_new_complete.sql")')
    else:
        lines.append('        InputType, OutputType = load_sql_typed("tests/sql/integration/infra/activity/insert_test_profile.sql")')
        lines.append('        sql_query = load_sql_query("tests/sql/integration/infra/activity/insert_test_profile.sql")')
    lines.append("        ")
    ...
    lines.append("        result = await conn.fetchrow(sql_query, *params.to_tuple())")
